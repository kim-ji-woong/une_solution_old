...
public class ExcelSearchSnowToday extends AbstractExcelView {
	public ExcelSearchSnowToday()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/일일조회_적설");
	}
	...
	{		
		...
		String fileName = cityName.length() == 0 ? "적설조회(금일).xls" : cityName + "_적설조회(금일).xls";
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		List<SearchSnowToday> snowList = (List<SearchSnowToday>)model.get("totalSearchSnow");
		
		int nRowIndex = 6;
		int nSnowCount = snowList.size();
		
		ChangeTableSize(wb, sheet, nSnowCount, nRowIndex - 1);
		
		for (int i=0;i<nSnowCount;i++)
		{
			SearchSnowToday snow = snowList.get(i);
			int col = 0;
