...
public class ExcelPrintReportSnowData extends AbstractExcelView {
	private String reportTime = "";
	private String period = "";
	
	public ExcelPrintReportSnowData()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/적설보고서");
	}
			...
			HttpServletResponse response) throws Exception 
	{
		...
		period = param;
		String fileTag = "적설량보고서(" + param + ").xls";
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		/// 측기별 내역 Header
		String snowHeader = "누적적설(cm)\r\n(" + period + ")";
		setText(getCell(sheet, 5, 7), snowHeader);
		...
		setText(getCell(sheet, 6, 7), yesterdayStr);
		setText(getCell(sheet, 6, 8), todayStr);
		////////////////////////
		...
		for (int i=0;i<nReportDataCount;i++)
		{
			...
			setText(getCell(sheet, nRowIndex, 10), items.get(5));
			
			yesterdayAvg = yesterdayAvg + Double.parseDouble(items.get(2));
			todayAvg = todayAvg + Double.parseDouble(items.get(3));
			sumAvg = sumAvg + Double.parseDouble(items.get(4));
		...
		setText(getCell(sheet, nRowIndex, 7), SearchRain.DoubleToString(yesterdayAvg)); 
		setText(getCell(sheet, nRowIndex, 8), SearchRain.DoubleToString(todayAvg));
		setText(getCell(sheet, nRowIndex, 9), SearchRain.DoubleToString(sumAvg));
		/*setText(getCell(sheet, nRowIndex, 7), dformat.format(yesterdayAvg)); 
		setText(getCell(sheet, nRowIndex, 8), dformat.format(todayAvg));
		setText(getCell(sheet, nRowIndex, 9), dformat.format(sumAvg));*/
		////////////////////////
	...
	private void AddImage(String url, HSSFWorkbook wb, HSSFSheet sheet) throws Exception
	{
		try
		{
	        ...
	        HSSFClientAnchor anchor = new HSSFClientAnchor(0,0,0,255,(short)1,10,(short)4,18); // 이미지 크기조절은 여기서..
