...
public class ExcelSearchSnowMonth extends AbstractExcelView {
	private int daysInMonth = 31;
	private String selectedYear = "";
	private String selectedMonth = "";
	
	public ExcelSearchSnowMonth()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/월간조회");
	}
			...
			HttpServletResponse response) throws Exception 
	{
		...
		//String param = excelParam == null ? "월간" : excelParam;
		String fileTag = "적설조회(" + param + ").xls";
	...
	private String getParams(Map<String, Object> model)
	{
		...
		if (excelParam != null)
		{
			...
			if (arrParams.length > 0)
			{
				...
				if (param.length() == 5)
				{
					selectedYear = param.substring(0, 4);
					selectedMonth = param.substring(4);

					selectedMonth = "0" + selectedMonth;
					param = selectedYear + selectedMonth;
				}
				...
				if (arrParams.length > 1)
				{
					...
					try
					{
						dayCount = Integer.parseInt(arrParams[1]);
					}
					catch (Exception e)
					{
						return param;
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		List<SearchSnow> snowList = (List<SearchSnow>)model.get("totalSearchSnow");
		
		int nRowIndex = 6;
		int nSnowCount = snowList.size();
		
		ChangeTableSize(wb, sheet, nSnowCount, nRowIndex - 2);
		
		for (int i=0;i<nSnowCount;i++)
		{
			SearchSnow snow = snowList.get(i);
			int col = 0;
			...
			for (int j=0;j<nItemCount-1;j++)
			{
				SearchSnow.SnowData data = items.get(j);
				setText(getCell(sheet, nRowIndex, col++), data.getValue());
			}
			
			if (nItemCount > 0)
			{
				SearchSnow.SnowData data = items.get(nItemCount - 1);
				setText(getCell(sheet, nRowIndex, 32), data.getValue());
			}
			
			/*for (SearchSnow.SnowData data : items)
			{
				setText(getCell(sheet, nRowIndex, col++), data.getValue());
			}*/
		...
		setText(getCell(sheet, 1, 0), "적설조회결과");
		setText(getCell(sheet, 2, 0), "검색조건 : 월간");
		
		if (selectedMonth.length() > 0)
			setText(getCell(sheet, 3, 0), "검색기간 : " + selectedYear + "-" + selectedMonth);
		else
			setText(getCell(sheet, 3, 0), "검색기간 : " + selectedYear);
