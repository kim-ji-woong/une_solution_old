...
public class ExcelSearchSnowPeriod extends AbstractExcelView {
	private String locationName = "";
	private String duration = "";
	
	public ExcelSearchSnowPeriod()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/기간선택(1)");
	}
			...
			HttpServletResponse response) throws Exception 
	{
		...
		String fileTag = locationName.length() == 0 ? "적설조회(" + param + ").xls" : locationName + "_적설조회(" + param + ").xls";
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		List<SearchSnowPeriod> snowList = (List<SearchSnowPeriod>)model.get("totalSearchSnow");
		...
		{
			SearchSnowPeriod snow = snowList.get(i);
			int col = 0;
