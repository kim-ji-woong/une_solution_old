...
public class ExcelSearchRainMonth extends AbstractExcelView {
	private int daysInMonth = 31;
	private String selectedYear = "";
	private String selectedMonth = "";
	
	public ExcelSearchRainMonth()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/월간조회");
	}
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		List<SearchRain> rainList = (List<SearchRain>)model.get("totalSearchRain");
		...
		{
			SearchRain rain = rainList.get(i);
			int col = 0;
			...
			{
				SearchRain.RainData data = items.get(j);
				setText(getCell(sheet, nRowIndex, col++), data.getValue());
			}
			
			if (nItemCount > 0)
			{
				SearchRain.RainData data = items.get(nItemCount - 1);
