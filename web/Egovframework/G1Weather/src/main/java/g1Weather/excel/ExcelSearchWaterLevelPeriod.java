...
public class ExcelSearchWaterLevelPeriod extends AbstractExcelView {
	private String locationName = "";
	private String duration = "";
	
	public ExcelSearchWaterLevelPeriod()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/기간선택(2)");
	}
			...
			HttpServletResponse response) throws Exception 
	{
		...
		String fileTag = locationName.length() == 0 ? "수위조회(" + param + ").xls" : locationName + "_수위조회(" + param + ").xls";
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		List<SearchWaterLevelPeriod> waterLevelList = (List<SearchWaterLevelPeriod>)model.get("totalSearchWaterLevel");
		
		int nRowIndex = 6;
		int nWaterLevelCount = waterLevelList.size();
		
		ChangeTableSize(wb, sheet, nWaterLevelCount, nRowIndex - 2);
		...
		{
			SearchWaterLevelPeriod waterLevel = waterLevelList.get(i);
			int col = 0;
		...
		setText(getCell(sheet, 1, 0), "수위조회결과");
