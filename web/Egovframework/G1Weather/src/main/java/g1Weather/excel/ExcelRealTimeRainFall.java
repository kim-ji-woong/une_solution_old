...
public class ExcelRealTimeRainFall extends AbstractExcelView {
	...
	{		
		...
		String fileName = cityName.length() == 0 ? "강우현황.xls" : cityName + "_강우현황.xls";
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		if (nRainCount > 0)
			cityName = rainList.get(0).getCityName();
	...
	private void NewFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		HSSFSheet sheet = wb.getSheetAt(0);
		// 기존 양식이 없을 경우 새로운 Excel Sheet를 만든다.
		//HSSFSheet sheet = wb.createSheet("강우현황");
		...
		String[] headers = new String[] { "", "시군", "지점번호", "지점명", "관측시각", "강우", "15M", "60M", "금일", "전일", "기온", "풍향1M", "풍속1M", "습도", "비고" };
		List<RealTimeRainFall> rainList = (List<RealTimeRainFall>)model.get("totalRRF");
