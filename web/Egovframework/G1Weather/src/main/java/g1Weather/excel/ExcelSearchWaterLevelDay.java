...
public class ExcelSearchWaterLevelDay extends AbstractExcelView {
	private String selectedDate = ""; 
	
	public ExcelSearchWaterLevelDay()
	{
		// 기존 양식을 이용한다.
		setUrl("/data/일일조회_수위");
	}
	...
	private String ReferenceFile(Map<String, Object> model, HSSFWorkbook wb)
	{
		...
		List<SearchWaterLevelDay> waterLevelList = (List<SearchWaterLevelDay>)model.get("totalSearchWaterLevel");
		
		int nRowIndex = 6;
		int nWaterLevelCount = waterLevelList.size();
		
		ChangeTableSize(wb, sheet, nWaterLevelCount, nRowIndex - 1);
		
		for (int i=0;i<nWaterLevelCount;i++)
		{
			SearchWaterLevelDay waterLevel = waterLevelList.get(i);
			int col = 0;
			
			//setText(cell = getCell(sheet, nRowIndex, col++), Integer.toString(i + 1));		
			setText(getCell(sheet, nRowIndex, col++), waterLevel.getLocationName());
			
			List<SearchWaterLevel.WaterLevelData> items = waterLevel.getItemValues();
			
			for (SearchWaterLevel.WaterLevelData data : items)
			{
		...
		setText(getCell(sheet, 2, 0), "검색조건 : 일일");
		setText(getCell(sheet, 3, 0), "검색기간 : " + selectedDate);
