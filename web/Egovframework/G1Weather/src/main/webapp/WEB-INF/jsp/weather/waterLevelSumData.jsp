...
<script type="text/javaScript" language="javascript" defer="defer">
        ...
        /*function fn_egov_link_page(pageNo){
        	document.waterLevelListForm.pageIndex.value = pageNo;
        	//document.listForm.method = "GET";
        	document.waterLevelListForm.action = "<c:url value='/waterLevelList.do'/>";
           	document.waterLevelListForm.submit();
        }*/
       
        
        function OnExportExcel()
	    {
        	window.parent.document.formWeatherFrame.action = "<c:url value='/downloadExcelWaterLevelList.do'/>";
...
<body>
	...
	<section class="bodyCenter">
		<!-- 페이지 타이틀과 설명 -->
		<ul class="pageTitle">
			<li class="txtPageTil">수위현황</li>
			<!--<li class="txtPageSub">| 수위 기상정보를 확인</li>-->
		</ul>
		...
		<div class="pageTbl">
			...
			<form:form commandName="searchVO" id="waterLevelListForm" name="waterLevelListForm" method="post" action="#LINK">
				<!--테이블 내용부분 -->
				<div class="tblBodyBox"> <!-- 10건 이상되면 자동 스크롤생김 -->
					...
					<tbody class="tblBodyBig">
						<c:forEach var="level" items="${resultList}" varStatus="status">
		           			<tr>
		           				<td><c:out value="${(searchVO.pageIndex-1) * searchVO.pageSize + status.count}"/></td>
		           				<td><c:out value="${level.locationName}"/>&nbsp;</td>
		           				<td><c:out value="${level.timeStamp}"/>&nbsp;</td>
		           				<td><c:out value="${level.standardWaterLevel}"/>&nbsp;</td>
		           				<td><c:out value="${level.waterLevel}"/>&nbsp;</td>
