...
<script type="text/javaScript" language="javascript" defer="defer">
        ...
        function fn_egov_link_page(pageNo){
        	...
        	document.searchRainListForm.pageIndex.value = pageNo;
        	OnSearchRain(pageNo);
        }
        
        function OnExportExcel()
	    {
        	window.parent.document.formWeatherFrame.action = "<c:url value='/downloadExcelSearchRainList.do'/>";
        ...
        function OnSearchRain(pageNo)
        {
        	...
        	if(pageNo == 1)
        	{
        		loadPage("강우조회", "/G1Weather/searchRainDataList.do?param=" + param);
        	}
        	else
       		{
        		var form = document.getElementById("searchRainListForm");
        		var url = "<c:url value='searchRainDataList.do?param=" + param + "'/>";
...
<body>
	...
	<section class="bodyCenter">
		...
		<ul class="searchBox">
			...
			<li class="rightFloat">
				<a href="#" class="btnBlueSc" >
					<input type="submit" value="조 회" onclick="OnSearchRain(1);return false;"/>
				</a>
			</li>
		</ul>
		...
		<form:form commandName="searchVO" id="searchRainListForm" name="searchRainListForm" method="post" action="#LINK">
			...
			<div class="tblBox "> 	
				<table class="tbl100p">				
					...
					<!--테이블 내용부분 -->
					<tbody class="tblBody">
						<c:forEach var="searchRain" items="${resultList}" varStatus="status">
							<c:choose>
								<c:when test="${searchRain.sumData == true}">
									<tr class="sumLine">
								</c:when>
								<c:otherwise>
									<tr>
								...
								<td><c:out value="${searchRain.locationName}"/>&nbsp;</td>
								<c:forEach var="itemValue" items="${searchRain.itemValues}" varStatus="status">
									<c:choose>
