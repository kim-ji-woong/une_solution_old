...
{
    // 참고: "리팩터링" 메뉴에서 "이름 바꾸기" 명령을 사용하여 코드, svc 및 config 파일에서 클래스 이름 "Download"을 변경할 수 있습니다.
    // 참고: 이 서비스를 테스트하기 위해 WCF 테스트 클라이언트를 시작하려면 솔루션 탐색기에서 Download.svc나 Download.svc.cs를 선택하고 디버깅을 시작하십시오.
    public class DownloadService : IDownload
    {
        ...
        /// <summary>
        /// 파일을 다운로드할때 몇개의 Segment로 분할하여야 하는지 알려준다.
        /// 즉, 특정 파일을 다운로드 받기 위하여 Download() 함수를 몇번 호출해야 하는지 알려주는 것이다.
        /// Download()를 호출하기 전에 반드시 GetFileSegmentCount()를 호출하여 Segment 크기를 확인하여야 한다.
        /// </summary>
        /// <param name="filePath">다운로드할 파일의 경로이며, 서버의 로컬경로가 사용된다.</param>
