		...
		<div class="content">
			...
			<div class="well">
				...
				<form id="formLocation" class="bbs_detail"
					action="${pageContext.request.contextPath}/Admin/manage/modify/<%=target.getID() %>"
					method="POST">
					<table>
						<thead>
							<tr>
								...
								<td colspan="3"><input type="text" name="Name"
									placeholder="이름을 입력하세요" value="<%= target.getName() %>"></input></td>
							</tr>
							<tr>
								<th width="80px" class="num">주소</th>
								<td colspan="3"><input type="text" name="Address"
									placeholder="주소를 입력하세요" value="<%= target.getAddress() %>" /> <input type="button"
									name="showAddress" value="검색"></input></td>
							</tr>
							<tr>
								<th width="80px" class="num">상세주소</th>
								<td colspan="3"><input type="text" name="DetailAddress"
									placeholder="상세주소를 입력하세요" value="<%= target.getDetailAddress()%>" /></td>
							</tr>
							<tr>
								<th width="73px" class="num">연락처</th>
								<td colspan="3"><input type="text" name="Phone" value="<%= target.getPhone() %>"
									placeholder="전화번호를 입력하세요" /></td>
							</tr>
							<tr>
								<th width="73px" class="num">구분</th>
								<td colspan="3"><input type="text" name="Description" value="<%= target.getDescription() %>"
                            ...
                            <div class="list">
                            	...
                            	<%
                            	   ...
                            	   for(NodeLocation node : nodeLocList ) {
	                            	   ...
	                            	   {
                            	%>
	                                <li id="<%= node.getID()%>" class="active" onclick="showEditNode(this);return false;" value="<%= node.getID()%>"><a href=""><%= node.getName() %></a></li>
	                            <% 
	                            		}
                            	   	else
                            	   	{
                            	%>
                            		   <li id="<%= node.getID()%>"  value="<%= node.getID() %>" onclick="showEditNode(this);return false;"><a href=""><%= node.getName() %></a></li>
                         		<%
                            	   	}
                            	   }
                            	%>
