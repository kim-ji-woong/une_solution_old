...
namespace UnE
{
	namespace GUI
	{
		public class RibbonButton : Button
		{
            ...
            private Image m_imgCheckedMouseOver = null;
			...
			protected StringFormat m_textFormat = TextData.GetStringFormat();
            ...
            // 버튼 상태 별로 글씨 색상을 변경할지 여부
            protected bool m_bForeColorsByTypeUse = false;
            public bool ForeColorsByTypeUse
            {
                get { return m_bForeColorsByTypeUse; }
                set { m_bForeColorsByTypeUse = value; }
            }

            protected Color m_ForeColorMouseOver = Color.White;
            public Color ForeColorMouseOver
            {
                get { return m_ForeColorMouseOver; }
                set { m_ForeColorMouseOver = value; }
            }
            protected Color m_ForeColorCheckedMouseOver = Color.White;
            public Color ForeColorCheckedMouseOver
            {
                get { return m_ForeColorCheckedMouseOver; }
                set { m_ForeColorCheckedMouseOver = value; }
            }
            protected Color m_ForeColorChecked = Color.White;
            public Color ForeColorChecked
            {
                get { return m_ForeColorChecked; }
                set { m_ForeColorChecked = value; }
            }
            protected Color m_ForeColorDisabled = Color.White;
            public Color ForeColorDisabled
            {
                get { return m_ForeColorDisabled; }
                set { m_ForeColorDisabled = value; }
            }
            ...
            /// <summary>
            /// Check 되어 있는 버튼에 마우스 오버했을 때 이미지
            /// </summary>
            public Image CheckedMouseOver
            {
                get { return m_imgCheckedMouseOver; }
                set { m_imgCheckedMouseOver = value; }
            }
			...
			protected override void OnPaint(PaintEventArgs pevent)
			{
				...
				if (Enabled)
				{
                    ...
                    // 버튼이 Checked 된 상태에서 Mouse over 이미지가 따로 있다면 적용
                    if (m_isChecked && m_isMouseOver && m_imgCheckedMouseOver != null)
                    {
                        ((Bitmap)m_imgCheckedMouseOver).SetResolution(pevent.Graphics.DpiX, pevent.Graphics.DpiY);
                        DrawImage(m_imgCheckedMouseOver, pevent.Graphics);
                    }
				}
				else
				{
				...
				if (Text.Length > 0)
				{
                    if (m_bForeColorsByTypeUse)
                    {
                        if (!Enabled)
                            brush = new SolidBrush(m_ForeColorDisabled);
                        else if (m_isChecked)
                        {
                            if (m_isMouseOver)
                                brush = new SolidBrush(m_ForeColorCheckedMouseOver);
                            else if (!m_isMouseOver)
                                brush = new SolidBrush(m_ForeColorChecked);
                        }
                        else
                        {
                            if (m_isMouseOver)
                                brush = new SolidBrush(m_ForeColorMouseOver);
                            else
                            {
                                // ForeColor를 사용함
                            }
