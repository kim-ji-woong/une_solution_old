...
void UDotSceneLoader::parseDotScene(const String &SceneName, const String &groupName, SceneManager *yourSceneMgr, SceneNode *pAttachNode, const String &sPrependNode)
{
	...
	{
		// Strip the path
		Ogre::String basename, path, scenName, ext;
		Ogre::StringUtil::splitFilename(SceneName, basename, path);
		Ogre::StringUtil::splitBaseFilename(basename, scenName, ext);

		Ogre::ResourceGroupManager& lRgMgr = Ogre::ResourceGroupManager::getSingleton();

		Ogre::ResourceGroupManager::getSingleton().addResourceLocation(path, "FileSystem", groupName, true);
		
		Ogre::String szMat = path + scenName + ".material";
		Ogre::ResourcePtr pMat = Ogre::MaterialManager::getSingleton().getByName(scenName, groupName);
		
		lRgMgr.loadResourceGroup(groupName);
...
void UDotSceneLoader::processScene(TiXmlElement *XMLRoot)
{
...
void UDotSceneLoader::processNodes(TiXmlElement *XMLNode)
{
	TiXmlElement *pElement;
...
void UDotSceneLoader::processExternals(TiXmlElement *XMLNode)
{
	//! @todo Implement this
}

void UDotSceneLoader::processEnvironment(TiXmlElement *XMLNode)
{
	TiXmlElement *pElement;
...
void UDotSceneLoader::processTerrain(TiXmlElement *XMLNode)
{
	//! @todo Implement this
}

void UDotSceneLoader::processUserDataReference(TiXmlElement *XMLNode, SceneNode *pParent)
{
	//! @todo Implement this
}

void UDotSceneLoader::processOctree(TiXmlElement *XMLNode)
{
	//! @todo Implement this
}

void UDotSceneLoader::processLight(TiXmlElement *XMLNode, SceneNode *pParent)
{
	// Process attributes
...
void UDotSceneLoader::processCamera(TiXmlElement *XMLNode, SceneNode *pParent)
{
	// Process attributes
	String name = getAttrib(XMLNode, "name");
	String id = getAttrib(XMLNode, "id");
...
void UDotSceneLoader::processNode(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	char buf[512];
	unsigned long nNode = UnE::Core::UDB::GetUDB()->GetNextCookie();	
	sprintf(buf, "%d#%s#%ld", (int)m_hWnd, pNode->getName().c_str(), nNode);
	...
	UnE::Core::USceneNode * pUNode = pUSeneMan->GetRootSceneNode()->CreateChild(name);
	pUNode->SetAliasName(buf);
	pUNode->SetTag(pNode);
...
void UDotSceneLoader::processLookTarget(TiXmlElement *XMLNode, SceneNode *pParent)
{
...
void UDotSceneLoader::processTrackTarget(TiXmlElement *XMLNode, SceneNode *pParent)
{
	// Process attributes
	String nodeName = getAttrib(XMLNode, "nodeName");
...
void UDotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	// Process attributes
	String name = getAttrib(XMLNode, "name");
	String id = getAttrib(XMLNode, "id");
	...
	try
	{
		...
		UnE::Core::WndCtx * pCtx = GetWndContext((HWND)m_hWnd);
		
		UnE::Core::UEntity * pObject = new UnE::Core::UEntity();
		pObject->SetInternal(pEntity);
		pObject->SetAlias(std::string(name.c_str()));
		pObject->SetName(std::string(pParent->getName().c_str()));
...
void UDotSceneLoader::processParticleSystem(TiXmlElement *XMLNode, SceneNode *pParent)
{
	// Process attributes
	String name = getAttrib(XMLNode, "name");
	String id = getAttrib(XMLNode, "id");
	...
	try
	{
		...
		pParent->attachObject((MovableObject*)pParticles);
	}
	catch (Ogre::Exception &/*e*/)
	{
...
void UDotSceneLoader::processBillboardSet(TiXmlElement *XMLNode, SceneNode *pParent)
{
	//! @todo Implement this
}

void UDotSceneLoader::processPlane(TiXmlElement *XMLNode, SceneNode *pParent)
{
	//! @todo Implement this
}

void UDotSceneLoader::processFog(TiXmlElement *XMLNode)
{
	// Process attributes
...
void UDotSceneLoader::processSkyBox(TiXmlElement *XMLNode)
{
	// Process attributes
...
void UDotSceneLoader::processSkyDome(TiXmlElement *XMLNode)
{
	// Process attributes
...
void UDotSceneLoader::processSkyPlane(TiXmlElement *XMLNode)
{
	// Process attributes
	String material = getAttrib(XMLNode, "material");
...
void UDotSceneLoader::processClipping(TiXmlElement *XMLNode)
{
	//! @todo Implement this
...
void UDotSceneLoader::processLightRange(TiXmlElement *XMLNode, Light *pLight)
{
	// Process attributes
...
void UDotSceneLoader::processLightAttenuation(TiXmlElement *XMLNode, Light *pLight)
{
	// Process attributes
...
String UDotSceneLoader::getAttrib(TiXmlElement *XMLNode, const String &attrib, const String &defaultValue)
{
...
Real UDotSceneLoader::getAttribReal(TiXmlElement *XMLNode, const String &attrib, Real defaultValue)
{
	if (XMLNode->Attribute(attrib.c_str()))
...
bool UDotSceneLoader::getAttribBool(TiXmlElement *XMLNode, const String &attrib, bool defaultValue)
{
	if (!XMLNode->Attribute(attrib.c_str()))
		return defaultValue;
...
Vector3 UDotSceneLoader::parseVector3(TiXmlElement *XMLNode)
{
	return Vector3(
...
Quaternion UDotSceneLoader::parseQuaternion(TiXmlElement *XMLNode)
{
	//! @todo Fix this crap!
...
ColourValue UDotSceneLoader::parseColour(TiXmlElement *XMLNode)
{
	return ColourValue(
...
String UDotSceneLoader::getProperty(const String &ndNm, const String &prop)
{
...
void UDotSceneLoader::processUserDataReference(TiXmlElement *XMLNode, Entity *pEntity)
{
