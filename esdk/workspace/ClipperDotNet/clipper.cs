...
namespace ClipperLib
{
    ...
    using VertexPolygonF = List<UnE.Geometry.Vertex2F>;
    using VertexPolygons = List<List<UnE.Geometry.Vertex2D>>;
    using VertexPolygonFs = List<List<UnE.Geometry.Vertex2F>>;
    using ExVertexPolygons = List<ExVertexPolygon>;
    using ExVertexPolygonFs = List<ExVertexPolygonF>;
    ...
    public class DoublePoint
    {
        ...
        public DoublePoint(DoublePoint pt)
        {
            this.X = pt.X; this.Y = pt.Y;
        }
    }

    public struct ExVertexPolygon
    {
        public VertexPolygon outer;
        public VertexPolygons holes;
    }

    public struct ExVertexPolygonF
    {
        public VertexPolygonF outer;
        public VertexPolygonFs holes;
    }

    public struct ExDoublePolygon
    {
        public DoublePolygon outer;
        public DoublePolygons holes;
    }   
    ///////////////////////////////////////////////////
    ...
    public class Clipper : ClipperBase
    {
        ...
        // ADD BY SKKIM 2012-09-19 : Support Double Polygon 
        //------------------------------------------------------------------------------
        double nEPS = 100000.0;
        private void VertexToInt(UnE.Geometry.Vertex2D p1, ref IntPoint p2)
        {
            p2.X = (Int64)(p1.x * nEPS);
            p2.Y = (Int64)(p1.y * nEPS);
        }
        private void VertexToInt(UnE.Geometry.Vertex2F p1, ref IntPoint p2)
        {
            p2.X = (Int64)(p1.x * nEPS);
            p2.Y = (Int64)(p1.y * nEPS);
        }
        private void VertexToInt(VertexPolygon a, ref Polygon b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                IntPoint p = new IntPoint();
                VertexToInt(a[i], ref p);
                b.Add(p);
            }
        }
        private void VertexToInt(VertexPolygonF a, ref Polygon b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
        ...
        private void VertexToInt(VertexPolygons a, ref Polygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
        ...
        private void VertexToInt(ExVertexPolygon a, ref ExPolygon b)
        {
            Polygons hPg = new Polygons();
            Polygon ePg = new Polygon();
            VertexToInt(a.holes, ref hPg);
            VertexToInt(a.outer, ref ePg);
            b.holes = hPg;
            b.outer = ePg;
        }
        private void VertexToInt(ExVertexPolygons a, ref ExPolygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                ExPolygon iExPoly = new ExPolygon();
                VertexToInt(a[i], ref iExPoly);
                b.Add(iExPoly);
            }
        }
        private void IntToVertex(IntPoint p1, ref UnE.Geometry.Vertex2D p2)
        {
            p2.x = ((double)p1.X / nEPS);
            p2.y = ((double)p1.Y / nEPS);
        }
        private void IntToVertex(IntPoint p1, ref UnE.Geometry.Vertex2F p2)
        {
            p2.x = (float)(p1.X / nEPS);
            p2.y = (float)(p1.Y / nEPS);
        }
        private void IntToVertex(Polygon a, ref VertexPolygon b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                UnE.Geometry.Vertex2D p = new UnE.Geometry.Vertex2D();
                IntToVertex(a[i], ref p);
                b.Add(p);
            }
        }
        private void IntToVertex(Polygon a, ref VertexPolygonF b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                UnE.Geometry.Vertex2F p = new UnE.Geometry.Vertex2F();
                IntToVertex(a[i], ref p);
                b.Add(p);
            }
        }
        private void IntToVertex(Polygons a, ref VertexPolygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                VertexPolygon p = new VertexPolygon(a[i].Count);
                IntToVertex(a[i],ref p);
                b.Add(p);
            }
        }
        private void IntToVertex(Polygons a, ref VertexPolygonFs b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                VertexPolygonF p = new VertexPolygonF(a[i].Count);
                IntToVertex(a[i], ref p);
                b.Add(p);
            }
        }
        private void IntToVertex(ExPolygon a, ref ExVertexPolygon b)
        {
            
            VertexPolygons hPg = new VertexPolygons();
            VertexPolygon ePg = new VertexPolygon();
            IntToVertex(a.holes, ref hPg);
            IntToVertex(a.outer, ref ePg);
            b.holes = hPg;
            b.outer = ePg;
        }
        private void IntToVertex(ExPolygon a, ref ExVertexPolygonF b)
        {

            VertexPolygonFs hPg = new VertexPolygonFs();
            VertexPolygonF ePg = new VertexPolygonF();
            IntToVertex(a.holes, ref hPg);
            IntToVertex(a.outer, ref ePg);
            b.holes = hPg;
            b.outer = ePg;
        }
        private void IntToVertex(ExPolygons a, ref ExVertexPolygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                ExVertexPolygon iExPoly = new ExVertexPolygon();
                IntToVertex(a[i], ref iExPoly);
                b.Add(iExPoly);
            }
        }
        private void IntToVertex(ExPolygons a, ref ExVertexPolygonFs b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                ExVertexPolygonF iExPoly = new ExVertexPolygonF();
                IntToVertex(a[i], ref iExPoly);
                b.Add(iExPoly);
            }
        }
        public bool AddPolygons(VertexPolygons ppg, PolyType polyType)
        {
            bool result = false;
            for (int i = 0; i < ppg.Count; ++i)
                if (AddPolygon(ppg[i], polyType)) result = true;
        ...
        public bool AddPolygon(VertexPolygon pg, PolyType polyType)
        {
            Polygon ipg = new Polygon(pg.Count);
            VertexToInt(pg, ref ipg);
            bool succeeded = base.AddPolygon(ipg, polyType);
            return succeeded;
        }
        public bool AddPolygon(VertexPolygonF pg, PolyType polyType)
        {
            Polygon ipg = new Polygon(pg.Count);
            VertexToInt(pg, ref ipg);
            bool succeeded = base.AddPolygon(ipg, polyType);
        ...
        public bool Execute(ClipType clipType, VertexPolygons solution,
           PolyFillType subjFillType, PolyFillType clipFillType)
        {
            Polygons iSolution = new Polygons();
            bool succeeded = Execute(clipType, iSolution, subjFillType, clipFillType);
            IntToVertex(iSolution, ref solution);
            return succeeded;
        }

        public bool Execute(ClipType clipType, ExVertexPolygons solution,
            PolyFillType subjFillType, PolyFillType clipFillType)
        {
            ExPolygons iSolution = new ExPolygons();
            bool succeeded = Execute(clipType, iSolution, subjFillType, clipFillType);
            solution.Clear();
            IntToVertex(iSolution, ref solution);
            return succeeded;
        ...
        public bool Execute(ClipType clipType, ExVertexPolygonFs solution,
            PolyFillType subjFillType, PolyFillType clipFillType)
        {
            ExPolygons iSolution = new ExPolygons();
            bool succeeded = Execute(clipType, iSolution, subjFillType, clipFillType);
        ...
        //========================================================================
        private void DoubleToInt(DoublePoint p1, IntPoint p2)
        {
            p2.X = (Int64)(p1.X * nEPS);
            p2.Y = (Int64)(p1.Y * nEPS);
        }
        private void IntToDouble(IntPoint p1, DoublePoint p2)
        {
            p2.X = ((double)p1.X / nEPS);
            p2.Y = ((double)p1.Y / nEPS);
        }

        private void DoubleToInt(DoublePolygon a, Polygon b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
        ...
        private void DoubleToInt(DoublePolygons a, Polygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
        ...
        private void DoubleToInt(ExDoublePolygon a, ExPolygon b)
        {
            Polygons hPg = new Polygons();
            Polygon ePg = new Polygon();
            DoubleToInt(a.holes, hPg);
            DoubleToInt(a.outer, ePg);
            b.holes = hPg;
            b.outer = ePg;
        }
        private void DoubleToInt(ExDoublePolygons a, ExPolygons b)
        {
            b.Clear();
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                ExPolygon iExPoly = new ExPolygon();
                DoubleToInt(a[i], iExPoly);
                b.Add(iExPoly);
            }
        }
       
        private void IntToDouble(Polygon a, DoublePolygon b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
        ...
        private void IntToDouble(Polygons a, DoublePolygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                DoublePolygon p = new DoublePolygon(a[i].Count);
                IntToDouble(a[i], p);
                b.Add(p);
            }
        }
        private void IntToDouble(ExPolygon a, ExDoublePolygon b)
        {
            DoublePolygons hPg = new DoublePolygons();
            DoublePolygon ePg = new DoublePolygon();
            IntToDouble(a.holes, hPg);
            IntToDouble(a.outer, ePg);
            b.holes = hPg;
            b.outer = ePg;
        }
        private void IntToDouble(ExPolygons a, ExDoublePolygons b)
        {
            int nCount = a.Count;
            for (int i = 0; i < nCount; i++)
            {
                ExDoublePolygon iExPoly = new ExDoublePolygon();
                IntToDouble(a[i], iExPoly);
                b.Add(iExPoly);
            }
        }

        public bool AddPolygons(DoublePolygons ppg, PolyType polyType)
        {
            bool result = false;
            for (int i = 0; i < ppg.Count; ++i)
                if (AddPolygon(ppg[i], polyType)) result = true;
        ...
        public bool AddPolygon(DoublePolygon pg, PolyType polyType)
        {
            Polygon ipg = new Polygon(pg.Count);
            DoubleToInt(pg, ipg);
            bool succeeded = base.AddPolygon(ipg, polyType);
        ...
        public bool Execute(ClipType clipType, DoublePolygons solution,
           PolyFillType subjFillType, PolyFillType clipFillType)
        {
            Polygons iSolution = new Polygons();
            bool succeeded = Execute(clipType, iSolution, subjFillType, clipFillType);
            IntToDouble(iSolution, solution);
            return succeeded;
        }     

        public bool Execute(ClipType clipType, ExDoublePolygons solution,
            PolyFillType subjFillType, PolyFillType clipFillType)
        {
            ExPolygons iSolution = new ExPolygons();
            bool succeeded = Execute(clipType, iSolution, subjFillType, clipFillType);
