...
namespace UnE
{
    namespace SOP
    {
        namespace History
        {
            public class HistoryManager : ISOPHistoryContainer
            {
                ...
                public HistorySectionDecisionData AddDecisionHistory(SectionDecision section, Workstate.State state, int nProcessDirections, Section nextSection, bool noDBWrite, DateTime time, bool showBoard)
                {
                    // 입력대기는 기록하지 않는다.
                ...
                // nextSection : 분기문을 통하여 실행된 Section
                public HistorySectionDecisionData AddDecisionHistory(SectionDecision section, Workstate.State state, int nProcessDirections, Section nextSection = null, bool showBoard = false)
                {
                    return AddDecisionHistory(section, state, nProcessDirections, nextSection, false, DateTime.Now, showBoard);
                }

                public HistorySectionInternalData AddInternalHistory(SectionInternal section, Workstate.State state, int nProcessDirections, int nCheckedRun, int nCheckedComplete, bool usePopupMessage, bool useSMS, bool useBroadcast, bool noDBWrite, DateTime time, bool showBoard, int nCheckedNotify1)
                {
                    // 입력대기는 기록하지 않는다.
                    //if (state == State.INPUT)
                    //    return;
                ...
                public HistorySectionInternalData AddInternalHistory(SectionInternal section, Workstate.State state, int nProcessDirections, int nCheckedNotify1, int nCheckedRun, int nCheckedComplete, bool usePopupMessage = false, bool useSMS = false, bool useBroadcast = false, bool showBoard = false)
                {
                    return AddInternalHistory(section, state, nProcessDirections, nCheckedRun, nCheckedComplete, usePopupMessage, useSMS, useBroadcast, false, DateTime.Now, showBoard, nCheckedNotify1);
                }
                ...
                public HistorySectionData AddSectionHistory(Section section, int nComponentHistoryID, Workstate.State state, int nProcessDirections, bool noDBWrite, DateTime time, bool showBoard, int nCheckedNotify1, int nCheckedNotify2, int nCheckedRun, int nCheckedComplete, Dictionary<int, List<HistorySectionData.DetailData>> detailDatas)
                {
                    ...
                    {
                        data = AddDecisionHistory((SectionDecision)section, state, nProcessDirections, null, noDBWrite, time, showBoard);
                        if (data != null)
                            data.ComponentHistoryID = nComponentHistoryID;
                    }
                    else if (type == Section.ComponentType.INTERNAL)
                    {
                        data = AddInternalHistory((SectionInternal)section, state, nProcessDirections, nCheckedRun, nCheckedComplete, false, false, false, noDBWrite, time, showBoard, nCheckedNotify1);
                        if (data != null)
                            data.ComponentHistoryID = nComponentHistoryID;
                    }
                ...
                public HistorySectionData AddSectionHistory(Section section, Workstate.State state, int nProcessDirections, bool showBoard, int nCheckedNotify1, int nCheckedNotify2, int nCheckedRun, int nCheckedComplete, Dictionary<int, List<HistorySectionData.DetailData>> detailDatas)
                {
                    return AddSectionHistory(section, -1, state, nProcessDirections, false, DateTime.Now, showBoard, nCheckedNotify1, nCheckedNotify2, nCheckedRun, nCheckedComplete, detailDatas);
                }
                ...
                private ActionStepInfo ReadOldActionStep(int nActionStepID)
                {
                    ...
                    ArrayList arrResult = ProxySOP.Instance.DBManager.GetResultData(strSQL, 0);
                    if (arrResult == null)
                        return null;
                ...
                private int GetMaxIDBatch(string strTableName, WebDBManager dbMgr, int nTransCode)
                {
                    string strSQL = "select max(ID) from " + strTableName;
                    ArrayList arrResult = dbMgr.GetBatchData(nTransCode, strSQL);
                ...
                private bool WriteSectionEndPointHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(2, strSQL) == null)
                            {
                                dbMgr.BatchRollback(2);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 2))
                                dbMgr.BatchCommit(2);
                            else
                                dbMgr.BatchRollback(2);
                        }
                ...
                private bool WriteSectionProcessHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(3, strSQL) == null)
                            {
                                dbMgr.BatchRollback(3);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 3))
                                dbMgr.BatchCommit(3);
                            else
                                dbMgr.BatchRollback(3);
                        }
                ...
                private bool WriteSectionDetail(WebDBManager dbMgr, int nComponentHistoryID, HistorySectionData data, SectionState sectionState, int nTransactionCode)
                {
                    foreach (KeyValuePair<int, List<HistorySectionData.DetailData>> pair in data.HistoryDetailDatas)
                    {
                        ...
                        //foreach (HistorySectionData.DetailData detail in pair.Value)
                        {
                            ...
                            int nID = GetMaxIDBatch("ComponentHistoryDetail", dbMgr, nTransactionCode) + 1;
                            ...
                            if (nTransactionCode > 0)
                            {
                                if (dbMgr.GetBatchData(nTransactionCode, strSQL) == null)
                                    return false;
                            }
                            else
                            {
                ...
                private bool WriteSectionDecisionHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionDecisionData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(4, strSQL) == null)
                            {
                                dbMgr.BatchRollback(4);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 4))
                                dbMgr.BatchCommit(4);
                            else
                                dbMgr.BatchRollback(4);
                        }
                ...
                private bool WriteSectionInternalHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionInternalData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(5, strSQL) == null)
                            {
                                dbMgr.BatchRollback(5);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 5))
                                dbMgr.BatchCommit(5);
                            else
                                dbMgr.BatchRollback(5);
                        }
                ...
                private bool WriteSectionExternalHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionExternalData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(6, strSQL) == null)
                            {
                                dbMgr.BatchRollback(6);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 6))
                                dbMgr.BatchCommit(6);
                            else
                                dbMgr.BatchRollback(6);
                        }
                ...
                private bool WriteSectionTransmissionHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionTransmissionData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(7, strSQL) == null)
                            {
                                dbMgr.BatchRollback(7);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 7))
                                dbMgr.BatchCommit(7);
                            else
                                dbMgr.BatchRollback(7);
                        }
                ...
                private bool WriteSectionTransSOPHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(strSQL, 8) == null)
                            {
                                dbMgr.BatchRollback(8);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 8))
                                dbMgr.BatchCommit(8);
                            else
                                dbMgr.BatchRollback(8);
                        }
                ...
                private bool WriteSectionLinkHistory(WebDBManager dbMgr, int nHistoryID, int nComponentID, HistorySectionData data, Workstate.SectionState sectionState, ref int nMaxComponentHistoryID, ref DateTime dtMaxComponentHistoryTime)
                {
                    ...
                    if (!data.NoDBWrite)
                    {
                        lock (m_objBatchQueryLock)
                        {
                            ...
                            if (dbMgr.GetBatchData(9, strSQL) == null)
                            {
                                dbMgr.BatchRollback(9);
                                return false;
                            }
                            //if (dbMgr.GetResultData(strSQL, 0) == null)
                            //    return false;
                            ...
                            if (WriteSectionDetail(dbMgr, nMaxComponentHistoryID, data, sectionState, 9))
                                dbMgr.BatchCommit(9);
                            else
                                dbMgr.BatchRollback(9);
                        }
                ...
                private void MonitorComponentHistory(int nActionStepID, bool isRealMode, bool isRegular, bool isNormal, string strActionStepFullPath, int nActionStepHistoryID)
                {
                    ...
                    string strFormat = "select ch.ID, ch.ComponentID, ch.ComponentType, ch.Time, ch.Status, ch.Task, ch.CompleteCount, ch.CheckedNotify1, ch.CheckedNotify2, " +
                                       " ch.CheckedRun, ch.CheckedComplete, ch.Description, ch.ShowBoard, ch.AccessedUserID, ch.ActionStepHistoryID, ash.SensorZoneHistoryID " +
                    ...
                    sopLog.AddSOPSectionLog(nActionStepID, arrComponentHistoryID, arrSections4Log, arrSectionStatus4Log, arrSectionProcessDirections4Log, arrTask, arrTime, arrDescription, arrShowBoard, arrCheckedNotify1, arrCheckedNotify2, arrCheckedRun, arrCheckedComplete, arrAccessedUserID, isRealMode, dicDetailDatas, workFlow);
