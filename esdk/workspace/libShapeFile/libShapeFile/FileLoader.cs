...
namespace libShapeFile
{
    public class FileLoader
    {
        ...
        protected IFileEventListener mListener = null;
        public FileLoader(IFileEventListener listener)
        {
            mListener = listener;
        }
        ...
        public List<Shape> LoadFile(string strShapeFilePath, out ShapeInfo shapeInfo)
        {
            ...
            if( mListener != null)
            {
                mListener.BeginReadFile(strShapeFilePath, "SHP", recordHeaders.Length);
            }
            ...
            switch (fileHeader.ShapeType)
            {
                case ShapeType.Point:
                    if (!Point.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.Polygon:
                    if (!Polygon.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PolyLine:
                    if (!PolyLine.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.MultiPoint:
                    if (!MultiPoint.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PointM:
                    if (!PointM.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PolygonM:
                    if (!PolygonM.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PolyLineM:
                    if (!PolyLineM.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.MultiPointM:
                    if (!MultiPointM.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PointZ:
                    if (!PointZ.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PolygonZ:
                    if (!PolygonZ.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.PolyLineZ:
                    if (!PolyLineZ.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                case ShapeType.MultiPointZ:
                    if (!MultiPointZ.Load(mListener, shapeFileStream, recordHeaders, shapes))
                        goto RETURN_FALSE;
                    break;
                default:
            ...
            if (mListener != null)
            {
                mListener.EndReadFile(strShapeFilePath, "SHP");
            }
