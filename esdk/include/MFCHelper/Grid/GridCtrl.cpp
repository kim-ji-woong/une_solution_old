...
//}}AFX_MSG_MAP
	...
	ON_NOTIFY(CBN_SELCHANGED, IDC_INPLACE_CONTROL, OnCbnSelChanged)
END_MESSAGE_MAP()
								...
								BOOL bSelectPastedCells /*=TRUE*/)
{
    ...
    CString strText = (TCHAR*)bstrBuffer;//szBuffer;
    delete szBuffer;
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    TRY
    {
        ...
        {
            m_arColWidths.InsertAt(nColumn,0, (int)0);
            if (!GetVirtualMode())
            {
                for (int row = 0; row < m_nRows; row++) 
                {
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    TRY
    {
        ...
        {
            m_arRowHeights.InsertAt(nRow,0, (int)0);
            if (!GetVirtualMode())
...
void CGridCtrl::SetCheckBox(int nRow, int nCol, BOOL bCheck, BOOL bDisable)
{
	if(!SetCellType(nRow,nCol, RUNTIME_CLASS(CGridCellCheck)))
		return;

	UINT dwState = GetItemState(nRow,nCol);
	//if(bDisable)
		dwState |= GVIS_READONLY;
	SetItemState(nRow, nCol, dwState);
	CGridCellCheck* pCell = (CGridCellCheck*)GetCell(nRow,nCol);
	pCell->SetCheck(bCheck);
	pCell->SetPosition(1);
}

BOOL CGridCtrl::GetCheck(int row, int col)
{
	CGridCellCheck* pCell = (CGridCellCheck*)GetCell(row,col);
	return pCell->GetCheck();
}

void CGridCtrl::OnCbnSelChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	GV_DISPINFO *pgvDispInfo = (GV_DISPINFO *)pNMHDR;

//	pgvDispInfo->hdr.idFrom   = GetDlgCtrlID();

	CWnd* pOwner = GetOwner();
    if (IsWindow(pOwner->GetSafeHwnd()))
        pOwner->SendMessage(WM_CBN_SELCHANGED, GetDlgCtrlID(), (LPARAM)pgvDispInfo );
}
