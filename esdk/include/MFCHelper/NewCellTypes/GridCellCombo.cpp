...
CStringArray& CGridCellCombo::GetStrings()
{
	return m_Strings;
}

int CGridCellCombo::GetItemCount() const
{
...
{
	int nSize = (int)m_Strings.GetSize();
	CString str = GetText();

	for (int i=0;i<nSize;i++)
	{
		if (str == m_Strings[i]) return i;
	}
...
void CInPlaceList::OnCbnSelchange()
{
	...
	int nSel = GetCurSel();
	CString str;
	this->GetLBText(nSel, str);

    dispinfo.hdr.hwndFrom = GetSafeHwnd();
    dispinfo.hdr.idFrom   = GetDlgCtrlID();
    dispinfo.hdr.code     = CBN_SELCHANGED;
