...
BOOL CGridCellCheck::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	// Do the draw
	UINT nState = m_bChecked ? DFCS_BUTTONCHECK | DFCS_CHECKED : DFCS_BUTTONCHECK;

	if ((m_nState & GVIS_DISABLED) == GVIS_DISABLED)
	{
...
CRect CGridCellCheck::GetCheckPlacement()
{
	...
	*/
	...
	if (m_nPosition == 1)
	{
		int nPlaceWidth = place.Width();
		place.left = m_Rect.left + m_Rect.Width() / 2 - nPlaceWidth / 2;
		place.right = place.left + nPlaceWidth;
	}

	return place;
}

void CGridCellCheck::SetPosition(int nPosition)
{
	m_nPosition = nPosition;
}

int CGridCellCheck::GetPosition() const
{
	return m_nPosition;
}
