#ifndef __UNE_CORE_FRUSTUM_H_INCLUDED__
#define __UNE_CORE_FRUSTUM_H_INCLUDED__
...
namespace UnE
{
	namespace Core
	{
		...
		*/
		...
		class CORE_API Frustum : public UCoreObject
		{
		protected:
			...
			UnE::Math::Vector2 mFrustumOffset;
			/// Focal length of frustum (for stereo rendering, defaults to 1.0)
			Real mFocalLength;

			/// The 6 main clipping planes
			mutable UnE::Math::Plane mFrustumPlanes[6];

			/// Stored versions of parent orientation / position
			mutable UnE::Math::Quaternion mLastParentOrientation;
			mutable UnE::Math::Vector3 mLastParentPosition;

			/// Pre-calced projection matrix for the specific render system
			mutable UnE::Math::Matrix4 mProjMatrixRS;
			/// Pre-calced standard projection matrix but with render system depth range
			mutable UnE::Math::Matrix4 mProjMatrixRSDepth;
			/// Pre-calced standard projection matrix
			mutable UnE::Math::Matrix4 mProjMatrix;
			/// Pre-calced view matrix
			mutable UnE::Math::Matrix4 mViewMatrix;
		...
		public:
			 ...
			 */
			virtual void setFOVy(const UnE::Math::Radian& fovy);

			/** Retrieves the frustums Y-dimension Field Of View (FOV).
			*/
			virtual const UnE::Math::Radian& getFOVy(void) const;
			...
			*/
			virtual void setFrustumOffset(const UnE::Math::Vector2& offset);
			...
			*/
			virtual const UnE::Math::Vector2& getFrustumOffset() const;
			...
			*/
			virtual const UnE::Math::Matrix4& getProjectionMatrixRS(void) const;
			...
			*/
			virtual const UnE::Math::Matrix4& getProjectionMatrixWithRSDepth(void) const;
			...
			*/
			virtual const UnE::Math::Matrix4& getProjectionMatrix(void) const;

			/** Gets the view matrix for this frustum. Mainly for use by OGRE internally.
			*/
			virtual const UnE::Math::Matrix4& getViewMatrix(void) const;
			...
			*/
			virtual void calcViewMatrixRelative(const UnE::Math::Vector3& relPos, UnE::Math::Matrix4& matToUpdate) const;
			...
			*/
			virtual void setCustomViewMatrix(bool enable, 
				const UnE::Math::Matrix4& viewMatrix = UnE::Math::Matrix4::IDENTITY);
			...
			*/
			virtual void setCustomProjectionMatrix(bool enable, 
				const UnE::Math::Matrix4& projectionMatrix = UnE::Math::Matrix4::IDENTITY);
			...
			*/
			virtual const UnE::Math::Plane* getFrustumPlanes(void) const;

			/** Retrieves a specified plane of the frustum (world space).
				@remarks
					Gets a reference to one of the planes which make up the frustum frustum, e.g. for clipping purposes.
			*/
			virtual const UnE::Math::Plane& getFrustumPlane( unsigned short plane ) const;
			...
			*/
			virtual bool isVisible(const UnE::Math::AxisAlignedBox& bound, FrustumPlane* culledBy = 0) const;
			...
			/** Tests whether the given container is visible in the Frustum.
				...
				@par
					Otherwise, false is returned.
			*/
			virtual bool isVisible(const UnE::Math::Sphere& bound, FrustumPlane* culledBy = 0) const;
			...
			/** Tests whether the given vertex is visible in the Frustum.
				...
				@par
					Otherwise, false is returned.
			*/
			virtual bool isVisible(const UnE::Math::Vector3& vert, FrustumPlane* culledBy = 0) const;

			/// Overridden from MovableObject::getTypeFlags
			uint32 getTypeFlags(void) const;

			/** Overridden from MovableObject */
			const UnE::Math::AxisAlignedBox& getBoundingBox(void) const;
			...
			/** Overridden from Renderable */
			void getWorldTransforms(UnE::Math::Matrix4* xform) const;
			...
			*/
			virtual const UnE::Math::Vector3* getWorldSpaceCorners(void) const;
			...
			*/
			//virtual void enableReflection(const UnE::Math::Plane& p);
			/** Modifies this frustum so it always renders from the reflection of itself through the
			...
			*/
			...
			virtual const UnE::Math::Matrix4& getReflectionMatrix(void) const { return mReflectMatrix; }
			/// Returns the reflection plane of the frustum if appropriate
			virtual const UnE::Math::Plane& getReflectionPlane(void) const { return mReflectPlane; }
			...
			*/
			virtual bool projectSphere(const UnE::Math::Sphere& sphere, 
			...
			*/
			//virtual void enableCustomNearClipPlane(const UnE::Math::Plane* plane);
			/** Links the frustum to a custom near clip plane, which can be used
				to clip geometry in a custom manner without using user clip planes.
			@remarks
				There are several applications for clipping a scene arbitrarily by
			...
			*/
			...
			virtual const UnE::Math::Vector3& getPositionForViewUpdate(void) const;
			/** Get the derived orientation of this frustum. */
			virtual const UnE::Math::Quaternion& getOrientationForViewUpdate(void) const;

			/** Gets a world-space list of planes enclosing the frustum.
			*/
			UnE::Math::PlaneBoundedVolume getPlaneBoundedVolume();
