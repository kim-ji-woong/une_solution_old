...
#define WM_FIND_ALLDOC_CMD		WM_USER+34
#define WM_REPLACE_ALLDOC_CMD	WM_USER+35
...
//---find & replace
	...
	BOOL FindReplaceDlg(BOOL iReplace);
	void OnFindReplaceCommand(int iCommand,BOOL iFocus=TRUE);
	VIFindReplaceDlg*	m_FindReplaceDlg;
	void OnFindCommand();
	void OnReplaceCommand();
	void ReplaceAll();

	XUndoUnit* ReplaceNext();
	BOOL FindNext(CHARRANGE &range,BOOL down=TRUE,BOOL select=FALSE,BOOL* oEndOfFilePassed=0);
public:
	XString	m_findStr;
...
//---- icon, breakpoint, any kind of mark, managment & margin management too
	...
	BOOL HasMarkInLine(int indexInImageList,int line);
