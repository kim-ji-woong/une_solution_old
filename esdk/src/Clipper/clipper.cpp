...
bool Clipper::IsTopHorz(const long64 XPos)
{
  ...
  while( e )
  {
    if(  ( XPos >= std::min<long64>(e->xcurr, e->xtop) ) &&
      ( XPos <= std::max<long64>(e->xcurr, e->xtop) ) ) return false;
    e = e->nextInSEL;
  }
  return true;
}
...
{
  int steps = std::max<int>(6, int(std::sqrt(std::fabs(r)) * std::fabs(a2 - a1)));
...
// 111212 KYJ
// 재정의
void Clipper::operator=(Clipper& obj)
{
	if(m_pPolygonSet)
		delete m_pPolygonSet;

	m_pPolygonSet = new Clipper_PolygonSet;

	int nCnt = obj.m_pPolygonSet->nCnt;
	m_pPolygonSet->pPolygon = new Clipper_Polygon[nCnt];
	m_pPolygonSet->nCnt = nCnt;
	for(int i=0; i<nCnt; i++)
	{
		m_pPolygonSet->pPolygon[i].bHole = obj.m_pPolygonSet->pPolygon[i].bHole;
		m_pPolygonSet->pPolygon[i].vcPoint = obj.m_pPolygonSet->pPolygon[i].vcPoint;
	}
}

Clipper_PolygonSet* Clipper::Clip(ClipType clipType, Clipper_PolygonSet* pPolygonSet, PolyFillType subjFillType, PolyFillType clipFillType)
{
	Polygons polygonSet1, polygonSet2;
	GetPolygons(m_pPolygonSet, polygonSet1);
	GetPolygons(pPolygonSet, polygonSet2);

	Clear();
	AddPolygons(polygonSet1, ptSubject);
	AddPolygons(polygonSet2, ptClip);

	Excute(clipType, subjFillType, clipFillType);

	return m_pPolygonSet;
}

Clipper_PolygonSet* Clipper::Clip(ClipType clipType, DoublePolygon& polygon, PolyFillType subjFillType, PolyFillType clipFillType)
{
	DoublePolygons polygons(1);
	polygons[0] = polygon;

	Polygons polygonSet1, polygonSet2;
	GetPolygons(m_pPolygonSet, polygonSet1);
	GetPolygons(polygons, polygonSet2);
...
bool Clipper::Excute(ClipType clipType, PolyFillType subjFillType, PolyFillType clipFillType)
{
	if( m_ExecuteLocked ) 
		return false;
...
void GetPolygons(Clipper_PolygonSet* pPolygonSet, Polygons& rResultPolygons)
{
	rResultPolygons.clear();

	if(pPolygonSet)
	{
		for(int i=0; i<pPolygonSet->nCnt; i++)
		{
			int PointSize = pPolygonSet->pPolygon[i].vcPoint.size();
			Polygon pointSet;
			for(int k=0; k<PointSize; k++)
			{
				long64 x = (LONG64)floor(pPolygonSet->pPolygon[i].vcPoint[k].X * 1000.+ 0.5);
				long64 y = (LONG64)floor(pPolygonSet->pPolygon[i].vcPoint[k].Y * 1000.+ 0.5);
				IntPoint point(x, y);
				pointSet.push_back(point);
			}
			rResultPolygons.push_back(pointSet);
		}
	}
}

void GetPolygons(DoublePolygons& rPolygons, Polygons& rResultPolygons)
{
	rResultPolygons.clear();

	int size = (int)rPolygons.size();
	if(size != 0)
	{
		for(int i=0; i<size; i++)
		{
			int PointSize = rPolygons[i].size();
			Polygon pointSet;
			for(int k=0; k<PointSize; k++)
			{
				long64 x = (LONG64)floor(rPolygons[i][k].X * 1000.+ 0.5);
				long64 y = (LONG64)floor(rPolygons[i][k].Y * 1000.+ 0.5);
				IntPoint point(x, y);
				pointSet.push_back(point);
			}
			rResultPolygons.push_back(pointSet);
...
Clipper_PolygonSet* Clipper::GetPolygonSet()
{
	return m_pPolygonSet;
}

void Clipper::SetPolygonSet(Clipper_PolygonSet* pPolygonSet)
{
	if(m_pPolygonSet)
		delete m_pPolygonSet;

	m_pPolygonSet = pPolygonSet;
}

void Clipper::BuildResult()
{
	m_pPolygonSet = new Clipper_PolygonSet;

	std::vector<Clipper_Polygon> vcPolygon;
	int size = m_PolyOuts.size();
	for (PolyOutList::size_type i = 0; i < m_PolyOuts.size(); ++i)
	{
		if (m_PolyOuts[i]->pts)
		{
			Clipper_Polygon set;
			set.bHole = m_PolyOuts[i]->isHole;

			OutPt* p = m_PolyOuts[i]->pts;

			do
			{
				double x = (double)((double)p->pt.X * 0.001);
				double y = (double)((double)p->pt.Y * 0.001);
				set.vcPoint.push_back(DoublePoint(x, y));

				p = p->next;
			} while (p != m_PolyOuts[i]->pts);

			//make sure each polygon has at least 3 vertices ...
			if (set.vcPoint.size() >= 3) 
				vcPolygon.push_back(set);
		}
	}

	size = (int)vcPolygon.size();
	if(size == 0)
		return;

	m_pPolygonSet->pPolygon = new Clipper_Polygon[size];
	m_pPolygonSet->nCnt = size;
	for(int i=0; i<(int)vcPolygon.size(); i++)
	{
		m_pPolygonSet->pPolygon[i].bHole = vcPolygon[i].bHole;
		m_pPolygonSet->pPolygon[i].vcPoint = vcPolygon[i].vcPoint;
	}
}

void Clipper::RemovePolygonSet()
{
	if(m_pPolygonSet)
	{
		delete m_pPolygonSet;
...
double Clipper::Get3DotValue(double dValue)
{
	//dValue = floor( dValue * 1000. + 0.5 ) * 0.001;
	return dValue;
}


bool lines_intersect(IntPoint AP1,IntPoint AP2,IntPoint BP1,IntPoint BP2,IntPoint& IP) 
{
	double t;
	double s; 
	...
	long64 _t = (BP2.X-BP1.X)*(AP1.Y-BP1.Y) - (BP2.Y-BP1.Y)*(AP1.X-BP1.X);
	long64 _s = (AP2.X-AP1.X)*(AP1.Y-BP1.Y) - (AP2.Y-AP1.Y)*(AP1.X-BP1.X); 
	...
	IP.X = AP1.X + (long64)(t * (double)(AP2.X-AP1.X));
	IP.Y = AP1.Y + (long64)(t * (double)(AP2.Y-AP1.Y));

	//플로팅 오류 보정
	IP.X = (long64)(((double)IP.X / 1000.) + 0.005) * 1000;
	IP.Y = (long64)(((double)IP.Y / 1000.) + 0.005) * 1000;
	return true;

}
DoublePolygon convIntPolyToDoublePoly(const Polygon& poly )
{
	DoublePolygon resultPoly;
	for(size_t i=0;i<poly.size();++i)
	{
		resultPoly.push_back(DoublePoint((double)poly[i].X/1000,(double)poly[i].Y/1000));
	}
	return resultPoly;
}
