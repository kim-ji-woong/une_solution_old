...
bool DXFLoader::Load(System::String^ strPath)
{
	...
	m_ctrl->SendBeginRead(strPath, ToSystemString(L"DXF"), (int)entMgr.GetAllEntityCount());
	...
	if (m_ctrl->Renderer == IPainter::RendererType::GDI_PLUS)
	{
		if (!m_useLastViewport)
		{
			if (vObjectCenter->x <= m_vObjectTL->x + UnE::Geometry::Math::HALF_TOLERANCE() ||
				vObjectCenter->y <= m_vObjectBR->y + UnE::Geometry::Math::HALF_TOLERANCE())
			{
				vObjectCenter->x = (m_vObjectTL->x + m_vObjectBR->x) / 2;
				vObjectCenter->y = (m_vObjectTL->y + m_vObjectBR->y) / 2;
			}

			double weight1 = m_ctrl->Size.Width * 0.85 / ((vObjectCenter->x - m_vObjectTL->x) * 2);
			double weight2 = m_ctrl->Size.Height * 0.85 / ((vObjectCenter->y - m_vObjectBR->y) * 2);
			dViewportWeight = weight1 < weight2 ? weight1 : weight2;
		}

		UnE::Geometry::Vertex2D^ vCenter = gcnew UnE::Geometry::Vertex2D(vObjectCenter->x, vObjectCenter->y);
		//UnE::Geometry::Vertex2D^ vObjectCenter = (m_vObjectTL + m_vObjectBR) / 2;
		...
		vObjectCenter->x += dMoveX;
		vObjectCenter->y += dMoveY;

		m_ctrl->MoveAll(dMoveX, dMoveY);

		m_ctrl->SetViewportCenter(vObjectCenter);
		m_ctrl->Zoom(dViewportWeight, vObjectCenter, false);

		/*if (m_ctrl->DownToTop())
		{
			// Y축이 아래에서 위쪽으로 향하는 방향이면, Window 좌표계와 반대이기 때문에
			// 화면 크기만큼 아래쪽으로 내려주는 보정을 해야 한다.
			UnE::Geometry::Vertex2D^ vViewportCenter = m_ctrl->GetViewportCenter();

			//double dy = vObjectCenter->y / 2 - vViewportCenter->y;
			double dy = vObjectCenter->y - vViewportCenter->y;
			vObjectCenter->y -= dy * 2 + m_ctrl->Size.Height * m_ctrl->GetViewportWeight();
		}

		Vertex2D^ vCurrent = CalcInitCenter(vObjectCenter);

		//m_ctrl->SetViewportWeight(dViewportWeight);
		m_ctrl->Zoom(dViewportWeight, vCurrent, false);*/
	}
	else if (m_ctrl->Renderer == IPainter::RendererType::OPEN_GL)
	{
		double left = vObjectCenter->x - dViewportWeight * m_ctrl->Size.Width / 2;
		double top = vObjectCenter->y + dViewportWeight * m_ctrl->Size.Height / 2;
		double right = vObjectCenter->x * 2 - left;
		double bottom = vObjectCenter->y * 2 - top;

		Viewport^ viewport = gcnew Viewport();

		viewport->TopLeft = gcnew Vertex2D(left, top);
		viewport->BottomLeft = gcnew Vertex2D(left, bottom);
		viewport->BottomRight = gcnew Vertex2D(right, bottom);
		viewport->Weight = dViewportWeight;

		m_ctrl->LoadViewport(viewport, false);
		m_ctrl->Zoom(dViewportWeight, vObjectCenter, false);
	}
	...
	//m_ctrl->SetViewportWeight(dViewportWeight);
	//m_ctrl->SetViewportCenter(vObjectCenter);
	//m_ctrl->Zoom(dViewportWeight, vCenter, false);

	if (m_ctrl->OpenNRefresh)
	{
		m_ctrl->_Refresh();
		m_ctrl->SaveHomeMatrix();
	}
	m_ctrl->SendEndRead(strPath, ToSystemString(L"DXF"));
	return true;
}
...
double DXFLoader::SetUnitOfLength(DXF::HEADER::CHeader& hdrMgr)
{
	...
	if (iter != rHeaderMap.end())
	{
		...
		if (nValue == 1)
			unitSrc = DXFViewer::UnitOfLength::INCH;
		else if (nValue == 2)
			unitSrc = DXFViewer::UnitOfLength::FEET;
		else if (nValue == 4)
			unitSrc = DXFViewer::UnitOfLength::MILLIMETER;
		else if (nValue == 5)
			unitSrc = DXFViewer::UnitOfLength::CENTIMETER;
		else if (nValue == 6)
			unitSrc = DXFViewer::UnitOfLength::METER;
	}
...
void DXFLoader::AddLineTypeFromDXF(DXF::TABLES::LType::Entity* pEntity)
{
	...
	/*bool isSolid = strLineType->IndexOf(L"Continuos", System::StringComparison::CurrentCultureIgnoreCase) >= 0 ||
		strLineType->IndexOf(L"Solid", System::StringComparison::CurrentCultureIgnoreCase) >= 0;*/
	...
	double dArr[16];
	int nArr[16];
	int nArrSize = 0, nType, i;

	bool bContinue = pEntity->GetData(pID,&dArr[0],(short*)&nType);

	while (bContinue && nArrSize < 15)
	{
		bContinue = pEntity->GetData(pID,&dArr[++nArrSize],(short*)&nType);
	}

	for (i=0;i<nArrSize;i++)
	{
		if (dArr[i] >= 0) nArr[i] = (int)dArr[i] + 1;
		else nArr[i] = (int)dArr[i];
	}

	int nFactor;
	unsigned short nStyle;
	GetLineType(nArr,nArrSize,&nStyle,&nFactor);

	PenWorld::LineType lType;
	lType.SetTypeName(strLineType);
	lType.SetFactor(nFactor);
	lType.SetStyle(nStyle);
	m_listLineType.push_back(lType);*/
}
...
void DXFLoader::ReadDXFLayer(DXF::HEADER::CHeader* pHdrMgr, DXF::TABLES::TableManager* pTblMgr)
{
	...
	for (;pEntity;pEntity=pLayer->GetEntity(pNod))
	{
		...
		{
			targetLayer = gcnew Layer(m_ctrl);
			targetLayer->LayerName = gcnew System::String(pEntity->GetLayerName());

			/*if (m_dicLayer->Count == 0)
			{
				targetLayer->LayerName = gcnew System::String(L"0");
				pEntity->SetColor(0);
			}*/
...
void DXFLoader::ReadDXFEntity(DXF::ENTITIES::EntityManager* pEntMgr, DXF::BLOCKS::BlockManager* pBlkMgr, DXF::TABLES::TableManager* pTblMgr, COLORREF* pCol, double dScale)
{
	...
	for (;pEntity;pEntity=pEntMgr->GetEntity(pNod))
	{
		...
		m_ctrl->SendReadEntity(ToSystemString(strEntityType), nCount++);
		...
		else if (!wcscmp(strEntityType, L"POINT"))
		{
			obj = (Shape^)GetPointFromDXF((DXF::ENTITIES::Point*)pEntity, dScale, layer);
		}
...
Point^ DXFLoader::GetPointFromDXF(DXF::ENTITIES::Point* pPointSrc, double dScale, Layer^ layer)
{
	double x = pPointSrc->X();
	double y = pPointSrc->Y();
	double z = pPointSrc->Z();

	Point^ point = gcnew Point(x, y);

	// 기본 레이어는 영역 계산에서 제외
	//if (wcscmp(pLineSrc->GetOwnLayer(), L"0"))
	if (!layer->Hidden && !layer->Frozen && wcscmp(pPointSrc->GetOwnLayer(), L"0"))
	{
		if (m_isInitArea)
		{
			if (m_vObjectTL->x > point->Vertex->x) m_vObjectTL->x = point->Vertex->x;
			if (m_vObjectTL->y < point->Vertex->y) m_vObjectTL->y = point->Vertex->y;

			if (m_vObjectBR->x < point->Vertex->x) m_vObjectBR->x = point->Vertex->x;
			if (m_vObjectBR->y > point->Vertex->y) m_vObjectBR->y = point->Vertex->y;
		...
		{
			m_vObjectTL->x = m_vObjectBL->x = m_vObjectBR->x = point->Vertex->x;
			m_vObjectTL->y = m_vObjectBL->y = m_vObjectBR->y = point->Vertex->y;
...
{	
	...
	DXFViewer::PolyLine^ polyLine = gcnew DXFViewer::PolyLine();
...
static double GetArcAngle(Vertex2D^ vCenter, Vertex2D^ vertex)
{
	Vertex2D^ vRight = gcnew Vertex2D(vCenter->x + 1000.0, vCenter->y);
...
static Arc2D^ MakeArc(Vertex2D^ vBegin, Vertex2D^ vEnd, Vertex2D^ vCenter, bool isClockWise)
{
	double dBeginAngle = GetArcAngle(vCenter, vBegin);
	double dEndAngle = GetArcAngle(vCenter, vBegin);
	double dArcAngle = dBeginAngle - dEndAngle;

	if (isClockWise)
	{
		if (dBeginAngle < dEndAngle)
			dArcAngle = UnE::Geometry::Math::_2PI() - (dEndAngle - dBeginAngle);
	}
	else
	{
		if (dBeginAngle > dEndAngle)
			dArcAngle = UnE::Geometry::Math::_2PI() - dArcAngle;
	}

	Arc2D^ arc = gcnew Arc2D(vCenter, vBegin->GetDistance(vCenter), dBeginAngle, dArcAngle, isClockWise);
	return arc;
}
...
static Vertex2D^ _GetPolylineArcCenter(UnE::Geometry::Vertex2D^ v1, UnE::Geometry::Vertex2D^ v2, double bulge, Arc2D^% arc)
{
	UnE::Geometry::Vertex2D^ v3 = (v1 + v2) / 2;

	double dLen = v1->GetDistance(v2);
	double s = dLen / 2 * bulge;

	UnE::Geometry::Vertex2D^ vM1 = UnE::Geometry::Math::GetRightVertex(v3, v1, s);
	UnE::Geometry::Vertex2D^ vM2 = UnE::Geometry::Math::GetRightVertex(v3, v1, -s);

	bool isClockWise = bulge > 0.0 ? false : true;

	UnE::Geometry::Vertex2D^ vCenter;
	arc = gcnew Arc2D(v1, vM1, v2);
	...
	else
	{
		...
		arc = MakeArc(v1, v2, vCenter, isClockWise);
	}
...
// bulge : Arc 매개변수
// isClockWise : Arc의 진행방향이 시계방향인가?
Vertex2D^ DXFLoader::GetPolylineArcCenter(UnE::Geometry::Vertex2D^ v1, UnE::Geometry::Vertex2D^ v2, double bulge, bool% isClockWise)
{
	Arc2D^ arc = nullptr;
	Vertex2D^ vCenter = _GetPolylineArcCenter(v1, v2, bulge, arc);
...
static void PolyLineToHatchBoundary(Hatch^ hatch, DXF::ENTITIES::Hatch::PolyLineType* pPolyLine, double dScale)
{
	if (pPolyLine->m_nPointSize < 2)
		return;

	Vertex2D^ vPrev = gcnew Vertex2D(pPolyLine->m_pArrX[pPolyLine->m_nPointSize - 1], pPolyLine->m_pArrY[pPolyLine->m_nPointSize - 1]);
	Vertex2D^ _vPrev = gcnew Vertex2D(pPolyLine->m_pArrX[pPolyLine->m_nPointSize - 1] * dScale, pPolyLine->m_pArrY[pPolyLine->m_nPointSize - 1] * dScale);
	double dBulge = pPolyLine->m_nHasBulge == 1 ? pPolyLine->m_pArrBulge[pPolyLine->m_nPointSize - 1] : 0.0;

	for (int i = 0; i < pPolyLine->m_nPointSize; i++)
	{
		Vertex2D^ vCurrent = gcnew Vertex2D(pPolyLine->m_pArrX[i], pPolyLine->m_pArrY[i]);
		Vertex2D^ _vCurrent = gcnew Vertex2D(pPolyLine->m_pArrX[i] * dScale, pPolyLine->m_pArrY[i] * dScale);

		if (fabs(dBulge) > (double)UnE::Geometry::Math::HALF_TOLERANCE())
		{
			Arc2D^ arc = nullptr;
			Vertex2D^ vCenter = _GetPolylineArcCenter(vPrev, vCurrent, dBulge, arc);
			
			vCenter->SetVertex(vCenter->x * dScale, vCenter->y * dScale);
			arc = gcnew Arc2D(vCenter, arc->GetRadius() * dScale, arc->GetBeginAngle(), arc->GetAngle(), arc->IsClockWise());
			hatch->AddArc(arc);
		}
		else
			hatch->AddLine(_vPrev, _vCurrent);

		vPrev = vCurrent;
		_vPrev = _vCurrent;
		dBulge = pPolyLine->m_nHasBulge == 1 ? pPolyLine->m_pArrBulge[i] : 0.0;
	}
}

static void ArcToHatchBoundary(Hatch^ hatch, DXF::ENTITIES::Hatch::ArcEdge* pArc, double dScale)
{
	Vertex2D^ vCenter = gcnew Vertex2D(pArc->m_ptCenter.m_pt[0] * dScale, pArc->m_ptCenter.m_pt[1] * dScale);
	double dBeginAngle = UnE::Geometry::Math::DegToRad(pArc->m_dBeginAngle);
	double dArcAngle = pArc->m_dAngle;
	double dRadius = pArc->m_dRadius * dScale;
	bool isClockWise = pArc->m_nDirection == 1 ? false : true;

	Arc2D^ arc = gcnew Arc2D(vCenter, dRadius, dBeginAngle, dArcAngle, isClockWise);
	hatch->AddArc(arc);
}

static void EArcToHatchBoundary(Hatch^ hatch, DXF::ENTITIES::Hatch::EArcEdge* pEArc, double dScale)
{
	if (pEArc->m_dRatio <= UnE::Geometry::Math::HALF_TOLERANCE())
		return;

	Vertex2D^ vCenter = gcnew Vertex2D(pEArc->m_dCenterPoint[0] * dScale, pEArc->m_dCenterPoint[1] * dScale);
	Vertex2D^ vMR = gcnew Vertex2D(pEArc->m_dLongAxisPoint[0] * dScale, pEArc->m_dLongAxisPoint[1] * dScale);
	Vertex2D^ vML = vCenter * 2 - vMR;

	double dLongRadius = vMR->GetDistance(vCenter);

	if (dLongRadius <= UnE::Geometry::Math::HALF_TOLERANCE())
		return;

	double dShortRadius = dLongRadius * pEArc->m_dRatio;
	Vertex2D^ vTL = UnE::Geometry::Math::GetRightVertex(vML, vMR, dShortRadius);
	Vertex2D^ vBL = vML * 2 - vTL;
	Vertex2D^ vBR = vMR - vML + vBL;

	double dBeginAngle = UnE::Geometry::Math::DegToRad(pEArc->m_dBeginAngle);
	bool isClockWise = pEArc->m_nDirection == 1 ? false : true;
	double dEArcAngle = 0.0;

	if (isClockWise)
	{
		if (pEArc->m_dBeginAngle > pEArc->m_dEndAngle)
		{
			dEArcAngle = pEArc->m_dBeginAngle - pEArc->m_dEndAngle;
		}
		else
		{
			dEArcAngle = 360 - pEArc->m_dEndAngle + pEArc->m_dBeginAngle;
		}
	}
	else
	{
		if (pEArc->m_dBeginAngle > pEArc->m_dEndAngle)
		{
			dEArcAngle = 360 - pEArc->m_dBeginAngle + pEArc->m_dEndAngle;
		}
		else
		{
			dEArcAngle = pEArc->m_dEndAngle - pEArc->m_dBeginAngle;
		}
	}

	dEArcAngle = UnE::Geometry::Math::DegToRad(dEArcAngle);

	EArc2D^ earc = gcnew EArc2D(vTL, vBL, vBR, dBeginAngle, dEArcAngle, isClockWise);
	hatch->AddEArc(earc);
}
	...
	while (pBoundaryManager = pHatchSrc->GetBoundaryManager(pID))
	{
		...
		for (; iter != pBoundaryManager->m_list.end(); iter++)
		{
			...
			if (type == DXF::ENTITIES::Hatch::Boundary::POLYLINE)
			{
				...
				PolyLineToHatchBoundary(hatch, pPolyLine, dScale);
			...
			else if (type == DXF::ENTITIES::Hatch::Boundary::ARCEDGE)
			{
				...
				ArcToHatchBoundary(hatch, pArc, dScale);
			...
			else if (type == DXF::ENTITIES::Hatch::Boundary::EARCEDGE)
			{
				...
				EArcToHatchBoundary(hatch, pEArc, dScale);
...
void DXFLoader::GetHatchBoundaryFromLineEdge(Hatch^ hatch, void* _pLine, double dScale)
{
	...
	{
		Utility::Vertex2D& rPrev = arrVertices[i-1];
		Utility::Vertex2D& rVertex = arrVertices[i];
		hatch->AddLine(gcnew UnE::Geometry::Vertex2D(rPrev.m_pt[0], rPrev.m_pt[1]), gcnew UnE::Geometry::Vertex2D(rVertex.m_pt[0], rVertex.m_pt[1]));
	}
...
Text^ DXFLoader::GetTextFromDXF(DXF::ENTITIES::Text* pTextSrc, DXF::TABLES::TableManager* pTblMgr, double dScale, Layer^ layer)
{
	...
	else 
	{
		...
		text->Position = ptPos;*/
		double dCorrectionX = -104.83127519234, dCorrectionY = 50;
		UnE::Geometry::Vertex2D^ vPos = gcnew UnE::Geometry::Vertex2D();
		vPos->x = (dFirstAlignPoint[0] + dCorrectionX) * dScale;
		// 수직정렬 기본값은 baseline이므로 Font 크기만큼 더한다.
		vPos->y = (dFirstAlignPoint[1] + dStrHeight + dCorrectionY) * dScale;
...
bool IsExistBlock(System::String^ strBlockName, System::Collections::ArrayList^ blocks)
{
...
void DXFLoader::GetEntityFromBlockData(DXF::BLOCKS::BlockData* pBlock, const Utility::Vertex3D& rInsertPoint, const wchar_t* strBlockName, DXF::BLOCKS::BlockManager* pBlkMgr, DXF::TABLES::TableManager* pTblMgr, double dScale)
{
	...
	if (IsExistBlock(gcnew System::String(strBlockName), m_ctrl->Blocks))
		return;

	double arrInsert[3];
	pBlock->GetInsertPoint(arrInsert[0], arrInsert[1], arrInsert[2]);
	Block^ block = gcnew Block(m_ctrl);

	block->OriginVertex = gcnew Vertex2D((rInsertPoint.m_pt[0] - arrInsert[0]) * dScale, (rInsertPoint.m_pt[1] - arrInsert[1]) * dScale);
	...
	while (pEntity = pBlock->GetEntity(pID))
	{
		...
		{
			Arc^ arc = GetArcFromDXF((DXF::ENTITIES::Arc*)pEntity, dScale, layer);
			if (System::Math::Abs(arc->ArcAngle) < UnE::Geometry::Math::HALF_TOLERANCE())
				continue;
		...
		{
			Point^ point = GetPointFromDXF((DXF::ENTITIES::Point*)pEntity, dScale, layer);
			shape = (Shape^)point;
		}
		else if (!wcscmp(strEntityType, L"LWPOLYLINE"))
		{
		...
		if (shape != nullptr)
		{
			SetObjectColor(shape, layer, pEntity, 0);
			shape->Move(block->OriginVertex->x, block->OriginVertex->y);
			m_dicShape[(__int64)pEntity] = shape;
		}
		else
			continue;
