...
bool UBMLWriter::WriteFileA(const std::vector<Element*>& rVecData, const char* strFilePath)
{
	if (strFilePath == 0 || m_pDoc == NULL)
		return false;

	if(WriteXML(rVecData, m_pDoc) == false)
		return false;
...
bool UBMLWriter::WriteFile(const std::vector<Element*>& rVecData, const wchar_t* strFilePath)
{
	if (strFilePath == 0 || m_pDoc == NULL)
		return false;
...
void UBMLWriter::IntToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(int*)pData);
}

void UBMLWriter::UIntToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(unsigned int*)pData);
}

void UBMLWriter::ShortToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(short*)pData);
}

void UBMLWriter::UShortToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(unsigned short*)pData);
}

void UBMLWriter::ByteToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, (int)*(char*)pData);
}

void UBMLWriter::UByteToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, (int)*(unsigned char*)pData);
}

void UBMLWriter::LONGLONGToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(__int64*)pData);
}

void UBMLWriter::ULONGLONGToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(unsigned int*)pData);
}

void UBMLWriter::FloatToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(float*)pData);
}

void UBMLWriter::DoubleToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	swprintf(str, nBufSize, strFormat, *(double*)pData);
}

void UBMLWriter::BoolToString(wchar_t* str, int nBufSize, const void* pData, const wchar_t* strFormat)
{
	if( *(bool*)pData == true)
		swprintf(str, nBufSize, strFormat, L"true");
	else
		swprintf(str, nBufSize, strFormat, L"false");
}

// isString : 0이면 문자열이 아님. 1이면 char*, 2이면 wchar_t*
ToStringFunc UBMLWriter::ToStringFunction(int nDataType, wchar_t* strFormat, int& isString)
{
	isString = 0;
...
bool UBMLWriter::WriteFile(const std::vector<Element*>& rVecElement, Document * pDoc, const wchar_t* strRootElementName)
{
...
bool UBMLWriter::WriteElement(Poco::XML::Document * pDoc, Poco::XML::Element *pNode, const Element* pElement)
{
	char buf[512];	
	char str[512];
...
bool UBMLWriter::WriteSegment(Poco::XML::Document * pDoc, Poco::XML::Element *pNode, const Segment* pSegment)
{
	char buf[512];
...
const std::wstring& UBMLWriter::GetErrorMessage() const
{
	return m_strErrorMessage;
}

bool UBMLWriter::WriteXML( const std::vector<Element*>& rVecElement, Document * pDoc , const wchar_t* strRootElementName /*= 0*/ )
{
	unsigned int nElementCount = (unsigned int)rVecElement.size();
	if (nElementCount == 0)
		return true;
	...
	for (unsigned int i=0;i < nElementCount;i++)
	{
		...
		for (unsigned int j=0;j<nDataCount;j++)
		{
			...
			if (pData->GetClassType() == UData::ELEMENT)
			{
				if (!AddElement(pDoc, pEleNode, (const Element*)pData))
					return false;
			}
			else
			{
				if (!AddSegment(pDoc, pEleNode, (const Segment*)pData))
					return false;
			}	
		}
	}
...
bool UBMLWriter::AddElement( Document * pDoc, Poco::XML::Element *pNode, const Element* pElement )
{
	char buf[512];	
	char str[512];
...
bool UBMLWriter::AddSegment( Document * pDoc, Poco::XML::Element *pNode, const Segment* pSegment )
{
	char buf[512];
	unsigned int nSegDataCount = pSegment->GetDataCount();
	if (nSegDataCount == 0)
	...
	if (isArray)
	{
		...
		if( pSegment->GetTagDescription() == L"Text" || pSegment->GetTagDescription() == L"")
		{
			Poco::XML::Text* pText = pDoc->createTextNode(std::string(buf));
		...
		else
		{
			// ADD DESCRIPTION ATTR
			char buf2[512];
			const std::wstring& strTag = pSegment->GetTagName();
			UnicodeToUTF8(buf2, strTag.c_str());
			pNode->setAttribute(std::string(buf2), std::string(buf));	
		}
