...
namespace UnE
{
	namespace Core
	{
		std::string Frustum::msMovableType = "Frustum";
		const Real Frustum::INFINITE_FAR_PLANE_ADJUST = 0.00001f;
		//-----------------------------------------------------------------------
		Frustum::Frustum(const std::string& name) : 
		...
		{
			if (nearPlane <= 0)
				throw std::exception("Near clip distance must be greater than zero. Frustum::setNearClipDistance");
			mNearDist = nearPlane;
			invalidateFrustum();
		}
		...
		void Frustum::setFocalLength(Real focalLength)
		{
			if (focalLength <= 0)
			{
				throw std::exception("Focal length must be greater than zero. Frustum::setFocalLength");
			}
		...
		bool Frustum::isVisible(const AxisAlignedBox& bound, FrustumPlane* culledBy) const
		{
			...
			for (int plane = 0; plane < 6; ++plane)
			{
				...
				Side side = mFrustumPlanes[plane].getSide(centre, halfSize);
				if (side == Side::NEGATIVE_SIDE)
				{
		...
		bool Frustum::isVisible(const Vector3& vert, FrustumPlane* culledBy) const
		{
			...
			for (int plane = 0; plane < 6; ++plane)
			{
				...
				if (mFrustumPlanes[plane].getSide(vert) == Side::NEGATIVE_SIDE)
				{
					// ALL corners on negative side therefore out of view
					if (culledBy)
						*culledBy = (FrustumPlane)plane;
		...
		{ 
			...
			else
			{
				...
				else if (mProjType == PT_PERSPECTIVE)
				{
					...
					Real tanThetaY = UMath::Tan(thetaY);
			...
			if (!mCustomProjMatrix)
			{
				...
				if (mProjType == PT_PERSPECTIVE)
				{
					...
					if (mObliqueDepthProjection)
					{
						...
						*/
						Vector4 qVec;
						qVec.x = (UMath::Sign(plane.normal.x) + mProjMatrix[0][2]) / mProjMatrix[0][0];
						qVec.y = (UMath::Sign(plane.normal.y) + mProjMatrix[1][2]) / mProjMatrix[1][1];
						qVec.z = -1;
		...
		void Frustum::updateViewImpl(void) const
		{
			...
			if (!mCustomViewMatrix)
			{
				...
				if( mReflect )
					mViewMatrix = UMath::makeViewMatrix(position, orientation, mReflectMatrix );
				else
					mViewMatrix = UMath::makeViewMatrix(position, orientation );
			}
		...
		//}
		//-----------------------------------------------------------------------
		const std::string& Frustum::getMovableType(void) const
		{
			...
			Real* left, Real* top, Real* right, Real* bottom) const
		{
			...
			*right = *top = 1.0f;
			...
			if (eyeSpacePos.z < 0)
			{
				...
				Real Lxz = UMath::Sqr(eyeSpacePos.x) + UMath::Sqr(eyeSpacePos.z);
				Real Lyz = UMath::Sqr(eyeSpacePos.y) + UMath::Sqr(eyeSpacePos.z);
				...
				Real c = rsq - UMath::Sqr(eyeSpacePos.z);
				Real D = b*b - 4.0f*a*c;

				// two roots?
				if (D > 0)
				{
					Real sqrootD = UMath::Sqrt(D);
