#ifndef __UNE_CORE_ASSIMPLOADER_H_INCLUDED__
#define __UNE_CORE_ASSIMPLOADER_H_INCLUDED__
...
namespace UnE
{
	namespace Core
	{
		...
		//-----------------------------------------------------------------------
		class UAssimpLoader : public UFileLoader
		{
		public:
			enum LoaderParams
			{
				...
				LP_REVERSE_FACE_INDEX = 1 <<4,
			};

			UAssimpLoader();
			virtual ~UAssimpLoader();

			// customAnimationName is only applied if the skeleton only has one animation
			bool convert(HWND hWnd , const Ogre::String& filename, 
				const Ogre::String& customAnimationName = "",
				int loaderParams = (LP_REVERSE_FACE_INDEX | LP_GENERATE_MATERIALS_AS_CODE) );

			Ogre::AxisAlignedBox GetBound() const { return mModelBound; }


			const Ogre::String& getBasename(){ return mBasename; }


			Ogre::String getExtensionList();
		...
		private:
			bool CheckMeshResouces(const aiNode* pNode, unsigned long& id);

			bool createSubMesh(const Ogre::String& name, int index, const aiNode* pNode, const aiMesh *mesh, const aiMaterial* mat, Ogre::MeshPtr meshp, Ogre::AxisAlignedBox& mAAB, const Ogre::String& mDir);
			...
			void loadDataFromNode(const aiScene* mScene,  const aiNode *pNode, const Ogre::String& mDir, bool bCreateEntity = false);
			void loadDataFromNode(const aiScene* mScene, const aiNode *pNode, const Ogre::String& mDir, Ogre::SceneNode* parentNode, bool bCreateEntity = false);
			...
			Ogre::AxisAlignedBox mModelBound;
...
#endif//__UNE_CORE_ASSIMPLOADER_H_INCLUDED__
