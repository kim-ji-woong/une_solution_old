...
Public Class posnegbar
    ...
    Public Sub createChart(viewer As WinChartViewer, img As String) _
        ...
        Dim data() As Double = {-6.3, 2.3, 0.7, -3.4, 2.2, -2.9, -0.1, -0.1, 3.3, _
            6.2, 4.3, 1.6}
        ...
        layer.addDataSet(New ArrayMath(data).selectGEZ(Nothing, Chart.NoValue _
            ).result(), &H6666ff)

        ' Select negative data and add it as data set with orange (ff6600) color
        layer.addDataSet(New ArrayMath(data).selectLTZ(Nothing, Chart.NoValue _
            ).result(), &Hff6600)

        ' Add labels to the top of the bar using 8 pt Arial Bold font. The font color
        ' is configured to be red (0xcc3300) below zero, and blue (0x3333ff) above
        ' zero.
        layer.setAggregateLabelStyle("Arial Bold", 8, layer.yZoneColor(0, &Hcc3300, _
            &H3333ff))
