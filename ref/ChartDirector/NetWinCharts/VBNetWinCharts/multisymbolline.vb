...
Public Class multisymbolline
    ...
    Public Sub createChart(viewer As WinChartViewer, img As String) _
        ...
        Dim dataY() As Double = {4.7, 4.7, 6.6, 2.2, 4.7, 4.0, 4.0, 5.1, 4.5, 4.5, _
            6.8, 4.5, 4, 2.1, 3, 2.5, 2.5, 3.1}
        Dim dataX() As Date = {DateSerial(1999, 7, 1), DateSerial(2000, 1, 1), _
            DateSerial(2000, 2, 1), DateSerial(2000, 4, 1), DateSerial(2000, 5, 8), _
            DateSerial(2000, 7, 5), DateSerial(2001, 3, 5), DateSerial(2001, 4, 7), _
            DateSerial(2001, 5, 9), DateSerial(2002, 2, 4), DateSerial(2002, 4, 4), _
            DateSerial(2002, 5, 8), DateSerial(2002, 7, 7), DateSerial(2002, 8, 30 _
            ), DateSerial(2003, 1, 2), DateSerial(2003, 2, 16), DateSerial(2003, _
            11, 6), DateSerial(2004, 1, 4)}

        ' Data points are assigned different symbols based on point type
        Dim pointType() As Double = {0, 1, 0, 1, 2, 1, 0, 0, 1, 1, 2, 2, 1, 0, 2, _
            1, 2, 0}
        ...
        c.setBackground(c.linearGradientColor(0, 0, 0, c.getHeight() / 2, &He8f0f8, _
            &Haaccff), &H88aaee)
        c.setRoundedFrame()
        c.setDropShadow()
        ...
        Dim title As ChartDirector.TextBox = c.addTitle( _
            "Multi-Symbol Line Chart Demo", "Arial Italic", 15)
        ...
        ' than the chart width, and the height to reserve 90 pixels at the bottom for
        ' the x-axis and the legend box. Use pale blue (e8f0f8) background,
        ' transparent border, and grey (888888) dotted horizontal and vertical grid
        ' lines.
        c.setPlotArea(50, title.getHeight(), c.getWidth() - 65, c.getHeight() - _
            title.getHeight() - 90, &He8f0f8, -1, Chart.Transparent, _
            c.dashLineColor(&H888888, Chart.DotLine), -1)

        ' Add a legend box where the bottom-center is anchored to the 12 pixels above
        ' the bottom-center of the chart. Use horizontal layout and 8 points Arial
        ' font.
        Dim legendBox As LegendBox = c.addLegend(c.getWidth() / 2, c.getHeight() - _
            12, False, "Arial Bold", 8)
        ...
        ' dataX.)
        c.addScatterLayer(Chart.CTime(dataX), New ArrayMath(dataY).selectEQZ( _
            pointType, Chart.NoValue).result(), "Point Type 0", Chart.StarShape(5), _
            15, &Hffff00)

        ' Similar to above, we select the points with pointType - 1 = 0 and use green
        ' (ff00) 13 pixels high six-sided polygon as symbols.
        c.addScatterLayer(Chart.CTime(dataX), New ArrayMath(dataY).selectEQZ( _
            New ArrayMath(pointType).sub(1).result(), Chart.NoValue).result(), _
            "Point Type 1", Chart.PolygonShape(6), 13, &H00ff00)

        ' Similar to above, we select the points with pointType - 2 = 0 and use red
        ' (ff0000) 13 pixels high X shape as symbols.
        c.addScatterLayer(Chart.CTime(dataX), New ArrayMath(dataY).selectEQZ( _
            New ArrayMath(pointType).sub(2).result(), Chart.NoValue).result(), _
            "Point Type 2", Chart.Cross2Shape(), 13, &Hff0000)
        ...
        c.packPlotArea(10, title.getHeight(), c.getWidth() - 25, c.layoutLegend( _
            ).getTopY() - 8)
