...
Public Class errline
    ...
    Public Sub createChart(viewer As WinChartViewer, img As String) _
        ...
        Dim data() As Double = {42, 49, 33, 38, 51, 46, 29, 41, 44, 57, 59, 52, 37, _
            34, 51, 56, 56, 60, 70, 76, 63, 67, 75, 64, 51}
        Dim errData() As Double = {5, 6, 5.1, 6.5, 6.6, 8, 5.4, 5.1, 4.6, 5.0, 5.2, _
            6.0, 4.9, 5.6, 4.8, 6.2, 7.4, 7.1, 6.0, 6.6, 7.1, 5.3, 5.5, 7.9, 6.1}
        ...
        ' is in CDML and includes embedded images for highlight. The text is on a
        ' light grey (dddddd) background, with glass lighting effect.
        c.addTitle( _
            "<*block,valign=absmiddle*><*img=star.png*><*img=star.png*> " & _
            "Molecular Temperature Control <*img=star.png*><*img=star.png*><*/*>", _
            "Times New Roman Bold Italic", 15).setBackground(&Hdddddd, 0, _
            Chart.glassEffect())
        ...
        ' diamond symbols
        lineLayer.addDataSet(data, &H0000ff).setDataSymbol(Chart.DiamondSymbol, 12, _
            &Hffff80)
        ...
        Dim errLayer As BoxWhiskerLayer = c.addBoxWhiskerLayer(Nothing, Nothing, _
            New ArrayMath(data).add(errData).result(), New ArrayMath(data).sub( _
            errData).result(), data, Chart.Transparent, &Hbb6633)
