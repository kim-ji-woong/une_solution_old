...
namespace CSharpChartExplorer
{
    public class posnegwaterfall : DemoModule
    {
        ...
        public void createChart(WinChartViewer viewer, string img)
        {
            ...
            // We want to plot a waterfall chart showing the 4 quarters as well as
            // the total
            string[] labels = {"1st Quarter", "2nd Quarter", "3rd Quarter",
                "4th Quarter", "Total"};

            // The top side of the bars in a waterfall chart is the accumulated data.
            // We use the ChartDirector ArrayMath utility to accumulate the data. The
            // "total" is handled by inserting a zero point at the end before
            // accumulation (after accumulation it will become the total).
            double[] boxTop = new ArrayMath(data).insert2(0, 1, data.Length).acc(
                ).result();

            // The botom side of the bars is just the top side of the previous bar.
            // So we shifted the top side data to obtain the bottom side data.
            double[] boxBottom = new ArrayMath(boxTop).shift(1, 0).result();

            // The last point (total) is different. Its bottom side is always 0.
            boxBottom[boxBottom.Length - 1] = 0;

            // In this example, we want to use different colors depending on the data
            // is positive or negative.
            int posColor = 0x00ff00;
            int negColor = 0xff0000;

            // Create a XYChart object of size 500 x 280 pixels. Set background color
            ...
            c.addTitle("Corporate Cash Flow - Year 2004", "Arial Bold Italic", 13,
                0xffffff).setBackground(0x000080);

            // Set the plotarea at (55, 50) and of size 430 x 215 pixels. Use
            // alternative white/grey background.
            c.setPlotArea(55, 50, 430, 215, 0xffffff, 0xeeeeee);

            // Add a legend box at (55, 25) using 8 pts Arial Bold font with
            // horizontal layout, with transparent background and border color.
            ...
            // Put data labels on the bars to show the cash flow using Arial Bold
            // font
