...
namespace CSharpChartExplorer
{
    public class posnegbar : DemoModule
    {
        ...
        public void createChart(WinChartViewer viewer, string img)
        {
            ...
            double[] data = {-6.3, 2.3, 0.7, -3.4, 2.2, -2.9, -0.1, -0.1, 3.3, 6.2,
                4.3, 1.6};
            ...
            layer.addDataSet(new ArrayMath(data).selectGEZ(null, Chart.NoValue
                ).result(), 0x6666ff);

            // Select negative data and add it as data set with orange (ff6600) color
            layer.addDataSet(new ArrayMath(data).selectLTZ(null, Chart.NoValue
                ).result(), 0xff6600);

            // Add labels to the top of the bar using 8 pt Arial Bold font. The font
            // color is configured to be red (0xcc3300) below zero, and blue
            // (0x3333ff) above zero.
            layer.setAggregateLabelStyle("Arial Bold", 8, layer.yZoneColor(0,
                0xcc3300, 0x3333ff));
