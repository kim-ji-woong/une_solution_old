...
{
	gp_Pnt ptBL(0, 0, 0);
	gp_Pnt ptTL(0, nCubeLen, 0);
	gp_Pnt ptTR(nCubeLen, nCubeLen, 0);
	gp_Pnt ptBR(nCubeLen, 0, 0);

	Handle(Geom_TrimmedCurve) segLeft   = GC_MakeSegment(ptTL , ptBL);
	Handle(Geom_TrimmedCurve) segTop    = GC_MakeSegment(ptTL , ptTR);
	Handle(Geom_TrimmedCurve) segRight  = GC_MakeSegment(ptTR , ptBR);
	Handle(Geom_TrimmedCurve) segBottom = GC_MakeSegment(ptBL , ptBR);

	TopoDS_Edge edgeLeft   = BRepBuilderAPI_MakeEdge(segLeft);
	TopoDS_Edge edgeTop    = BRepBuilderAPI_MakeEdge(segTop);
	TopoDS_Edge edgeRight  = BRepBuilderAPI_MakeEdge(segRight);
	TopoDS_Edge edgeBottom = BRepBuilderAPI_MakeEdge(segBottom);

	TopoDS_Wire wire = BRepBuilderAPI_MakeWire(edgeLeft , edgeTop , edgeRight, edgeBottom);

	//Body : Prism the Profile
	TopoDS_Face faceProfile = BRepBuilderAPI_MakeFace(wire);
	gp_Vec prismVec(0, 0, nCubeLen);

	TopoDS_Shape body = BRepPrimAPI_MakePrism(faceProfile , prismVec);

	TopoDS_Compound cube;
	BRep_Builder builder;
	builder.MakeCompound(cube);

	builder.Add(cube, body);
	return cube;
}
