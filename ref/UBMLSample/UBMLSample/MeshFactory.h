...
public:
	...
	double CurrentElapsedTimeSec() const
	{
		LONGLONG stopCount = Counter();
		...
		double dElapsedTimeSec = (stopCount - m_startCount) * 1.0 / Frequency();
		return dElapsedTimeSec;
	}
...
public:	// Read
	bool Read(std::wstring strPath, UnE::UBML::Reader& rReader, FILE* fp, CStopwatch& watch);
...
protected:	// Read
	...
	// Body
	bool ReadBody(UnE::UBML::Element& rElement, FILE* fp, CStopwatch& watch);
