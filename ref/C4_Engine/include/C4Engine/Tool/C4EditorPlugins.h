...
namespace C4
{
	...
	//
	//# \privbase	PluginMgr/Plugin						An $EditorPlugin$ is a specific type of plugin.
	...
	class SceneImportPlugin : public EditorPlugin, public ListElement<SceneImportPlugin>
	{
		...
		public:
			...
			virtual void ImportGeometry(Editor *editor, const char *name, const GeometryImportData *importData) = 0;
			virtual bool ImportAnimation(const char *name, Model *model, const AnimationImportData *importData) = 0;
	};
	...
	class SceneImportPicker : public FilePicker
	{
		...
		public:
			...
			C4EDITORAPI SceneImportPicker(const char *title, SceneImportType type, const char *subdir = nullptr, const char *panel = nullptr);
			...
			C4EDITORAPI void HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData);
	};
