...
namespace C4
{
	...
	enum
	{
		#if C4WINDOWS || C4MACOS || C4LINUX
		
			kOutputBufferFrameCount		= 1024,
		
		#else
		
			kOutputBufferFrameCount		= 4096,
	...
	enum
	{
		kSoundFadeActive			= 1 << 0,
		kSoundFadeEndStop			= 1 << 1, 
		kSoundFadeEnding			= 1 << 2
	};
	...
	void Reverse(WaveHeader *header);
	void Reverse(PredictorCoefficient *coefficient);
	void Reverse(ADPCMWaveHeader *header);
	...
	class C4_API SoundResource : public Resource<SoundResource>
	{
		...
		public:
			...
			SoundResource(const char *name, ResourceCatalog *catalog);
			...
			ResourceResult LoadWaveHeader(ResourceLoader *loader, WaveHeader *waveHeader) const;
			
			static bool DetermineStreaming(const char *name);
	};
	
	
	class C4_API SoundLoader : public ResourceLoader, public ListElement<SoundLoader>
	{
		private:
	...
	//# \function	SoundStreamer::GetTotalFrameCount	Called to retrieve the total frame count for a stream.
	//
	//# \proto	virtual int32 GetTotalFrameCount(void);
	//
	//#	\desc
	//# The Sound Manager calls the $GetTotalFrameCount$ function when it needs to know the total number of audio frames
	//# contained in a stream. By default, this function returns zero, which means that the length of the stream is
	//# indefinite. Subclasses of the $SoundStreamer$ class should implement an override of the $GetTotalFrameCount$
	...
	class C4_API SoundStreamer
	{
		...
		protected:
			...
			void AllocateStreamMemory(unsigned_int32 workSize, unsigned_int32 streamSize);
	...
	class C4_API WaveStreamer : public SoundStreamer
	{
		private:
		...
		public:
			...
			int32 GetTotalFrameCount(void);
			
			SoundResult StartStream(void);
			SoundResult StartStreamComponent(int32 index);
	...
	//# in addition to each sound's internal volume and the Sound Manager's master volume.
	//# 
	...
	//# \function	SoundGroup::SetVolume		Returns the volume of a sound group.
	//
	...
	class C4_API SoundGroup : public MapElement<SoundGroup>, public LinkTarget<SoundGroup>
	{
		private:
	...
	class C4_API SoundRoom : public LinkTarget<SoundRoom>, public ListElement<SoundRoom>
	{
		...
		public:
			...
			void SetRoomPosition(const Point3D& position);
	};
	...
	//#
	//# A sound is made persistent when the $Stream$ function is called, meaning that it
	//# must be explicitly destroyed when it is no longer in use.
	//
	//# \also	$@Sound::Load@$
	//# \also	$@SoundStreamer@$
	//# \also	$@WaveStreamer@$
	...
	//# stopped, playing, paused, etc. The code returned can be one of the following values.
	//
	//# \table	SoundState
	//
	...
	//# \div
	//# \function	Sound::Fade		Fades a sound to a specific volume.
	//
	//# \proto	void Fade(float targetVolume, int32 fadeTime, bool endStop = false);
	//
	//# \param	targetVolume	The volume to which the sound should be faded.
	//# \param	fadeTime		The time interval, in milliseconds, over which the fade should occur.
	//# \param	endStop			Indicates whether the sound should be stopped at the end of the fade.
	//
	//#	\desc
	//# The $Fade$ function fades the sound to the volume given by the $targetVolume$ parameter over the
	//# time interval given by the $fadeTime$ parameter. At the end of the fade period, the sound's fade
	//# procedure is invoked if one has been installed using the $@Sound::SetFadeProc@$ function.
	//# 
	//# If the $endStop$ parameter is $true$, then the sound is stopped at the end of the fade period.
	//# If the sound is nonpersistent, then it is released at this point.
	//# 
	//# It can be determined whether a sound is currently fading by calling the $@Sound::Fading@$ function.
	//
	//# \also		$@Sound::Fading@$
	//# \also		$@Sound::SetFadeProc@$
	
	
	//# \function	Sound::Fading		Returns a boolean value indicating whether a sound is fading.
	//
	//# \proto	bool Fading(void) const;
	//
	//#	\desc
	//# The $Fading$ functions returns $true$ is the sound is currently fading; otherwise it returns $false$.
	//# If the sound is not playing, then the $Fading$ function always returns $false$.
	//
	//# \also		$@Sound::Fade@$
	//# \also		$@Sound::SetFadeProc@$
	
	
	//# \function	Sound::SetFadeProc		Installs a callback procedure which is invoked when a fade has ended for a sound.
	//
	//# \proto	void SetFadeProc(FadeProc *proc, void *cookie = nullptr);
	//
	//# \param	proc	The fade callback procedure. This parameter may be $nullptr$, in which case
	//#					the sound has no fade callback procedure.
	//# \param	cookie	The cookie that is passed to the fade callback procedure as its last parameter.
	//
	//#	\desc
	//# The $SetFaceProc$ function installs a callback procedure which is invoked every time that a
	//# fade operation completes for this particular sound. The $FadeProc$ type is defined as
	//
	//# \code	typedef void FadeProc(Sound *, void *);
	//	
	//# When the function pointed to by the $proc$ parameter is called, the pointer to the sound object
	//# and the value supplied in the $cookie$ parameter are passed to it. By default, there is no fade
	//# callback procedure installed.
	//# 
	//# If the $proc$ parameter is set to $nullptr$, then no callback is made when fade operations
	//# complete for this sound.
	//
	//# \also		$@Sound::Fade@$
	//# \also		$@Sound::Fading@$
	...
	class C4_API Sound : public Completable<Sound>, public ListElement<Sound>, public Memory<Sound>
	{
		friend class SoundMgr;
		...
		private:
			...
			volatile bool				mixRelease;
			volatile bool				streamRelease;
		...
		public:
			...
			{
				return ((fadeFlags & kSoundFadeActive) != 0);
			}
			
			void SetFadeProc(FadeProc *proc, void *cookie = nullptr)
			{
				fadeProc = proc;
				fadeCookie = cookie;
			}
			...
			void Pause(void);
			void Resume(void);
			void Fade(float targetVolume, int32 fadeTime, bool endStop = false);
			
			int32 GetDuration(void) const;
	};
	...
	//# be played. Any of these sounds which are nonpersistent are released.
	//
	...
	class C4_API SoundMgr : public Manager<SoundMgr>
	{
		friend class Sound;
		friend class SoundGroup;
		...
		private:
			...
			void MixRoomEffects(const SoundRoom *soundRoom, RoomMixBuffer *roomMixBuffer, int32 outputOffset);
			
			void AllocateListenerRoomMixBuffer(SoundRoom *soundRoom);
			int32 AllocateSoundRoomMixBuffer(SoundRoom *soundRoom);
			
			void MixSounds(OutputSample *output);
		...
		public:
			...
			void SetListenerRoom(SoundRoom *room);
			
			void SoundTask(void);
	};
	
	
	C4_API extern SoundMgr *TheSoundMgr;
}
