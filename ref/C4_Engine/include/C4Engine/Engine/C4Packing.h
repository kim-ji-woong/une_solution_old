...
namespace C4
{
	...
	//# \param	size	The size of the chunk's data, in bytes. This size does not include the chunk header.
	//
	//# \data	ChunkHeader
	...
	C4_API void Reverse(ChunkHeader *ch);
	
	
	class C4_API ObjectLink : public ListElement<ObjectLink>, public Memory<ObjectLink>
	{
		public:
	...
	class NodeLink : public ListElement<NodeLink>, public Memory<ObjectLink>
	{
		public:
	...
	class C4_API Package
	{
		...
		public:
			...
			Package(void *storage, unsigned_int32 size = 0);
			~Package();
			...
			void *Reserve(unsigned_int32 size);
	};
	...
	//#				Writes the data given by the $data$ parameter to the output stream.
	//
	...
	//# with the $<<$ operator. Instead, such values should first be cast to a 32-bit type. Note that the
	//# $bool$ type is given special handling by the $<<$ operator so that it always occupies four bytes.
	//
	//# \also	$@Unpacker@$
	//# \also	$@Packable@$
	
	
	//# \function	Packer::WriteArray		Writes an array of data to the output stream.
	//
	//# \proto	template <typename type> void WriteArray(int32 count, const type *array);
	//
	//# \desc
	//# The $WriteArray$ function writes $count$ items of the type given by the $type$ template parameter
	...
	class C4_API Packer
	{
		...
		public:
			...
			Packer& operator <<(const char *string);
			
			PackHandle BeginSection(void);
			void EndSection(PackHandle handle);
			
			PackHandle BeginChunk(unsigned_int32 type);
	...
	//# \def	class Unpacker
	//
	//# \ctor	Unpacker(const void *ptr, int32 end = 1, int32 ver = 0);
	//
	//# \param	ptr		A pointer to a memory block containing the packed data.
	//# \param	end		The endianness of the packed data. A value of 0x00000001 corresponds to the endianness
	//#					of the local machine, and a value of 0x01000000 corresponds to the opposite endianness.
	//# \param	ver		The storage format version of the packed data. The constant $kWorldVersion$ corresponds to the current version.
	//
	//# \desc
	...
	//# \function	Unpacker::ReadArray		Reads an array of data from the input stream.
	//
	//# \proto	template <typename type> void ReadArray(int32 count, type *array);
	//
	//# \desc
	//# The $ReadArray$ function reads $count$ items of the type given by the $type$ template parameter
	...
	class C4_API Unpacker
	{
		...
		public:
			...
			Unpacker(const void *ptr, int32 end = 1, int32 ver = 0);
			~Unpacker();
			...
			void Skip(unsigned_int32 size)
			{
				pointer = static_cast<const char *>(mark) + size;
			}
			...
			{
				Type type = *static_cast<const Type *>(pointer);
				if (endian != 1) Reverse(&type);
				return (type);
			}
			...
			template <typename type> void ReadArray(int32 count, type *array)
			{
				...
				pointer = static_cast<const char *>(pointer) + size;
				if (endian != 1) for (machine a = 0; a < count; a++) Reverse(&array[a]);
			}
	...
	//
	//# \code	bool UnpackChunk(const ChunkHeader *chunkHeader, Unpacker& data, unsigned_int32 unpackFlags);
	//
	...
	class C4_API Packable
	{
		...
		public:
			...
			virtual void Pack(Packer& data, unsigned_int32 packFlags) const;
			virtual void Unpack(Unpacker& data, unsigned_int32 unpackFlags);
			virtual void *BeginSettingsUnpack(void);
			virtual void EndSettingsUnpack(void *cookie);
	};
	...
	template <class type> inline void Packable::UnpackChunkList(Unpacker& data, unsigned_int32 unpackFlags)
	{
		...
		for (;;)
		{
			...
			data.SetMark();
			if (!packable->type::UnpackChunk(&chunkHeader, data, unpackFlags)) data.Skip(chunkHeader.chunkSize);
		}
	}
	
	
	C4_API extern ChunkHeader TerminatorChunk;
}
