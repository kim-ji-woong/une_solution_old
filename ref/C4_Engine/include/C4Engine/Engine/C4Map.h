...
namespace C4
{
	...
	//# class template using the same template parameters as the $Map$ container. A particular object can be a
	//# member of only one map at a time.
	//# 
	...
	//# If the object is a member of a different map, then it is first removed from that map before
	//# being added to the new map.
	//# 
	...
	//#
	//# If the object being inserted is a member of a different map, then it is first removed from that
	//# map before being added to the new map. If the object is already a member of the map for which
	...
	template <class type> class Map : public MapBase
	{
		public:
			...
			void Insert(type *element, const MapReservation *reservation);
	...
	template <class type> bool Map<type>::Insert(type *element)
	{
		type *node = GetRootNode();
		if (node)
		{
			...
			for (;;)
			{
				...
				if (key < nodeKey)
				{
					type *subnode = node->GetLeftSubnode();
					if (!subnode)
					{
				...
				{
					type *subnode = node->GetRightSubnode();
					if (!subnode)
					{
	...
	template <class type> type *Map<type>::InsertReplace(type *element)
	{
		type *node = GetRootNode();
		if (node)
		{
	...
	template <class type> void Map<type>::Insert(type *element, const MapReservation *reservation)
	{
		MapElementBase *node = reservation->superNode;
		if (node)
		{
			if (reservation->direction < 0) InsertLeftSubnode(node, element);
