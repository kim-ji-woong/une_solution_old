...
namespace C4
{
	...
	class C4_API ForceRegistration : public Registration<Force, ForceRegistration>
	{
		private:
	...
	template <class classType> class ForceReg : public ForceRegistration
	{
		public:
			...
			Force *Construct(void) const
			{
				return (new classType);
			}
			
			bool ValidField(const Field *field) const
			{
	...
	//# \function	Force::ApplyForce		Calculates the force to exert on a rigid body.
	//
	//# \proto	bool ApplyForce(RigidBodyController *rigidBody, const Transform4D& worldTransform, Vector3D *force, Vector3D *torque);
	//
	...
	class C4_API Force : public ListElement<Force>, public Packable, public Configurable, public Registrable<Force, ForceRegistration>
	{
		friend class Field;
		...
		public:
			...
			static Force *New(ForceType type);
			...
			virtual bool ApplyForce(RigidBodyController *rigidBody, const Transform4D& worldTransform, Vector3D *force, Vector3D *torque);
	};
	...
	class C4_API GravityForce : public Force
	{
		private:
		...
		public:
			...
			void GetGravityAcceleration(float acceleration)
			{
			...
			bool ApplyForce(RigidBodyController *rigidBody, const Transform4D& worldTransform, Vector3D *force, Vector3D *torque);
	};
	...
	class C4_API FluidForce : public Force
	{
		private:
	...
	class C4_API WindForce : public Force
	{
		private:
