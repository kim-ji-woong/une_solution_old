...
namespace C4
{
	...
	//# \function	Text::GetTextHash		Returns the hash value for a character string.
	//
	//# \proto	unsigned_int32 GetTextHash(const char *text);
	//
	//# \param	text	A pointer to a null-terminated character string.
	//
	//# \desc
	//# The $GetTextHash$ function returns a 32-bit hash value for the character string specified by the $text$ parameter.
	...
	//
	//# \note
	//# The $GetTextHash$ function is <i>not</i> intended for cryptographic applications and does not produce a secure hash value.
	...
	//#
	//# If the character given by $k$ is found, the the byte position within the string of the first occurrence is returned.
	...
	//#
	//# If the character given by $k$ is found outside of double quotes, the the byte position within the string of the first occurrence is returned. 
	//# If it is not found, then the return value is &minus;1.
	//
	//# \also	$@Text::FindChar@$
	...
	//# \function	Text::GetWhitespaceLength		Returns the number of whitespace characters at the beginning of a character string.
	//
	...
	//# the character string specified by the $text$ parameter.
	//# 
	//# Any characters having an ASCII value between 1 and 32, inclusive, is considered whitespace by this
	//# function. Also, if this function encounters two consecutive forward slashes, then it treats all of
	//# the characters between the slashes and the end of the line on which they appear as part of a comment,
	//# which is considered whitespace.
	//
	//# \also	$@Text::ReadIdentifier@$
	//# \also	$@Text::ReadString@$
	
	
	//# \function	Text::ReadIdentifier		Reads an identifier from a character string.
	//
	//# \proto	int32 ReadIdentifier(const char *text, char *identifier, int32 max);
	//
	//# \param	text		A pointer to a character string.
	//# \param	identifier	A pointer to a buffer that will receive the identifier string.
	//# \param	max			The maximum length of a string that can be stored in the buffer specified by the $identifier$ parameter.
	//
	//# \desc
	//# The $ReadIdentifier$ function reads an identifier from the character string specified by the $text$ parameter
	//# and returns the number of characters that were read. The characters composing the identifier are stored in
	//# the buffer specified by the $identifier$ parameter. The maximum number of characters returned in this buffer
	//# is specified by the $max$ parameter. The buffer specified by the $identifier$ parameter should be large enough
	//# to hold $max$ characters plus a zero terminator.
	//# 
	//# An identifier is made up of a sequence of consecutive characters from the set {0-9, A-Z, a-z, _, &#036;},
	//# that is, all uppercase and lowercase letters, all numbers, the underscore, and the dollar sign.
	//
	//# \also	$@Text::ReadString@$
	//# \also	$@Text::GetWhitespaceLength@$
	...
	//# $@Text::GetWhitespaceLength@$ function. If the first character is a double quote, then the $ReadString$ function
	//# reads all of the characters between the first double quote and a closing double quote, including any
	...
	namespace Text
	{
		C4_API int32 ReadGlyphCodeUTF8(const char *text, unsigned_int32 *code);
		C4_API int32 WriteGlyphCodeUTF8(char *text, unsigned_int32 code);
		
		C4_API int32 GetGlyphCodeByteCountUTF8(unsigned_int32 code);
		C4_API int32 GetGlyphCountUTF8(const char *text);
		C4_API int32 GetGlyphCountUTF8(const char *text, int32 max);
		
		C4_API int32 GetPreviousGlyphByteCountUTF8(const char *text, int32 max);
		C4_API int32 GetNextGlyphByteCountUTF8(const char *text, int32 max);
		C4_API int32 GetGlyphStringByteCountUTF8(const char *text, int32 glyphCount);
		
		C4_API int32 GetTextLength(const char *text);
		C4_API unsigned_int32 GetTextHash(const char *text);
		
		C4_API int32 FindChar(const char *text, unsigned_int32 k);
		C4_API int32 FindChar(const char *text, unsigned_int32 k, int32 max);
		C4_API int32 FindUnquotedChar(const char *text, unsigned_int32 k);
		
		C4_API int32 CopyText(const char *source, char *dest);
		C4_API int32 CopyText(const char *source, char *dest, int32 max);
		C4_API int32 CopyTextUTF8(const char *source, char *dest, int32 maxCount, int32 *length = nullptr);
		
		C4_API void ConvertToLowerCase(char *text);
		C4_API void ConvertToUpperCase(char *text);
		
		C4_API bool CompareText(const char *s1, const char *s2);
		C4_API bool CompareText(const char *s1, const char *s2, int32 max);
		C4_API bool CompareTextCaseless(const char *s1, const char *s2);
		C4_API bool CompareTextCaseless(const char *s1, const char *s2, int32 max);
		C4_API bool CompareTextLessThan(const char *s1, const char *s2);
		C4_API bool CompareTextLessThan(const char *s1, const char *s2, int32 max);
		C4_API bool CompareTextLessThanCaseless(const char *s1, const char *s2);
		C4_API bool CompareTextLessThanCaseless(const char *s1, const char *s2, int32 max);
		C4_API bool CompareTextLessEqual(const char *s1, const char *s2);
		C4_API bool CompareTextLessEqual(const char *s1, const char *s2, int32 max);
		C4_API bool CompareTextLessEqualCaseless(const char *s1, const char *s2);
		C4_API bool CompareTextLessEqualCaseless(const char *s1, const char *s2, int32 max);
		
		C4_API int32 FindText(const char *s1, const char *s2);
		
		C4_API int32 IntegerToString(int32 num, char *text, int32 max);
		C4_API int32 StringToInteger(const char *text);
		C4_API int32 Integer64ToString(int64 num, char *text, int32 max);
		C4_API int64 StringToInteger64(const char *text);
		C4_API int32 FloatToString(float num, char *text, int32 max);
		C4_API float StringToFloat(const char *text);
		
		C4_API int32 GetResourceNameLength(const char *text);
		C4_API int32 GetDirectoryPathLength(const char *text);
		...
		C4_API int32 GetWhitespaceLength(const char *text);
		C4_API int32 ReadIdentifier(const char *text, char *identifier, int32 max);
		C4_API int32 ReadInteger(const char *text, char *number, int32 max);
		C4_API int32 ReadFloat(const char *text, char *number, int32 max);
		C4_API int32 ReadString(const char *text, char *string, int32 max);
		C4_API int32 ReadType(const char *text, unsigned_int32 *type);
	...
	//
	//# \proto	void ConvertToLowerCase(void);
	//
	//# \desc
	...
	//
	//# \proto	void ConvertToUpperCase(void);
	//
	//# \desc
	...
	template <int32 len = 0> class String
	{
		...
		public:
			...
			{
				Text::ConvertToLowerCase(c);
			}
			
			void ConvertToUpperCase(void)
			{
				Text::ConvertToUpperCase(c);
			}
	...
	template <> class String<0>
	{
		...
		public:
			...
			void ConvertToLowerCase(void)
			{
				Text::ConvertToLowerCase(stringPointer);
			}
			
			void ConvertToUpperCase(void)
			{
				Text::ConvertToUpperCase(stringPointer);
			}
			
			C4API void Clear(void);
			C4API String& Set(const char *s, int32 length);
			
			#if C4RVALUEREF

				String<0>& operator =(String&& s)
				{
					logicalSize = s.logicalSize;
					physicalSize = s.physicalSize;
					stringPointer = s.stringPointer;
	...
	class ConstCharKey
	{
		...
		public:
			...
			bool operator <=(const char *c) const
			{
				return (Text::CompareTextLessEqual(ptr, c));
			}
	...
	class StringKey
	{
		...
		public:
			...
			bool operator <=(const char *c) const
			{
				return (Text::CompareTextLessEqualCaseless(ptr, c));
			}
	...
	namespace Text
	{
		...
		C4API extern const char identifierCharFlag[256];
