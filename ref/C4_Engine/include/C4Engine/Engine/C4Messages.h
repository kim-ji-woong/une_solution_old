...
namespace C4
{
	...
	enum
	{
		...
		kMaxFileChunkSize				= (kMaxMessageDataSize - 1) & ~3,
	...
	class Compressor
	{
		...
		public:
			...
			{
				Assert(size < kMaxMessageDataSize, "Message data size overflow");
			...
			{
				Assert(size + 2 <= kMaxMessageDataSize, "Message data size overflow");
			...
			{
				Assert(size + 4 <= kMaxMessageDataSize, "Message data size overflow");
			...
			{
				Assert(size + 8 <= kMaxMessageDataSize, "Message data size overflow");
			...
			{
				Assert(size + Text::GetTextLength(text) + 1 <= kMaxMessageDataSize, "Message data size overflow");
			...
			{
				Assert(size + dataSize <= kMaxMessageDataSize, "Message data size overflow");
	...
	class Decompressor
	{
		...
		public:
			...
			Decompressor& operator >>(int16& x)
			{
				...
				x = (int16) ((*reinterpret_cast<const char *>(p) << 8) | p[1]);
			...
			{
				const unsigned_int8 *p = reinterpret_cast<const unsigned_int8 *>(pointer + size);
				x = (*reinterpret_cast<const char *>(p) << 24) | (p[1] << 16) | (p[2] << 8) | p[3];
			...
			{
				const unsigned_int8 *p = reinterpret_cast<const unsigned_int8 *>(pointer + size);
				x = ((int64) *reinterpret_cast<const char *>(p) << 56) | ((int64) p[1] << 48) | ((int64) p[2] << 40) | ((int64) p[3] << 32) | (p[4] << 24) | (p[5] << 16) | (p[6] << 8) | p[7];
			...
			{
				const unsigned_int8 *p = reinterpret_cast<const unsigned_int8 *>(pointer + size);
				x = ((unsigned_int64) p[0] << 56) | ((unsigned_int64) p[1] << 48) | ((unsigned_int64) p[2] << 40) | ((unsigned_int64) p[3] << 32) | (p[4] << 24) | (p[5] << 16) | (p[6] << 8) | p[7];
	...
	//# having an application-defined type, it calls the $@System/Application::ConstructMessage@$ function with
	//# the message type, and this function should return a newly constructed $Message$ subclass corresponding
	...
	//
	//# \desc
	//# The $GetMessageType$ function is typically called from the $@System/Application::ConstructMessage@$ function
	...
	//
	//# \also	$@System/Application::ConstructMessage@$
	...
	//# The $flags$ parameter may be zero or a combination (through logical OR) of the following values.
	//
	//# \table	MessageFlags
	//
	...
	//
	//# \proto	virtual void Compress(Compressor& data) const;
	//
	...
	//
	//# \proto	virtual bool Decompress(Decompressor& data);
	//
	...
	//
	//# \proto	virtual bool HandleMessage(Player *sender) const;
	//
	...
	class C4_API Message : public ListElement<Message>, public Memory<MessageMgr>
	{
		...
		protected:
			...
			Message(MessageType type, unsigned_int32 flags = 0);
		...
		public:
			...
			virtual bool HandleMessage(Player *sender) const;
	};
	...
	class FileReceiveMessage : public Message
	{
		...
		public:
			...
			FileReceiveMessage(const char *name, unsigned_int32 size);
			~FileReceiveMessage();
	...
	//# \also	$@Controller/Controller::ConstructMessage@$
	...
	//# $@Controller/Controller::ConstructMessage@$ function to determine what type of controller message
	...
	class C4_API ControllerMessage : public Message, public ListElement<ControllerMessage>
	{
		private:
		...
		public:
			...
			ControllerMessage(ControllerMessageType type, int32 index, unsigned_int32 flags = 0);
			~ControllerMessage();
			...
			virtual bool HandleControllerMessage(Controller *controller) const;
			virtual bool OverridesMessage(const ControllerMessage *message) const;
	};
	
	
	class C4_API CombinedMessage : public ListElement<CombinedMessage>, public Memory<MessageMgr>
	{
		private:
	...
	class C4_API StateSender : public ListElement<StateSender>
	{
		public:
		...
		public:
			...
			StateSender(SenderProc *proc, void *cookie);
			~StateSender();
	...
	//
	//# \def	class Player : public MapElement<Player>, public GraphElement<Player, Channel>, public LinkTarget<Player>,
	//# \def2	public Completable<Player>
	//
	...
	//
	//# \also	$@MessageMgr@$
	//# \also	$@MessageMgr::SetPlayerConstructor@$
	...
	//
	//# \proto	unsigned_int32 GetReceiveFileSize(void) const;
	//
	//# \desc
	...
	//
	//# \proto	unsigned_int32 GetReceiveFilePosition(void) const;
	//
	//# \desc
	...
	class C4_API Player : public MapElement<Player>, public GraphElement<Player, Channel>, public LinkTarget<Player>, public Completable<Player>
	{
		friend class MessageMgr;
		...
		private:
			...
			unsigned_int32					sendFileSize;
			unsigned_int32					receiveFileSize;
		...
		public:
			...
			unsigned_int32 GetReceiveFileSize(void) const
			{
				return (receiveFile.Open() ? receiveFileSize : 0);
			}
			
			unsigned_int32 GetReceiveFilePosition(void) const
			{
			...
			void StartReceivingFile(const char *name, unsigned_int32 size);
			...
			virtual ChatStreamer *ConstructChatStreamer(void);
	...
	//# \function	MessageMgr::SetPlayerConstructor	Sets the construction function for Player objects.
	//
	//# \proto	void SetPlayerConstructor(PlayerConstructProc *proc, void *cookie = nullptr);
	//
	//# \param	proc	A pointer to the player construction function.
	//# \param	cookie	The cookie that is passed to the player construction function.
	//
	//# \desc
	//# When the Message Manager needs to construct a new $@Player@$ object, it first checks to see if a
	//# player constructor function has been installed. If so, then it calls the player construction
	//# function, allowing the application to construct an object of a type that is a subclass of the $Player$ class.
	//# Initially, there is no player constructor function, so the Message Manager just constructs the $Player$ base class.
	//#
	//# The $SetPlayerConstructor$ function should be called by an application's startup code when it wants to
	...
	//
	//# \code	typedef Player *PlayerConstructProc(PlayerKey, void *);
	//
	...
	//# $SetPlayerConstructor$ function. Ordinarily, this callback should simply construct a new object having type
	//# that is a subclass of $Player$ and return it.
	//#
	//# The current player constructor can be uninstalled by calling the $SetPlayerConstructor$ function with a value
	...
	class MessageMgr : public Manager<MessageMgr>
	{
		...
		public:
			...
			typedef ControllerMessage *ControllerMessageConstructProc(ControllerMessageType, int32, Decompressor&, void *);
		...
		private:
			...
			PlayerConstructProc					*playerConstructorProc;
			void								*playerConstructorCookie;
			
			ControllerMessageConstructProc		*controllerMessageConstructor;
			...
			Message *ConstructMessage(MessageType type, Decompressor& data);
		...
		public:
			...
			void SetPlayerConstructor(PlayerConstructProc *proc, void *cookie = nullptr)
			{
				playerConstructorProc = proc;
				playerConstructorCookie = cookie;
			}
			...
			void SetControllerMessageProcs(ControllerMessageConstructProc *constructor, ControllerMessageReceiveProc *receiver, void *cookie = nullptr)
			{
				controllerMessageConstructor = constructor;
