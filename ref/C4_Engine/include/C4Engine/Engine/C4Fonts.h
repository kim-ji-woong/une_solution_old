...
namespace C4
{
	...
	struct GlyphData
	{
		...
		Point2D			glyphTexcoord[2];	//## The texture coordinates corresponding to the upper-left and bottom-right corners of the glyph within the font's texture.
	};
	
	void Reverse(GlyphData *gd);
	...
	class Font : public Shared
	{
		...
		public:
			...
			{
				return (fontHeader->GetPageTable()[(fontHeader->GetPageIndex()[code >> 8] << 8) + (code & 0x00FF)]);
			}
			...
			const TextureMapAttribute *GetTextureMapAttribute(void) const
			{
				return (&textureMapAttribute);
			}
