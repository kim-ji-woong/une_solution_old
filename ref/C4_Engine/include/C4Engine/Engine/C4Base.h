...
namespace C4
{
	...
	template <> inline void Exchange<float>(float& x, float& y)
	{
		...
		inline int32 Cntlz(register unsigned_int32 n)
		{
			return (__cntlzw(n));
		}
			...
			inline int32 Cntlz(unsigned long n)
			{
				...
				if (_BitScanReverse(&x, n) == 0) return (32);
				return (31 - x);
			}
	...
	inline int16 ReadLittleEndianS16(const int16 *ptr)
	{
			...
			return (*ptr);
				...
				return ((int16) __lhbrx(ptr, 0));
			
			#else
			
				return ((int16) __lhbrx(ptr));
	...
	inline int32 ReadLittleEndianS32(const int32 *ptr)
	{
			...
			return (*ptr);
				...
				return ((int32) __lwbrx(ptr, 0));
			
			#else
			
				return ((int32) __lwbrx(ptr));
	...
	inline void AtomicAnd(volatile int32 *ptr, int32 x)
	{
		#if C4WINDOWS
		
			_InterlockedAnd(reinterpret_cast<volatile long *>(ptr), x);
		
		#elif C4MACOS
		
			OSAtomicAnd32(x, reinterpret_cast<volatile uint32_t *>(ptr));
		
		#elif C4POWERPC
		
			while (!__stwcx(ptr, __lwarx(ptr) & x)) {}
		
		#endif
	}
	
	inline void AtomicOr(volatile int32 *ptr, int32 x)
	{
		#if C4WINDOWS
		
			_InterlockedOr(reinterpret_cast<volatile long *>(ptr), x);
		
		#elif C4MACOS
		
			OSAtomicOr32(x, reinterpret_cast<volatile uint32_t *>(ptr));
	...
	{
		return (static_cast<const volatile char *>(ptr) - static_cast<char *>(nullptr));
	}
	
	#define GetEnclosingStruct(ptr, type, member) (reinterpret_cast<type *>(reinterpret_cast<char *>(ptr) - GetPointerAddress(&reinterpret_cast<const volatile char&>(static_cast<type *>(nullptr)->member))))
	...
	//# Each call the the $Acquire$ function must be balanced by a corresponding call to the
	...
	//# Each call the the $TryAcquire$ function that returns $true$ must be balanced by a
	...
	template <class type> class Manager
	{
		...
		template <class type> EngineResult Manager<type>::New(void)
		{
			*managerPointer = &managerObject;
			...
			if (result != kEngineOkay) *managerPointer = nullptr;
			return (result);
		}
