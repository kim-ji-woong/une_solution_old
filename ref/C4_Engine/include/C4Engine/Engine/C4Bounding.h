	...
	class Box3D : public Range<Point3D>
	{
		public:
			...
			Box3D(const Point3D& pmin, const Point3D& pmax) : Range<Point3D>(pmin, pmax)
			{
				...
				Box3D(float4 pmin, float4 pmax)
				{
					SimdStore3D(pmin, &min.x);
					SimdStore3D(pmax, &max.x);
				}
				
				Box3D& Set(float4 pmin, float4 pmax)
				{
					SimdStore3D(pmin, &min.x);
					SimdStore3D(pmax, &max.x);
					return (*this);
			...
			Vector3D IncludePoint(const Point3D& p)
			{
				...
				float4 IncludePoint(float4 p)
				{
					float4 pmin = SimdMin(SimdLoadUnaligned(&min.x), p);
					float4 pmax = SimdMax(SimdLoadUnaligned(&max.x), p);
					SimdStore3D(pmin, &min.x);
					SimdStore3D(pmax, &max.x);
					return (SimdSub(pmax, pmin));
				}
				
				void Union(const Box3D& box)
				{
					float4 pmin = SimdMin(SimdLoadUnaligned(&min.x), SimdLoadUnaligned(&box.min.x));
					float4 pmax = SimdMax(SimdLoadUnaligned(&max.x), SimdLoadUnaligned(&box.max.x));
					SimdStore3D(pmin, &min.x);
					SimdStore3D(pmax, &max.x);
				}
				...
				{
					float4 point = SimdLoadUnaligned(&p.x);
					float4 pmin = SimdMin(SimdLoadUnaligned(&min.x), point);
					float4 pmax = SimdMax(SimdLoadUnaligned(&max.x), point);
					SimdStore3D(pmin, &min.x);
					SimdStore3D(pmax, &max.x);
				}
