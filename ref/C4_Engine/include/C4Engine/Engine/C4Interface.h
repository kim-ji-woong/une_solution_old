...
namespace C4
{
	...
	class Cursor : public Renderable, public Transformable
	{
		private:
			...
			Point3D					cursorVertex[4];
			Point2D					cursorTexcoord[4];
		...
		public:
			...
			{
				textureMap.SetTexture(name);
			}
			
			const Point3D& GetMinBounds(void) const
			{
				return (cursorVertex[0]);
			}
			
			const Point3D& GetMaxBounds(void) const
			{
				return (cursorVertex[2]);
			}
			
			void Render(const Point3D& position, List<Renderable> *renderList);
	};
	...
	class InterfaceMgr : public Manager<InterfaceMgr>
	{
		private:
			...
			Constructor<Widget>			widgetConstructor;
			...
			int32						balloonTime;
			Link<Widget>				balloonSourceWidget;
			...
			static Widget *ConstructWidget(Unpacker& data, unsigned_int32 unpackFlags);
			...
			Widget *FindHoverWidget(Widget **root) const;
			void DisplayBalloon(Widget *widget, Widget *root);
