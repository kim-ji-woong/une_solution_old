...
namespace C4
{
	...
	enum
	{
		...
		kMaterialVertexAmbientOcclusion		= 1 << 4,			//## Apply per-vertex ambient occlusion using the alpha channel of the vertex color.
	...
	//# \also	$@Physics/Shape::GetShapeSubstance@$ 
	//# \also	$@Physics/Shape::SetShapeSubstance@$ 
	 
	
	class C4_API Substance : public MapElement<Substance> 
	{
		private:
		...
		public:
			...
			Substance(SubstanceType type, const char *name);
			virtual ~Substance();
	...
	class C4_API MaterialRegistration : public MapElement<MaterialRegistration>
	{
		private:
		...
		public:
			...
			MaterialRegistration(MaterialType type, const char *name, const void *cookie = nullptr);
	...
	class C4_API MaterialResource : public Resource<MaterialResource>
	{
	...
	//# in a material. The blend mode can be one of the following values.
	//
	...
	class C4_API MaterialObject : public Object, public MapElement<MaterialObject>
	{
		...
		private:
			...
			static Map<Substance>				substanceMap;
			static Map<MaterialRegistration>	registrationMap;
		...
		public:
			...
			MaterialObject();
			MaterialObject(const MaterialObject& materialObject);
			...
			MaterialObject *Clone(void) const;
			...
			static MaterialObject *Get(MaterialType type);
			static MaterialObject *Get(const char *name);
			static void ReleaseMaterialCache(void);
			
			bool operator ==(const MaterialObject& object) const;
			
			bool ShaderMaterial(void) const;
