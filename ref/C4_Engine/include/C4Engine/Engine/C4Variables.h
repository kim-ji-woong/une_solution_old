...
namespace C4
{
	...
	//# a combination (through logical OR) of the following values.
	//
	//# \table	VariableFlags
	//
	...
	//
	//# \desc
	//# The $GetVariableFlags$ function returns the variable flags, which can be a combination (through logical OR) of the following values.
	//
	//# \table	VariableFlags
	//
	...
	class C4_API Variable : public MapElement<Variable>, public Observable<Variable>
	{
		private:
		...
		public:
			...
			explicit Variable(const char *name, unsigned_int32 flags = 0, ObserverType *observer = nullptr);
			virtual ~Variable();
