...
namespace C4
{
	...
	//
	//# \def	template <class observerType, class observableType> class Observer :
	//# \def2	public ObserverBase<observableType, typename observableType::ObservableEventType>
	//
	...
	template <class observableType> class ExclusiveObserverBase<observableType, void> : public LinkTarget<ExclusiveObserverBase<observableType, void> >
	{
		...
		public:
			...
			void InvokeCallback(observableType *observable)
			{
				if (callbackFunction) (observerObject->*callbackFunction)(observable);
			}
	};
