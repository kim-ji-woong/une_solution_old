...
namespace C4
{
	...
	//
	//# \proto	Matrix3D& SetRotationAboutAxis(float angle, const Vector3D& axis);
	//
	//# \param	angle	The angle through which to rotate, in radians.
	...
	//
	//# \proto	Matrix3D& Orthonormalize(void);
	//
	//# \desc
	//# The $Orthonormalize$ function uses Gram-Schmidt orthogonalization to orthogonalize the columns
	//# of a matrix. The columns are then normalized to unit length.
	...
	class Matrix3D
	{
		...
		public:
			...
			{
				return (Antivector3D(n[0][i], n[1][i], n[2][i]));
			}
			...
			C4API Matrix3D& SetRotationAboutAxis(float angle, const Vector3D& axis);
			...
			C4API Matrix3D& Orthonormalize(void);
	...
	C4API Matrix3D Transform(const Matrix3D& m2, const Transform4D& m1);
