...
namespace C4
{
	...
	enum
	{
		kShapeBox				= 'BOX ',		//## A box shape.
		kShapePyramid			= 'PYRA',		//## A pyramid shape.
		kShapeCylinder			= 'CYLD',		//## A cylinder shape.
		kShapeCone				= 'CONE',		//## A cone shape.
		kShapeSphere			= 'SPHR',		//## A sphere shape.
		kShapeDome				= 'DOME',		//## A dome shape.
		kShapeCapsule			= 'CPSL',		//## A capsule shape.
		kShapeTruncatedPyramid	= 'TPYR',		//## A truncated pyramid shape.
		kShapeTruncatedCone		= 'TCON',		//## A truncated cone shape.
		kShapeTruncatedDome		= 'TDOM'		//## A truncated dome shape.
	};
	...
	//
	//# \desc
	//# The $GetShapeType$ function returns the specific shape type, which may be one of the following values.
	//
	//# \table	ShapeType
	...
	class ShapeObject : public Object, public VolumeObject
	{
		...
		private:
			...
			static ShapeObject *Construct(Unpacker& data, unsigned_int32 unpackFlags);
	...
	class BoxShapeObject : public ShapeObject, public BoxVolume
	{
		...
		public:
			...
			Point3D CalculateCenterOfMass(void) const;
			InertiaTensor CalculateInertiaTensor(void) const;
			
			Point3D GetInitialSupportPoint(void) const;
			Point3D CalculateSupportPoint(const Vector3D& direction) const;
			void CalculateOpposingSupportPoints(const Vector3D& direction, Point3D *support) const;
			void CalculateSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const;
	};
	...
	class SphereShapeObject : public ShapeObject, public SphereVolume
	{
		...
		public:
			...
			bool DetectSegmentIntersection(const Point3D& p1, const Point3D& p2, float radius, ShapeHitData *shapeHitData) const;
			float CalculateSubmergedVolume(const Antivector4D& plane, Point3D *centroid) const;
	};
	...
	class Shape : public Node, public ListElement<Shape>
	{
		...
		private:
			...
			struct TriangleIntersectionParams
			{
				...
				const GeometryLevel		*geometryLevel;
			...
			Transform4D		inverseMoveTransform;
			
			static Shape *Construct(Unpacker& data, unsigned_int32 unpackFlags);
			...
			bool StaticIntersectPrimitive(const Geometry *geometry, IntersectionData *intersectionData, int32 *dimension1, int32 *dimension2, const Vector3D& worldOffset) const;
			bool FindMinimumPrimitivePenetration(const Geometry *geometry, const Transform4D& shapeTransform, const Matrix3D& shapeInverseMatrix3D, IntersectionData *intersectionData) const;
			...
			static int32 CalculateTriangleSupportPoint(const Point3D *vertex, const Vector3D& direction);
			bool StaticIntersectTriangle(const Point3D *triangleVertex, const Triangle *triangle, const TriangleIntersectionParams *params, IntersectionData *intersectionData) const;
			bool DynamicIntersectTriangle(const Point3D *triangleVertex, const Triangle *triangle, const Vector3D& displacement, const TriangleIntersectionParams *params, IntersectionData *intersectionData) const;
			int32 StaticIntersectGeometryOctree(const CollisionOctree *octree, const Box3D& collisionBox, const TriangleIntersectionParams *params, IntersectionData *intersectionData, int32 contactCount) const;
			int32 DynamicIntersectGeometryOctree(const Vector3D& displacement, const CollisionOctree *octree, const Box3D& collisionBox, const TriangleIntersectionParams *params, IntersectionData *intersectionData, int32 contactCount) const;
			int32 MixedIntersectGeometryOctree(const Vector3D& displacement, const CollisionOctree *octree, const Box3D& collisionBox, int32 contactTriangleCount, const unsigned_int32 *contactTriangleIndex, const TriangleIntersectionParams *params, IntersectionData *intersectionData, int32 contactCount) const;
		...
		public:
			...
			void SetInverseMoveTransform(const Transform4D& transform)
			{
				inverseMoveTransform = transform;
			}
			...
			bool StaticIntersectPrimitive(const Geometry *geometry, GeometryContact *geometryContact, IntersectionData *intersectionData) const;
			bool DynamicIntersectPrimitive(const Geometry *geometry, const Vector3D& displacement, IntersectionData *intersectionData) const;
	...
	class BoxShape : public Shape
	{
		...
		public:
			...
			bool CalculateBoundingBox(Box3D *box) const;
	};
