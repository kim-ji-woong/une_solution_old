...
namespace C4
{
	...
	{
		return (row[0] * v.x + row[1] * v.y + row[2] * v.z + row[3]);
	}
	...
	class Matrix4D
	{
		...
		public:
			...
			Matrix4D& SetRow(int32 i, const Antivector4D& row)
			{
				...
				{
					SimdStore(SimdLoad(&m.n[0][0], 0), &n[0][0], 0);
					SimdStore(SimdLoad(&m.n[0][0], 4), &n[0][0], 4);
					SimdStore(SimdLoad(&m.n[0][0], 8), &n[0][0], 8);
					SimdStore(SimdLoad(&m.n[0][0], 12), &n[0][0], 12);
					return (*this);
				}
	...
	//#				Returns the product of the matrix $m$ and the column vector $v$. The <i>w</i> coordinate of $v$ is assumed to be 0.
	//
	//# \action		Vector3D operator *(const Vector3D& v, const Transform4D& m);
	...
	//#				Returns the product of the matrix $m$ and the column vector $p$. The <i>w</i> coordinate of $p$ is assumed to be 1.
	//
	//# \action		Point3D operator *(const Point3D& p, const Transform4D& m);
	...
	//#				Returns the product of the matrix $m$ and the column vector $v$. The <i>z</i> and <i>w</i> coordinates of $v$ are assumed to be 0.
	//
	//# \action		Vector2D operator *(const Vector2D& v, const Transform4D& m);
	...
	//#				and the <i>w</i> coordinate of $p$ is assumed to be 1.
	//
	//# \action		Point2D operator *(const Point2D& p, const Transform4D& m);
	...
	//# \proto	Transform4D& SetMatrix3D(float n00, float n01, float n02, float n10, float n11, float n12,
	//# \proto2	float n20, float n21, float n22);
	//
	//# \param	m		A 3&nbsp;&times;&nbsp;3 matrix that is copied to the upper-left 3&nbsp;&times;&nbsp;3 portion of the matrix.
	//# \param	c1		The new values of the entries residing in first column.
	//# \param	c2		The new values of the entries residing in second column.
	...
	//
	//# \proto	Transform4D& SetRotationAboutAxis(float angle, const Vector3D& axis);
	//
	//# \param	angle	The angle through which to rotate, in radians.
	...
	class Transform4D : public Matrix4D
	{
		public:
			...
			C4API Transform4D& SetRotationAboutAxis(float angle, const Vector3D& axis);
			...
			friend C4API Vector3D operator *(const Vector3D& v, const Transform4D& m);
			friend C4API Point3D operator *(const Transform4D& m, const Point3D& p);
			friend C4API Point3D operator *(const Point3D& p, const Transform4D& m);
			friend C4API Vector2D operator *(const Transform4D& m, const Vector2D& v);
			friend C4API Vector2D operator *(const Vector2D& v, const Transform4D& m);
			friend C4API Point2D operator *(const Transform4D& m, const Point2D& p);
			friend C4API Point2D operator *(const Point2D& p, const Transform4D& m);
			
			friend C4API Matrix3D Transform(const Matrix3D& m2, const Transform4D& m1);
			
			#if C4SIMD
			
				friend C4API float4 TransformVector3D(const Transform4D& m, float4 v);
				friend C4API float4 TransformPoint3D(const Transform4D& m, float4 p);
	...
	struct ConstTransform4D
	{
		...
		C4API float4 TransformVector3D(const Transform4D& m, float4 v);
		C4API float4 TransformPoint3D(const Transform4D& m, float4 p);
