...
namespace C4
{
	...
	enum
	{
		kMovieVideoAlphaChannel			= 1 << 0
	};


	enum
	{
		kMovieVideoCompressBaseFrame	= 1 << 0,
		kMovieVideoCompressDeltaFrame	= 1 << 1
	};
	...
	struct MovieVideoFrameHeader
	{
		MovieVideoChannelData	luminanceData;
		MovieVideoChannelData	chrominanceData;
	};


	struct MovieVideoAlphaFrameHeader : MovieVideoFrameHeader
	{
		MovieVideoChannelData	alphaData;
	};


	struct MovieVideoFrameData
	{
		unsigned_int32			videoFrameOffset;
		unsigned_int32			videoFrameSize;
		int32					videoBaseFrameIndex;
	};


	struct MovieVideoTrackHeader
	{
		...
		MovieVideoFrameData		videoFrameData[1];
	};


	struct MovieAudioChannelData
	{
	...
	struct MovieAudioBlockHeader
	{
		unsigned_int32			audioBlockFlags;
		MovieAudioChannelData	audioChannelData[1];
	};


	struct MovieAudioBlockData
	{
		unsigned_int32			audioBlockOffset;
		unsigned_int32			audioBlockSize;
	};


	struct MovieAudioTrackHeader
	{
		...
		MovieAudioBlockData		audioBlockData[1];
	};
	...
	struct MovieTrackHeader
	{
		...
		int32					movieTrackOffset;

		const MovieVideoTrackHeader *GetVideoTrackHeader(void) const
		{
			return (reinterpret_cast<const MovieVideoTrackHeader *>(reinterpret_cast<const char *>(this) + movieTrackOffset));
		}

		const MovieAudioTrackHeader *GetAudioTrackHeader(void) const
		{
			return (reinterpret_cast<const MovieAudioTrackHeader *>(reinterpret_cast<const char *>(this) + movieTrackOffset));
		}
	};
	...
	class MovieResource : public Resource<MovieResource>
	{
		...
		public:
			...
			ResourceResult LoadVideoFrameData(ResourceLoader *loader, MovieResourceHeader *resourceHeader, const MovieVideoTrackHeader *videoTrackHeader, int32 frameIndex, MovieVideoFrameHeader *videoFrameHeader) const;
			ResourceResult LoadAudioBlockData(ResourceLoader *loader, MovieResourceHeader *resourceHeader, const MovieAudioTrackHeader *audioTrackHeader, int32 blockIndex, MovieAudioBlockHeader *audioBlockHeader) const;
	...
	class VideoCompressor
	{
		private:
			...
			unsigned_int32 CompressLuminance(MovieVideoFrameHeader *videoFrameHeader, float quantScale, unsigned_int32 flags, int32 index);
			unsigned_int32 CompressChrominance(MovieVideoFrameHeader *videoFrameHeader, float quantScale, unsigned_int32 flags, int32 index, unsigned_int32 codeOffset);
			unsigned_int32 CompressAlpha(MovieVideoAlphaFrameHeader *videoFrameHeader, float quantScale, unsigned_int32 flags, int32 index, unsigned_int32 codeOffset);

		public:

			C4API VideoCompressor(const MovieVideoTrackHeader *videoTrackHeader);
			...
			C4API unsigned_int32 CompressFrame(const void *image, ImageFormat format, MovieVideoFrameHeader *videoFrameHeader, float quantScale, unsigned_int32 flags);
	};
	...
	class VideoDecompressor
	{
		...
		private:
			...
			MovieVideoFrameHeader	*videoFrameHeader;
		...
		public:
			...
			VideoDecompressor(const MovieVideoTrackHeader *videoTrackHeader);
			~VideoDecompressor();

			MovieVideoFrameHeader *GetVideoFrameHeader(void) const
			{
	...
	class AudioCompressor
	{
		...
		public:
			...
			C4API AudioCompressor(const MovieAudioTrackHeader *audioTrackHeader);
			...
			C4API unsigned_int32 CompressBlock(const Sample *audio, int32 frameCount, MovieAudioBlockHeader *audioBlockHeader);
	};
	...
	class AudioDecompressor
	{
		private:
			...
			MovieAudioBlockHeader	*audioBlockHeader;
		...
		public:
			...
			AudioDecompressor(const MovieAudioTrackHeader *audioTrackHeader);
			~AudioDecompressor();

			MovieAudioBlockHeader *GetAudioBlockHeader(void) const
			{
	...
	//
	//# \desc
	//# The $GetMovieState$ function returns the current state of a movie, which can be one of the following values.
	//
	//# \table	MovieState
	//
	...
	class Movie : public SoundStreamer, public SoundConduit, public ListElement<Movie>, public Completable<Movie>
	{
		...
		private:
			...
			const MovieVideoTrackHeader		*videoTrackHeader;
			const MovieAudioTrackHeader		*audioTrackHeader;
	...
	class MovieProcess : public Process
	{
		private:
	...
	//
	//# \def	class MovieWidget : public QuadWidget
	//
	...
	class MovieWidget : public QuadWidget
	{
	...
	class GetMovieWidgetTimeFunction : public WidgetFunction
	{
		private:
	...
	class SetMovieWidgetTimeFunction : public WidgetFunction
	{
		private:
