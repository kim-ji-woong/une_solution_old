...
namespace C4
{
	...
	class TargaImageImportPlugin : public ImageImportPlugin
	{
		...
		public:
			...
			const char *GetImageTypeName(void) const;
			const ResourceDescriptor *GetImageResourceDescriptor(void) const;
			ImageFileFilterProc *GetImageFileFilterProc(void) const;

			ImageFormat GetImageFormat(void) const;
			EngineResult GetImageFileInfo(const char *name, Integer2D *size, unsigned_int32 *flags = nullptr, int32 *count = nullptr);
			EngineResult ImportImageFile(const char *name, void **image, Integer2D *size, unsigned_int32 *flags = nullptr, int32 index = 0);
			void ReleaseImageData(void *image);
	};
	...
	class TargaImageExportPlugin : public ImageExportPlugin
	{
		private:
			...
			static unsigned_int32 CompressTarga_BGRA(const Color4C *image, unsigned_int8 *restrict data, int32 pixelCount);
			static unsigned_int32 CompressTarga_BGR(const Color4C *image, unsigned_int8 *restrict data, int32 pixelCount);
			static unsigned_int32 CompressTarga_RGBA(const Color4C *image, unsigned_int8 *restrict data, int32 pixelCount);
			static unsigned_int32 CompressTarga_RGB(const Color4C *image, unsigned_int8 *restrict data, int32 pixelCount);
