...
ResourceDescriptor ColladaResource::descriptor("dae", kResourceExtraByte);
...
static bool ReadIdentifier(const char *& text, String<Collada::kMaxIdentifierLength> *identifier)
{
	...
	for (;; length++)
	{
		...
		if ((c - 'A' < 26U) || (c - 'a' < 26U) || (c - '0' < 10U) || (c == '_')) continue;
		break;
	}
	
	if (length != 0)
	{
		identifier->Set(text, length);
		text += length;
		return (true);
	}
...
Collada::Scene *Collada::Element::GetOwningScene(void) const
{
	...
	while ((element) && (element->GetElementType() != Collada::kElementScene)) element = element->GetSuperNode();
...
Collada::Element *Collada::Element::FindSubElement(const char *id, Collada::Element *root)
{
	...
	do
	{
		if (element->GetSubIdentifier() == id) break;
		element = root->GetNextNode(element);
	} while (element);
	...
	if (targetIdentifier[0] != 0)
	{
		...
		else
		{
			...
			else
			{
				...
				if ((comp == "X") || (comp == "S") || (comp == "R") || (comp == "U")) targetComponent = 0;
				else if ((comp == "Y") || (comp == "T") || (comp == "G") || (comp == "V")) targetComponent = 1;
				else if ((comp == "Z") || (comp == "P") || (comp == "B")) targetComponent = 2;
				else if ((comp == "W") || (comp == "Q") || (comp == "A") || (comp == "ANGLE")) targetComponent = 3;
			}
		}
	}
}
...
void Collada::InterleavingElement::Preprocess(void)
{
	...
	while (element)
	{
		...
		else if (type == Collada::kElementInput)
		{
			...
			while (prev)
			{
				if (prev->GetInputIndex() == index) goto next;
			...
			if (input->GetInputSemantic() == Collada::kSemanticTexcoord) texcoordBase = Min(texcoordBase, input->GetTexcoordSet());
		}
		else
		{
			break;
	...
	for (;;)
	{
		...
		if (input->GetInputSemantic() == Collada::kSemanticTexcoord) input->SetTexcoordSet(input->GetTexcoordSet() - texcoordBase);
...
C4::MeshGeometry *Collada::Mesh::BuildGeometry(Collada::Scene *scene, const List<Collada::BindMaterial> *materialBindingList, Collada::Skin *skin) const
{
	List<GeometrySurface>	surfaceList;
	
	Array<C4::MaterialObject *> materialArray(4);
	...
	if (!surfaceList.Empty())
	{
		...
		if ((!skin) || (skinData))
		{
			C4::MeshGeometry *geometry = new C4::MeshGeometry(1, &surfaceTable, materialArray, &skinData);
			...
			if (skinData)
			{
				...
				object->SetGeometryFlags(kGeometrySkinnedModelFlags);
...
C4::MeshGeometry *Collada::Geometry::BuildGeometry(Collada::Scene *scene, const List<Collada::BindMaterial> *materialBindingList, Collada::Skin *skin) const
{
	const Collada::Element *element = GetFirstSubnode();
	while (element)
	{
		...
		{
			C4::MeshGeometry *mesh = static_cast<const Collada::Mesh *>(element)->BuildGeometry(scene, materialBindingList, skin);
			if (mesh) return (mesh);
		}
...
C4::PathMarker *Collada::Geometry::BuildPath(void) const
{
	C4::PathMarker *pathMarker = new C4::PathMarker(K::z_unit);
	C4::Path *path = pathMarker->GetPath();
	...
	if (path->GetFirstPathComponent()) return (pathMarker);
...
void Collada::NameArray::ProcessContent(const char *& text)
{
	...
	{
		ReadWhitespace(text);
		if (!ReadString(text, &string)) break;
...
void Collada::Accessor::Preprocess(void)
{
	...
	if (root) root = root->GetSuperNode();
...
void Collada::Source::Preprocess(void)
{
	...
	while (element)
	{
		...
		if (type == Collada::kElementTechnique)
		{
			...
			while (subelement)
			{
				if (subelement->GetElementType() == Collada::kElementAccessor) accessorList.Append(static_cast<Collada::Accessor *>(subelement));
				subelement = subelement->Next();
			}
		}
	...
	if (paramIdentifier[0] != 0)
	{
		...
		if (super)
		{
			...
			if (super)
			{
				...
				if (super)
				{
					Collada::Element *element = FindSubElement(paramIdentifier, super);
					if ((element) && (element->GetElementType() == Collada::kElementNewParam)) newParamElement = static_cast<Collada::NewParam *>(element);
				}
			}
		}
	}
...
{
	ReadWhitespace(text);
	if (!ReadString(text, &paramIdentifier)) paramIdentifier[0] = 0;
}
...
void Collada::Input::Preprocess(void)
{
	...
	if (root) root = root->GetSuperNode();
	sourceElement = FindElement(sourceIdentifier, root);
	if (!sourceElement) sourceElement = FindElement(sourceIdentifier);
}
	...
	if ((inputAccessor) && (outputAccessor))
	{
		...
		if ((timeCount > 0) && (outputAccessor->GetAccessCount() > 0))
		{
			...
			if (time <= beginTime)
			{
				...
				for (machine comp = 0; comp < compCount; comp++) value[comp] = beginValue[comp];
			}
			else if (time >= endTime)
			{
				const float *endValue = outputAccessor->GetFloatValue(timeCount - 1);
				for (machine comp = 0; comp < compCount; comp++) value[comp] = endValue[comp];
			}
			else
			{
				float t1 = beginTime;
					...
					if (time < t2)
					{
						...
						if ((type == Collada::kElementRotate) && ((compCount == 1) || (compCount == 4)))
						{
							...
							if (Fabs(r2 - r1) > 180.0F)
							{
								...
								for (machine a = 0; a < compCount - 1; a++) adjustedValue[a] = v2[a];
...
void Collada::Animation::Preprocess(void)
{
	...
	while (element)
	{
		...
		if (type == Collada::kElementAnimation)
		{
			...
			Tree<Collada::Animation>::AddSubnode(animation);
...
void Collada::Combiner::Preprocess(void)
{
	...
	if ((jointAccessor) && (weightAccessor))
	{
			...
			while (element)
			{
				if (element->GetElementType() == Collada::kElementIndexArray)
				{
					...
					for (machine a = 0; a < vertexCount; a++)
					{
						...
						weightSize += CountVertexWeights(start, count, indexArray) * 2;
						start += count;
					}
		...
		else
		{
			...
			while (element)
			{
				if (element->GetElementType() == Collada::kElementIndexArray)
				{
					...
					weightSize += CountVertexWeights(0, count, indexArray) * 2;
				}
		...
		weightDataSize = weightSize * 4;
	}
}

int32 Collada::Combiner::CountVertexWeights(int32 start, int32 count, const Collada::IndexArray *indexArray) const
{
	int32 inputCount = GetInputCount();
	...
	for (machine a = start; a < end; a++)
	{
		...
		if ((weight) && (*weight != 0.0F))
		{
			...
			{
				if (indexArray->GetIndexValue(b * inputCount + jointIndex) == index) goto skip;
			}
...
C4::WeightedVertex *Collada::Combiner::GenerateVertexWeights(int32 maxBoneIndex, C4::WeightedVertex *weightedVertex, int32 start, int32 count, const Collada::IndexArray *indexArray) const
{
	int32 inputCount = GetInputCount();
	...
	if (finalCount != 0)
	{
		weightedVertex->boneCount = finalCount;
	}
	else
	{
		weightedVertex->boneCount = 1;
		boneWeight->weight = 1.0F;
		boneWeight->boneIndex = 0;
		boneWeight++;
	}
	
	return (reinterpret_cast<C4::WeightedVertex *>(boneWeight));
}

bool Collada::Combiner::GenerateWeightData(int32 maxBoneIndex, C4::WeightedVertex *weightedVertex, const C4::WeightedVertex **weightDataTable) const
{
	int32 inputCount = GetInputCount();
	...
	if (countArray)
	{
		...
		while (element)
		{
			if (element->GetElementType() == Collada::kElementIndexArray)
			{
				...
				for (machine a = 0; a < vertexCount; a++)
				{
					*weightDataTable++ = weightedVertex;
					
					int32 count = boneCount[a];
					if (count == 0) return (false);
					
					weightedVertex = GenerateVertexWeights(maxBoneIndex, weightedVertex, start, count, indexArray);
					start += count;
				}
	...
	else
	{
		...
		while (element)
		{
			if (element->GetElementType() == Collada::kElementIndexArray)
			{
				...
				*weightDataTable++ = weightedVertex;
				weightedVertex = GenerateVertexWeights(maxBoneIndex, weightedVertex, 0, count, indexArray);
			}
	...
	while (element)
	{
		if (element->GetElementType() == Collada::kElementInput)
		{
			...
			if (source)
			{
				...
				{
					Collada::SemanticType semantic = input->GetInputSemantic();
					if (semantic == Collada::kSemanticBindPosition) bindPositionAccessor = accessor;
					else if (semantic == Collada::kSemanticBindNormal) bindNormalAccessor = accessor;
					else if (semantic == Collada::kSemanticInvBindMatrix) invBindMatrixAccessor = accessor;
					else if (semantic == Collada::kSemanticJoint) jointNameAccessor = accessor;
					else if (semantic == Collada::kSemanticJointWeight) PreprocessJointWeightSource(source);
				}
			}
		}
	...
	if (jointNameAccessor)
	{
		...
		for (machine a = 0; a < boneCount; a++)
		{
			...
			if (name)
			{
				...
				if (node) node->SetNodeType(Collada::kNodeJoint);
			}
		}
	}
}
...
const C4::SkinData *Collada::Skin::GetSkinData(Collada::Scene *scene)
{
	if (!skinStorage)
	{
		if ((jointWeightCombiner) && (jointNameAccessor) && (invBindMatrixAccessor))
		{
			...
			int32 vertexCount = jointWeightCombiner->GetWeightedVertexCount();
			unsigned_int32 dataSize = (jointWeightCombiner->GetWeightDataSize() + 15) & ~15;
			
			skinStorage = new char[boneCount * (sizeof(Transform4D) + 4) + vertexCount * sizeof(C4::WeightedVertex *) + dataSize];
			Transform4D *inverseBindTransform = reinterpret_cast<Transform4D *>(skinStorage);
			unsigned_int32 *nodeHash = reinterpret_cast<unsigned_int32 *>(inverseBindTransform + boneCount);
			const C4::WeightedVertex **weightDataTable = reinterpret_cast<const C4::WeightedVertex **>(nodeHash + boneCount);
			C4::WeightedVertex *weightData = reinterpret_cast<C4::WeightedVertex *>(weightDataTable + vertexCount);
			
			skinData.nodeHashArray = nodeHash;
			...
			if (bindShapeMatrix) bindTransform = bindShapeMatrix->GetMatrix();
			else bindTransform.SetIdentity();
			...
			if (!jointWeightCombiner->GenerateWeightData(boneCount - 1, weightData, weightDataTable))
			{
				delete[] skinStorage;
				skinStorage = nullptr;
				return (nullptr);
...
Collada::Faces::Faces(Collada::FacesType type) : Collada::InterleavingElement(Collada::kElementFaces)
{
	...
	for (machine a = 0; a < kMaxGeometryTexcoordCount; a++) texcoordAccessor[a] = nullptr;
...
GeometryPolygon *Collada::Faces::BuildPolygon(const int32 *entry, const Collada::IndexArray *indexArray, const Collada::Accessor *posAccessor, const Collada::Accessor *nrmAccessor) const
{
	...
	for (machine a = 0; a < 3; a++)
	{
		...
		vertex->skinIndex = index;
		
		const float *value = posAccessor->GetFloatValue(index);
		if (value) vertex->position.Set(value[0], value[1], value[2]);
		else vertex->position.Set(0.0F, 0.0F, 0.0F);
		
		if (nrmAccessor)
		{
			index = indexArray->GetIndexValue(entry[a] * inputCount + normalIndex);
			value = nrmAccessor->GetFloatValue(index);
			if (value) vertex->normal.Set(value[0], value[1], value[2]);
			else vertex->normal.Set(0.0F, 0.0F, 1.0F);
		}
		...
		if (colorAccessor)
		{
			...
			if (value) vertex->color.Set(value[0], value[1], value[2], 1.0F);
			else vertex->color.Set(1.0F, 1.0F, 1.0F, 1.0F);
		}
		...
		for (machine b = 0; b < kMaxGeometryTexcoordCount; b++)
		{
			...
			if (texcoordAccessor[b])
			{
				...
				if (value) vertex->texcoord[b].Set(value[0], value[1]);
			}
		}
	...
	{
		gv[0]->tangent.Set(0.0F, 0.0F, 0.0F, 0.0F);
		gv[1]->tangent.Set(0.0F, 0.0F, 0.0F, 0.0F);
		gv[2]->tangent.Set(0.0F, 0.0F, 0.0F, 0.0F);
	}
...
GeometrySurface *Collada::Faces::BuildSurface(const Collada::Scene *scene, const Collada::Skin *skin) const
{
	...
	if (skin)
	{
		...
		if (bindPositionAccessor) posAccessor = bindPositionAccessor;
	}
	
	if (!posAccessor) return (nullptr);
	
	const Collada::Accessor *nrmAccessor = normalAccessor;
	if (skin)
	{
		const Collada::Accessor *bindNormalAccessor = skin->GetBindNormalAccessor();
		if (bindNormalAccessor) nrmAccessor = bindNormalAccessor;
	}
	...
	{
		const Collada::CountArray *countArray = GetCountArray();
		if ((!countArray) || (countArray->GetArraySize() != GetItemCount())) return (nullptr);
		vertexCount = countArray->GetArray();
	}
	
	GeometrySurface *surface = new GeometrySurface(kSurfaceValidTangents);
	if (nrmAccessor) surface->surfaceFlags |= kSurfaceValidNormals;
	if (colorAccessor) surface->surfaceFlags |= kSurfaceValidColors;
	
	for (machine a = 1; a < kMaxGeometryTexcoordCount; a++)
	{
		if (texcoordAccessor[a]) surface->texcoordCount = a + 1;
	}
			...
			if (v2)
			{
				...
				if (v3)
				{
					if (SquaredMag((v2->position - v1->position) % (v3->position - v1->position)) > 0.0F)
					{
						...
						v1->normal = (transform * v1->normal).Normalize();
						v2->normal = (transform * v2->normal).Normalize();
						v3->normal = (transform * v3->normal).Normalize();
	...
	if (surface->polygonList.First()) return (surface);
...
void Collada::Spline::BuildPathComponents(Path *path) const
{
	...
	if (accessor)
	{
		...
		for (machine a = 0; a < count; a++)
		{
			...
			for (machine b = 0; b < dimension; b++) (*p)[b] = value[b];
			p++;
		}
...
void Collada::Instance::Preprocess(void)
{
	...
	while (element)
	{
		if (element->GetElementType() == Collada::kElementBindMaterial) materialBindingList.Append(static_cast<Collada::BindMaterial *>(element));
		element = element->Next();
	}
}
	...
	if (resource)
	{
		...
		else if (type == Collada::kElementController)
		{
			...
			if (skin)
			{
				...
				if (!target) target = skin->GetSourceElement();
				
				if ((target) && (target->GetElementType() == Collada::kElementGeometry))
				{
					C4::MeshGeometry *mesh = static_cast<const Collada::Geometry *>(target)->BuildGeometry(scene, &materialBindingList, skin);
					if (mesh)
					{
						C4::SkinController *skinController = new C4::SkinController;
						mesh->SetController(skinController);
						return (mesh);
					}
				}
			}
	...
	while (element)
	{
		switch (element->GetElementType())
		{
			...
			case Collada::kElementLookat:
			{
				...
				Vector3D z = (p - lookat->GetTargetPosition()).Normalize();
				Vector3D x = (lookat->GetUpDirection() % z).Normalize();
...
void Collada::Node::AddToScene(C4::Node *node, C4::Node *superNode) const
{
	...
	if ((!controller) || (controller->GetControllerType() != kControllerSkin))
	{
		...
		{
			if ((type == C4::kNodeGeometry) && (Abs(Determinant(transform) - 1.0F) > 0.015625F))
			{
				const C4::Controller *controller = node->GetController();
				if ((!controller) || (controller->GetControllerType() != kControllerSkin))
				{
					...
					{
						C4::GeometryLevel *geometryLevel = object->GetGeometryLevel(level);
						geometryLevel->TransformGeometryLevel(transform);
					}
	...
	else
	{
		...
		if (identifier[0] != 0) node->SetNodeName(identifier);
	}
...
void Collada::Node::BuildNode(Collada::Scene *scene, C4::Node *superNode)
{
	...
	if (!primaryNode) primaryNode = new C4::Node;
...
void Collada::Scene::Preprocess(void)
{
	...
	while (element)
	{
		...
		{
			Collada::Node *node = static_cast<Collada::Node *>(element);
			node->SetNodeHash(Text::GetTextHash(node->GetNodeName()));
			count++;
		}
...
Collada::Node *Collada::Scene::FindNode(const char *name)
{
	...
	if ((element) && (element->GetElementType() == Collada::kElementNode)) return (static_cast<Collada::Node *>(element));
...
void Collada::Scene::BuildNodes(const Collada::Element *rootElement, C4::Node *superNode)
{
	...
	while (element)
	{
		...
		else if (type == Collada::kElementInstance)
		{
			...
			if ((subelement) && (subelement->GetElementType() == Collada::kElementScene)) BuildNodes(subelement, superNode);
		}
...
void Collada::Scene::RemoveDeadNodes(C4::Node *rootNode)
{
	...
	if ((rootNode->GetNodeType() == kNodeGroup) && (!rootNode->GetFirstSubnode())) delete rootNode;
}
...
void Collada::Matrix::ProcessContent(const char *& text)
{
	...
	for (machine i = 0; i < 3; i++)
	{
		...
		{
			ReadWhitespace(text);
			if (!ReadString(text, &string)) return;
...
void Collada::UpAxis::ProcessContent(const char *& text)
{
	...
	if (ReadString(text, &string))
	{
		if (string == "Z_UP") upAxis = kAxisZ;
		else if (string == "Y_UP") upAxis = kAxisY;
		else if (string == "X_UP") upAxis = kAxisX;
	}
}
...
void Collada::Texture::Preprocess(void)
{
	...
	if (!imageElement) imageElement = FindSubElement(imageIdentifier);
}
...
void Collada::Texture::BuildAttributeList(List<C4::Attribute> *attributeList) const
{
	...
	if (textureImage)
	{
		C4::Attribute *attribute = GetColladaContext()->NewTextureMapAttribute(textureType, textureImage->GetImageName(), nullptr);
		if (attribute) attributeList->Append(attribute);
	}
}
...
C4::MaterialObject *Collada::Material::GetMaterialObject(void)
{
	if (!materialObject)
	{
		...
		if ((flags & Collada::kImportReuseNamedMaterials) && (materialName[0] != 0))
		{
		...
		if ((flags & Collada::kImportMergeMaterials) && (materialObject))
		{
	...
	while (element)
	{
		...
		else if (type == Collada::kElementInput)
		{
			...
			if (input->GetInputSemantic() == Collada::kSemanticTexture)
			{
				...
				if ((texture) && (texture->GetElementType() == Collada::kElementTexture)) builder = texture;
			}
		}
		
		if (builder) static_cast<const MaterialAttributeElement *>(builder)->BuildAttributeList(material->GetAttributeList());
...
C4::MaterialObject *Collada::ShadingModel::BuildMaterial(void) const
{
	...
	for (machine a = 0; a < 2; a++)
	{
						...
						if (image)
						{
							...
							if (image->GetElementType() == Collada::kElementNewParam)
							{
								...
								if (sampler2D)
								{
									...
									if (surface) image = surface->GetImageElement();
								}
							}
							...
							if ((image) && (image->GetElementType() == Collada::kElementImage))
							{
								...
								C4::Attribute *attribute = GetColladaContext()->NewTextureMapAttribute(propertyType, name, sampler2D);
								if (attribute) material->AddAttribute(attribute);
							}
						}
					}
		...
		root = root->Next();
		if ((!root) || (root->GetElementType() != Collada::kElementExtra)) break;
		
		root = root->GetFirstSubnode();
		if ((!root) || (root->GetElementType() != Collada::kElementTechnique)) break;
	}
...
C4::MaterialObject *Collada::Effect::BuildMaterial(void) const
{
	...
	while (element)
	{
		if (element->GetElementType() == Collada::kElementCommonProfile)
		{
			...
			while (subelement)
			{
				if (subelement->GetElementType() == Collada::kElementTechnique) break;
				subelement = subelement->Next();
			}
...
{
	paramType = type;
	for (machine a = 0; a < 4; a++) floatValue[a] = 0.0F;
}
...
void Collada::Surface::Preprocess(void)
{
	...
	while (element)
	{
		if (element->GetElementType() == Collada::kElementInitFrom)
		{
			...
			if ((initElement) && (initElement->GetElementType() == Collada::kElementImage)) imageElement = static_cast<const Collada::Image *>(initElement);
			break;
		}
	...
	while (element)
	{
		...
		if (type == Collada::kElementSource)
		{
			if (!surfaceElement)
			{
				...
				if (newParam) surfaceElement = newParam->GetSurfaceElement();
			}
		}
...
void Collada::Wrap::ProcessContent(const char *& text)
{
	...
	if (ReadString(text, &string))
	{
		if (string == "CLAMP") wrapMode = kTextureClamp;
		else if (string == "BORDER") wrapMode = kTextureClampBorder;
		else if (string == "MIRROR") wrapMode = kTextureMirrorRepeat;
	}
}
...
void Collada::NewParam::Preprocess(void)
{
	...
	{
		Collada::ElementType type = element->GetElementType();
		if (type == Collada::kElementSurface) surfaceElement = static_cast<const Collada::Surface *>(element);
		else if (type == Collada::kElementSampler2D) sampler2DElement = static_cast<const Collada::Sampler2D *>(element);
...
C4::Light *Collada::Directional::BuildLight(void) const
{
	...
	while (element)
	{
		if (element->GetElementType() == Collada::kElementParam)
		{
			...
			if (type == Collada::kParamColor) color = param->GetColorValue();
		}
...
C4::Light *Collada::Spot::BuildLight(void) const
{
	...
	while (element)
	{
		if (element->GetElementType() == Collada::kElementParam)
		{
			...
			if (type == Collada::kParamColor) color = param->GetColorValue();
			else if (type == Collada::kParamFalloffAngle) apex = Tan(param->GetFloatValue(0) * 0.5F);
		}
...
Collada::Scene *Collada::Root::FindSceneElement(void) const
{
	...
	{
		Collada::ElementType type = element->GetElementType();
		if (type == Collada::kElementScene) return (static_cast<Collada::Scene *>(element));
...
const Collada::Library *Collada::Root::FindAnimationElement(void) const
{
	...
	while (element)
	{
		...
		if (type == Collada::kElementLibrary)
		{
			...
			if (library->GetLibraryType() == Collada::kLibraryAnimation) return (library);
		}
...
void ColladaResource::Preprocess(void)
{
...
Collada::Element *ColladaContext::ConstructElement(const String<Collada::kMaxIdentifierLength>& elementType)
{
	...
	if (Text::CompareText(elementType, "library", 7))
	{
		...
		if (type[0] == 0) return (new Collada::Library);
	...
	if (Text::CompareText(elementType, "instance", 8))
	{
		...
		if ((type[0] == 0) || (Text::CompareText(type, "_geometry"))
						   ...
						   || (Text::CompareText(type, "_material"))) return (new Collada::Instance);
	}
...
bool ColladaContext::ProcessAttributes(const char *& text, Collada::Element *element)
{
	for (;;)
	{
		...
		ReadWhitespace(text);
		if (!ReadIdentifier(text, &identifier)) break;
		...
		ReadWhitespace(text);
		if (!ReadAttribute(text, &attribute)) break;
		...
		else if (identifier == "type")
		{
			if (type == Collada::kElementLibrary)
			{
				Collada::LibraryType type = 0;
				
				if (attribute == "GEOMETRY") type = Collada::kLibraryGeometry;
				else if (attribute == "MATERIAL") type = Collada::kLibraryMaterial;
				else if (attribute == "IMAGE") type = Collada::kLibraryImage;
				else if (attribute == "TEXTURE") type = Collada::kLibraryTexture;
				else if (attribute == "LIGHT") type = Collada::kLibraryLight;
				else if (attribute == "CONTROLLER") type = Collada::kLibraryController;
				else if (attribute == "ANIMATION") type = Collada::kLibraryAnimation;
				
				static_cast<Collada::Library *>(element)->SetLibraryType(type);
			}
			else if (type == Collada::kElementNode)
			{
				if (attribute == "JOINT") static_cast<Collada::Node *>(element)->SetNodeType(Collada::kNodeJoint);
			}
		}
		else if (identifier == "name")
		{
			...
			{
				Collada::ParamType type = 0;
				
				if (attribute == "COLOR") type = Collada::kParamColor;
				else if (attribute == "DIFFUSE") type = Collada::kParamDiffuse;
				else if (attribute == "SPECULAR") type = Collada::kParamSpecular;
				else if (attribute == "EMISSION") type = Collada::kParamEmission;
				else if (attribute == "SHININESS") type = Collada::kParamShininess;
				else if (attribute == "BUMP") type = Collada::kParamBump;
				
				if (type != 0) static_cast<Collada::Param *>(element)->SetParamType(type);
			}
		}
		else if (identifier == "count")
		{
			if (type == Collada::kElementNameArray) static_cast<Collada::NameArray *>(element)->SetArraySize(Text::StringToInteger(attribute));
			else if (type == Collada::kElementFloatArray) static_cast<Collada::FloatArray *>(element)->SetArraySize(Text::StringToInteger(attribute));
			else if (type == Collada::kElementAccessor) static_cast<Collada::Accessor *>(element)->SetAccessCount(Text::StringToInteger(attribute));
			else if ((type == Collada::kElementFaces) || (type == Collada::kElementCombiner)) static_cast<Collada::InterleavingElement *>(element)->SetItemCount(Text::StringToInteger(attribute));
		}
		else if (identifier == "stride")
		{
			if (type == Collada::kElementAccessor) static_cast<Collada::Accessor *>(element)->SetAccessStride(Text::StringToInteger(attribute));
		}
		else if (identifier == "source")
		{
			if (type == Collada::kElementAccessor) static_cast<Collada::Accessor *>(element)->SetSourceIdentifier(attribute);
			else if (type == Collada::kElementInput) static_cast<Collada::Input *>(element)->SetSourceIdentifier(attribute);
			else if (type == Collada::kElementChannel) static_cast<Collada::Channel *>(element)->SetSourceIdentifier(attribute);
			else if (type == Collada::kElementSkin) static_cast<Collada::Skin *>(element)->SetSourceIdentifier(attribute);
			else if (type == Collada::kElementImage) static_cast<Collada::Image *>(element)->SetSourceString(attribute);
		}
		else if ((identifier == "idx") || (identifier == "offset"))
		{
			if (type == Collada::kElementInput) static_cast<Collada::Input *>(element)->SetInputIndex(Text::StringToInteger(attribute));
		}
		...
		else if (identifier == "profile")
		{
			if (type == Collada::kElementTechnique)
			{
				...
				if (attribute == "COMMON") profile = Collada::kTechniqueCommon;
		...
		{
			if (type == Collada::kElementInstance) static_cast<Collada::Instance *>(element)->SetResourceIdentifier(attribute);
		}
		else if (identifier == "material")
		{
			if (type == Collada::kElementFaces) static_cast<Collada::Faces *>(element)->SetMaterialIdentifier(attribute);
		}
		else if (identifier == "target")
		{
			if (type == Collada::kElementController) static_cast<Collada::Controller *>(element)->SetTargetIdentifier(attribute);
			else if (type == Collada::kElementChannel) static_cast<Collada::Channel *>(element)->SetTargetIdentifier(attribute);
			else if (type == Collada::kElementInstance) static_cast<Collada::Instance *>(element)->SetResourceIdentifier(attribute);
		}
		else if (identifier == "method")
		{
			if (type == Collada::kElementMorph)
			{
				...
				if (attribute == "RELATIVE") method = Collada::kMethodRelative;
		...
		{
			if (type == Collada::kElementUnit) static_cast<Collada::Unit *>(element)->SetMeterScale(Text::StringToFloat(attribute));
		}
	}
...
bool ColladaContext::ProcessContent(const char *& text, Collada::Element *element)
{
	...
	for (;;)
	{
		...
		if (subelement) element->AddSubnode(subelement);
	}
...
Collada::Element *ColladaContext::ProcessElement(const char *& text)
{
	...
	{
		Collada::Element *element = ConstructElement(elementType);
...
Collada::Root *ColladaContext::ProcessFile(const char *text)
{
	...
	if (FindTag(text)) element = ProcessElement(text);
...
C4::Attribute *ColladaContext::NewTextureMapAttribute(Collada::ParamType type, const char *name, const Collada::Sampler2D *sampler)
{
	...
	if (name[0] != 0)
	{
		...
		if (type == Collada::kParamDiffuse) attribute = new TextureMapAttribute(virtualName);
		else if (type == Collada::kParamBump) attribute = new NormalMapAttribute(virtualName);
		else if (type == Collada::kParamSpecular) attribute = new GlossMapAttribute(virtualName);
		else if (type == Collada::kParamEmission) attribute = new EmissionMapAttribute(virtualName);
	}
...
ColladaWindow::ColladaWindow(Editor *editor, const char *name) : Window("ColladaImporter/Import")
{
	worldEditor = editor;
	resourceName = name;
	
	SetWindowTitle((String<127>(GetWindowTitle()) += name) += ColladaResource::GetDescriptor()->GetExtension());
	editor->AddSubwindow(this);
}

ColladaWindow::~ColladaWindow()
{
}

void ColladaWindow::Preprocess(void)
{
	Window::Preprocess();
	...
	importTexturesBox = static_cast<CheckWidget *>(FindWidget("Texture"));
	mergeMaterialsBox = static_cast<CheckWidget *>(FindWidget("Merge"));
	reuseNamedMaterialsBox = static_cast<CheckWidget *>(FindWidget("Reuse"));
	
	scaleBox = static_cast<EditTextWidget *>(FindWidget("Scale"));
}

bool ColladaWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
...
void ColladaWindow::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetActivate)
	{
		if (widget == okayButton)
		{
			ColladaResource *collada = ColladaResource::Get(resourceName, 0, ThePluginMgr->GetImportCatalog());
			if (collada)
			{
				unsigned_int32 flags = 0;
				if (importTexturesBox->GetValue() != 0) flags |= Collada::kImportTextureMaps;
				if (mergeMaterialsBox->GetValue() != 0) flags |= Collada::kImportMergeMaterials;
				if (reuseNamedMaterialsBox->GetValue() != 0) flags |= Collada::kImportReuseNamedMaterials;
				
				float scale = Text::StringToFloat(scaleBox->GetText());
				if (scale < K::min_float) scale = 1.0F;
				
				ColladaContext context(worldEditor->GetEditorObject(), flags, scale);
				...
				if (root)
				{
					...
					if (scene)
					{
						...
						if (zone->GetFirstSubnode()) worldEditor->ImportScene(zone);
						delete zone;
					}
...
void ColladaImporter::ImportGeometry(Editor *editor, const char *name, const GeometryImportData *importData)
{
	new ColladaWindow(editor, name);
}

bool ColladaImporter::WriteAnimationResource(const char *name, const char *buffer, unsigned_int32 size)
{
	File			file;
	ResourcePath	path;
	
	TheResourceMgr->GetGenericCatalog()->GetResourcePath(AnimationResource::GetDescriptor(), name, &path);
...
bool ColladaImporter::GenerateAnimation(const char *name, const Model *model, const AnimationImportData *importData, Collada::Scene *scene, const Collada::Library *animation)
{
	Matrix3D		rotation;
	...
	for (machine a = 0; a < nodeCount; a++)
	{
		...
		const char *name = modelNode->GetNodeName();
		if (name)
		{
			const Collada::Node *importNode = scene->FindNode(name);
			if ((importNode) || (importData->importFlags & kAnimationImportPreserveMissing))
			{
				animationNodeArray.AddElement(AnimationNode(modelNode, importNode, Text::GetTextHash(name)));
			}
		}
	}
	...
	for (machine a = 0; a < nodeCount; a++)
	{
		unsigned_int32 hash = animationNodeArray[a].hashValue;
	...
	unsigned_int32 transformTrackSize = sizeof(TransformTrackHeader) + nodeCount * frameCount * sizeof(TransformFrameData);
	unsigned_int32 storageSize = headerSize + hashTableSize + transformTrackSize;
	char *storage = new char[storageSize];
	
	AnimationHeader *animationHeader = reinterpret_cast<AnimationHeader *>(storage);
	...
	animationHeader->trackData[0].trackOffset = headerSize + hashTableSize;
	...
	transformTrackHeader->frameCount = frameCount;
	transformTrackHeader->frameDuration = frameDuration;
	...
	for (machine frame = 0; frame < frameCount; frame++)
	{
		...
		TransformFrameData *transformFrameData = const_cast<TransformFrameData *>(transformTrackHeader->GetFrameData() + frame * nodeCount);
		
		for (machine a = 0; a < nodeCount; a++)
		{
			const AnimationNode *animationNode = &animationNodeArray[a];
			const C4::Node *modelNode = animationNode->modelNode;
			const Collada::Node *importNode = animationNode->importNode;
				...
				if (flags & (kAnimationImportAnchorXY | kAnimationImportAnchorZ | kAnimationImportFreezeRoot))
				{
					if (frame != 0)
					{
						...
						else
						{
							...
							if (flags & kAnimationImportAnchorZ) position.z = p.z;
						}
					}
					else
					{
	...
	bool result = WriteAnimationResource(name, storage, storageSize);
...
bool ColladaImporter::ImportAnimation(const char *name, Model *model, const AnimationImportData *importData)
{
	bool result = false;
	
	ColladaResource *collada = ColladaResource::Get(name, 0, ThePluginMgr->GetImportCatalog());
	if (collada)
	{
		...
		if (root)
		{
			...
			if ((scene) && (animation)) result = GenerateAnimation(name, model, importData, scene, animation);
