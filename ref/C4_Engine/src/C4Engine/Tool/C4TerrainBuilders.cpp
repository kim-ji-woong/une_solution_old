...
Setting *FlatPlaneTerrainBuilder::GetSetting(int32 index) const
{
	...
	if (index < count) return (TerrainBuilder::GetSetting(index));
...
unsigned_int8 *TargaHeightFieldTerrainBuilder::GenerateHeightField(const TargaHeader *header, int32 fieldWidth, int32 fieldHeight)
{
	int32 pixelDepth = header->pixelDepth;
	int32 imageType = header->imageType;
	
	if ((header->colorMapType != 0) || ((pixelDepth != 8) && (pixelDepth != 16) && (pixelDepth != 24) && (pixelDepth != 32)) || ((imageType != 2) && (imageType != 3) && (imageType != 10) && (imageType != 11))) return (nullptr);
	
	int32 imageWidth = header->width;
	int32 imageHeight = header->height;
	
	int32 imageOffset = 0;
	int32 imageRowSize = imageWidth;
	
	if (header->imageDescriptor & 0x20)
	{
		imageOffset = imageRowSize * (imageHeight - 1);
	...
	if ((imageType & 8) == 0)
	{
		if (pixelDepth == 8)
		{
			...
			{
				for (machine x = 0; x < fieldWidth; x++) field[x] = data[x];
		...
		else if (pixelDepth == 16)
		{
			...
			for (machine y = 0; y < fieldHeight; y++)
			{
				for (machine x = 0; x < fieldWidth; x++)
				{
					...
					field[x] = Max(Max(red, green), blue);
					data += 2;
				}
				
				field += fieldWidth;
				data += (imageRowSize - fieldWidth) * 2;
			}
		}
		else if (pixelDepth == 24)
		{
			...
			for (machine y = 0; y < fieldHeight; y++)
			{
				for (machine x = 0; x < fieldWidth; x++)
				{
					field[x] = Max(Max(data[0], data[1]), data[2]);
					data += 3;
				}
				
				field += fieldWidth;
				data += (imageRowSize - fieldWidth) * 3;
			}
		}
		else
		{
			...
			for (machine y = 0; y < fieldHeight; y++)
			{
				...
				field += fieldWidth;
				data += (imageRowSize - fieldWidth) * 4;
			}
		}
	}
	else
	{
		int32 pixelCount = imageWidth * imageHeight;
		Color4C *pixelImage = new Color4C[pixelCount];
		Color4C *image = pixelImage;
		...
		else Image::DecompressImageRLE_RGB32(data, image, pixelCount);
		...
		for (machine y = 0; y < fieldHeight; y++)
		{
			for (machine x = 0; x < fieldWidth; x++)
			{
				const Color4C& c = *image;
				field[x] = Max(Max(c.GetRed(), c.GetGreen()), c.GetBlue());
				image++;
			}
			
			field += fieldWidth;
			image += (imageRowSize - fieldWidth) * 4;
		}
...
bool TargaHeightFieldTerrainBuilder::BuildTerrain(TerrainBlock *block)
{
	...
	TargaResource *resource = TargaResource::Get(imageName, 0, ThePluginMgr->GetImportCatalog());
	if (resource)
	{
		const TargaHeader *header = resource->GetTargaHeader();
		
		int32 blockSizeX = header->width >> kTerrainLogDimension;
		int32 blockSizeY = header->height >> kTerrainLogDimension;
		if ((blockSizeX > 0) && (blockSizeY > 0))
		{
			...
			unsigned_int8 *field = GenerateHeightField(header, blockSizeX << kTerrainLogDimension, blockSizeY << kTerrainLogDimension);
			if (field)
			{
...
bool RawVoxelMapTerrainBuilder::BuildTerrain(TerrainBlock *block)
{
	...
	if (resource)
	{
		...
		int32 maxVoxelCount = resource->GetSize() >> 2;
			...
			for (machine a = 0; a < maxVoxelCount; a++) Reverse(&const_cast<float *>(data)[a]);
			...
			for (machine k = 0; k < blockSizeZ; k++)
			{
				for (machine j = 0; j < blockSizeY; j++)
				{
					for (machine i = 0; i < blockSizeX; i++)
					{
						...
						block->StoreVoxels(kTerrainChannelDensity, Integer3D(i, j, k), voxel, kTerrainDimension, kTerrainDimension * kTerrainDimension);
					}
				}
			}
...
BuildTerrainWindow::BuildTerrainWindow(Editor *editor, TerrainBlock *block) : Window("WorldEditor/terrain/Build")
{
	...
	for (machine a = 0; a < kTerrainBuilderCount; a++) terrainBuilder[a] = nullptr;
}

BuildTerrainWindow::~BuildTerrainWindow()
{
	for (machine a = kTerrainBuilderCount - 1; a >= 0; a--) delete terrainBuilder[a];
}
...
bool BuildTerrainWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
...
void BuildTerrainWindow::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	else if (eventType == kEventWidgetChange)
	{
		if (widget == builderList)
		{
			...
			if (!currentBuilder)
			{
				if (index == 0) currentBuilder = new FlatPlaneTerrainBuilder;
				else if (index == 1) currentBuilder = new TargaHeightFieldTerrainBuilder;
				else if (index == 2) currentBuilder = new RawHeightFieldTerrainBuilder;
				else if (index == 3) currentBuilder = new RawVoxelMapTerrainBuilder;
...
void TerrainProgressWindow::TerrainJob(Job *job, void *cookie)
{
	...
	if (window->terrainBuilder->BuildTerrain(window->blockNode))
	{
		...
		job->SetJobMagnitude(size0.x * size0.y * size0.z + size1.x * size1.y * size1.z / 8 + size2.x * size2.y * size2.z / 64);
		int32 progress = 0;
		...
		for (machine k = 0; k < size2.z; k += 4)
		{
			for (machine j = 0; j < size2.y; j += 4)
			{
				for (machine i = 0; i < size2.x; i += 4)
				{
					...
					if (object->GetGeometryLevel(0)->GetFaceCount() != 0) window->geometryList.Append(geometry);
...
bool TerrainProgressWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyDown)
	{
...
{
	if ((eventData->eventType == kEventWidgetActivate) && (widget == stopButton)) TheJobMgr->CancelJob(&terrainJob);
}
