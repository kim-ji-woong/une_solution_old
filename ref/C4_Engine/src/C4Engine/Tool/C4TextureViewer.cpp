...
namespace
{
	const float kTextureDisplayPadding = 64.0F;
}
...
static float GetTextureDisplayWidth(const TextureHeader *header)
{
	if (header->textureType != kTextureCube) return (Fmin((float) header->imageWidth, TheInterfaceMgr->GetDesktopSize().y - kTextureDisplayPadding));
	return ((float) header->imageWidth * 4.0F);
}

static float GetTextureDisplayHeight(const TextureHeader *header)
{
	if (header->textureType != kTextureCube) return (Fmin((float) header->imageHeight, TheInterfaceMgr->GetDesktopSize().y - kTextureDisplayPadding));
	return ((float) header->imageHeight * 3.0F);
}


TextureWindow::TextureWindow(const char *name, TextureResource *resource, const TextureHeader *header) :
		Window(Vector2D(Fmax(GetTextureDisplayWidth(header), 128.0F) + 196.0F, Fmax(GetTextureDisplayHeight(header), 196.0F) + 8.0F), nullptr, kWindowCloseBox | kWindowBackground | kWindowCenter | kWindowStrip)
{
	resourceName = name;
...
void TextureWindow::Preprocess(void)
{
	...
	{
		float limit = TheInterfaceMgr->GetDesktopSize().y - kTextureDisplayPadding;
		Vector2D size(Fmin((float) textureHeader->imageWidth, limit), Fmin((float) textureHeader->imageHeight, limit));
		
		BorderWidget *border = new BorderWidget(size);
		border->SetWidgetPosition(Point3D(4.0F, 4.0F, 0.0F));
		AddNewSubnode(border);
		
		colorBackground = new QuadWidget(size, ColorRGBA(0.0F, 0.0F, 0.0F, 1.0F));
		colorBackground->SetWidgetPosition(Point3D(4.0F, 4.0F, 0.0F));
		AddNewSubnode(colorBackground);
		
		checkerBackground = new ImageWidget(size, "C4/checker");
		checkerBackground->SetWidgetPosition(Point3D(4.0F, 4.0F, 0.0F));
		checkerBackground->SetImageScale(Vector2D(size.x * 0.0625F, size.y * 0.0625F));
		checkerBackground->Hide();
		AddNewSubnode(checkerBackground);
		
		textureImage[0] = new ImageWidget(size, resourceName);
		textureImage[0]->SetImageBlendState(kBlendReplace);
		AddNewSubnode(textureImage[0]);
		
		if (type == kTexture3D) textureImage[0]->SetImagePCoordinate(0.5F / (float) textureHeader->imageDepth); 
	}
	else 
	{ 
		float width = (float) textureHeader->imageWidth; 
		Vector2D totalSize(width * 4.0F, width * 3.0F);
		Vector2D faceSize(width, width); 
		
		BorderWidget *border = new BorderWidget(totalSize);
		border->SetWidgetPosition(Point3D(4.0F, 4.0F, 0.0F));
		AddNewSubnode(border); 
		
		colorBackground = new QuadWidget(totalSize, ColorRGBA(0.0F, 0.0F, 0.0F, 1.0F));
		colorBackground->SetWidgetPosition(Point3D(4.0F, 4.0F, 0.0F));
		AddNewSubnode(colorBackground); 
		
		checkerBackground = new ImageWidget(totalSize, "C4/checker");
		checkerBackground->SetWidgetPosition(Point3D(4.0F, 4.0F, 0.0F));
		checkerBackground->SetImageScale(Vector2D(totalSize.x * 0.0625F, totalSize.y * 0.0625F));
		checkerBackground->Hide();
		AddNewSubnode(checkerBackground);
		
		textureImage[0] = new ImageWidget(faceSize, resourceName);
		textureImage[0]->SetImageBlendState(kBlendReplace);
		AddNewSubnode(textureImage[0]);
		
		textureImage[1] = new ImageWidget(faceSize, resourceName);
		textureImage[1]->SetImageBlendState(kBlendReplace);
		textureImage[1]->SetCubeFaceIndex(1);
		AddNewSubnode(textureImage[1]);
		
		textureImage[2] = new ImageWidget(faceSize, resourceName);
		textureImage[2]->SetImageBlendState(kBlendReplace);
		textureImage[2]->SetCubeFaceIndex(2);
		AddNewSubnode(textureImage[2]);
		
		textureImage[3] = new ImageWidget(faceSize, resourceName);
		textureImage[3]->SetImageBlendState(kBlendReplace);
		textureImage[3]->SetCubeFaceIndex(3);
		AddNewSubnode(textureImage[3]);
		
		textureImage[4] = new ImageWidget(faceSize, resourceName);
		textureImage[4]->SetImageBlendState(kBlendReplace);
		textureImage[4]->SetCubeFaceIndex(4);
		AddNewSubnode(textureImage[4]);
		
		textureImage[5] = new ImageWidget(faceSize, resourceName);
		textureImage[5]->SetImageBlendState(kBlendReplace);
		textureImage[5]->SetCubeFaceIndex(5);
		AddNewSubnode(textureImage[5]);
	}
	...
	if (type == kTextureCube) flipBox->Disable();
...
void TextureWindow::SetImagePosition(void)
{
	...
	{
		float limit = TheInterfaceMgr->GetDesktopSize().y - kTextureDisplayPadding;
		float x = Fmin((float) textureHeader->imageWidth, limit);
		float y = Fmin((float) textureHeader->imageHeight, limit);
		float w = Fmin((float) Max(textureHeader->imageWidth >> mipmapLevel, 1), limit);
		float h = Fmin((float) Max(textureHeader->imageHeight >> mipmapLevel, 1), limit);
		
		textureImage[0]->SetWidgetSize(Vector2D(w, h));
		textureImage[0]->SetWidgetPosition(Point3D((x - w) * 0.5F + 4.0F, (y - h) * 0.5F + 4.0F, 0.0F));
		textureImage[0]->Invalidate();
	}
	else
	{
		float s = (float) Max(textureHeader->imageWidth >> mipmapLevel, 1);
		float cx = (float) textureHeader->imageWidth * 2.0F - s + 4.0F;
		float cy = (float) textureHeader->imageWidth * 1.5F - s * 0.5F + 4.0F;
		Vector2D size(s, s);
...
bool TextureWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyCommand)
	{
		...
		if (code == 'M')
		{
			...
			if (level >= textureHeader->mipmapCount) level = 0;
			mipmapLevel = level;
		...
		else if (code == 'D')
		{
			...
			if (type == kTextureArray2D)
			{
				...
				if (p >= (float) texture->GetTextureDepth()) p = 0.0F;
	...
	if (eventData->eventType == kEventWidgetChange)
	{
		...
		else if (widget == backgroundMenu)
		{
			...
			if (selection < 3)
			{
				...
				if (selection == 0) colorBackground->SetWidgetColor(ColorRGBA(0.0F, 0.0F, 0.0F, 1.0F));
				else if (selection == 1) colorBackground->SetWidgetColor(K::white);
				else colorBackground->SetWidgetColor(ColorRGBA(0.5F, 0.5F, 0.5F, 1.0F));
			}
			else
			{
