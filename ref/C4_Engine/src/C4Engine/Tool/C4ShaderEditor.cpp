...
const Triangle ProcessWidget::processTriangle[16] =
{
	{{ 0,  4,  1}},
	{{ 1,  4,  5}}, 
	{{ 1,  5,  2}},
...
const ConstPoint2D ProcessWidget::backgroundVertex[4] =
{
	{-9.0F, -5.0F},
	{-9.0F, 111.0F},
	{129.0F, -5.0F},
...
const ConstPoint2D ProcessWidget::backgroundTexcoord[4] =
{
	{0.0F, 116.0F},
	{0.0F, 0.0F},
	{138.0F, 116.0F},
...
const ConstPoint2D ProcessWidget::outputVertex[4] =
{
	{113.0F, 41.0F},
	{113.0F, 58.0F},
	{130.0F, 41.0F},
...
const ConstPoint2D ProcessWidget::outputTexcoord[4] =
{
	{0.0F, 17.0F},
	{0.0F, 0.0F},
	{17.0F, 17.0F},
...
const Triangle ProcessWidget::portBoxTriangle[kMaxProcessPortCount * 10] =
{
	{{ 0,  1,  2}},
	{{ 0,  2,  3}},
	{{ 4,  8,  5}},
...
const ConstColorRGB RouteWidget::swizzleColor[12] =
{
...
const Triangle RouteWidget::swizzleTriangle[10] =
{
	{{ 0,  1,  2}},
	{{ 0,  2,  3}},
	{{ 4,  8,  5}},
...
const Triangle ShaderSectionWidget::sectionTriangle[21] =
{
	{{ 0,  1,  2}},
	{{ 0,  2,  3}},
	{{ 4,  5,  6}},
...
CreateShaderUndoData::CreateShaderUndoData(ProcessWidget *process)
{
	createdProcessList.Append(new ProcessReference(process));
}

CreateShaderUndoData::CreateShaderUndoData(RouteWidget *route)
{
	createdRouteList.Append(new RouteReference(route));
}

CreateShaderUndoData::CreateShaderUndoData(ShaderSectionWidget *section)
{
	createdSectionList.Append(new ShaderSectionReference(section));
}

CreateShaderUndoData::CreateShaderUndoData(const List<ProcessWidget> *processList, const List<RouteWidget> *routeList, const List<ShaderSectionWidget> *sectionList)
{
...
CreateShaderUndoData::~CreateShaderUndoData()
{
}

void CreateShaderUndoData::Undo(ShaderEditor *shaderEditor)
{
...
DeleteShaderUndoData::DeleteShaderUndoData(List<ProcessWidget> *processList, List<RouteWidget> *routeList, List<ShaderSectionWidget> *sectionList)
{
	if (processList)
	{
		for (;;)
...
DeleteShaderUndoData::~DeleteShaderUndoData()
{
...
void DeleteShaderUndoData::Undo(ShaderEditor *shaderEditor)
{
	for (;;)
	{
...
MoveShaderUndoData::MoveShaderUndoData(const List<ProcessWidget> *processList, const List<ShaderSectionWidget> *sectionList)
{
	ProcessWidget *processWidget = processList->First();
	while (processWidget)
	{
...
MoveShaderUndoData::~MoveShaderUndoData()
{
}

MoveShaderUndoData::MovedProcessReference::MovedProcessReference(ProcessWidget *element) : ProcessReference(element)
{
	position = element->GetShaderProcess()->GetProcessPosition();
}

MoveShaderUndoData::MovedSectionReference::MovedSectionReference(ShaderSectionWidget *element) : ShaderSectionReference(element)
{
	position = element->GetSectionProcess()->GetProcessPosition();
}

void MoveShaderUndoData::Undo(ShaderEditor *shaderEditor)
{
...
ResizeShaderUndoData::ResizeShaderUndoData(ShaderSectionWidget *widget)
{
...
ResizeShaderUndoData::~ResizeShaderUndoData()
{
}

void ResizeShaderUndoData::Undo(ShaderEditor *shaderEditor)
{
...
RouteShaderUndoData::RouteShaderUndoData(const List<RouteWidget> *selectionList)
{
...
RouteShaderUndoData::~RouteShaderUndoData()
{
}

RouteShaderUndoData::DetailedReference::DetailedReference(RouteWidget *widget) : RouteReference(widget)
{
	flags = widget->GetShaderRoute()->GetRouteFlags();
}

void RouteShaderUndoData::Undo(ShaderEditor *shaderEditor)
{
...
ProcessWidget::ProcessWidget(ShaderEditor *editor, Process *process, const ProcessRegistration *registration) :
		...
		backgroundTextureMap("ShaderEditor/Graph"),
		backgroundRenderable(kRenderTriangleStrip),
		outputDiffuseAttribute(kAttributeMutable),
		outputTextureMap("ShaderEditor/Output"),
		outputRenderable(kRenderTriangleStrip),
		portTextureMap(&portTextureHeader, portTextureImage),
		portRenderable(kRenderIndexedTriangles)
{
	...
	processRenderable.SetAttributeArray(kArrayVertex, processVertex);
	processRenderable.SetAttributeArray(kArrayColor0, processColor);
	processRenderable.SetTriangleArray(8, processTriangle);
	...
	backgroundDiffuseAttribute.SetDiffuseColor(ColorRGBA(rb, 1.0F, rb, 1.0F));
	
	for (machine a = 0; a < 8; a++) processColor[a].Set(0.0F, 0.0F, 0.0F, 1.0F);
	const ColorRGB& hiliteColor = TheInterfaceMgr->GetInterfaceColor(kInterfaceColorHilite).GetColorRGB();
	for (machine a = 8; a < 16; a++) processColor[a].Set(hiliteColor, 0.625F);
	...
	if (portCount != 0)
	{
		...
		portRenderable.SetAttributeArray(kArrayVertex, portVertex);
		portRenderable.SetAttributeArray(kArrayColor0, portColor);
		portRenderable.SetAttributeArray(kArrayTexture0, portTexcoord);
		portRenderable.SetTriangleArray(portCount * 10, portBoxTriangle);
		portAttributeList.Append(&portTextureMap);
		...
		{
			UpdatePortColor(port, false);
			
			ColorRGBA *color = portColor + port * 12;
			for (machine a = 4; a < 12; a++) color[a].Set(0.0F, 0.0F, 0.0F, 1.0F);
...
void ProcessWidget::UpdatePortColor(int32 port, bool hilite)
{
	ColorRGBA *color = portColor + port * 12;
	if (hilite)
	{
		const ColorRGB& hiliteColor = TheInterfaceMgr->GetInterfaceColor(kInterfaceColorHilite).GetColorRGB();
		for (machine a = 0; a < 4; a++) color[a].Set(hiliteColor, 1.0F);
	}
	else
	{
		for (machine a = 0; a < 4; a++) color[a].Set(0.9325F, 0.9325F, 0.9325F, 1.0F);
	}
}
...
void ProcessWidget::UpdateContent(void)
{
	...
	else if (base == kProcessTextureMap)
	{
		...
		AddNewSubnode(textureBox);
		
		if ((texture) && (texture->GetTextureType() == kTexture2D))
		{
			float h = textureBox->GetVertexTexcoord2D(3).y;
			for (machine a = 0; a < 4; a++)
			{
				const Point2D& texcoord = textureBox->GetVertexTexcoord2D(a);
				textureBox->SetVertexTexcoord2D(a, Point2D(texcoord.x, h - texcoord.y));
			}
		}
...
WidgetPart ProcessWidget::TestPosition(const Point3D& position) const
{
	...
	if ((portCount != 0) && (position.x < kPortBoxWidth * 0.5F + kPortBoxOutdent))
	{
		...
		{
			if ((position.y >= y) && (position.y <= y + kPortBoxHeight)) return (kWidgetPartPort + a);
			y += dy;
		}
	}
...
void ProcessWidget::Build(void)
{
	...
	processVertex[0].Set(0.0F, 0.0F);
	processVertex[1].Set(0.0F, kProcessBoxHeight);
	processVertex[2].Set(kProcessBoxWidth, kProcessBoxHeight);
	processVertex[3].Set(kProcessBoxWidth, 0.0F);
	
	processVertex[4].Set(-thickness, -thickness);
	processVertex[5].Set(-thickness, kProcessBoxHeight + thickness);
	processVertex[6].Set(kProcessBoxWidth + thickness, kProcessBoxHeight + thickness);
	processVertex[7].Set(kProcessBoxWidth + thickness, -thickness);
	
	processVertex[8].Set(-thickness, -thickness);
	processVertex[9].Set(-thickness, kProcessBoxHeight + thickness);
	processVertex[10].Set(kProcessBoxWidth + thickness, kProcessBoxHeight + thickness);
	processVertex[11].Set(kProcessBoxWidth + thickness, -thickness);
	
	thickness = Fmax(kProcessBoxThickness, viewportScale * 4.0F);
	
	processVertex[12].Set(-thickness, -thickness);
	processVertex[13].Set(-thickness, kProcessBoxHeight + thickness);
	processVertex[14].Set(kProcessBoxWidth + thickness, kProcessBoxHeight + thickness);
	processVertex[15].Set(kProcessBoxWidth + thickness, -thickness);
	...
	if (portCount != 0)
	{
		...
		Point2D *vertex = portVertex;
		Point2D *texcoord = portTexcoord;
		
		for (machine a = 0; a < portCount; a++)
		{
			vertex[0].Set(kPortBoxWidth * -0.5F + kPortBoxOutdent, y);
			vertex[1].Set(kPortBoxWidth * -0.5F + kPortBoxOutdent, y + kPortBoxHeight);
			vertex[2].Set(kPortBoxWidth * 0.5F + kPortBoxOutdent, y + kPortBoxHeight);
			vertex[3].Set(kPortBoxWidth * 0.5F + kPortBoxOutdent, y);
			
			vertex[4].Set(kPortBoxWidth * -0.5F + kPortBoxOutdent, y);
			vertex[5].Set(kPortBoxWidth * -0.5F + kPortBoxOutdent, y + kPortBoxHeight);
			vertex[6].Set(kPortBoxWidth * 0.5F + kPortBoxOutdent, y + kPortBoxHeight);
			vertex[7].Set(kPortBoxWidth * 0.5F + kPortBoxOutdent, y);
			
			vertex[8].Set(kPortBoxWidth * -0.5F + kPortBoxOutdent - viewportScale, y - viewportScale);
			vertex[9].Set(kPortBoxWidth * -0.5F + kPortBoxOutdent - viewportScale, y + kPortBoxHeight + viewportScale);
			vertex[10].Set(kPortBoxWidth * 0.5F + kPortBoxOutdent + viewportScale, y + kPortBoxHeight + viewportScale);
			vertex[11].Set(kPortBoxWidth * 0.5F + kPortBoxOutdent + viewportScale, y - viewportScale);
			...
			if (shaderProcess->GetPortFlags(a) & kProcessPortOptional)
			{
				...
				texcoord[1].Set(0.0F, h);
				texcoord[2].Set(w, h);
				texcoord[3].Set(w, 0.0F);
				
				texcoord[4].Set(0.0F, 0.0F);
				texcoord[5].Set(0.0F, h);
				texcoord[6].Set(w, h);
				texcoord[7].Set(w, 0.0F);
				
				texcoord[8].Set(-s, -s);
				texcoord[9].Set(-s, h + s);
				texcoord[10].Set(w + s, h + s);
				texcoord[11].Set(w + s, -s);
			}
			else
			{
				for (machine b = 0; b < 12; b++) texcoord[b].Set(0.125F, 0.125F);
			}
...
void ProcessWidget::Render(List<Renderable> *renderList)
{
	...
	if (outputRenderable.GetVertexCount() != 0) renderList->Append(&outputRenderable);
	if (portRenderable.GetVertexCount() != 0) renderList->Append(&portRenderable);
...
RouteWidget::RouteWidget(ShaderEditor *editor, Route *route) :
		...
		routeDiffuseAttribute(ColorRGBA(0.0F, 0.0F, 0.0F, 1.0F)),
		routeTextureMapAttribute(&routeTextureHeader, routeTextureImage),
		selectionDiffuseAttribute(TheInterfaceMgr->GetInterfaceColor(kInterfaceColorHilite)),
		selectionTextureMapAttribute(&routeTextureHeader, routeTextureImage),
		...
		swizzleRenderable(kRenderIndexedTriangles)
{
	...
	routeAttributeList.Append(&routeDiffuseAttribute);
	routeAttributeList.Append(&routeTextureMapAttribute);
	SetMaterialAttributeList(&routeAttributeList);
	
	SetVertexCount(70);
	SetAttributeArray(kArrayVertex, routeVertex);
	SetAttributeArray(kArrayTangent, routeTangent);
	SetAttributeArray(kArrayTexture0, routeTexcoord);
	...
	selectionRenderable.SetAttributeArray(kArrayVertex, routeVertex);
	...
	swizzleRenderable.SetAttributeArray(kArrayVertex, swizzleVertex);
	swizzleRenderable.SetAttributeArray(kArrayColor0, &swizzleColor[0]);
	swizzleRenderable.SetTriangleArray(10, swizzleTriangle);
	
	for (machine a = 0; a <= 64; a += 2)
	{
		routeTexcoord[a].Set(0.0, 0.0F);
		routeTexcoord[a + 1].Set(0.4375F, 0.0F);
	}
	
	routeTexcoord[66].Set(0.5, 0.9375F);
	routeTexcoord[67].Set(0.9375F, 0.9375F);
	routeTexcoord[68].Set(0.5, 0.0625F);
	routeTexcoord[69].Set(0.9375F, 0.0625F);
...
{
	float xmin = routeVertex[0].x;
	float xmax = routeVertex[0].x;
	float ymin = routeVertex[0].y;
	float ymax = routeVertex[0].y;
	
	for (machine a = 2; a < 70; a += 2)
	{
		const Point2D& p = routeVertex[a];
		xmin = Fmin(xmin, p.x);
		xmax = Fmax(xmax, p.x);
		ymin = Fmin(ymin, p.y);
		ymax = Fmax(ymax, p.y);
...
WidgetPart RouteWidget::TestPosition(const Point3D& position) const
{
	for (machine a = 0; a <= 66; a += 2)
	{
		if (a != 64)
		{
			Point3D q1 = routeVertex[a];
			Point3D q2 = routeVertex[a + 2];
			Vector3D dq = q2 - q1;
...
void RouteWidget::Build(void)
{
	...
	if (dx >= 8.0F)
	{
		...
		routeVertex[68] = p5;
		routeVertex[69] = p5;
		routeTangent[68].Set(tangent.x, tangent.y, 0.0F, -3.0F);
		routeTangent[69].Set(tangent.x, tangent.y, 0.0F, 4.0F);
		...
		for (machine a = 0; a <= 64; a += 2)
		{
			...
			routeVertex[a] = p.GetPoint2D();
			routeVertex[a + 1] = p.GetPoint2D();
			routeTangent[a].Set(t, -3.0F);
			routeTangent[a + 1].Set(t, 4.0F);
			...
			routeTexcoord[a].y = texcoord;
			routeTexcoord[a + 1].y = texcoord;
	...
	routeVertex[66] = routeVertex[64];
	routeVertex[67] = routeVertex[65];
	routeTangent[66] = routeTangent[64];
	routeTangent[67] = routeTangent[65];
	
	for (machine a = 0; a < 70; a += 2)
	{
		selectionTangent[a].Set(routeTangent[a].GetVector3D(), -15.0F);
		selectionTangent[a + 1].Set(routeTangent[a + 1].GetVector3D(), 16.0F);
	}
	
	if (swizzleText)
	{
		float thickness = viewportScale;
		float x = Floor(routeVertex[32].x) - kSwizzleBoxWidth * 0.5F;
		float y = Floor(routeVertex[32].y) - kSwizzleBoxHeight * 0.5F;
		
		swizzleVertex[0].Set(x, y);
		swizzleVertex[1].Set(x, y + kSwizzleBoxHeight);
		swizzleVertex[2].Set(x + kSwizzleBoxWidth, y + kSwizzleBoxHeight);
		swizzleVertex[3].Set(x + kSwizzleBoxWidth, y);
		
		swizzleVertex[4].Set(x, y);
		swizzleVertex[5].Set(x, y + kSwizzleBoxHeight);
		swizzleVertex[6].Set(x + kSwizzleBoxWidth, y + kSwizzleBoxHeight);
		swizzleVertex[7].Set(x + kSwizzleBoxWidth, y);
		
		swizzleVertex[8].Set(x - thickness, y - thickness);
		swizzleVertex[9].Set(x - thickness, y + kSwizzleBoxHeight + thickness);
		swizzleVertex[10].Set(x + kSwizzleBoxWidth + thickness, y + kSwizzleBoxHeight + thickness);
		swizzleVertex[11].Set(x + kSwizzleBoxWidth + thickness, y - thickness);
...
{
	if (routeWidgetState & kRouteWidgetSelected) renderList->Append(&selectionRenderable);
	RenderableWidget::Render(renderList);
	
	if (swizzleText) renderList->Append(&swizzleRenderable);
}
...
void ShaderSectionWidget::UpdateColor(void)
{
	const ColorRGBA& color = sectionProcess->GetSectionColor();
...
void PreviewWidget::Preprocess(void)
{
	...
	previewZone->AddNewSubnode(previewLight);
...
void PreviewWidget::SetPreviewGeometry(PrimitiveType type)
{
	...
	switch (type)
	{
		case kPrimitivePlate:
		{
			...
			cameraDistance = 2.0F;
			SetCameraAngles(K::pi_over_2, -K::pi_over_2);
			break;
		}
		...
		case kPrimitiveBox:
		{
			...
			cameraDistance = 2.5F;
			SetCameraAngles(K::three_pi_over_4, -0.589F);
			break;
		}
		...
		case kPrimitiveDome:
		{
			...
			cameraDistance = 2.5F;
			SetCameraAngles(K::three_pi_over_4, -K::pi_over_8);
			break;
		}
	...
	previewZone->AddNewSubnode(previewGeometry);
}
...
void ShaderProcessesPage::SetShaderGraph(int32 shader)
{
	for (machine a = 0; a < kProcessPaneCount; a++) listWidget[a]->PurgeListItems();
	...
	while (registration)
	{
		if ((registration->ValidShader(shader)) && (registration->GetProcessName()[0] != 0))
		{
			...
			else if (group == 'MATH') listWidget[1]->InsertSortedListItem(widget);
			else if (group == 'COMP') listWidget[2]->InsertSortedListItem(widget);
			else listWidget[3]->InsertSortedListItem(widget);
		}
...
{
	for (machine a = 0; a < kProcessPaneCount; a++) listWidget[a]->UnselectAllListItems();
}
...
{
	static const ShaderType shaderType[kShaderGraphCount] =
	{
		kShaderAmbient, kShaderPointLight
	};
	...
	{
		ShaderGraph *graph = previewAttribute->GetShaderGraph(a);
		graph->Purge();
		
		ShaderAttribute::CloneShader(&shaderGraph[a], graph);
		if (result) result[a] = previewAttribute->TestShader(shaderType[a], kShaderVariantNormal, 0, geometry, geometry->GetFirstRenderSegment());
	}
...
bool ProcessInfoWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
...
bool ShaderSectionInfoWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyDown)
	{
		unsigned_int32 code = eventData->keyCode;
...
ShaderEditor::ShaderEditor(MaterialObject *material, PrimitiveType primitiveType) :
		...
		routeDiffuseColor(ColorRGBA(0.0F, 0.0F, 0.0F, 1.0F)),
		routeTextureMap(&routeTextureHeader, routeTextureImage),
		routeRenderable(kRenderQuads)
{
	...
	if (shaderAttribute)
	{
		for (machine a = 0; a < kShaderGraphCount; a++) ShaderAttribute::CloneShader(shaderAttribute->GetShaderGraph(a), &shaderGraph[a]);
	}
	
	if (shaderGraph[kShaderGraphAmbient].Empty()) TranslateAmbientAttributes();
	if (shaderGraph[kShaderGraphLight].Empty()) TranslateLightAttributes();
	
	for (machine a = 0; a < kShaderGraphCount; a++)
	{
		graphRoot[a] = new Widget;
		sectionRoot[a] = new Widget;
		BuildShaderGraph(a);
	}
}
...
ShaderEditor::~ShaderEditor()
{
	...
	for (machine a = kShaderGraphCount - 1; a >= 0; a--)
	{
		delete sectionRoot[a];
		delete graphRoot[a];
	}
}
...
void ShaderEditor::Preprocess(void)
{
	...
	shaderMultipaneWidget = static_cast<MultipaneWidget *>(FindWidget("Shader"));
	
	for (machine a = 0; a < kShaderGraphCount; a++)
	{
		OrthoViewportWidget *widget = new OrthoViewportWidget(Vector2D(0.0F, 0.0F), Vector2D(1.0F, 1.0F));
		widget->SetWidgetPosition(shaderMultipaneWidget->GetWidgetPosition());
		if (a != kShaderGraphAmbient) widget->Hide();
		shaderViewport[a] = widget;
		AddNewSubnode(widget);
		
		widget->SetMouseEventProc(&ViewportHandleMouseEvent, this);
		widget->SetTrackTaskProc(&ViewportTrackTask, this);
		widget->SetRenderProc(&ViewportRender, this);
		
		OrthoCamera *camera = widget->GetViewportCamera();
	...
	for (machine a = 0; a < kShaderGraphCount; a++)
	{
		...
		if (a != kShaderGraphAmbient) text->Hide();
		statusMessage[a] = text;
		AddNewSubnode(text);
	}
	
	currentShader = kShaderGraphAmbient;
	...
	processesPage->SetShaderGraph(kShaderGraphAmbient);
	...
	routeRenderable.SetAttributeArray(kArrayVertex, routeVertex);
	routeRenderable.SetAttributeArray(kArrayTangent, routeTangent);
	routeRenderable.SetAttributeArray(kArrayTexture0, routeTexcoord);
	routeRenderable.SetShaderFlags(kShaderAmbientEffect | kShaderVertexPolyboard | kShaderLinearPolyboard | kShaderOrthoPolyboard);
	routeRenderable.SetAmbientBlendState(BlendState(kBlendOne, kBlendInvSourceAlpha));
	routeAttributeList.Append(&routeDiffuseColor);
	routeAttributeList.Append(&routeTextureMap);
	routeRenderable.SetMaterialAttributeList(&routeAttributeList);
	
	routeTexcoord[0].Set(0.0F, 1.0F);
	routeTexcoord[1].Set(0.4375F, 1.0F);
	routeTexcoord[2].Set(0.4375F, 1.0F);
	routeTexcoord[3].Set(0.0F, 1.0F);
	
	for (machine a = 0; a < kShaderGraphCount; a++) UpdateShaderGraph(a);
}
...
void ShaderEditor::PositionWidgets(void)
{
	...
	const Point3D& position = shaderMultipaneWidget->GetWidgetPosition();
	Vector2D viewportSize(width - position.x - 4.0F, height - position.y - 32.0F);
	
	shaderMultipaneWidget->SetWidgetSize(viewportSize);
	for (machine a = 0; a < kShaderGraphCount; a++) shaderViewport[a]->SetWidgetSize(viewportSize);
	...
	for (machine a = 0; a < kShaderGraphCount; a++)
	{
		statusMessage[a]->SetWidgetPosition(Point3D(position.x, position.y + viewportSize.y + 6.0F, 0.0F));
		statusMessage[a]->Invalidate();
	}
...
void ShaderEditor::BuildMenus(void)
{
	...
	widget = new MenuItemWidget(table->GetString(StringID('SHDR', 'MENU', 'SHDR', 'INFO')), WidgetObserver<ShaderEditor>(this, &ShaderEditor::HandleGetInfoMenuItem), Shortcut('I'));
	shaderMenuItem[kShaderMenuGetInfo] = widget;
	widget->Disable();
	shaderMenu->AppendMenuItem(widget);
	
	widget = new MenuItemWidget(table->GetString(StringID('SHDR', 'MENU', 'SHDR', 'TDET')), WidgetObserver<ShaderEditor>(this, &ShaderEditor::HandleToggleDetailLevelMenuItem), Shortcut('L'));
	shaderMenuItem[kShaderMenuToggleDetailLevel] = widget;
	widget->Disable();
	shaderMenu->AppendMenuItem(widget);
	
	shaderMenu->AppendMenuItem(new MenuItemWidget(kLineSolid));
	
	widget = new MenuItemWidget(table->GetString(StringID('SHDR', 'MENU', 'SHDR', 'AMBT')), WidgetObserver<ShaderEditor>(this, &ShaderEditor::HandleShowViewportMenuItem), Shortcut('1'));
	shaderMenuItem[kShaderMenuShowAmbientShader] = widget;
	shaderMenu->AppendMenuItem(widget);
	
	widget = new MenuItemWidget(table->GetString(StringID('SHDR', 'MENU', 'SHDR', 'LITE')), WidgetObserver<ShaderEditor>(this, &ShaderEditor::HandleShowViewportMenuItem), Shortcut('2'));
	shaderMenuItem[kShaderMenuShowLightShader] = widget;
	shaderMenu->AppendMenuItem(widget);
}

void ShaderEditor::TranslateAmbientAttributes(void)
{
	static const ConstPoint2D processPosition[kAmbientGraphProcessCount] =
	{
		{-1520.0F, 448.0F},		// kAmbientGraphTexcoord1
		{-1520.0F, 288.0F},		// kAmbientGraphTexcoord2
		{-1320.0F, 448.0F},		// kAmbientGraphParallax
		{-1064.0F, 0.0F},		// kAmbientGraphDiffuseColor
		{-1064.0F, 448.0F},		// kAmbientGraphEmissionColor
		{-384.0F, 200.0F},		// kAmbientGraphReflectionColor
		{-1064.0F, 1000.0F},	// kAmbientGraphRefractionColor
		{-1064.0F, 1296.0F},	// kAmbientGraphEnvironmentColor
		{-1064.0F, -256.0F},	// kAmbientGraphTextureMap1
		{-1064.0F, -128.0F},	// kAmbientGraphTextureMap2
		{-1064.0F, 160.0F},		// kAmbientGraphNormalMap1
		{-1064.0F, 288.0F},		// kAmbientGraphNormalMap2
		{-1064.0F, 576.0F},		// kAmbientGraphEmissionMap
		{-1064.0F, 1168.0F},	// kAmbientGraphGlossMap
		{-1064.0F, 872.0F},		// kAmbientGraphOpacityMap
		{-1520.0F, -256.0F},	// kAmbientGraphVertexColor
		{-864.0F, -192.0F},		// kAmbientGraphTextureCombiner
		{-864.0F, 224.0F},		// kAmbientGraphNormalCombiner
		{-864.0F, -64.0F},		// kAmbientGraphColorMultiply
		{-656.0F, -160.0F},		// kAmbientGraphDiffuseMultiply
		{-528.0F, -288.0F},		// kAmbientGraphOcclusionMultiply
		{-864.0F, 512.0F},		// kAmbientGraphEmissionMultiply
		{-384.0F, 0.0F},		// kAmbientGraphOpacityMultiply
		{-1064.0F, 744.0F},		// kAmbientGraphConstantUnity
		{-768.0F, 808.0F},		// kAmbientGraphOpacitySubtract
		{-528.0F, 872.0F},		// kAmbientGraphRefractionMultiply
		{-528.0F, 1240.0F},		// kAmbientGraphEnvironmentMultiply
		{-160.0F, 72.0F},		// kAmbientGraphAmbientOutput
		{-160.0F, -160.0F},		// kAmbientGraphAmbientAlphaOutput
		{-160.0F, -288.0F},		// kAmbientGraphAlphaTestOutput
		{-160.0F, 448.0F},		// kAmbientGraphEmissionOutput
		{-160.0F, 288.0F},		// kAmbientGraphReflectionOutput
		{-160.0F, 744.0F},		// kAmbientGraphRefractionOutput
		{-160.0F, 920.0F},		// kAmbientGraphEnvironmentOutput
		{-160.0F, 1048.0F},		// kAmbientGraphTerrainEnvironmentOutput
		{-160.0F, 576.0F},		// kAmbientGraphGlowOutput
		{-160.0F, 1224.0F}		// kAmbientGraphImpostorDepthOutput
	};
	
	Process		*process[kAmbientGraphProcessCount];
	
	ShaderAttribute::BuildAmbientShaderGraph(nullptr, nullptr, materialObject, nullptr, &shaderGraph[kShaderGraphAmbient], process);
	
	for (machine a = 0; a < kAmbientGraphProcessCount; a++)
	{
		if (process[a]) process[a]->SetProcessPosition(processPosition[a]);
	}
	
	if (!process[kAmbientGraphAmbientOutput])
	{
		AmbientOutputProcess *ambientProcess = new AmbientOutputProcess;
		ambientProcess->SetProcessPosition(processPosition[kAmbientGraphAmbientOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(ambientProcess);
	}
	
	if (!process[kAmbientGraphAmbientAlphaOutput])
	{
		AmbientAlphaOutputProcess *ambientAlphaProcess = new AmbientAlphaOutputProcess;
		ambientAlphaProcess->SetProcessPosition(processPosition[kAmbientGraphAmbientAlphaOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(ambientAlphaProcess);
	}
	
	if (!process[kAmbientGraphAlphaTestOutput])
	{
		AlphaTestOutputProcess *alphaTestProcess = new AlphaTestOutputProcess;
		alphaTestProcess->SetProcessPosition(processPosition[kAmbientGraphAlphaTestOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(alphaTestProcess);
	}
	
	if (!process[kAmbientGraphEmissionOutput])
	{
		EmissionOutputProcess *emissionProcess = new EmissionOutputProcess;
		emissionProcess->SetProcessPosition(processPosition[kAmbientGraphEmissionOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(emissionProcess);
	}
	
	if (!process[kAmbientGraphGlowOutput])
	{
		GlowOutputProcess *glowProcess = new GlowOutputProcess;
		glowProcess->SetProcessPosition(processPosition[kAmbientGraphGlowOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(glowProcess);
	}
	
	if (!process[kAmbientGraphReflectionOutput])
	{
		ReflectionOutputProcess *reflectionProcess = new ReflectionOutputProcess;
		reflectionProcess->SetProcessPosition(processPosition[kAmbientGraphReflectionOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(reflectionProcess);
	}
	
	if (!process[kAmbientGraphRefractionOutput])
	{
		RefractionOutputProcess *refractionProcess = new RefractionOutputProcess;
		refractionProcess->SetProcessPosition(processPosition[kAmbientGraphRefractionOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(refractionProcess);
	}
	
	if (!process[kAmbientGraphEnvironmentOutput])
	{
		EnvironmentOutputProcess *environmentProcess = new EnvironmentOutputProcess;
		environmentProcess->SetProcessPosition(processPosition[kAmbientGraphEnvironmentOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(environmentProcess);
	}
	
	if (!process[kAmbientGraphTerrainEnvironmentOutput])
	{
		TerrainEnvironmentOutputProcess *terrainEnvironmentProcess = new TerrainEnvironmentOutputProcess;
		terrainEnvironmentProcess->SetProcessPosition(processPosition[kAmbientGraphTerrainEnvironmentOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(terrainEnvironmentProcess);
	}
	
	if (!process[kAmbientGraphImpostorDepthOutput])
	{
		ImpostorDepthOutputProcess *impostorDepthProcess = new ImpostorDepthOutputProcess;
		impostorDepthProcess->SetProcessPosition(processPosition[kAmbientGraphImpostorDepthOutput]);
		shaderGraph[kShaderGraphAmbient].AddElement(impostorDepthProcess);
	}
}

void ShaderEditor::TranslateLightAttributes(void)
{
	static const ConstPoint2D processPosition[kLightGraphProcessCount] =
	{
		{-1632.0F, -128.0F},	// kLightGraphTexcoord1
		{-1432.0F, 88.0F},		// kLightGraphTexcoord2
		{-1432.0F, -128.0F},	// kLightGraphParallax
		{-1176.0F, -216.0F},	// kLightGraphDiffuseColor
		{-1176.0F, 504.0F},		// kLightGraphSpecularColor
		{-1176.0F, 248.0F},		// kLightGraphSpecularExponent
		{-768.0F, 248.0F},		// kLightGraphMicrofacet
		{-1176.0F, 248.0F},		// kLightGraphMicrofacetReflectivity
		{-1176.0F, -472.0F},	// kLightGraphTextureMap1
		{-1176.0F, -344.0F},	// kLightGraphTextureMap2
		{-1176.0F, -40.0F},		// kLightGraphNormalMap1
		{-1176.0F, 88.0F},		// kLightGraphNormalMap2
		{-1176.0F, 376.0F},		// kLightGraphGlossMap
		{-1632.0F, -472.0F},	// kLightGraphVertexColor
		{-976.0F, -408.0F},		// kLightGraphTextureCombiner
		{-976.0F, 24.0F},		// kLightGraphNormalCombiner
		{-976.0F, -280.0F},		// kLightGraphColorMultiply
		{-768.0F, -344.0F},		// kLightGraphDiffuseMultiply1
		{-552.0F, -280.0F},		// kLightGraphDiffuseMultiply2
		{-976.0F, 440.0F},		// kLightGraphSpecularMultiply1
		{-552.0F, 328.0F},		// kLightGraphSpecularMultiply2
		{-768.0F, -216.0F},		// kLightGraphDiffuseReflection
		{-768.0F, 248.0F},		// kLightGraphSpecularReflection
		{-552.0F, -472.0F},		// kLightGraphTangentLightDirection
		{-400.0F, 24.0F},		// kLightGraphLightSum
		{-160.0F, -72.0F},		// kLightGraphHorizon
		{-160.0F, 328.0F},		// kLightGraphBloom1
		{-160.0F, 200.0F},		// kLightGraphBloom2
		{128.0F, -216.0F},		// kLightGraphLightOutput
		{128.0F, -472.0F},		// kLightGraphAlphaTestOutput
		{128.0F, 24.0F}			// kLightGraphBloomOutput
	};
	
	Process		*process[kLightGraphProcessCount];
	
	ShaderAttribute::BuildLightShaderGraph(nullptr, nullptr, materialObject, nullptr, &shaderGraph[kShaderGraphLight], process);
	
	for (machine a = 0; a < kLightGraphProcessCount; a++)
	{
		if (process[a]) process[a]->SetProcessPosition(processPosition[a]);
	}
	
	if (!process[kLightGraphLightOutput])
	{
		LightOutputProcess *lightProcess = new LightOutputProcess;
		lightProcess->SetProcessPosition(processPosition[kLightGraphLightOutput]);
		shaderGraph[kShaderGraphLight].AddElement(lightProcess);
	}
	
	if (!process[kLightGraphAlphaTestOutput])
	{
		AlphaTestOutputProcess *alphaTestProcess = new AlphaTestOutputProcess;
		alphaTestProcess->SetProcessPosition(processPosition[kLightGraphAlphaTestOutput]);
		shaderGraph[kShaderGraphLight].AddElement(alphaTestProcess);
	}
	
	if (!process[kLightGraphBloomOutput])
	{
		BloomOutputProcess *bloomProcess = new BloomOutputProcess;
		bloomProcess->SetProcessPosition(processPosition[kLightGraphBloomOutput]);
		shaderGraph[kShaderGraphLight].AddElement(bloomProcess);
	}
}

void ShaderEditor::BuildShaderGraph(int32 index)
{
	Process *process = shaderGraph[index].GetFirstElement();
	while (process)
	{
		...
		if (type != kProcessSection)
		{
			...
			graphRoot[index]->AddSubnode(widget);
			processWidgetList[index].Append(widget);
		...
		else
		{
			...
			sectionRoot[index]->AddSubnode(widget);
			sectionWidgetList[index].Append(widget);
	...
	process = shaderGraph[index].GetFirstElement();
	while (process)
	{
		...
		while (route)
		{
			...
			graphRoot[index]->AddFirstSubnode(widget);
			routeWidgetList[index].Append(widget);
	...
	graphRoot[index]->Preprocess();
	sectionRoot[index]->Preprocess();
	
	graphRoot[index]->Update();
	sectionRoot[index]->Update();
}

void ShaderEditor::UpdateShaderGraph(int32 index)
{
	...
	ShaderResult	result[kShaderGraphCount];
	
	previewPage->UpdatePreviewMaterial(shaderGraph, result);
	
	for (machine a = 0; a < kShaderGraphCount; a++)
	{
		if (result[a] == kShaderOkay) statusMessage[a]->SetText(nullptr);
		else statusMessage[a]->SetText(TheWorldEditor->GetStringTable()->GetString(StringID('SHDR', 'MESS', message[result[a] - 1])));
	}
}

void ShaderEditor::ReattachProcess(ProcessWidget *widget)
{
	shaderGraph[currentShader].AddElement(widget->GetShaderProcess());
	graphRoot[currentShader]->AddSubnode(widget);
	processWidgetList[currentShader].Append(widget);
	
	widget->SetViewportScale(shaderViewport[currentShader]->GetOrthoScale().x);
}

void ShaderEditor::ReattachRoute(RouteWidget *widget)
{
	widget->GetShaderRoute()->Attach();
	graphRoot[currentShader]->AddFirstSubnode(widget);
	routeWidgetList[currentShader].Append(widget);
...
{
	shaderGraph[currentShader].AddElement(widget->GetSectionProcess());
	sectionRoot[currentShader]->AddSubnode(widget);
	sectionWidgetList[currentShader].Append(widget);
...
{
	RouteWidget *routeWidget = routeWidgetList[currentShader].First();
	while (routeWidget)
	{
		const Route *route = routeWidget->GetShaderRoute();
		if ((route->GetStartElement() == process) || (route->GetFinishElement() == process)) routeWidget->Rebuild();
		
		routeWidget = routeWidget->Next();
	}
	
	routeWidget = selectedRouteList[currentShader].First();
	while (routeWidget)
	{
		const Route *route = routeWidget->GetShaderRoute();
		if ((route->GetStartElement() == process) || (route->GetFinishElement() == process)) routeWidget->Rebuild();
...
void ShaderEditor::SelectProcessTool(const ProcessRegistration *registration)
{
	...
	{
		if (currentMode != kShaderEditorModeProcess) UnselectCurrentTool();
...
void ShaderEditor::UpdateViewportScale(float scale)
{
	scale = Clamp(scale, 1.0F, 8.0F);
	
	shaderViewport[currentShader]->SetOrthoScale(Vector2D(scale, scale));
	editorState |= kShaderEditorUpdateGrid;
	
	ProcessWidget *process = processWidgetList[currentShader].First();
	while (process)
	{
	...
	process = selectedProcessList[currentShader].First();
	while (process)
	{
		process->SetViewportScale(scale);
		process = process->Next();
	...
	RouteWidget *route = routeWidgetList[currentShader].First();
	while (route)
	{
	...
	route = selectedRouteList[currentShader].First();
	while (route)
	{
		route->SetViewportScale(scale);
		route = route->Next();
	...
	ShaderSectionWidget *section = sectionWidgetList[currentShader].First();
	while (section)
	{
	...
	section = selectedSectionList[currentShader].First();
	while (section)
	{
		section->SetViewportScale(scale);
		section = section->Next();
...
void ShaderEditor::SelectProcess(ProcessWidget *widget, unsigned_int32 state)
{
	selectedProcessList[currentShader].Append(widget);
	widget->Select(state);
	
	editorState |= kShaderEditorUpdateMenus;
}

void ShaderEditor::UnselectProcess(ProcessWidget *widget)
{
	processWidgetList[currentShader].Append(widget);
	widget->Unselect();
...
void ShaderEditor::SelectRoute(RouteWidget *widget)
{
	selectedRouteList[currentShader].Append(widget);
	widget->Select();
	
	editorState |= kShaderEditorUpdateMenus;
}

void ShaderEditor::UnselectRoute(RouteWidget *widget)
{
	routeWidgetList[currentShader].Append(widget);
	widget->Unselect();
...
void ShaderEditor::SelectSection(ShaderSectionWidget *widget)
{
	selectedSectionList[currentShader].Append(widget);
	widget->Select();
	
	editorState |= kShaderEditorUpdateMenus;
}

void ShaderEditor::UnselectSection(ShaderSectionWidget *widget)
{
	sectionWidgetList[currentShader].Append(widget);
	widget->Unselect();
...
void ShaderEditor::SelectAll(void)
{
	for (;;)
	{
		RouteWidget *widget = selectedRouteList[currentShader].First();
		if (!widget) break;
	...
	for (;;)
	{
		ProcessWidget *widget = processWidgetList[currentShader].First();
		if (!widget) break;
	...
	for (;;)
	{
		ShaderSectionWidget *widget = sectionWidgetList[currentShader].First();
		if (!widget) break;
...
void ShaderEditor::UnselectAll(void)
{
	for (;;)
	{
		ProcessWidget *widget = selectedProcessList[currentShader].First();
		if (!widget) break;
	...
	for (;;)
	{
		ShaderSectionWidget *widget = selectedSectionList[currentShader].First();
		if (!widget) break;
...
void ShaderEditor::UnselectAllTemp(void)
{
	...
	while (widget)
	{
		...
		if (widget->GetProcessWidgetState() & kProcessWidgetTempSelected) UnselectProcess(widget);
		widget = next;
	}
}
...
void ShaderEditor::DeleteProcess(ProcessWidget *processWidget, List<ProcessWidget> *deletedProcessList, List<RouteWidget> *deletedRouteList)
{
	...
	{
		shaderGraph[currentShader].RemoveElement(process);
...
void ShaderEditor::DeleteSection(ShaderSectionWidget *sectionWidget, List<ShaderSectionWidget> *deletedSectionList)
{
	...
	{
		shaderGraph[currentShader].RemoveElement(sectionWidget->GetSectionProcess());
...
void ShaderEditor::AddUndoData(ShaderUndoData *data)
{
	List<ShaderUndoData> *list = &undoList[currentShader];
	
	if (list->GetElementCount() >= kMaxShaderUndoCount)
	{
		for (;;)
		{
			ShaderUndoData *undoData = list->First();
			if (!undoData) break;
	...
	editorState |= kShaderEditorModified;
}

void ShaderEditor::RemoveUndoData(ShaderUndoData *data)
{
	delete data;
	
	if (undoList[currentShader].Empty()) shaderMenuItem[kShaderMenuUndo]->Disable();
	editorState |= kShaderEditorModified;
}
...
void ShaderEditor::HandleUndoMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	for (;;)
	{
		ShaderUndoData *undoData = undoList[currentShader].Last();
		if (!undoData) break;
...
void ShaderEditor::HandleCopyMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	const ProcessWidget *processWidget = processWidgetList[currentShader].First();
	while (processWidget)
	{
	...
	processWidget = selectedProcessList[currentShader].First();
	while (processWidget)
	{
		Process *process = processWidget->GetShaderProcess();
	...
	while (processWidget)
	{
		...
		if (process->GetBaseProcessType() != kProcessOutput)
		{
			...
			while (route)
			{
				...
				if (start) new Route(*route, start, finish);
	...
	ShaderSectionWidget *sectionWidget = selectedSectionList[currentShader].First();
	while (sectionWidget)
	{
...
void ShaderEditor::HandlePasteMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	while (process)
	{
		...
		if (type != kProcessSection)
		{
			const ProcessRegistration *registration = Process::FindRegistration(type);
			if (registration->ValidShader(currentShader))
			{
	...
	const Box2D *box = graphRoot[currentShader]->GetBoundingBox();
	...
	while (process)
	{
		...
		if (type != kProcessSection)
		{
			ProcessWidget *widget = new ProcessWidget(this, process, Process::FindRegistration(type));
			widget->SetViewportScale(shaderViewport[currentShader]->GetOrthoScale().x);
			graphRoot[currentShader]->AddNewSubnode(widget);
			widget->SetWidgetPosition(position);
		...
		else
		{
			...
			widget->SetViewportScale(shaderViewport[currentShader]->GetOrthoScale().x);
			sectionRoot[currentShader]->AddNewSubnode(widget);
			widget->SetWidgetPosition(position);
	...
	for (;;)
	{
		...
		shaderGraph[currentShader].AddElement(process);
	}
	
	AddUndoData(new CreateShaderUndoData(&processList, &routeList, &sectionList));
	...
	graphRoot[currentShader]->Update();
	sectionRoot[currentShader]->Update();
...
void ShaderEditor::HandleClearMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	RouteWidget *routeWidget = selectedRouteList[currentShader].First();
	while (routeWidget)
	{
		RouteWidget *next = routeWidget->Next();
	...
	ProcessWidget *processWidget = selectedProcessList[currentShader].First();
	while (processWidget)
	{
		ProcessWidget *next = processWidget->Next();
		
		const Process *process = processWidget->GetShaderProcess();
		if (process->GetBaseProcessType() != kProcessOutput) DeleteProcess(processWidget, &deletedProcessList, &deletedRouteList);
	...
	if ((!deletedProcessList.Empty()) || (!deletedRouteList.Empty()) || (!deletedSectionList.Empty())) AddUndoData(new DeleteShaderUndoData(&deletedProcessList, &deletedRouteList, &deletedSectionList));
}
...
{
	const ProcessWidget *processWidget = selectedProcessList[currentShader].First();
	if (processWidget)
	{
		if (selectedProcessList[currentShader].Last() == processWidget) AddSubwindow(new ProcessInfoWindow(this));
		return;
	}
	
	const RouteWidget *routeWidget = selectedRouteList[currentShader].First();
	if (routeWidget)
	{
		if (selectedRouteList[currentShader].Last() == routeWidget) AddSubwindow(new RouteInfoWindow(this));
		return;
	}
	
	const ShaderSectionWidget *sectionWidget = selectedSectionList[currentShader].First();
	if (sectionWidget)
	{
		if (selectedSectionList[currentShader].Last() == sectionWidget) AddSubwindow(new ShaderSectionInfoWindow(this));
		return;
	}
}
...
{
	const List<RouteWidget> *routeList = &selectedRouteList[currentShader];
	AddUndoData(new RouteShaderUndoData(routeList));
...
void ShaderEditor::HandleShowViewportMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	int32 graphIndex = (menuItem != shaderMenuItem[kShaderMenuShowAmbientShader]);
	shaderMultipaneWidget->SetSelection(graphIndex, true);
}

Point3D ShaderEditor::ViewportToGraphPosition(const Point3D& p) const
{
	OrthoViewportWidget *viewport = shaderViewport[currentShader];
	
	const Vector2D& scale = viewport->GetOrthoScale();
	OrthoCamera *camera = viewport->GetViewportCamera();
...
void ShaderEditor::ShowGraphPosition(float x, float y)
{
	...
	float w = viewport->GetWidgetSize().x * scale.x;
	float h = viewport->GetWidgetSize().y * scale.y;
...
void ShaderEditor::AutoScroll(const Point3D& p)
{
	...
	float w = viewport->GetWidgetSize().x;
	float h = viewport->GetWidgetSize().y;
...
ProcessWidget *ShaderEditor::FindProcessWidget(const Point3D& position) const
{
	...
	Widget *widget = graphRoot[currentShader]->DetectWidget(position, 0, &part);
	if ((widget) && (part == kWidgetPartInterior)) return (static_cast<ProcessWidget *>(widget));
...
{
	RouteWidget *widget = routeWidgetList[currentShader].First();
	while (widget)
	{
		if (widget->GetShaderRoute() == route) return (widget);
		widget = widget->Next();
	}
	
	widget = selectedRouteList[currentShader].First();
	while (widget)
	{
		if (widget->GetShaderRoute() == route) return (widget);
		widget = widget->Next();
...
void ShaderEditor::SortSelectedProcessSublist(List<ProcessWidget> *list, float dmin, float dmax)
{
	...
	{
		List<ProcessWidget> *finalList = &selectedProcessList[currentShader];
		while (widget)
		{
			ProcessWidget *prev = widget->Previous();
			finalList->Append(widget);
			widget = prev;
		}
	}
	else
...
bool ShaderEditor::BoxIntersectsProcessWidget(const Point3D& p1, const Point3D& p2, const ProcessWidget *widget)
{
	...
	if ((x2 < position.x) || (x1 > position.x + kProcessBoxWidth)) return (false);
	if ((y2 < position.y) || (y1 > position.y + kProcessBoxHeight)) return (false);
...
bool ShaderEditor::ProcessBoxIntersectsAnyProcessWidget(float x, float y, const ProcessWidget *exclude)
{
	...
	const ProcessWidget *widget = processWidgetList[currentShader].First();
	while (widget)
	{
		if ((widget != exclude) && (BoxIntersectsProcessWidget(p1, p2, widget))) return (true);
		widget = widget->Next();
	}
	
	widget = selectedProcessList[currentShader].First();
	while (widget)
	{
		if ((widget != exclude) && (BoxIntersectsProcessWidget(p1, p2, widget))) return (true);
		widget = widget->Next();
...
void ShaderEditor::BeginTool(const Point3D& p, unsigned_int32 eventFlags)
{
	...
	if (tool == kShaderToolProcessMove)
	{
		...
		Widget *widget = graphRoot[currentShader]->DetectWidget(previousPosition, 0, &part);
		if (widget)
		{
			if (part == kWidgetPartCurve)
			{
				...
				if (shift)
				{
					if (!selected) SelectRoute(routeWidget);
					else UnselectRoute(routeWidget);
				}
				else
				{
					if (!selected)
			...
			else if (part == kWidgetPartOutput)
			{
				...
				routeVertex[0] = anchorPosition.GetPoint2D();
				routeVertex[1] = anchorPosition.GetPoint2D();
				routeVertex[2] = anchorPosition.GetPoint2D();
				routeVertex[3] = anchorPosition.GetPoint2D();
				
				routeTangent[0].Set(1.0F, 0.0F, 0.0F, -3.0F);
				routeTangent[1].Set(1.0F, 0.0F, 0.0F, 4.0F);
				routeTangent[2].Set(1.0F, 0.0F, 0.0F, 4.0F);
				routeTangent[3].Set(1.0F, 0.0F, 0.0F, -3.0F);
			...
			else
			{
				...
				if (shift)
				{
					if (!selected) SelectProcess(processWidget, kProcessWidgetTempSelected);
					else UnselectProcess(processWidget);
				}
				else
				{
					if (!selected)
		...
		{
			Widget *widget = sectionRoot[currentShader]->DetectWidget(previousPosition, 0, &part);
			if (widget)
			{
				...
				else if (part == kWidgetPartResize)
				{
					...
					AddUndoData(new ResizeShaderUndoData(sectionWidget));
				}
			}
			else
			{
...
void ShaderEditor::TrackTool(const Point3D& p)
{
	...
	if (tool == kShaderToolGraphSelect)
	{
		...
		if ((boxSelectFlag) || (dx > 3.0F) || (dy > 3.0F))
		{
			...
			if (position != previousPosition)
			{
				...
				dragRect.Build(anchorPosition.GetPoint2D(), position.GetPoint2D(), shaderViewport[currentShader]->GetOrthoScale().x);
				...
				if ((dx >= 1.0F) && (dy >= 1.0F))
				{
					...
					{
						ProcessWidget *next = widget->Next();
						if (BoxIntersectsProcessWidget(anchorPosition, position, widget)) SelectProcess(widget, kProcessWidgetTempSelected);
						widget = next;
					}
				}
			}
	...
	else if (tool == kShaderToolProcessMove)
	{
		...
		if (position != previousPosition)
		{
			...
			if (state & kShaderEditorUndoPending)
			{
				...
				AddUndoData(new MoveShaderUndoData(&selectedProcessList[currentShader], &selectedSectionList[currentShader]));
			}
	...
	else
	{
		...
		{
			float dy = previousPoint.y - p.y;
			if (dy != 0.0F) UpdateViewportScale(shaderViewport[currentShader]->GetOrthoScale().x * Exp(dy * -0.01F));
		}
	}
...
void ShaderEditor::BeginSection(const Point3D& p)
{
	...
	shaderGraph[currentShader].AddElement(section);
	...
	SelectSection(widget);
	AddUndoData(new CreateShaderUndoData(widget));
...
void ShaderEditor::TrackRoute(const Point3D& p)
{
	...
	if (position != previousPosition)
	{
		...
		routeVertex[2] = position.GetPoint2D();
		routeVertex[3] = position.GetPoint2D();
		
		Vector3D tangent = (position - anchorPosition).Normalize();
		routeTangent[0].Set(tangent, -3.0F);
		routeTangent[1].Set(tangent, 4.0F);
		routeTangent[2].Set(tangent, 4.0F);
		routeTangent[3].Set(tangent, -3.0F);
		
		if (routeFinishProcess)
		{
			routeFinishProcess->UpdatePortColor(routePort, false);
			routeFinishProcess = nullptr;
		}
		...
		if ((widget) && (widget != routeStartProcess))
		{
			...
			while (prevRoute)
			{
				if ((prevRoute->GetRoutePort() == port) && (prevRoute->GetStartElement() == start)) return;
				prevRoute = prevRoute->GetNextIncomingEdge();
			}
			
			if (!shaderGraph[currentShader].Predecessor(finish, start))
			{
				widget->UpdatePortColor(port, true);
...
void ShaderEditor::EndRoute(const Point3D& p)
{
	...
	if (routeFinishProcess)
	{
		...
		while (prevRoute)
		{
			...
			{
				if (prevRoute->GetStartElement() == start) return;
				...
				if (widget)
				{
					...
					AddUndoData(new DeleteShaderUndoData(nullptr, &routeList, nullptr));
					coupledFlag = true;
					break;
				}
			}
		...
		Route *route = new Route(start, finish);
		route->SetRoutePort(routePort);
		...
		ShaderUndoData *undoData = new CreateShaderUndoData(widget);
		undoData->SetCoupledFlag(coupledFlag);
...
void ShaderEditor::ViewportHandleMouseEvent(const MouseEventData *eventData, ViewportWidget *viewport, void *cookie)
{
	...
	if ((eventType == kEventMouseDown) || (eventType == kEventMiddleMouseDown))
	{
		...
		if (mode == kShaderEditorModeTool) shaderEditor->BeginTool(eventData->mousePosition, eventData->eventFlags);
		else if (mode == kShaderEditorModeProcess) shaderEditor->CreateProcess(eventData->mousePosition);
		else if (mode == kShaderEditorModeSection) shaderEditor->BeginSection(eventData->mousePosition);
	}
	else if ((eventType == kEventMouseUp) || (eventType == kEventMiddleMouseUp))
	{
		if (shaderEditor->toolTracking)
		{
			...
			if (mode == kShaderEditorModeTool) shaderEditor->EndTool(eventData->mousePosition);
			else if (mode == kShaderEditorModeSection) shaderEditor->EndSection(eventData->mousePosition);
			else if (mode == kShaderEditorModeRoute) shaderEditor->EndRoute(eventData->mousePosition);
	...
	else if (eventType == kEventMouseWheel)
	{
		if (!shaderEditor->toolTracking)
		{
			...
			shaderEditor->UpdateViewportScale(orthoViewport->GetOrthoScale().x * Exp(eventData->mousePosition.y * -0.16F));
		}
	}
}
...
void ShaderEditor::ViewportTrackTask(const Point3D& position, ViewportWidget *viewport, void *cookie)
{
	...
	{
		int32 mode = shaderEditor->trackingMode;
		if (mode == kShaderEditorModeTool) shaderEditor->TrackTool(position);
		else if (mode == kShaderEditorModeSection) shaderEditor->TrackSection(position);
		else if (mode == kShaderEditorModeRoute) shaderEditor->TrackRoute(position);
	}
}
...
{
	ShaderEditor *shaderEditor = static_cast<ShaderEditor *>(cookie);
	int32 index = shaderEditor->currentShader;
	...
	if (state & kShaderEditorUpdateGrid)
	{
		...
		const OrthoViewportWidget *orthoViewport = shaderEditor->shaderViewport[index];
	...
	Widget *widget = shaderEditor->sectionRoot[index];
	widget->Update();
	widget->RenderTree(renderList);
	
	widget = shaderEditor->graphRoot[index];
	widget->Update();
	widget->RenderTree(renderList);
	
	if (shaderEditor->boxSelectFlag) renderList->Append(&shaderEditor->dragRect);
	else if ((shaderEditor->toolTracking) && (shaderEditor->trackingMode == kShaderEditorModeRoute)) renderList->Append(&shaderEditor->routeRenderable);
}
...
void ShaderEditor::ConfirmationDialogComplete(Dialog *dialog, void *cookie)
{
	if (dialog->GetDialogStatus() == kDialogOkay) static_cast<ShaderEditor *>(cookie)->Close();
}
...
void ShaderEditor::Move(void)
{
	if ((TheInterfaceMgr->GetActiveWindow() == this) && (!TheInterfaceMgr->GetActiveMenu()))
	{
		...
		if (!toolCursor)
		{
			Vector2D vp = position.GetVector2D() - shaderViewport[0]->GetWorldPosition().GetVector2D();
			if ((vp.x >= 0.0F) && (vp.x < shaderViewport[0]->GetWidgetSize().x) && (vp.y >= 0.0F) && (vp.y < shaderViewport[0]->GetWidgetSize().y)) toolCursor = true;
		}
		...
		if (toolCursor)
		{
			...
			{
				if (tool == kShaderToolViewportScroll) cursorIndex = (toolTracking) ? kEditorCursorDrag : kEditorCursorHand;
				else if (tool == kShaderToolViewportZoom) cursorIndex = kEditorCursorGlass;
			}
			else
			{
				...
				{
					Point3D p = AlignPositionToGrid(ViewportToGraphPosition(position - shaderViewport[0]->GetWorldPosition().GetVector3D()));
					if (ProcessBoxIntersectsAnyProcessWidget(p.x, p.y)) cursorIndex = kEditorCursorStop;
				}
			}
		}
	...
	{
		if (undoList[currentShader].Empty()) shaderMenuItem[kShaderMenuUndo]->Disable();
		else shaderMenuItem[kShaderMenuUndo]->Enable();
		
		const List<ProcessWidget> *processList = &selectedProcessList[currentShader];
		const List<RouteWidget> *routeList = &selectedRouteList[currentShader];
		const List<ShaderSectionWidget> *sectionList = &selectedSectionList[currentShader];
		
		bool processSelection = !processList->Empty();
		bool routeSelection = !routeList->Empty();
		bool sectionSelection = !sectionList->Empty();
		...
		if ((processSelection) || (sectionSelection))
		{
			...
			if (processSelection)
			{
				if ((!(routeSelection | sectionSelection)) && (processList->First() == processList->Last())) getInfoItem->Enable();
			}
			else
			{
				if ((!routeSelection) && (sectionList->First() == sectionList->Last())) getInfoItem->Enable();
			}
		}
		else
		{
			...
			if (routeSelection)
			{
				if (routeList->First() == routeList->Last()) getInfoItem->Enable();
				
				const RouteWidget *routeWidget = routeList->First();
				while (routeWidget)
				{
					const Route *route = routeWidget->GetShaderRoute();
					const Process *process = route->GetFinishElement();
		...
		if (processSelection | routeSelection | sectionSelection) menuItem->Enable();
		else menuItem->Disable();
	}
	
	if (editorState & kShaderEditorUpdateGraph) UpdateShaderGraph(currentShader);
	
	editorState &= ~(kShaderEditorUpdateMenus | kShaderEditorUpdateGraph);
	
	graphRoot[currentShader]->Move();
	sectionRoot[currentShader]->Move();
	Window::Move();
}
...
void ShaderEditor::EnterForeground(void)
{
	...
	if (!editorClipboard.Empty()) shaderMenuItem[kShaderMenuPaste]->Enable();
}
...
bool ShaderEditor::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	...
	if (eventType == kEventKeyDown)
	{
		...
		if ((code >= '1') && (code <= '7'))
		{
			if (code == '1') toolButton[kShaderToolProcessMove]->SetValue(1, true);
			else if (code == '6') toolButton[kShaderToolViewportScroll]->SetValue(1, true);
			else if (code == '7') toolButton[kShaderToolViewportZoom]->SetValue(1, true);
			return (true);
		}
...
void ShaderEditor::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		if (widget == okayButton)
		{
			ShaderAttribute *shaderAttribute = static_cast<ShaderAttribute *>(materialObject->FindAttribute(kAttributeShader));
			if (shaderAttribute)
			{
				for (machine a = 0; a < kShaderGraphCount; a++) shaderAttribute->GetShaderGraph(a)->Purge();
			}
			else
			{
				shaderAttribute = new ShaderAttribute;
				materialObject->AddAttribute(shaderAttribute);
			}
			
			for (machine a = 0; a < kShaderGraphCount; a++)
			{
				ShaderAttribute::CloneShader(&shaderGraph[a], shaderAttribute->GetShaderGraph(a));
			}
		...
		{
			if (editorState & kShaderEditorModified)
			{
				const StringTable *table = TheWorldEditor->GetStringTable();
				
				Dialog *dialog = new Dialog(Vector2D(342.0F, 120.0F), table->GetString(StringID('SHDR')), table->GetString(StringID('BTTN', 'DSCD')), table->GetString(StringID('BTTN', 'CANC')));
	...
	else if (eventType == kEventWidgetChange)
	{
		...
		else if (widget == shaderMultipaneWidget)
		{
			int32 selection = shaderMultipaneWidget->GetSelection();
			
			for (machine a = 0; a < kShaderGraphCount; a++)
			{
				OrthoViewportWidget *viewport = shaderViewport[a];
				if (a == selection)
				{
					viewport->Show();
					statusMessage[a]->Show();
				...
				{
					viewport->Hide();
					statusMessage[a]->Hide();
				}
			}
			
			currentShader = selection;
			processesPage->SetShaderGraph(selection);
			
			editorState |= kShaderEditorUpdateMenus | kShaderEditorUpdateGrid;
		}
	}
}
