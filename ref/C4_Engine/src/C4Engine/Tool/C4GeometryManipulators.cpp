		...
		selectionRenderable(kRenderIndexedTriangles)
{
	...
	selectedFaceArray = nullptr;
	maxSelectedFaceCount = 0;
...
{
	delete[] selectedFaceArray;
	delete[] selectedSurfaceArray;
}

Manipulator *GeometryManipulator::Construct(Geometry *geometry)
{
	...
	else if (geometryType == kGeometryMesh)
	{
		return (new MeshGeometryManipulator(static_cast<MeshGeometry *>(geometry)));
	} 
...
const char *GeometryManipulator::GetDefaultNodeName(void) const
{
	...
	if (type == kGeometryPrimitive)
	{
		...
		return (TheWorldEditor->GetStringTable()->GetString(StringID('NAME', kNodeGeometry, primitive->GetPrimitiveType())));
	}
	
	return (TheWorldEditor->GetStringTable()->GetString(StringID('NAME', kNodeGeometry)));
}

void GeometryManipulator::Invalidate(void)
{
	EditorManipulator::Invalidate();
	
	GetTargetNode()->InvalidateStaticShadowVolumes();
}
...
void GeometryManipulator::SelectSurface(unsigned_int32 index)
{
	if (!selectedSurfaceArray)
	{
		...
		for (machine a = 0; a < surfaceCount; a++) selectedSurfaceArray[a] = false;
	}
...
int32 GeometryManipulator::GetSelectedSurfaceCount(void) const
{
	...
	if (GetSelectionType() == kEditorSelectionSurface)
	{
		...
		for (machine a = 0; a < surfaceCount; a++) count += selectedSurfaceArray[a];
	}
...
void GeometryManipulator::UpdateSurfaceSelection(void)
{
	if (selectedSurfaceArray)
	{
		...
		if (maxSelectedFaceCount != faceCount)
		{
			delete[] selectedFaceArray;
			Triangle *face = new Triangle[faceCount];
			
			selectedFaceArray = face;
			selectionRenderable.SetTriangleArray(face);
		}
		
		int32 selectedFaceCount = 0;
		const Triangle *triangle = geometryLevel->GetArray<Triangle>(kArrayFace);
		
		const unsigned_int16 *surfaceIndex = geometryLevel->GetArray<unsigned_int16>(kArraySurfaceIndex);
		if (surfaceIndex)
		{
			for (machine a = 0; a < faceCount; a++)
			{
				if (selectedSurfaceArray[surfaceIndex[triangle[a].index[0]]]) selectedFaceArray[selectedFaceCount++] = triangle[a];
			}
			
			selectionRenderable.SetFaceCount(selectedFaceCount);
		}
		else if (selectedSurfaceArray[0])
		{
			selectedFaceCount = faceCount;
			selectionRenderable.SetFaceCount(faceCount);
			for (machine a = 0; a < faceCount; a++) selectedFaceArray[a] = triangle[a];
		}
		
		SetSelectionType((selectedFaceCount != 0) ? kEditorSelectionSurface : kEditorSelectionObject);
	}
}
...
const MaterialObject *GeometryManipulator::PickupMaterial(void) const
{
	return (GetTargetNode()->GetMaterialObject(0));
}

void GeometryManipulator::SetMaterial(MaterialObject *materialObject)
{
	Geometry *geometry = GetTargetNode();
	
	int32 materialCount = geometry->GetMaterialCount();
	for (machine a = 0; a < materialCount; a++) geometry->SetMaterialObject(a, materialObject);
}
...
Box3D GeometryManipulator::CalculateNodeBoundingBox(void) const
{
	...
	if (GetTargetNode()->CalculateBoundingBox(&box)) return (box);
...
bool GeometryManipulator::Pick(const Ray *ray, PickData *data) const
{
	...
	if (object->GetCollisionOctree())
	{
		...
		if (object->DetectCollision(p1, p2, 0.0F, &geometryHitData))
		{
			...
			data->rayParam = ray->tmin * (1.0F - t) + ray->tmax * t;
	...
	else
	{
		...
		const ArrayBundle *triangleBundle = geometry->GetArrayBundle(kArrayFace);
		if (triangleBundle)
		{
			const Triangle *triangle = static_cast<Triangle *>(triangleBundle->pointer);
			
			const ArrayBundle *vertexBundle = geometry->GetArrayBundle(kArrayVertex);
			if (vertexBundle)
			{
				int32 componentCount = vertexBundle->descriptor.componentCount;
				if (componentCount == 3)
				{
					...
					for (machine index = 0; index < triangleCount; index++)
					{
						...
						if (Math::IntersectSegmentAndTriangle(q1, q2, v1, v2, v3, &position, &normal, &t))
						{
							data->rayParam = ray->tmin * (1.0F - t) + ray->tmax * t;
				...
				{
					const Point2D *vertex = static_cast<Point2D *>(vertexBundle->pointer);
...
bool GeometryManipulator::RegionPick(const Region *region) const
{
	Antivector4D	objectPlane[kMaxRegionPlaneCount];
	
	const Geometry *geometry = GetTargetNode();
	if (!geometry->Visible(region)) return (false);
	...
	for (machine a = 0; a < planeCount; a++) objectPlane[a] = planeArray[a] * worldTransform;
	...
	if (triangleBundle)
	{
		...
		if (vertexBundle)
		{
			...
			for (machine index = 0; index < triangleCount; index++)
			{
				Point3D		vertexArray[2][kMaxRegionPlaneCount + 3];
				...
				{
					vertexCount = Math::ClipPolygonAgainstPlane(vertexCount, vertexArray[parity], objectPlane[a], location, vertexArray[parity ^ 1]);
					if (vertexCount == 0) goto next;
					parity ^= 1;
				}
...
void GeometryManipulator::Render(const ManipulatorRenderData *renderData)
{
	...
	if (GetSelectionType() == kEditorSelectionSurface)
	{
		if (selectionDetailLevel != geometry->GetDetailLevel()) UpdateSurfaceSelection();
		
		selectionRenderable.SetTransformable(geometry->GetTransformable());
		selectionRenderable.SetVertexCount(geometry->GetVertexCount());
		selectionRenderable.SetAttributeArray(kArrayVertex, geometry->GetAttributeArray<Point3D>(kArrayVertex));
...
MeshGeometryManipulator::MeshGeometryManipulator(MeshGeometry *mesh) : GeometryManipulator(mesh)
{
}

MeshGeometryManipulator::~MeshGeometryManipulator()
{
}

int32 MeshGeometryManipulator::GetHandleTable(Point3D *handle) const
{
...
void MeshGeometryManipulator::GetHandleData(int32 index, ManipulatorHandleData *handleData) const
{
...
void MeshGeometryManipulator::BeginResize(const ManipulatorResizeData *resizeData)
{
	GeometryManipulator::BeginResize(resizeData);
	originalBounds = GetObject()->GetBoundingBox();
}

bool MeshGeometryManipulator::Resize(const ManipulatorResizeData *resizeData)
{
	MeshGeometryObject *object = GetObject();
	...
	{
		if (handleFlags & kManipulatorHandlePositiveX) newMeshSize.x = Fmax(oldMeshSize->x + dx * 2.0F, kSizeEpsilon);
		else if (handleFlags & kManipulatorHandleNegativeX) newMeshSize.x = Fmax(oldMeshSize->x - dx * 2.0F, kSizeEpsilon);
		else newMeshSize.x = oldMeshSize->x;
		
		if (handleFlags & kManipulatorHandlePositiveY) newMeshSize.y = Fmax(oldMeshSize->y + dy * 2.0F, kSizeEpsilon);
		else if (handleFlags & kManipulatorHandleNegativeY) newMeshSize.y = Fmax(oldMeshSize->y - dy * 2.0F, kSizeEpsilon);
		else newMeshSize.y = oldMeshSize->y;
		
		if (handleFlags & kManipulatorHandlePositiveZ) newMeshSize.z = Fmax(oldMeshSize->z + dz * 2.0F, kSizeEpsilon);
		else if (handleFlags & kManipulatorHandleNegativeZ) newMeshSize.z = Fmax(oldMeshSize->z - dz * 2.0F, kSizeEpsilon);
		else newMeshSize.z = oldMeshSize->z;
	...
	{
		if (handleFlags & kManipulatorHandlePositiveX) newMeshSize.x = Fmax(oldMeshSize->x + dx, kSizeEpsilon);
		else if (handleFlags & kManipulatorHandleNegativeX) newMeshSize.x = Fmax(oldMeshSize->x - dx, kSizeEpsilon);
		else newMeshSize.x = oldMeshSize->x;
		
		if (handleFlags & kManipulatorHandlePositiveY) newMeshSize.y = Fmax(oldMeshSize->y + dy, kSizeEpsilon);
		else if (handleFlags & kManipulatorHandleNegativeY) newMeshSize.y = Fmax(oldMeshSize->y - dy, kSizeEpsilon);
		else newMeshSize.y = oldMeshSize->y;
		
		if (handleFlags & kManipulatorHandlePositiveZ) newMeshSize.z = Fmax(oldMeshSize->z + dz, kSizeEpsilon);
		else if (handleFlags & kManipulatorHandleNegativeZ) newMeshSize.z = Fmax(oldMeshSize->z - dz, kSizeEpsilon);
		else newMeshSize.z = oldMeshSize->z;
...
bool HoleGeometryManipulator::Resize(const ManipulatorResizeData *resizeData)
{
	...
	if (handleFlags & kManipulatorHandleNonzeroX)
	{
		...
		if (resizeData->resizeFlags & kManipulatorResizeConstrain) newInnerSize.y = Fmin(newInnerSize.x, oldOuterSize->y - kSizeEpsilon);
		else newInnerSize.y = oldInnerSize->y;
	}
	else
	{
		if (handleFlags & kManipulatorHandleNegativeY) dy = -dy;
		...
		if (resizeData->resizeFlags & kManipulatorResizeConstrain) newInnerSize.x = Fmin(newInnerSize.y, oldOuterSize->x - kSizeEpsilon);
		else newInnerSize.x = oldInnerSize->x;
	}
...
bool SphereGeometryManipulator::Resize(const ManipulatorResizeData *resizeData)
{
	...
	if (resizeData->resizeFlags & kManipulatorResizeCenter)
	{
		...
		{
			if (handleFlags & kManipulatorHandleNegativeZ) dz = -dz;
...
bool TorusGeometryManipulator::Resize(const ManipulatorResizeData *resizeData)
{
	...
	if (handleFlags & kManipulatorHandleNonzeroX)
	{
		if (handleFlags & kManipulatorHandlePositiveX) dx = -dx;
		object->SetSecondaryRadius(Clamp(oldSecondaryRadius + dx, kSizeEpsilon, maxSecondaryRadius));
	}
	else
	{
		if (handleFlags & kManipulatorHandlePositiveY) dy = -dy;
	...
	if (Fabs(vz) > K::min_float)
	{
		...
		if ((t > ray->tmin) && (t < ray->tmax))
		{
			...
			if ((x > -expand) && (x < clothSize.x + expand) && (y > -expand) && (y < clothSize.y + expand))
			{
				...
				bool b1 = (vz < 0.0F);
				bool b2 = (triangle->index[0] < level->GetVertexCount() / 2);
				
				data->triangleIndex = (b1 ^ b2) ? level->GetFaceCount() / 2 : 0;
				return (true);
			}
		}
	}
...
bool ClothGeometryManipulator::Resize(const ManipulatorResizeData *resizeData)
{
	...
	if (resizeData->resizeFlags & kManipulatorResizeConstrain)
	{
		if (Fabs(dx) > Fabs(dy)) dy = dx * oldClothSize->y / oldClothSize->x;
		else dx = dy * oldClothSize->x / oldClothSize->y;
	}
