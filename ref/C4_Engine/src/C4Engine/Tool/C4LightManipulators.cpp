...
const ConstPoint3D InfiniteLightManipulator::directionVertex[4] =
{
	{0.0F, 0.0F, -2.0F}, {0.0F, 0.0F, -2.0F}, {0.0F, 0.0F, 0.0F}, {0.0F, 0.0F, 0.0F}
};

const ConstPoint2D InfiniteLightManipulator::directionTexcoord[4] =
{
	{0.0F, 1.0F}, {1.0F, 1.0F}, {1.0F, 0.0F}, {0.0F, 0.0F}
};

const float InfiniteLightManipulator::directionRadius[4] =
{
	-1.0F, 1.0F, 1.0F, -1.0F
};
...
Manipulator *LightManipulator::Construct(Light *light)
{
...
void LightManipulator::Invalidate(void)
{
...
InfiniteLightManipulator::InfiniteLightManipulator(InfiniteLight *infiniteLight) :
		...
		directionDiffuseColor(kLightInteriorColor),
		...
		directionRenderable(kRenderQuads)
{
	...
	directionRenderable.SetAttributeArray(kArrayVertex, &directionVertex[0]);
	directionRenderable.SetAttributeArray(kArrayTexture0, &directionTexcoord[0]);
	directionRenderable.SetAttributeArray(kArrayRadius, directionRadius);
}
...
{
	if (!(GetManipulatorState() & kManipulatorUpdated)) SetSphereSize(GetObject()->GetLightRange());
	LightManipulator::Update();
}

void PointLightManipulator::Render(const ManipulatorRenderData *renderData)
{
	if (Selected()) SphereVolumeManipulator::Render(renderData);
...
{
	if (Selected()) ProjectionVolumeManipulator::Render(renderData);
	EditorManipulator::Render(renderData);
}
