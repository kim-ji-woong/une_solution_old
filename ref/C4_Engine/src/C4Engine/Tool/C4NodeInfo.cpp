...
{
	configurationWidget->BuildCategoryConfiguration(configurable, categoryType);
}

void CategoryInfoPane::CommitSettings(Configurable *configurable)
{
	configurationWidget->CommitCategoryConfiguration(configurable, categoryType);
}
...
void FireEffectInfoPane::BuildSettings(const Configurable *configurable)
{
	...
	if (!previewFire)
	{
		...
		previewZone->AddNewSubnode(previewFire);
		previewZone->Update();
...
{
	if ((newProperty) && (!newProperty->GetOwningMap())) delete newProperty;
}
...
void PropertyInfoPane::BuildSettings(const Configurable *configurable)
{
	...
	for (;;)
	{
		while (property)
		{
			const PropertyRegistration *registration = Property::FindRegistration(property->GetPropertyType());
			if (registration)
			{
	...
	for (;;)
	{
		while (property)
		{
			...
			if (Property::FindRegistration(type))
			{
				...
				while (data)
				{
					if (data->GetPropertyRegistration()->GetPropertyType() == type) break;
					data = data->Next();
				}
				
				if (!data) delete property;
			}
...
void PropertyInfoPane::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		...
		else if (widget == removeButton)
		{
			...
			Widget *widget = assignedListWidget->GetFirstSelectedListItem();
			assignedListWidget->RemoveListItem(widget);
			delete widget;
			...
			if (selection >= 0) SelectAvailableProperty(selection);
		}
	}
...
void ModifierInfoPane::BuildSettings(const Configurable *configurable)
{
	...
	while (modifier)
	{
		...
		const ModifierRegistration *registration = Modifier::FindRegistration(clone->GetModifierType());
		if (registration)
		{
			const char *name = (registration) ? registration->GetModifierName() : table->GetString(StringID('INFO', 'MDFR', 'UNKN'));
...
void ModifierInfoPane::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	else if (eventType == kEventWidgetChange)
	{
		if (widget == availableListWidget)
		{
			if (availableListWidget->GetFirstSelectedListItem()) assignButton->Enable();
		}
		else if (widget == assignedListWidget)
		{
...
{
	const Widget *widget = assignedListWidget->GetFirstListItem();
	while (widget)
	{
		const char *text = static_cast<const TextWidget *>(widget)->GetText();
		if (Text::CompareText(text, key)) return (true);
...
void ConnectorInfoPane::BuildSettings(const Configurable *configurable)
{
	...
	for (machine a = 0; a < count; a++)
	{
		...
		if (key) availableListWidget->AppendListItem(key);
	}
	...
	if (hub)
	{
		...
		while (connector)
		{
			...
			{
				if (hub->FindOutgoingConnector(key)) assignedListWidget->AppendListItem(key);
			}
...
void ConnectorInfoPane::CommitSettings(Configurable *configurable)
{
	...
	if (hub)
	{
		...
		while (connector)
		{
			...
			if (key[0] != '%')
			{
				...
				while (data)
				{
					if (data->connector == connector) break;
					data = data->Next();
				}
				
				if (!data) delete connector;
			}
			else
			{
				if (!InternalConnectorAssigned(key)) delete connector;
			}
	...
	while (data)
	{
		...
		if (key[0] != 0)
		{
			...
			if (connector) connector->SetConnectorKey(key);
			else node->AddConnector(key);
		}
	...
	while (widget)
	{
		const char *key = static_cast<const TextWidget *>(widget)->GetText();
		if ((!hub) || (!hub->FindOutgoingConnector(key))) node->AddConnector(key);
...
void ConnectorInfoPane::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		if ((widget == assignButton) || (widget == availableListWidget))
		{
			const Widget *widget = availableListWidget->GetFirstSelectedListItem();
			if (widget)
			{
				const char *key = static_cast<const TextWidget *>(widget)->GetText();
		...
		else if (widget == removeButton)
		{
			...
			if (widget)
			{
				...
				if ((availableWidget) && (Text::CompareText(key, static_cast<const TextWidget *>(availableWidget)->GetText()))) assignButton->Enable();
	...
	else if (eventType == kEventWidgetChange)
	{
		...
		{
			if (assignedListWidget->GetFirstSelectedListItem()) removeButton->Enable();
...
ControllerInfoPane::~ControllerInfoPane()
{
	if (targetNode)
	{
		...
		for (machine a = 0; a < count; a++)
		{
			if (controllerTable[a] != controller) delete controllerTable[a];
		}
	...
	if (index != selectionIndex)
	{
		...
		if (index >= 0)
		{
			...
			if (!controller)
			{
				...
				if ((targetController) && (targetController->GetControllerType() == controllerType)) controller = targetController;
				if (!controller) controller = Controller::New(controllerType);
				controllerTable[index] = controller;
			}
			
			configurationWidget->BuildConfiguration(controller);
			
			if (controllerType == kControllerScript) scriptButton->Show();
			else if (controllerType == kControllerPanel) panelButton->Show();
		}
	}
}
...
void ControllerInfoPane::BuildSettings(const Configurable *configurable)
{
	...
	for (machine a = 0; a < count; a++)
	{
		...
		const ControllerRegistration *registration = validControllerArray[a];
...
void ControllerInfoPane::CommitSettings(Configurable *configurable)
{
	...
	if (index >= 0)
	{
		...
		if (controller == originalController) return;
	...
	if (eventType == kEventWidgetActivate)
	{
		if (widget == scriptButton)
		{
			...
			if (type == kControllerScript)
			{
				...
				AddSubwindow(new ScriptEditor(targetNode, object));
			}
		}
...
ForceInfoPane::~ForceInfoPane()
{
	if (targetField)
	{
		...
		for (machine a = 0; a < count; a++)
		{
			if (forceTable[a] != force) delete forceTable[a];
		}
	...
	if (index != selectionIndex)
	{
		...
		if (index >= 0)
		{
			...
			if (!force)
			{
				...
				if ((targetForce) && (targetForce->GetForceType() == forceType)) force = targetForce;
				if (!force) force = Force::New(forceType);
...
void ForceInfoPane::BuildSettings(const Configurable *configurable)
{
	...
	for (machine a = 0; a < count; a++) forceTable[a] = nullptr;
	...
	for (machine a = 0; a < count; a++)
	{
		const ForceRegistration *registration = validForceArray[a];
...
void NodeInfoWindow::Preprocess(void)
{
			...
			for (machine a = 0; a < count; a++)
			{
				...
				while (pane)
				{
					if (pane->GetNodeInfoType() == kNodeInfoObjectCategory)
					{
						...
						if (categoryInfoPane->GetCategoryType() == category) break;
					}
	...
	if (first == nodeList->Last())
	{
		...
		{
			NodeInfoPane *pane = new ConnectorInfoPane;
			paneList.Append(pane);
			AddNewSubnode(pane);
			pane->BuildSettings(node);
	...
	while (pane)
	{
		...
		{
			if ((selectType >= kNodeInfoCategoryTypeCount) || (static_cast<CategoryInfoPane *>(pane)->GetCategoryType() == selectCategory)) paneSelection = index;
		}
...
{
	const List<NodeReference> *nodeList = worldEditor->GetSelectionList();
	worldEditor->AddUndoData(new NodeInfoUndoData(nodeList));
		...
		if (pane->GetVisitedFlag())
		{
			...
			while (reference)
			{
				...
				if (type == kNodeInfoObjectCategory)
				{
					...
					if (object) pane->CommitSettings(object);
				}
				else
				{
					pane->CommitSettings(node);
					
					if ((type == kNodeInfoProperties) && (!node->GetSuperNode())) worldEditor->SetProcessPropertiesFlag();
				}
			...
			if (index == selection)
			{
				Type category = (type < kNodeInfoCategoryTypeCount) ? static_cast<CategoryInfoPane *>(pane)->GetCategoryType() : 0;
	...
	while (reference)
	{
		...
		else if (type == kNodeEffect)
		{
			...
			while (node)
			{
				if ((node->GetNodeType() == kNodeEffect) && (node->GetObject() == object))
				{
					...
					if (effect->GetEffectType() == kEffectTube) static_cast<TubeEffectObject *>(object)->Build();
					effect->Preprocess();
				}
		...
		{
			if (worldEditor->GetEditorObject()->GetEditorFlags() & kEditorExpandWorlds) preprocessFlag = false;
		}
		else if (type == kNodeModel)
		{
			if (worldEditor->GetEditorObject()->GetEditorFlags() & kEditorExpandModels) preprocessFlag = false;
		}
		...
		if (preprocessFlag)
		{
			...
			targetNode->SetWorld(world);
			targetNode->Preprocess();
		}
		
		if ((type != kNodeGeometry) && (reference == gizmoTarget)) worldEditor->PostEvent(GizmoEditorEvent(kEditorEventGizmoTargetModified, targetNode));
...
bool NodeInfoWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
...
void NodeInfoWindow::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		...
		else
		{
			...
			if (pane) pane->HandleWidgetEvent(widget, eventData);
		}
	}
	else
	{
