...
const ConstPoint2D TerrainTextureWidget::imageVertex[12] =
{
	{  0.0F,  47.0F}, { 47.0F,  47.0F}, { 47.0F,  0.0F}, {  0.0F,  0.0F},
	{ 53.0F,  47.0F}, {100.0F,  47.0F}, {100.0F,  0.0F}, { 53.0F,  0.0F},
	{106.0F,  47.0F}, {153.0F,  47.0F}, {153.0F,  0.0F}, {106.0F,  0.0F}
};

const ConstPoint2D TerrainTextureWidget::backgroundVertex[12] =
{
		...
		cylinderRenderable(kRenderIndexedTriangles, kRenderDepthInhibit)
{
	...
	sphereObject->SetGeometryFlags(sphereObject->GetGeometryFlags() | (kGeometryShadowInhibit | kGeometryMotionBlurInhibit | kGeometryRenderEffectPass));
	...
	cylinderObject->SetGeometryFlags(cylinderObject->GetGeometryFlags() | (kGeometryShadowInhibit | kGeometryMotionBlurInhibit | kGeometryRenderEffectPass)); 
...
{
	for (machine a = 0; a < kVertexBufferCount; a++) renderable->SetVertexBuffer(a, brush->GetVertexBuffer(a));
	renderable->SetDynamicArrayFlags(brush->GetDynamicArrayFlags());
	
	renderable->SetVertexCount(brush->GetVertexCount());
	for (machine a = 0; a < kMaxAttributeArrayCount; a++)
	{
		renderable->SetAttributeArray(a, brush->GetAttributeArray(a), brush->GetComponentCount(a));
		renderable->SetAttributeOffset(a, brush->GetAttributeOffset(a));
	}
	
	renderable->SetTriangleArray(brush->GetFaceCount(), brush->GetTriangleArray());
	renderable->SetFaceOffset(brush->GetFaceOffset());
}
...
void TerrainBlockManipulator::BuildBrush(float radius, float height)
{
	...
	if (renderMode == TerrainPage::kEditorTerrainModeSculpt) brushDiffuseColor->SetDiffuseColor(sculptBrushColor[renderTool]);
	else brushDiffuseColor->SetDiffuseColor(paintBrushColor[renderTool]);
	...
	if (renderBrush == TerrainPage::kEditorTerrainBrushSphere)
	{
		...
		sphereBrush->SetNodeTransform(K::identity_4D);
	...
	else
	{
		...
		cylinderBrush->SetNodeTransform(K::identity_4D);
...
bool TerrainBlockManipulator::RegionPick(const Region *region) const
{
	Vector3D boxSize = GetTargetNode()->GetBlockBoxSize();
...
bool TerrainBlockManipulator::Resize(const ManipulatorResizeData *resizeData)
{
	...
	if ((ax >= ay) && (ax >= az)) newScale = oldScale * (newSize.x / oldSize.x);
	else if (ay >= az) newScale = oldScale * (newSize.y / oldSize.y);
	else newScale = oldScale * (newSize.z / oldSize.z);
	
	if (newScale != oldScale) RescaleBlock(newScale);
	return (false);
}
...
void TerrainBlockManipulator::SetRenderTool(int32 mode, int32 tool, int32 brush)
{
	...
	if (tool >= 0) state |= kManipulatorForceRender;
	else state &= ~kManipulatorForceRender;
	...
	while (node)
	{
		if (node->GetNodeType() == kNodeGeometry)
		{
			...
			if (geometry->GetGeometryType() == kGeometryTerrain)
			{
				...
				GeometryLevel *geometryLevel = geometryObject->GetGeometryLevel(0);
				
				int32 vertexCount = geometryLevel->GetVertexCount();
				Point3D *position0 = geometryLevel->GetArray<Point3D>(kArrayPosition0);
				Point3D *position1 = geometryLevel->GetArray<Point3D>(kArrayPosition1);
				...
				{
					for (machine a = 0; a < vertexCount; a++) position0[a] *= factor;
				}
				
				Antivector4D *plane = geometryLevel->GetArray<Antivector4D>(kArrayPlane);
				if (plane)
				{
					int32 planeCount = geometryLevel->GetArrayDescriptor(kArrayPlane)->elementCount;
					for (machine a = 0; a < planeCount; a++) plane[a].w *= factor;
				}
...
{
	return (TheWorldEditor->GetStringTable()->GetString(StringID('NAME', kGeometryTerrain)));
}

bool TerrainGeometryManipulator::Pick(const Ray *ray, PickData *data) const
{
	if (GetObject()->GetDetailLevel() > 0) return (false);
	return (GeometryManipulator::Pick(ray, data));
}

bool TerrainGeometryManipulator::RegionPick(const Region *region) const
{
	if (GetObject()->GetDetailLevel() > 0) return (false);
...
void TerrainGeometryManipulator::CopyTriangleMesh(List<TerrainGeometryManipulator> *terrainPaintList)
{
	if (!paintMeshStorage)
	{
		...
		paintTriangleCount = level->GetFaceCount();
...
TerrainPaintUndoData::TerrainPaintUndoData(TerrainBlock *block, int32 channel1, int32 channel2) : UndoData(kUndoTerrainPaint)
{
	blockNode = block;
	...
	if (channel2 >= 0)
	{
		...
		for (machine a = 1; a < kTerrainEditLevelCount; a++) materialObject[a] = materialObject[a - 1] + terrainCount[a - 1];
		...
		for (machine a = 1; a < kTerrainEditLevelCount; a++) terrainFlags[a] = terrainFlags[a - 1] + terrainCount[a - 1];
	...
	for (machine a = 1; a < kTerrainEditLevelCount; a++)
	{
		...
		for (machine b = 0; b < count; b++) terrainFlags[a][b] = 0;
	}
}

TerrainPaintUndoData::~TerrainPaintUndoData()
{
	...
	for (machine a = 0; a < count; a++)
	{
		if (terrainFlags[0][a] & kTerrainPaintUndoModified)
		{
			...
			if (terrainChannel[1] >= 0) TerrainStorage::DeleteVoxelData(drawingChannelData[1][a]);
...
bool TerrainPaintUndoData::AddTerrain(const Integer3D& coord, int32 level)
{
	int32 ds = (1 << level) - 1;
	...
	if (!(terrainFlags[level][index] & kTerrainPaintUndoModified))
	{
		if (level == 0)
		{
			...
			if (terrainChannel[1] >= 0) drawingChannelData[1][index] = blockNode->SaveVoxelData(terrainChannel[1], coord);
		}
		...
		terrainFlags[level][index] = flags;
		return (true);
	}
...
bool TerrainPaintUndoData::TerrainModified(const Integer3D& coord, int32 level) const
{
	int32 ds = (1 << level) - 1;
	const Integer3D& size = blockNode->GetBlockSize();
	int32 index = ((coord.z >> level) * ((size.y + ds) >> level) + (coord.y >> level)) * ((size.x + ds) >> level) + (coord.x >> level);
...
VoxelMap *TerrainPaintUndoData::OpenSavedVoxelMap(const Integer3D& vmin, const Integer3D& vmax) const
{
...
void TerrainPaintUndoData::CloseSavedVoxelMap(VoxelMap *voxelMap)
{
	delete voxelMap;
}

void TerrainPaintUndoData::Undo(Editor *editor)
{
	const Integer3D& size = blockNode->GetBlockSize();
...
TerrainTextureWidget::TerrainTextureWidget(int32 groupIndex) :
		...
		backgroundDiffuseAttribute(ColorRGBA(0.0F, 0.0F, 0.0F, 1.0F)),
...
WidgetPart TerrainTextureWidget::TestPosition(const Point3D& position) const
{
	...
	{
		const Point2D *vertex = &imageVertex[a * 4];
		if ((position.x >= vertex[0].x) && (position.x <= vertex[2].x) && (position.y >= vertex[2].y) && (position.y <= vertex[0].y)) return (kWidgetPartTexture + a);
	}
...
void TerrainTextureWidget::Preprocess(void)
{
	...
	SetVertexCount(12);
	SetAttributeArray(kArrayVertex, &imageVertex[0]);
	SetAttributeArray(kArrayColor0, imageColor);
	
	imageAttributeList.Append(&textureMapAttribute);
	...
	for (machine a = 0; a < 12; a++) imageColor[a].Set(1.0F, 1.0F, 1.0F);
}

void TerrainTextureWidget::SetTexture(const char *name)
{
	textureMapAttribute.SetTexture(name ? name : "C4/black");
	InvalidateShaderData();
	
	const Texture *texture = textureMapAttribute.GetTexture();
	if ((texture) && (texture->GetTextureFlags() & kTextureImagePalette))
	{
		...
		SetAttributeArray(kArrayTexture0, imageTexcoord, (texture->GetTextureType() == kTextureArray2D) ? 3 : 2);
	}
	else
	{
		entryCountX = 1;
		...
		SetAttributeArray(kArrayTexture0, imageTexcoord, 2);
	}
	
	for (machine a = 0; a < 3; a++) UpdateFaceTexture(a);
}

void TerrainTextureWidget::SetTerrainMaterial(TerrainMaterial *material)
{
	if (textureGroupIndex == 0) textureSelector = &material->primaryMaterial;
	else textureSelector = &material->secondaryMaterial;
...
{
	for (machine a = 0; a < 12; a++) imageColor[a].Set(1.0F, 1.0F, 1.0F);
	if (face >= 0) for (machine a = 0; a < 4; a++) imageColor[face * 4 + a].Set(0.5F, 0.5F, 0.5F);
}

void TerrainTextureWidget::UpdateFaceTexture(int32 face)
{
	float index = (float) paletteIndex[face];
	
	const Texture *texture = textureMapAttribute.GetTexture();
	if ((texture) && (texture->GetTextureType() == kTextureArray2D))
	{
		Point3D *texcoord = reinterpret_cast<Point3D *>(imageTexcoord) + face * 4;
		texcoord[0].Set(0.25F, 0.25F, index);
		texcoord[1].Set(0.75F, 0.25F, index);
		texcoord[2].Set(0.75F, 0.75F, index);
		texcoord[3].Set(0.25F, 0.75F, index);
	}
	else
	{
		...
		Point2D *texcoord = reinterpret_cast<Point2D *>(imageTexcoord) + face * 4;
		texcoord[0].Set(s, t);
		texcoord[1].Set(s + ds * 0.5F, t);
		texcoord[2].Set(s + ds * 0.5F, t + dt * 0.5F);
		texcoord[3].Set(s, t + dt * 0.5F);
	}
}
...
void TerrainTextureWidget::SelectPaletteEntry(TerrainTextureWindow *window, TerrainTextureWidget *widget)
{
	...
	if (face == 0) widget->textureSelector->SetRed(entry);
	else if (face == 1) widget->textureSelector->SetGreen(entry);
	else widget->textureSelector->SetBlue(entry);
	
	widget->paletteIndex[face] = entry;
	widget->UpdateFaceTexture(face);
}
...
void TerrainTextureWidget::HandleMouseEvent(const PanelMouseEventData *eventData)
{
	...
	else
	{
		...
		{
			if (inside) SetHilitedFace(eventData->widgetPart - kWidgetPartTexture);
			else SetHilitedFace(-1);
		}
		else if (eventType == kEventMouseUp)
		{
			...
			if (inside)
			{
				...
				if ((texture) && (texture->GetTextureFlags() & kTextureImagePalette))
				{
					...
					if (position.x + windowWidth > desktopSize.x) position.x -= windowWidth + widgetWidth + 8.0F;
					if (position.y + windowHeight > desktopSize.y) position.y = desktopSize.y - windowHeight;
...
void TerrainTextureWindow::Preprocess(void)
{
	...
	AddNewSubnode(new QuadWidget(GetWidgetSize(), K::black));
...
OptimizeTerrainWindow::~OptimizeTerrainWindow()
{
	...
	while (reference)
	{
		...
		{
			Geometry *geometry = static_cast<Geometry *>(node);
			if (geometry->GetGeometryType() == kGeometryTerrain) worldEditor->InvalidateGeometry(geometry);
		}
...
void OptimizeTerrainWindow::OptimizeTerrainJob(Job *job, void *cookie)
{
	...
	if (detailLevel == 0)
	{
		geometryLevel->OptimizeMesh(&tempLevel, threshold);
		
		tempLevel.BuildPlaneArray(geometryLevel);
		geometryLevel->BuildEdgeArray(&tempLevel);
	...
	else
	{
		...
		int32 triangleCount = tempLevel.GetFaceCount();
		geometryLevel->OptimizeMesh(&tempLevel, threshold * (float) (1 << detailLevel), levelObject->GetBaseTriangleCount());
		levelObject->OffsetBorderRenderData(geometryLevel->GetFaceCount() - triangleCount);
	}
...
void OptimizeTerrainWindow::Preprocess(void)
{
	...
	worldEditor->AddUndoData(new GeometryUndoData(worldEditor->GetSelectionList(), kGeometryTerrain));
	...
	while (reference)
	{
		...
		{
			Geometry *geometry = static_cast<Geometry *>(node);
			if (geometry->GetGeometryType() == kGeometryTerrain) count++;
		}
...
bool OptimizeTerrainWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyDown)
	{
		if (eventData->keyCode == kKeyCodeEscape)
...
void RebuildTerrainBlockWindow::CommitSettings(void) const
{
	...
	for (machine a = 0; a < 3; a++)
	{
		...
		offset[a] = Text::StringToInteger(offsetBox[a]->GetText());
	}
		...
		if (node->GetNodeType() == kNodeGeometry)
		{
			...
			if (geometry->GetGeometryType() == kGeometryTerrain)
			{
				...
				if (((unsigned_int32) coord.x < (unsigned_int32) size.x) && ((unsigned_int32) coord.y < (unsigned_int32) size.y) && ((unsigned_int32) coord.z < (unsigned_int32) size.z))
				{
					...
					GeometryLevel *geometryLevel = terrainObject->GetGeometryLevel(0);
					int32 vertexCount = geometryLevel->GetVertexCount();
					
					Point3D *position0 = geometryLevel->GetArray<Point3D>(kArrayPosition0);
					for (machine a = 0; a < vertexCount; a++) position0[a] += dv;
					
					Point3D *position1 = geometryLevel->GetArray<Point3D>(kArrayPosition1);
					if (position1)
					{
						for (machine a = 0; a < vertexCount; a++) position1[a] += dv;
					}
					...
					{
						int32 planeCount = geometryLevel->GetArrayDescriptor(kArrayPlane)->elementCount;
						for (machine a = 0; a < planeCount; a++) plane[a].w -= plane[a] ^ dv;
					}
...
bool RebuildTerrainBlockWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyDown)
	{
...
TerrainRebuildUndoData::TerrainRebuildUndoData(TerrainBlock *block) : UndoData(kUndoTerrainRebuild)
{
	blockNode = block;
...
TerrainRebuildUndoData::~TerrainRebuildUndoData()
{
}

void TerrainRebuildUndoData::Undo(Editor *editor)
{
	for (;;)
	{
		Node *node = blockNode->GetFirstSubnode();
...
TerrainPage::TerrainPage() :
		EditorPage(kEditorPageTerrain, "WorldEditor/terrain/Terrain"),
...
void TerrainPage::Preprocess(void)
{
	...
	terrainPaintWidget[0]->SetWidgetPosition(Point3D(5.0F, 494.0F, 0.0F));
	AddSubnode(terrainPaintWidget[0]);
	
	terrainPaintWidget[1] = new TerrainTextureWidget(1);
	terrainPaintWidget[1]->SetWidgetPosition(Point3D(5.0F, 576.0F, 0.0F));
	AddSubnode(terrainPaintWidget[1]);
	...
	for (machine a = 0; a < 2; a++) terrainPaintWidget[a]->SetTerrainMaterial(&terrainMaterial);
	...
	MenuItemWidget *widget = new MenuItemWidget(table->GetString(StringID('PAGE', kEditorPageTerrain, 'OPTM')), WidgetObserver<TerrainPage>(this, &TerrainPage::HandleOptimizeTerrainMenuItemEvent));
...
void TerrainPage::UpdateSlider(SliderWidget *widget)
{
	...
	else if (widget == offsetSlider)
	{
		...
		if (offset == 0) offsetText->SetText("0%");
		else if (offset > 0) offsetText->SetText((String<15>("+") += offset) += '%');
		else offsetText->SetText((String<15>("\xE2\x88\x92") += 1 - offset) += '%');	// U+2212
	}
	else if (widget == bulldozerSlider)
	{
		int32 bias = GetBulldozerBias();
		if (bias == 0) bulldozerText->SetText("0");
		else if (bias > 0) bulldozerText->SetText(String<15>("+") += bias);
		else bulldozerText->SetText(String<15>("\xE2\x88\x92") += -bias);	// U+2212
	}
...
void TerrainPage::UpdateMaterial(const MaterialObject *materialObject)
{
	...
	while (attribute)
	{
		...
		if (attributeType == kAttributeTextureMap)
		{
			textureName = static_cast<const TextureMapAttribute *>(attribute)->GetTextureName();
			break;
		}
		else if (attributeType == kAttributeShader)
		{
			const ShaderGraph *graph = static_cast<const ShaderAttribute *>(attribute)->GetShaderGraph(kShaderGraphAmbient);
	...
	for (machine a = 0; a < 2; a++) terrainPaintWidget[a]->SetTexture(textureName);
}
...
void TerrainPage::HandleBrushEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		for (machine a = 0; a < kEditorTerrainBrushCount; a++)
		{
			if (widget == brushButton[a]) currentBrush = a;
			else brushButton[a]->SetValue(0);
		}
	}
}
...
void TerrainPage::HandleSlopeEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		for (machine a = 0; a < kEditorTerrainSlopeCount; a++)
		{
			if (widget == slopeButton[a]) currentSlope = a;
			else slopeButton[a]->SetValue(0);
		}
	}
}
...
void TerrainPage::HandleSliderEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		...
		if (sliderWidget == radiusSlider) radiusValue = value;
		else if (sliderWidget == heightSlider) heightValue = value;
		else if (sliderWidget == offsetSlider) offsetValue = value;
		else if (sliderWidget == bulldozerSlider) bulldozerValue = value;
		else if (sliderWidget == blendSlider) blendValue = value;
...
void TerrainPage::HandleCheckEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		...
		if (checkWidget == stylusWidget) stylusValue = value;
		else if (checkWidget == materialWidget) materialValue = value;
	}
}
	...
	if (eventData->eventType == kEventWidgetActivate)
	{
		...
		while (reference)
		{
			...
			if (type == kNodeGeometry)
			{
				...
				if (geometry->GetGeometryType() == kGeometryTerrain) geometryCount++;
			}
		...
		if (geometryCount != 0) terrainMenuItem[kTerrainMenuOptimizeTerrain]->Enable();
		else terrainMenuItem[kTerrainMenuOptimizeTerrain]->Disable();
		
		if (blockCount == 1) terrainMenuItem[kTerrainMenuRebuildTerrainBlock]->Enable();
		else terrainMenuItem[kTerrainMenuRebuildTerrainBlock]->Disable();
...
void TerrainPage::Disengage(Editor *editor, void *cookie)
{
	if (currentTool != -1)
	{
		if (currentMode == kEditorTerrainModeBlock) blockButton->SetValue(0);
		else if (currentMode == kEditorTerrainModeSculpt) sculptButton[currentTool]->SetValue(0);
		else if (currentMode == kEditorTerrainModePaint) paintButton[currentTool]->SetValue(0);
		else pickupButton->SetValue(0);
	...
	if (trackData->viewportType != kEditorViewportGraph)
	{
		...
		else if ((currentMode == kEditorTerrainModeSculpt) || (currentMode == kEditorTerrainModePaint))
		{
			...
			if (editor->GetEditorWorld()->DetectCollision(p1, p2, 0.0F, kCollisionCamera, &collisionData))
			{
				...
				if (geometry->GetGeometryType() == kGeometryTerrain)
				{
					...
					{
						drawingBlock = block;
						drawingChannel = kTerrainChannelDensity;
						if (currentMode != kEditorTerrainModeSculpt)
						{
							drawingChannel = (currentTool <= kEditorTerrainPaintBlend2) ? kTerrainChannelBlend : kTerrainChannelMaterial;
						}
						...
						if (currentBrush != kEditorTerrainBrushSlope) brushNormal = drawingPlane.GetAntivector3D();
						else brushNormal = (drawingPlane.z >= 0.0F) ? K::z_unit : K::minus_z_unit;
						...
						if (currentBrush != kEditorTerrainBrushSlope) manipulator->SetBrushTransform(previousPoint, drawingPlane.GetAntivector3D());
						else manipulator->SetBrushTransform(previousPoint, K::z_unit);
						
						undoData = new TerrainPaintUndoData(block, drawingChannel, (drawingChannel != kTerrainChannelMaterial) ? kTerrainChannelMaterial : 0);
		...
		else if (currentMode == kEditorTerrainModePickup)
		{
			...
			if (node)
			{
				...
				if (geometry->GetGeometryType() == kGeometryTerrain)
				{
					const GeometryLevel *geometryLevel = geometry->GetObject()->GetGeometryLevel(0);
					...
					const Color4C *color0 = geometryLevel->GetArray<Color4C>(kArrayColor0);
					const Color4C *color1 = geometryLevel->GetArray<Color4C>(kArrayColor1);
...
bool TerrainPage::TrackTool(Editor *editor, EditorTrackData *trackData)
{
	...
	else
	{
		...
		{
			if (currentTool == kEditorTerrainSculptAdd) offset += (drawingHeight - drawingOffset) * TheEngine->GetStylusPressure();
			else if (currentTool == kEditorTerrainSculptSubtract) offset -= (drawingOffset + drawingHeight) * TheEngine->GetStylusPressure();
		}
		...
		if (currentPlane != kEditorTerrainPlaneNone)
		{
			...
			if ((t < trackData->worldRay.tmin) || (t > trackData->worldRay.tmax)) return (true);
		...
		else
		{
			...
			if (!DetectTerrainIntersection(drawingBlock, currentMode, &blockRay, &collisionPoint)) return (true);
			
			drawingPlane.Set(collisionPoint.normal.Normalize(), collisionPoint.position);
			if (currentBrush != kEditorTerrainBrushSlope) brushNormal = collisionPoint.normal;
			else brushNormal = (collisionPoint.normal.z >= 0.0F) ? K::z_unit : K::minus_z_unit;
		...
		if (currentBrush != kEditorTerrainBrushSlope) manipulator->SetBrushTransform(currentPoint, drawingPlane.GetAntivector3D());
		else manipulator->SetBrushTransform(currentPoint, K::z_unit);
...
bool TerrainPage::EndTool(Editor *editor, EditorTrackData *trackData)
{
	...
	if (currentMode == kEditorTerrainModeBlock)
	{
		...
		if (result) editor->AddSubwindow(new BuildTerrainWindow(editor, static_cast<TerrainBlock *>(trackData->trackNode)));
	}
	else
	{
...
bool TerrainPage::DetectTerrainIntersection(const Node *node, int32 mode, Ray *ray, CollisionPoint *collisionPoint) const
{
	...
	if (subnode)
	{
		...
		do
		{
			if (DetectTerrainIntersection(subnode, mode, ray, collisionPoint)) result = true;
	...
	else if (node->GetNodeType() == kNodeGeometry)
	{
		...
		if (geometry->GetGeometryType() == kGeometryTerrain)
		{
			...
			bool modifiedFlag = undoData->TerrainModified(object->GetGeometryCoord(), object->GetDetailLevel());
...
void TerrainPage::DrawTerrain(Editor *editor, const Point3D& endpoint1, const Point3D& endpoint2)
{
		...
		for (machine k = kmin; k <= kmax; k++)
		{
			for (machine j = jmin; j <= jmax; j++)
			{
				for (machine i = imin; i <= imax; i++)
				{
					...
					if (undoData->AddTerrain(coord, 0))
					{
						const TerrainGeometry *geometry = drawingBlock->FindTerrainGeometry(coord);
						if (geometry) static_cast<TerrainGeometryManipulator *>(geometry->GetManipulator())->CopyTriangleMesh(&terrainPaintList);
					}
				}
			}
		}
	...
	else
	{
		for (machine k = kmin; k <= kmax; k++)
		{
			for (machine j = jmin; j <= jmax; j++)
			{
				for (machine i = imin; i <= imax; i++) undoData->AddTerrain(Integer3D(i, j, k), 0);
			}
		}
	}
	...
	for (machine a = 1; a < kTerrainEditLevelCount; a++)
	{
		...
		for (machine k = kmin & mask; k <= kmax; k += ds)
		{
			...
			{
				for (machine i = imin & mask; i <= imax; i += ds) undoData->AddTerrain(Integer3D(i, j, k), a);
			}
		}
	}
	...
	if ((GetMaterialUpdateState()) || (drawingChannel == kTerrainChannelMaterial))
	{
		...
		if (drawingChannel != kTerrainChannelMaterial) materialMap = drawingBlock->OpenVoxelMap(kTerrainChannelMaterial, vmin, vmax);
	}
	
	if (drawingChannel == kTerrainChannelBlend)
	{
		...
		{
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalMinPaintBrush(drawingMap, materialMap, p1, p2, fuzzy, blend, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalMinPaintBrush(drawingMap, materialMap, p1, p2, topPlane, bottomPlane, fuzzy, blend, material);
		}
		else
		{
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalMaxPaintBrush(drawingMap, materialMap, p1, p2, fuzzy, blend, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalMaxPaintBrush(drawingMap, materialMap, p1, p2, topPlane, bottomPlane, fuzzy, blend, material);
		}
	}
	else if (drawingChannel == kTerrainChannelMaterial)
	{
		if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalMaterialBrush(drawingMap, p1, p2, material);
		else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalMaterialBrush(drawingMap, p1, p2, topPlane, bottomPlane, material);
	}
	else
	{
		if (currentTool == kEditorTerrainSculptAdd)
		{
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalAdditiveBrush(drawingMap, materialMap, p1, p2, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalAdditiveBrush(drawingMap, materialMap, p1, p2, topPlane, bottomPlane, material);
			else if (currentBrush == kEditorTerrainBrushSlope) DrawSlopeAdditiveBrush(drawingMap, materialMap, p1, p2, material);
		}
		else if (currentTool == kEditorTerrainSculptSubtract)
		{
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalSubtractiveBrush(drawingMap, materialMap, p1, p2, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalSubtractiveBrush(drawingMap, materialMap, p1, p2, topPlane, bottomPlane, material);
			else if (currentBrush == kEditorTerrainBrushSlope) DrawSlopeSubtractiveBrush(drawingMap, materialMap, p1, p2, material);
		}
		else if (currentTool == kEditorTerrainSculptMin)
		{
			VoxelMap *savedVoxelMap = undoData->OpenSavedVoxelMap(vmin, vmax);
			
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalMinBrush(drawingMap, materialMap, savedVoxelMap, p1, p2, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalMinBrush(drawingMap, materialMap, savedVoxelMap, p1, p2, topPlane, bottomPlane, material);
			
			TerrainPaintUndoData::CloseSavedVoxelMap(savedVoxelMap);
		}
		else if (currentTool == kEditorTerrainSculptMax)
		{
			VoxelMap *savedVoxelMap = undoData->OpenSavedVoxelMap(vmin, vmax);
			
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalMaxBrush(drawingMap, materialMap, savedVoxelMap, p1, p2, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalMaxBrush(drawingMap, materialMap, savedVoxelMap, p1, p2, topPlane, bottomPlane, material);
		...
		else if (currentTool == kEditorTerrainSculptSmooth)
		{
			...
			if (currentBrush == kEditorTerrainBrushSphere) DrawSphericalSmoothingBrush(drawingMap, materialMap, savedVoxelMap, p1, p2, material);
			else if (currentBrush == kEditorTerrainBrushCylinder) DrawCylindricalSmoothingBrush(drawingMap, materialMap, savedVoxelMap, p1, p2, topPlane, bottomPlane, material);
			
			TerrainPaintUndoData::CloseSavedVoxelMap(savedVoxelMap);
		}
	}
	
	if (materialMap) drawingBlock->CloseVoxelMap(kTerrainChannelMaterial, materialMap);
	...
	for (machine a = 0; a < kTerrainEditLevelCount; a++)
	{
		...
		for (machine k = kmin & mask; k <= kmax; k += ds)
		{
			for (machine j = jmin & mask; j <= jmax; j += ds)
			{
				for (machine i = imin & mask; i <= imax; i += ds) UpdateBlock(editor, drawingBlock, Integer3D(i, j, k), a, materialObject);
			}
		}
	}
			...
			for (machine j = voxelBounds.min.y; j <= voxelBounds.max.y; j++)
			{
				...
				for (machine i = voxelBounds.min.x; i <= voxelBounds.max.x; i++)
				{
					...
					if (d < 1.0F)
					{
						...
						if ((materialMap) && (d < 0.0F)) materialMap->SetUnsignedVoxelOctet(i, j, k, material);
					}
				}
			}
		}
			...
			for (machine j = voxelBounds.min.y; j <= voxelBounds.max.y; j++)
			{
				...
				for (machine i = voxelBounds.min.x; i <= voxelBounds.max.x; i++)
				{
					...
					if (r < squaredRadius)
					{
						...
						if ((materialMap) && (f < 0.0F)) materialMap->SetUnsignedVoxelOctet(i, j, k, material);
					}
				}
			}
		}
			...
			for (machine j = voxelBounds.min.y; j <= voxelBounds.max.y; j++)
			{
				...
				for (machine i = voxelBounds.min.x; i <= voxelBounds.max.x; i++)
				{
					...
					if (SquaredMag(q) < squaredRadius)
					{
						...
						if (materialMap) materialMap->SetUnsignedVoxelOctet(i, j, k, material);
					}
				}
			}
		}
		...
		for (machine k = voxelBounds.min.z; k <= voxelBounds.max.z; k++)
		{
			...
			for (machine j = voxelBounds.min.y; j <= voxelBounds.max.y; j++)
			{
				...
				for (machine i = voxelBounds.min.x; i <= voxelBounds.max.x; i++)
				{
					...
					if (SquaredMag(q) < squaredRadius) materialMap->SetUnsignedVoxelOctet(i, j, k, material);
				}
			}
		}
	}
	else
	{
		for (machine k = voxelBounds.min.z; k <= voxelBounds.max.z; k++)
		{
			...
			for (machine j = voxelBounds.min.y; j <= voxelBounds.max.y; j++)
			{
				...
				{
					Point3D position((float) i, y, z);
					if (SquaredMag(position - p1) < squaredRadius) materialMap->SetUnsignedVoxelOctet(i, j, k, material);
				}
			}
		}
	}
			...
			for (machine j = voxelBounds.min.y; j <= voxelBounds.max.y; j++)
			{
				...
				for (machine i = voxelBounds.min.x; i <= voxelBounds.max.x; i++)
				{
					...
					if (d3 < 0.0F)
					{
						...
						float d2 = topPlane ^ position;
						float d = Fmax(d1, d2, d3);
						if (d < 0.0F) materialMap->SetUnsignedVoxelOctet(i, j, k, material);
					}
				}
			}
		}
...
void TerrainPage::UpdateBlock(Editor *editor, TerrainBlock *block, const Integer3D& coord, int32 level, MaterialObject *materialObject, unsigned_int32 flags)
{
	...
	if (!(flags & kTerrainUpdateForceDelete))
	{
		if (!geometry)
		{
			if (level == 0) geometry = new TerrainGeometry(block, coord);
			else geometry = new TerrainLevelGeometry(block, coord, level);
		...
		if (object->GetGeometryLevel(0)->GetVertexCount() != 0)
		{
			if (!geometry->GetManipulator()) EditorManipulator::Install(editor, geometry, false);
		...
		{
			if (geometry->GetManipulator()) editor->DeleteNode(geometry);
			else delete geometry;
		}
	}
	else
	{
		if (geometry) editor->DeleteNode(geometry);
	}
}
