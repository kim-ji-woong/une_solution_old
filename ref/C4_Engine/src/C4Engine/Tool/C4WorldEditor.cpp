...
Editor::Editor(const char *name) : Window("WorldEditor/Window")
{
	...
	SetMinWindowSize(Vector2D(434.0F + (float) kMaxEditorToolBookCount * (kEditorPageWidth + 5.0F), 558.0F));
	...
	if (name)
	{
		...
		if ((object) && (object->GetObjectType() == kObjectEditor)) editorObject = static_cast<EditorObject *>(object);
	...
	if (!GetTargetZone()) editorObject->SetTargetZone(static_cast<Zone *>(rootNode));
	...
	for (machine a = 0; a < kEditorToolCount; a++) standardTool[a] = nullptr;
...
Editor::~Editor()
{
	if (editorWorld)
	{
		...
		for (machine a = kEditorViewportCount - 1; a >= 0; a--) delete editorViewport[a];
		for (machine a = kEditorToolCount - 1; a >= 0; a--) delete standardTool[a];
...
void Editor::Preprocess(void)
{
	...
	for (machine a = 0; a < kEditorToolCount; a++) toolButton[a] = static_cast<IconButtonWidget *>(FindWidget(toolIdentifier[a]));
	for (machine a = 0; a < kEditorFlagCount; a++) flagButton[a] = static_cast<IconButtonWidget *>(FindWidget(flagIdentifier[a]));
	...
	if (editorFlags & kEditorShowBackfaces) flagButton[kEditorFlagBackfaces]->SetValue(1);
	if (editorFlags & kEditorRenderLighting) flagButton[kEditorFlagLighting]->SetValue(1);
	if (editorFlags & kEditorDrawFromCenter) flagButton[kEditorFlagCenter]->SetValue(1);
	if (editorFlags & kEditorCapGeometry) flagButton[kEditorFlagCap]->SetValue(1);
	...
	{
		BookWidget *book = new BookWidget(Vector2D(kEditorPageWidth, GetWidgetSize().y - menuBarHeight - 8.0F));
		bookWidget[a] = book;
	...
	while (page)
	{
		int32 index = Min(MaxZero(page->GetBookIndex()), kMaxEditorToolBookCount - 1);
		bookWidget[index]->AppendPage(page);
...
void Editor::PositionWidgets(void)
{
	...
	for (machine a = 0; a < kMaxEditorToolBookCount; a++) bookWidget[a]->SetWidgetSize(Vector2D(kEditorPageWidth, GetWidgetSize().y - menuBarHeight - 8.0F));
...
void Editor::BuildMenus(void)
{
	...
	if (TheWorldEditor->GetSceneImportPluginList()->Empty()) widget->Disable();
	...
	if (TheWorldEditor->GetSceneExportPluginList()->Empty()) widget->Disable();
	worldMenu->AppendMenuItem(widget);
	...
	if (!editorClipboard) widget->Disable();
	editMenu->AppendMenuItem(widget);
	...
	widget = new MenuItemWidget(table->GetString(StringID('MENU', 'EDIT', 'CXFM')), WidgetObserver<Editor>(this, &Editor::HandleCopyTransformMenuItem));
	editorMenuItem[kEditorMenuCopyTransform] = widget;
	editMenu->AppendMenuItem(widget);
	
	widget = new MenuItemWidget(table->GetString(StringID('MENU', 'EDIT', 'PXFM')), WidgetObserver<Editor>(this, &Editor::HandlePasteTransformMenuItem));
	editorMenuItem[kEditorMenuPasteTransform] = widget;
	editMenu->AppendMenuItem(widget);
	...
	widget = new MenuItemWidget(table->GetString(StringID('MENU', 'NODE', 'MZON')), WidgetObserver<Editor>(this, &Editor::HandleMoveToTargetZoneMenuItem), Shortcut('Z', kShortcutShift));
	...
	widget = new MenuItemWidget(table->GetString(StringID('MENU', 'NODE', 'ACON')), WidgetObserver<Editor>(this, &Editor::HandleAutoConnectPortalMenuItem), Shortcut('L', kShortcutShift));
	editorMenuItem[kEditorMenuAutoConnectPortal] = widget;
	nodeMenu->AppendMenuItem(widget);
	
	widget = new MenuItemWidget(table->GetString(StringID('MENU', 'NODE', 'CRZN')), WidgetObserver<Editor>(this, &Editor::HandleConnectRootZoneMenuItem));
	editorMenuItem[kEditorMenuConnectRootZone] = widget;
	nodeMenu->AppendMenuItem(widget);
	...
	viewMenu->AppendMenuItem(new MenuItemWidget(table->GetString(StringID('MENU', 'VIEW', 'SHOW')), WidgetObserver<Editor>(this, &Editor::HandleShowAllMenuItem)));
	
	widget = new MenuItemWidget(table->GetString(StringID('MENU', 'VIEW', 'HIDE')), WidgetObserver<Editor>(this, &Editor::HandleHideSelectedMenuItem), Shortcut('H'));
	editorMenuItem[kEditorMenuHideSelected] = widget;
	viewMenu->AppendMenuItem(widget);
	
	viewMenu->AppendMenuItem(new MenuItemWidget(kLineSolid));
	viewMenu->AppendMenuItem(new MenuItemWidget(table->GetString(StringID('MENU', 'VIEW', 'STZN')), WidgetObserver<Editor>(this, &Editor::HandleShowAllInTargetZoneMenuItem)));
	...
	if (editorFlags & kEditorShowBackfaces) widget->ShowBullet();
	...
	if (editorFlags & kEditorExpandWorlds) widget->ShowBullet();
	layoutMenu->AppendMenuItem(widget);
	...
	if (editorFlags & kEditorExpandModels) widget->ShowBullet();
	layoutMenu->AppendMenuItem(widget);
	...
	if (editorFlags & kEditorRenderLighting) widget->ShowBullet();
	layoutMenu->AppendMenuItem(widget);
	...
	if (editorFlags & kEditorDrawFromCenter) widget->ShowBullet();
	layoutMenu->AppendMenuItem(widget);
	...
	if (editorFlags & kEditorCapGeometry) widget->ShowBullet();
	layoutMenu->AppendMenuItem(widget);
	...
	layoutMenu->AppendMenuItem(new MenuItemWidget(kLineSolid));
	layoutMenu->AppendMenuItem(new MenuItemWidget(table->GetString(StringID('MENU', 'LAYO', 'SETT')), WidgetObserver<Editor>(this, &Editor::HandleEditorSettingsMenuItem)));
...
void Editor::BuildPageMenu(void)
{
	Map<EditorPage>		pageMap;
	
	EditorPage *page = static_cast<EditorPage *>(editorObject->GetFirstEditorPage());
	while (page)
	{
		pageMap.Insert(page);
		page = page->ListElement<EditorPage>::Next();
	...
	pageMenu = new PulldownMenuWidget(table->GetString(StringID('MENU', 'PAGE')));
	menuBar->AppendMenu(pageMenu);
	
	pageMenu->AppendMenuItem(new MenuItemWidget(table->GetString(StringID('MENU', 'PAGE', 'SHWA')), WidgetObserver<Editor>(this, &Editor::HandleShowAllPagesMenuItem)));
	pageMenu->AppendMenuItem(new MenuItemWidget(kLineSolid));
	
	EditorPage *editorPage = pageMap.First();
	while (editorPage)
	{
		MenuItemWidget *widget = new MenuItemWidget(editorPage->GetPageTitle(), WidgetObserver<EditorPage>(editorPage, &EditorPage::HandlePageMenuItem));
		editorPage->SetMenuItem(widget);
		if (editorPage->Visible()) widget->ShowBullet();
		pageMenu->AppendMenuItem(widget);
		
		editorPage = editorPage->MapElement<EditorPage>::Next();
	}
...
void Editor::EnableSelectionItems(void)
{
	...
	editorMenuItem[kEditorMenuCopyTransform]->Enable();
	editorMenuItem[kEditorMenuPasteTransform]->Enable();
...
void Editor::DisableSelectionItems(void)
{
	...
	editorMenuItem[kEditorMenuCopyTransform]->Disable();
	editorMenuItem[kEditorMenuPasteTransform]->Disable();
	...
	if (!selectedConnectorList.First())
	{
		editorMenuItem[kEditorMenuUnconnectNode]->Disable();
		editorMenuItem[kEditorMenuSelectConnectedNode]->Disable();
	}
	
	editorMenuItem[kEditorMenuAutoConnectPortal]->Disable();
	editorMenuItem[kEditorMenuConnectRootZone]->Disable();
	...
	editorMenuItem[kEditorMenuHideSelected]->Disable();
}
...
void Editor::UpdateConditionalItems(void)
{
	...
	while (reference)
	{
		...
		if (++totalSelectCount == 1) selectedNode = node;
		
		if (node != targetZone) targetZoneCount++;
		...
		switch (node->GetNodeType())
		{
			case kNodeGeometry:
			{
				...
				{
					primitiveCount++;
					if (static_cast<const PrimitiveGeometry *>(geometry)->PathPrimitive()) pathRebuildableCount++;
				}
			...
			case kNodeEffect:
			{
				...
				if (type == kEffectTube) pathRebuildableCount++;
				break;
			}
			...
			case kNodePortal:
				...
				if (static_cast<const Portal *>(node)->GetPortalType() != kPortalOcclusion) portalCount++;
				break;
	...
	if (totalSelectCount == 0) return;
	
	if (editorState & kEditorSelectionLocked) editorMenuItem[kEditorMenuUnlockSelection]->Enable();
	else editorMenuItem[kEditorMenuLockSelection]->Enable();
	
	if (!rootNode->GetManipulator()->Selected()) editorMenuItem[kEditorMenuGroup]->Enable();
	
	if (totalSelectCount == 1)
	{
		editorMenuItem[kEditorMenuMoveViewportCameraToNode]->Enable();
		
		if (!(GetManipulator(selectedNode)->GetManipulatorFlags() & kManipulatorLockedSubtree)) editorMenuItem[kEditorMenuPasteSubnodes]->Enable();
		
		const Controller *controller = selectedNode->GetController();
		if (controller)
		{
			ControllerType type = controller->GetControllerType();
			if ((type == kControllerScript) || (type == kControllerPanel)) editorMenuItem[kEditorMenuEditController]->Enable();
		}
	}
	
	if (zoneCount == 1) editorMenuItem[kEditorMenuSetTargetZone]->Enable();
	if (portalCount != 0) editorMenuItem[kEditorMenuAutoConnectPortal]->Enable();
	if (targetZoneCount != 0) editorMenuItem[kEditorMenuMoveToTargetZone]->Enable();
	if (instanceCount != 0) editorMenuItem[kEditorMenuOpenInstancedWorld]->Enable();
	...
	if (materialSettable)
	{
		...
		if (materialRemovable) editorMenuItem[kEditorMenuRemoveMaterial]->Enable();
	}
	
	if ((primitiveCount != 0) || (terrainCount != 0) || (waterCount != 0) || (meshCount != 0)) editorMenuItem[kEditorMenuRebuildGeometry]->Enable();
	if (pathRebuildableCount != 0) editorMenuItem[kEditorMenuRebuildWithNewPath]->Enable();
	...
	if ((geometryCount >= 2) && (geometryCount <= kMaxCombineGeometryCount)) editorMenuItem[kEditorMenuCombineDetailLevels]->Enable();
	...
	while (manipulator)
	{
		if ((totalSelectCount == 1) && (selectedNode != manipulator->GetTargetNode())) connectCount++;
		if (manipulator->GetConnectorSelectionTarget()) unconnectCount++;
		
		manipulator = manipulator->Next();
	}
	
	if (connectCount != 0) editorMenuItem[kEditorMenuConnectNode]->Enable();
	...
	if (!selectedConnectorList.Empty()) editorMenuItem[kEditorMenuConnectRootZone]->Enable();
}
...
bool Editor::EntireGroupSelected(const Node *root)
{
	...
	while (node)
	{
		if (node->GetNodeType() != kNodeGroup)
		{
			...
			if (!manipulator->Selected()) return (false);
		}
		
		if (!EntireGroupSelected(node)) return (false);
		node = node->Next();
	}
...
bool Editor::SaveWorld(bool strip)
{
	...
	if (result == kFileLocked) id = 'LOCK';
	else if (result == kFileAccessDenied) id = 'ACES';
	else if (result == kFileWriteProtected) id = 'PROT';
	else if (result == kFileDiskFull) id = 'DFUL';
...
{
	if (!zone) zone = GetRootNode();
	
	Zone *targetZone = editorObject->GetTargetZone();
	if (zone != targetZone)
	{
		if (targetZone) static_cast<ZoneManipulator *>(GetManipulator(targetZone))->SetTarget(false);
		if (zone) static_cast<ZoneManipulator *>(GetManipulator(zone))->SetTarget(true);
...
void Editor::PostEvent(const EditorEvent& event)
{
	...
	if (type == kEditorEventMaterialModified)
	{
				...
				for (machine a = 0; a < count; a++)
				{
					if (geometry->GetMaterialObject(a) == materialObject)
					{
						...
						while (segment)
						{
							...
							if ((pointer) && (*pointer == materialObject)) segment->InvalidateShaderData();
...
void Editor::Paste(Node *targetNode)
{
	...
	while (nodeReference)
	{
		nodeReference->GetNode()->Preprocess();
		nodeReference = nodeReference->Next();
	}
	
	HandleSelectSubtreeMenuItem(nullptr, nullptr);
	
	AddUndoData(new PasteUndoData(GetSelectionList()));
	GetRootNode()->Update();
	
	nodeReference = GetFirstSelectedNode();
	while (nodeReference)
	{
		Node *node = nodeReference->GetNode();
		NodeType type = node->GetNodeType();
		if (type == kNodeGeometry)
		{
			Geometry *geometry = static_cast<Geometry *>(node);
			...
			for (machine a = 0; a < materialCount; a++)
			{
				...
				if (materialObject)
				{
					...
					if (mo) geometry->SetMaterialObject(a, mo);
				}
			}
		...
		else if (type == kNodeSkybox)
		{
			...
			{
				MaterialObject *mo = editorObject->FindMatchingMaterial(materialObject);
				if (mo) skybox->SetMaterialObject(mo);
			}
...
void Editor::SelectNode(Node *node)
{
	if (!(node->GetNodeFlags() & kNodeNonpersistent))
	{
		...
		if (!manipulator->Selected())
		{
			...
			unsigned_int32 state = (editorState & ~kEditorDisableSelectionItems) | (kEditorRedrawViewports | kEditorSelectionUpdated);
...
void Editor::UnselectNode(Node *node)
{
	...
	if (manipulator->Selected())
	{
		...
		while (reference)
		{
			...
			{
				if (reference == gizmoTarget) DisableGizmo(reference);
				delete reference;
				break;
			}
		...
		unsigned_int32 state = editorState | (kEditorRedrawViewports | kEditorSelectionUpdated);
...
{
	if (root->GetSuperNode()) SelectNode(root);
...
void Editor::UnselectAll(void)
{
	for (;;)
	{
		...
		UnselectNode(reference->GetNode());
	}
}
...
void Editor::SelectConnector(Node *node, int32 index, bool toggle)
{
	...
	if (manipulator->GetManipulatorState() & kManipulatorConnectorSelected) selectedConnectorList.Append(manipulator);
	else manipulator->Detach();
...
void Editor::UnselectNonGeometryNodes(void)
{
	...
	while (reference)
	{
		...
		if (node->GetNodeType() != kNodeGeometry) UnselectNode(node);
...
{
	unsigned_int32 mask = editorObject->GetSelectionMask();
	if ((root->GetSuperNode()) && (NodeSelectable(root, mask))) SelectNode(root);
	
	Node *node = root->GetFirstSubnode();
	while (node)
	{
		if (NodeSelectable(node, mask)) SelectNode(node);
		node = root->GetNextNode(node);
	}
}
			...
			if (node->GetNodeType() == kNodeGeometry)
			{
				...
				if (surfaceCount > 0)
				{
					...
					for (machine a = 0; a < materialCount; a++)
					{
						...
						{
							SelectNode(node);
							GeometryManipulator *manipulator = static_cast<GeometryManipulator *>(GetManipulator(node));
							
							for (machine b = 0; b < surfaceCount; b++)
							{
								const SurfaceData *data = object->GetSurfaceData(b);
								if (geometry->GetMaterialObject(data->materialIndex) == materialObject) manipulator->SelectSurface(b);
							}
				...
				{
					if (geometry->GetMaterialObject(0) == materialObject) SelectNode(geometry);
				}
			}
...
void Editor::ExpandWorld(Instance *instance, int32 depth)
{
	if (editorObject->GetEditorFlags() & kEditorExpandWorlds)
	{
		...
		if (depth < kWorldMaxInstanceDepth) ExpandAllWorlds(instance, depth + 1);
...
void Editor::ExpandAllModels(void)
{
	...
	while (node)
	{
		if (node->GetNodeType() == kNodeModel) ExpandModel(static_cast<Model *>(node));
		node = root->GetNextNode(node);
	}
}
...
void Editor::AddUndoData(UndoData *data)
{
	if (undoList.GetElementCount() >= kMaxEditorUndoCount)
	{
		for (;;)
		{
			UndoData *undoData = undoList.First();
			if (!undoData) break;
...
void Editor::DeleteLastUndoData(void)
{
...
void Editor::DeleteNode(Node *node, bool undoable)
{
	if (node != rootNode)
	{
		...
		if (hub)
		{
			...
			while (connector)
			{
				referenceList.Append(new NodeReference(connector->GetStartElement()->GetNode()));
		...
		while (reference)
		{
			reference->GetNode()->ProcessInternalConnectors();
			reference = reference->Next();
		}
...
void Editor::ReparentSelectedNodes(Node *super)
{
	AddUndoData(new ReparentUndoData(&selectionList));
	
	Zone *newZone = (super->GetNodeType() == kNodeZone) ? static_cast<Zone *>(super) : super->GetOwningZone();
	
	const NodeReference *reference = GetFirstSelectedNode();
	while (reference)
	{
		Node *node = reference->GetNode();
		if ((node != super) && (!node->Successor(super)) && (node->GetSuperNode() != super))
		{
			if (!node->GetSuperNode()->GetManipulator()->Selected())
			{
				EditorManipulator *manipulator = GetManipulator(node);
				manipulator->InvalidateGraph();
				...
				node->SetNodeTransform(super->GetInverseWorldTransform() * node->GetWorldTransform());
				super->AddSubnode(node);
				
				if (newZone != oldZone) node->Preprocess();
				
				if (node->GetNodeType() == kNodeGeometry) InvalidateGeometry(static_cast<Geometry *>(node));
	...
	super->Invalidate();
	GetManipulator(super)->InvalidateGraph();
}
...
void Editor::InvalidateGeometry(Geometry *geometry)
{
	...
	editorState |= kEditorRedrawViewports;
	if ((gizmoTarget) && (gizmoTarget->GetNode() == geometry)) PostEvent(GizmoEditorEvent(kEditorEventGizmoTargetModified, geometry));
}
...
void Editor::RebuildGeometry(Geometry *geometry)
{
	...
	else if (type == kGeometryTerrain)
	{
		...
		terrainObject->Build(geometry);
		if (terrainObject->GetDetailLevel() != 0) static_cast<TerrainLevelGeometry *>(geometry)->ProcessStructure();
	}
	...
	{
		if (type == kGeometryMesh) static_cast<MeshGeometryObject *>(object)->Rebuild(geometry);
...
void Editor::RegenerateTexcoords(Geometry *geometry)
{
	...
	for (machine a = 0; a < levelCount; a++)
	{
		...
		{
			if (level->GetArray(kArrayTangent)) level->CalculateTangentArray();
		}
	}
...
void Editor::RegenerateTexcoords(const List<NodeReference> *nodeList)
{
	...
	{
		Node *node = reference->GetNode();
		if (node->GetNodeType() == kNodeGeometry) RegenerateTexcoords(static_cast<Geometry *>(node));
...
bool Editor::NodeSelectable(const Node *node, unsigned_int32 mask)
{
	if ((mask & kEditorNonpersistentMask) || (!(node->GetNodeFlags() & kNodeNonpersistent)))
	{
		unsigned_int32 bit = 0;
		
		switch (node->GetNodeType())
		{
			case kNodeGeometry:
				
				bit = 1 << kEditorNodeGeometry;
				break;
			
			case kNodeCamera:
				
				bit = 1 << kEditorNodeCamera;
				break;
			
			case kNodeLight:
				
				bit = 1 << kEditorNodeLight;
				break;
			
			case kNodeSource:
				
				bit = 1 << kEditorNodeSource;
				break;
			
			case kNodeSpace:
				
				bit = 1 << kEditorNodeSpace;
				break;
			
			case kNodeZone:
				
				bit = 1 << kEditorNodeZone;
				break;
			
			case kNodePortal:
				
				bit = 1 << kEditorNodePortal;
				break;
			...
			case kNodeInstance:
				...
				bit = 1 << kEditorNodeMarker;
				break;
			
			case kNodeTrigger:
				
				bit = 1 << kEditorNodeTrigger;
				break;
			...
			case kNodePhysics:
				...
				bit = 1 << kEditorNodeEffect;
				break;
			
			case kNodeModel:
				
				bit = 1 << kEditorNodeModel;
				break;
...
bool Editor::MaskedPickFilter(const Node *node, const PickData *pickData, void *cookie)
{
	const EditorManipulator *manipulator = GetManipulator(node);
	if (!manipulator->Hidden()) return (NodeSelectable(node, *static_cast<unsigned_int32 *>(cookie)));
	return (false);
}

bool Editor::MaskedSelectedPickFilter(const Node *node, const PickData *pickData, void *cookie)
{
	const EditorManipulator *manipulator = GetManipulator(node);
	if ((manipulator->GetManipulatorState() & (kManipulatorHidden | kManipulatorSelected)) == kManipulatorSelected) return (NodeSelectable(node, *static_cast<unsigned_int32 *>(cookie)));
	return (false);
}

Node *Editor::PickNode(Node *root, Ray *ray, PickData *pickData, PickFilterProc *filterProc, void *filterCookie) const
{
	float			t1, t2;
	unsigned_int32	pickMask;
	
	Node *hitNode = nullptr;
	if (!filterProc)
	{
		filterProc = &MaskedPickFilter;
		pickMask = editorObject->GetSelectionMask();
		filterCookie = &pickMask;
	}
	...
	if (r == 0.0F) r = kFrustumRenderScale * 12.0F;
...
Node *Editor::PickHandle(int32 viewportIndex, Ray *ray, int32 *index) const
{
	...
	{
		Node *node = reference->GetNode();
		if (NodeSelectable(node, editorObject->GetSelectionMask()))
		{
			float	t1, t2;
...
Node *Editor::PickNode(EditorTrackData *trackData, PickData *pickData, PickFilterProc *filterProc, void *filterCookie) const
{
	if (trackData->viewportType != kEditorViewportGraph) return (PickNode(rootNode, &trackData->worldRay, pickData, filterProc, filterCookie));
	return (nullptr);
}

Node *Editor::PickNode(EditorTrackData *trackData, PickData *pickData, unsigned_int32 mask) const
{
	if (trackData->viewportType != kEditorViewportGraph)
	{
		PickFilterProc *filterProc = ((editorState & kEditorSelectionLocked) != 0) ? &MaskedSelectedPickFilter : &MaskedPickFilter;
		if (mask != 0)
		{
			return (PickNode(rootNode, &trackData->worldRay, pickData, filterProc, &mask));
		}
		else
		{
			mask = editorObject->GetSelectionMask() | kEditorNonpersistentMask;
			Node *node = PickNode(rootNode, &trackData->worldRay, pickData, filterProc, &mask);
			if (node)
			{
				while (node->GetNodeFlags() & kNodeNonpersistent) node = node->GetSuperNode();
				return (node);
			}
		}
	}
...
{
	if (trackData->viewportType == kEditorViewportOrtho) return (PickHandle(trackData->viewportIndex, &trackData->worldRay, index));
	return (nullptr);
}
...
Node *Editor::PickGizmoMover(EditorTrackData *trackData) const
{
	if (gizmoTarget)
	{
		...
		if (gizmo)
		{
			int32 index = gizmo->PickMover(trackData, &trackData->worldRay);
			if (index >= 0)
			{
...
Node *Editor::PickGizmoRotator(EditorTrackData *trackData) const
{
	if (gizmoTarget)
	{
		Node *node = gizmoTarget->GetNode();
		...
		if (gizmo)
		{
			int32 index = gizmo->PickRotator(trackData, &trackData->worldRay);
			if (index >= 0)
			{
				trackData->gizmo = gizmo;
				trackData->gizmoIndex = index;
...
void Editor::AutoScroll(const EditorTrackData *trackData)
{
	...
	if ((x < 0.0F) || (y < 0.0F) || (x > width) || (y > height))
	{
		...
		if (x < 0.0F) dx = Fmax((x - 15.0F) * dt, width * -0.5F);
		else if (x > width) dx = Fmin((x - width + 16.0F) * dt, width * 0.5F);
		
		if (y < 0.0F) dy = Fmax((float) (y - 15.0F) * dt, height * -0.5F);
		else if (y > height) dy = Fmin((float) (y - height + 16.0F) * dt, height * 0.5F);
...
void Editor::InitNewNode(EditorTrackData *trackData, Node *node)
{
	...
	{
		Transform4D transform(K::identity_3D, trackData->currentPickPoint);
		if (GetManipulator(node)->GetManipulatorFlags() & kManipulatorModifiablePlacement) ModifyPlacement(&transform, trackData->currentPickNormal);
		node->SetNodeTransform(targetZone->GetInverseWorldTransform() * transform);
		targetZone->AddSubnode(node);
	}
	else if (viewportType == kEditorViewportGraph)
	{
		trackData->superNode->AddSubnode(node);
	}
	else
	{
	...
	else
	{
		if (type == kNodeEffect)
		{
			...
			if (effect->GetEffectType() == kEffectTube) static_cast<TubeEffect *>(effect)->GetObject()->Build();
		}
...
void Editor::CommitNewNode(EditorTrackData *trackData, bool commit)
{
	...
	if (commit)
	{
		AddUndoData(new CreateUndoData(node));
	}
	else
	{
		if (gizmoTarget) DisableGizmo(gizmoTarget);
		selectionList.Purge();
		delete node;
...
EditorTrackData *Editor::CalculateTrackData(const ViewportWidget *viewport, const Point3D& position)
{
	...
	void *filterCookie = pickFilterCookie;
	
	if (!filterProc)
	{
		filterProc = &MaskedPickFilter;
		pickMask = (1 << kEditorNodeGeometry) | kEditorNonpersistentMask;
		filterCookie = &pickMask;
	}
...
bool Editor::SetTrackPickFilter(EditorTrackData *trackData, PickFilterProc *proc, void *cookie)
{
	PickData	pickData;
...
void Editor::ModifyPlacement(Transform4D *transform, const Vector3D& normal) const
{
	if (placementModifierProc) (*placementModifierProc)(this, transform, normal, placementModifierCookie);
}
...
{
	currentCursor = nullptr;
	if (currentTool) currentTool->Disengage(this, cookie);
	if (tool) tool->Engage(this, cookie);
	currentTool = tool;
}
...
void Editor::ConstructViewport(int32 index)
{
	...
	{
		OrthoEditorViewport *orthoViewport = new OrthoEditorViewport(this, editorObject->GetViewportData(index), mode);
	...
	{
		FrustumEditorViewport *frustumViewport = new FrustumEditorViewport(this);
	...
	{
		GraphEditorViewport *graphViewport = new GraphEditorViewport(this, editorObject->GetViewportData(index));
	...
	ViewportWidget *viewportWidget = viewport->GetViewportWidget();
	viewportWidget->SetViewportIndex(index);
	AddNewSubnode(viewportWidget);
}
...
void Editor::ShowViewport(int32 index)
{
	...
	{
		if (index == editorObject->GetFullViewportIndex()) editorObject->SetCurrentViewportLayout(editorObject->GetPreviousViewportLayout());
		else editorObject->SetFullViewportIndex(index);
	}
	else
	{
...
void Editor::UpdateViewportStructures(void)
{
	...
	unsigned_int32 bookLocation = editorObject->GetBookLocation();
	for (machine a = 0; a < kMaxEditorToolBookCount; a++)
	{
		unsigned_int32 location = bookLocation & 0x0F;
		bookLocation >>= 4;
		
		if (location == 1) leftMargin += kEditorPageWidth + 5.0F;
		else if (location == 2) rightMargin += kEditorPageWidth + 5.0F;
	}
	
	toolGroup->SetWidgetPosition(Point3D(leftMargin + 2.0F, toolGroup->GetWidgetPosition().y, 0.0F));
	toolGroup->Invalidate();
	
	float leftPosition = 4.0F;
	float rightPosition = width - rightMargin + 5.0F;
	
	bookLocation = editorObject->GetBookLocation();
	for (machine a = 0; a < kMaxEditorToolBookCount; a++)
	{
		unsigned_int32 location = bookLocation & 0x0F;
		bookLocation >>= 4;
		...
		else
		{
			...
			Page *page = bookWidget[a]->GetFirstPage();
			while (page)
			{
				if (page->GetWidgetState() & kWidgetCollapsed) page->Collapse();
				page = page->Next();
			}
			
			if (location == 1)
			{
				bookWidget[a]->SetWidgetPosition(Point3D(leftPosition, menuBarHeight + 4.0F, 0.0F));
				leftPosition += kEditorPageWidth + 5.0F;
			}
			else if (location == 2)
			{
				bookWidget[a]->SetWidgetPosition(Point3D(rightPosition, menuBarHeight + 4.0F, 0.0F));
				rightPosition += kEditorPageWidth + 5.0F;
			}
		}
	}
	...
	if (layout == kEditorLayout1)
	{
		...
		for (machine a = 0; a < kEditorViewportCount; a++)
		{
			if (a == index) editorViewport[a]->Show();
			else editorViewport[a]->Hide();
		}
	...
	else
	{
		...
		for (machine a = 0; a < kEditorViewportCount; a++)
		{
			if ((flags >> a) & 1) editorViewport[a]->Show();
			else editorViewport[a]->Hide();
		}
	}
...
void Editor::InvalidateFrustumViewports(void)
{
	for (machine a = 0; a < kEditorViewportCount; a++)
	{
		EditorViewport *viewport = editorViewport[a];
		if (viewport->GetEditorViewportType() == kEditorViewportFrustum) viewport->Invalidate();
	}
}
...
{
	if (editorState & kEditorSelectionUpdated) PostEvent(EditorEvent(kEditorEventSelectionUpdated));
	
	unsigned_int32 state = editorState;
	if (state & kEditorProcessProperties) editorWorld->ProcessWorldProperties();
	
	if ((state & kEditorRedrawViewports) && (!GetFirstSubwindow()))
	{
		state &= ~kEditorRedrawViewports;
		for (machine a = 0; a < kEditorViewportCount; a++) editorViewport[a]->Invalidate();
	}
	
	if (state & kEditorDisableSelectionItems) DisableSelectionItems();
	else if (state & kEditorEnableSelectionItems) EnableSelectionItems();
	else if (state & kEditorUpdateConditionalItems) UpdateConditionalItems();
	...
	{
		Cursor *cursor = (trackingTool) ? currentCursor : nullptr;
		...
		for (machine a = 0; a < kEditorViewportCount; a++)
		{
			...
			if (viewportWidget->Visible())
			{
				...
				if ((p.x >= 0.0F) && (p.x < viewportWidget->GetWidgetSize().x) && (p.y >= 0.0F) && (p.y < viewportWidget->GetWidgetSize().y))
				{
					if (!trackingTool)
					{
						if (InterfaceMgr::GetOptionKey()) cursor = TheWorldEditor->GetEditorCursor(kEditorCursorHand);
						else cursor = currentCursor;
						
						if ((!cursor) && (editorState & kEditorSelectionLocked)) cursor = TheWorldEditor->GetEditorCursor(kEditorCursorLock);
					}
...
void Editor::EnterForeground(void)
{
	...
	if (editorClipboard) editorMenuItem[kEditorMenuPaste]->Enable();
}
	...
	if (eventType == kEventKeyDown)
	{
		if (trackingTool)
		{
			...
			if (trackingTool == tool)
			{
				...
				if (code - 'a' < 26U) code -= 0x0020;
				
				unsigned_int32 flags = tool->GetCameraFlags();
				
				if (code == 'W') flags |= kFreeCameraForward;
				else if (code == 'S') flags |= kFreeCameraBackward;
				else if (code == 'A') flags |= kFreeCameraLeft;
				else if (code == 'D') flags |= kFreeCameraRight;
				else if (code == ' ') flags |= kFreeCameraUp;
				else if (code == 'C') flags |= kFreeCameraDown;
		...
		else
		{
			...
			if (!GetFocusWidget())
			{
				...
				{
					if (code == '0') toolButton[kEditorToolNodeBoxSelect]->SetValue(1, true);
					else if (code == '1') toolButton[kEditorToolNodeSelect]->SetValue(1, true);
					else if (code == '2') toolButton[kEditorToolNodeMove]->SetValue(1, true);
					else if (code == '3') toolButton[kEditorToolNodeRotate]->SetValue(1, true);
					else if (code == '4') toolButton[kEditorToolNodeResize]->SetValue(1, true);
					else if (code == '5') toolButton[kEditorToolConnect]->SetValue(1, true);
					else if (code == '6') toolButton[kEditorToolSurfaceSelect]->SetValue(1, true);
					else if (code == '8') toolButton[kEditorToolOrbitCamera]->SetValue(1, true);
					else if (code == '9') toolButton[kEditorToolFreeCamera]->SetValue(1, true);
					return (true);
				}
			}
		}
...
void Editor::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	{
		if (widget == flagButton[kEditorFlagBackfaces])
		{
...
WorldEditor::WorldEditor() :
		...
		manipulatorConstructor(&EditorManipulator::Construct),
		...
		openModelItem(stringTable.GetString(StringID('MODL', 'MOPN')), WidgetObserver<WorldEditor>(this, &WorldEditor::HandleOpenModelMenuItem))
{
	...
	Manipulator::InstallConstructor(&manipulatorConstructor);
	
	for (machine a = 0; a < kEditorCursorCount; a++) editorCursor[a] = new Cursor(cursorTextureName[a]);
...
WorldEditor::~WorldEditor()
{
	...
	for (machine a = kEditorCursorCount - 1; a >= 0; a--) delete editorCursor[a];
}
...
Object *WorldEditor::ConstructObject(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kObjectEditor:
			
			if ((unpackFlags & (kUnpackEditor | kUnpackNonpersistent)) == kUnpackEditor) return (new EditorObject);
			break;
	}
...
void WorldEditor::HandleNewWorldMenuItem(Widget *widget, const WidgetEventData *eventData)
{
	Editor::Open(nullptr);
}

void WorldEditor::HandleOpenWorldMenuItem(Widget *widget, const WidgetEventData *eventData)
{
...
void WorldEditor::HandleReopenWorldMenuItem(Widget *widget, const WidgetEventData *eventData)
{
...
void WorldEditor::HandleNewPanelMenuItem(Widget *widget, const WidgetEventData *eventData)
{
	PanelEditor::Open(nullptr);
}

void WorldEditor::HandleOpenPanelMenuItem(Widget *widget, const WidgetEventData *eventData)
{
...
void WorldEditor::HandleOpenModelMenuItem(Widget *widget, const WidgetEventData *eventData)
{
