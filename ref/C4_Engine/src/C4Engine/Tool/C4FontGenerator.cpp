...
const ConstPoint3D GlyphWidget::backgroundVertex[12] =
{
	{0.0F, 0.0F, 0.0F}, {0.0F, 32.0F, 0.0F}, {32.0F, 32.0F, 0.0F}, {32.0F, 0.0F, 0.0F},
	{0.0F, 0.0F, 0.0F}, {0.0F, 32.0F, 0.0F}, {32.0F, 32.0F, 0.0F}, {32.0F, 0.0F, 0.0F},
	{-1.0F, -1.0F, 0.0F}, {-1.0F, 33.0F, 0.0F}, {33.0F, 33.0F, 0.0F}, {33.0F, -1.0F, 0.0F}
};

const ConstColorRGB GlyphWidget::backgroundColor[12] =
{
...
const Triangle GlyphWidget::backgroundTriangle[10] =
{
	{{0, 1, 2}}, {{0, 2, 3}},
	{{4, 9, 5}}, {{4, 8, 9}},
	{{5, 10, 6}}, {{5, 9, 10}},
...
{
	validPage[0] = true;
	for (machine a = 1; a < 256; a++) validPage[a] = false;
		...
		for (machine a = 0; a < count; a++)
		{
			...
			if (first < 0xFFF0)
			{
				...
				first >>= 8;
				last >>= 8;
				for (unsigned_machine b = first; b <= last; b++) validPage[b] = true;
			}
		}
	
	#elif C4MACOS
	
		CTFontRef coreTextFont = CTFontCreateWithFontDescriptor(fontData->fontDescriptor, (CGFloat) size, nullptr);
		...
		graphicsFont = CTFontCopyGraphicsFont(coreTextFont, nullptr);
...
bool SystemFont::ValidBlock(const UnicodeBlock *block) const
{
		...
		for (machine a = 0; a < count; a++)
		{
			unsigned_int32 first = glyphSet->ranges[a].wcLow;
			unsigned_int32 last = glyphSet->ranges[a].cGlyphs + first - 1;
			if ((first <= block->lastCode) && (last >= block->firstCode)) return (true);
		}
...
{
	if ((code < 0x20U) || (code - 0x7F < 0x21U) || (code >= 0xFFF0)) return (false);
		...
		for (machine a = 0; a < count; a++)
		{
			...
			if (code - first < length) return (true);
		}
...
bool SystemFont::GetGlyphImage(unsigned_int32 code, GlyphMetric *metric, Color2C **image) const
{
		...
		*reinterpret_cast<Fixed *>(&matrix.eM22) = 0x00010000;
			...
			if ((code & 0xFF7F) == 0x0020)
			{
				...
				if (image)
				{
					...
					*image = img;
					...
					for (machine i = 0; i < w; i++) img[i].Set(0, 0);
				}
				...
				if (image)
				{
					...
					*image = img;
					...
					for (machine j = 0; j < h; j++)
					{
						...
						{
							unsigned_int32 value = (src[i] * 255UL) >> 6;
							img[i].Set(255, value);
						}
		...
		else
		{
			...
			CGContextShowGlyphsAtPoint(graphicsContext, 0.0F, (CGFloat) (fontHeight - fontAscent), &glyph, 1);
			...
			for (; top < fontHeight; top++)
			{
				...
				for (machine i = 1; i < contextWidth; i++) accum |= src[i];
				if (accum != 0) break;
				src += contextRowBytes;
			}
			...
			if (top < fontHeight)
			{
				...
				for (; left < contextWidth; left++)
				{
					...
					for (machine j = top + 1; j <= bottom; j++) accum |= src[contextRowBytes * j];
					if (accum != 0) break;
					src++;
				}
				...
				if (image)
				{
					...
					*image = img;
					...
					const unsigned_int8 *src = imageStorage + contextRowBytes * top + left;
					for (machine j = 0; j < h; j++)
					{
						for (machine i = 0; i < w; i++)
						{
...
void GlyphWidget::Preprocess(void)
{
	...
	backgroundRenderable.SetAttributeArray(kArrayColor0, &backgroundColor[0]);
	backgroundRenderable.SetTriangleArray(10, backgroundTriangle);
}

void GlyphWidget::Build(void)
{
	if (GetWidgetState() & kWidgetHilited) diffuseAttribute.SetDiffuseColor(TheInterfaceMgr->GetInterfaceColor(kInterfaceColorHilite));
	else diffuseAttribute.SetDiffuseColor(K::white);
...
void FontGeneratorWindow::Preprocess(void)
{
	...
	for (machine a = 0x0020; a < 0x007F; a++) glyphIncluded[a] = true;
	for (machine a = 0x00A0; a < 0x0100; a++) glyphIncluded[a] = true;
	...
	if (systemFont.GetGlyphImage(code, &gm, (image) ? &img1 : nullptr))
	{
		if (shadowFlag)
		{
			...
			if (image)
			{
				...
				int32 divider = 9;
				if (r == 1) divider = kBlur3x3Total;
				else if (r == 2) divider = kBlur5x5Total;
				divider *= 255;
				...
				for (machine j = 0; j < h2; j++)
				{
					...
					for (machine i = 0; i < w2; i++)
					{
						...
						for (machine t = t1; t <= t2; t++)
						{
							...
							for (machine s = s1; s <= s2; s++) accum += src[s].GetAlpha() * k[s];
						}
	...
	for (machine a = 0; a < 256; a++)
	{
		...
		if (systemFont.ValidGlyph(code))
		{
			...
			if (GetShadowGlyphImage(code, &metric, &image))
			{
				...
				tableGroup->AddNewSubnode(glyph);
				...
				if (glyphIncluded[code]) glyph->Include();
				else glyph->Exclude();
				
				if ((code >= selectionBegin) && (code <= selectionEnd)) glyph->Select();
			}
		}
	}
}
...
void FontGeneratorWindow::SetSelection(unsigned_int32 begin, unsigned_int32 end)
{
	...
	while (glyph)
	{
		...
		if ((code >= begin) && (code <= end)) glyph->Select();
		else glyph->Unselect();
...
void FontGeneratorWindow::HandleConfigurationEvent(SettingInterface *settingInterface)
{
	...
	if ((identifier == 'SHAD') || (identifier == 'STYP') || (identifier == 'DARK') || (identifier == 'SHDX') || (identifier == 'SHDY'))
	{
		...
		if (flag | shadowFlag) BuildGlyphTable();
	}
}
...
bool FontGeneratorWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyDown)
	{
		...
		else if (((code == kKeyCodeLeftArrow) && (GetFocusWidget() == blockList)) || (code == kKeyCodePageUp))
		{
		...
		else if (((code == kKeyCodeRightArrow) && (GetFocusWidget() == blockList)) || (code == kKeyCodePageDown))
		{
...
void FontGeneratorWindow::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		...
		else if (widget == includeButton)
		{
			...
			for (unsigned_machine a = begin; a <= end; a++) glyphIncluded[a] = true;
			
			GlyphWidget *glyph = glyphList.First();
			while (glyph)
			{
				unsigned_int32 code = glyph->GetGlyphCode();
				if ((code >= begin) && (code <= end)) glyph->Include();
		...
		else if (widget == excludeButton)
		{
			...
			unsigned_int32 end = selectionEnd;
			for (unsigned_machine a = begin; a <= end; a++) glyphIncluded[a] = false;
			
			GlyphWidget *glyph = glyphList.First();
			while (glyph)
			{
				unsigned_int32 code = glyph->GetGlyphCode();
				if ((code >= begin) && (code <= end)) glyph->Exclude();
...
void FontGeneratorWindow::LoadRangesPicked(FilePicker *picker, void *cookie)
{
	...
	if (ranges)
	{
		...
		{
			unsigned_int32 code = glyph->GetGlyphCode();
			if (window->glyphIncluded[code]) glyph->Include();
			else glyph->Exclude();
...
EngineResult FontGeneratorWindow::GenerateFont(void)
{
			...
			for (machine b = 0; b < 256; b++)
			{
				...
				if ((glyphIncluded[code]) && (systemFont.ValidGlyph(code)))
				{
					...
					if (GetShadowGlyphImage(code, &metric))
					{
						...
						if (code - 0x0030 < 10U) digitWidth = Max(digitWidth, metric.width);
	...
	for (machine a = 0; a < 256; a++)
	{
		if (pageIndex[a] != 0) pageIndex[a] = (unsigned_int8) pageCount++;
	}
	...
	for (machine a = 0; a < glyphCount; a++)
	{
		...
		if (u >= textureWidth - glyphShadowWidth)
		{
			u = kGlyphPadding;
			v += rowSpacing;
			if (v >= textureHeight - fontShadowHeight) imageFlag = false;
		}
		...
		if (imageFlag)
		{
			...
			{
				for (machine i = 0; i < metric.shadowWidth; i++) dst[i] = src[i];
		...
		data->glyphFlags = 0;
		data->glyphWidth = metric.width + extraPadding;
		data->glyphHeight = fontHeight;
		data->glyphTop = 0;
	...
	if (result == kTextureImportOkay)
	{
		...
		if (result == kFileOkay)
		{
			...
			fontHeader.fontHeight = fontHeight;
			fontHeader.fontBaseline = systemFont.GetFontAscent();
			fontHeader.fontSpacing = fontSpacing;
			fontHeader.fontLeading = fontLeading;
...
FontPicker::FontWidget::~FontWidget()
{
	...
	int CALLBACK FontPicker::EnumFontsCallback(const LOGFONTA *logData, const TEXTMETRICA *physData, DWORD fontType, LPARAM data)
	{
		...
		if (name[0] > '@') fontList->InsertSortedListItem(new FontWidget(fontList->GetNaturalListItemSize(), name, fontList->GetFontName()));
		return (1);
	}
...
void FontPicker::Preprocess(void)
{
	...
	sizeBox = static_cast<EditTextWidget *>(FindWidget("Size"));
		...
		EnumFontFamiliesExA(GetDC(TheEngine->GetWindow()), &fontData, &EnumFontsCallback, (LPARAM) fontList, 0);
		...
		char **nameList = XListFonts(TheEngine->GetEngineDisplay(), "*", 1024, &count);
		
		for (machine a = 0; a < count; a++)
		{
			fontList->InsertSortedListItem(new FontWidget(fontList->GetNaturalListItemSize(), nameList[a], fontList->GetFontName()));
		}
...
void FontPicker::Move(void)
{
	...
	if ((fontList->GetFirstSelectedListItem()) && (sizeBox->GetText()[0] != 0)) selectButton->Enable();
	else selectButton->Disable();
}

bool FontPicker::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	if (eventData->eventType == kEventKeyDown)
	{
		unsigned_int32 code = eventData->keyCode;
	...
	if (eventData->eventType == kEventWidgetActivate)
	{
		if ((widget == selectButton) || ((widget == fontList) && (sizeBox->GetText()[0] != 0)))
		{
			...
			const FontWidget *fontWidget = static_cast<FontWidget *>(fontList->GetFirstSelectedListItem());
				...
				if (boldBox->GetValue() != 0) flags |= kFontImportBold;
				if (italicBox->GetValue() != 0) flags |= kFontImportItalic;
...
void FontGenerator::HandleGenerateFontMenuItem(Widget *widget, const WidgetEventData *eventData)
{
