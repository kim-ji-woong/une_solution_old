...
{
	if (TheTerrainPaletteWindow) TheInterfaceMgr->SetActiveWindow(TheTerrainPaletteWindow);
	else TheInterfaceMgr->AddWidget(new TerrainPaletteWindow);
}
...
void TerrainPaletteWindow::OutputTexturePicked(FilePicker *picker, void *cookie)
{
	...
	const Widget *widget = textureList->GetFirstListItem();
	while (widget)
	{
		const Widget *subwidget = widget->GetFirstSubnode();
		while (subwidget)
		{
			if (subwidget->GetWidgetType() == kWidgetText)
			{
				const TextWidget *textWidget = static_cast<const TextWidget *>(subwidget);
...
void TerrainPaletteWindow::PaletteEntryPicked(FilePicker *picker, void *cookie)
{
	...
	if (textureList->GetFirstListItem()) window->saveButton->Enable(); 
}

bool TerrainPaletteWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
...
void TerrainPaletteWindow::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		...
		else if (widget == removeButton)
		{
			for (;;)
			{
				Widget *widget = textureList->GetFirstSelectedListItem();
				if (!widget) break;
				
				textureList->RemoveListItem(widget);
				delete widget;
			}
			
			if (!textureList->GetFirstListItem()) saveButton->Disable();
	...
	else if (eventType == kEventWidgetChange)
	{
		...
		{
			if (textureList->GetFirstSelectedListItem()) removeButton->Enable();
...
void TerrainPaletteWindow::GenerateTerrainPalette(const char *outputName, const Array<ResourceName>& inputNameArray)
{
	...
	for (machine a = 0; a < count; a++)
	{
		...
		if (textureHeader->textureType != kTexture2D)
		{
