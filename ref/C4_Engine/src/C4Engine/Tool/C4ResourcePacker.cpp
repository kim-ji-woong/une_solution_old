...
void FileEntry::CalculatePositions(unsigned_int32& mapOffset, unsigned_int32& dataSize)
{
	...
	if (entry) entry->CalculatePositions(mapOffset, dataSize);
...
void FileEntry::WriteMapData(File *file) const
{
	...
	if (leftEntry) leftEntry->WriteMapData(file);
	if (rightEntry) rightEntry->WriteMapData(file);
}
...
void FileEntry::WriteResourceData(File *file) const
{
	...
	if (entry) entry->WriteResourceData(file);
...
void TypeEntry::CalculateTypePositions(unsigned_int32& mapOffset)
{
	...
	if (entry) entry->CalculateTypePositions(mapOffset);
...
void TypeEntry::CalculateFilePositions(unsigned_int32& mapOffset, unsigned_int32& dataSize) 
{
	...
	if (fileEntry) fileEntry->CalculatePositions(mapOffset, dataSize);
	
	TypeEntry *typeEntry = GetLeftSubnode();
	if (typeEntry) typeEntry->CalculateFilePositions(mapOffset, dataSize); 
...
void TypeEntry::WriteTypeMapData(File *file) const
{
	...
	if (leftEntry) leftEntry->WriteTypeMapData(file);
	if (rightEntry) rightEntry->WriteTypeMapData(file);
}

void TypeEntry::WriteFileMapData(File *file) const
{
	FileEntry *fileEntry = fileMap.GetRootNode();
	if (fileEntry) fileEntry->WriteMapData(file);
	
	TypeEntry *typeEntry = GetLeftSubnode();
	if (typeEntry) typeEntry->WriteFileMapData(file);
...
{
	FileEntry *fileEntry = fileMap.GetRootNode();
	if (fileEntry) fileEntry->WriteResourceData(file);
	
	TypeEntry *typeEntry = GetLeftSubnode();
	if (typeEntry) typeEntry->WriteResourceData(file);
...
void DirectoryEntry::CalculatePositions(unsigned_int32& mapOffset, unsigned_int32& dataSize)
{
	...
	if (directoryEntry) directoryEntry->CalculatePositions(mapOffset, dataSize);
...
void DirectoryEntry::WriteMapData(File *file) const
{
	...
	if (directoryEntry) directoryEntry->WriteMapData(file);
}

void DirectoryEntry::WriteResourceData(File *file) const
{
	DirectoryEntry *directoryEntry = GetLeftSubnode();
	if (directoryEntry) directoryEntry->WriteResourceData(file);
...
DirectoryEntry *ResourcePacker::ProcessDirectory(const char *name, const char *path)
{
	...
	FileMgr::BuildFileList(path, &fileList);
	if (!fileList.Empty())
	{
		...
		while (reference)
		{
			...
			if (reference->GetFlags() & kFileDirectory)
			{
				...
				if (entry) directoryEntry->AddDirectory(entry);
			}
			else
			{
