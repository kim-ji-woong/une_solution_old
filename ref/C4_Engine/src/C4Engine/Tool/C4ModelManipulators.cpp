		...
		boneRenderable(kRenderLines)
{
	...
	boneRenderable.SetAttributeArray(kArrayVertex, boneVertex);
...
void BoneManipulator::InvalidateNode(void)
{
	...
	while (node)
	{
		...
		if ((type != kNodeBone) && (type != kNodeGroup))
		{
			...
			while (subnode)
			{
				if (subnode->GetNodeType() == kNodeGeometry)
				{
					...
					if ((controller) && (controller->GetControllerType() == kControllerSkin)) controller->Invalidate();
				}
...
void BoneManipulator::Render(const ManipulatorRenderData *renderData)
{
	...
	if (renderList)
	{
		...
		if ((super) && (super->GetNodeType() == kNodeBone))
		{
			boneVertex[0] = node->GetWorldPosition();
			boneVertex[1] = super->GetWorldPosition();
...
Box3D ModelManipulator::CalculateNodeBoundingBox(void) const
{
	...
	if (node)
	{
		for (;;)
		{
			...
			if (node->CalculateBoundingBox(&box)) modelBox.Union(Transform(box, inverseTransform * node->GetWorldTransform()));
		}
...
{
	const Model *model = GetTargetNode();
	if (model->GetFirstOutgoingEdge()) return (model->GetWorldBoundingBox());
