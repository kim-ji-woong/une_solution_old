...
EditorWidgetManipulator::EditorWidgetManipulator(Widget *widget) :
		...
		widgetDiffuseColor(K::black),
		widgetTextureMap(&widgetTextureHeader, widgetTextureImage),
		widgetRenderable(kRenderQuads, kRenderDepthInhibit)
{
	SetManipulatorState(kWidgetManipulatorBuild);
	
	if (widget->GetWidgetType() != kWidgetGeneric) widgetRenderable.SetTransformable(widget);
	widgetRenderable.SetAmbientBlendState(kBlendInterpolate);
	
	widgetAttributeList.Append(&widgetDiffuseColor);
	widgetAttributeList.Append(&widgetTextureMap);
	widgetRenderable.SetMaterialAttributeList(&widgetAttributeList);
	
	widgetRenderable.SetVertexCount(48);
	widgetRenderable.SetAttributeArray(kArrayVertex, widgetVertex);
	widgetRenderable.SetAttributeArray(kArrayTexture0, widgetTexcoord);
}
...
Box2D EditorWidgetManipulator::GetWidgetBounds(void) const
{
	...
	if (box) return (Box2D(Point2D(box->min.x - 2.0F, box->min.y - 2.0F), Point2D(box->max.x + 2.0F, box->max.y + 2.0F)));
...
void EditorWidgetManipulator::BuildHandle(Point2D *vertex, Point2D *texcoord, float x, float y, float scale)
{
	float f1 = scale * -3.0F;
	float f2 = scale * 4.0F;
	
	vertex[0].Set(x + f1, y + f1);
	vertex[1].Set(x + f1, y + f2);
	vertex[2].Set(x + f2, y + f2);
	vertex[3].Set(x + f2, y + f1);
	
	texcoord[0].Set(0.0F, 0.5F);
	texcoord[1].Set(0.0F, 0.9375F);
	texcoord[2].Set(0.4375F, 0.9375F);
	texcoord[3].Set(0.4375F, 0.5F);
}
...
void EditorWidgetManipulator::Build(float scale)
{
	...
	if (Hidden()) widget->SetEditorState(widget->GetEditorState() | kWidgetHidden);
	else widget->SetEditorState(widget->GetEditorState() & ~kWidgetHidden);
	...
	widgetVertex[0].Set(box.min.x, box.min.y + f1);
	widgetVertex[1].Set(box.min.x, box.min.y + f2);
	widgetVertex[2].Set(sx, box.min.y + f2);
	widgetVertex[3].Set(sx, box.min.y + f1);
	
	widgetVertex[4].Set(box.min.x, sy + f1);
	widgetVertex[5].Set(box.min.x, sy + f2);
	widgetVertex[6].Set(sx, sy + f2);
	widgetVertex[7].Set(sx, sy + f1);
	
	widgetVertex[8].Set(box.min.x + f1, box.min.y);
	widgetVertex[9].Set(box.min.x + f1, sy);
	widgetVertex[10].Set(box.min.x + f2, sy);
	widgetVertex[11].Set(box.min.x + f2, box.min.y);
	
	widgetVertex[12].Set(sx + f1, box.min.y);
	widgetVertex[13].Set(sx + f1, sy);
	widgetVertex[14].Set(sx + f2, sy);
	widgetVertex[15].Set(sx + f2, box.min.y);
	...
	if (widget->GetWidgetType() != kWidgetGeneric)
	{
		widgetTexcoord[0].Set(0.0F, 0.0F);
		widgetTexcoord[1].Set(0.0F, 0.1875F);
		widgetTexcoord[2].Set(sx * 0.0625F, 0.1875F);
		widgetTexcoord[3].Set(sx * 0.0625F, 0.0F);
		
		widgetTexcoord[4].Set(0.0F, 0.0F);
		widgetTexcoord[5].Set(0.0F, 0.1875F);
		widgetTexcoord[6].Set(sx * 0.0625F, 0.1875F);
		widgetTexcoord[7].Set(sx * 0.0625F, 0.0F);
		
		widgetTexcoord[8].Set(0.0F, 0.0F);
		widgetTexcoord[9].Set(sy * 0.0625F, 0.0F);
		widgetTexcoord[10].Set(sy * 0.0625F, 0.1875F);
		widgetTexcoord[11].Set(0.0F, 0.1875F);
		
		widgetTexcoord[12].Set(0.0F, 0.0F);
		widgetTexcoord[13].Set(sy * 0.0625F, 0.0F);
		widgetTexcoord[14].Set(sy * 0.0625F, 0.1875F);
		widgetTexcoord[15].Set(0.0F, 0.1875F);
	}
	else
	{
		widgetTexcoord[0].Set(0.0F, 0.25F);
		widgetTexcoord[1].Set(0.0F, 0.4375F);
		widgetTexcoord[2].Set(sx * 0.0625F, 0.4375F);
		widgetTexcoord[3].Set(sx * 0.0625F, 0.25F);
		
		widgetTexcoord[4].Set(0.0F, 0.25F);
		widgetTexcoord[5].Set(0.0F, 0.4375F);
		widgetTexcoord[6].Set(sx * 0.0625F, 0.4375F);
		widgetTexcoord[7].Set(sx * 0.0625F, 0.25F);
		
		widgetTexcoord[8].Set(0.0F, 0.25F);
		widgetTexcoord[9].Set(sy * 0.0625F, 0.25F);
		widgetTexcoord[10].Set(sy * 0.0625F, 0.4375F);
		widgetTexcoord[11].Set(0.0F, 0.4375F);
		
		widgetTexcoord[12].Set(0.0F, 0.25F);
		widgetTexcoord[13].Set(sy * 0.0625F, 0.25F);
		widgetTexcoord[14].Set(sy * 0.0625F, 0.4375F);
		widgetTexcoord[15].Set(0.0F, 0.4375F);
	}
	
	GetHandlePositions(handle);
	Point2D *vertex = widgetVertex + 16;
	Point2D *texcoord = widgetTexcoord + 16;
	for (machine a = 0; a < 8; a++)
	{
		BuildHandle(vertex, texcoord, handle[a].x, handle[a].y, scale);
		vertex += 4;
		texcoord += 4;
	}
}
...
void EditorWidgetManipulator::Install(Widget *root, bool recursive)
{
	if (!(root->GetWidgetState() & kWidgetNonpersistent))
	{
		...
		if (!manipulator) root->SetManipulator(new EditorWidgetManipulator(root));
...
CreatePanelUndoData::CreatePanelUndoData(Widget *widget)
{
	createList.Append(new WidgetReference(widget));
}

CreatePanelUndoData::CreatePanelUndoData(const List<EditorWidgetManipulator> *selectionList)
{
...
CreatePanelUndoData::~CreatePanelUndoData()
{
}

void CreatePanelUndoData::Undo(PanelEditor *panelEditor)
{
...
DeletePanelUndoData::DeletePanelUndoData(Widget *rootWidget)
{
	Widget *widget = rootWidget->GetFirstSubnode();
	while (widget)
	{
		WidgetManipulator *manipulator = widget->GetManipulator();
		if ((manipulator) && (manipulator->Selected())) deleteList.Append(new DeleteReference(widget));
		
		widget = rootWidget->GetNextNode(widget);
	}
}

DeletePanelUndoData::~DeletePanelUndoData()
{
...
DeletePanelUndoData::DeleteReference::DeleteReference(Widget *widget) : WidgetReference(widget)
{
	superWidget = widget->GetSuperNode();
	precedingWidget = widget->Previous();
}

void DeletePanelUndoData::Undo(PanelEditor *panelEditor)
{
	const DeleteReference *reference = static_cast<const DeleteReference *>(deleteList.First());
	while (reference)
	{
		Widget *widget = reference->GetWidget();
		Widget *super = reference->GetSuperWidget();
		Widget *after = reference->GetPrecedingWidget();
		
		if (after) super->AddSubnodeAfter(widget, after);
		else super->AddFirstSubnode(widget);
...
TransformPanelUndoData::TransformPanelUndoData(const List<EditorWidgetManipulator> *selectionList)
{
	const EditorWidgetManipulator *manipulator = selectionList->First();
	while (manipulator)
	{
...
TransformPanelUndoData::~TransformPanelUndoData()
{
}

TransformPanelUndoData::TransformReference::TransformReference(Widget *widget) : WidgetReference(widget)
{
	widgetTransform = widget->GetWidgetTransform();
	widgetSize = widget->GetWidgetSize();
}

void TransformPanelUndoData::Undo(PanelEditor *panelEditor)
{
...
WarpPanelUndoData::WarpPanelUndoData(const List<EditorWidgetManipulator> *selectionList)
{
	const EditorWidgetManipulator *manipulator = selectionList->First();
	while (manipulator)
	{
...
WarpPanelUndoData::~WarpPanelUndoData()
{
}

WarpPanelUndoData::WarpReference::WarpReference(ImageWidget *widget) : WidgetReference(widget)
{
	imageOffset = widget->GetImageOffset();
	imageScale = widget->GetImageScale();
}

WarpPanelUndoData::ScaleReference::ScaleReference(TextWidget *widget) : WidgetReference(widget)
{
	textScale = widget->GetTextScale();
}

void WarpPanelUndoData::Undo(PanelEditor *panelEditor)
{
...
ArrangePanelUndoData::ArrangePanelUndoData(Widget *rootWidget)
{
	Widget *widget = rootWidget->GetFirstSubnode();
	while (widget)
	{
		WidgetManipulator *manipulator = widget->GetManipulator();
		if ((manipulator) && (manipulator->Selected())) arrangeList.Append(new ArrangeReference(widget));
		
		widget = rootWidget->GetNextNode(widget);
	}
}

ArrangePanelUndoData::~ArrangePanelUndoData()
{
}

ArrangePanelUndoData::ArrangeReference::ArrangeReference(Widget *widget) : WidgetReference(widget)
{
	precedingWidget = widget->Previous();
}

void ArrangePanelUndoData::Undo(PanelEditor *panelEditor)
{
	const WidgetReference *reference = arrangeList.First();
	while (reference)
	{
		Widget *widget = reference->GetWidget();
		Widget *after = static_cast<const ArrangeReference *>(reference)->GetPrecedingWidget();
...
GroupPanelUndoData::GroupPanelUndoData(Widget *group, const Widget *rootWidget)
{
	groupWidget = group;
	...
	while (widget)
	{
		if (widget->GetManipulator()->Selected()) groupList.Append(new GroupReference(widget));
		widget = widget->Next();
	}
}

GroupPanelUndoData::~GroupPanelUndoData()
{
}

GroupPanelUndoData::GroupReference::GroupReference(Widget *widget) : WidgetReference(widget)
{
	precedingWidget = widget->Previous();
	widgetTransform = widget->GetWidgetTransform();
}

void GroupPanelUndoData::Undo(PanelEditor *panelEditor)
{
	...
	while (reference)
	{
		...
		Widget *after = groupReference->GetPrecedingWidget();
		
		if (after) rootWidget->AddSubnodeAfter(widget, after);
		else rootWidget->AddFirstSubnode(widget);
...
UngroupPanelUndoData::UngroupPanelUndoData(const List<EditorWidgetManipulator> *selectionList)
{
	const EditorWidgetManipulator *manipulator = selectionList->First();
	while (manipulator)
	{
...
UngroupPanelUndoData::~UngroupPanelUndoData()
{
}

UngroupPanelUndoData::TransformReference::TransformReference(Widget *widget) : WidgetReference(widget)
{
	widgetTransform = widget->GetWidgetTransform();
}

UngroupPanelUndoData::UngroupReference::UngroupReference(Widget *widget) : WidgetReference(widget)
{
	precedingWidget = widget->Previous();
...
void UngroupPanelUndoData::Undo(PanelEditor *panelEditor)
{
	RootWidget *rootWidget = panelEditor->GetRootWidget();
	...
	while (reference)
	{
		...
		Widget *after = ungroupReference->GetPrecedingWidget();
		
		if (after) rootWidget->AddSubnodeAfter(group, after);
		else rootWidget->AddFirstSubnode(group);
...
TexturePanelUndoData::TexturePanelUndoData(const List<EditorWidgetManipulator> *selectionList)
{
	const EditorWidgetManipulator *manipulator = selectionList->First();
	while (manipulator)
	{
...
TexturePanelUndoData::~TexturePanelUndoData()
{
}

TexturePanelUndoData::TextureReference::TextureReference(ImageWidget *widget) : WidgetReference(widget)
{
	textureName = widget->GetTextureName();
}

void TexturePanelUndoData::Undo(PanelEditor *panelEditor)
{
...
ScriptPanelUndoData::ScriptPanelUndoData(const List<EditorWidgetManipulator> *selectionList)
{
	const EditorWidgetManipulator *manipulator = selectionList->First();
	while (manipulator)
	{
		Widget *widget = manipulator->GetTargetWidget();
		if (widget->GetScriptObject()) scriptList.Append(new ScriptReference(widget));
		
		manipulator = manipulator->Next();
	}
}

ScriptPanelUndoData::~ScriptPanelUndoData()
{
}

ScriptPanelUndoData::ScriptReference::ScriptReference(Widget *widget) : WidgetReference(widget)
{
	scriptObject = widget->GetScriptObject();
	scriptObject->Retain();
}

ScriptPanelUndoData::ScriptReference::~ScriptReference()
{
	scriptObject->Release();
}

void ScriptPanelUndoData::Undo(PanelEditor *panelEditor)
{
...
void PanelTransformPage::HandlePositionTextEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		...
		{
			undoFlag = true;
			editor->AddUndoData(new TransformPanelUndoData(selectionList));
		}
...
String<15> PanelTransformPage::FloatToString(float num)
{
	...
	if ((len > 0) && (string[len] == '.') && (string[len + 1] == '0')) string[len] = 0;
	return (string);
}
...
bool WindowSettingsWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
...
void PanelEditor::Preprocess(void)
{
	...
	cameraObject->SetClearFlags(kClearColorBuffer | kClearDepthBuffer);
	...
	mutatorListBox = static_cast<ListWidget *>(FindWidget("MutatorList"));
...
void PanelEditor::BuildMenus(void)
{
	...
	panelMenu->AppendMenuItem(new MenuItemWidget(table->GetString(StringID('PANL', 'MENU', 'PANL', 'SAVP')), WidgetObserver<PanelEditor>(this, &PanelEditor::HandleSavePanelMenuItem), Shortcut('S')));
	...
	if (!editorClipboard.GetFirstSubnode()) widget->Disable();
	editMenu->AppendMenuItem(widget);
...
void PanelEditor::UpdateMutators(const WidgetManipulator *manipulator)
{
	...
	mutatorListBox->PurgeListItems();
	
	mutatorConfigurationWidget->ReleaseConfiguration();
	validMutatorArray.SetElementCount(0);
	...
	while (mutator)
	{
		const MutatorRegistration *registration = Mutator::FindRegistration(mutator->GetMutatorType());
		mutatorListBox->AppendListItem(registration->GetMutatorName());
...
void PanelEditor::HandleWidgetConfigurationEvent(SettingInterface *settingInterface)
{
	...
	editorState |= kPanelEditorModified;
}
...
void PanelEditor::SelectWidgetTool(const WidgetRegistration *registration)
{
	...
	{
		if (currentMode == kPanelEditorModeTool) toolButton[currentTool]->SetValue(0);
...
void PanelEditor::UpdateViewportScale(float scale)
{
...
void PanelEditor::UnselectAllTemp(void)
{
	...
	while (manipulator)
	{
		...
		if (manipulator->GetManipulatorState() & kWidgetManipulatorTempSelected) UnselectWidget(manipulator->GetTargetWidget());
		manipulator = next;
	}
}
...
bool PanelEditor::SuperWidgetSelected(const Widget *widget) const
{
	...
	while (super)
	{
		if (super == rootWidget) break;
		
		const WidgetManipulator *manipulator = super->GetManipulator();
		if (manipulator->Selected()) return (true);
...
void PanelEditor::DeleteWidget(Widget *widget, bool undoable)
{
	...
	if (undoable) widget->Detach();
	else delete widget;
}

void PanelEditor::AddUndoData(PanelUndoData *data)
{
	if (undoList.GetElementCount() >= kMaxPanelUndoCount) delete undoList.First();
...
void PanelEditor::RemoveUndoData(PanelUndoData *data)
{
	delete data;
	
	if (undoList.Empty()) widgetMenuItem[kPanelMenuUndo]->Disable();
...
void PanelEditor::HandleSavePanelMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	if (targetPanel)
	{
		...
		while (widget)
		{
			targetPanel->AddSubnode(widget->Clone());
			widget = widget->Next();
		}
	...
	{
		if (resourceName[0] == 0) HandleSavePanelAsMenuItem(nullptr, nullptr);
		else SavePanel();
	}
...
{
	PanelUndoData *undoData = undoList.Last();
	if (undoData)
	{
		undoData->Undo(this);
...
void PanelEditor::HandleCopyMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	{
		const Widget *widget = manipulator->GetTargetWidget();
		if (!SuperWidgetSelected(widget)) editorClipboard.AddSubnode(widget->Clone());
...
void PanelEditor::HandlePasteMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	while (widget)
	{
		...
		rootWidget->AddNewSubnode(newItem);
		SelectWidget(newItem);
		
		widget = widget->Next();
	}
	
	AddUndoData(new CreatePanelUndoData(&selectionList));
}
...
void PanelEditor::HandleClearMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
		...
		if ((group->GetWidgetType() == kWidgetGeneric) && (!group->GetManipulator()->Selected()))
		{
			...
			if (widget)
			{
				do
				{
					...
					if ((!manipulator) || (!manipulator->Selected())) goto next;
					widget = widget->Next();
				} while (widget);
	...
	AddUndoData(new DeletePanelUndoData(rootWidget));
...
void PanelEditor::BringWidgetsForward(Widget *root)
{
	...
	while (widget)
	{
		...
		{
			root->AddSubnodeAfter(previous, widget);
		}
	...
	while (widget)
	{
		if (widget->GetManipulator()) BringWidgetsForward(widget);
		widget = widget->Next();
	}
}
...
void PanelEditor::BringWidgetsToFront(Widget *root)
{
	...
	while (widget)
	{
		...
		if ((manipulator) && (manipulator->Selected()))
		{
			root->AddSubnode(widget);
		}
		
		if (widget == last) break;
		widget = next;
	}
	...
	while (widget)
	{
		if (widget->GetManipulator()) BringWidgetsToFront(widget);
		widget = widget->Previous();
	}
}
...
void PanelEditor::SendWidgetsBackward(Widget *root)
{
	...
	while (widget)
	{
		...
		if ((manipulator) && (manipulator->Selected()))
		{
			root->AddSubnodeBefore(next, widget);
		}
	...
	while (widget)
	{
		if (widget->GetManipulator()) SendWidgetsBackward(widget);
		widget = widget->Previous();
	}
}
...
void PanelEditor::SendWidgetsToBack(Widget *root)
{
	...
	while (widget)
	{
		...
		if ((manipulator) && (manipulator->Selected()))
		{
			root->AddFirstSubnode(widget);
		}
		
		if (widget == first) break;
		widget = previous;
	}
	...
	while (widget)
	{
		if (widget->GetManipulator()) SendWidgetsToBack(widget);
		widget = widget->Next();
	}
}
...
{
	AddUndoData(new ArrangePanelUndoData(rootWidget));
...
void PanelEditor::HandleGroupSelectionMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	AddUndoData(new GroupPanelUndoData(group, rootWidget));
	...
	while (manipulator)
	{
		const Point3D& position = manipulator->GetTargetWidget()->GetWidgetPosition();
	...
	rootWidget->AddNewSubnode(group);
	group->Update();
...
void PanelEditor::HandleUngroupSelectionMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	AddUndoData(new UngroupPanelUndoData(&selectionList));
	...
	for (;;)
	{
		...
		while (widget)
		{
			...
			root->AddSubnodeBefore(widget, group);
...
{
	AddUndoData(new TransformPanelUndoData(&selectionList));
	...
	while (manipulator)
	{
		...
		widget->SetWidgetMatrix3D(K::identity_3D);
...
{
	AddUndoData(new WarpPanelUndoData(&selectionList));
	...
	while (manipulator)
	{
		...
		if (widget->GetWidgetType() == kWidgetImage)
		{
			...
			image->SetImageOffset(Vector2D(0.0F, 0.0F));
...
void PanelEditor::HandleEditScriptMenuItem(Widget *menuItem, const WidgetEventData *eventData)
{
	...
	if (!object)
	{
		...
		widget->SetScriptObject(object);
		object->Release();
	}
	
	ScriptEditor *scriptEditor = new ScriptEditor(targetNode, object);
...
{
	AddUndoData(new ScriptPanelUndoData(&selectionList));
...
void PanelEditor::ScriptEditorComplete(ScriptEditor *scriptEditor, void *cookie)
{
	...
	ScriptObject *object = scriptEditor->GetScriptObject();
	if ((!object->GetScriptGraph()->GetFirstElement()) && (!object->GetFirstValue()))
	{
		Widget *widget = panelEditor->selectionList.First()->GetTargetWidget();
		widget->SetScriptObject(nullptr);
	}
	
	panelEditor->editorState |= kPanelEditorModified;
}
	...
	while (widget)
	{
		if (widget->GetWidgetType() != kWidgetGeneric)
		{
			...
			if ((manipulator) && ((manipulator->GetManipulatorState() & (kWidgetManipulatorHidden | kWidgetManipulatorLocked)) == 0))
			{
				...
				{
					Widget *super = widget->GetSuperNode();
					if (super == rootWidget) return (widget);
					
					manipulator = GetManipulator(super);
					if ((manipulator) && ((manipulator->GetManipulatorState() & (kWidgetManipulatorHidden | kWidgetManipulatorLocked)) == 0))
					{
						if (InterfaceMgr::GetCommandKey()) return (widget);
						return (super);
					}
				}
			}
...
void PanelEditor::MoveSelectedWidgets(const Point3D& p)
{
	...
	if (InterfaceMgr::GetShiftKey())
	{
		...
		if (Fabs(dy) > Fabs(dx)) position.x = anchorPosition.x;
		else position.y = anchorPosition.y;
	}
...
void PanelEditor::RotateSelectedWidgets(const Point3D& p)
{
	...
	if (InterfaceMgr::GetShiftKey()) angle = Floor(angle / K::pi_over_4 + 0.5F) * K::pi_over_4;
		...
		while (manipulator)
		{
			...
			if (widget->GetWidgetType() != kWidgetGeneric)
			{
				...
				if ((handleIndex == 0) || (handleIndex == 1) || (handleIndex == 4))
				{
					...
					if (editorFlags & kPanelEditorSnapToGrid) newPosition.x = SnapToGrid(newPosition.x);
					newSize.x = oldPosition.x - newPosition.x + oldSize.x;
				}
				else if ((handleIndex == 2) || (handleIndex == 3) || (handleIndex == 6))
				{
					newSize.x += delta.x;
					if (editorFlags & kPanelEditorSnapToGrid) newSize.x = SnapToGrid(newPosition.x + newSize.x) - newPosition.x;
				}
				
				if ((handleIndex == 0) || (handleIndex == 3) || (handleIndex == 7))
				{
					newPosition.y += delta.y;
					if (editorFlags & kPanelEditorSnapToGrid) newPosition.y = SnapToGrid(newPosition.y);
					newSize.y = oldPosition.y - newPosition.y + oldSize.y;
				}
				else if ((handleIndex == 1) || (handleIndex == 2) || (handleIndex == 5))
				{
					newSize.y += delta.y;
					if (editorFlags & kPanelEditorSnapToGrid) newSize.y = SnapToGrid(newPosition.y + newSize.y) - newPosition.y;
				}
				...
				if (InterfaceMgr::GetShiftKey())
				{
					...
					if ((handleIndex == 0) || (handleIndex == 3) || (handleIndex == 7)) newPosition.y = oldPosition.y + oldSize.y - newSize.y;
				}
	...
	if (position != previousPosition)
	{
		...
		while (manipulator)
		{
			...
			if (widget->GetWidgetType() == kWidgetImage)
			{
				...
				if (InterfaceMgr::GetShiftKey())
				{
					if (Fabs(delta.y) > Fabs(delta.x)) delta.x = 0.0F;
					else delta.y = 0.0F;
				}
	...
	if (position != previousPosition)
	{
		...
		while (manipulator)
		{
			...
			else if (type == kWidgetText)
			{
				...
				if (editorFlags & kPanelEditorSnapToGrid) s = SnapToGrid(s * 64.0F) * 0.015625F;
...
bool PanelEditor::BoxIntersectsWidget(const Point3D& p1, const Point3D& p2, const Widget *widget)
{
	...
	int32 vertexCount = Math::ClipPolygonAgainstPlane(4, vertex[0], Antivector4D(sx, 0.0F, 0.0F, -sx * p1.x), location, vertex[1]);
	if (vertexCount == 0) return (false);
	
	vertexCount = Math::ClipPolygonAgainstPlane(vertexCount, vertex[1], Antivector4D(-sx, 0.0F, 0.0F, sx * p2.x), location, vertex[0]);
	if (vertexCount == 0) return (false);
	
	vertexCount = Math::ClipPolygonAgainstPlane(vertexCount, vertex[0], Antivector4D(0.0F, sy, 0.0F, -sy * p1.y), location, vertex[1]);
	if (vertexCount == 0) return (false);
	
	vertexCount = Math::ClipPolygonAgainstPlane(vertexCount, vertex[1], Antivector4D(0.0F, -sy, 0.0F, sy * p2.y), location, vertex[0]);
...
void PanelEditor::BeginTool(const Point3D& p)
{
	...
	if ((tool >= kPanelToolWidgetSelect) && (tool <= kPanelToolTextureScale))
	{
		...
		if ((tool == kPanelToolWidgetRotate) || (tool == kPanelToolWidgetResize)) handleWidget = PickWidgetHandle(anchorPosition, &handleIndex);
		...
		if (!widget)
		{
			...
			if (widget)
			{
				...
				{
					if (shift) UnselectWidget(widget);
				}
				else
				{
		...
		else
		{
			switch (tool)
			{
				...
				case kPanelToolTextureOffset:
				{
					...
					while (manipulator)
					{
						if (manipulator->GetTargetWidget()->GetWidgetType() == kWidgetImage) manipulator->SaveOriginalOffset();
						manipulator = manipulator->Next();
					}
				...
				case kPanelToolTextureScale:
				{
					...
					while (manipulator)
					{
						...
						if (type == kWidgetImage) manipulator->SaveOriginalImageScale();
						else if (type == kWidgetText) manipulator->SaveOriginalTextScale();
...
void PanelEditor::BeginWidget(const Point3D& p)
{
	...
	switch (type)
	{
		...
		case kWidgetEditText:
			...
			creationWidget = new EditTextWidget(Vector2D(0.0F, 0.0F), 255, "font/Gui");
			break;
		
		case kWidgetPassword:
			
			creationWidget = new PasswordWidget(Vector2D(0.0F, 0.0F), PasswordWidget::kMaxPasswordLength, "font/Gui");
			break;
		
		case kWidgetQuad:
			
			creationWidget = new QuadWidget(Vector2D(0.0F, 0.0F), K::white);
			break;
		
		case kWidgetImage:
			
			creationWidget = new ImageWidget(Vector2D(0.0F, 0.0F), "C4/missing");
			break;
		
		case kWidgetCheck:
			
			creationWidget = new CheckWidget(Vector2D(0.0F, 0.0F), currentWidgetReg->GetWidgetName(), "font/Gui");
			break;
		
		case kWidgetRadio:
			
			creationWidget = new RadioWidget(Vector2D(0.0F, 0.0F), currentWidgetReg->GetWidgetName(), "font/Gui");
			break;
		
		case kWidgetPushButton:
			
			creationWidget = new PushButtonWidget(Vector2D(0.0F, 0.0F), currentWidgetReg->GetWidgetName(), "font/Heading");
			break;
		
		case kWidgetIconButton:
			
			creationWidget = new IconButtonWidget(Vector2D(0.0F, 0.0F), "C4/missing");
			break;
		
		case kWidgetTextButton:
			
			creationWidget = new TextButtonWidget(Vector2D(0.0F, 0.0F), currentWidgetReg->GetWidgetName(), "font/Gui");
			break;
		...
		case kWidgetList:
			...
			creationWidget = new ListWidget(Vector2D(0.0F, 0.0F));
			break;
		
		case kWidgetMultipane:
			
			creationWidget = new MultipaneWidget(Vector2D(0.0F, 0.0F), "font/Gui");
			break;
		
		case kWidgetPopupMenu:
			
			creationWidget = new PopupMenuWidget(Vector2D(0.0F, 0.0F), "font/Gui");
			break;
		
		case kWidgetMenuBar:
			
			creationWidget = new MenuBarWidget(Vector2D(0.0F, 0.0F), "font/Gui");
			break;
		
		case kWidgetConfiguration:
			
			creationWidget = new ConfigurationWidget(Vector2D(0.0F, 0.0F), 0.5F);
			break;
		
		case kWidgetCamera:
			
			creationWidget = new CameraWidget(Vector2D(0.0F, 0.0F));
			break;
	...
	AddUndoData(new CreatePanelUndoData(creationWidget));
	...
	rootWidget->AddNewSubnode(creationWidget);
...
void PanelEditor::EndWidget(const Point3D& p)
{
	if (TrackWidget(p))
	{
		if (creationWidget->GetBaseWidgetType() == kWidgetText) SetFocusWidget(textStringBox);
	}
	else
	{
		UnselectAll();
		delete creationWidget;
		RemoveUndoData(undoList.Last());
	}
}
...
void PanelEditor::ViewportHandleMouseEvent(const MouseEventData *eventData, ViewportWidget *viewport, void *cookie)
{
	...
	if ((eventType == kEventMouseDown) || (eventType == kEventMiddleMouseDown))
	{
		...
		if (mode == kPanelEditorModeTool) panelEditor->BeginTool(eventData->mousePosition);
		else panelEditor->BeginWidget(eventData->mousePosition);
	}
	else if ((eventType == kEventMouseUp) || (eventType == kEventMiddleMouseUp))
	{
		if (panelEditor->toolTracking)
		{
			if (panelEditor->trackingMode == kPanelEditorModeTool) panelEditor->EndTool(eventData->mousePosition);
			else panelEditor->EndWidget(eventData->mousePosition);
	...
	else if (eventType == kEventMouseWheel)
	{
		if (!panelEditor->toolTracking)
		{
			...
			panelEditor->UpdateViewportScale(orthoViewport->GetOrthoScale().x * Exp(eventData->mousePosition.y * -0.16F));
...
void PanelEditor::ViewportTrackTask(const Point3D& position, ViewportWidget *viewport, void *cookie)
{
	...
	if (panelEditor->toolTracking)
	{
		if (panelEditor->trackingMode == kPanelEditorModeTool) panelEditor->TrackTool(position);
		else panelEditor->TrackWidget(position);
	}
}
...
void PanelEditor::ViewportRender(List<Renderable> *renderList, ViewportWidget *viewport, void *cookie)
{
	...
	if (panelEditor->boxSelectFlag) renderList->Append(&panelEditor->dragRect);
}
	...
	if (eventData->eventType == kEventWidgetChange)
	{
		...
		for (machine a = 0; a < kPanelToolCount; a++)
		{
			if (widget == toolButton[a])
			{
				...
				{
					if (currentMode == kPanelEditorModeTool) toolButton[currentTool]->SetValue(0);
					else widgetsPage->UnselectWidgetTool();
...
void PanelEditor::SavePickerProc(FilePicker *picker, void *cookie)
{
	...
	PanelEditor *editor = static_cast<PanelEditor *>(cookie);
	editor->resourceName = &name[len];
	editor->resourceLocation.GetPath().Set(name, len - 1);
	editor->SavePanel();
	
	ResourcePath title(editor->resourceName);
	editor->SetWindowTitle(title += PanelResource::GetDescriptor()->GetExtension());
...
void PanelEditor::CloseDialogComplete(Dialog *dialog, void *cookie)
{
	...
	{
		if ((editor->targetPanel) || (editor->resourceName[0] != 0))
		{
			editor->HandleSavePanelMenuItem(nullptr, nullptr);
			editor->Window::Close();
		}
		else
		{
			editor->HandleSavePanelAsMenuItem(nullptr, nullptr);
		}
	}
...
void PanelEditor::Move(void)
{
	if ((TheInterfaceMgr->GetActiveWindow() == this) && (!TheInterfaceMgr->GetActiveMenu()))
	{
		...
		if (!toolCursor)
		{
			...
			if ((vp.x >= 0.0F) && (vp.x < panelViewport->GetWidgetSize().x) && (vp.y >= 0.0F) && (vp.y < panelViewport->GetWidgetSize().y)) toolCursor = true;
		}
		...
		if (toolCursor)
		{
			...
			{
				if (tool == kPanelToolViewportScroll) cursorIndex = (toolTracking) ? kEditorCursorDrag : kEditorCursorHand;
				else if (tool == kPanelToolViewportZoom) cursorIndex = kEditorCursorGlass;
			}
			else
			{
	...
	if (editorState & kPanelEditorUpdateMenus)
	{
		...
		else
		{
			...
			while (manipulator)
			{
				...
				{
					ungroupEnable = false;
					if (widget->GetSuperNode() != rootWidget) groupEnable = false;
					
					if ((type == kWidgetImage) || (type == kWidgetIconButton)) imageEnable = true;
				}
			...
			for (machine a = 0; a < kPanelMenuItemCount; a++)
			{
				if (widgetMenuItemSelectFlag[a])
				{
					...
					if ((imageEnable) || (!widgetMenuItemImageFlag[a])) menuItem->Enable();
					else menuItem->Disable();
				}
			}
			
			if (groupEnable) widgetMenuItem[kPanelMenuGroupSelection]->Enable();
			else widgetMenuItem[kPanelMenuGroupSelection]->Disable();
			
			if (ungroupEnable) widgetMenuItem[kPanelMenuUngroupSelection]->Enable();
			else widgetMenuItem[kPanelMenuUngroupSelection]->Disable();
		}
	}
	...
	if (editorState & kPanelEditorUpdateSettings)
	{
		...
		else
		{
			...
			widgetMenuItem[kPanelMenuEditScript]->Disable();
			if (!manipulator) widgetConfigurationWidget->ReleaseConfiguration();
			mutatorConfigurationWidget->ReleaseConfiguration();
		}
	}
	...
	{
		const EditorWidgetManipulator *manipulator = selectionList.First();
		if ((manipulator) && (!manipulator->Next())) transformPage->UpdateTransform(manipulator->GetTargetWidget());
		else transformPage->UpdateTransform(nullptr);
	}
...
void PanelEditor::EnterForeground(void)
{
	...
	if (editorClipboard.GetFirstSubnode()) widgetMenuItem[kPanelMenuPaste]->Enable();
}
...
bool PanelEditor::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	...
	if (eventType == kEventKeyDown)
	{
		...
		if (!GetFocusWidget())
		{
			if ((code >= '1') && (code <= '7'))
			{
				if (code == '1') toolButton[kPanelToolWidgetSelect]->SetValue(1, true);
				else if (code == '2') toolButton[kPanelToolWidgetMove]->SetValue(1, true);
				else if (code == '3') toolButton[kPanelToolWidgetRotate]->SetValue(1, true);
				else if (code == '4') toolButton[kPanelToolWidgetResize]->SetValue(1, true);
				else if (code == '6') toolButton[kPanelToolViewportScroll]->SetValue(1, true);
				else if (code == '7') toolButton[kPanelToolViewportZoom]->SetValue(1, true);
				return (true);
			}
			else if ((code == kKeyCodeDelete) || (code == kKeyCodeBackspace))
			{
				if (!selectionList.Empty()) HandleClearMenuItem(nullptr, nullptr);
				return (true);
			}
		}
	}
...
void PanelEditor::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		if (widget == addMutatorButton)
		{
			...
			Widget *widget = selectionList.First()->GetTargetWidget();
			widget->AddMutator(mutator);
			
			rootWidget->RemoveMovingWidget(widget);
			widget->Preprocess();
			
			mutatorListBox->AppendListItem(registration->GetMutatorName());
			mutatorListBox->SelectListItem(mutatorListBox->GetListItemCount() - 1);
		...
		else if (widget == removeMutatorButton)
		{
			...
			Widget *listItem = mutatorListBox->GetFirstSelectedListItem();
			mutatorListBox->RemoveListItem(listItem);
			delete listItem;
	...
	{
		if (widget == mutatorListBox)
		{
			mutatorConfigurationWidget->ReleaseConfiguration();
			
			const Widget *widget = selectionList.First()->GetTargetWidget();
			int32 selection = mutatorListBox->GetFirstSelectedIndex();
			if (selection >= 0)
			{
				Mutator *mutator = widget->GetFirstMutator();
				while (selection != 0)
...
{
	if (editorState & kPanelEditorModified)
	{
		const StringTable *table = TheWorldEditor->GetStringTable();
		...
		if (resourceName[0] != 0) confirmText += resourceName;
		else confirmText += table->GetString(StringID('PANL', 'PANL'));
		confirmText += '?';
