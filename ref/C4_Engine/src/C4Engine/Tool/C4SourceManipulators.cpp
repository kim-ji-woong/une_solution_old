...
Manipulator *SourceManipulator::Construct(Source *source)
{
...
void SourceManipulator::Select(void)
{
	...
	if (volumeManipulator) volumeManipulator->Select();
}

void SourceManipulator::Unselect(void)
{
	EditorManipulator::Unselect();
	if (volumeManipulator) volumeManipulator->Unselect();
}
...
{ 
	if (!(GetManipulatorState() & kManipulatorUpdated)) SetSphereSize(GetObject()->GetSourceRange());
