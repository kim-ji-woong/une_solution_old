...
void MaterialWidget::HandleMouseEvent(const PanelMouseEventData *eventData)
{
	...
	{
		if (eventData->eventFlags & kMouseDoubleClick) PostWidgetEvent(WidgetEventData(kEventWidgetActivate));
	}
}
...
void MaterialWindow::Preprocess(void) 
{
	...
	for (machine a = 0; a < kMaterialPaneCount; a++) configurationWidget[a] = static_cast<ConfigurationWidget *>(FindWidget(paneIdentifier[a]));
...
void MaterialWindow::HandleConfigurationEvent(SettingInterface *settingInterface)
{
	...
	CommitMaterialSettings(container);
...
void MaterialWindow::SelectMaterial(MaterialWidget *materialWidget)
{
	...
	if (container->GetUsageCount() != 0) deleteButton->Disable();
	else deleteButton->Enable();
}
...
void MaterialWindow::UpdatePaneVisibility(void)
{
	...
	{
		if (a != selection) configurationWidget[a]->Hide();
		else configurationWidget[a]->Show();
	}
	
	Widget *widget = GetFocusWidget();
	if ((widget) && (widget->GetGlobalWidgetState() & kWidgetHidden)) SetFocusWidget(nullptr);
}
...
void MaterialWindow::UpdateMaterialConfiguration(const MaterialContainer *container)
{
	...
	else
	{
		...
		if (multipaneWidget->GetSelection() == kWidgetValueNone) multipaneWidget->SetSelection(3);
	}
...
{
	if ((!currentMaterial) && (selectedMaterial)) currentMaterial = selectedMaterial->GetMaterialContainer();
...
void MaterialWindow::HandleMaterialTableEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	else if (eventType == kEventWidgetChange)
	{
		...
		if (selection)
		{
			...
			if (materialWidget != selectedMaterial) SelectMaterial(materialWidget);
		}
	}
}
...
void MaterialWindow::ExportMaterial(FilePicker *picker, void *cookie)
{
	...
	if (object)
	{
		...
		{
			int32 header[3] = {1, kWorldVersion, 0};
...
void MaterialWindow::ShaderEditorComplete(ShaderEditor *shaderEditor, void *cookie)
{
	...
	while (attribute)
	{
		...
		if (attribute->GetAttributeType() != kAttributeShader) delete attribute;
		attribute = next;
	}
	
	window->UpdateMaterialConfiguration(container);
	window->selectedMaterial->UpdatePreview();
	window->previewWidget->UpdatePreview();
	
	window->worldEditor->PostEvent(MaterialEditorEvent(kEditorEventMaterialModified, materialObject));
}
...
bool MaterialWindow::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	...
	if (eventType == kEventKeyDown)
	{
		...
		if ((code == kKeyCodeReturn) || (code == kKeyCodeEscape))
		{
			Close();
			return (true);
		}
	...
	if (eventType == kEventWidgetActivate)
	{
		...
		else if (widget == deleteButton)
		{
			...
			if (materialWidget)
			{
				...
				if (!newSelection) newSelection = static_cast<MaterialWidget *>(materialWidget->Previous());
				delete materialWidget;
				
				if (newSelection) UpdateMaterialList(newSelection->GetMaterialContainer());
				else newButton->Activate();
			}
		}
		...
		else if (widget == cleanupButton)
		{
			...
			if (selectedMaterial) editorObject->SelectMaterial(selectedMaterial->GetMaterialContainer());
		...
		else if (widget == shaderButton)
		{
			...
			ShaderEditor *shaderEditor = new ShaderEditor(materialObject, primitiveType[previewMenuWidget->GetSelection()]);
...
void MaterialWindow::Close(void)
{
	...
	if (materialWidget) worldEditor->SelectMaterial(materialWidget->GetMaterialContainer());
