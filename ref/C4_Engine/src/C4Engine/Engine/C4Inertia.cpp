...
Vector3D C4::operator *(const InertiaTensor& m, const Vector3D& v)
{
	return (Vector3D(m[0] * v.x + m[1] * v.y + m[2] * v.z, m[1] * v.x + m[3] * v.y + m[4] * v.z, m[2] * v.x + m[4] * v.y + m[5] * v.z));
}
...
void C4::Reverse(InertiaTensor *m)
{ 
	for (machine a = 0; a < 6; a++) Reverse(&(*m)[a]);
}
