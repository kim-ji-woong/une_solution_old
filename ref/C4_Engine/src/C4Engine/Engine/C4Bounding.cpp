...
void Box3D::Transform(const Transform4D& transform)
{
	...
	p[6] = transform * max;
		...
		float4 pmin = TransformPoint3D(transform, SimdLoadUnaligned(&min.x));
		float4 pmax = pmin;
		
		for (machine a = 0; a < 7; a++)
		{
			float4 v = SimdLoadUnaligned(&p[a].x);
			pmin = SimdMin(pmin, v);
			pmax = SimdMax(pmax, v);
		}
		
		SimdStore3D(pmin, &min.x);
		SimdStore3D(pmax, &max.x);
...
void Box3D::Calculate(int32 vertexCount, const Point3D *vertex)
{
		...
		float4 pmin = SimdLoadUnaligned(&vertex[0].x);
		float4 pmax = pmin;
		
		for (machine a = 1; a < vertexCount; a++)
		{
			float4 p = SimdLoadUnaligned(&vertex[a].x);
			pmin = SimdMin(pmin, p);
			pmax = SimdMax(pmax, p);
		}
...
{
	if (center.x < min.x - radius) return (true);
	if (center.x > max.x + radius) return (true);
	if (center.y < min.y - radius) return (true);
	if (center.y > max.y + radius) return (true);
	if (center.z < min.z - radius) return (true);
	if (center.z > max.z + radius) return (true);
...
{
	if ((!ExteriorSphere(p1, radius)) || (!ExteriorSphere(p2, radius))) return (false);
...
bool Box3D::Intersection(const Box3D& box) const
{
		...
		if (SimdCmpltAny3D(SimdLoadUnaligned(&max.x), SimdLoadUnaligned(&box.min.x))) return (false);
		if (SimdCmpgtAny3D(SimdLoadUnaligned(&min.x), SimdLoadUnaligned(&box.max.x))) return (false);
...
Box3D C4::Union(const Box3D& box1, const Box3D& box2)
{
		...
		float4 pmin = SimdMin(SimdLoadUnaligned(&box1.min.x), SimdLoadUnaligned(&box2.min.x));
		float4 pmax = SimdMax(SimdLoadUnaligned(&box1.max.x), SimdLoadUnaligned(&box2.max.x));
...
Box3D C4::Transform(const Box3D& box, const Transform4D& transform)
{
	...
	p[6] = transform * box.max;
		...
		float4 pmin = TransformPoint3D(transform, SimdLoadUnaligned(&box.min.x));
		float4 pmax = pmin;
...
bool BoundingSphere::IntersectsSweptSphere(const Point3D& p1, const Point3D& p2, float radius) const
{
	...
	if (e1 * dp < 0.0F) return (e1 * e1 < r * r);
	
	Vector3D e2 = sphereCenter - p2;
	if (e2 * dp > 0.0F) return (e2 * e2 < r * r);
