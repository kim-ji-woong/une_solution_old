...
namespace C4
{
	...
	template <> Heap Memory<MessageMgr>::heap("MessageMgr", 8192, kHeapMutexless);
	template class Memory<MessageMgr>;
}
...
bool ChatMessage::HandleMessage(Player *sender) const
{
	...
	if (player) TheApplication->HandlePlayerEvent(kPlayerChatReceived, player, chatText);
...
FileReceiveMessage::FileReceiveMessage(const char *name, unsigned_int32 size) : Message(kMessageFileReceive)
{
	fileSize = size;
	fileName = name;
}
...
bool FileChunkMessage::Decompress(Decompressor& data)
{
	...
	data >> size;
	if (size > kMaxFileChunkSize) return (false);
...
bool ControllerMessage::HandleMessage(Player *sender) const
{
	...
	if (receiver) (*receiver)(this, TheMessageMgr->GetControllerMessageCookie());
	return (true);
}

bool ControllerMessage::HandleControllerMessage(Controller *controller) const
{
	return (false);
}
...
bool CombinedMessage::AddMessage(MessageType type, unsigned_int32 size, const char *data)
{
	if (messageCount < kMaxCombinedMessageCount)
	{
		...
		if (newSize <= kMaxCombinedMessageSize)
		{
			messageCount++;
			messageSize = newSize;
			if (type == kMessageFileChunk) packetFlags |= kPacketFlagFile;
...
NetworkResult CombinedMessage::Send(const NetworkAddress& address)
{
	...
	if (!(combinedFlags & kMessageUnreliable)) return (TheNetworkMgr->SendReliablePacket(address, messageSize, messageBuffer, packetFlags));
	if (combinedFlags & kMessageUnordered) return (TheNetworkMgr->SendUnorderedPacket(address, messageSize, messageBuffer));
...
Player::~Player()
{
	...
	if (chatSound) chatSound->Release();
}
...
void Player::AddMessage(const Message& message, unsigned_int32 size, const char *data)
{
	...
	if (!(flags & kMessageCombineInhibit))
	{
		...
		if ((combinedMessage) && (combinedMessage->GetCombinedFlags() == combinedFlags) && (combinedMessage->AddMessage(type, size, data))) return;
	}
...
void Player::StartSendingFile(const char *name)
{
	if (!sendFile.Open())
	{
		...
		{
			sendFileSize = (unsigned_int32) sendFile.GetSize();
...
void Player::StartReceivingFile(const char *name, unsigned_int32 size)
{
	if (!receiveFile.Open())
	{
...
void Player::SendFileChunk(void)
{
	...
	{
		unsigned_int32 size = Min(sendFileSize - sendFile.GetPosition(), fileChunkSize);
...
ChatStreamer *Player::ConstructChatStreamer(void)
{
	chatSound = new Sound;
...
void Player::SetChatSoundGroup(SoundGroup *group)
{
	...
	if (chatSound) chatSound->SetSoundGroup(chatSoundGroup);
}

void Player::SetChatVolume(float volume)
{
	chatVolume = volume;
	if (chatSound) chatSound->SetSoundProperty(kSoundVolume, volume);
}
...
EngineResult MessageMgr::Construct(void)
{
	...
	playerConstructorProc = nullptr;
	controllerMessageConstructor = nullptr;
...
NetworkAddress MessageMgr::StringToAddress(const char *text)
{
	...
	for (;;)
	{
		...
		{
			if (b) part = part * 10 + c - 48;
			else port = port * 10 + c - 48;
		}
	}
	
	return (NetworkAddress(addr, (unsigned_int16) port));
}
...
Player *MessageMgr::NewPlayer(PlayerKey key)
{
	...
	Player *player = (playerConstructorProc) ? (*playerConstructorProc)(key, playerConstructorCookie) : new Player(key);
...
Player *MessageMgr::GetPlayer(const NetworkAddress& address) const
{
	if (address.GetAddress() != 0)
	{
		...
		while (player)
		{
			if (player->GetPlayerAddress() == address) return (player);
			player = player->Next();
		}
	}
	else
	{
		if (address == kAddressSelf) return (localPlayer);
		if (address == kAddressServer) return (GetPlayer(kPlayerServer));
	}
...
Player *MessageMgr::GetFirstClientPlayer(void) const
{
	...
	while (player)
	{
		if (player->GetPlayerKey() > kPlayerNone) return (player);
		player = player->Next();
	}
...
{
	if (snapshotSenderList.Member(sender)) snapshotSenderList.Remove(sender);
}
...
Message *MessageMgr::ConstructMessage(MessageType type, Decompressor& data)
{
	switch (type)
	{
		case kMessagePing:
		...
		case kMessageController:
			...
			if (controllerMessageConstructor)
			{
				...
				return ((*controllerMessageConstructor)(controllerMessageType, controllerIndex, data, controllerMessageCookie));
			}
		...
		default:
			...
			return (TheApplication->ConstructMessage(type, data));
	}
...
void MessageMgr::ReceiveMessage(const NetworkAddress& address, const Message *message)
{
	...
	if ((sender) && (message->HandleMessage(sender))) return;
	...
	switch (type)
	{
		...
		case kMessageServerQuery:
			...
			if ((serverFlag) && (multiplayerFlag))
			{
				...
				if (m->GetProtocol() == TheNetworkMgr->GetProtocol()) TheApplication->HandleConnectionEvent(kConnectionQueryReceived, address, nullptr);
			}
		...
		case kMessageConnect:
		{
			...
			if (!player) player = NewPlayer(key);
			...
			if ((sender) && (serverFlag))
			{
				...
				while (player)
				{
					if (player != sender)
					{
						...
						if (key != kPlayerServer) player->SendMessage(newPlayerInfo);
					}
...
void MessageMgr::SendMessage(PlayerKey key, const Message& message)
{
	...
	{
		if (serverFlag) ReceiveMessage(kAddressServer, &message);
	}
...
void MessageMgr::SendMessageAll(const Message& message, bool self)
{
	...
	if (self) ReceiveMessage(kAddressSelf, &message);
}
...
void MessageMgr::SendMessageClients(const Message& message, const Player *exclude)
{
	if (serverFlag)
	{
		...
		if (player)
		{
			...
			do
			{
				if (player != exclude) player->AddMessage(message, size, data);
				player = player->Next();
			} while (player);
		}
	}
...
bool MessageMgr::SendChatMessage(const char *text)
{
	if (chatSendTime <= 0)
	{
		...
		if (!serverFlag) ReceiveMessage(kAddressSelf, &message);
...
NetworkResult MessageMgr::Connect(const NetworkAddress& address)
{
	if (!multiplayerFlag)
	{
		...
		if (result != kNetworkOkay) return (result);
	}
	...
	while (TheNetworkMgr->ReceivePacket(&address, &size, data) == kNetworkOkay)
	{
		...
		if (messageCount <= kMaxCombinedMessageCount)
		{
			...
			for (machine a = 0; a < messageCount; a++)
			{
				...
				Message *message = ConstructMessage(type, decompressor);
				if (!message) break;
...
void MessageMgr::SendTask(void)
{
	...
	if (chatSendTime > 0) chatSendTime -= dt;
