...
PaintState::PaintState(const PaintState& paintState)
{
	...
	for (machine a = 0; a < 4; a++) channelMask[a] = paintState.channelMask[a];
}
...
void Painter::EndPainting(void)
{
	if (!paintBounds.Empty())
	{
		...
		{
			for (machine i = 0; i < width; i++) transfer[i] = 0;
			transfer += row;
		}
	}
}
...
{
	const ColorRGBA& color = paintState->GetBrushColor();
	unsigned_int32 brushLum = Max(Max((unsigned_int32) (color.red * 255.0F), (unsigned_int32) (color.green * 255.0F)), (unsigned_int32) (color.blue * 255.0F));
	...
	for (machine j = 0; j < height; j++)
	{
		for (machine i = 0; i < width; i++)
		{
			...
			if (a != 0)
			{
				...
				paint[i] = (brushLum * a + previous[i] * b) >> 8;
			}
		}
...
void Painter::UpdateImage2(void)
{
	const ColorRGBA& color = paintState->GetBrushColor();
	unsigned_int32 brushLum = Max(Max((unsigned_int32) (color.red * 255.0F), (unsigned_int32) (color.green * 255.0F)), (unsigned_int32) (color.blue * 255.0F));
	unsigned_int32 brushAlpha = (unsigned_int32) (color.alpha * 255.0F);
...
{
	const ColorRGBA& color = paintState->GetBrushColor();
	unsigned_int32 brushRed = (unsigned_int32) (color.red * 255.0F);
	unsigned_int32 brushGreen = (unsigned_int32) (color.green * 255.0F);
	unsigned_int32 brushBlue = (unsigned_int32) (color.blue * 255.0F);
	unsigned_int32 brushAlpha = (unsigned_int32) (color.alpha * 255.0F);
...
bool Painter::UpdateImage(void)
{
	if (!paintBounds.Empty())
	{
		...
		if (count == 4) UpdateImage4();
		else if (count == 2) UpdateImage2();
		else UpdateImage1();
...
const void *Painter::CreateUndoImage(const Rect& rect) const
{
	...
	for (machine j = 0; j < height; j++)
	{
		for (machine i = 0; i < width; i++) delta[i] = paint[i] ^ previous[i];
...
void Painter::ApplyUndoImage(const PaintSpaceObject *object, const Rect& rect, const void *undoImage)
{
	...
	for (machine j = 0; j < height; j++)
	{
		for (machine i = 0; i < width; i++) paint[i] ^= delta[i];
		
		paint += rowSkip;
		delta += width;
	}
	
	if (compressedSize != 0) delete[] deltaImage;
}
...
void Painter::DrawLine(const Point2D& p1, const Point2D& p2)
{
	...
	if (ax < ay)
	{
		if (ay > 1.0F)
		{
			...
			for (machine j = top; j < bottom; j++)
			{
				int32 left = MaxZero((int32) Floor(x) - size);
				int32 right = Min((int32) Ceil(x) + size + 2, paintResolution.x);
				float k = (float) j * dy;
	...
	else if (ax > 1.0F)
	{
		...
		for (machine i = left; i < right; i++)
		{
			int32 top = MaxZero((int32) Floor(y) - size);
			int32 bottom = Min((int32) Ceil(y) + size + 2, paintResolution.y);
			float k = (float) i * dx;
...
void PaintWidget::UpdateImage(void)
{
	if (painter->UpdateImage())
	{
		...
		texture->Update(painter->GetPaintBounds());
	}
}
...
void PaintWidget::HandleMouseEvent(const PanelMouseEventData *eventData)
{
	...
	{
		if (!painter) painter = new Painter(paintResolution, 4, paintImage, &paintState);
