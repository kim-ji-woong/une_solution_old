...
namespace C4
{
	...
	template <> Heap Memory<Bond>::heap("Bond", MemoryMgr::CalculatePoolSize(64, sizeof(Bond)), kHeapMutexless);
	template class Memory<Bond>;
	
	template <> Heap Memory<Cell>::heap("Cell", MemoryMgr::CalculatePoolSize(256, sizeof(Cell)), kHeapMutexless);
...
C4::Cell::~Cell()
{
	...
	if (bond) static_cast<Cell *>(bond->GetStartElement())->subcell[GetCellIndex()] = nullptr;
}
...
C4::Cell *CellGraph::UpdateCell(Cell *superCell, int32 i, int32 j, float cellSize)
{
	...
	if (!cell)
	{
		...
		Point3D p(rootBoxMin.x + (float) i * cellSize, rootBoxMin.y + (float) j * cellSize, rootBoxMin.z);
		cell->SetWorldBoundingBox(p, Point3D(p.x + cellSize, p.y + cellSize, rootCell.GetWorldBoundingBox().max.z));
	}
	
	return (cell);
}

void CellGraph::AddSite(Site *site)
{
	if (rootCell.GetFirstIncomingEdge())
	{
		...
		if (siteSize < levelCellSize * kCellFraction)
		{
			...
			int32 x1 = MaxZero((int32) (offset.x * inverseCellSize));
			int32 y1 = MaxZero((int32) (offset.y * inverseCellSize)); 
			int32 x2 = Min((int32) ((offset.x + size.x) * inverseCellSize), maxCellCoord.x);
			int32 y2 = Min((int32) ((offset.y + size.y) * inverseCellSize), maxCellCoord.y);
			...
			for (machine level = levelCount - 1; level >= 0; level--)
			{
				...
				if (i2 == i1) subcell[1] = cell;
				else subcell[1] = UpdateCell(superCell[1], i2, j1, levelCellSize);
				...
				else
				{
					...
					if (i2 == i1) subcell[3] = cell;
					else subcell[3] = UpdateCell(superCell[3], i2, j2, levelCellSize);
				}
				
				levelCellSize *= 0.5F;
				if (siteSize > levelCellSize * kCellFraction) break;
			...
			if (i2 != i1) new Bond(subcell[1], site);
			
			if (j2 != j1)
			{
				new Bond(subcell[2], site);
				if (i2 != i1) new Bond(subcell[3], site);
			}
...
void CellGraph::RemoveSite(Site *site)
{
	for (;;)
	{
		...
		if (predecessor != superSite)
		{
			...
			{
				if (predecessor->GetFirstOutgoingEdge()) break;
