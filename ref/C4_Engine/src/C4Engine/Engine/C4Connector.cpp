...
namespace C4
{
	template <> Heap Memory<Connector>::heap("Connector", 16384, kHeapMutexless);
	template class Memory<Connector>;
}
...
Node *Connector::GetConnectorTarget(void) const
{
	...
	if (finish != GetStartElement()) return (finish->GetNode());
	return (nullptr);
}
...
void Connector::SetConnectorTarget(Node *node)
{
	...
	else
	{
		...
		if (finish->Isolated()) delete finish;
	}
}
...
Hub::~Hub()
{
	...
	while (connector)
	{
		...
		if (start != this) connector->SetFinishElement(start);
...
bool Hub::HasOutgoingConnection(void) const
{
	...
	while (connector)
	{
		if (connector->GetFinishElement() != this) return (true);
...
bool Hub::HasIncomingConnection(void) const
{
	...
	while (connector) 
	{
		if (connector->GetStartElement() != this) return (true); 
...
{ 
	...
	while (connector)
	{
		...
		if (connector->GetFinishElement() != this) connector->SetConnectorTarget(nullptr);
		connector = next;
	}
}
...
void Hub::BreakAllIncomingConnections(void)
{
	...
	{
		Connector *next = connector->GetNextIncomingEdge();
		if (connector->GetStartElement() != this) connector->SetConnectorTarget(nullptr);
		connector = next;
	}
...
Connector *Hub::FindOutgoingConnector(const char *key) const
{
	...
	while (connector)
	{
		if (connector->GetConnectorKey() == key) return (connector);
		connector = connector->GetNextOutgoingEdge();
	}
