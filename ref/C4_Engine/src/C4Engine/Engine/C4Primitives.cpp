...
namespace C4
{
	template class Constructable<PrimitiveGeometry>;
}
...
PrimitiveGeometryObject *PrimitiveGeometryObject::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
...
int32 PrimitiveGeometryObject::GetCategorySettingCount(Type category) const
{
	...
	if (category == kObjectGeometry) count += 6;
	return (count);
}
...
Setting *PrimitiveGeometryObject::GetCategorySetting(Type category, int32 index, unsigned_int32 flags) const
{
	...
	{
		int32 count = GeometryObject::GetCategorySettingCount(kObjectGeometry);
		if (index >= count)
		{
...
void PrimitiveGeometryObject::SetCategorySetting(Type category, const Setting *setting)
{
	if (category == kObjectGeometry)
	{
		...
		if (identifier == 'CAPS')
		{
			...
			if (b) primitiveFlags |= kPrimitiveEndcap;
			else primitiveFlags &= ~kPrimitiveEndcap;
		}
		else if (identifier == 'IVRT')
		{
			bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
			if (b) primitiveFlags |= kPrimitiveInvert;
			else primitiveFlags &= ~kPrimitiveInvert;
		}
...
void PrimitiveGeometryObject::Preprocess(unsigned_int32 dynamicFlags)
{
	...
	if ((primitiveFlags & (kPrimitiveEndcap | kPrimitiveInvert)) != kPrimitiveEndcap) ClearConvexPrimitiveFlag();
}
...
void PrimitiveGeometryObject::BuildStandardArrays(GeometryLevel *inputLevel, GeometryLevel *outputLevel, const Transformable *transformable) const
{
	GeometryLevel	tempLevel;
	
	inputLevel->GenerateTexcoords(transformable, this);
	inputLevel->TransformTexcoords(transformable, this);
	tempLevel.BuildTangentArray(inputLevel);
	
	if ((GetGeometryFlags() & kGeometryShadowInhibit) == 0)
	{
		inputLevel->BuildPlaneArray(&tempLevel);
		tempLevel.BuildEdgeArray(inputLevel);
	}
	
	outputLevel->BuildSegmentArray(&tempLevel, GetSurfaceCount(), GetSurfaceData());
}
...
void PrimitiveGeometryObject::Build(Geometry *geometry)
{
	...
	for (machine a = 0; a < kMaxAttributeArrayCount; a++) geometry->SetAttributeArray(a, nullptr, 0);
	
	for (machine a = 0; a < buildLevelCount; a++)
	{
		BuildPrimitiveLevel(a, geometry);
		if (primitiveFlags & kPrimitiveInvert) GetGeometryLevel(a)->InvertGeometryLevel();
	}
...
{
	if (Fabs(center.z) > radius) return (true);
	
	if (center.x > plateSize.x + radius) return (true);
	if (center.x < -radius) return (true);
	if (center.y > plateSize.y + radius) return (true);
	if (center.y < -radius) return (true);
...
{
	if ((p1.z * p2.z > 0.0F) && (Fmin(Fabs(p1.z), Fabs(p2.z)) > radius)) return (true);
	
	float sx = plateSize.x + radius;
	if ((p1.x > sx) && (p2.x > sx)) return (true);
	if ((p1.x < -radius) && (p2.x < -radius)) return (true);
	
	float sy = plateSize.y + radius;
	if ((p1.y > sy) && (p2.y > sy)) return (true);
	if ((p1.y < -radius) && (p2.y < -radius)) return (true);
...
void PlateGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	desc[3].identifier = kArrayFace;
	desc[3].elementCount = triangleCount;
	desc[3].elementSize = sizeof(Triangle);
	desc[3].componentCount = 1;
	
	geometryLevel.AllocateStorage(vertexCount, 4, desc);
	
	Point3D *vertex = geometryLevel.GetArray<Point3D>(kArrayVertex) - 1;
	Point2D *texcoord = geometryLevel.GetArray<Point2D>(kArrayTexture0) - 1;
	...
	for (machine a = 0; a < vertexCount; a++) normal[a].Set(0.0F, 0.0F, 1.0F);
	...
	BuildStandardArrays(&geometryLevel, GetGeometryLevel(level), transformable);
}
...
void DiskGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	if (ydiv == 1)
	{
		...
		TriangulateDisk(xdiv, 0, geometryLevel.GetArray<Triangle>(kArrayFace));
	}
	else
	{
...
bool HoleGeometryObject::ExteriorSphere(const Point3D& center, float radius) const
{
	...
	if (center.x > rx) return (true);
	if (center.x < -rx) return (true);
	
	float ry = outerSize.y + radius;
	if (center.y > ry) return (true);
	if (center.y < -ry) return (true);
...
bool HoleGeometryObject::ExteriorSweptSphere(const Point3D& p1, const Point3D& p2, float radius) const
{
	...
	float rx = outerSize.x + radius;
	if ((p1.x > rx) && (p2.x > rx)) return (true);
	if ((p1.x < -rx) && (p2.x < -rx)) return (true);
	
	float ry = outerSize.y + radius;
	if ((p1.y > ry) && (p2.y > ry)) return (true);
	if ((p1.y < -ry) && (p2.y < -ry)) return (true);
...
{
	if (center.z > boxSize.z + radius) return (true);
	if (center.z < -radius) return (true);
	if (center.y > boxSize.y + radius) return (true);
	if (center.y < -radius) return (true);
	if (center.x > boxSize.x + radius) return (true);
	if (center.x < -radius) return (true);
...
bool BoxGeometryObject::ExteriorSweptSphere(const Point3D& p1, const Point3D& p2, float radius) const
{
	...
	if ((p1.z > sz) && (p2.z > sz)) return (true);
	if ((p1.z < -radius) && (p2.z < -radius)) return (true);
...
Point3D BoxGeometryObject::CalculatePrimitiveSupportPoint(const Vector3D& direction) const
{
		...
		float4 size = SimdLoadUnaligned(&boxSize.x);
		float4 mask = SimdMaskCmpgt(SimdLoadUnaligned(&direction.x), SimdGetZero());
		SimdStore3D(SimdAnd(size, mask), &result.x);
		return (result);
...
void BoxGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	geometryLevel.AllocateStorage(vertexCount, 5, desc);
	...
	Vector3D *normal = geometryLevel.GetArray<Vector3D>(kArrayNormal) - 1;
...
{
	if (center.z > pyramidHeight + radius) return (true);
...
{
	if (center.z > cylinderHeight + radius) return (true);
	if (center.z < -radius) return (true);
...
void CylinderGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	const Point3D *vtx = geometryLevel.GetArray<Point3D>(kArrayVertex);
	for (machine j = 0; j < zdiv; j++)
	{
		*++vertex = vtx[j];
		(++normal)->Set(1.0F, 0.0F, 0.0F);
	...
	*++vertex = vtx[zdiv];
	...
	int32 count = (xdiv + 1) * (zdiv + 1);
	unsigned_int16 *surfaceIndex = geometryLevel.GetArray<unsigned_int16>(kArraySurfaceIndex) - 1;
	for (machine a = 0; a < count; a++) *++surfaceIndex = 0;
...
Point3D ConeGeometryObject::CalculatePrimitiveSupportPoint(const Vector3D& direction) const
{
	...
	if (r > K::min_float)
	{
		...
		if (t2 > t1) return (Point3D(0.0F, 0.0F, coneHeight));
		return (Point3D(x, y, 0.0F));
	}
	
	if (direction.z > 0.0F) return (Point3D(0.0F, 0.0F, coneHeight));
...
void ConeGeometryObject::CalculatePrimitiveSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
	for (machine a = 0; a < count; a++)
	{
		...
		if (r > K::min_float)
		{
			...
			if (t2 > t1) support->Set(0.0F, 0.0F, coneHeight);
			else support->Set(x, y, 0.0F);
		}
		else
		{
			if (direction->z > 0.0F) support->Set(0.0F, 0.0F, coneHeight);
...
void ConeGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	const Point3D *vtx = geometryLevel.GetArray<Point3D>(kArrayVertex);
	const Vector3D *nrm = geometryLevel.GetArray<Vector3D>(kArrayNormal);
	for (machine j = 0; j < zdiv; j++)
	{
		*++vertex = vtx[j];
		*++normal = nrm[j];
...
{
	if (center.z > coneHeight + radius) return (true);
	if (center.z < -radius) return (true);
...
Point3D TruncatedConeGeometryObject::CalculatePrimitiveSupportPoint(const Vector3D& direction) const
{
	...
	if (r > K::min_float)
	{
		...
		if (t1 > t2) return (Point3D(x1, y1, 0.0F));
...
void TruncatedConeGeometryObject::CalculatePrimitiveSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
	for (machine a = 0; a < count; a++)
	{
		...
		if (r > K::min_float)
		{
			...
			if (t1 > t2) support->Set(x1, y1, 0.0F);
			else support->Set(x2, y2, coneHeight);
		}
		else
		{
			if (direction->z > 0.0F) support->Set(0.0F, 0.0F, coneHeight);
...
Point3D SphereGeometryObject::CalculatePrimitiveSupportPoint(const Vector3D& direction) const
{
		...
		float4 p = SimdMul(SimdMul(size, size), dir);
...
void SphereGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	const Point3D *vtx = geometryLevel.GetArray<Point3D>(kArrayVertex);
	const Vector3D *nml = geometryLevel.GetArray<Vector3D>(kArrayNormal);
	for (machine j = 0; j < zdiv - 1; j++)
	{
		*++vertex = vtx[j];
		*++normal = nml[j];
...
void DomeGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	for (machine i = 0; i < xdiv; i++)
	{
		...
		for (machine j = 0; j < zdiv; j++)
		{
			...
			Vector2D u = CosSin(t * K::pi_over_2);
...
void TorusGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	const Point3D *vtx = geometryLevel.GetArray<Point3D>(kArrayVertex);
	const Vector3D *nml = geometryLevel.GetArray<Vector3D>(kArrayNormal);
	const Point2D *tex = geometryLevel.GetArray<Point2D>(kArrayTexture0);
...
void PathPrimitiveGeometryObject::Unpack(Unpacker& data, unsigned_int32 unpackFlags)
{
	...
	UnpackChunkList<PathPrimitiveGeometryObject>(data, unpackFlags);
		...
		if (data.GetVersion() < 29)
		{
			const GeometryLevel *geometryLevel = GetGeometryLevel(0);
			primitiveBoundingBox.Calculate(geometryLevel->GetVertexCount(), geometryLevel->GetArray<Point3D>(kArrayVertex));
		}
...
int32 PathPrimitiveGeometryObject::GetCategorySettingCount(Type category) const
{
	...
	if (category == kObjectGeometry) count += 1;
	return (count);
}
...
Setting *PathPrimitiveGeometryObject::GetCategorySetting(Type category, int32 index, unsigned_int32 flags) const
{
	if (category == kObjectGeometry)
	{
		...
		if (index >= count)
		{
			...
			{
				const StringTable *table = TheInterfaceMgr->GetStringTable();
				const char *title = table->GetString(StringID(kObjectGeometry, kGeometryPrimitive, 'TSCL'));
...
void TubeGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	while (component)
	{
		...
		if (nextComponent) endPosition = &nextComponent->GetBeginPosition();
		else endPosition = (closed) ? &pathBeginPosition : &component->GetEndPosition();
		...
		if (component->GetPathType() == kPathLinear)
		{
			...
			Vector3D beginRight = (beginTangent % tubePath->GetPathNormal()).Normalize();
			Vector3D beginDown = beginTangent % beginRight;
			
			Vector3D endRight = (endTangent % tubePath->GetPathNormal()).Normalize();
			...
			const Point3D *vtx = geometryLevel.GetArray<Point3D>(kArrayVertex) + baseVertexIndex;
			const Vector3D *nrm = geometryLevel.GetArray<Vector3D>(kArrayNormal) + baseVertexIndex;
		...
		else
		{
			...
			for (machine i = 0; i < xdiv; i++)
			{
				...
				for (machine j = 0; j < zdiv; j++)
				{
					...
					if (j != 0) tex2 += Magnitude(p2 - p1) * invLength;
					p1 = p2;
					
					Vector3D forward = component->GetTangent(t).Normalize();
					Vector3D right = (forward % tubePath->GetPathNormal()).Normalize();
				...
				Vector3D right = (endTangent % tubePath->GetPathNormal()).Normalize();
			...
			const Point3D *vtx = geometryLevel.GetArray<Point3D>(kArrayVertex) + baseVertexIndex;
			const Vector3D *nrm = geometryLevel.GetArray<Vector3D>(kArrayNormal) + baseVertexIndex;
			const Point2D *tex = geometryLevel.GetArray<Point2D>(kArrayTexture0) + baseVertexIndex;
	...
	if (endcap)
	{
		...
		Vector3D nrml = -component->GetBeginTangent().Normalize();
	...
	BuildStandardArrays(&geometryLevel, GetGeometryLevel(level), transformable);
	if (level == 0) primitiveBoundingBox.Calculate(vertexCount, geometryLevel.GetArray<Point3D>(kArrayVertex));
}
...
void ExtrusionGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	do
	{
		...
		const Point3D& beginPosition = component->GetBeginPosition();
		if (beginPosition != *endPosition) endcap = false;
		endPosition = &component->GetEndPosition();
		
		winding += ((beginPosition - bounds.min) % (*endPosition - bounds.min)).z;
		
		componentCount++;
		component = component->Next();
	} while (component);
	
	winding = (winding > 0.0F) ? 1.0F : -1.0F;
	
	bool closed = (extrusionPath->GetLastPathComponent()->GetEndPosition() == pathBeginPosition);
	if (!closed) endcap = false;
	...
	do
	{
		...
		else
		{
			...
			for (machine i = 0; i < xdiv; i++)
			{
				...
				if (i != 0) tex1 += Magnitude(p2 - p1) * invLength;
				p1 = p2;
	...
	if (endcap)
	{
		...
		do
		{
			if (component->GetPathType() == kPathLinear)
			{
				...
				*--bottomSurfaceIndex = componentCount;
				
				*++topVertex = p2;
				(++topNormal)->Set(0.0F, 0.0F, winding);
				(++topTexcoord)->Set(p2.x * tx, p2.y * ty);
				*++topSurfaceIndex = componentCount + 1;
		...
		for (machine a = count; a < capTriangleCount; a++) triangle[a].Set(capVertexIndex, capVertexIndex, capVertexIndex);
...
void RevolutionGeometryObject::BuildPrimitiveLevel(int32 level, const Transformable *transformable)
{
	...
	while (component)
	{
		...
		const PathComponent *nextComponent = component->Next();
		if (nextComponent) endPosition = &nextComponent->GetBeginPosition();
		else endPosition = &component->GetEndPosition();
...
PrimitiveGeometry *PrimitiveGeometry::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kPrimitivePlate:
...
bool PathPrimitiveGeometry::UnpackChunk(const ChunkHeader *chunkHeader, Unpacker& data, unsigned_int32 unpackFlags)
{
	...
	void PathPrimitiveGeometry::PathLinkProc(Node *node, void *cookie)
	{
		PathPrimitiveGeometry *pathPrimitive = static_cast<PathPrimitiveGeometry *>(cookie);
		pathPrimitive->SetConnectedPathMarker(static_cast<PathMarker *>(node));
	}
...
const char *PathPrimitiveGeometry::GetInternalConnectorKey(int32 index) const
{
	...
	if (index == count) return (kConnectorKeyPath);
	return (nullptr);
}
