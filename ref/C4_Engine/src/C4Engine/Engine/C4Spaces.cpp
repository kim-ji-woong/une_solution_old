...
Space *Space::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kSpaceFog:
...
bool FogSpace::FogVisible(const Node *node, const Region *region)
{
	const FogSpace *fog = static_cast<const FogSpace *>(node);
	return (region->PolygonVisible(4, fog->worldVertex));
}

bool FogSpace::FogOccluded(const Node *node, const Region *region)
{
	const FogSpace *fog = static_cast<const FogSpace *>(node);
	...
	do
	{
		if (region->PolygonOccluded(4, vertex)) return (true);
		region = region->Next();
	} while (region);
...
void FogSpace::CalculatePostTransform(void)
{
...
void FogSpace::EnterZone(Zone *zone)
{
	zone->AddFogSpace(this);
	
	Space::EnterZone(zone);
}
...
AmbientSpace::AmbientSpace() : Space(kSpaceAmbient)
{
}

AmbientSpace::AmbientSpace(const Vector3D& size, int32 x, int32 y, int32 z, const char *name) : Space(kSpaceAmbient)
{
	SetNewObject(new AmbientSpaceObject(size, x, y, z, name));
}

AmbientSpace::AmbientSpace(const AmbientSpace& ambientSpace) : Space(ambientSpace)
{
}
...
Node *AmbientSpace::Replicate(void) const
{
	return (new AmbientSpace(*this));
}

void AmbientSpace::EnterZone(Zone *zone)
{
	zone->SetConnectedAmbientSpace(this);
	Space::EnterZone(zone);
}
...
{
	if (soundRoom) soundRoom->Release();
}
...
void AcousticsSpace::Preprocess(void)
{
	...
	if (!GetManipulator())
	{
		...
		if (!soundRoom) soundRoom = new SoundRoom(object->GetBoxSize());
		else soundRoom->SetRoomSize(object->GetBoxSize());
...
void AcousticsSpace::EnterZone(Zone *zone)
{
	zone->SetConnectedAcousticsSpace(this);
	Space::EnterZone(zone);
}
...
bool OcclusionSpace::BoxVisible(const Node *node, const Region *region)
{
...
void OcclusionSpace::CalculatePostTransform(void)
{
...
void OcclusionSpace::EnterZone(Zone *zone)
{
...
CameraRegion *OcclusionSpace::CalculateFrustumOcclusionRegion(const FrustumCamera *camera, Zone *zone) const
{
	...
	const Transform4D& transform = GetWorldTransform();
	for (machine a = 0; a < vertexCount; a++) tempVertex[1][a] = transform * (occlusionVertexPosition[vertexIndex[a]] & size);
	...
	region->SetFrustumOcclusionPortalPlanes(vertexCount, vertex, faceCount, facePlane);
	return (region);
}

CameraRegion *OcclusionSpace::CalculateOrthoOcclusionRegion(const OrthoCamera *camera, Zone *zone) const
{
	...
	for (machine a = 0; a < vertexCount; a++)
	{
		...
		Vector3D normal = (viewDirection % (v2 - v1)).Normalize();
		*plane++ = Antivector4D(normal, v1) * inverseTransform;
		
		v1 = v2;
	}
	
	for (machine a = 0; a < 3; a++) plane[a] = facePlane[a] * inverseTransform;
	
	region->SetPlaneCount(vertexCount + 3);
	region->SetNonlateralPlaneCount(3);
	return (region);
}
...
void PaintSpace::CalculatePostTransform(void)
{
