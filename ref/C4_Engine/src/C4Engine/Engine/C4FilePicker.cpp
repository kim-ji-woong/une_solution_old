		...
		currentDirectory(catalog->GetRootPath())
{
	...
	if (visit) directory = visit->GetVisitPath();
	else if ((directory) && (directory[0] == 0)) directory = nullptr;
	
	if (directory) currentDirectory += directory;
	else currentDirectory[rootPathLength - 1] = 0;
}
...
void FilePicker::Preprocess(void)
{
	...
	{
		if (filePickerFlags & kFilePickerMultipleSelection) resourceList->SetListFlags(resourceList->GetListFlags() | kListMultipleSelection);
...
void FilePicker::UpdateButtonState(void)
{
	if (!saveButton)
	{
		const Widget *widget = resourceList->GetFirstSelectedListItem();
		if (widget) openButton->Enable();
		else openButton->Disable();
	}
	else
	{ 
		...
		if ((nameBox->GetText()[0] == 0) || ((!(filePickerFlags & kFilePickerEnableRootDirectory)) && (directoryDepth == 1))) saveState |= kWidgetDisabled; 
		...
		{
			const Widget *widget = resourceList->GetFirstSelectedListItem();
			if ((!widget) || (widget->GetNodeIndex() >= directoryCount)) openState |= kWidgetHidden;
			else saveState |= kWidgetHidden; 
		}
...
void FilePicker::BuildFileList(void)
{
	...
	if (resourceCatalog)
	{
		...
		resourceCatalog->BuildResourceList(resourceDescriptor, &currentDirectory[start], &fileList);
	}
	else
	{
		FileMgr::BuildFileList(currentDirectory, &fileList);
	}
	...
	if (!nameBox) resourceList->SelectListItem(0);
...
void FilePicker::BuildDirectoryMenu(void)
{
	...
	int32 start = (filePickerFlags & kFilePickerStayInsideRootDirectory) ? rootPathLength : 0;
	...
	if (count > 1) upButton->Enable();
	else upButton->Disable();
}
...
void FilePicker::WritePersistentConfig(File& file, void *cookie)
{
	const FilePickerVisit *visit = visitMap.First();
	while (visit)
	{
		String<kMaxCommandLength> command("visit '");
		command += Text::TypeToString(visit->GetVisitType());
		command += "' \"";
		command += visit->GetVisitPath();
		command += "\";\r\n";
		file << command;
...
{
	const Widget *widget = resourceList->GetFirstSelectedListItem();
	if ((widget) && (!resourceList->GetNextSelectedListItem(widget)) && (widget->GetNodeIndex() < directoryCount))
	{
		String<kMaxFileNameLength> directory(currentDirectory);
		if (directory[directory.Length() - 1] != '/') directory += '/';
		
		currentDirectory = directory + GetListItemText(widget);
		updateFileListFlag = true;
		return (true);
	}
...
const char *FilePicker::GetListItemText(const Widget *widget)
{
	...
	while (widget)
	{
		if (widget->GetWidgetType() == kWidgetText) return (static_cast<const TextWidget *>(widget)->GetText());
		widget = widget->Next();
	}
...
String<kMaxFileNameLength> FilePicker::GetFileName(int32 index) const
{
	...
	if (!nameBox)
	{
		const Widget *widget = resourceList->GetSelectedListItem(index);
		if ((widget) && (widget->GetNodeIndex() >= directoryCount))
		{
			name += GetListItemText(widget);
			return (name);
		}
	}
	else
	{
		...
		if (resourceDescriptor) name += resourceDescriptor->GetExtension();
		return (name);
	}
...
ResourceName FilePicker::GetResourceName(int32 index) const
{
	...
	if ((!resourceDescriptor) || (!(resourceDescriptor->GetFlags() & kResourceDontAppendType))) resourceName[Text::GetResourceNameLength(resourceName)] = 0;
	return (resourceName);
}
...
bool FilePicker::ConfirmReplace(void)
{
	if (nameBox)
	{
		...
		const Widget *widget = resourceList->GetFirstListItem();
		while (widget)
		{
			if (name == GetListItemText(widget))
			{
...
bool FilePicker::HandleKeyboardEvent(const KeyboardEventData *eventData)
{
	...
	if (eventType == kEventKeyDown)
	{
		...
		if (code == kKeyCodeReturn)
		{
			...
			if (openButton->Visible()) openButton->Activate();
			else saveButton->Activate();
			return (true);
		}
		...
		else if (code == kKeyCodeBackspace)
		{
		...
		else if (FileNameFilter(code))
		{
			if (GetFocusWidget() == resourceList)
			{
				...
				if (time - matchTime < 1000)
				{
					...
					if (length < kMaxFileNameLength) matchText[length] = (char) code;
					matchLength = length + 1;
				}
				else
				{
					matchLength = 1;
					matchText[0] = (char) code;
				}
				...
				{
					bool match = true;
					const char *text = GetListItemText(widget);
...
void FilePicker::HandleWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	...
	if (eventType == kEventWidgetActivate)
	{
		...
		{
			const Widget *widget = resourceList->GetFirstSelectedListItem();
			if ((widget) && (!OpenFolder()) && (ConfirmReplace()))
			{
				TheInterfaceMgr->RemoveWidget(this);
				RememberVisit();
				CallCompletionProc();
	...
	else if (eventType == kEventWidgetChange)
	{
		...
		{
			MenuItemWidget *widget = directoryMenu->GetSelectedMenuItem();
			currentDirectory = widget->GetTextWidget()->GetText();
			updateFileListFlag = true;
		}
		else if (widget == resourceList)
		{
			...
			const Widget *widget = resourceList->GetFirstSelectedListItem();
			if ((widget) && (nameBox) && (widget->GetNodeIndex() >= directoryCount))
			{
				ResourceName name(GetListItemText(widget));
...
{
	if ((eventData->eventType == kEventWidgetChange) && (widget == GetResourceListWidget())) UpdatePreview();
				...
				if (size < 1048576U)
				{
					...
					if (world)
					{
						...
						while (node)
						{
							...
							if ((node->GetNodeType() == kNodeGeometry) && (node->CalculateBoundingBox(&box)))
							{
						...
						if (boxFlag)
						{
							...
							previewViewport->SetCameraAngles(K::pi_over_6 * -5.0F, K::pi_over_6 * -0.75F);
						}
						else
						{
						...
						light->SetNodeMatrix3D(Matrix3D().SetRotationAboutZ(-K::pi_over_4) * Matrix3D().SetRotationAboutY(K::pi_over_4));
						zone->AddNewSubnode(light);
						light->Update();
