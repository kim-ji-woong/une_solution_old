...
Setting *Setting::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
...
SettingInterface::SettingInterface(Setting *setting, const char *title) : valueNameObserver(this, &SettingInterface::HandleValueNameEvent)
{
	settingData = setting;
...
void SettingInterface::HandleValueNameEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		if (static_cast<EditTextWidget *>(widget)->GetText()[0] != 0) SetDeterminantValue();
	}
}

void SettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	settingGroup = group;
	
	TextWidget *text = new TextWidget(Vector2D(config->GetTitleColumnWidth() - 4.0F, kSettingSpacing), GetSettingTitle(), "font/Gui");
	text->SetWidgetPosition(Point3D(4.0F, 0.0F, 0.0F));
	group->AddSubnode(text);
	
	if ((config->GetConfigurationFlags() & kConfigurationScript) && (settingData->GetSettingType() != kSettingHeading))
	{
		valueNameBox = new EditTextWidget(Vector2D(kValueNameWidth - 6.0F, kSettingSpacing - 5.0F), kMaxValueNameLength, "font/Gui");
		valueNameBox->SetWidgetPosition(Point3D(config->GetFullSettingWidth() - (kValueNameWidth - 3.0F), 0.0F, 0.0F));
		valueNameBox->SetEditTextFlags(kEditTextOverflow | kEditTextRenderPlain);
		valueNameBox->SetFilterProc(&EditTextWidget::IdentifierFilter);
		valueNameBox->SetObserver(&valueNameObserver);
		group->AddSubnode(valueNameBox);
	}
	else
	{
		valueNameBox = nullptr;
	}
}
...
{
	if (valueNameBox) settingData->SetSettingValueName(valueNameBox->GetText());
	return (false);
}
...
void HeadingSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	QuadWidget *quad = new QuadWidget(Vector2D(config->GetFullSettingWidth(), kSettingSpacing - 1.0F));
	...
	quad->SetVertexColor(0, top);
	quad->SetVertexColor(1, bottom);
	quad->SetVertexColor(2, top);
	quad->SetVertexColor(3, bottom);
	
	quad->SetWidgetPosition(Point3D(0.0F, -2.0F, 0.0F));
	group->AddSubnode(quad);
	
	SettingInterface::BuildInterface(group, config);
}
...
{
	if (eventData->eventType == kEventWidgetChange) PostEvent();
}

void BooleanSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	checkWidget = new CheckWidget(Vector2D(16.0F, kSettingSpacing));
	checkWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + 4.0F, 0.0F, 0.0F));
	checkWidget->SetObserver(&checkWidgetObserver);
	group->AddSubnode(checkWidget);
...
SliderSettingInterface::SliderSettingInterface(Setting *setting, const char *title) : SettingInterface(setting, title)
{
}
...
IntegerSetting::IntegerSetting(Type identifier, int32 value, const char *title, int32 min, int32 max, int32 step) : Setting(kSettingInteger, identifier)
{
	integerValue = value;
	
	SetSettingInterface(new IntegerSettingInterface(this, title, min, max, step));
}
...
IntegerSettingInterface::IntegerSettingInterface(IntegerSetting *setting, const char *title, int32 min, int32 max, int32 step) :
		SliderSettingInterface(setting, title),
...
void IntegerSettingInterface::HandleSliderWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		int32 value = static_cast<SliderWidget *>(widget)->GetValue() * stepValue + minValue;
...
void IntegerSettingInterface::HandleTextWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		int32 value = Text::StringToInteger(static_cast<EditTextWidget *>(widget)->GetText());
		value = Min(Max(value, minValue), maxValue);
		sliderWidget->SetValue((value - minValue) / stepValue);
		
		PostEvent();
	}
}

void IntegerSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	float valueWidth = config->GetValueColumnWidth();
	float valuePosition = config->GetValueColumnPosition();
	...
	group->AddSubnode(textWidget);
	
	SliderSettingInterface::BuildInterface(group, config);
}
...
void IntegerSettingInterface::SetDeterminantValue(void)
{
	...
	textWidget->SetText(Text::IntegerToString(integerValue));
}
...
PowerTwoSetting::PowerTwoSetting(Type identifier, int32 value, const char *title, int32 min, int32 max) : Setting(kSettingPowerTwo, identifier)
{
	integerValue = value;
	
	SetSettingInterface(new PowerTwoSettingInterface(this, title, min, max));
}
...
PowerTwoSettingInterface::PowerTwoSettingInterface(PowerTwoSetting *setting, const char *title, int32 min, int32 max) :
		SliderSettingInterface(setting, title),
...
void PowerTwoSettingInterface::HandleSliderWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		int32 value = minValue * (1 << static_cast<SliderWidget *>(widget)->GetValue());
		textWidget->SetText(Text::IntegerToString(value));
...
void PowerTwoSettingInterface::HandleTextWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		int32 value = Text::StringToInteger(static_cast<EditTextWidget *>(widget)->GetText());
		value = Min(Max(value, minValue), maxValue);
		sliderWidget->SetValue(31 - Cntlz(value / minValue));
		
		PostEvent();
	}
}

void PowerTwoSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	float valueWidth = config->GetValueColumnWidth();
	float valuePosition = config->GetValueColumnPosition();
...
FloatSetting::FloatSetting(Type identifier, float value, const char *title, float min, float max, float step) : Setting(kSettingFloat, identifier)
{
	floatValue = value;
	
	SetSettingInterface(new FloatSettingInterface(this, title, min, max, step));
}
...
FloatSettingInterface::FloatSettingInterface(FloatSetting *setting, const char *title, float min, float max, float step) :
		SliderSettingInterface(setting, title),
...
void FloatSettingInterface::HandleSliderWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		float value = (float) static_cast<SliderWidget *>(widget)->GetValue() * stepValue + minValue;
...
void FloatSettingInterface::HandleTextWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetChange)
	{
		float value = Text::StringToFloat(static_cast<EditTextWidget *>(widget)->GetText());
		value = Clamp(value, minValue, maxValue);
		sliderWidget->SetValue((int32) ((value - minValue) / stepValue + 0.5F));
		
		PostEvent();
	}
}

void FloatSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	float valueWidth = config->GetValueColumnWidth();
	float valuePosition = config->GetValueColumnPosition();
...
void FloatSettingInterface::SetDeterminantValue(void)
{
	...
	textWidget->SetText(Text::FloatToString(floatValue));
}
...
void TextSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	textWidget = new EditTextWidget(Vector2D(config->GetValueColumnWidth() - 4.0F, kSettingSpacing - 5.0F), maxTextLength, "font/Gui");
	textWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + 2.0F, 0.0F, 0.0F));
...
void TextSettingInterface::SetIndeterminantValue(void)
{
	if (!stripesWidget)
	{
		...
		textWidget->AddNewSubnode(stripesWidget);
		textWidget->SetText(nullptr);
	}
}
...
void MenuSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	menuWidget = new PopupMenuWidget(Vector2D(config->GetValueColumnWidth(), 13.0F), "font/Gui");
	menuWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition(), 0.0F, 0.0F));
	menuWidget->SetPopupMenuFlags(kPopupMenuRenderPlain);
	menuWidget->SetObserver(&menuWidgetObserver);
	group->AddSubnode(menuWidget);
	
	for (machine a = 0; a < menuItemCount; a++) menuWidget->AppendMenuItem(new MenuItemWidget(menuString[a]));
...
{
	if (setting.GetSettingType() == kSettingColor)
	{
...
void ColorSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	colorWidget = new ColorWidget(Vector2D(24.0F, 11.0F));
	colorWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + 4.0F, 1.0F, 0.0F));
...
void ColorSettingInterface::SetIndeterminantValue(void)
{
	if (!stripesWidget)
	{
		...
		colorWidget->AddNewSubnode(stripesWidget);
...
void CheckColorSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	checkWidget = new CheckWidget(Vector2D(16.0F, kSettingSpacing));
	checkWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + 4.0F, 0.0F, 0.0F));
	checkWidget->SetObserver(&checkWidgetObserver);
	...
	colorWidget = new ColorWidget(Vector2D(24.0F, 11.0F));
	colorWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + 23.0F, 1.0F, 0.0F));
	colorWidget->SetColorPickerFlags(pickerFlags);
	colorWidget->SetColorPickerTitle(pickerString);
	colorWidget->SetObserver(&colorWidgetObserver);
	group->AddSubnode(colorWidget);
...
bool CheckColorSettingInterface::ExtractCurrentSetting(void)
{
	...
	int32 value = checkWidget->GetValue();
	if ((value == kWidgetValueIndeterminant) || (stripesWidget)) return (false);
...
void FilePickerSettingInterface::SetIndeterminantValue(void)
{
	if (!stripesWidget)
	{
		...
		resourceWidget->AddNewSubnode(stripesWidget);
...
void ResourceSettingInterface::HandleBrowseWidgetEvent(Widget *widget, const WidgetEventData *eventData)
{
	if (eventData->eventType == kEventWidgetActivate)
	{
		...
		if (resourceDescriptor->DescribesType("tex")) picker = new TexturePicker(type, pickerString, catalog, resourceDescriptor, subdirectory);
		else if (resourceDescriptor->DescribesType("wav")) picker = new SoundPicker(type, pickerString, catalog, resourceDescriptor, subdirectory);
		else if (resourceDescriptor->DescribesType("wld")) picker = new WorldPicker(type, pickerString, catalog, resourceDescriptor, subdirectory);
		else picker = new FilePicker(type, pickerString, catalog, resourceDescriptor, subdirectory);
...
void ResourceSettingInterface::ResourcePickProc(FilePicker *picker, void *cookie)
{
	...
	if (settingInterface->settingFlags & kResourceSettingGenericPath) settingInterface->resourceWidget->SetText(picker->GetFileName());
	else settingInterface->resourceWidget->SetText(picker->GetResourceName());
	
	settingInterface->PostEvent();
}

void ResourceSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	browseWidget = new GuiButtonWidget(Vector2D(19.0F, 19.0F), Point2D(0.5625F, 0.5F), Point2D(0.6875F, 0.625F));
	browseWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + config->GetValueColumnWidth() - 18.0F, -2.0F, 0.0F));
	...
	group->AddSubnode(browseWidget);
	
	resourceWidget = new EditTextWidget(Vector2D(config->GetValueColumnWidth() - 22.0F, kSettingSpacing - 5.0F), kMaxResourceNameLength, "font/Gui");
	resourceWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + 2.0F, 0.0F, 0.0F));
	...
	group->AddSubnode(resourceWidget);
	
	FilePickerSettingInterface::BuildInterface(group, config);
}
...
{
	if (resourceList[0] != 0) resourceList += ';';
	resourceList += name;
}
...
void MultiResourceSettingInterface::ResourcePickProc(FilePicker *picker, void *cookie)
{
	...
	if (settingInterface->settingFlags & kResourceSettingGenericPath) string += picker->GetFileName();
	else string += picker->GetResourceName();
	settingInterface->resourceWidget->SetText(string);
	
	settingInterface->PostEvent();
}

void MultiResourceSettingInterface::BuildInterface(Widget *group, const ConfigurationWidget *config)
{
	browseWidget = new GuiButtonWidget(Vector2D(19.0F, 19.0F), Point2D(0.5625F, 0.5F), Point2D(0.6875F, 0.625F));
	browseWidget->SetWidgetPosition(Point3D(config->GetValueColumnPosition() + config->GetValueColumnWidth() - 18.0F, -2.0F, 0.0F));
	browseWidget->SetWidgetColor(TheInterfaceMgr->GetInterfaceColor(kInterfaceColorButton));
...
void ConfigurationWidget::SetSetting(const Setting *setting)
{
	...
	else if (identifier == 'SCPT')
	{
		...
		if (b) configurationFlags |= kConfigurationScript;
		else configurationFlags &= ~kConfigurationScript;
...
{
	if (type == kWidgetColorLine) return (lineColor);
...
{
	if (type == kWidgetColorBackground) backgroundColor.alpha = alpha;
...
void ConfigurationWidget::SetDynamicWidgetColor(const ColorRGBA& color, WidgetColorType type)
{
	if (type == kWidgetColorLine)
	{
		if (vertexStorage)
		{
			...
			for (machine a = 4; a < count; a++) configurationColor[a] = color.GetColorRGB();
		}
	}
	else if (type == kWidgetColorBackground)
	{
		if (vertexStorage)
		{
			for (machine a = 0; a < 4; a++) configurationColor[a] = color;
		}
	}
	else
	{
...
void ConfigurationWidget::SetDynamicWidgetAlpha(float alpha, WidgetColorType type)
{
	if (type == kWidgetColorBackground)
	{
		if (vertexStorage)
		{
			for (machine a = 0; a < 4; a++) configurationColor[a].alpha = alpha;
		}
	}
	else
	{
...
void ConfigurationWidget::CalculateColumnWidths(void)
{
	...
	if (configurationFlags & kConfigurationScript) width -= kValueNameWidth;
...
{
	if (!(colorOverrideFlags & kWidgetOverrideLineColor)) SetDefaultLineColor();
	
	settingGroup.SetWidgetState(settingGroup.GetWidgetState() | kWidgetNonpersistent);
	AddSubnode(&settingGroup);
...
void ConfigurationWidget::CalculateStructure(void)
{
	...
	if (configurationFlags & kConfigurationScript) vertexCount += 4;
	...
	if ((!vertexStorage) || (GetVertexCount() != vertexCount))
	{
		...
		configurationVertex = reinterpret_cast<Point2D *>(vertexStorage);
		configurationColor = reinterpret_cast<ColorRGBA *>(configurationVertex + vertexCount);
		
		SetVertexCount(vertexCount);
		SetAttributeArray(kArrayVertex, configurationVertex);
		SetAttributeArray(kArrayColor0, configurationColor);
	...
	while (widget)
	{
		if ((unsigned_int32) (index - displaySettingIndex) < (unsigned_int32) displaySettingCount) widget->Show();
		else widget->Hide();
...
void ConfigurationWidget::Build(void)
{
	...
	Point2D *vertex = configurationVertex;
	...
	ConfigurationWidget::SetDynamicWidgetColor(lineColor, kWidgetColorLine);
	ConfigurationWidget::SetDynamicWidgetColor(backgroundColor, kWidgetColorBackground);
	...
	if (configurationFlags & kConfigurationScript)
	{
		...
		vertex[0].Set(x, 0.0F);
		vertex[1].Set(x, h);
		vertex[2].Set(x + 1.0F, h);
		vertex[3].Set(x + 1.0F, 0.0F);
		vertex += 4;
	}
	...
	{
		vertex[0].Set(0.0F, y);
		vertex[1].Set(0.0F, y + 1.0F);
		vertex[2].Set(w, y + 1.0F);
		vertex[3].Set(w, y);
...
void ConfigurationWidget::HandleMouseEvent(const PanelMouseEventData *eventData)
{
	if (eventData->eventType == kEventMouseWheel)
	{
		...
		if (maxIndex > 0)
		{
			int32 index = Min(MaxZero(displaySettingIndex - (int32) eventData->mousePosition.y), maxIndex);
...
Setting *ConfigurationWidget::FindSetting(Type identifier) const
{
	...
	while (setting)
	{
		if (setting->GetSettingIdentifier() == identifier) return (setting);
		setting = setting->Next();
	}
...
void ConfigurationWidget::BuildConfiguration(const Configurable *configurable)
{
	...
	for (machine a = 0; a < settingCount; a++)
	{
		...
		if (setting)
		{
			...
			while (existingSetting)
			{
				...
				{
					if (!(*existingSetting == *setting)) existingSetting->GetSettingInterface()->SetIndeterminantValue();
			...
			if (previousSetting)
			{
				...
				settingGroup.AddSubnodeAfter(subgroup, previousSetting->GetSettingInterface()->settingGroup);
				subgroup->Preprocess();
			}
			else
			{
				settingList.Append(setting);
				settingGroup.AddNewSubnode(subgroup);
			}
	...
	{
		totalCount++;
		setting->GetSettingInterface()->UpdateCurrentSetting();
		setting = setting->Next();
	}
...
void ConfigurationWidget::CommitConfiguration(Configurable *configurable) const
{
	...
	while (setting)
	{
		if (setting->GetSettingInterface()->ExtractCurrentSetting()) configurable->SetSetting(setting);
		setting = setting->Next();
	}
...
void ConfigurationWidget::CommitCategoryConfiguration(Configurable *configurable, Type category) const
{
	...
	while (setting)
	{
		if (setting->GetSettingInterface()->ExtractCurrentSetting()) configurable->SetCategorySetting(category, setting);
		setting = setting->Next();
	}
