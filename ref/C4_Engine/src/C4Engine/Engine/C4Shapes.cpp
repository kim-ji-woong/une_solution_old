...
ShapeObject *ShapeObject::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kShapeBox:
...
{
	if (category == kObjectShape) return (4);
	return (0);
}
...
Setting *ShapeObject::GetCategorySetting(Type category, int32 index, unsigned_int32 flags) const
{
	if (category == kObjectShape)
	{
		...
		if (index == 3)
		{
			...
			while (substance)
			{
				if (substance->GetSubstanceType() == shapeSubstance) selection = count;
...
void ShapeObject::SetCategorySetting(Type category, const Setting *setting)
{
	if (category == kObjectShape)
	{
		...
		else if (identifier == 'COLL')
		{
			...
			if ((b) || (shapeDensity < 0.0F)) shapeFlags |= kShapeCollisionInhibit;
			else shapeFlags &= ~kShapeCollisionInhibit;
		}
...
bool ShapeObject::InteriorPoint(const Point3D& p) const
{
	...
	for (machine a = 0; a < polygonCount; a++)
	{
		...
		if (normal * p > normal * v1) return (false);
	}
		...
		if ((!(d1 < radius)) && (d2 < radius))
		{
			...
			if ((!((line ^ edgeLine) > 0.0F)) && (!((line ^ Bivector4D(v2, v3)) > 0.0F)))
			{
				...
				for (machine c = 3; c <= polygonVertexCount; c++)
				{
					...
					if ((line ^ Bivector4D(*u1, *u2)) > 0.0F) goto next;
					u1 = u2;
				}
...
Point3D BoxShapeObject::CalculateSupportPoint(const Vector3D& direction) const
{
		...
		float4 size = SimdLoadUnaligned(&GetBoxSize().x);
		float4 d = SimdLoadSmearScalar(&GetShapeShrinkSize());
		
		float4 mask = SimdMaskCmpgt(SimdLoadUnaligned(&direction.x), SimdGetZero());
		float4 p = SimdSelect(d, SimdSub(size, d), mask);
		
		SimdStore3D(p, &result.x);
		return (result);
...
void BoxShapeObject::CalculateOpposingSupportPoints(const Vector3D& direction, Point3D *support) const
{
		...
		SimdStore3D(p, &support[0].x);
		SimdStore3D(SimdSub(size, p), &support[1].x);
...
void BoxShapeObject::CalculateSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
		...
		float4 zero = SimdGetZero();
		float4 size = SimdLoadUnaligned(&GetBoxSize().x);
		
		for (machine a = 0; a < count; a++)
		{
			float4 mask = SimdMaskCmpgt(SimdLoadUnaligned(&direction->x), zero);
			SimdStore3D(SimdAnd(size, mask), &support->x);
...
{
	const Vector2D& size = GetCylinderSize();
	return (size.x * size.y * GetCylinderHeight() * K::pi);
}
...
InertiaTensor CylinderShapeObject::CalculateInertiaTensor(void) const
{
	...
	float k = size.x * size.y * height * (GetShapeDensity() * K::pi_over_12);
...
{
	const Vector2D& size = GetConeSize();
	return (size.x * size.y * GetConeHeight() * K::pi_over_3);
}
...
InertiaTensor ConeShapeObject::CalculateInertiaTensor(void) const
{
	...
	float k = size.x * size.y * height * (GetShapeDensity() * K::pi_over_20);
...
Point3D ConeShapeObject::CalculateSupportPoint(const Vector3D& direction) const
{
	...
	if (r > K::min_float)
	{
		...
		if (u > t) return (Point3D(0.0F, 0.0F, h));
		return (Point3D(x, y, d));
	}
	
	if (direction.z > 0.0F) return (Point3D(0.0F, 0.0F, height - coneShrinkSize.z));
	return (Point3D(0.0F, 0.0F, GetShapeShrinkSize()));
}
...
void ConeShapeObject::CalculateSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
	...
	for (machine a = 0; a < count; a++)
	{
		...
		if (r > K::min_float)
		{
			...
			if (u > t) support->Set(0.0F, 0.0F, height);
			else support->Set(x, y, 0.0F);
		}
		else
		{
			if (direction->z > 0.0F) support->Set(0.0F, 0.0F, height);
			else support->Set(0.0F, 0.0F, 0.0F);
		}
...
{
	const Vector3D& size = GetSphereSize();
	return (size.x * size.y * size.z * K::four_pi_over_3);
}
...
InertiaTensor SphereShapeObject::CalculateInertiaTensor(void) const
{
	...
	float k = size.x * size.y * size.z * (K::four_pi_over_15 * GetShapeDensity());
	return (InertiaTensor((y2 + z2) * k, (x2 + z2) * k, (x2 + y2) * k));
}
...
Point3D SphereShapeObject::CalculateSupportPoint(const Vector3D& direction) const
{
		...
		float4 size = SimdLoadUnaligned(&GetSphereSize().x);
		float4 s = SimdSub(size, SimdLoadSmearScalar(&GetShapeShrinkSize()));
		
		float4 dir = SimdLoadUnaligned(&direction.x);
		float4 p = SimdMul(SimdMul(s, s), dir);
		p = SimdMul(p, SimdSmearX(SimdInverseSqrtScalar(SimdDot3D(p, dir))));
...
void SphereShapeObject::CalculateOpposingSupportPoints(const Vector3D& direction, Point3D *support) const
{
		...
		SimdStore3D(p, &support[0].x);
		SimdStore3D(SimdSub(SimdGetZero(), p), &support[1].x);
...
void SphereShapeObject::CalculateSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
		...
		float4 size = SimdLoadUnaligned(&GetSphereSize().x);
		float4 s2 = SimdMul(size, size);
		
		for (machine a = 0; a < count; a++)
		{
			float4 dir = SimdLoadUnaligned(&direction->x);
			float4 p = SimdMul(s2, dir);
			p = SimdMul(p, SimdSmearX(SimdInverseSqrtScalar(SimdDot3D(p, dir))));
			
			SimdStore3D(p, &support->x);
...
float SphereShapeObject::CalculateSubmergedVolume(const Antivector4D& plane, Point3D *centroid) const
{
	if (sphericalFlag)
	{
		...
		{
			centroid->Set(0.0F, 0.0F, 0.0F);
			return (r * r * r * K::four_pi_over_3);
		}
		
		float d2 = d * d;
		float r2 = r * r;
		float volume = r2 * (r * K::two_pi_over_3 + d * K::pi) - d2 * d * K::pi_over_3;
		
		float z = (r2 * d2 * K::pi_over_2 - (r2 * r2 + d2 * d2) * K::pi_over_4) / volume;
...
{
	const Vector3D& size = GetDomeSize();
	return (size.x * size.y * size.z * K::two_pi_over_3);
}
...
InertiaTensor DomeShapeObject::CalculateInertiaTensor(void) const
{
	...
	float k = size.x * size.y * size.z * (K::two_pi_over_15 * GetShapeDensity());
...
float CapsuleShapeObject::CalculateVolume(void) const
{
	...
	return (rxry * size.z * K::four_pi_over_3 + rxry * GetCapsuleHeight() * K::pi);
}
...
InertiaTensor CapsuleShapeObject::CalculateInertiaTensor(void) const
{
	...
	float rxry = size.x * size.y;
	float k1 = rxry * height * (GetShapeDensity() * K::pi_over_12);
	float k2 = rxry * size.z * (GetShapeDensity() * K::two_pi_over_3);
...
Point3D CapsuleShapeObject::CalculateSupportPoint(const Vector3D& direction) const
{
		...
		float4 size = SimdLoadUnaligned(&GetCapsuleSize().x);
		float4 s = SimdSub(size, SimdLoadSmearScalar(&GetShapeShrinkSize()));
		...
		float4 z = SimdSmearZ(p);
		z = SimdSelect(z, SimdAdd(z, SimdLoadScalar(&GetCapsuleHeight())), SimdMaskCmpgt(SimdSmearZ(dir), SimdGetZero()));
		
		SimdStoreX(p, &result.x, 0);
		SimdStoreY(p, &result.x, 1);
		SimdStoreScalar(z, &result.z);
		return (result);
		...
		z *= r;
		if (direction.z > 0.0F) z += GetCapsuleHeight();
		return (Point3D(x * r, y * r, z));
...
void CapsuleShapeObject::CalculateOpposingSupportPoints(const Vector3D& direction, Point3D *support) const
{
		...
		if (direction.z > 0.0F)
		{
			SimdStoreX(p, &support[0].x, 0);
			SimdStoreY(p, &support[0].x, 1);
			SimdStoreScalar(SimdAdd(z, SimdLoadScalar(&GetCapsuleHeight())), &support[0].z);
			
			SimdStore3D(SimdNegate(p), &support[1].x);
		}
		else
		{
			SimdStore3D(p, &support[0].x);
			...
			p = SimdNegate(p);
			SimdStoreX(p, &support[1].x, 0);
			SimdStoreY(p, &support[1].x, 1);
			SimdStoreScalar(SimdSub(SimdLoadScalar(&GetCapsuleHeight()), z), &support[1].z);
		}
...
void CapsuleShapeObject::CalculateSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
		...
		float4 height = SimdLoadScalar(&GetCapsuleHeight());
		float4 size = SimdLoadUnaligned(&GetCapsuleSize().x);
		float4 s2 = SimdMul(size, size);
		...
		for (machine a = 0; a < count; a++)
		{
			...
			float4 z = SimdSmearZ(p);
			z = SimdSelect(z, SimdAdd(z, height), SimdMaskCmpgt(SimdSmearZ(dir), SimdGetZero()));
			
			SimdStoreX(p, &support->x, 0);
			SimdStoreY(p, &support->x, 1);
			SimdStoreScalar(z, &support->z);
...
void TruncatedPyramidShapeObject::CalculateSupportPointArray(int32 count, const Vector3D *direction, Point3D *support) const
{
	...
	for (machine a = 0; a < count; a++)
	{
		...
		if (t > u) support->Set(x1, y1, 0.0F);
		else support->Set(x2, y2, height);
...
float TruncatedConeShapeObject::CalculateVolume(void) const
{
	...
	return (size.x * size.y * (GetConeHeight() * K::pi_over_3) * (ratio * ratio + ratio + 1.0F));
}
...
InertiaTensor TruncatedConeShapeObject::CalculateInertiaTensor(void) const
{
	...
	float k1 = size.x * size.y * height * (GetShapeDensity() * K::pi_over_20);
...
Point3D TruncatedConeShapeObject::CalculateSupportPoint(const Vector3D& direction) const
{
	...
	if (direction.z > 0.0F) return (Point3D(0.0F, 0.0F, h));
	return (Point3D(0.0F, 0.0F, d));
}
...
float TruncatedDomeShapeObject::CalculateVolume(void) const
{
	...
	return (size.x * size.y * (GetDomeHeight() * K::pi_over_3) * (ratio * ratio + 2.0F));
}
...
InertiaTensor TruncatedDomeShapeObject::CalculateInertiaTensor(void) const
{
	...
	float k = size.x * size.y * height * (GetShapeDensity() * K::pi_over_240) / (r2 + 2.0F);
...
Point3D TruncatedDomeShapeObject::CalculateSupportPoint(const Vector3D& direction) const
{
	...
	if (r > K::min_float)
	{
		if (direction.z > 0.0F)
		{
			...
			z = z * f + d;
			if (z < h) return (Point3D(x * f, y * f, z));
	...
	if (direction.z > 0.0F) return (Point3D(0.0F, 0.0F, GetDomeHeight() - d));
	return (Point3D(0.0F, 0.0F, d));
}
...
Shape *Shape::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kShapeBox:
...
bool Shape::StaticIntersectShape(const Shape *betaShape, RigidBodyContact *rigidBodyContact, IntersectionData *intersectionData) const
{
	...
	Transform4D alphaTransform = alphaBody->GetFinalWorldTransform() * GetNodeTransform();
	Transform4D betaTransform = betaBody->GetFinalWorldTransform() * betaShape->GetNodeTransform();
	...
	for (unsigned_int32 simplexParity = 0;;)
	{
		...
		unsigned_int32 mask = PhysicsController::CalculateSimplexMinimum(simplexVertexCount, deltaVertex[simplexParity], &minPoint);
		if (SquaredMag(minPoint) == 0.0F) break;
	...
	if (rigidBodyContact) rigidBodyContact->cachedSimplexVertexCount = 0;
...
bool Shape::StaticIntersectShape(const Shape *betaShape, IntersectionData *intersectionData, int32 *dimension1, int32 *dimension2, const Vector3D& worldOffset) const
{
	...
	unsigned_int32 simplexParity = 0;
	...
	for (;;)
	{
		...
		if (!(SquaredMag(minPoint) < SquaredMag(prevMinPoint) - kSupportPointTolerance))
		{
			...
			intersectionData->alphaContact = c1 + normal * shrink1 + worldOffset;
			intersectionData->betaContact = c2 - normal * shrink2;
			intersectionData->contactNormal = normal;
			intersectionData->contactParam = 1.0F;
			*dimension1 = dim1;
...
bool Shape::FindMinimumShapePenetration(const Shape *betaShape, const Transform4D& alphaTransform, const Transform4D& alphaInverseTransform, const Transform4D& betaTransform, const Transform4D& betaInverseTransform, IntersectionData *intersectionData) const
{
	...
	if (!StaticIntersectShape(betaShape, intersectionData, &dim1, &dim2, delta))
	{
		if (dim1 < dim2)
		{
			...
			intersectionData->contactNormal = ProjectOnto(intersectionData->contactNormal, direction).Normalize();
		}
		else if (dim2 < dim1)
		{
...
bool Shape::StaticIntersectPrimitive(const Geometry *geometry, GeometryContact *geometryContact, IntersectionData *intersectionData) const
{
	int32		simplexVertexCount;
	...
	Transform4D shapeTransform = rigidBody->GetFinalWorldTransform() * GetNodeTransform();
	...
	if (geometryContact) geometryContact->cachedSimplexVertexCount = 0;
	return (FindMinimumPrimitivePenetration(geometry, shapeTransform, shapeInverseMatrix3D, intersectionData));
}

bool Shape::StaticIntersectPrimitive(const Geometry *geometry, IntersectionData *intersectionData, int32 *dimension1, int32 *dimension2, const Vector3D& worldOffset) const
{
	Point3D		shapeVertex[2][4];
	Point3D		geometryVertex[2][4];
	Point3D		deltaVertex[2][4];
...
bool Shape::FindMinimumPrimitivePenetration(const Geometry *geometry, const Transform4D& shapeTransform, const Matrix3D& shapeInverseMatrix3D, IntersectionData *intersectionData) const
{
	int32		dim1, dim2;
...
bool Shape::DynamicIntersectPrimitive(const Geometry *geometry, const Vector3D& displacement, IntersectionData *intersectionData) const
{
	if (SquaredMag(displacement) > kIntersectionDisplacementEpsilon)
	{
		Point3D		shapeVertex[2][4];
...
int32 Shape::CalculateTriangleSupportPoint(const Point3D *vertex, const Vector3D& direction)
{
	#if C4SIMD
	
		float4 u = SimdLoadUnaligned(&direction.x);
		float4 d1 = SimdDot3D(SimdLoadUnaligned(&vertex[0].x), u);
		float4 d2 = SimdDot3D(SimdLoadUnaligned(&vertex[1].x), u);
		float4 d3 = SimdDot3D(SimdLoadUnaligned(&vertex[2].x), u);
		
		bool m1 = SimdCmpgtScalar(d2, d1);
		bool m2 = SimdCmpgtScalar(d3, d1);
		bool m3 = SimdCmpgtScalar(d3, d2);
...
bool Shape::StaticIntersectTriangle(const Point3D *triangleVertex, const Triangle *triangle, const TriangleIntersectionParams *params, IntersectionData *intersectionData) const
{
	Point3D			support[2];
	Point3D			shapeVertex[2][4];
	Point3D			geometryVertex[2][4];
	...
	Vector3D triangleNormal = (triangleVertex[1] - triangleVertex[0]) % (triangleVertex[2] - triangleVertex[0]);
	triangleNormal.Normalize();
	
	const ShapeObject *shapeObject = GetObject();
	float shrink = shapeObject->GetShapeShrinkSize();
	
	shapeObject->CalculateOpposingSupportPoints(params->shapeInverseMatrix3D * triangleNormal, shapeVertex[0]);
	support[0] = params->shapeTransform * shapeVertex[0][0];
	support[1] = params->shapeTransform * shapeVertex[0][1];
	deltaVertex[0][0] = support[1] - triangleVertex[0];
	
	if (((support[0] - triangleVertex[0]) * triangleNormal < -shrink) || (deltaVertex[0][0] * triangleNormal > shrink)) return (false);
	
	Point3D prevMinPoint = deltaVertex[0][0];
	shapeVertex[0][0] = shapeVertex[0][1];
	geometryVertex[0][0] = triangleVertex[0];
	supportIndex[0][0] = 0;
	...
	for (unsigned_int32 simplexParity = 0;;)
	{
		...
		int32 vertexIndex = CalculateTriangleSupportPoint(triangleVertex, direction);
		supportIndex[simplexParity][simplexVertexCount] = (unsigned_int8) vertexIndex;
		geometryVertex[simplexParity][simplexVertexCount] = triangleVertex[vertexIndex];
		...
		if (!(SquaredMag(minPoint) < SquaredMag(prevMinPoint) - kSupportPointTolerance))
		{
			...
			const Vector3D *normalArray = params->geometryLevel->GetArray<Vector3D>(kArrayNormal);
			...
			if (simplexVertexCount == 1)
			{
				...
				edgeNormal = *params->geometryTransform * normalArray[triangle->index[supportIndex[simplexParity][0]]];
			...
			else if (simplexVertexCount == 2)
			{
				...
				edgeNormal = *params->geometryTransform * (normalArray[triangle->index[supportIndex[simplexParity][0]]] * w1 + normalArray[triangle->index[supportIndex[simplexParity][1]]] * w2);
			...
			if (m2 > K::min_float)
			{
				if (m2 < shrink * shrink)
				{
					...
					intersectionData->betaContact = c2;
					if (params->geometryObject->GetGeometryFlags() & kGeometryInvisible) intersectionData->contactNormal = normal * m2;	// [HACK] This prevents getting stuck on block seams.
					else intersectionData->contactNormal = -*geometryNormal;
					intersectionData->contactParam = 1.0F;
					return (true);
				}
	...
	intersectionData->alphaContact = support[1] - triangleNormal * shrink;
	intersectionData->betaContact = intersectionData->alphaContact - (triangleNormal * (intersectionData->alphaContact - triangleVertex[0])) * triangleNormal;
	intersectionData->contactNormal = -triangleNormal;
	intersectionData->contactParam = 1.0F;
	return (true);
}

bool Shape::DynamicIntersectTriangle(const Point3D *triangleVertex, const Triangle *triangle, const Vector3D& displacement, const TriangleIntersectionParams *params, IntersectionData *intersectionData) const
{
	Point3D			shapeVertex[2][4];
	Point3D			geometryVertex[2][4];
	Point3D			deltaVertex[2][4];
	...
	Vector3D triangleNormal = (triangleVertex[1] - triangleVertex[0]) % (triangleVertex[2] - triangleVertex[0]);
	if (triangleNormal * displacement > 0.0F) return (false);
	triangleNormal.Normalize();
	...
	Point3D initialShapeVertex = shapeObject->CalculateSupportPoint(params->shapeInverseMatrix3D * -triangleNormal);
	Vector3D initialDeltaVertex = params->shapeTransform * initialShapeVertex - displacement - triangleVertex[0];
	if (initialDeltaVertex * triangleNormal > -shrink)
	{
		start:
		deltaVertex[0][0] = initialDeltaVertex + ds;
		Point3D prevMinPoint = deltaVertex[0][0];
		...
		for (unsigned_int32 simplexParity = 0;;)
		{
			...
			if (f1 > (tmin + minAdvance) * f2)
			{
				...
				tmin = f1 / f2;
				if (tmin > intersectionData->contactParam) return (false);
	...
	return (StaticIntersectTriangle(triangleVertex, triangle, params, intersectionData));
}

int32 Shape::StaticIntersectGeometryOctree(const CollisionOctree *octree, const Box3D& collisionBox, const TriangleIntersectionParams *params, IntersectionData *intersectionData, int32 contactCount) const
{
	if (octree->collisionBounds.Intersection(collisionBox))
	{
		const GeometryLevel *geometryLevel = params->geometryLevel;
		const Point3D *geometryVertex = geometryLevel->GetArray<Point3D>(kArrayVertex);
		const Triangle *geometryTriangle = geometryLevel->GetArray<Triangle>(kArrayFace);
		...
		const unsigned_int16 *triangleIndex = octree->GetIndexArray();
			...
			float4 bmin = SimdLoadUnaligned(&shapeCollisionBox.min.x);
			float4 bmax = SimdLoadUnaligned(&shapeCollisionBox.max.x);
		
		#endif
		
		for (machine a = 0; a < triangleCount; a++)
		{
			Point3D		worldVertex[3];
			Point3D		contact1, contact2;
			
			const Triangle *triangle = &geometryTriangle[triangleIndex[a]];
			
			#if C4SIMD
			
				register float4 c1 = SimdLoad(&(*params->geometryTransform)(0,0));
				register float4 c2 = SimdLoad(&(*params->geometryTransform)(0,0), 4);
				register float4 c3 = SimdLoad(&(*params->geometryTransform)(0,0), 8);
				register float4 c4 = SimdLoad(&(*params->geometryTransform)(0,0), 12);
				
				float4 v1 = SimdTransformPoint3D(c1, c2, c3, c4, SimdLoadUnaligned(&geometryVertex[triangle->index[0]].x));
				float4 v2 = SimdTransformPoint3D(c1, c2, c3, c4, SimdLoadUnaligned(&geometryVertex[triangle->index[1]].x));
				float4 v3 = SimdTransformPoint3D(c1, c2, c3, c4, SimdLoadUnaligned(&geometryVertex[triangle->index[2]].x));
				
				float4 vmin = SimdMin(SimdMin(v1, v2), v3);
				float4 vmax = SimdMax(SimdMax(v1, v2), v3);
				if ((SimdCmpgtAny3D(vmin, bmax)) || (SimdCmpltAny3D(vmax, bmin))) continue;
				
				SimdStore3D(v1, &worldVertex[0].x);
				SimdStore3D(v2, &worldVertex[1].x);
				SimdStore3D(v3, &worldVertex[2].x);
			
			#else
			
				worldVertex[0] = *params->geometryTransform * geometryVertex[triangle->index[0]];
				worldVertex[1] = *params->geometryTransform * geometryVertex[triangle->index[1]];
				worldVertex[2] = *params->geometryTransform * geometryVertex[triangle->index[2]];
				
				float xmin = Fmin(worldVertex[0].x, worldVertex[1].x, worldVertex[2].x);
				float xmax = Fmax(worldVertex[0].x, worldVertex[1].x, worldVertex[2].x);
				float ymin = Fmin(worldVertex[0].y, worldVertex[1].y, worldVertex[2].y);
				float ymax = Fmax(worldVertex[0].y, worldVertex[1].y, worldVertex[2].y);
				float zmin = Fmin(worldVertex[0].z, worldVertex[1].z, worldVertex[2].z);
				float zmax = Fmax(worldVertex[0].z, worldVertex[1].z, worldVertex[2].z);
				
				if (!shapeCollisionBox.Intersection(Box3D(Point3D(xmin, ymin, zmin), Point3D(xmax, ymax, zmax)))) continue;
			
			#endif
			
			if (StaticIntersectTriangle(worldVertex, triangle, params, &intersectionData[contactCount]))
			{
				intersectionData[contactCount].triangleIndex = a;
				if (++contactCount == kMaxIntersectionContactCount) return (kMaxIntersectionContactCount);
			}
		}
...
int32 Shape::DynamicIntersectGeometryOctree(const Vector3D& displacement, const CollisionOctree *octree, const Box3D& collisionBox, const TriangleIntersectionParams *params, IntersectionData *intersectionData, int32 contactCount) const
{
	if (octree->collisionBounds.Intersection(collisionBox))
	{
		const GeometryLevel *geometryLevel = params->geometryLevel;
		...
		for (machine a = 0; a < triangleCount; a++)
		{
			...
			if (DynamicIntersectTriangle(worldVertex, triangle, displacement, params, &intersectionData[0]))
			{
				intersectionData[0].triangleIndex = a;
				contactCount = 1;
			}
		}
...
int32 Shape::MixedIntersectGeometryOctree(const Vector3D& displacement, const CollisionOctree *octree, const Box3D& collisionBox, int32 contactTriangleCount, const unsigned_int32 *contactTriangleIndex, const TriangleIntersectionParams *params, IntersectionData *intersectionData, int32 contactCount) const
{
	if (octree->collisionBounds.Intersection(collisionBox))
	{
		const GeometryLevel *geometryLevel = params->geometryLevel;
		...
		for (machine a = 0; a < triangleCount; a++)
		{
			...
			if (DynamicIntersectTriangle(worldVertex, triangle, displacement, params, &intersectionData[contactCount]))
			{
				intersectionData[contactCount].triangleIndex = a;
				if (++contactCount == kMaxIntersectionContactCount) return (kMaxIntersectionContactCount);
			}
...
int32 Shape::StaticIntersectGeometry(const Geometry *geometry, IntersectionData *intersectionData) const
{
	...
	const RigidBodyController *rigidBody = static_cast<RigidBodyController *>(GetSuperNode()->GetController());
	params.shapeTransform = rigidBody->GetFinalWorldTransform() * GetNodeTransform();
	params.shapeInverseMatrix3D = Adjugate3D(params.shapeTransform);
	
	const GeometryObject *geometryObject = geometry->GetObject();
	params.geometryObject = geometryObject;
	params.geometryLevel = geometryObject->GetGeometryLevel(geometryObject->GetCollisionLevel());
...
int32 Shape::MixedIntersectGeometry(const Geometry *geometry, const Vector3D& displacement, int32 contactTriangleCount, const unsigned_int32 *contactTriangleIndex, IntersectionData *intersectionData) const
{
	if (SquaredMag(displacement) > kIntersectionDisplacementEpsilon)
	{
		...
		for (machine a = 0; a < kMaxIntersectionContactCount; a++) intersectionData[a].contactParam = 1.0F;
