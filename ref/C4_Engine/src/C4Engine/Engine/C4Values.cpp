...
Value *Value::Construct(Unpacker& data)
{
	switch (data.GetType())
	{
		case kValueBoolean:
...
Setting *Value::GetSetting(int32 index) const
{
	...
	{
		int32 selection = 0;
		if (valueType == kValueInteger) selection = 1;
		else if (valueType == kValueFloat) selection = 2;
		else if (valueType == kValueString) selection = 3;
		else if (valueType == kValueColor) selection = 4;
		else if (valueType == kValueVector) selection = 5;
		else if (valueType == kValuePoint) selection = 6;
	...
	if (index == 2)
	{
		int32 selection = (valueScope == kValueScopeObject);
		
		const char *title = table->GetString(StringID('VALU', 'SCOP'));
		MenuSetting *menu = new MenuSetting('SCOP', selection, title, 2);
		
		menu->SetMenuItemString(0, table->GetString(StringID('VALU', 'SCOP', kValueScopeScript)));
		menu->SetMenuItemString(1, table->GetString(StringID('VALU', 'SCOP', kValueScopeObject)));
...
void Value::SetSetting(const Setting *setting)
{
	...
	{
		int32 selection = static_cast<const MenuSetting *>(setting)->GetMenuSelection();
		valueScope = (selection == 0) ? kValueScopeScript : kValueScopeObject;
	}
}
...
Setting *BooleanValue::GetSetting(int32 index) const
{
	...
	if (index < count) return (Value::GetSetting(index));
