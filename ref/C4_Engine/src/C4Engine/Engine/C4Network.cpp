...
namespace
{
	inline unsigned_int32 ReadBigEndianU32(const unsigned_int32 *ptr)
	{
			...
			const unsigned_int8 *d = reinterpret_cast<const unsigned_int8 *>(ptr);
	...
	inline void WriteBigEndianU32(unsigned_int32 *ptr, unsigned_int32 data)
	{
			...
			*ptr = data;
			...
			unsigned_int8 *d = reinterpret_cast<unsigned_int8 *>(ptr);
			d[0] = (unsigned_int8) (data >> 24);
			d[1] = (unsigned_int8) (data >> 16);
			d[2] = (unsigned_int8) (data >> 8);
			d[3] = (unsigned_int8) data;
...
NetworkConnection::NetworkConnection(const NetworkAddress& address, ConnectionStatus status, unsigned_int32 packetNumber)
{
	...
	for (machine a = 0; a < kMaxIncomingPacketCount; a++) incomingReliablePacketPool.Append(&incomingReliablePacket[a]);
	for (machine a = 0; a < kMaxIncomingPacketCount; a++) incomingUnreliablePacketPool.Append(&incomingUnreliablePacket[a]);
	for (machine a = 0; a < kMaxOutgoingPacketCount; a++) outgoingDataPacketPool.Append(&outgoingDataPacket[a]);
	for (machine a = 0; a < kMaxOutgoingPacketCount; a++) outgoingControlPacketPool.Append(&outgoingControlPacket[a]);
}
...
NetworkResult NetworkSocket::Open(const NetworkAddress& address)
{
		...
		socketHandle = WSASocketA(AF_INET, SOCK_DGRAM, IPPROTO_UDP, nullptr, 0, 0);
		if (socketHandle == INVALID_SOCKET) return (kNetworkInitFailed);
		...
		if (address.GetPort() == 0)
		{
			...
			if (getsockname(socketHandle, reinterpret_cast<SOCKADDR *>(&inetAddress), &size) == 0) socketAddress.SetPort(ntohs(inetAddress.sin_port));
		}
		...
		if (socketDesc == -1) return (kNetworkInitFailed);
		...
		if (address.GetPort() == 0)
		{
			...
			if (getsockname(socketDesc, reinterpret_cast<sockaddr *>(&inetAddress), &size) == 0) socketAddress.SetPort(ntohs(inetAddress.sin_port));
		}
...
void NetworkSocket::SocketThread(const Thread *thread, void *cookie)
{
	NetworkSocket *networkSocket = static_cast<NetworkSocket *>(cookie);
		...
		for (;;)
		{
			...
			if (d == WSA_WAIT_EVENT_0) break;
			
			if (WSAEnumNetworkEvents(networkSocket->socketHandle, networkSocket->socketEvent[1], &events) == 0)
			{
				if (events.lNetworkEvents & FD_READ) networkSocket->ReceivePackets();
				if (events.lNetworkEvents & FD_WRITE) networkSocket->SendPackets();
			}
		}
		...
		for (;;)
		{
			...
			if (FD_ISSET(pipeDesc, &readSet)) break;
...
bool NetworkSocket::Send(NetworkPacket *packet)
{
		...
		{
			int error = errno;
			if ((error == EWOULDBLOCK) || (error == ENOBUFS)) sendBlocked = true;
		}
...
void NetworkSocket::SendPackets(void)
{
	...
	for (;;)
	{
		...
		{
			if (packet->GetPacketType() == kPacketAcknowledge) packet->packetPool->Append(packet);
			break;
		}
	}
	...
	while (connection)
	{
		for (;;)
		{
			...
			{
				if (packet->GetPacketType() >= kPacketUnreliable) packet->packetPool->Append(packet);
				goto exit;
			}
		}
				...
				if (rawPacket.packetSize > 4)
				{
					...
					if ((connection) && (connection->GetStatus() == kConnectionOpen))
					{
						...
						if (packet)
						{
							...
							if (delta >= 0)
							{
								if (delta >= kMaxIncomingPacketCount) break;
...
DomainNameResolver::~DomainNameResolver()
{
		...
		if (resolveHost != nullptr)
		{
			...
			if (resolveResult == kNetworkResolvePending) CFHostCancelInfoResolution(resolveHost, kCFHostAddresses);
		...
		if (resolveFlag)
		{
			if (resolveResult == kNetworkResolvePending) pthread_cancel(resolveThread);
...
String<kMaxHostNameLength> DomainNameResolver::GetName(void) const
{
	String<kMaxHostNameLength> name("");
		...
		if (resolveResult == kNetworkOkay) name = hostInfo.h_name;
...
NetworkAddress DomainNameResolver::GetAddress(void) const
{
		...
		if (resolveResult == kNetworkOkay) return (NetworkAddress(ntohl(*reinterpret_cast<unsigned_int32 *>(hostInfo.h_addr_list[0]))));
	...
	void DomainNameResolver::ResolveCallback(HANDLE handle, int error)
	{
		...
		while (resolver)
		{
			...
			{
				if (error) resolver->resolveResult = kNetworkResolveNotFound;
				else resolver->resolveResult = kNetworkOkay;
	...
	void *DomainNameResolver::ResolveThread(void *cookie)
	{
		...
		if (getaddrinfo(resolver->resolveName, nullptr, &info, &resolver->resolveAddress) == 0) resolver->resolveResult = kNetworkOkay;
		else resolver->resolveResult = kNetworkResolveNotFound;
...
void DomainNameResolver::ResolveAddress(const char *name)
{
		...
		resolveHandle = WSAAsyncGetHostByName(TheEngine->GetWindow(), kWindowsMessageResolve, name, hostData, MAXGETHOSTSTRUCT);
		if (resolveHandle) resolveResult = kNetworkResolvePending;
		else resolveResult = kNetworkResolveFailed;
		...
		if (resolveHost != nullptr)
		{
			if (CFHostSetClient(resolveHost, &ResolveCallback, &resolveContext))
			{
				...
				if (CFHostStartInfoResolution(resolveHost, kCFHostAddresses, nullptr)) result = kNetworkResolvePending;
			}
		}
...
{
	if (networkSocket.Open()) return (kNetworkOkay);
		...
		if (getifaddrs(&addressList) != 0) return (kNetworkInitFailed);
		...
		if (!address) return (kNetworkInitFailed);
		
		broadcastAddress = (inetAddress & subnetMask) | ~subnetMask;
		
		NetworkAddress localAddress(inetAddress, localPortNumber);
		if (networkSocket.Open(localAddress) != kNetworkOkay) return (kNetworkInitFailed);
...
int32 NetworkMgr::GetPacketCount(const NetworkAddress& address, unsigned_int32 flags)
{
	...
	if (connection)
	{
		...
		while (packet)
		{
			if ((packet->packetAddress == address) && ((packet->packetFlags & flags) == flags)) count++;
			packet = packet->Next();
		}
...
NetworkResult NetworkMgr::SendControlPacket(const NetworkAddress& address, unsigned_int32 number, unsigned_int32 *param)
{
	...
	if (send) networkSocket.SendPackets();
	return (result);
}
...
void NetworkMgr::FlushPackets(const NetworkAddress& address)
{
	...
	{
		NetworkPacket *next = packet->Next();
		if (packet->packetAddress == address) packet->packetPool->Append(packet);
		packet = next;
	}
...
void NetworkMgr::ResetPackets(void)
{
	...
	for (machine a = 0; a < kMaxIncomingPacketCount; a++) incomingGeneralPacketPool.Append(&incomingGeneralPacket[a]);
	for (machine a = 0; a < kMaxOutgoingPacketCount; a++) outgoingDataPacketPool.Append(&outgoingDataPacket[a]);
	for (machine a = 0; a < kMaxOutgoingPacketCount; a++) outgoingControlPacketPool.Append(&outgoingControlPacket[a]);
}
			...
			switch (packet->packetNumber.GetNumber())
			{
				case kPacketNumberDisconnect:
				{
					if (connection)
					{
						while (next)
						{
							if (next->packetAddress != connection->GetAddress()) break;
							next = next->Next();
						}
						...
						if (connection->GetStatus() != kConnectionDisconnect)
						{
							...
							if (networkEventProc) (*networkEventProc)(kNetworkEventClose, connection->GetAddress(), 0);
						}
				...
				case kPacketNumberConnectRequest:
				{
					...
					else
					{
						if (!connection)
						{
							...
							if (networkEventProc) (*networkEventProc)(kNetworkEventOpen, connection->GetAddress(), 0);
						}
						else
						{
				...
				case kPacketNumberConnectAccept:
				{
					if ((connection) && (connection->GetStatus() == kConnectionRequest))
					{
						...
						if (networkEventProc) (*networkEventProc)(kNetworkEventAccept, connection->GetAddress(), 0);
					}
				...
				case kPacketNumberConnectDeny:
				{
					if ((connection) && (connection->GetStatus() == kConnectionRequest))
					{
						...
						if (networkEventProc) (*networkEventProc)(kNetworkEventFail, connection->GetAddress(), param);
						delete connection;
					}
			...
			if (time - connection->controlSendTime >= reliableResendTime)
			{
				...
				if (status == kConnectionRequest)
				{
					...
					else
					{
						...
						if (networkEventProc) (*networkEventProc)(kNetworkEventFail, connection->GetAddress(), kNetworkFailTimeout);
						delete connection;
					}
				}
				else
			...
			while (packet)
			{
				...
				if (time - packet->sentTime >= packet->resendTime)
				{
					...
					else
					{
						...
						{
							FlushPackets(connection->GetAddress());
							if (networkEventProc) (*networkEventProc)(kNetworkEventTimeout, connection->GetAddress(), 0);
