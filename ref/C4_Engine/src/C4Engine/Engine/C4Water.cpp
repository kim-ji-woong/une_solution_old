...
namespace C4
{
	template <> Heap Memory<WaveGenerator>::heap("WaveGenerator", 8192, kHeapMutexless);
	template class Memory<WaveGenerator>;
}
...
WaveGenerator *WaveGenerator::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
	...
	for (machine v = minBlockCoord.y; v <= maxBlockCoord.y; v++)
	{
		for (machine u = minBlockCoord.x; u <= maxBlockCoord.x; u++)
		{
			...
			if (geometry)
			{
				...
				if ((u == maxBlockCoord.x) || (!geometry->GetAdjacentWater(kWaterDirectionEast))) data->generatorBox.max.x = Min(data->generatorBox.max.x, dimension - 1);
				if ((u == minBlockCoord.x) || (!geometry->GetAdjacentWater(kWaterDirectionWest))) data->generatorBox.min.x = Max(data->generatorBox.min.x, 1);
				if ((v == maxBlockCoord.y) || (!geometry->GetAdjacentWater(kWaterDirectionNorth))) data->generatorBox.max.y = Min(data->generatorBox.max.y, dimension - 1);
				if ((v == minBlockCoord.y) || (!geometry->GetAdjacentWater(kWaterDirectionSouth))) data->generatorBox.min.y = Max(data->generatorBox.min.y, 1);
				
				if ((data->generatorBox.max.x >= data->generatorBox.min.x) && (data->generatorBox.max.y >= data->generatorBox.min.y)) count++;
			}
		}
	}
...
bool PointWaveGenerator::Execute(const WaterController *controller)
{
	...
	{
		if (data[a].waterGeometry->GetWaterState() != kWaterAwake) return ((timeStepCount == 0) || (--timeStepCount > 0));
	}
	...
	for (machine v = minBlockCoord.y; v <= maxBlockCoord.y; v++)
	{
		for (machine u = minBlockCoord.x; u <= maxBlockCoord.x; u++)
		{
			...
			if (geometry)
			{
				...
				Integer2D base(u * dimension, v * dimension);
				if (slopeFlag) data->lineOffset = lineOffset - (base.y << 16) + lineSlope * base.x;
				else data->lineOffset = lineOffset - (base.x << 16) + lineSlope * base.y;
...
{
	if (category == kObjectWaterBlock) return (4);
	return (0);
}
...
void WaterBlock::Preprocess(void)
{
	...
	for (machine a = 0; a < geometryCount; a++) waterGeometryTable[a] = nullptr;
...
{
	if (((unsigned_int32) u < (unsigned_int32) blockSize.x) && ((unsigned_int32) v < (unsigned_int32) blockSize.y)) return (waterGeometryTable[v * blockSize.x + u]);
	return (nullptr);
}
...
WaterGeometryObject::WaterGeometryObject(const Integer2D& coord) : GeometryObject(kGeometryWater)
{
	...
	SetGeometryFlags(kGeometryShadowInhibit | kGeometryMarkingInhibit);
...
void WaterGeometryObject::Build(Geometry *geometry)
{
	...
	geometryLevel->AllocateStorage(vertexCount, 3, desc);
	
	float scale = blockObject->GetWaterFieldScale();
	Point2D *restrict position = geometryLevel->GetArray<Point2D>(kArrayVertex) - 1;
	...
	Vector4D *restrict normal = geometryLevel->GetArray<Vector4D>(kArrayNormal);
	for (machine a = 0; a < vertexCount; a++) normal[a].Set(0.0F, 0.0F, 1.0F, 0.0F);
...
void HorizonWaterGeometryObject::Build(Geometry *geometry)
{
	...
	if (waterDirection == kWaterDirectionEast)
	{
		...
		for (machine j = 0; j < boundaryCount; j++) (++position)->Set(x, (float) j * scale);
	...
	{
		for (machine j = 0; j < boundaryCount; j++) (++position)->Set(0.0F, (float) (boundaryCount - 1 - j) * scale);
	...
	else if (waterDirection == kWaterDirectionNorth)
	{
		...
		for (machine i = 0; i < boundaryCount; i++) (++position)->Set((float) (boundaryCount - 1 - i) * scale, y);
	...
	{
		for (machine i = 0; i < boundaryCount; i++) (++position)->Set((float) i * scale, 0.0F);
	...
	Point3D *restrict elevation = geometryLevel->GetArray<Point3D>(kArrayPosition1);
	Vector3D *restrict normal = geometryLevel->GetArray<Vector3D>(kArrayNormal);
	Vector3D *restrict tangent = geometryLevel->GetArray<Vector3D>(kArrayTangent);
...
WaterGeometry::WaterGeometry() :
		...
		waterMoveJob(&WaterMoveJob, this),
		waterUpdateJob(&WaterUpdateJob, &FinalizeUpdate, this),
		dynamicVertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		vertexBufferObserver(this, &WaterGeometry::InitVertexBuffer)
{
	blockNode = nullptr;
	
	waterState = kWaterAsleep;
	waterMultiplier = 0.0F;
	for (machine a = 0; a < 4; a++) diagonalMultiplier[a] = 0.0F;
...
{
	if (!fieldStorage) AllocateFieldStorage();
...
void WaterGeometry::Preprocess(void)
{
	...
	if ((node) && (node->GetNodeType() == kNodeWaterBlock))
	{
		...
		if (waterController)
		{
			...
			{
				SetDynamicArrayFlags(1 << kArrayNormal);
				dynamicVertexBuffer.Initialize(waterVertexCount * sizeof(Vector4D), sizeof(Vector4D), &vertexBufferObserver);
				SetVertexBuffer(kVertexBufferDynamicArray, &dynamicVertexBuffer);
			}
			
			SetAttributeArray(kArrayNormal, GetObject()->GetGeometryLevel(0)->GetArray<Vector4D>(kArrayNormal));
			SetAttributeOffset(kArrayNormal, 0);
			...
			{
				if ((!diagonalWater[kWaterDirectionSouthwest]) && (adjacentWater[kWaterDirectionWest])) deadCornerPosition[deadCount++] = 0;
				if ((!diagonalWater[kWaterDirectionSoutheast]) && (adjacentWater[kWaterDirectionEast])) deadCornerPosition[deadCount++] = dimension;
			}
			
			if (adjacentWater[kWaterDirectionNorth])
			{
				if ((!diagonalWater[kWaterDirectionNorthwest]) && (adjacentWater[kWaterDirectionWest])) deadCornerPosition[deadCount++] = dimension * waterFieldWidth;
				if ((!diagonalWater[kWaterDirectionNortheast]) && (adjacentWater[kWaterDirectionEast])) deadCornerPosition[deadCount++] = dimension * waterFieldWidth + dimension;
			}
			
			deadCornerCount = deadCount;
			
			unsigned_int32 state = waterState;
			if (state == kWaterAwake) SetWaterMultiplier(1.0F);
			else if (state == kWaterDecaying) SetWaterMultiplier(waterMultiplier);
		}
	}
	...
	if (materialObject) SetTexcoordParameterPointer(&materialObject->GetTexcoordGeneration());
	else SetNullTexcoordParameterPointer();
}

void WaterGeometry::InitVertexBuffer(VertexBuffer *vertexBuffer)
{
	int32 vertexCount = waterVertexCount;
	Vector4D *restrict normal = static_cast<Vector4D *>(vertexBuffer->BeginUpdate());
	for (machine a = 0; a < vertexCount; a++) normal[a].Set(0.0F, 0.0F, 1.0F, 0.0F);
...
void WaterGeometry::SetWaterMultiplier(float multiplier)
{
	...
	for (machine a = 0; a < 4; a++)
	{
		...
		if (diagonal) diagonal->diagonalMultiplier[a ^ 3] = multiplier;
	}
}

void WaterGeometry::WaterMoveJob(Job *job, void *cookie)
{
	...
	for (machine a = 0; a < count; a++) elevation2[geometry->deadCornerPosition[a]] = 0.0F;
}

void WaterGeometry::WaterUpdateJob(Job *job, void *cookie)
{
	WaterGeometry *geometry = static_cast<WaterGeometry *>(cookie);
...
void WaterGeometry::FinalizeUpdate(Job *job, void *cookie)
{
	WaterGeometry *geometry = static_cast<WaterGeometry *>(cookie);
...
void HorizonWaterGeometry::Preprocess(void)
{
	...
	if (object) SetTexcoordParameterPointer(&object->GetTexcoordGeneration());
	else SetNullTexcoordParameterPointer();
}
...
bool WaterController::UnpackChunk(const ChunkHeader *chunkHeader, Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (chunkHeader->chunkType)
	{
		...
		case 'WGEN':
		{
			WaveGenerator *generator = WaveGenerator::Construct(data, unpackFlags);
			if (generator)
			{
...
void WaterController::SetSetting(const Setting *setting)
{
	...
	else if (identifier == 'RWAV')
	{
		...
		if (b) waterFlags |= kWaterRandomWaves;
		else waterFlags &= ~kWaterRandomWaves;
	}
...
void WaterController::Preprocess(void)
{
	...
	{
		if (waterFlags & kWaterRandomWaves) generatorList.Append(&randomWaveGenerator);
		...
		while (node)
		{
			if (node->GetNodeType() == kNodeGeometry)
			{
				...
				if (geometry->GetGeometryType() == kGeometryWater)
				{
					...
					if (state == kWaterAwake) moveList.Append(waterGeometry);
					else if (state == kWaterDecaying) decayList.Append(waterGeometry);
				}
			}
...
void WaterController::Advance(int32 dt)
{
	...
	while (generator)
	{
		...
		if (generator->GetWaveGeneratorFlags() & kWaveGeneratorUpdate) generator->Update(this);
		
		if (!generator->Execute(this))
		{
			generatorList.Remove(generator);
			if (generator->GetWaveGeneratorFlags() & kWaveGeneratorNonpersistent) delete generator;
		}
	...
	switch (chunkHeader->chunkType)
	{
		case 'PCON':
			...
			case 'CKEY':
			{
				...
				data >> key1;
				data >> key2;
				connectorKey[0] = Text::TypeToString(key1);
				connectorKey[1] = Text::TypeToString(key2);
				return (true);
			}
