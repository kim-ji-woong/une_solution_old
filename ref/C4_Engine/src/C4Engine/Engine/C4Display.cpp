...
DisplayMode::~DisplayMode()
{
	...
	C4::Display::Display(const char *name)
	{
		...
		for (DWORD modeNum = 0;; modeNum++)
		{
			...
			if (!EnumDisplaySettingsA(name, modeNum, &devMode)) break;
			
			int32 width = devMode.dmPelsWidth;
			int32 height = devMode.dmPelsHeight;
			if ((width >= 640) && (height >= 480) && (width >= height) && (devMode.dmBitsPerPel >= 32))
			{
				int32 frequency = 0;
				if (devMode.dmFields & DM_DISPLAYFREQUENCY)
				{
					frequency = devMode.dmDisplayFrequency;
					if ((frequency < 60) || (frequency > 75)) frequency = 0;
				}
				...
				while (displayMode)
				{
					...
					{
						if (frequency <= displayMode->GetDisplayFrequency()) goto next; 
						delete displayMode;
						break; 
					} 
	...
	C4::Display::Display(CGDirectDisplayID id)
	{
		...
		for (machine a = 0; a < count; a++)
		{
			...
			if (CFGetTypeID(value) == CGDisplayModeGetTypeID())
			{
				...
				if ((width >= 640) && (height >= 480) && (width >= height) && (CFStringCompare(encoding, CFSTR(IO32BitDirectPixels), 0) == kCFCompareEqualTo))
				{
...
const DisplayMode *C4::Display::FindDisplayMode(int32 width, int32 height) const
{
	...
	while (mode)
	{
		if ((mode->GetDisplayWidth() == width) && (mode->GetDisplayHeight() == height)) break;
		mode = mode->Next();
	}
...
EngineResult DisplayMgr::Construct(void)
{
		...
		for (DWORD devNum = 0;; devNum++)
		{
			...
			if (!EnumDisplayDevicesA(nullptr, devNum, &device, 0)) break;
			
			Display *display = new Display(device.DeviceName);
			if (device.StateFlags & DISPLAY_DEVICE_PRIMARY_DEVICE) displayList.Prepend(display);
			else displayList.Append(display);
		}
		...
		for (machine a = 0; a < count; a++)
		{
			...
			if (a == primary) displayList.Prepend(display);
			else displayList.Append(display);
		}
	...
	if (!currentDisplay) return (kDisplayInitFailed);
	...
	Variable *samplesVar = TheEngine->InitVariable("displaySamples", "1", kVariablePermanent);
	...
	if (syncVar->GetIntegerValue() != 0)
	{
		...
		if (tearVar->GetIntegerValue() != 0) flags |= kDisplaySyncTear;
	}
...
void DisplayMgr::Destruct(void)
{
	...
	ShowCursor();
		...
		if (displayWindow) DestroyWindow(displayWindow);
		if (displayChanged) ChangeDisplaySettingsA(nullptr, 0);
		...
		{
			TerminateWindow();
			DisposeWindow(displayWindow);
		}
		
		if (displayChanged)
		{
			CGDisplayRelease(currentDisplay->GetDisplayID());
	...
	void DisplayMgr::GlobalToLocalPoint(HIPoint *point) const
	{
		if (displayFlags & kDisplayFullscreen)
		{
			CGRect rect = CGDisplayBounds(currentDisplay->GetDisplayID());
			point->x -= rect.origin.x;
			point->y -= rect.origin.y;
		}
		else
		{
			HIPointConvert(point, kHICoordSpaceScreenPixel, nullptr, kHICoordSpaceWindow, displayWindow);
		}
	}
...
EngineResult DisplayMgr::SetDisplayMode(int32 width, int32 height, int32 samples, unsigned_int32 flags)
{
		...
		//if ((flags & kDisplayFullscreen) && (!mode)) return (kDisplayInitFailed);
		...
		//ShowCursor();
			...
			int borderHeight = GetSystemMetrics(SM_CYFIXEDFRAME);
			int captionHeight = GetSystemMetrics(SM_CYCAPTION);
			
			HWND mainWindow = TheEngine->GetWindow();
			...
			if (flags & kDisplayFullscreen)
			{
				...
				if (ChangeDisplaySettingsA(&devMode, CDS_FULLSCREEN) != DISP_CHANGE_SUCCESSFUL) return (kDisplayInitFailed);
				displayChanged = true;
			...
			else
			{
				...
				//SetWindowPos(mainWindow, HWND_NOTOPMOST, screenWidth / 2 - width / 2 - borderWidth, screenHeight / 2 - height / 2 - borderHeight, width + borderWidth * 2, height + borderHeight * 2 + captionHeight, SWP_NOCOPYBITS);
				//SetWindowLongPtrA(mainWindow, GWL_STYLE, WS_CLIPCHILDREN );//| WS_BORDER | WS_CAPTION | WS_MINIMIZEBOX | WS_SYSMENU);
				//SetWindowPos(mainWindow, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_FRAMECHANGED | SWP_SHOWWINDOW);
				//SetWindowPos(mainWindow, HWND_NOTOPMOST, 0, 0, 0, 0, 0 );
			}
			
			//displayWindow = CreateWindowExW(0, L"C4", L"C4", WS_CHILD | WS_VISIBLE, 0, 0, width, height, mainWindow, nullptr, TheEngine->GetInstance(), nullptr);
			...
			CGDirectDisplayID displayID = currentDisplay->GetDisplayID();
			DMGetGDeviceByDisplayID((DisplayIDType) displayID, &displayDevice, false);
			
			if (flags & kDisplayFullscreen)
			{
				CGDisplayCapture(displayID);
				
				if (CGDisplaySetDisplayMode(displayID, mode->GetDisplayMode(), nullptr) != kCGErrorSuccess)
				{
			...
			else
			{
				...
				int32 x = ((int32) bounds.size.width >> 1) - (width >> 1);
				int32 y = ((int32) bounds.size.height >> 1) - (height >> 1);
				SetRect(&rect, x, y, x + width, y + height);
				
				CreateNewWindow(kDocumentWindowClass, kWindowCloseBoxAttribute | kWindowCollapseBoxAttribute, &rect, &displayWindow);
				
				RGBColor black = {0, 0, 0};
				SetWindowContentColor(displayWindow, &black);
				...
				SetWindowTitleWithCFString(displayWindow, string);
				CFRelease(string);
				
				ShowWindow(displayWindow);
				displayPort = GetWindowPort(displayWindow);
			...
			if (flags & kDisplayFullscreen)
			{
				if (XRRSetScreenConfig(engineDisplay, screenConfig, rootWindow, mode->GetSizeIndex(), RR_Rotate_0, CurrentTime) != 0) return (kDisplayInitFailed);
				displayChanged = true;
			...
			if (flags & kDisplayFullscreen)
			{
				XSetInputFocus(engineDisplay, PointerRoot, RevertToPointerRoot, CurrentTime);
			}
	...
	OSStatus DisplayMgr::WindowEventHandler(EventHandlerCallRef handlerCallRef, EventRef event, void *userData)
	{
		UInt32 kind = GetEventKind(event);
		if (kind == kEventWindowClickDragRgn)
		{
			CallNextEventHandler(handlerCallRef, event);
			TheGraphicsMgr->UpdateOpenGLContext();
		}
		
		return (noErr);
	}
	
	void DisplayMgr::InitializeWindow(void)
	{
		EventTypeSpec	eventList[1];
		
		EventTargetRef windowTarget = GetWindowEventTarget(displayWindow);
		InstallStandardEventHandler(windowTarget);
		
		eventList[0].eventClass = kEventClassWindow;
		eventList[0].eventKind = kEventWindowClickDragRgn;
		
		windowEventHandlerUPP = NewEventHandlerUPP(&WindowEventHandler);
		InstallEventHandler(windowTarget, windowEventHandlerUPP, 1, eventList, this, &windowEventHandlerRef);
	}
	
	void DisplayMgr::TerminateWindow(void)
	{
