...
PathComponent *PathComponent::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kPathLinear:
...
Point3D EllipticalPathComponent::GetPosition(float t) const
{
	...
	Vector3D tangent = (controlPoint[2] - controlPoint[0]).Normalize();
	Vector3D tx = ProjectOnto(dp, tangent);
	Vector3D ty = dp - tx;
	
	Vector2D v = CosSin(t * K::pi_over_2);
...
bool Path::UnpackChunk(const ChunkHeader *chunkHeader, Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (chunkHeader->chunkType)
	{
		...
		case 'COMP':
		{
			PathComponent *component = PathComponent::Construct(data, unpackFlags);
			if (component)
			{
...
Point3D Path::GetPathState(float t, Vector3D *tangent, Vector3D *binormal) const
{
	...
	if (tangent)
	{
		*tangent = component->GetTangent(t);
		if (binormal) *binormal = *tangent % pathNormal;
	}
...
bool Path::LinearPath(void) const
{
	...
	while (component)
	{
		if (component->GetPathType() != kPathLinear) return (false);
		component = component->Next();
	}
