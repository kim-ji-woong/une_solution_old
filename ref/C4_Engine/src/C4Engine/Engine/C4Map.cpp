...
{
	if (owningMap) owningMap->RemoveNode(this);
}
...
{
	if (leftSubnode) return (leftSubnode->Last());
	...
	for (;;)
	{
		...
		if (super->rightSubnode == element) return (super);
		element = super;
	}
...
{
	if (rightSubnode) return (rightSubnode->First());
	...
	for (;;)
	{
		...
		if (super->leftSubnode == element) return (super);
		element = super;
	}
...
{
	if (leftSubnode) leftSubnode->RemoveSubtree();
	if (rightSubnode) rightSubnode->RemoveSubtree();
...
{
	if (leftSubnode) leftSubnode->PurgeSubtree();
	if (rightSubnode) rightSubnode->PurgeSubtree();
...
MapElementBase *MapBase::RotateLeft(MapElementBase *node)
{
	...
	if (node != rootNode)
	{
		...
		if (super->leftSubnode == node) super->leftSubnode = right;
		else super->rightSubnode = right;
	...
	if (subnode) subnode->superNode = node;
...
MapElementBase *MapBase::RotateRight(MapElementBase *node)
{
	...
	{
		MapElementBase *super = node->superNode;
		if (super->leftSubnode == node) super->leftSubnode = left;
		else super->rightSubnode = left;
...
MapElementBase *MapBase::ZigZagLeft(MapElementBase *node)
{
	...
	{
		MapElementBase *super = node->superNode;
		if (super->leftSubnode == node) super->leftSubnode = top;
		else super->rightSubnode = top;
		top->superNode = super;
	}
	else
	{
	...
	if (subLeft) subLeft->superNode = node;
	node->rightSubnode = subLeft;
	
	MapElementBase *subRight = top->rightSubnode;
	if (subRight) subRight->superNode = right;
...
MapElementBase *MapBase::ZigZagRight(MapElementBase *node)
{
	...
	MapElementBase *subLeft = top->leftSubnode;
	if (subLeft) subLeft->superNode = left;
	left->rightSubnode = subLeft;
	
	MapElementBase *subRight = top->rightSubnode;
	if (subRight) subRight->superNode = node;
...
void MapBase::ReplaceNode(MapElementBase *node, MapElementBase *element)
{
	...
	if (map) map->RemoveNode(element);
	
	element->superNode = node->superNode;
	element->leftSubnode = node->leftSubnode;
	element->rightSubnode = node->rightSubnode;
...
{
	MapElementBase *super = node->superNode;
	if (subnode) subnode->superNode = super;
...
void MapBase::RemoveNode(MapElementBase *node)
{
	...
	if ((left) && (right))
	{
		...
		if (left) left->superNode = top;
		top->leftSubnode = left;
		
		right = node->rightSubnode;
		if (right) right->superNode = top;
		top->rightSubnode = right;
