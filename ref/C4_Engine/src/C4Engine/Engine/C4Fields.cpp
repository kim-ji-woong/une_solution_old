...
FieldObject *FieldObject::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kFieldBox:
...
{
	if (category == kObjectField) return (2);
	return (0);
}
...
void FieldObject::SetCategorySetting(Type category, const Setting *setting)
{
	if (category == kObjectField)
	{
		...
		if (identifier == 'EXCL')
		{
			...
			if (b) fieldFlags |= kFieldExclusive;
			else fieldFlags &= ~kFieldExclusive;
		}
	}
}
...
Field::~Field()
{
	...
	if (zone) zone->RemoveField(this);
	
	delete fieldForce;
}

Field *Field::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		case kFieldBox:
...
bool Field::UnpackChunk(const ChunkHeader *chunkHeader, Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (chunkHeader->chunkType)
	{
		case 'FORC':
			...
			Force *force = Force::Construct(data, unpackFlags);
			if (force)
			{
...
void Field::CalculatePostTransform(void)
{
...
{
	Node::Preprocess();
	if (fieldForce) fieldForce->Preprocess();
}

void Field::EnterZone(Zone *zone)
{
	if ((fieldForce) && (!(GetObject()->GetFieldFlags() & kFieldExclusive))) zone->AddField(this);
}

void Field::ExitZone(Zone *zone)
{
	zone->RemoveField(this);
}
...
void Field::SetForce(Force *force)
{
	if (fieldForce != force)
	{
		if (fieldForce) fieldForce->SetTargetField(nullptr);
		if (force) force->SetTargetField(this);
		fieldForce = force;
	}
}
