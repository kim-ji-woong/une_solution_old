...
namespace
{
	...
	const int16 blockMatrix[kAudioMessageBlockSize * kAudioMessageBlockSize] =
	{
	...
	const unsigned_int8 blockNormalizer[kAudioMessageBlockSize] =
	{
		20, 20, 20, 20, 21, 21, 21, 21, 22, 22, 23, 23, 24, 24, 24, 24
	};
	
	const unsigned_int8 blockDenormalizer[kAudioMessageBlockSize] =
	{
...
AudioMessage::AudioMessage(unsigned_int32 time, int32 count, const Sample *data) : Message(kMessageAudio, kMessageUnreliable | kMessageUnordered)
{
	...
	{
		int32 y = data[0];
		for (machine i = 1; i < kAudioMessageBlockSize; i++) y += data[i];
		output[1] = (int8) Min(Max(y >> 10, -128), 127);
		
		for (machine j = 1; j < kAudioMessageBlockSize; j++)
		{
			y = data[0] * matrix[j];
			for (machine i = 1; i < kAudioMessageBlockSize; i++) y += data[i] * matrix[i * kAudioMessageBlockSize + j];
			output[j + 1] = (int8) Min(Max(y >> blockNormalizer[j], -128), 127);
		}
		
		unsigned_int32 size = 1;
		for (machine j = kAudioMessageBlockSize; j > 1; j--)
		{
			if (Abs(output[j]) > 1)
			{
				size = j;
	...
	audioDataSize = output - audioData;
} 
...
bool AudioMessage::HandleMessage(Player *sender) const
{
		...
		while (channel)
		{
			if (channel->Enabled())
			{
				...
				else
				{
					...
					if (!streamer) streamer = localPlayer->ConstructChatStreamer();
	...
	else
	{
		...
		if ((player) && (player != localPlayer))
		{
			...
			if (!streamer) streamer = player->ConstructChatStreamer();
			streamer->ReceiveAudioMessage(this);
		}
	}
...
unsigned_int32 AudioMessage::GetAudioData(Sample *data, unsigned_int32 offset) const
{
	...
	for (machine a = 0; a < count; a++)
	{
		...
		{
			int32 x = input[0] << 10;
			for (machine j = 1; j < size; j++) x += (input[j] << blockDenormalizer[j]) * matrix[i * kAudioMessageBlockSize + j];
...
SoundResult ChatStreamer::StartStream(void)
{
	...
	SetChannelCount(1);
	SetSampleRate(kSoundInputSampleRate);
...
bool ChatStreamer::FillBuffer(unsigned_int32 bufferSize, Sample *buffer, int32 *count)
{
	...
	*count = frameCount;
	...
	if ((int32) (playTimeStamp - dataTimeStamp) >= kChatRingBufferFrameCount * 4) Pause();
	return (true);
}
...
void ChatStreamer::ReceiveAudioMessage(const AudioMessage *message)
{
	...
	else
	{
		...
		if ((int32) (timeStamp - dataTimeStamp) > 0) dataTimeStamp = timeStamp;
	}
...
EngineResult AudioCaptureMgr::Construct(void)
{
			...
			for (machine a = 0; a < 3; a++) captureEvent[a] = CreateEventA(nullptr, false, false, nullptr);
			...
			if (AudioObjectGetPropertyData(kAudioObjectSystemObject, &propertyAddress, 0, nullptr, &size, &inputDeviceID) != noErr) return (kAudioCaptureOkay);
			...
			Component component = FindNextComponent(nullptr, &componentDescription);
			if ((!component) || (OpenAComponent(component, &audioUnit) != noErr))
			{
				audioUnit = nullptr;
				return (kAudioCaptureOkay);
			}
...
{
	if (audioBuffer) StopAudioCapture();
			...
			if (directSoundCapture)
			{
				...
				for (machine a = 2; a >= 0; a--) CloseHandle(captureEvent[a]);
			...
			if (audioUnit) CloseComponent(audioUnit);
		...
		OSStatus AudioCaptureMgr::CaptureCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData)
		{
			...
			if (inNumberFrames > audioCaptureMgr->inputBufferFrameCount) return (kAudioUnitErr_TooManyFramesToProcess);
		...
		void AudioCaptureMgr::CaptureThread(const Thread *thread, void *cookie)
		{
			...
			for (machine index = 0;; index ^= 1)
			{
				...
				if (audioCaptureMgr->captureExitFlag) break;
...
AudioCaptureResult AudioCaptureMgr::StartAudioCapture(bool paused)
{
			...
			if (!directSoundCapture) return (kAudioCaptureUnavailable);
		
		#elif C4MACOS
		
			if (!audioUnit) return (kAudioCaptureUnavailable);
		
		#elif C4LINUX
		
			if (!captureHandle) return (kAudioCaptureUnavailable);
		...
		if (audioCaptureState == 0)
		{
			...
			audioBuffer = new Sample[kCaptureBufferFrameCount * 2];
				...
				if (!paused) captureBuffer->Start(DSCBSTART_LOOPING);
				...
				audioBufferList.mBuffers[0].mDataByteSize = inputBufferFrameCount * 4;
				audioBufferList.mBuffers[0].mData = inputBuffer;
				
				audioWritePosition = 0;
				if (!paused) AudioOutputUnitStart(audioUnit);
				...
				if (!paused) snd_pcm_start(captureHandle);
...
unsigned_int32 AudioCaptureMgr::GetChatReceiveRate(void) const
{
	...
	for (machine a = 1; a < kAudioInfoFrameCount; a++) size += chatReceiveInfo[a].size;
...
void AudioCaptureMgr::AudioCaptureTask(void)
{
	...
	for (machine a = 0; a < 2; a++)
	{
		if (audioAvailable[parity])
		{
			...
			{
				for (unsigned_machine frame = 0; frame < kCaptureBufferFrameCount; frame += kAudioMessageFrameCount)
				{
			...
			if (audioCaptureProc) (*audioCaptureProc)(timeStamp, buffer, kCaptureBufferFrameCount, audioCaptureCookie);
