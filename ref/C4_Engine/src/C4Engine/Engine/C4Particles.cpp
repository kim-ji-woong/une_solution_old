...
ParticleSystem::ParticleSystem(ParticleSystemType type, ParticlePoolBase *pool, RenderType renderType, unsigned_int32 renderState) : Effect(kEffectParticleSystem, renderType, renderState)
{
...
ParticleSystem::ParticleSystem(const ParticleSystem& particleSystem, ParticlePoolBase *pool) : Effect(particleSystem)
{
...
ParticleSystem *ParticleSystem::New(ParticleSystemType type)
{
	...
	return (Registrable<ParticleSystem, ParticleSystemRegistration>::Construct(unpacker));
}
...
void ParticleSystem::Pack(Packer& data, unsigned_int32 packFlags) const
{
	...
	if (!(particleSystemFlags & kParticleSystemNonpersistent))
	{
		int32 count = 0;
...
bool ParticleSystem::UnpackChunk(const ChunkHeader *chunkHeader, Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (chunkHeader->chunkType)
	{
		...
		case 'TIME':
			...
			case 'LINK':
			{
				...
				data >> nodeIndex;
				data.AddNodeLink(nodeIndex, &EmitterLinkProc, this);
				return (true);
			}
	...
	void ParticleSystem::EmitterLinkProc(Node *node, void *cookie)
	{
		ParticleSystem *effect = static_cast<ParticleSystem *>(cookie);
		effect->SetConnectedEmitter(static_cast<Emitter *>(node));
	}
...
{
	if (index == 0) return (kConnectorKeyEmitter);
	return (nullptr);
}
...
{
	if (key == kConnectorKeyEmitter) return (node->GetNodeType() == kNodeEmitter);
...
void ParticleSystem::SetConnectedEmitter(Emitter *emitter)
{
	...
	if (emitter)
	{
		...
		if (hub)
		{
			Connector *connector = hub->FindOutgoingConnector(kConnectorKeyEmitter);
			if (connector)
			{
				connector->SetConnectorTarget(emitter);
				return;
			}
		}
		
		AddConnector(kConnectorKeyEmitter, emitter);
	}
	else
	{
		RemoveConnector(kConnectorKeyEmitter);
	}
}
...
void ParticleSystem::Preprocess(void)
{
	...
	if (particleSystemFlags & kParticleSystemSoftDepth) attributeList.Append(&deltaDepthAttribute);
}

void ParticleSystem::Neutralize(void)
{
	delete[] arrayStorage;
	arrayStorage = nullptr;
...
void *ParticleSystem::AllocateArrayStorage(unsigned_int32 size)
{
	delete[] arrayStorage;
	arrayStorage = new char[size];
	return (arrayStorage);
}
...
void ParticleSystem::Move(void)
{
	...
	if ((renderFlag) || (!(flags & kParticleSystemFreezeInvisible)) || ((flags & ~(((machine) GetCurrentUpdateFlags() - 1) >> 31)) & kParticleSystemUnfreezeDynamic))
	{
		AnimateParticles();
		...
		if (particle)
		{
				...
				float4 min = SimdLoadUnaligned(&particle->position.x);
				float4 max = min;
				
				float4 r = SimdLoadSmearScalar(&particle->radius);
				min = SimdSub(min, r);
				max = SimdAdd(max, r);
				...
				for (;;)
				{
					...
					float4 p = SimdLoadUnaligned(&particle->position.x);
					r = SimdLoadSmearScalar(&particle->radius);
					
					min = SimdMin(min, SimdSub(p, r));
					max = SimdMax(max, SimdAdd(p, r));
				}
	...
	if (!renderFlag)
	{
		invisibleTime += TheTimeMgr->GetDeltaTime();
	}
	else
	{
		invisibleTime = 0;
...
void ParticleSystem::FreeParticle(Particle *particle)
{
	...
	if (firstUsedParticle == particle) firstUsedParticle = next;
	if (lastUsedParticle == particle) lastUsedParticle = prev;
	
	if (next) next->prevParticle = prev;
	if (prev) prev->nextParticle = next;
...
PointParticleSystem::PointParticleSystem(ParticleSystemType type, ParticlePoolBase *pool, const char *textureName) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		vertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(textureName, &particleTextureHeader, particleTextureImage)
{
}
...
PointParticleSystem::PointParticleSystem(const PointParticleSystem& pointParticleSystem, ParticlePoolBase *pool) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		vertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(pointParticleSystem.textureMap)
{
}
...
void PointParticleSystem::Preprocess(void)
{
	...
	AddAttribute(&textureMap);
	
	if ((GetParticleSystemFlags() & kParticleSystemPointSprite) && (TheGraphicsMgr->GetCapabilities()->extensionFlag[kExtensionPointSprite]))
	{
		spriteFlag = true;
		SetRenderType(kRenderPoints);
		
		int32 vertexCount = GetTotalParticleCount();
		vertexBuffer.Initialize(vertexCount * sizeof(SpriteVertex), sizeof(SpriteVertex));
		
		SetAttributeOffset(kArrayVertex, 0, 3);
		SetAttributeOffset(kArrayColor0, sizeof(Point3D), 4);
		SetAttributeOffset(kArrayRadius, sizeof(Point3D) + sizeof(ColorRGBA), 1);
	}
	else
	{
		spriteFlag = false;
		...
		vertexBuffer.Initialize(vertexCount * sizeof(PointVertex), sizeof(PointVertex));
		
		SetAttributeOffset(kArrayVertex, 0, 3);
		SetAttributeOffset(kArrayColor0, sizeof(Point3D), 4);
		SetAttributeOffset(kArrayTexture0, sizeof(Point3D) + sizeof(ColorRGBA), 2);
		SetAttributeOffset(kArrayBillboard, sizeof(Point3D) + sizeof(ColorRGBA) + sizeof(Point2D), 4);
	}
	
	SetVertexBuffer(kVertexBufferStaticArray, &vertexBuffer);
}

void PointParticleSystem::RenderParticles(Job *job, void *cookie)
{
	...
	{
		SpriteVertex *restrict vertex = static_cast<SpriteVertex *>(particleJob->attributeBuffer);
	...
	{
		PointVertex *restrict vertex = static_cast<PointVertex *>(particleJob->attributeBuffer);
...
void PointParticleSystem::FinalizeParticles(Job *job, void *cookie)
{
	...
	if (particleSystem->GetVertexCount() == 0) particleSystem->ListElement<Renderable>::GetOwningList()->Remove(particleSystem);
}

void PointParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
...
TexcoordPointParticleSystem::TexcoordPointParticleSystem(const TexcoordPointParticleSystem& texcoordPointParticleSystem, ParticlePoolBase *pool) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		vertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(texcoordPointParticleSystem.textureMap)
{
}
...
void TexcoordPointParticleSystem::RenderParticles(Job *job, void *cookie)
{
	const ParticleJob *particleJob = static_cast<ParticleJob *>(job);
...
void TexcoordPointParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
	renderJob.attributeBuffer = vertexBuffer.BeginUpdate();
	GetWorld()->SubmitWorldJob(&renderJob);
...
InfinitePointParticleSystem::InfinitePointParticleSystem(const InfinitePointParticleSystem& infinitePointParticleSystem, ParticlePoolBase *pool) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		vertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(infinitePointParticleSystem.textureMap)
{
}
...
void InfinitePointParticleSystem::Preprocess(void)
{
	...
	else
	{
		...
		SetAttributeOffset(kArrayColor0, sizeof(Point3D), 4);
		SetAttributeOffset(kArrayTexture0, sizeof(Point3D) + sizeof(ColorRGBA), 2);
		SetAttributeOffset(kArrayBillboard, sizeof(Point3D) + sizeof(ColorRGBA) + sizeof(Point2D), 2);
	}
	
	SetVertexBuffer(kVertexBufferStaticArray, &vertexBuffer);
}

void InfinitePointParticleSystem::RenderParticles(Job *job, void *cookie)
{
	const ParticleJob *particleJob = static_cast<ParticleJob *>(job);
...
void InfinitePointParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
	renderJob.attributeBuffer = vertexBuffer.BeginUpdate();
	GetWorld()->SubmitWorldJob(&renderJob);
...
LineParticleSystem::LineParticleSystem(const LineParticleSystem& lineParticleSystem, ParticlePoolBase *pool) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		attributeVertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(lineParticleSystem.textureMap)
{
...
void LineParticleSystem::Preprocess(void)
{
	...
	int32 vertexCount = GetTotalParticleCount() * 4;
	attributeVertexBuffer.Initialize(vertexCount * sizeof(LineVertex), sizeof(LineVertex));
	SetVertexBuffer(kVertexBufferStaticArray, &attributeVertexBuffer);
	
	SetAttributeOffset(kArrayVertex, 0, 3);
	SetAttributeOffset(kArrayColor0, sizeof(Point3D), 4);
	SetAttributeOffset(kArrayTangent, sizeof(Point3D) + sizeof(ColorRGBA), 4);
	SetAttributeOffset(kArrayTexture0, sizeof(Point3D) + sizeof(ColorRGBA) + sizeof(Vector4D), 2);
	
	SetShaderFlags(GetShaderFlags() | (kShaderVertexPolyboard | kShaderLinearPolyboard));
}

void LineParticleSystem::RenderParticles(Job *job, void *cookie)
{
	const ParticleJob *particleJob = static_cast<ParticleJob *>(job);
	...
	int32 vertexCount = 0;
	LineVertex *restrict vertex = static_cast<LineVertex *>(particleJob->attributeBuffer);
...
void LineParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
...
QuadParticleSystem::QuadParticleSystem(const QuadParticleSystem& quadParticleSystem, ParticlePoolBase *pool) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		vertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(quadParticleSystem.textureMap)
{
}
...
void QuadParticleSystem::Preprocess(void)
{
	...
	int32 vertexCount = GetTotalParticleCount() * 4;
	vertexBuffer.Initialize(vertexCount * sizeof(QuadVertex), sizeof(QuadVertex));
	SetVertexBuffer(kVertexBufferStaticArray, &vertexBuffer);
...
void QuadParticleSystem::RenderParticles(Job *job, void *cookie)
{
	const ParticleJob *particleJob = static_cast<ParticleJob *>(job);
	...
	int32 vertexCount = 0;
	QuadVertex *restrict vertex = static_cast<QuadVertex *>(particleJob->attributeBuffer);
...
void QuadParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
	renderJob.attributeBuffer = vertexBuffer.BeginUpdate();
	GetWorld()->SubmitWorldJob(&renderJob);
...
FireParticleSystem::FireParticleSystem(const FireParticleSystem& fireParticleSystem, ParticlePoolBase *pool) :
		...
		renderJob(&RenderParticles, &FinalizeParticles, this),
		vertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		textureMap(fireParticleSystem.textureMap),
		noiseMap("C4/noise")
{
}
...
void FireParticleSystem::Preprocess(void)
{
	ParticleSystem::Preprocess();
	AddAttribute(&textureMap);
	AddAttribute(&noiseMap);
	
	int32 vertexCount = GetTotalParticleCount() * 4;
	vertexBuffer.Initialize(vertexCount * sizeof(FireVertex), sizeof(FireVertex));
	SetVertexBuffer(kVertexBufferStaticArray, &vertexBuffer);
	
	SetAttributeOffset(kArrayVertex, 0, 3);
	SetAttributeOffset(kArrayColor0, sizeof(Point3D), 4);
	SetAttributeOffset(kArrayRadius, sizeof(Point3D) + sizeof(ColorRGBA), 4);
	SetAttributeOffset(kArrayTexture0, sizeof(Point3D) + sizeof(ColorRGBA) + sizeof(Vector4D), 4);
	SetAttributeOffset(kArrayTexture1, sizeof(Point3D) + sizeof(ColorRGBA) + sizeof(Vector4D) * 2, 4);
	
	SetShaderFlags(GetShaderFlags() | (kShaderFireArrays | kShaderVertexPostboard));
}

void FireParticleSystem::RenderParticles(Job *job, void *cookie)
{
	const ParticleJob *particleJob = static_cast<ParticleJob *>(job);
	...
	int32 vertexCount = 0;
	FireVertex *restrict vertex = static_cast<FireVertex *>(particleJob->attributeBuffer);
...
void FireParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
	renderJob.attributeBuffer = vertexBuffer.BeginUpdate();
	GetWorld()->SubmitWorldJob(&renderJob);
...
PolyboardParticleSystem::PolyboardParticleSystem(const PolyboardParticleSystem& polyboardParticleSystem, ParticlePoolBase *pool) :
		...
		attributeVertexBuffer(kVertexBufferAttribute | kVertexBufferDynamic),
		indexVertexBuffer(kVertexBufferIndex | kVertexBufferDynamic),
		textureMap(polyboardParticleSystem.textureMap)
{
}
...
void PolyboardParticleSystem::Preprocess(void)
{
	...
	attributeVertexBuffer.Initialize(vertexCount * sizeof(PolyVertex), sizeof(PolyVertex));
	indexVertexBuffer.Initialize(triangleCount * sizeof(Triangle), 0);
	SetVertexBuffer(kVertexBufferStaticArray, &attributeVertexBuffer);
...
void PolyboardParticleSystem::RenderParticles(Job *job, void *cookie)
{
	const ParticleJob *particleJob = static_cast<ParticleJob *>(job);
	...
	PolyVertex *restrict vertex = static_cast<PolyVertex *>(particleJob->attributeBuffer);
	Triangle *restrict triangle = static_cast<Triangle *>(particleJob->indexBuffer);
	...
	particleSystem->SetVertexCount(vertexCount);
	particleSystem->SetFaceCount(triangleCount);
}
...
void PolyboardParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	renderJob.camera = static_cast<const FrustumCamera *>(camera);
	renderJob.attributeBuffer = attributeVertexBuffer.BeginUpdate();
...
BlobParticleSystem::~BlobParticleSystem()
{
	...
	{
		int32 count = GetTotalParticleCount();
		for (machine a = count - 1; a >= 0; a--) jobArray[a].~BlobJob();
...
void BlobParticleSystem::Preprocess(void)
{
	...
	if (!GetManipulator())
	{
		...
		Assert(maxParticleVertexCount * particleCount < 65535, "The ratio of maxRadius to scale is too large.");
		...
		for (machine a = 0; a < particleCount; a++) new(&jobArray[a]) BlobJob(&RenderBlob, this);
		...
		attributeVertexBuffer.Initialize(vertexCount * sizeof(BlobVertex), sizeof(BlobVertex));
		indexVertexBuffer.Initialize(triangleCount * sizeof(Triangle), 0);
		SetVertexBuffer(kVertexBufferStaticArray, &attributeVertexBuffer);
		SetVertexBuffer(kVertexBufferIndexArray, &indexVertexBuffer);
		
		SetAttributeOffset(kArrayVertex, 0, 3);
		SetAttributeOffset(kArrayNormal, sizeof(Point3D), 3);
		
		SetVertexCount(0);
		SetAmbientBlendState(kBlendReplace);
		SetShaderFlags((GetShaderFlags() | kShaderNormalizeBasisVectors) & ~kShaderAmbientEffect);
		segment->SetMaterialState(0);
	}
}
	...
	for (machine k = 0; k < cellCount.z; k++)
	{
		...
		for (machine j = 0; j < cellCount.y; j++)
		{
			...
			for (machine i = 0; i < cellCount.x; i++)
			{
				...
				if (mainDensity == 0)
				{
					if (globalVertexCount == blobJob->maxVertexCount) goto end;
					
					unsigned_int16 index = (unsigned_int16) globalVertexCount++;
					currentDeck[j * rowSize + i][0] = index;
					
					BlobVertex *vertex = &blobJob->vertexArray[index];
				...
				if ((code ^ ((mainDensity >> 7) & 0xFF)) != 0)
				{
					...
					if (globalTriangleCount + triangleCount > blobJob->maxTriangleCount) goto end;
					...
					for (machine a = 0; a < vertexCount; a++)
					{
						...
						if ((t & (kVoxelFixedUnit - 1)) != 0)
						{
							...
							if (deltaCode < 8)
							{
								if (deltaCode & 4) index = previousDeck[(j - ((deltaCode >> 1) & 1)) * rowSize + (i - (deltaCode & 1))][edgeIndex];
								else index = currentDeck[(j - (deltaCode >> 1)) * rowSize + (i - (deltaCode & 1))][edgeIndex];
							}
							else
							{
								if (globalVertexCount == blobJob->maxVertexCount) goto end;
								...
								if (v1 == 7) currentDeck[j * rowSize + i][edgeIndex] = index;
							}
						}
						else if (t == 0)
						{
							...
							else
							{
								...
								if (deltaCode & 4) index = previousDeck[(j - ((deltaCode >> 1) & 1)) * rowSize + (i - (deltaCode & 1))][0];
								else index = currentDeck[(j - (deltaCode >> 1)) * rowSize + (i - (deltaCode & 1))][0];
							}
						}
						else
						{
					...
					Triangle *restrict triangle = &blobJob->triangleArray[globalTriangleCount];
...
void BlobParticleSystem::SingleRenderVoxels(Voxel *voxelMap, int32 voxelMapSize, float center, float radius, const Vector3D& scaleAxis, float inverseScale)
{
	...
	for (machine a = 0; a < deckSize; a++) *voxelMap++ = 0x7F;
	
	float z = 1.0F;
	for (machine k = 2; k < voxelMapSize; k++)
	{
		for (machine a = 0; a < voxelMapSize; a++) *voxelMap++ = 0x7F;
...
void BlobParticleSystem::RenderBlob(Job *job, void *cookie)
{
	Voxel			*voxel;
...
void BlobParticleSystem::Render(const Camera *camera, List<Renderable> *effectList)
{
	const FrustumCamera *frustumCamera = static_cast<const FrustumCamera *>(camera);
	...
	if (particle)
	{
		...
		if (!(GetParticleSystemFlags() & kParticleSystemObjectSpace))
		{
			do
			{
				...
				const Point3D& p = particle->position;
				if (frustumCamera->FrustumCamera::SphereVisible(p, r))
				{
					float x = p.x - r;
					xmin = Fmin(xmin, x);
					xmax = Fmax(xmax, x);
		...
		else
		{
			...
			do
			{
				float r = particle->radius + voxelScale;
				const Point3D& p = particle->position;
				if (frustumCamera->FrustumCamera::SphereVisible(transform * p, r))
				{
					float x = p.x - r;
					xmin = Fmin(xmin, x);
					xmax = Fmax(xmax, x);
		...
		if (particleCount != 0)
		{
			...
			BlobVertex *restrict vertexArray = static_cast<BlobVertex *>(attributeVertexBuffer.BeginUpdate());
			Triangle *restrict triangleArray = static_cast<Triangle *>(indexVertexBuffer.BeginUpdate());
			
			unsigned_int32 particleIndex = 0;
			BlobJob *blobJob = jobArray;
			for (;;)
			{
				BlobParticle *particle = singleList.First();
				if (!particle) break;
			...
			BlobParticle *particle = blobBatch.multipleList.First();
			while (particle)
			{
				blobJob->particle = particle;
			...
			blobBatch.jobCount = blobJob - jobArray;
...
void BlobParticleSystem::Finalize(List<Renderable> *effectList)
{
	...
	if (renderCount != 0) GetFirstRenderSegment()->SetMultiRenderCount(renderCount);
	else effectList[GetEffectListIndex()].Remove(this);
}
