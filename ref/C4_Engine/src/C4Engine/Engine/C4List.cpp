...
{
	if (owningList) owningList->Remove(this);
}
...
void ListBase::Prepend(ListElementBase *element)
{
	...
	if (list)
	{
		...
		if (list->firstListElement == element) list->firstListElement = next;
		if (list->lastListElement == element) list->lastListElement = prev;
	}
	...
	{
		firstListElement = lastListElement = element;
	}
...
void ListBase::InsertBefore(ListElementBase *element, ListElementBase *before)
{
	...
	if (list)
	{
		ListElementBase *prev = element->prevListElement;
		ListElementBase *next = element->nextListElement;
		if (prev) prev->nextListElement = next;
		if (next) next->prevListElement = prev;
		if (list->firstListElement == element) list->firstListElement = next;
		if (list->lastListElement == element) list->lastListElement = prev;
	}
	...
	if (after) after->nextListElement = element;
	else firstListElement = element;
...
void ListBase::InsertAfter(ListElementBase *element, ListElementBase *after)
{
	...
	if (before) before->prevListElement = element;
	else lastListElement = element;
...
void ListBase::Remove(ListElementBase *element)
{
	...
	if (firstListElement == element) firstListElement = next;
	if (lastListElement == element) lastListElement = prev;
...
{
	while (firstListElement) delete firstListElement;
}
