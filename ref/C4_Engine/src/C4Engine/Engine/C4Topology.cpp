...
bool TopoEdge::CalculateCollapseCost(float collapseThreshold, const Point3D *vertexArray)
{
	...
	if (opposingEdge)
	{
		...
		while (edge)
		{
			if (!vertex1->FindOutgoingEdge(edge->GetStartElement())) return (false);
			
			const TopoFace *face = edge->GetFace(); 
			if ((face != face1) && (face != face2)) averageNormal += face->CalculateNormal(vertexArray);
		...
		Antivector4D plane((averageNormal % dp).Normalize(), p2);
		float d = 1.0F; 
		...
		while (edge)
		{
			...
			if ((face != face1) && (face != face2))
			{
				...
				for (machine a = 0; a < 3; a++)
				{
					...
					if (vertex != vertex1)
					{
						float d = plane ^ vertexArray[vertex->GetIndex()];
						if (d > kCollapsePlaneEpsilon) positive = true;
						else if (d < -kCollapsePlaneEpsilon) negative = true;
					}
				}
				...
				const Point3D& p0 = vertexArray[vertex2->GetIndex()];
				const Point3D& p1 = vertexArray[face->GetVertex(i2)->GetIndex()];
				const Point3D& p2 = vertexArray[face->GetVertex(i3)->GetIndex()];
				...
				if (positive)
				{
					...
					float newArea = normal1 * ((p1 - p0) % (p2 - p0));
					if (newArea / oldArea < 0.125F) return (false);
				}
				...
				if (negative)
				{
					...
					float newArea = normal2 * ((p1 - p0) % (p2 - p0));
					if (newArea / oldArea < 0.125F) return (false);
				}
			}
...
TopoFace::~TopoFace()
{
	for (machine a = 0; a < 3; a++) delete topoEdge[a];
}
...
int32 TopoFace::GetVertexIndex(const TopoVertex *vertex) const
{
	for (machine a = 0; a < 3; a++)
	{
		if (topoVertex[a] == vertex) return (a);
	}
...
TopoMesh::TopoMesh(const GeometryLevel *level, int32 faceCount)
{
	geometryLevel = level;
	...
	if (faceCount < 0) faceCount = level->GetFaceCount();
...
void TopoMesh::CollapseEdge(TopoVertex *collapseVertex, TopoVertex *keepVertex, TopoFace *collapseFace)
{
	...
	while (edge)
	{
		...
		if (face != collapseFace)
		{
			...
			if (index2 < 0) index2 = face->GetVertexIndex(keepVertex);
		...
		if (vertex->FindCollinearBoundaryEdges(vertexArray, collinearEdge))
		{
			...
			else if (edgeCount > 2)
			{
				...
				if ((interiorEdge0) && (interiorEdge1))
				{
					...
					bool collapse0 = (collapseFace0->CalculateNormal(vertexArray).Normalize() * interiorFace0->CalculateNormal(vertexArray).Normalize() > kCoplanarFaceEpsilon);
					bool collapse1 = (collapseFace1->CalculateNormal(vertexArray).Normalize() * interiorFace1->CalculateNormal(vertexArray).Normalize() > kCoplanarFaceEpsilon);
					if ((collapse0 & collapse1) && (collinearEdge[0]->GetCollapseCost() > collinearEdge[1]->GetCollapseCost())) collapse0 = false;
...
void TopoMesh::OptimizeMesh(const Point3D *vertexArray, float collapseThreshold)
{
	...
	while (face)
	{
		for (machine a = 0; a < 3; a++)
		{
			...
			if (edge->CalculateCollapseCost(collapseThreshold, vertexArray)) InsertCollapseEdge(edge, &edgeList);
		}
	...
	for (machine a = 0;; a++)
	{
		...
		for (;;)
		{
			...
			if (newEdge->CalculateCollapseCost(collapseThreshold, vertexArray)) InsertCollapseEdge(newEdge, &edgeList);
			else newList.Remove(newEdge);
		}
	}
