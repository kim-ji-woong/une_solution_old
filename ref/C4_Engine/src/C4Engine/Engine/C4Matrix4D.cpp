...
Transform4D& Transform4D::SetRotationAboutAxis(float angle, const Vector3D& axis)
{
...
Transform4D C4::operator *(const Transform4D& m1, const Transform4D& m2)
{
		...
		register float4 c1 = SimdLoad(&m1[0][0]);
		register float4 c2 = SimdLoad(&m1[0][0], 4);
		register float4 c3 = SimdLoad(&m1[0][0], 8);
		SimdStore3D(SimdTransformVector3D(c1, c2, c3, SimdLoad(&m2[0][0])), &result[0][0]);
		SimdStore3D(SimdTransformVector3D(c1, c2, c3, SimdLoad(&m2[0][0], 4)), &result[1][0]);
		SimdStore3D(SimdTransformVector3D(c1, c2, c3, SimdLoad(&m2[0][0], 8)), &result[2][0]);
		SimdStore3D(SimdTransformPoint3D(c1, c2, c3, SimdLoad(&m1[0][0], 12), SimdLoad(&m2[0][0], 12)), &result[3][0]);
		
		result.n[0][3] = result.n[1][3] = result.n[2][3] = 0.0F;
		result.n[3][3] = 1.0F;
		return (result);
...
Vector3D C4::operator *(const Transform4D& m, const Vector3D& v)
{
		...
		SimdStore3D(SimdTransformVector3D(SimdLoad(&m.n[0][0]), SimdLoad(&m.n[0][0], 4), SimdLoad(&m.n[0][0], 8), SimdLoadUnaligned(&v.x)), &result.x);
		return (result);
...
Vector3D C4::operator *(const Vector3D& v, const Transform4D& m)
{
	return (Vector3D(m.n[0][0] * v.x + m.n[0][1] * v.y + m.n[0][2] * v.z,
					 m.n[1][0] * v.x + m.n[1][1] * v.y + m.n[1][2] * v.z,
					 m.n[2][0] * v.x + m.n[2][1] * v.y + m.n[2][2] * v.z));
}
...
Point3D C4::operator *(const Transform4D& m, const Point3D& p)
{
		...
		SimdStore3D(SimdTransformPoint3D(SimdLoad(&m.n[0][0]), SimdLoad(&m.n[0][0], 4), SimdLoad(&m.n[0][0], 8), SimdLoad(&m.n[0][0], 12), SimdLoadUnaligned(&p.x)), &result.x);
		return (result);
...
Point3D C4::operator *(const Point3D& p, const Transform4D& m)
{
	return (Point3D(m.n[0][0] * p.x + m.n[0][1] * p.y + m.n[0][2] * p.z,
					m.n[1][0] * p.x + m.n[1][1] * p.y + m.n[1][2] * p.z,
					m.n[2][0] * p.x + m.n[2][1] * p.y + m.n[2][2] * p.z));
}
...
Vector2D C4::operator *(const Vector2D& v, const Transform4D& m)
{
	return (Vector2D(m.n[0][0] * v.x + m.n[0][1] * v.y,
					 m.n[1][0] * v.x + m.n[1][1] * v.y));
}
...
Point2D C4::operator *(const Point2D& p, const Transform4D& m)
{
	return (Point2D(m.n[0][0] * p.x + m.n[0][1] * p.y,
					m.n[1][0] * p.x + m.n[1][1] * p.y));
}

Matrix3D C4::Transform(const Matrix3D& m2, const Transform4D& m1)
{
	...
	float4 C4::TransformVector3D(const Transform4D& m, float4 v)
	{
		return (SimdTransformVector3D(SimdLoad(&m.n[0][0]), SimdLoad(&m.n[0][0], 4), SimdLoad(&m.n[0][0], 8), v));
	}
	
	float4 C4::TransformPoint3D(const Transform4D& m, float4 p)
	{
		return (SimdTransformPoint3D(SimdLoad(&m.n[0][0]), SimdLoad(&m.n[0][0], 4), SimdLoad(&m.n[0][0], 8), SimdLoad(&m.n[0][0], 12), p));
	}
