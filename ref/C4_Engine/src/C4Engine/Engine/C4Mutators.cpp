...
void Mutator::RegisterStandardMutators(void)
{
	...
	static MutatorReg<PulsateMutator> pulsateRegistration(kMutatorPulsate, table->GetString(StringID('MUTR', 'PULS')));
	static MutatorReg<RandomizeMutator> randomizeRegistration(kMutatorRandomize, table->GetString(StringID('MUTR', 'RAND')));
	static MutatorReg<ScrollMutator> scrollRegistration(kMutatorScroll, table->GetString(StringID('MUTR', 'SCRL')));
	static MutatorReg<RotateMutator> rotateRegistration(kMutatorRotate, table->GetString(StringID('MUTR', 'ROTR')));
	static MutatorReg<ScaleMutator> scaleRegistration(kMutatorScale, table->GetString(StringID('MUTR', 'SCAL')));
	static MutatorReg<FadeMutator> fadeRegistration(kMutatorFade, table->GetString(StringID('MUTR', 'FADE')));
	static MutatorReg<TickerMutator> tickerRegistration(kMutatorTicker, table->GetString(StringID('MUTR', 'TICK')));
	static MutatorReg<AnimateMutator> animateRegistration(kMutatorAnimate, table->GetString(StringID('MUTR', 'ANIM')));
}
...
void Mutator::Pack(Packer& data, unsigned_int32 packFlags) const
{
	...
	if (targetWidget->GetManipulator()) state &= ~kMutatorTerminated;
...
Setting *PulsateMutator::GetSetting(int32 index) const
{
	...
	if (index < count) return (Mutator::GetSetting(index));
...
void PulsateMutator::Move(void)
{
	...
	if (state > 1.0F) state -= PositiveFloor(state);
	else if (state < 0.0F) state += PositiveFloor(-state) + 1.0F;
...
void RandomizeMutator::Move(void)
{
	...
	image->SetVertexTexcoord2D(0, Point2D(x, y + scale.y));
	image->SetVertexTexcoord2D(1, Point2D(x, y));
	image->SetVertexTexcoord2D(2, Point2D(x + scale.x, y + scale.y));
	image->SetVertexTexcoord2D(3, Point2D(x + scale.x, y));
}
...
void ScrollMutator::Move(void)
{
	...
	const Vector2D& offset = image->GetImageOffset();
	const Vector2D& scale = image->GetImageScale();
	
	float x1 = offset.x - dx;
	float x2 = x1 + scale.x;
	float y1 = offset.y - dy;
	float y2 = y1 + scale.y;
	
	image->SetVertexTexcoord2D(0, Point2D(x1, y2));
	image->SetVertexTexcoord2D(1, Point2D(x1, y1));
	image->SetVertexTexcoord2D(2, Point2D(x2, y2));
	image->SetVertexTexcoord2D(3, Point2D(x2, y1));
}
...
void RotateMutator::Move(void)
{
	...
	if (GetMutatorState() & kMutatorReverse) speed = -speed;
	...
	Vector2D t = CosSin(angle * K::two_pi);
	
	ImageWidget *image = static_cast<ImageWidget *>(GetTargetWidget());
	const Vector2D& offset = image->GetImageOffset();
	const Vector2D& scale = image->GetImageScale();
	
	float cx = offset.x + scale.x * 0.5F;
	float cy = offset.y + scale.y * 0.5F;
	
	float dx = cx - t.x * cx + t.y * cy;
	float dy = cy - t.y * cx - t.x * cy;
	
	float x1 = offset.x;
	float x2 = offset.x + scale.x;
	float y1 = offset.y;
	float y2 = offset.y + scale.y;
	
	image->SetVertexTexcoord2D(0, Point2D(t.x * x1 - t.y * y2 + dx, t.y * x1 + t.x * y2 + dy));
	image->SetVertexTexcoord2D(1, Point2D(t.x * x1 - t.y * y1 + dx, t.y * x1 + t.x * y1 + dy));
	image->SetVertexTexcoord2D(2, Point2D(t.x * x2 - t.y * y2 + dx, t.y * x2 + t.x * y2 + dy));
	image->SetVertexTexcoord2D(3, Point2D(t.x * x2 - t.y * y1 + dx, t.y * x2 + t.x * y1 + dy));
}
...
void ScaleMutator::SetSetting(const Setting *setting)
{
	...
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) scaleMutatorFlags |= kScaleMutatorRestartable;
		else scaleMutatorFlags &= ~kScaleMutatorRestartable;
	}
	else if (identifier == 'TEXC')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) scaleMutatorFlags |= kScaleMutatorScaleTexcoords;
		else scaleMutatorFlags &= ~kScaleMutatorScaleTexcoords;
	}
	else if (identifier == 'FLPH')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) scaleMutatorFlags |= kScaleMutatorFlipHorizontal;
		else scaleMutatorFlags &= ~kScaleMutatorFlipHorizontal;
	}
	else if (identifier == 'FLPV')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) scaleMutatorFlags |= kScaleMutatorFlipVertical;
		else scaleMutatorFlags &= ~kScaleMutatorFlipVertical;
	}
...
void ScaleMutator::SetMutatorState(unsigned_int32 state)
{
	...
	if ((state & (kMutatorDisabled | kMutatorTerminated)) == 0)
	{
		...
		if (widget)
		{
			...
			if (controller) controller->ExtendAnimationTime(scaleTime);
		}
...
void ScaleMutator::UpdateScale(float time) const
{
	...
	if (flags & kScaleMutatorFlipHorizontal) dx = size.x * (1.0F - scale.x);
	if (flags & kScaleMutatorFlipVertical) dy = size.y * (1.0F - scale.y);
	...
	if (flags & kScaleMutatorScaleTexcoords)
	{
		...
		const Vector2D& imageOffset = imageWidget->GetImageOffset();
		const Vector2D& imageScale = imageWidget->GetImageScale();
		
		float x1 = imageOffset.x;
		float x2 = imageScale.x * scale.x + imageOffset.x;
		float y1 = 1.0F - imageOffset.y;
		float y2 = 1.0F - imageScale.y * scale.y - imageOffset.y;
		
		imageWidget->SetVertexTexcoord2D(0, Point2D(x1, y1));
		imageWidget->SetVertexTexcoord2D(1, Point2D(x1, y2));
		imageWidget->SetVertexTexcoord2D(2, Point2D(x2, y1));
		imageWidget->SetVertexTexcoord2D(3, Point2D(x2, y2));
	}
}
...
void FadeMutator::SetSetting(const Setting *setting)
{
	...
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) fadeMutatorFlags |= kFadeMutatorRestartable;
		else fadeMutatorFlags &= ~kFadeMutatorRestartable;
	}
	...
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) fadeMutatorFlags |= kFadeMutatorFinishHide;
		else fadeMutatorFlags &= ~kFadeMutatorFinishHide;
	}
	else
	{
		Mutator::SetSetting(setting);
...
void FadeMutator::SetMutatorState(unsigned_int32 state)
{
	...
	if ((state & (kMutatorDisabled | kMutatorTerminated)) == 0)
	{
		...
		{
			PanelController *controller = widget->GetPanelController();
			if (controller) controller->ExtendAnimationTime(fadeTime);
		}
...
void FadeMutator::Move(void)
{
	...
	if (!(state & kMutatorReverse))
	{
		...
		if (time >= fadeTime)
		{
			...
			{
				Widget *widget = GetTargetWidget();
				if (!widget->GetManipulator()) widget->SetWidgetState(widget->GetWidgetState() | kWidgetDisabled);
			}
		}
	}
	else
...
void TickerMutator::SetSetting(const Setting *setting)
{
	...
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) tickerMutatorFlags |= kTickerMutatorRestartable;
		else tickerMutatorFlags &= ~kTickerMutatorRestartable;
	}
	else if (identifier == 'LOOP')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) tickerMutatorFlags |= kTickerMutatorLooping;
		else tickerMutatorFlags &= ~kTickerMutatorLooping;
	}
...
void AnimateMutator::SetSetting(const Setting *setting)
{
	...
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) animateMutatorFlags |= kAnimateMutatorRestartable;
		else animateMutatorFlags &= ~kAnimateMutatorRestartable;
	}
	else if (identifier == 'LOOP')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) animateMutatorFlags |= kAnimateMutatorLooping;
		else animateMutatorFlags &= ~kAnimateMutatorLooping;
	}
...
void AnimateMutator::UpdateFrame(float frame) const
{
	...
	float s = (float) i * w;
	float t = 1.0F - (float) j * h;
	
	ImageWidget *widget = static_cast<ImageWidget *>(GetTargetWidget());
	widget->SetVertexTexcoord2D(0, Point2D(s, t));
	widget->SetVertexTexcoord2D(1, Point2D(s, t - h));
	widget->SetVertexTexcoord2D(2, Point2D(s + w, t));
	widget->SetVertexTexcoord2D(3, Point2D(s + w, t - h));
}
