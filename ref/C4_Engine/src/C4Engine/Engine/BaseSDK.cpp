...
char * MultiByteToUTF8( wchar_t *in )
{
	wchar_t *wc = in;

	char *buffer;  
	int nLen = WideCharToMultiByte( CP_UTF8, 0, wc, -1, NULL, 0, 0, 0 );
	buffer = (char *)malloc(nLen);  
	WideCharToMultiByte( CP_UTF8, 0, wc, -1, buffer, nLen, 0, 0 );
...
void MovementAction::Begin(void)
{
	...
	BaseWorld *world = static_cast<BaseWorld *>(TheWorldMgr->GetWorld());
	if (world)
	{
...
void BallController::Preprocess(void)
{
	...
	SetLinearVelocity(Math::RandomUnitVector() * 2.0F);
}
...
RigidBodyStatus BallController::HandleNewRigidBodyContact(const RigidBodyContact *contact, RigidBodyController *contactBody)
{
	...
	if (contactBody->GetControllerType() == kControllerBall)
	{
		...
		SparkParticleSystem *sparks = new SparkParticleSystem(10000);
...
BaseWorld::BaseWorld(const char *name) :
		World(name),
...
const LocatorMarker *BaseWorld::FindSpectatorLocator(const Zone *zone)
{
	...
	while (marker)
	{
		if (marker->Enabled())
		{
			...
			if (type == kMarkerLocator)
			{
				...
				if (locator->GetLocatorType() == kLocatorSpectator) return (locator);
			}
		}
...
WorldResult BaseWorld::Preprocess(void)
{
		...
		locatorReg(kLocatorSpectator, "Spectator Camera")
{
	...
	USES_CONVERSION;
	char * buf = MultiByteToUTF8(L"ÇÑ±ÛÀß³ª¿Èaa");
	TextWidget *pText = new TextWidget(Vector2D(200.0F, 40.0F), buf);

	free(buf);
	pText->SetFont("font/arial3");
	pText->SetWidgetColor(ColorRGBA(0.0f, 0.0f, 1.0f));
	pText->SetWidgetPosition(Point3D(50, 50, 0));

	// Adds widget to the screen
	
	FrustumViewportWidget * pWidget = new FrustumViewportWidget(Vector2D(400, 400), 46.0f);
	

	// Report a message to the log file
	Engine::Report("<br /><div>Hello World!</div>", kReportLog);
	...
	TheWorldMgr->LoadWorld("world/IsLand");


	World * pWorld = TheWorldMgr->GetWorld();
	
	
	Zone * pRootZone = pWorld->GetRootNode();

	Point3D ptWorld = pRootZone->GetWorldPosition();
	Box3D boxWorld = pRootZone->GetWorldBoundingBox();
	//TheInterfaceMgr->AddWidget(new StartWindow);
	TheInterfaceMgr->AddWidget(pText);
	TheInterfaceMgr->AddWidget(pWidget);
}
...
World *BaseSDK::ConstructWorld(const char *name, void *cookie)
{
	...
	return (new BaseWorld(name));
}

void BaseSDK::HandleDisplayEvent(const DisplayEventData *eventData, void *cookie)
{
...
void BaseSDK::EscapeProc(void *cookie)
{
