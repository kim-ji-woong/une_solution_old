...
namespace C4
{
	namespace Math
	{
		...
		void FindFirstHullEdge(int32 vertexCount, const HullVertex *hullVertex, int32 *i1, int32 *i2);
...
void Math::FindFirstHullEdge(int32 vertexCount, const HullVertex *hullVertex, int32 *i1, int32 *i2)
{
	int32 xminIndex = 0;
	int32 xmaxIndex = 0;
	int32 yminIndex = 0;
...
void Math::RemoveInteriorHullVertices(Graph<HullTriangle, HullEdge> *hullGraph, List<HullVertex> *vertexList)
{
	...
	while (vertex)
	{
		...
		do
		{
			if ((triangle->plane ^ vertex->position) > kHullEpsilon) goto next;
...
int32 Math::ComputeConvexHull(int32 vertexCount, const Point3D *vertex, const Box3D& boundingBox, unsigned_int16 *hullIndex)
{
	if (vertexCount > 4)
	{
		...
		HullVertex *hullVertex = static_cast<HullVertex *>(*buffer);
		...
		for (machine a = 0; a < vertexCount; a++) new(&hullVertex[a]) HullVertex((vertex[a] & scale) - center, a);
		...
		Vector3D tangent = (v2 - v1).Normalize();
				...
				for (;;)
				{
					...
					if (maxVertex)
					{
						...
						while (ht)
						{
							...
							if ((ht->plane ^ position) > kHullEpsilon) AugmentConvexHull(&triangleList, ht, maxVertex, position);
							ht = next;
						}
	...
	for (machine a = 0; a < vertexCount; a++) hullIndex[a] = (unsigned_int16) a;
	return (vertexCount);
}
