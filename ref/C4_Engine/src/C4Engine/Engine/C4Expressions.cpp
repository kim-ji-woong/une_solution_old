...
IdentifierToken::IdentifierToken(const char *identifier, int32 len) : Token(kTokenIdentifier)
{
	identifierString.Set(identifier, len);
}
...
Evaluator *Evaluator::Construct(Unpacker& data, unsigned_int32 unpackFlags)
{
	switch (data.GetType())
	{
		...
		case 'TREE':
		{
			...
			if (evaluatorCount != 0)
			{
				...
				for (machine a = 0; a < evaluatorCount; a++)
				{
					...
					Evaluator *evaluator = Evaluator::Construct(data, unpackFlags);
					if (evaluator)
					{
						evaluator->Unpack(++data, unpackFlags);
...
Token *ExpressionMethod::BuildNumberToken(const char *& text)
{
	String<255>		string;
	...
	if ((text[0] == '0') && ((text[1] == 'x') || (text[1] == 'X')))
	{
		...
		for (int32 len = 2;; len++)
		{
			...
			unsigned_int32 d = (unsigned_int32) (c - '0');
			if (d < 10U)
			{
				value = (value << 4) | d;
				continue;
			...
			unsigned_int32 e = (unsigned_int32) (c - 'a');
			if (e < 6U)
			{
				value = (value << 4) | (e + 10);
				continue;
			...
			if ((d < 26U) || (e < 26U) || (c == '_')) return (nullptr);
			break;
		}
	...
	for (int32 c = 0;; len++)
	{
		...
		if ((unsigned_int32) (c - '0') < 10U) continue;
		...
		if (((c == '-') || (c == '+')) && (prev == 'e')) continue;
		
		if (((unsigned_int32) (c - 'A') < 26U) || ((unsigned_int32) (c - 'a') < 26U) || (c == '_')) return (nullptr);
		break;
	}
	...
	if (integer) return (new IntegerToken(Text::StringToInteger(string)));
	return (new FloatToken(Text::StringToFloat(string)));
}

Token *ExpressionMethod::BuildIdentifierToken(const char *& text)
{
	int32 len = 1;
	for (;; len++)
	{
		...
		if (((unsigned_int32) (c - 'A') < 26U) || ((unsigned_int32) (c - 'a') < 26U) || ((unsigned_int32) (c - '0') < 10U) || (c == '_')) continue;
	...
	Token *token = new IdentifierToken(text, len);
	text += len;
	return (token);
}

Token *ExpressionMethod::BuildStringToken(const char *& text)
{
	String<kMaxStringValueLength>	string;
	
	int32 len = 0;
	bool backslash = false;
	
	for (; len < kMaxStringValueLength; text++)
	{
		int32 c = *text;
		if (c == 0) break;
...
Token *ExpressionMethod::BuildCharacterToken(const char *& text)
{
	unsigned_int32 value = 0;
...
bool ExpressionMethod::TokenizeText(const char *text, List<Token> *tokenList)
{
	for (;;)
	{
		...
		int32 c = *text;
		if ((unsigned_int32) (c - '0') < 10U)
		{
			token = BuildNumberToken(text);
		}
		else if (((unsigned_int32) (c - 'A') < 26U) || ((unsigned_int32) (c - 'a') < 26U) || (c == '_'))
		{
			token = BuildIdentifierToken(text);
		}
		else if (c == 34)
		{
			text++;
			token = BuildStringToken(text);
		}
		else if (c == 39)
		{
			text++;
			token = BuildCharacterToken(text);
		}
		else if (c == '(')
		{
			text++;
				...
				if (nextType == kTokenDot)
				{
					...
					if ((nextToken) && (nextToken->GetTokenType() == kTokenIdentifier))
					{
						...
						if ((c - 'a' < 26U) && (subname[1] == 0))
						{
							...
							evaluator->AddSubnode(new IntegerEvaluator(c));
					...
					if (nextToken)
					{
						...
						if (subevaluator)
						{
							if ((nextToken) && (nextToken->GetTokenType() == kTokenRightBracket))
							{
								...
								evaluator->AddSubnode(subevaluator);
								token = nextToken->Next();
							}
							else
							{
	...
	if (!evaluator)
	{
		...
		if (nextToken)
		{
			...
			else if ((type == kTokenMinus) || (type == kTokenBitwiseNot))
			{
				...
				if (subevaluator)
				{
					if (type == kTokenMinus) evaluator = new NegateEvaluator;
					else evaluator = new InvertEvaluator;
...
{
	if (!leftEvaluator) leftEvaluator = ParseUnaryExpression(token);
		...
		if (nextToken)
		{
			...
			if ((type == kTokenMultiply) || (type == kTokenDivide) || (type == kTokenModulo))
			{
				...
				if (rightEvaluator)
				{
					...
					if (type == kTokenMultiply) evaluator = new MultiplyEvaluator;
					else if (type == kTokenDivide) evaluator = new DivideEvaluator;
					else evaluator = new ModuloEvaluator;
					
					evaluator->AddSubnode(leftEvaluator);
					evaluator->AddSubnode(rightEvaluator);
					
					token = nextToken;
					if (token) return (ParseMultiplicativeExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseMultiplicativeExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if ((type == kTokenPlus) || (type == kTokenMinus))
			{
				...
				if (rightEvaluator)
				{
					...
					if (type == kTokenPlus) evaluator = new AddEvaluator;
					else evaluator = new SubtractEvaluator;
					
					evaluator->AddSubnode(leftEvaluator);
					evaluator->AddSubnode(rightEvaluator);
					
					token = nextToken;
					if (token) return (ParseAdditiveExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseAdditiveExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if ((type == kTokenShiftLeft) || (type == kTokenShiftRight))
			{
				...
				if (rightEvaluator)
				{
					...
					if (type == kTokenShiftLeft) evaluator = new ShiftLeftEvaluator;
					else evaluator = new ShiftRightEvaluator;
					
					evaluator->AddSubnode(leftEvaluator);
					evaluator->AddSubnode(rightEvaluator);
					
					token = nextToken;
					if (token) return (ParseShiftExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseShiftExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if ((type == kTokenLess) || (type == kTokenGreater) || (type == kTokenLessEqual) || (type == kTokenGreaterEqual))
			{
				...
				if (rightEvaluator)
				{
					...
					if (type == kTokenLess) evaluator = new LessEvaluator;
					else if (type == kTokenGreater) evaluator = new GreaterEvaluator;
					else if (type == kTokenLessEqual) evaluator = new LessEqualEvaluator;
					else evaluator = new GreaterEqualEvaluator;
					
					evaluator->AddSubnode(leftEvaluator);
					evaluator->AddSubnode(rightEvaluator);
					
					token = nextToken;
					if (token) return (ParseRelationalExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseRelationalExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if ((type == kTokenEqual) || (type == kTokenNotEqual))
			{
				...
				if (rightEvaluator)
				{
					...
					if (type == kTokenEqual) evaluator = new EqualEvaluator;
					else evaluator = new NotEqualEvaluator;
					
					evaluator->AddSubnode(leftEvaluator);
					evaluator->AddSubnode(rightEvaluator);
					
					token = nextToken;
					if (token) return (ParseEqualityExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseEqualityExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if (type == kTokenBitwiseAnd)
			{
				...
				if (rightEvaluator)
				{
					...
					token = nextToken;
					if (token) return (ParseAndExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseAndExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if (type == kTokenBitwiseXor)
			{
				...
				if (rightEvaluator)
				{
					...
					token = nextToken;
					if (token) return (ParseXorExpression(token, evaluator));
...
{
	if (!leftEvaluator) leftEvaluator = ParseXorExpression(token);
	if ((leftEvaluator) && (token))
	{
		const Token *nextToken = token->Next();
		if (nextToken)
		{
			...
			if (type == kTokenBitwiseOr)
			{
				...
				if (rightEvaluator)
				{
					...
					token = nextToken;
					if (token) return (ParseOrExpression(token, evaluator));
...
void ExpressionMethod::SetExpressionText(const char *text)
{
	...
	if (text[0] != 0) expressionText = text;
	else expressionText.Clear();
...
void ExpressionMethod::Execute(const ScriptState *state)
{
	if (evaluatorRoot)
	{
		...
		if (value)
		{
			...
			if (output) output->SetValue(value);
		}
	}
