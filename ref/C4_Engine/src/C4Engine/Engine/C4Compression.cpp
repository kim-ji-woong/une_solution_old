...
void Comp::UpdateDictEntry(DictEntry *dictTable, DictEntry **hashTable, const unsigned_int8 *data)
{
	...
	if (entry->data)
	{
		...
		else hashTable[GetDataHash(entry->data)] = next;
...
unsigned_machine Comp::CountMatchingBytes(const unsigned_int8 *x, const unsigned_int8 *y, unsigned_machine max)
{
	...
	if (count >= kMinMatchCount) return (count);
	return (0); 
}
...
{ 
	...
	if (length != 0)
	{
		...
		for (unsigned_machine a = 0; a < length; a++) code[a] = data[a];
	}
	
	return (code - start + length);
}
...
unsigned_int32 Comp::CompressData(const void *input, unsigned_int32 dataSize, unsigned_int8 *restrict code)
{
	...
	for (machine a = 0; a < kWindowSize; a++) dictTable[a].data = nullptr;
	
	DictEntry **hashTable = new DictEntry *[kHashTableSize];
	for (machine a = 0; a < kHashTableSize; a++) hashTable[a] = nullptr;
	...
	for (;;)
	{
		...
		if (bestMatchLength < maxLength)
		{
			...
			while (entry)
			{
				...
				if (matchLength - controlSize > bestMatchLength - bestControlSize)
				{
					...
					if (bestMatchLength == maxLength) break;
				}
				
				if (++depth == kMaxMatchDepth) break;
				entry = entry->next;
			}
		}
		...
		if (bestMatchLength != 0)
		{
			...
			unsigned_machine size = WriteCompressedCode(bestMatchLength, compressData - bestMatchData, &code[codeSize], dataSize - codeSize);
			if (size == 0)
			{
				codeSize = 0;
				break;
			...
			for (unsigned_machine a = 0; a < bestMatchLength; a++) UpdateDictEntry(dictTable, hashTable, &compressData[a]);
	...
	for (unsigned_machine a = codeSize; a < totalSize; a++) code[a] = kCompressionTerminator << 5;
	return (totalSize);
}

void Comp::DecompressData(const unsigned_int8 *code, unsigned_int32 codeSize, void *restrict output)
{
	unsigned_int8 *restrict data = static_cast<unsigned_int8 *>(output);
	
	const unsigned_int8 *end = code + codeSize;
	while (code < end)
	{
		unsigned_machine	length, distance;
		
		unsigned_machine control = code[0];
		switch (control >> 5)
		{
			case kUncompressedTinyLength:
			{
				length = (control & 0x1F) + 1;
				code++;
				
				for (unsigned_machine a = 0; a < length; a++) data[a] = code[a];
				code += length;
				data += length;
				break;
			}
			
			case kUncompressedStandardLength:
			{
				if ((control & 0x1F) != 0)
				{
			...
			case kCompressedTinyLengthShortDistance:
			{
				...
				for (unsigned_machine a = 0; a < length; a++) data[a] = source[a];
				data += length;
				break;
			}
			
			case kCompressedStandardLengthShortDistance:
			{
				if ((control & 0x1F) != 0)
				{
			...
			case kCompressedTinyLengthLongDistance:
			{
				length = (control & 0x1F) + kMinMatchCount;
			...
			case kCompressedStandardLengthLongDistance:
			{
				if ((control & 0x1F) != 0)
				{
					length = ((code[1] << 8) | code[2]) + (kMaxShortLength + 1);
