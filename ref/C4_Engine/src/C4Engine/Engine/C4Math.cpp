...
const unsigned_int32 align_address(128) C4::Math::trigTable[256][2] =
{
	...
	{
		if (x < K::min_float) return (K::infinity);
	...
	float C4::Sqrt(float x)
	{
			...
			float4 v = _mm_load_ss(&x);
			float4 mask = _mm_cmplt_ps(v, SimdLoadConstant<0x00800000>());
			
			float4 r = _mm_rsqrt_ss(v);
			r = _mm_mul_ss(_mm_mul_ss(_mm_sub_ss(SimdLoadConstant<0x40400000>(), _mm_mul_ss(v, _mm_mul_ss(r, r))), r), SimdLoadConstant<0x3F000001>());
	...
	float C4::InverseSqrt(float x) 
	{
			...
			_mm_store_ss(&result, _mm_or_ps(_mm_andnot_ps(mask, r), _mm_and_ps(mask, SimdLoadConstant<0x7F800000>())));
			return (result);
...
float C4::Sin(float x)
{
		...
		float4 b = SimdMulScalar(SimdAndc(SimdLoadScalar(&x), SimdGetNegativeZero()), SimdLoadConstant<0x4222F983>());
		float4 i = SimdPositiveFloorScalar(b);
		b = SimdMulScalar(SimdSubScalar(b, i), SimdLoadConstant<0x3CC90FDB>());
		
		const Vector2D& cossin = Math::GetTrigTable()[SimdTruncateConvert(i) & 255];
		float4 cosine_alpha = SimdLoadScalar(&cossin.x);
		float4 sine_alpha = SimdLoadScalar(&cossin.y);
		
		float4 b2 = SimdMulScalar(b, b);
		float4 sine_beta = SimdNmsubScalar(SimdMulScalar(b, b2), SimdNmsubScalar(b2, SimdLoadConstant<0x3E2AAAAB>(), SimdLoadConstant<0x3C088889>()), b);
		float4 cosine_beta = SimdNmsubScalar(b2, SimdNmsub(b2, SimdLoadConstant<0x3D2AAAAB>(), SimdLoadConstant<0x3F000000>()), SimdLoadConstant<0x3F800000>());
		
		float4 sine = SimdMaddScalar(sine_alpha, cosine_beta, SimdMulScalar(cosine_alpha, sine_beta));
		
		SimdStoreScalar((x < 0.0F) ? SimdNegate(sine) : sine, &result);
		return (result);
...
float C4::Cos(float x)
{
		...
		SimdStoreScalar(SimdSubScalar(SimdMulScalar(cosine_alpha, cosine_beta), SimdMulScalar(sine_alpha, sine_beta)), &result);
		return (result);
...
float C4::Tan(float x)
{
		...
		SimdStoreScalar(SimdMaddScalar(sine_alpha, cosine_beta, SimdMulScalar(cosine_alpha, sine_beta)), &sine);
		SimdStoreScalar(SimdSubScalar(SimdMulScalar(cosine_alpha, cosine_beta), SimdMulScalar(sine_alpha, sine_beta)), &cosine);
...
void C4::CosSin(float x, float *c, float *s)
{
		...
		float4 sine = SimdMaddScalar(sine_alpha, cosine_beta, SimdMulScalar(cosine_alpha, sine_beta));
		float4 cosine = SimdSubScalar(SimdMulScalar(cosine_alpha, cosine_beta), SimdMulScalar(sine_alpha, sine_beta));
		
		SimdStoreScalar(cosine, c);
		SimdStoreScalar((x < 0.0F) ? SimdNegate(sine) : sine, s);
...
Vector2D C4::CosSin(float x)
{
		...
		SimdStoreScalar(cosine, &result.x);
		SimdStoreScalar((x < 0.0F) ? SimdNegate(sine) : sine, &result.y);
		return (result);
...
Vector3D Math::CreatePerpendicular(const Vector3D& v)
{
	...
	if ((az > ax) && (az > ay)) return (Vector3D(0.0F, z, -y));
	if (ay > ax) return (Vector3D(y, -x, 0.0F));
