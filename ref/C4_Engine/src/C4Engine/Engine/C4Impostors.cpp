...
{ 
	...
	{
		int32 selection = 0;
		if (textureUsage == kImpostorNormalMap) selection = 1;
		else if (textureUsage == kImpostorShadowMap) selection = 2;
...
void ImpostorProperty::SetSetting(const Setting *setting)
{
	...
	else if (identifier == 'USAG')
	{
		...
		if (selection == 0) textureUsage = kImpostorColorMap;
		else if (selection == 1) textureUsage = kImpostorNormalMap;
		else textureUsage = kImpostorShadowMap;
	}
	else if (identifier == 'ALFA')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) impostorFlags |= kImpostorAlphaChannel;
		else impostorFlags &= ~kImpostorAlphaChannel;
	}
	else if (identifier == 'CLP1')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) impostorFlags |= kImpostorKeepBottom;
		else impostorFlags &= ~kImpostorKeepBottom;
	}
	else if (identifier == 'CLP2')
	{
		bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
		if (b) impostorFlags |= kImpostorKeepTop;
		else impostorFlags &= ~kImpostorKeepTop;
	}
}
...
Impostor::~Impostor()
{
	if (impostorSystem) impostorSystem->RemoveImpostor(this);
...
{
	if (category == kNodeImpostor) return (3);
...
void Impostor::SetCategorySetting(Type category, const Setting *setting)
{
	if (category == kNodeImpostor)
	{
		...
		{
			bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
			if (b) perspectiveInstantMask |= kPerspectiveReflection;
			else perspectiveInstantMask &= ~kPerspectiveReflection;
		}
		else if (identifier == 'REFR')
		{
			bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
			if (b) perspectiveInstantMask |= kPerspectiveRefraction;
			else perspectiveInstantMask &= ~kPerspectiveRefraction;
		}
		else if (identifier == 'CAMR')
		{
			bool b = static_cast<const BooleanSetting *>(setting)->GetBooleanValue();
			if (b) perspectiveInstantMask |= kPerspectiveCameraWidget;
			else perspectiveInstantMask &= ~kPerspectiveCameraWidget;
		}*/
	}
	else
	{
...
void Impostor::Invalidate(void)
{
	...
	if (impostorSystem) impostorSystem->buildFlag = true;
}
...
void Impostor::Preprocess(void)
{
	...
	if (materialObject)
	{
		...
		{
			const ShaderGraph *shaderGraph = static_cast<const ShaderAttribute *>(attribute)->GetShaderGraph(kShaderGraphAmbient);
			...
			while (process)
			{
				if (process->GetProcessType() == kProcessImpostorTexture)
				{
					...
					while (node)
					{
						if (node->GetNodeType() == kNodeGeometry) static_cast<Geometry *>(node)->SetRenderParameterPointer(renderParameter);
...
void Impostor::CalculateWorldTransform(void)
{
...
ImpostorSystem::ImpostorSystem(MaterialObject *material, const float *clipData) :
		...
		attributeVertexBufferObserver(this, &ImpostorSystem::FillAttributeVertexBuffer),
		...
		impostorRenderable(kRenderIndexedTriangles, kRenderDepthTest)
{
	...
	impostorRenderable.SetVertexBuffer(kVertexBufferStaticArray, &attributeVertexBuffer);
	impostorRenderable.SetVertexBuffer(kVertexBufferIndexArray, &indexVertexBuffer);
	
	impostorRenderable.SetAttributeOffset(kArrayVertex, 0, 3);
	impostorRenderable.SetAttributeOffset(kArrayRadius, sizeof(Point3D), 3);
	impostorRenderable.SetAttributeOffset(kArrayTexture0, sizeof(Point3D) + sizeof(Vector3D), 2);
	
	impostorRenderable.SetFaceOffset(0);
}

ImpostorSystem::~ImpostorSystem()
{
	delete[] impostorTriangle;
}
...
void ImpostorSystem::RemoveImpostor(Impostor *impostor)
{
	...
	if (impostorList.Empty()) delete this;
}
...
void ImpostorSystem::Build(void)
{
	if (buildFlag)
	{
		...
		attributeVertexBuffer.Initialize(impostorCount * (geometryTriangleCount + 2) * sizeof(ImpostorVertex), sizeof(ImpostorVertex), &attributeVertexBufferObserver);
		indexVertexBuffer.Initialize(impostorCount * geometryTriangleCount * sizeof(Triangle), 0);
		
		delete[] impostorTriangle;
		impostorTriangle = new Triangle[impostorCount * geometryTriangleCount];
...
{
	ImpostorVertex *restrict vertex = static_cast<ImpostorVertex *>(vertexBuffer->BeginUpdate());
...
void ImpostorSystem::RenderImpostor1(const Impostor *impostor)
{
	int32 count = impostorRenderCount;
	impostorRenderCount = count + 1;
	...
	Triangle *triangle = &impostorTriangle[count * 2];
	triangle[0].Set(i1, i2, i3);
	triangle[1].Set(i2, i4, i3);
}

void ImpostorSystem::RenderImpostor2(const Impostor *impostor)
{
	int32 count = impostorRenderCount;
	impostorRenderCount = count + 1;
	...
	Triangle *triangle = &impostorTriangle[count * 4];
	triangle[0].Set(i1, i2, i3);
	triangle[1].Set(i2, i4, i3);
...
void ImpostorSystem::RenderImpostor3(const Impostor *impostor)
{
	int32 count = impostorRenderCount;
	impostorRenderCount = count + 1;
	...
	Triangle *triangle = &impostorTriangle[count * 6];
	triangle[0].Set(i1, i2, i3);
	triangle[1].Set(i2, i4, i3);
	triangle[2].Set(i3, i4, i5);
	triangle[3].Set(i4, i6, i5);
...
void ImpostorSystem::RenderSystem(List<Renderable> *renderList)
{
	int32 count = impostorRenderCount;
	if (count != 0)
	{
		...
		int32 triangleCount = geometryTriangleCount;
		impostorRenderable.SetVertexCount(count * (triangleCount + 2));
		impostorRenderable.SetFaceCount(count * triangleCount);
		
		indexVertexBuffer.UpdateBuffer(0, count * triangleCount * sizeof(Triangle), impostorTriangle);
		renderList->Append(&impostorRenderable);
	}
}
