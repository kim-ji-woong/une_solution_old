...
bool PlateVolume::InteriorPoint(const Point3D& p) const
{
	...
	if ((p.x < 0.0F) || (p.x > plateSize.x)) return (false);
	if ((p.y < 0.0F) || (p.y > plateSize.y)) return (false);
...
{
	if ((p.x < 0.0F) || (p.x > boxSize.x)) return (false);
	if ((p.y < 0.0F) || (p.y > boxSize.y)) return (false);
	if ((p.z < 0.0F) || (p.z > boxSize.z)) return (false);
...
bool PyramidVolume::InteriorPoint(const Point3D& p) const
{
	...
	if ((p.x < sx * m1) || (p.x > sx * m2)) return (false);
	if ((p.y < sy * m1) || (p.y > sy * m2)) return (false);
...
{
	if ((p.z < 0.0F) || (p.z > cylinderHeight)) return (false);
...
{
	if (center.z > sphereSize.z + radius) return (true);
	if (center.z < -sphereSize.z - radius) return (true);
	if (center.y > sphereSize.y + radius) return (true);
	if (center.y < -sphereSize.y - radius) return (true);
	if (center.x > sphereSize.x + radius) return (true);
	if (center.x < -sphereSize.x - radius) return (true);
...
{
	if (center.z > domeSize.z + radius) return (true);
	if (center.z < -radius) return (true);
	
	if (center.y > domeSize.y + radius) return (true);
	if (center.y < -domeSize.y - radius) return (true);
	if (center.x > domeSize.x + radius) return (true);
	if (center.x < -domeSize.x - radius) return (true);
...
{
	if (center.z > capsuleHeight + capsuleSize.z + radius) return (true);
	if (center.z < capsuleSize.z - radius) return (true);
...
bool CapsuleVolume::InteriorPoint(const Point3D& p) const
{
	...
	if ((p.z > 0.0F) && (p.z < h)) return (!(p.x * p.x * sy + p.y * p.y * sx > sx * sy));
	
	float f = sx * sy;
	float sz = capsuleSize.z;
	sz *= sz;
	
	if (p.z < h) return (!(p.x * p.x * (sy * sz) + p.y * p.y * (sx * sz) + p.z * p.z * f > f * sz));
...
{
	if (center.z > GetPyramidHeight() + radius) return (true);
	return (center.z < -radius);
}
...
{
	if (center.z > GetConeHeight() + radius) return (true);
	if (center.z < -radius) return (true);
...
{
	if (center.z > domeHeight + radius) return (true);
	if (center.z < -radius) return (true);
