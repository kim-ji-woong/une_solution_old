...
public class ImporterMain {
	...
	public ImporterMain(String inputFileName, String outputFileName) {
		
		factory.setNamespaceAware(false);
		JSONArray jsonArray = new JSONArray();
		try {
			File file = new File(inputFileName);
//			File file = new File("disease.htm");
		    ...
		    while ((line = br.readLine()) != null) {
		    	...
		    	if(line.contains("IMG")) {
			    	...
			    	String regexStr = "IMG src=\".\\\\(\\S*)\\.(.{3})\"";
					Pattern pat = Pattern.compile(regexStr);
					Matcher matcher = pat.matcher(line);
					
		    	}		    	
		    	
		    	if(line.contains("<begin>") && stringbuilder.length() > 0) {   		
		    				    			
    				datalist.add(stringbuilder);
	    			System.out.println("dataset : " + stringbuilder);
		    ...
		    System.out.println("dataset total count: " + count);
...
//			BufferedWriter out = new BufferedWriter(new FileWriter("disease.json"));		//각 매뉴얼을 json파일로 만든 후 붙여야 한다
		    ...
		    for(int i =0; i < datalist.size(); i++) {
				...
				NodeList cols = (NodeList)xpath.evaluate("//SPAN", document, XPathConstants.NODESET);
