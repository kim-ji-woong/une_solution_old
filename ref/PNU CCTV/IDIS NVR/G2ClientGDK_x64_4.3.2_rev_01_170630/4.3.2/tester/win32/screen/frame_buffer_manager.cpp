...
#include "frame_buffer/frame_buffer_watch.h"
#include "frame_buffer/frame_buffer_search_g2.h"
#include "frame_buffer/frame_buffer_search.h"
...
frame_buffer_manager::frame_buffer_manager(void)
    ...
    , _layout(client::screen_formatter_::LAYOUT_4x4)
{
...
void frame_buffer_manager::setup_buffer(short channel, client::frame_buffer::TYPE type, client::frame_buffer::PLAYTIME time)
{
    ...
    switch (type) {
        case frame_buffer::TYPE_WATCH:
            buff = new frame_buffer_watch(type, frame_buffer::SIZE_WATCH);
            break;
        case frame_buffer::TYPE_SEARCH:
            buff = new frame_buffer_search(type, frame_buffer::SIZE_SEARCH, time);
            break;;
        case frame_buffer::TYPE_SEARCH_SAMBA:
            buff = new frame_buffer_search(type, frame_buffer::SIZE_SEARCH_SAMBA, time);
            break;;
        case frame_buffer::TYPE_SEARCH_IDR:
            buff = new frame_buffer_search(type, frame_buffer::SIZE_SEARCH_IDR, time);
            break;;
...
bool frame_buffer_manager::put_frame(const G2FRAME& frame, short channel, short basecamera, unsigned __int64 refcameras)
{
    ...
    if (typeFrame != G2FRAME::AUDIO) {
        ...
        if (from == G2FRAME::FROM_SEARCH ||
            ...
            g2::scoped_criticalsection lock(_spnRemove[channel]);
            if (frame_buffer* buff = _container[channel]) {
                buff->push(frame, channel, basecamera, refcameras);
                result = true;
            }
...
void frame_buffer_manager::on_screen_layout_changed(int layout, int count, int changed)
{
    _layout = layout;
    ...
    for (int i = (int)_container.size() - 1; i >= 0; --i) {
        if (frame_buffer* buff = _container[i]) {
            buff->changed_layout(layout, count, changed);
        }
    }
}
