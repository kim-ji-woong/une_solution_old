// search_event_list.h : header file
//

#ifndef _SEARCH_EVENT_LIST_H_
#define _SEARCH_EVENT_LIST_H_
...
class search_event_list : public CWnd
{
    DECLARE_DYNAMIC(search_event_list)
public:
    search_event_list(void);
    virtual ~search_event_list(void);
...
protected:
    CWnd* _parent;
    listener_search_event_list* _listener;
    ...
    std::vector<G2SEARCH_LOG_INFO> _eventLog;
    std::vector<G2TEXT_IN> _textInLog;
...
public:
    ...
    void set_listenr(listener_search_event_list* listener);
    ...
    void insert(const std::vector<G2SEARCH_LOG_INFO>& info);
    void insert(const std::vector<G2TEXT_IN>& info);
