...
{
    public partial class form_play : screen_listener
    {
        ...
        public void on_screen_changed_format(screen_format.FORMAT format, screen_format.CHANGED mode)
        {
            if (_controller != null && _adaptor.is_connected(_channel))
            {
                ...
                if (format == screen_format.FORMAT.LAYOUT1X1)
                {
                    ...
                    channelset.insert(_play_info.channelext_from_panel(pane));
                }
                else
                {
                    foreach (int ch in _play_info.channelexts)
                    {
                ...
                int channelext = _play_info.channelext_from_panel(panel);
