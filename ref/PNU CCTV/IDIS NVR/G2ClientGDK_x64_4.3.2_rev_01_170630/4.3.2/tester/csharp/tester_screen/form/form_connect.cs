    ...
    public partial class form_connect : Form
    {
        ...
        protected bool imp_load()
        {
            try
            {
                using (StreamReader file = new StreamReader(file_device))
                {
                    ...
                    XmlSerializer ar = XmlSerializer.FromTypes(types)[0];
                    internal_data load = (internal_data)ar.Deserialize(file);
        ...
        protected bool imp_save()
        {
            try
            {
                ...
                {
                    Type[] types = new Type[] { internal_inf.GetType() };
                    XmlSerializer ar = XmlSerializer.FromTypes(types)[0];
                    ar.Serialize(file, internal_inf);
                }
                return true;
            }
            catch (Exception) { }
