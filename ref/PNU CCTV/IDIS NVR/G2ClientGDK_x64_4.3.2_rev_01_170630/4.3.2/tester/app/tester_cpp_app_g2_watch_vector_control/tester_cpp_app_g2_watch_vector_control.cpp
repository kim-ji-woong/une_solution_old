// tester_cpp_app_g2_watch_vector_control.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//
...
public:
	struct OPTIONS {
		OPTIONS(void) : ni(), cameras(), threads(0), renderer(NULL),/* renderer2(NULL),*/ surface(NULL) {
...
public:
	...
	bool is_relative(void) const { return _move_type == G2LIVE_COMMAND_CONTROL_PTZ::MT_RELATIVE; }
...
public:
	...
	void change_move_type(void)
	{
		...
		if (is_relative()) {
			...
			_move_type = G2LIVE_COMMAND_CONTROL_PTZ::MT_RELATIVE;
		}
	...
	bool request_command_control(size_t key)
	{
		...
		if(key == SDLK_RIGHT ||
			key == SDLK_LEFT) {
				...
				if (is_relative()) {
					status._control._pan = sign * ((_relative) ? (((float)_relative / RELATIVE_SPEED_STEP) * status._range._min_pan) : 0);
				}
				else {
	...
	void send_command_control_ptz_IDIS(POINT begin, POINT end)
	{
		int channel = _channel;
		if (_adaptor->is_connected(channel)){
		}
	...
	static void print_help(void)
	{
		...
		std::cout << "KEY_PAD(8)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(9)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(4)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(5)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(6)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(1)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(2)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(3)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(-)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(+)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(/)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(*)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(0)	PTZ_MOVE_NW" << std::endl;
		std::cout << "KEY_PAD(.)	PTZ_MOVE_NW" << std::endl;
...
{
	boost::program_options::options_description options_desc("app_g2_watch_vector_control");
	...
	if (options.count("help") || need_help) {
		...
		std::cout << "sample: tester_cpp_app_g2_watch_vector_control --address \"127.0.0.1:8016\" --user \"admin\" --password \"\" --camera 0" << std::endl;
		return 0;
	}
	...
	SDL_Window* window = SDL_CreateWindow("GDK G2VectorControl", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 960, 480, SDL_WINDOW_INPUT_FOCUS | SDL_WINDOW_SHOWN);
		...
		for( ; ; ) {
			...
			if(evt.type == SDL_MOUSEBUTTONDOWN) {
				...
				if(evt.button.button == SDL_BUTTON_LEFT){
					...
					pe.x = evt.button.x;
					pe.y = evt.button.y;
					std::cout <<"pb.x:"<<pb.x<<", pb.y:"<<pb.y<<", pe.x:"<<pe.x<<", pe.y:"<<pe.y<< std::endl;
				}
			}
			else if(evt.type == SDL_MOUSEMOTION) {
				if(evt.button.button == SDL_BUTTON_LEFT){
					POINT offset = {0};
					offset.x = evt.button.x - pb.x;
					offset.y = evt.button.y - pb.y;

					const float delta = 16.0F;
					float fP = 0.0F, fT = 0.0F, fZ = 0.0F;
					if (offset.x || offset.y) {
						float size = 800;
						int w, h;
						SDL_GetWindowSize(window, &w, &h);
						size = (float)max(w, h);
						if (offset.x) fP = boost::algorithm::clamp((delta * offset.x) / size, -delta, +delta);
						if (offset.y) fT = boost::algorithm::clamp((delta * offset.y) / size * -1.0F, -delta, +delta);
						
						std::cout.precision(6);
						std::cout << ">>moust_PTZ_IDIS::Pan " << fP << " / Tilt " << fT << std::endl;
						//std::cout <<"fp:" << fP << "  ft:" << fT<< std::endl;
					}
				}
			}
