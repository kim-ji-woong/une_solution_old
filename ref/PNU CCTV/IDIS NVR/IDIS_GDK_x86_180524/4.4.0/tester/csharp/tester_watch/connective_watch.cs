...
namespace GDK_tester
{
    ...
    public partial class form_watch : g2watch_listener
    {
        ...
        public void on_g2watch_connected(G2HWATCH handle, int channel)
        {
            ...
            if (_finalize != true)
            {
                this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_connected(channel); });
            }
        }
        ...
        public void on_g2watch_disconnected(G2HWATCH handle, int channel, G2DISCONNECT_REASON.TYPE reason)
        {
            ...
            if (_finalize != true)
            {
                this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_disconnected(channel, reason); });
            }
        }
        ...
        public void on_g2watch_receive_event(G2HWATCH handle, int channel, ref G2EVENT_INFO ei)
        {
            ...
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_event(channel, ref info); });
        }
        public void on_g2watch_receive_device_status(G2HWATCH handle, int channel, ref G2DEVICE_STATUS status)
        {
            ...
            if (_sitename != status._sitename)
            {
                ...
                this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_update_sitename(); });
            }
        }
        public void on_g2watch_receive_ptz_preset(G2HWATCH handle, int channel, int camera, ref G2LIVE_PTZ_PRESET preset)
        {
            ...
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_ptz_preset(channel, ref info); });
        }
        public void on_g2watch_receive_ptz_menu(G2HWATCH handle, int channel, int camera, ref G2LIVE_PTZ_MENU menu)
        {
            ...
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_ptz_menu(channel, ref info); });
        }
        ...
        {
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_audio_out_not_available(); });
        }
        ...
        public void on_g2watch_receive_command_result_control_ptz_status(G2HWATCH handle, int channel, int camera, ref G2LIVE_COMMAND_CONTROL_PTZ control, ref G2LIVE_COMMAND_CONTROL_PTZ_RANGE range)
        {
            ...
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_ptz_command_control_status(channel, camera, ref c, ref r); });
        }
        ...
        public void on_g2watch_receive_network_alarm_result(G2HWATCH handle, int channel, ref G2LIVE_NETWORK_ALARM_RESULT result)
        {
            ...
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_network_alarm_result(channel, ref info); });
        }
        ...
        {
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_instant_recording_start(channel, result, status); });
        }
        public void on_g2watch_receive_instant_recording_stop(G2HWATCH handle, int channel, G2INSTANT_RECORDING_RESULT.TYPE result)
        {
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_instant_recording_stop(channel, result); });
        }
        ...
        {
            this.BeginInvoke((MethodInvoker)delegate() { on_post_watch_receive_instant_recording_status(channel, result, status); });
        }
        ...
        private void on_post_watch_connected(int channel)
        {
            ...
            _screen.set_pane_select(6);
            _screen.set_format2(screen_format.FORMAT.LAYOUT1X1, true, 6);
        }
