    ...
    public partial class form_watch
    {
        public void feature_control_audio_screen_changed_pane(int pane)
        {
            ...
            if (enable)
            {
                ...
                if (cp != null)
                {
                    enable = (cp._mode == camera_pane.MODE.LIVE);
                }
        ...
        private void on_feature_control_audio_in(object sender, EventArgs e)
        {
            int camera = _screen.selected_pane_channelext();
            bool active = feature_control_audio_is_active_audio_in(camera) ? false : true;

            if (active)
            {
                if (_adaptor.is_enable_audio_device_play(this.Handle) != true)
                {
                    feature_control_audio_set_enable(true);
                    _screen.message().disp("there is no speaker or sound output device connected to the computer.", 5 * 1000, false);
                    return;
                }
            }

            _adaptor.set_enable_audio_streaming(_channel, camera, active);

            bool contains = _adaptor.is_contains_audio_streaming(_channel, camera);
        ...
        private void on_feature_control_audio_out(object sender, EventArgs e)
        {
            int camera = _screen.selected_pane_channelext();
            bool active = feature_control_audio_is_active_audio_out(camera) ? false : true;

            if (active)
            {
                if (_adaptor.is_enable_audio_device_record() != true)
                {
                    feature_control_audio_set_enable(true);
                    _screen.message().disp("there is no microphone or sound input device connected to the computer.", 5 * 1000, false);
                    return;
                }
            }

            bool res = _adaptor.set_enable_audio_capturing(_channel, camera, active);
