...
namespace GDK_tester
{
    public partial class form_watch
    {
        ...
        public bool feature_control_PTZ(int camera, int stream_id, G2LIVE_PTZ_COMMAND.COMMAND cmd, feature_PTZ.PARAM param)
        {
            int arg = (param == feature_PTZ.PARAM.HOLD) ? 1 : 0;

            if (param == feature_PTZ.PARAM.RELEASE)
            {
                if (cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_N ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_NE ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_E ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_SE ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_S ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_SW ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_W ||
                    cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MOVE_NW)
                {
                    cmd = G2LIVE_PTZ_COMMAND.COMMAND.PTZ_STOP_MOVE;
                    arg = 0;
                }
                else if (cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_ZOOM_IN ||
                         cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_ZOOM_OUT)
                {
                    cmd = G2LIVE_PTZ_COMMAND.COMMAND.PTZ_STOP_ZOOM;
                    arg = 0;
                }
                else if (cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_FOCUS_FAR ||
                         cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_FOCUS_NEAR)
                {
                    cmd = G2LIVE_PTZ_COMMAND.COMMAND.PTZ_STOP_FOCUS;
                    arg = 0;
                }
                else if (cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_IRIS_CLOSE ||
                         cmd == G2LIVE_PTZ_COMMAND.COMMAND.PTZ_IRIS_OPEN)
                {
                    cmd = G2LIVE_PTZ_COMMAND.COMMAND.PTZ_STOP_IRIS;
                    arg = 0;
                }
            }
            ...
            pc._command = (int)cmd;
            pc._argument = arg;
            pc._stream_id = stream_id;
            pc._reserved = 0;
            return _adaptor.set_ptz_command(_channel, camera, ref pc);
        }
        public bool feature_control_ptz_menu(int camera, G2LIVE_PTZ_COMMAND.COMMAND cmd, int index)
        {
            G2LIVE_PTZ_COMMAND pc = new G2LIVE_PTZ_COMMAND();
            pc._command = (int)cmd;
            pc._argument = index;
        ...
        private void on_feature_control_PTZ_move(object sender, EventArgs e)
        {
            ...
            feature_control_PTZ(camera, stream_id, cmd, param);
        }
        private void on_feature_control_PTZ_function(object sender, EventArgs e)
        {
            feature_PTZ.PARAM param = feature_PTZ.PARAM.RELEASE;
            G2LIVE_PTZ_COMMAND.COMMAND cmd = G2LIVE_PTZ_COMMAND.COMMAND.PTZ_STOP_MOVE;
            ...
            {
                cmd = G2LIVE_PTZ_COMMAND.COMMAND.PTZ_FOCUS_ONEPUSH;
            }
            else if (sender == CONTROL_PTZ_MENU)
            {
                if (_feature_PTZ.form_menu == null)
                {
                    ...
                    _feature_PTZ.form_menu.camera = camera;
                    _feature_PTZ.form_menu.handler += new System.EventHandler(on_feature_control_PTZ_menu);
                    _adaptor.request_ptz_menu(_channel, _feature_PTZ.form_menu.camera);
                }
                return;
            }
            else if (sender == CONTROL_PTZ_OSD)
            {
                if ((_adaptor.get_status_ptz_function(_channel, camera) & G2DEVICE_STATUS.PTZ_FUNCTION.OSD_MENU_OFF_NONE) != 0)
                {
                ...
                cmd = _feature_PTZ.on_OSD ? G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MENU_ON : G2LIVE_PTZ_COMMAND.COMMAND.PTZ_MENU_OFF;

                feature_control_ptz_menu(camera, cmd, 0);
                return;
            }
            else
            {
        ...
        private void on_feature_control_PTZ_preset(object sender, EventArgs e)
        {
            ...
            if (_feature_PTZ.form_preset == null)
            {
                ...
                _feature_PTZ.form_preset.camera = camera;
                _feature_PTZ.form_preset.handler_move += new System.EventHandler(on_feature_control_PTZ_preset_move);
                _adaptor.request_ptz_preset(_channel, camera);
            }
        }
        private void on_feature_control_PTZ_preset_move(object sender, EventArgs e)
        {
        ...
        private void on_feature_control_PTZ_menu(object sender, EventArgs e)
        {
            form_ptz_menu.event_args args = e as form_ptz_menu.event_args;
            feature_control_ptz_menu(args.camera, args.command, args.i);
        }
        private void on_feature_control_PTZ_command_control(object sender, EventArgs e)
        {
            if (_feature_PTZ.form_command == null)
            {
                _feature_PTZ.form_command = new form_ptz_command();
                _feature_PTZ.form_command.channel = _channel;
                _feature_PTZ.form_command.camera = _screen.selected_pane_channelext();
                _feature_PTZ.form_command.handler_set += new System.EventHandler(on_feature_control_PTZ_command_control_direct);
                _adaptor.send_command_control_ptz_status(_channel, _feature_PTZ.form_command.camera);
            }
            return;
        }
        private void on_feature_control_PTZ_command_control_direct(object sender, EventArgs e)
        {
            form_ptz_command form = sender as form_ptz_command;
            if (form != null)
            {
                G2LIVE_COMMAND_CONTROL_PTZ data = form.data;
                int channel = form.channel;
                if (_adaptor.is_connected(channel))
                {
                    _adaptor.send_command_control_ptz(channel, form.camera, ref data);
                }
            }
        }
