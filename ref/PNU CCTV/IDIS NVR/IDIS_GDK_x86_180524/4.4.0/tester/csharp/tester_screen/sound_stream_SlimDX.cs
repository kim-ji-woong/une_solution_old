...
namespace GDK_tester
{
    ...
    public class sound_stream
    {
        ...
        public void create(Control focus)
        {
            ...
            _device.SetCooperativeLevel(focus.Handle, CooperativeLevel.Priority);
        }
        public void cleanup()
        {
        ...
        public void set_sound_format(ref G2AUDIO_DEVICE_INFO di)
        {
            ...
            format.BlockAlignment = (short)(di._internal.channel * (di._internal.bits_per_sample / 8));
        ...
        public bool create_sound_buf(int buf_size, int buf_count)
        {
            ...
            desc.SizeInBytes = _buf_size_ds;
            desc.Format = _format;
            desc.Flags = BufferFlags.GetCurrentPosition2 | BufferFlags.ControlFrequency | BufferFlags.ControlVolume | BufferFlags.GlobalFocus;
            
            _ds_buf = new SecondarySoundBuffer(_device, desc);
        ...
        public void write(byte[] data, int len, G2SPOT spot, long pts)
        {
            ...
            _ds_buf.Write(_sound_data, _offset_next_write, LockFlags.None);
            _sound_data_len = 0;
        ...
        public void fill_buf()
        {
            ...
            {
                _ds_buf.Write(new byte[_buf_size_ds], 0, LockFlags.None);
            }
            else
            {
                _ds_buf.Write(_sound_data, 0, LockFlags.None);
                _sound_data_len = 0;
            }
        }
        ...
        public void play()
        {
            ...
            _ds_buf.Play(0, PlayFlags.Looping);
        ...
        public bool restore()
        {
            ...
            if (_ds_buf.Status == BufferStatus.BufferLost)
            {
                do
                {
                    _ds_buf.Restore();
                }
                while (_ds_buf.Status == BufferStatus.BufferLost);
                return true;
            }
            return false;
        }
        public void reset()
        {
            ...
            _ds_buf.CurrentPlayPosition = 0;
            _offset_next_write = 0;
            _ds_buf.Write(new byte[_buf_size_ds], 0, LockFlags.EntireBuffer);
        }
        ...
        public bool is_play()
        {
            return is_valid() ? _ds_buf.Status == BufferStatus.Playing : false;
        }

        public int pos_current()
        {
            return is_valid() ? (_ds_buf.CurrentPlayPosition / _buf_size) % _buf_count : -1;
        }
        ...
        public bool spot_current(out spot_element spot, out int play_msec)
        {
            ...
            int pos = _ds_buf.CurrentPlayPosition;
        ...
        protected DirectSound _device;
        protected SecondarySoundBuffer _ds_buf;
