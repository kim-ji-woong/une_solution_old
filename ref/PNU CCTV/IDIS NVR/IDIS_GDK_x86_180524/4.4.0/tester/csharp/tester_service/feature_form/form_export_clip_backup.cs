...
{
    public partial class form_export_clip_backup : Form, g2backup_saver_listener
    {
        ...
        public form_export_clip_backup()
        {
            InitializeComponent();
        ...
        public void set_invoke_saver(G2GUID service, G2GUID site)
        {
            _adaptor = new g2backup_saver();
            _adaptor.set_listener(this);
            _adaptor.startup(1);
            G2CONNECT_RES res;
            _channel = _adaptor.connect(ref service, ref site, out res);
            _site = site;
        }
        ...
        private g2backup_saver _adaptor;
        ...
        public void imp_on_receive_scope_list(int channel, G2SCOPE[] scopes)
        {
            ...
            uint total = (uint)Math.Min(free_space, 64L * 1024L * 1024L * 1024L);
            ...
            _adaptor.request_clipcopy_measure_size(channel, ref _channel_set, ref scope, total);
        }
        ...
        #region g2backup_saver_listener 멤버

        public void on_g2backup_saver_connected(int handle, int channel)
        {
            _adaptor.request_backup_site(channel, ref _site);
        }

        public void on_g2backup_saver_disconnected(int handle, int channel, G2DISCONNECT_REASON.TYPE reason)
        {
            _channel = -1;
        ...
        public void on_g2backup_saver_receive_backup_site_result(int handle, int channel, ref G2BACKUP_SITE_RESULT result)
        {
            if (result.result != G2BACKUP_SITE_RESULT.RESULT.SITE_OK)
            {
                MessageBox.Show(this, "not ready backup site.", "Clip-copy", MessageBoxButtons.OK, MessageBoxIcon.Error);
                _adaptor.disconnect(channel);
            }
        }

        public void on_g2backup_saver_receive_record_channels(int handle, int channel, G2BACKUP_CHANNEL_INFO[] channels)
        {
            Debug.WriteLine("callback [on_g2backup_saver_receive_record_channels] is not implemented.");
        }

        public void on_g2backup_saver_receive_response_no_recorded_data(int handle, int channel, G2BACKUP_CHANNEL_INFO[] channels)
        {
            Debug.WriteLine("callback [on_g2backup_saver_receive_response_no_recorded_data] is not implemented.");
        }

        public void on_g2backup_saver_receive_frame_data(int handle, int channel, ref G2FRAME frame)
        {
            Debug.WriteLine("callback [on_g2backup_saver_receive_frame_data] is not implemented.");
        }

        public void on_g2backup_saver_receive_notify_out_of_scope(int handle, int channel, G2PLAYER.OUT_OF_SCOPE playtype)
        {
            Debug.WriteLine("callback [on_g2backup_saver_receive_notify_out_of_scope] is not implemented.");
        }

        public void on_g2backup_saver_receive_notify_player_error(int handle, int channel, G2PLAYER.PLAYER_ERROR errorcode)
        {
            Debug.WriteLine("callback [on_g2backup_saver_receive_notify_player_error] is not implemented.");
        }

        public void on_g2backup_saver_receive_scope_list(int handle, int channel, G2SCOPE[] scopes)
        {
        ...
        public void on_g2backup_saver_receive_no_recorded_data(int handle, int channel)
        {
            Debug.WriteLine("callback [on_g2backup_saver_receive_no_recorded_data] is not implemented.");
        }

        public void on_g2backup_saver_receive_clipcopy_size(int handle, int channel, G2CLIPCOPY_STATUS.TYPE status, ref G2CLIPCOPY_SIZE_INFO csi)
        {
            if (_cancel_status == CANCEL_STATUS.SEND_CANCELED) return;
            if (_cancel_status == CANCEL_STATUS.CANCELED)
            {
        ...
        public void on_g2backup_saver_receive_clipcopy_data(int handle, int channel, ulong offset, uint size, IntPtr data, uint progress)
        {
            if (_cancel_status == CANCEL_STATUS.SEND_CANCELED) return;
            if (_cancel_status == CANCEL_STATUS.CANCELED)
            {
        ...
        public void on_g2backup_saver_receive_clipcopy_canceled(int handle, int channel)
        {
            post_canceled(false);
        }

        public void on_g2backup_saver_receive_clipcopy_set_password(int handle, int channel, uint result)
        {
            if (_cancel_status == CANCEL_STATUS.SEND_CANCELED) return;
            if (_cancel_status == CANCEL_STATUS.CANCELED)
            {
        ...
        public void on_g2backup_saver_receive_clipcopy_job_started(int handle, int channel, G2CLIPCOPY_JOB.TYPE job, uint num, uint total)
        {
            if (_cancel_status == CANCEL_STATUS.SEND_CANCELED) return;
            if (_cancel_status == CANCEL_STATUS.CANCELED)
            {
        ...
        public void on_g2backup_saver_receive_clipcopy_job_finished(int handle, int channel, G2CLIPCOPY_JOB.TYPE job, uint num, uint total)
        {
            if (_cancel_status == CANCEL_STATUS.SEND_CANCELED) return;
            if (_cancel_status == CANCEL_STATUS.CANCELED)
            {
