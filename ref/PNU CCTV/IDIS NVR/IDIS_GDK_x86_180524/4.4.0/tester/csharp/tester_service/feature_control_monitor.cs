...
namespace GDK_tester
{
    public partial class form_admin
    {
        ...
        public bool string_abbereviate_channel_set(ref byte[] element, ref string str, G2MONITORING_DEVICE_STATUS.CHECK_SYSTEM compare_target)
        {
            ...
            for (int i = 0; i < element.Length; i++)
            {
                if (element[i].Equals((byte)compare_target))
                {
                    if (count == 0)
                    {
                        str += ((str.Length == 0) ? (i + 1).ToString() : ("," + (i + 1).ToString()));
                    }
                    ++count;
                }
                else 
        ...
        public void parse_health_info_array(ref G2MONITORING_DEVICE_HEALTH health, out statusElementHealth.element element)
        {
            ...
            G2GUIDLIST groupGuids = new G2GUIDLIST();
            g2admin.get().get_group_guid_from_device_guid(ref health._site, groupGuids);
            if (groupGuids.Count > 0)
            {
                G2GUID groupGuid = groupGuids[0];
                G2DEVICE_GROUP group;
                g2admin.get().get_device_group_info(ref groupGuid, out group);
            ...
            if (proceed)
            {
                ...
                if (health._record._begin.valid != true || health._record._end.valid != true)
                {
                    element._recordPeriod = " - - - - - ";
                }
                else
