// search_g2_event_list.cpp : implementation file
//

#include "stdafx.h"
#include "search_g2_event_list.h"
#include "listener/listener_search_g2_event_list.h"
...
search_g2_event_list::search_g2_event_list(void)
    : _parent(NULL)
    , _listener(NULL)
    , _initialized(false)
    , _enable(false)
...
IMPLEMENT_DYNAMIC(search_g2_event_list, CWnd)

void search_g2_event_list::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(search_g2_event_list, CWnd)
    ON_WM_CREATE()
    ON_WM_DESTROY()
    ON_WM_SIZE()
    ON_WM_ENABLE()
...
int search_g2_event_list::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void search_g2_event_list::OnDestroy()
{
    CWnd::OnDestroy();
...
void search_g2_event_list::OnSize(UINT nType, int cx, int cy)
{
...
void search_g2_event_list::OnEnable(BOOL bEnable)
{
...
void search_g2_event_list::OnTimer(UINT_PTR nIDEvent)
{
    ...
    if (nIDEvent == timer_::id_::LOAD_EVENT_IMAGE) {
        ...
        if (selected >= 0 && g2_time_to_time32_t(&spot._time) > 0) {
            if (_listener) {
                _listener->on_request_load_event_image_search_g2(spot);
            }
        }
    }
}
...
bool search_g2_event_list::create(CWnd* parent, const CRect& rect, unsigned int id)
{
    if (parent == NULL ||
...
void search_g2_event_list::set_listenr(listener_search_g2_event_list* listener)
{
    _listener = listener;
}

void search_g2_event_list::set_host_id(short hostId)
{
...
void search_g2_event_list::clear(bool post /*= false*/)
{
...
void search_g2_event_list::set_enable(bool enable)
{
...
void search_g2_event_list::show(bool show)
{
    G2RETURN_IF_FAIL(GetSafeHwnd());
...
void search_g2_event_list::focus(void)
{
...
void search_g2_event_list::initialize(void)
{
    CRect rect;
    GetClientRect(&rect);
    rect.right -= 50;
...
void search_g2_event_list::finalize(void)
{
    _initialized = false;
...
void search_g2_event_list::OnBtnNext(void)
{
    if (_listener) {
        _listener->on_request_more_event_search_g2();
    }
}

void search_g2_event_list::OnLstItemChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
...
LRESULT search_g2_event_list::on_receive_result_query(WPARAM wParam, LPARAM lParam)
{
    ...
    if (mode == G2SEARCH_G2_QUERY::EVENT) {        
        ...
        for (std::vector<G2EVENT>::const_iterator itr(_g2eventLog.begin());
            itr != _g2eventLog.end();
            ++itr, ++logIndex) {
                const G2EVENT& log = *itr;
                g2_get_string_event_type_g2(log._type, &strEvent);
                index = _list.InsertItem(0, strEvent._string);

                if (index != -1) {
                    _list.SetItemText(index, 1, log._data._string);
                    _list.SetItemText(index, 2, CTime(g2_time_to_time32_t(&log._spot._time)).Format(_T("%Y-%m-%d %H:%M:%S")));
                    _list.SetItemData(index, (DWORD_PTR)new item_data(logIndex, log._spot));
                }
        }
        std::vector<G2EVENT>().swap(_g2eventLog);
        lock.free();
    }
    else if (mode == G2SEARCH_G2_QUERY::TEXT_IN) {
        column.mask = LVCF_FMT;
        if (_list.GetColumn(column_::CONTENT, &column)) {
            column.mask = LVCF_TEXT;
        ...
        for (std::vector<G2EVENT>::const_iterator itr(_g2eventLog.begin());
            ...
            if (index != -1) {
                _list.SetItemText(index, 1, CString(log._text_in._data));
                _list.SetItemText(index, 2, CTime(g2_time_to_time32_t(&log._spot._time)).Format(_T("%Y-%m-%d %H:%M:%S")));
                _list.SetItemData(index, (DWORD_PTR)new item_data(logIndex, log._spot));
            }
        }
...
LRESULT search_g2_event_list::on_clear_list(WPARAM wParam, LPARAM lParam)
{
    clear();
...
void search_g2_event_list::insert(const std::vector<G2EVENT>& info, int mode /*= G2SEARCH_G2_QUERY::EVENT*/)
{
    G2RETURN_IF_FAIL(info.empty() != true);

    g2::scoped_criticalsection lock(_syncEvent);
    _g2eventLog.insert(_g2eventLog.begin(), info.begin(), info.end());
    lock.free();

    PostMessage(message_::UM_RECEIVE_RESULT_QUERY, WPARAM(mode));
}
