...
bool g2decoder::decompress(G2DECODER_VIDEO_PARAM_V3& param, G2DECODER_VIDEO_RESULT_V3* res)
{
    return g2_decoder_video_decompress_v3(_handle, &param, res);
}
...
int g2decoder::picture_buf_len(int width, int height, int pix_format)
{
    return g2_decoder_picture_buf_len(width, height, pix_format);
}

void g2decoder::picture_fill(G2PICTURE* pic, int pix_format, int width, int height, unsigned char* buf, int len)
{
    g2_decoder_picture_fill(pic, pix_format, width, height, buf, len);
}

bool g2decoder::picture_scale(G2PICTURE* dst, int dst_pix_format, int dst_cx, int dst_cy, unsigned char* buf, int len, int filter, const G2PICTURE* src, int src_pix_format, int src_cx, int src_cy, int color_space, int color_range)
{
    return g2_decoder_picture_scale(dst, dst_pix_format, dst_cx, dst_cy, buf, len, filter, src, src_pix_format, src_cx, src_cy, color_range, color_space);
}
