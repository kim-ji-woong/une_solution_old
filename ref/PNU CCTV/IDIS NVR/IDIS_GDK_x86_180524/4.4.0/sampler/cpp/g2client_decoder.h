...
public:
    ...
    bool decompress(G2DECODER_VIDEO_PARAM_V3& param, G2DECODER_VIDEO_RESULT_V3* res);
...
public:
    static int picture_buf_len(int width, int height, int pix_format);
    static void picture_fill(G2PICTURE* pic, int pix_format, int width, int height, unsigned char* buf, int len);
    static bool picture_scale(G2PICTURE* dst, int dst_pix_format, int dst_cx, int dst_cy, unsigned char* buf, int len, int filter, const G2PICTURE* src, int src_pix_format, int src_cx, int src_cy, int color_space, int color_range);
