...
struct G2DEVICE_STATUS {
    ...
    enum PTZ_FUNCTION {
		...
		PTZ_FUNC_RELATIVE_MOVE_IDIS = 16384
    };
    enum COVERT {
        NORMAL = 0,
        COVERT1,
...
struct G2DEVICE_STATUS_IP_CAMERA_INFO {
    ...
    enum REGION_COMPOSITION {
        ORIGINAL = 0,//< Original 1x1
        DEPRECATED,
        P1,          //< Panorama
        P1R2,        //< Panorama(Top) + ROIs (Left, Right)
        P2,          //< Panorama(Top and Bottom)
        R4,          //< ROIs 2x2
        O1R7,        //< Original(Left, Top) + ROIs 7
    };
    struct DEWARPING_DETAIL {
        unsigned short _region_composition;
        int _original_region;
        unsigned int _eptz_function;
        signed char _mount_type;
    };

    signed char _dewarping_status;
    DEWARPING_DETAIL _dewarping_detail;
};
...
struct G2EVENT {
    ...
    enum TYPE {
        ...
        VA_PEOPLE_COUNT_SETUP_CHANGED   = 800000,
        VA_PEOPLE_COUNT_IN              = 800001,
        VA_PEOPLE_COUNT_OUT             = 800002,
        VA_QUEUE_MAX_OBJECT_EXCEED_ON   = 800003,
        VA_QUEUE_MAX_OBJECT_EXCEED_OFF  = 800004,
        VA_QUEUE_MAX_WAIT_TIME_EXCEED_ON = 800005,
        VA_QUEUE_MAX_WAIT_TIME_EXCEED_OFF = 800006,
        VA_INIT                         = 800007,
        VA_LOSS_ON                      = 800008,
        VA_LOSS_OFF                     = 800009,
        ANPR_ON                         = 810001,
...
struct G2EVENT_INFO {
    ...
    enum TYPE_LEVEL2 {
        ...
        L2_VA_EVENT_BEGIN = 3499,
        L2_VA_PEOPLE_COUNT_SETUP_CHANGED = 3500,
        L2_VA_PEOPLE_COUNT_IN,
        L2_VA_PEOPLE_COUNT_OUT,
        L2_VA_QUEUE_MAX_OBJECT_EXCEED_ON,
        L2_VA_QUEUE_MAX_OBJECT_EXCEED_OFF,
        L2_VA_QUEUE_MAX_WAIT_TIME_EXCEED_ON,
        L2_VA_QUEUE_MAX_WAIT_TIME_EXCEED_OFF,
        L2_VA_INIT,
        L2_VA_LOSS_ON,
        L2_VA_LOSS_OFF,
        L2_VA_EVENT_END,

        L2_PIR_EVENT_BEGIN = 3599,
        L2_PIR_ON = 3600,
        L2_PIR_OFF,
        L2_PIR_IGNORED_PIR_ON,
        L2_PIR_EVENT_END,

        L2_ANPR_EVENT_BEGIN = 3699,
        L2_ANPR_ON = 3700,
        L2_ANPR_OFF,
        L2_ANPR_EVENT_END,
...
struct G2FISHEYE_CONTROL {
    enum {
        FISHEYE_COMMAND_UNKNOWN            = -1,
        FISHEYE_COMMAND_MOVE_PAN_TILT      = 0,
        FISHEYE_COMMAND_CHANGE_DEWARP_TYPE = 1,
        FISHEYE_COMMAND_SAVE               = 10,
        FISHEYE_COMMAND_RESET              = 100,
    };
    enum {
        DEWARP_TYPE_UNKNOWN   = -1,
        DEWARP_TYPE_NORMAL    = 0,
        DEWARP_TYPE_PANORAMA  = 2,
        DEWARP_TYPE_SUBREGION = 3,
    };
    int _camera;
    int _command;
    short _sub_region;
...
struct G2PICTURE {
    ...
    unsigned char* data[NUM_DATA_POINTERS];
