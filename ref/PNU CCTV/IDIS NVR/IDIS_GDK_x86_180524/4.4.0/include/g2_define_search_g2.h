...
struct G2SEARCH_G2_SOLE_PLAYER_OPTIONS {
    ...
    unsigned int _anpr_load_tolerance;
    bool _segment_less_search;
    unsigned int _key_lookup_min_threshold_for_move_to;     // in seconds. for HOUR_PRECISION, MINUTE_PRECISION, SECOND_PRECISION
    unsigned int _key_lookup_max_threshold_for_move_to;     //                 EVENT_PRECISION, TEXT_IN_PRECISION, FRAME_PRECISION
    unsigned int _key_lookup_threshold_for_backward_play;   // in seconds.
    unsigned int _key_skip_interval;    // in miliseconds
    unsigned char _key_frame_only_type;
};
