...
public class SelectionModel : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        Outline outline = mTextOwner.AddComponent<Outline>();
    ...
    {
        Debug.logger.Log("SetTextColor : " + color.ToString());
        MeshRenderer render = mTextOwner.GetComponent<MeshRenderer>();
        if (render != null)
        {
    ...
    void Update()
    {
        ...
        if (boxCollider != null)
        {
           ...
           // float distance = dir.magnitude;
            float distance = Vector3.Distance(Camera.main.transform.position, textPos2);
            if (distance < ModelManager.Instance.TextLODDistance)
            {
                ...
                if (mr2 != null )
                {
                    ...
                    mNameText.transform.gameObject.GetComponent<Outline>();
