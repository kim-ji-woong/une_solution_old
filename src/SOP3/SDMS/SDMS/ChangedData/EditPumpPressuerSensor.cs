...
namespace SDMS
{
    public class EditPumpPressuerSensor : ChangedData
    {
        ...
        private bool Insert(DBUtility.WebDBManager dbMgr)
        {
			...
			strSQL = string.Format("Insert into PumpPressureSensor (ID, Name, PositionName, X, Y, Z, ZoneID, IsIndoor, Description) values ({0}, '{1}', '{2}', {3}, {4}, {5}, {6}, {7}, NULL)",
				nID, "PumpPressureSensor", zone.BroadcastName, poi.X, poi.Y, poi.Z, zone.ID, poi.IsIndoor ? 1 : 0);
			...
			strSQL = string.Format("Insert into SensorZone (ID, Type, Connected, EquipZoneID, Data, Description, OrgSensorID, Zone) values ({0}, {1}, {2}, {3}, {4}, '{5}', {6}, {7})",
			   nSensorZoneID, "3", "NULL", m_Sensor.EquipZoneID, "NULL", m_Sensor.Description, nID, zone.ID);
        ...
        override public bool Update(DBUtility.WebDBManager dbMgr)
        {
            ...
            if (m_zone != null)
            {
                strValue = string.Format("PositionName = '{0}', ZoneID = {1}, IsIndoor = {2}", m_zone.BroadcastName, m_zone.ID, m_Sensor.POI.IsIndoor ? 1 : 0);
