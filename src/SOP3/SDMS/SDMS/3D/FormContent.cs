...
namespace SDMS
{
    public partial class FormContent : Form
    {
        ...
        {			
            ...
            if (!File.Exists(m_strOutDaeName) || ModelManager.Instance.ExtractOutside == true)
            {
                try
                {
			...
			if (!File.Exists(szFloorFile) || ModelManager.Instance.ExtractInside == true)
            {
                try
                {
