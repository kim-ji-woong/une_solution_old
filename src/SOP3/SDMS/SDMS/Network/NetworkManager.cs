...
namespace SDMS
{
    public class NetworkManager
    {
		...
		private ClientProvider m_providerSub = null;
        ...
        public void RecvLog(byte[] bytes, int nLine)
        {
            ...
            if (bytes[0] != TCP_ID.ARE_YOU_THERE || !m_exceptPingLog)
            {
                ...
                string strLog = string.Format("RecvMessage : Header({0}), Length({1}), {2}", (int)nHeader, (int)bytes.Length, nLine);
                string strBytes = "";
			...
			lock (this)
			{
				...
				if (nResult > 0)
				{
					...
					if (bytes[0] != TCP_ID.I_AM_HERE || !m_exceptPingLog)
					{
                        ...
                        string strLog = string.Format("SendMessage : {0} Header({1}), Length({2}), {3} ", 
        ...
        protected NetworkManager()
        {
            ...
            string strServerURL = DBUtility.RegUtil.ReadRegValue("Server Connection Info", "webserver_url");
        ...
        private void DeleteLog()
        {
            try
            {
                ...
                string szFileName = System.IO.Path.GetFileName(strPath);

                string[] arrFiles = System.IO.Directory.GetFiles(szParentPath + "\\logs");
                                
                string strKey = "SDMSClient.log-";
                if (szFileName.IndexOf("1") != -1)
                {
                    strKey = "SDMSClient_1.log-";
                }
                ...
                foreach (string strFile in arrFiles)
                {
                    ...
                    {
                        WriteLineLog("DeleteLog : " + strFile.ToString());
            ...
            {
                WriteLineLog("Log Delete Error : " + e.Message);
                WriteLineLog(e.StackTrace);
            }
        }
        ...
        private void CheckValidProcess(ref DateTime dtPrev)
        {
            ...
            {
                //ConnectionLogEx.Instance.WriteLine("CheckValidProcess");
                try
                {
		...
		public void SendChangeEquipZoneCCTV(int nEquipZoneID)
		{
            ...
            m_provider.SendChangedConfig(TCP_CLIENT.SDMS_CLIENT, SDMSConfig.GetPropertyName(SDMSConfig.ConfigType.EQUIPZONE_CCTV), nEquipZoneID.ToString());
        ...
        public void SendChangeFacilityManager()
        {
            ...
            int nChangedConfig = (int)SDMSConfig.ConfigType.ENTIRE_FACILITY_MANAGER | (int)SDMSConfig.ConfigType.EQUIPZONE_FACILITY_MANAGER | (int)SDMSConfig.ConfigType.BUILDING_FACILITY_MANAGER;
            m_provider.SendChangedConfig(TCP_CLIENT.SDMS_CLIENT, SDMSConfig.PropertyName, nChangedConfig.ToString());
            /*lock (this)
            {
		...
		public void SendRequestRestore()
		{
			if (m_provider == null)
				return;
