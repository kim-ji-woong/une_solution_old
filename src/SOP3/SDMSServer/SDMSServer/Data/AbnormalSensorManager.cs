...
namespace SDMSServer
{
    public class AbnormalSensorManager : IDisposable
    {
		...
		{
			dbManager = FormMain.Instance.DBManager;
		}
        public void Dispose()
        {
            Stop();
		...
		{
			string szSQL = "SELECT ID, AbnormalSensorDetectPolicy, IgnoreDurate FROM OptionSDMS where ID = 1";
			...
			else
			{
				...
				for (int i = 0; i < nCount - 2; i += 3)
				{
					...
					int nDP = WebDBManager.GetIntField(arResult[i + 1].ToString(), -1);
					int nDur = WebDBManager.GetIntField(arResult[i + 2].ToString(), -1);
