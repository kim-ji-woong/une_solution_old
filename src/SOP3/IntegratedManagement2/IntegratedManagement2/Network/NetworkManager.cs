...
namespace IntegratedManagement2
{
    public class NetworkManager
    {
		...
		public bool RegisterUser(int nMemberID, string szID, string szPass, string szNickName)
		{
			if (m_provider != null)
			{
				return m_provider.SendRegisterUser(nMemberID, szID, szPass, szNickName);
			}
			return false;
		}
        ...
        public NetworkManager(DBUtility.WebDBManager dbMgr)
        {
            ...
            try
            {
                string strServerURL = RegUtil.ReadRegValue("Server Connection Info", "webserver_url");
                m_dbMgr = dbMgr;
