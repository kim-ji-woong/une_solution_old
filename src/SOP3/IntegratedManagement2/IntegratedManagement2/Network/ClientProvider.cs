    ...
    public class ClientProvider : ClientServiceProvider
    {
        ...
        private void ProcessServerCommand(ArrayList arrDatas)
        {
            ...
            if (nCommand == ServerCommandType.RUN_SDMS)
                ...
                FormMain.Instance.CheckNUpdateSystem(null);
        }
		...
		public bool SendRegisterUser(int nMemberID, string szID, string szPass, string szNickName)
		{
			if (this.IsConnected == false)
			{
				return false;
			...
			byte[] bytes = new byte[dataBytes.Length + dataBytes1.Length + dataBytes2.Length + dataBytes3.Length + 6];
