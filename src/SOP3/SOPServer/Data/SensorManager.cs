...
namespace SDMSServer
{
	public class SensorManager
	{
		...
		public int ProcessSensorData(byte[] bytesSensorData, out int outSensorID, out int data, out bool bconnected, ref int nPrevSensorHistoryID)
		{
            ...
            int nEquipZoneID = BitConverter.ToInt32(bytesSensorData, 20);
            int nSensorData = BitConverter.ToInt32(bytesSensorData, 29);
        ...
        public void ReadSensorHistory(ServiceProvider provider)
        {
            ...
            szText += "         WHERE szh2.Id = srh2.SensorHistoryID and srh2.ReactionType = 0 ) ";
            ...
            /*string strSQL = string.Format("select srh.ID, srh.SensorHistoryID, srh.ReactionType, srh.Time, srh.Message, srh.Param1, srh.Param2, szh.SensorID " +
			"from SensorReactionHistory as srh, SensorZoneHistory as szh where SensorHistoryID > {0} and srh.SensorHistoryID = szh.ID and "+
			"srh.SensorHistoryID not in (select SensorHistoryID from SensorReactionHistory where ReactionType in ( 50, 23, 21, 33)) order by SensorHistoryID",
