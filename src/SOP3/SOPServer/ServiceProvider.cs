...
namespace SDMSServer
{
    ...
    public class ServiceProvider : TcpServiceProvider
    {
        ...
        private string m_strSMSCaller = "07088982203";
        ...
        public void RecvLog(byte[] bytes, ConnectionState state)
        {
            ...
            if (bytes[0] != TCP_ID.I_AM_HERE || !m_exceptPingLog)
            {
                ...
                if (data != null)
                {
					...
					else if (data.Type == ClientData.ClientType.SOP_MONITOR2)
        ...
        private void ReadOption()
        {
            ...
            m_strSMSCaller = getinivalue("Server Connection Info", "sms_caller", strFilePath);
		...
		{            
            ...
            ArrayList arrOperatorPhoneNumbers = GetOperatorPhoneNumber(log);
        ...
        public void SendReciverState(int nReciver, bool bConnected, ClientData.ClientType type, bool noLock = false, bool noOnDropConnection = false)
        {
            ...
            if (noLock)
                SendReceiverState(bytes, type, noLock, noOnDropConnection);
            else
            {
                ICollection<ConnectionState> arClients = null;
                DdMonitor.Enter(m_arrClients, true);
        ...
        public void SendSensorZoneData(int nData, int nSensorID, ClientData.ClientType type, bool noLock = false)
        {
            ...
            //    try
            //    {
            //        SendSensorZoneData(bytes, type, noLock);
            //    }
            //    catch (Exception e)
            //    {
            //        ConnectionLog.Instance.WriteLine(e.StackTrace);
            //    }
            //}

            SendClientData(bytes, type, true);

        }

        private void SendSensorZoneData(byte[] bytes, ClientData.ClientType type, bool noLock)
        {
        ...
        public void SendClearDetectReport(int nSensorHistoryID, ClientData.ClientType type, bool writeClearLog = true, bool noLock = false)
        {
            if (writeClearLog)
        ...
        private void SendClearDetectReport(byte[] bytes, ClientData.ClientType type, bool noLock)
        {
            SendClientData(bytes, type, true);
        }
        ...
        {            
            ...
            SendClearDetectReport(nHistoryID, ClientData.ClientType.SOP_SIMULATOR, writeClearLog);            
        }
		...
		private void SendDisconnectAllReciverState(bool noLock, bool noOnDropConnection)
		{
			...
			foreach(Reciver reciver in arRecivers)
			{
                ...
                //SendReciverState(reciver.ID, false, ClientData.ClientType.SDMS_CLIENT, noLock, noOnDropConnection);
			}		
		}
		...
		{
            SendClientData(bytes, ClientData.ClientType.SOP_SIMULATOR, true);
		}
    }
