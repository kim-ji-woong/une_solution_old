...
namespace SamDuty
{
    class DataManager
    {
        ...
        public void SetNightDuty(int idx, int nID, int nTeamID, DateTime dtNow)
        {
            string nCur = string.Format("{0} {1:00}:{2:00}:{3:00}", dtNow.ToShortDateString(), dtNow.Hour, dtNow.Minute, dtNow.Second);
            string szSQL1 = "SELECT max(ID)  FROM Duty";
            ArrayList arResult = m_dbMgr.GetResultData(szSQL1, 0);
            if (arResult == null || arResult.Count == 0)
                return;

            int maxID = WebDBManager.GetIntField(arResult[0].ToString(), 0) + 1;
            string szSQL = string.Format("INSERT INTO Duty (ID, MemberID, TeamID, InsertTime, Description) VALUES ({0}, {1} ,{2}, '{3}', NULL )", maxID, nID, nTeamID, nCur);
            m_dbMgr.GetResultData(szSQL, 0);
        }

        public DataCompanyMember GetCompanyMember(int nID)
        {
            foreach (DataCompanyMember member in m_arrCompanyMember)
            {
                if (member.ID == nID)
                    return member;
            }
        ...
        public DataTeam GetRegularTeam(int nID)
        {
            foreach (DataTeam team in m_arRegularTeam)
            {
                if (team.ID == nID)
                    return team;
            }
    ...
    class SendingMember
    {
        ...
        private DataCompanyMember member = null;
