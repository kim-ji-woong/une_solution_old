...
namespace BroadcastServer
{
	...
	class DBManager
	{
		...
		public BroadcastServer.Utility Ini
		{
			get { return m_ini; }
			set { m_ini = value; }
		}
		...
		public void AddMessage(BroadcastMessage msg)
		{
			...
			string szSQL = string.Format("INSERT INTO Broadcast (Text, UseSiren, PlayOption, RepeatCount, AddTime, SiteID) VALUES('{0}','{1}','{2}','{3}','{4} {5:00}:{6:00}:{7:00}',{8})",
				msg.Message, msg.UseSiren, msg.PlayOption, msg.RepeatCount, nDate.ToShortDateString(), nDate.Hour, nDate.Minute, nDate.Second, m_nSiteID);
        ...
        {
            string szSQL = " SELECT BroadcastState.HOSTADDRESS, BroadcastState.HEARTBEAT, BroadcastState.BSTATE, BroadcastState.BDescription from BroadcastState WHERE SiteID = "+ m_nSiteID.ToString();
		...
		{
			string szSQL = " SELECT bs.HOSTADDRESS, bs.HEARTBEAT, bs.BSTATE, bs.BDescription from BroadcastState as bs WHERE SiteID = "+ m_nSiteID.ToString();
		...
		public void HeartBeat(int nState)
		{
			...
			string szSQL = string.Format("UPDATE BroadcastState SET HEARTBEAT= '{0} {1:00}:{2:00}:{3:00}', BSTATE ={4} WHERE ID = 1 and SiteID = {5}"
				, nDate.ToShortDateString(), nDate.Hour, nDate.Minute, nDate.Second, nState, m_nSiteID);
