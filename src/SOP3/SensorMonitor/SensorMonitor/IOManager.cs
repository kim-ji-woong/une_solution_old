...
//" FROM SensorZone as sz, EquipmentZone as ez WHERE sz.EquipZoneID = ez.ID and ez.SiteID = {0}";
		...
		{
            WebDBManager m_dbMgr = SOPMonitor.Instance.DbMgr;
            string strSQL = "select ID,Place, IP, MacAddr, Baudrate, Mode, FlowCtrl, Multiport, Timeout, Description from SensorServerInfo";// where SiteID =" + m_nSiteID.ToString();
		...
		{
            WebDBManager m_dbMgr = SOPMonitor.Instance.DbMgr;
            string strSQL = string.Format("select ID, TagNo, SensorName, SensorType, EquipZoneID, Description from SensorTagInfo where SensorServerID = {0}", reciver.ID);
