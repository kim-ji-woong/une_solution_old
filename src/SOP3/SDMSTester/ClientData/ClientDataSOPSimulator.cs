...
namespace SDMSServer
{
    public class ClientDataSOPSimulator : ClientData, ControlMonitoring.IControlClientOwner
    {
        ...
        private void ProcessConfirmTakeControl()
        {
            ...
            // this 객체 이외에 다음으로 우선권을 가진 Client를 찾는다.
            /*IControlClientOwner client = conMgr.GetNextControlOwner(this);

            if (client == null)
            {
                // 제어권을 원치 않는 Client 가운데에서 다음으로 우선권을 가진 Client를 찾는다.
                client = conMgr.GetMostHighLevelClient(this);
            }

            if (client != null)
            {
                // 해당 Client에게 제어권 부여
                ClientDataSOPSimulator clientData = (ClientDataSOPSimulator)client;
                SendGiveControl(clientData.ConnectionState);
            }*/
        }
        ...
        private void SendRequestControl(ConnectionState state, ControlClient data)
        {
            ...
            /*byte[] userIDBytes = ServiceProvider.MakeBytes(data.UserID);
            byte[] userNameBytes = ServiceProvider.MakeBytes(data.UserName);
