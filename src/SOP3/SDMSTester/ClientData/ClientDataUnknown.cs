...
namespace SDMSServer
{
    public class ClientDataUnknown : ClientData
    {
        ...
        private bool SetClientType(byte[] bytes, ConnectionState state)
        {
            ...
            NetworkServer.Instance.UpdateClientType(state);
            return true;
        }
        ...
        private ClientData NewClientData(ClientData.ClientType type)
        {
            switch (type)
            {
                case ClientType.SDMS_CLIENT:
                    return new ClientDataSDMS(this.m_provider);

                case ClientType.SOP_SIMULATOR:
                    return new ClientDataSOPSimulator(this.m_provider);

                case ClientType.SENSOR_SIMULATOR:
                    return new ClientDataSensorSimulator(this.m_provider);

                case ClientType.SOP_MONITOR:
                    return new ClientDataSOPMonitor(this.m_provider);

                case ClientType.SOP_RESOTRE:
                    return new ClientDataSOPRestore(this.m_provider);

                case ClientType.INTEGRATE_MANAGER:
                    return new ClientDataIntegrateManager(this.m_provider);

				case ClientType.SDMS_CLIENT_SECOND:
					return new ClientDataSDMSSub(this.m_provider);
            }
