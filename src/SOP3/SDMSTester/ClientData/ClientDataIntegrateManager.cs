...
namespace SDMSServer
{
    public class ClientDataIntegrateManager : ClientData
    {
		...
		private int JoinUser(ConnectionState state, byte[] bytes)
		{
			...
			string strSQL = "select id from SOPGenUser where UserID = '" + szMemberID + "'";
			...
			strSQL = string.Format("Insert into SOPGenUser (ID, MemberID, UserLevel, Password, UserID, NickName) values ({0}, {1}, {2}, '{3}', '{4}', '{5}')",
				nID, nMemberID, 2, szPass, szMemberID, szNickName);
		...
		private bool ChangePassword(ConnectionState state, byte[] bytes)
		{
			...
			string strSQL = "select Password from SOPGenUser where id = " + nUserID;
			ArrayList arrResult = dbMgr.GetResultData(strSQL, 0);
        ...
        private bool ChangeNickName(ConnectionState state, byte[] bytes, out string szNickName)
        {
            ...
            string strSQL = string.Format("Update SOPGenUser set NickName = '{0}' where id = {1}",
                szNickName, nUserID);
		...
		private bool CheckLoginUser(byte[] bytes, out int nID, out int MemberID, out string MemberName, out string szGenUser, out string szNickName)
		{
			...
			DBUtility.WebDBManager dbMgr = NetworkServer.Instance.DBManager;
			string szSQL = string.Format("select Password, gen.MemberID, cm.MemberName, gen.NickName, gen.ID from SOPGenUser as gen, CompanyMember as cm where gen.MemberID = cm.ID and UserID='{0}'", szUserID);
