...
namespace SDMSServer
{
	public class ClientDataSDMS : ClientData
	{
		...
		private bool GetBroadcastMessage(SensorReactionLog log, out string szBroadcastMessage, out int nRepeat, out bool bSiren)
		{
            ...
            // 화재 신고시 방송
			string strSQL = "select id, UseBroadcast, Message, UseSiren, RepeatCount from SDMSBroadcastConfig where SituationType = 1";
		...
		private void RunBroadcast(SensorReactionLog log)
		{
			...
			if (bResult)
			{
				if (BroadcastManager.Instance.IsEnabled(BroadcastManager.SituationType.REPORT_FIRE) == true)
				{
                    ...
                    BroadcastManager.Instance.AddSpeech(szBroadcastMsg, nRepeat, bUseSiren, BroadcastManager.SituationType.REPORT_FIRE);
				}
        ...
        protected override bool OnReceive(ConnectionState state, byte[] bytes, int nHeader, ArrayList arrDatas)
		{
			if (nHeader == TCP_ID.FIRE_DETECT_REPORT)
			{
				...
				{
					//if (m_provider.CheckSituationForManual(log.SensorHistoryID))
					//{
						// 수동 신고의 처리
						if (log.Param2 == "0")
						{
			...
			else
			{
				if (log.Param2 == "0")
				{
				    ...
				    if (equipZone != null)
				    {
						...
						//if (strReportFrom != "")
						//{
						//    log.Message = string.Format("[{0}]에서 [{1}]에 화재발생을 신고하였습니다", strReportFrom, szZoneName);
						//}
						//else
						{
