...
namespace SDMSServer
{
	public class LoginManager
	{
		...
		public bool IsValidUser(string szUserName, int nMemberID)
		{
			...
			string strSQL = "select j.LevelNo from CompanyMember as c, JobLevel as j where c.ID = '" + nMemberID.ToString() + "' and c.MemberName = '" + szUserName + "' and c.LevelID = j.ID";
        ...
        public LoginInfo FindLoginUser(int nID, string strUserName)
        {
            ...
            strSQL += "where sg.MemberID = cm.ID and sg.ID = " + nID.ToString() + " and cm.MemberName = '" + strUserName + "'";
            ...
            /*foreach (KeyValuePair<ConnectionState, LoginInfo> pair in m_logInUsers)
            {
                LoginInfo info = pair.Value;

                if (info.ID == nID && info.UserName == strUserName)
                    return info;
            }
        ...
        public LoginInfo FindLoginUser(string strUserID)
        {
            ...
            /*foreach (KeyValuePair<ConnectionState, LoginInfo> pair in m_logInUsers)
            {
                ...
                if (info.SOPGenUserID == strUserID)
                    return info;
            }
