...
namespace SDMSServer
{
	...
	public class BroadcastManager : IDisposable
	{
		...
		private int ReadHeartBeat()
		{
			...
			string szSQL = "SELECT HOSTADDRESS, HEARTBEAT, BSTATE, BDescription from BroadcastState";
        ...
        {
            string szSQL = "SELECT PropertyValue FROM OptionSDMS where PropertyName='BroadcastOn'";
            ArrayList arResult = mDBMgr.GetResultData(szSQL, 0);
            if (arResult == null || arResult.Count == 0)
            {
                m_bEnabled = false;
