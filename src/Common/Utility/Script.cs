    ...
    namespace Utility
    {
        ...
        public class SOPSimulatorScript : Script
        {
            ...
            static public string Parse(string strMsg, DateTime dtTime, string strPlace, int nRealMode = -1, int nNormalMode = -1)
            {
                ...
                {
                    int nIndex1 = strMsg.IndexOf('{', nBeginIndex);

                    if (nIndex1 < 0)
                    {
                        strResult += strMsg.Substring(nBeginIndex);
                        return strResult;
                    }

                    int nIndex2 = strMsg.IndexOf('}', nIndex1 + 1);
                    ...
                    if (nIndex1 > nBeginIndex)
                        strResult += strMsg.Substring(nBeginIndex, nIndex1 - nBeginIndex);

                    strResult += ChangeSpecialMessage(strMsg.Substring(nIndex1, nIndex2 - nIndex1 + 1), dtTime, strPlace, nRealMode, nNormalMode, strResult);
            ...
            static private string ChangeSpecialMessage(string strToken, DateTime dtTime, string strPlace, int nRealMode, int nNormalMode, string strPrevText)
            {
                ...
                {
                    if (GetTimeString(ref strResult, nBeginIndex, nEndIndex, strToken, dtTime))
                        return strResult;
                }
                else if (string.Compare(strTag, "location", true) == 0)
                {
                    if (GetLocationString(ref strResult, nBeginIndex, nEndIndex, strToken, strPlace))
                        return strResult;
                }
                else if (string.Compare(strTag, "real_day", true) == 0 && nColonIndex >= 0)
                {
                    // 실제모드 && 주간모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nRealMode == 1 && nNormalMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "real_night", true) == 0 && nColonIndex >= 0)
                {
                    // 실제모드 && 야간 및 휴일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nRealMode == 1 && nNormalMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "virtual_day", true) == 0 && nColonIndex >= 0)
                {
                    // 훈련모드 && 주간모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nRealMode == 0 && nNormalMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "virtual_night", true) == 0 && nColonIndex >= 0)
                {
                    // 훈련모드 && 야간 및 휴일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nRealMode == 0 && nNormalMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "real", true) == 0 && nColonIndex >= 0)
                {
                    // 실제모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nRealMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "virtual", true) == 0 && nColonIndex >= 0)
                {
                    //  훈련모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nRealMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "day", true) == 0 && nColonIndex >= 0)
                {
                    // 평일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nNormalMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "night", true) == 0 && nColonIndex >= 0)
                {
                    //  야간 및 휴일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, nNormalMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "sopmode", true) == 0)
                {
                    if (GetSOPModeString(ref strResult, nRealMode))
                        return strResult;
                }
                else if (string.Compare(strTag, "sopfullmode", true) == 0)
                {
                    if (GetSOPFullModeString(ref strResult, nRealMode))
                        return strResult;
                }
            ...
            static private bool GetSOPFullModeString(ref string strResult, int nRealMode)
            {
                if (nRealMode == 1)
                    strResult = "실제상황";
                else if (nRealMode == 0)
            ...
            static private bool GetSOPModeString(ref string strResult, int nRealMode)
            {
                if (nRealMode == 1)
                    strResult = "실제";
                else if (nRealMode == 0)
