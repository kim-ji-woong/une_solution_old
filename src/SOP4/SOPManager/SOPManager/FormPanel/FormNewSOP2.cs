...
namespace SOPManager
{
	public partial class FormNewSOP2 : Form, IRibbonButtonOwner
	{
		...
		private void AddTeamRow(string strValue, bool isUser)
		{
			if (strValue == "")
			{
				UnE.Utility.UMessageBox.Show("조직(기관) 이름을 설정하십시오.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
			...
			{
				UnE.Utility.UMessageBox.Show("같은 이름의 조직(기관)을 사용할 수 없습니다.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
			}
		}
		...
		private void btnDeleteExternalTeam_Click(object sender, EventArgs e)
		{
			...
			foreach (DataGridViewRow row in rows)
			{
				...
				if (data != null)
				{
					...
					{
						UnE.Utility.UMessageBox.Show("현재 사용 중인 조직은 삭제할 수 없습니다.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
					}
				}
			}
		...
		private void btnCreateSOP_Click(object sender, EventArgs e)
		{
			...
			{
				UnE.Utility.UMessageBox.Show("재난종류를 먼저 선택해야 합니다.", "오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
