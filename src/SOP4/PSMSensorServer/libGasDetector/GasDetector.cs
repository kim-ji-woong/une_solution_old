...
namespace GasDetector
{
    internal class GasDetector
    {
        ...
        private GasDetector m_otherDetector = null;
        public GasDetector OtherDetector
        {
            get { return m_otherDetector; }
            set { m_otherDetector = value; }
        }
        ...
        public void CheckValue(SerialManager sm, int nTimeOut)
        {
            if (sm.IsConnected == true)
            {
                ...
                byte[] data = MsgHelper.MakeData((byte)0, 0, 0, (byte)255, (byte)m_nFunction, (byte)m_nID);
        ...
        public void CheckValue(NetworkManager sm, int nTimeOut)
        {
            if (sm.IsConnected == true)
            {
                ...
                if (sm.ReciverData == true)
                {
                    ...
                    if (smReadBuffer != null)
                    {
                        //if (smReadBuffer.Length >= 2 && smReadBuffer[1] == 0x03)
                        //{
                            SetDetectorValue(smReadBuffer);
                            sm.ClearBuffer();
                        ...
                        /*}
                        else if (smReadBuffer.Length >= 2 && smReadBuffer[1] == 0x01)
                        {
                            SetUnitStatus(smReadBuffer);
                            sm.ClearBuffer();
                            sm.ReciverData = false;
                        }*/
                    }
        ...
        internal void SendCommand(NetworkManager sm, int nTimeOut)
        {
            byte[] data = null;
            if (m_bAddedStop == true)
            {
        ...
        public void UpdateValue(NetworkManager sm, int nTimeOut)
        {
            // Check Notify
        ...
        private void SetDetectorValue(byte[] bStatus)
        {
            ...
            if (nLength > m_UnitValues.Length)
                m_UnitValues = new float[nLength];
            ...
            for (int i = 0; i < nLength; i += 2)
            {
                ...
                if( aUnit != null)
                {
                    float ratio = aUnit.Ratio;
                    float value = BitConverter.ToInt16(temp, 0) / ratio;
                    if (value < 0.0f)
                        value = 0.0f;
...
#if VALUE_DEBUG
        public void SetValue(int nUnit, float fValue)
        {
            if (m_UnitValues == null)
                return;
            if (nUnit < 0 || m_UnitValues.Length <= nUnit)
                return;
            m_UnitValues[nUnit] = fValue;
...
#if VALUE_DEBUG
        internal void SetDebugUnitStatus(byte value)
        {
            m_Units[1].SetStatus(value);
        }
#endif
    ...
    internal class AlarmUnit
    {
        ...
        public void SetStatus(byte nStatus)
        {
            ...
            for(int i = 0 ; i < m_Channel.Length ; i++)
            {
                ...
                if (m_Channel[i].Status != nResult && m_bReset == false)// && m_Channel[i].First == false)
                {
                ...
                if (m_bRequestAlarm == true)
                {                   
                    if (m_Channel[i].TargetValue == nResult)
                    {
                        m_Channel[i].FireNotify = true;
                    }
                }
