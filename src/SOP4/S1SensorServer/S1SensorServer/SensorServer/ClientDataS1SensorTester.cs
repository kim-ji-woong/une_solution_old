...
namespace S1SensorServer
{
    public class ClientDataS1SensorTester : ClientData
    {
        ...
        private int GetSensorTagInfoID(DBUtility.WebDBManager dbMgr, int nSensorServerID, int nTagNo, ref int nSensorType)
        {
            ...
            if (sensorType != null)
            {
                if (sensorType.Data == (int)Facility.FacilityType.PSM_SENSOR)
                    nSensorType = sensorType.Data;
                else if (sensorType.Data == (int)Facility.FacilityType.FIRE_SENSOR ||
                    (sensorType.Data >= (int)Facility.FacilityType.FireSensor_TypeA && sensorType.Data <= (int)Facility.FacilityType.FireSensor_MonitoringType))
                    nSensorType = (int)Facility.FacilityType.FIRE_SENSOR;
                 else if (sensorType.Data == (int)Facility.FacilityType.Fire_S1 ||sensorType.Data == (int)Facility.FacilityType.FireF1_S1)
                    nSensorType = (int)Facility.FacilityType.FIRE_SENSOR;
            }
