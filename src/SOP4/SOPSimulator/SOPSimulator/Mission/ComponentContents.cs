	...
	public partial class ComponentContents : Form
	{
                                ...
                                if (tabPage != null)
                                {
                                    ...
                                    if (WorkFlowManager.Instance.Get(nActionStepID, (nRealMode == 1 ? true : false)) != null)
                                    {
                                        ...
                                        if (work.State == WorkFlowState.RUN)
                                        {
                                            ...
                                            {
                                                NetworkManager.Instance.SendSelectMission(nActionStepID, nRealMode, nComponentID, strRowIndex);
		...
		public void SetTitle(string strTitle, DateTime time, string strStatus)
		{
            ...
            if (m_strLocation != null && m_dtDetect != null)
                labelTitle.Text = Parse(strTitle, m_dtDetect, m_strLocation, m_strPSMMaterialName, m_psmDistance, m_strAmountSnowfall);
            else
        ...
        //private void PreStart(DBUtility.VariousData<DateTime> dtDetect, string strLocation, string strBroadcastLocationName, string strPSMMaterialName, VariousData<int> psmDistance, string strAmountSnowfall)
        {
            ...
            DBUtility.VariousData<DateTime> dtDetect = null;
            ...
            ChangeTitle();
            labelTitle.Text = Parse(labelTitle.Text, dtDetect, strLocation, strPSMMaterialName, psmDistance, strAmountSnowfall);
        ...
        private string ChangePSMString(string str, WorkflowOptionPSM option)
        {
            ...
            str = str.Replace(FormSpecialMessageHelpPSM.GetVariableString(FormSpecialMessageHelpPSM.VariableType.PSMDistanceKM), (option.PSMDistance / 1000).ToString());

            return str;
        }

        private string ChangeClimateString(string str, WorkflowOptionSnowFall option)
        {
            if (option.UseAmountSnowFall)
            {
                str = str.Replace(FormSpecialMessageHelpClimate.GetVariableString(FormSpecialMessageHelpClimate.VariableType.SNOW_DEPTH), option.AmountSnowFall.ToString());
            }
        ...
        private string ChangeProcessMissionText(WorkflowOption option, string strMissionText, bool isRealMode)
        {
            ...
            UnE.SOP.Utility.SOPSimulatorScript.DataParameter param = new UnE.SOP.Utility.SOPSimulatorScript.DataParameter(strMissionText, option.DetectTime == null ? DateTime.Now : option.DetectTime.Data, option.PositionName);
            ...
            foreach (MissionItem item in data.MissionItems)
            {
                if (item is MissionItemExternal)
                {
                    ...
                    for (int i=0;i<nArgumentCount;i++)
                    {
                        if (option is WorkflowOptionEarthquake)
                        {
                            _item.Arguments[i] = ChangeEarthquakeString(_item.Arguments[i], (WorkflowOptionEarthquake)option);
                        }
                        else if (option is WorkflowOptionPSM)
                        {
                            _item.Arguments[i] = ChangePSMString(_item.Arguments[i], (WorkflowOptionPSM)option);
                        }
                        else if (option is WorkflowOptionSnowFall)
                        {
                            _item.Arguments[i] = ChangeClimateString(_item.Arguments[i], (WorkflowOptionSnowFall)option);
                        }
                    }
                }
            }
        ...
        private string Parse(string strMsg, VariousData<DateTime> dtDetect, string strLocation, string strPSMMaterialName, VariousData<int> psmDistance, string strAmountSnowfall)
        {
            ...
            UnE.SOP.Utility.SOPSimulatorScript.DataParameter param = new UnE.SOP.Utility.SOPSimulatorScript.DataParameter(strMsg, dtDetect == null ? DateTime.Now : dtDetect.Data, strLocation);
        ...
        private static void AddTemporaryMemberPhoneNumbers(Dictionary<string, string> dicPhoneNumbers, List<TemporaryMember> members)
        {
            ...
            foreach (TemporaryMember member in members)
            {
                ...
                {
                    DataCompanyMember companyMember = FormSOP.Instance.SDMSDataManager.GetCompanyMember(member.MemberID);
                ...
                else if (member._MemberType == TemporaryMember.MemberType.RegularTeam)
                {
                    if (member.TeamLeader == 1)
                    {
                        ...
                        DataCompanyMember companyMember = FormSOP.Instance.SDMSDataManager.GetCompanyMember(nLeaderID);
                    ...
                    {
                        List<DataCompanyMember> companyMembers = FormSOP.Instance.SDMSDataManager.GetCompanyMembers(member.MemberID, member.IncludeChildTeams);
                ...
                {
                    DataExternalMember externalMember = FormSOP.Instance.SDMSDataManager.GetExternalMember(member.MemberID);
                ...
                {
                    List<DataCompanyMember> companyMembers = FormSOP.Instance.SDMSDataManager.GetCompanyMembers(member.MemberID);
        ...
        {
            List<DataExternalMember> externalMembers = FormSOP.Instance.SDMSDataManager.GetExternalMembers(nTeamID, includeChildTeams);
        ...
        private static ArrayList GetReceiverPhoneNumbers(ArrayList arrTeams, bool onlyTeamLeader, /*bool includeChildTeams, */out string strReceiverNames)
        {
            ...
            //foreach (SOPTeam teamData in arrTeams)
            {
                ...
                {
                    List<DataCompanyMember> companyMembers = FormSOP.Instance.SDMSDataManager.GetCompanyMembers(teamData.TeamID, teamData.IncludeChildTeams);
        ...
        private bool CheckDetailCount(int nComponentHistoryID, List<UnE.SOP.History.HistorySectionData.DetailData> detailDatas)
        {
            ...
            //ArrayList arrResult2 = FormSOP.Instance.DBManager.GetResultData(strSQL2, 0);
