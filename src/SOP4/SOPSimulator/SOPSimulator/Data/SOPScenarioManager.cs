...
{
	public class SOPScenarioManager : ISOPPageContainer
	{
		...
		public void AddSOPScenario(string strPath, int nActionStepID, bool bReal, int nActionStepHistoryID, int nSensorZoneHistoryID)
		{
			bool bRegular = FormSOP.Instance.IsRegular;
			bool bNormal = FormSOP.Instance.IsNormal;
            AddSOPScenario(strPath, nActionStepID, bReal, bRegular, bNormal, nActionStepHistoryID, nSensorZoneHistoryID);
		}

        public void AddSOPScenario(SOPScenario sopSC)
        {
            ...
            if (m_arrScenario.Count == 0)
            {
                ...
                FormSOP.Instance.GetPageHome().AddScenario(sopSC);
            }
            else
            {
                bool isSame = false;
		...
		public void AddSOPScenario(string strPath, int nActionStepID, bool bReal, bool bRegular, bool bNormal, int nActionStepHistoryID, int nSensorZoneHistoryID)
		{
        ...
        private Dictionary<int, int> GetActionStepHistoryInfo(WebDBManager dbMgr, Dictionary<int, Data_ActionStepHistory> dicActionStepHistories)
        {
            ...
            {
                DBUtility.VariousData<int> disasterID = WebDBManager.GetIntField(arrResult[i].ToString());
                DBUtility.VariousData<int> actionStepHistoryID = WebDBManager.GetIntField(arrResult[i + 1].ToString());
        ...
        private bool LoadHistory(WebDBManager dbMgr, Dictionary<int, Data_ActionStepHistory> dicActionStepHistories)
        {
            ...
            foreach (KeyValuePair<int, Data_ActionStepHistory> pair in dicActionStepHistories)
            {
                ...
                HistoryManager.Instance.AddHistoryDisasterPosition(actionStepHistory.ID, actionStepHistory.ActionStepID, isRealMode);
                HistoryManager.Instance.AddHistoryDisasterNoPosition(actionStepHistory.ActionStepID, isRealMode, actionStepHistory.HistoryDisasterNoPositionInfo);
                HistoryManager.Instance.SetActionStepHistory(isRealMode ? actionStepHistory.ActionStepID : -actionStepHistory.ActionStepID, actionStepHistory.ID);
            }
		...
		private bool LoadHistory(WebDBManager dbMgr, Dictionary<string, DisasterInfo> dicData, Dictionary<int, Data_ActionStepHistory> dicActionStepHistories, bool isRealMode, bool isRegular, bool isNormal)
		{
			...
			foreach (Data_ActionStepHistory history in arrActionStepHistories)
			{
				...
				HistoryManager.Instance.AddHistoryDisasterPosition(history.ID, history.ActionStepID, isRealMode);
                HistoryManager.Instance.AddHistoryDisasterNoPosition(history.ActionStepID, isRealMode, history.HistoryDisasterNoPositionInfo);
				HistoryManager.Instance.SetActionStepHistory(isRealMode ? history.ActionStepID : -history.ActionStepID, history.ID);
			}
		...
		private bool LoadActionSteps(DBUtility.WebDBManager dbMgr, string strSQL, ArrayList arrActionStepID, ArrayList arrDisaster, Dictionary<ActionStepInfo, ArrayList> dicStepMember, bool isNormal)
		{
			ArrayList arrResult = dbMgr.GetResultData(strSQL, 0);
        ...
        // dicStepMember : ActionStepInfo, StepMemberData List
        private bool LoadActionSteps(DBUtility.WebDBManager dbMgr, string strSQL, ArrayList arrActionStepID, ArrayList arrDisaster, Dictionary<ActionStepInfo, ArrayList> dicStepMember)
        {
            ArrayList arrResult = dbMgr.GetResultData(strSQL, 0);
            if (arrResult == null) return false;
        ...
        public bool LoadActionStepPanel(DBUtility.WebDBManager dbMgrex, string strActionstepIDs, ArrayList arrActionStepHistoryID, ArrayList arrActionStepID, ArrayList arrActionStepBeginTime, ArrayList arrActionStepDetectTime, ArrayList arrDisaster, ArrayList arrSensorZoneHistories, bool isRealMode, Dictionary<int, DisasterInfo> dicDisaster, Dictionary<DisasterInfo, string> dicDisasterFullPath, bool isRegular, bool isNormal)
        {
            return _LoadActionStepPanel(dbMgrex, null, strActionstepIDs, arrActionStepHistoryID, arrActionStepID, arrActionStepBeginTime, arrActionStepDetectTime, arrDisaster, arrSensorZoneHistories, isRealMode, dicDisaster, dicDisasterFullPath, isRegular, isNormal);
        }
        ...
        private bool _LoadActionStepPanel(DBUtility.WebDBManager dbMgrex, ArrayList arrActionStepHistories, string strActionstepIDs, ArrayList arrActionStepHistoryID, ArrayList arrActionStepID, ArrayList arrActionStepBeginTime, ArrayList arrActionStepDetectTime, ArrayList arrDisaster, ArrayList arrSensorZoneHistories, bool isRealMode, Dictionary<int, DisasterInfo> dicDisaster, Dictionary<DisasterInfo, string> dicDisasterFullPath, bool isRegular, bool isNormal)
        {
            ...
            foreach (KeyValuePair<ActionStepInfo, ArrayList> pair in dicStepMember)
            {
                ...
                SectionTabPage tabPage = (SectionTabPage)pageHome.AddTabPage(actionStep.ToData_ActionStep(), isRealMode);
                ...
                if (nIndex >= 0 && nActionStepHistoryID > 0)
                {
                    ...
                    HistoryActionStepData data = HistoryManager.Instance.AddActionStepHistory(actionStep.ActionStepID, isRealMode, WorkFlowState.RUN, (DateTime)arrActionStepBeginTime[nIndex], true, bUseSMS);
                ...
                if (tabPage.CreateNew == true)
                {
                    ...
                    HistoryDisasterPosition pos = HistoryManager.Instance.FindHistoryDisasterPosition(tabPage.ActionStepID, !tabPage.VirtualMode);
                    ...
                    HistoryDisasterNoPosition noPos = HistoryManager.Instance.FindHistoryDisasterNoPosition(tabPage.ActionStepID, !tabPage.VirtualMode);
            ...
            for (int i = 0; i < allTabPages.Count; i++)
            {
                ...
                {
                    pageHome.TabControls.SelectedTab = (page);
                }
            }
            return true;
        }
            ...
            foreach (Control ctrl in page.Controls)
            {
                if (ctrl is PanelSection)
                {
                    ...
                    foreach (Section section in panel.Sections)
                    {
                        ...
                        {
                            ComponentContents contents = pageHome.GetComponentContents(section);
        ...
        private void SelectComponentContentsThread(object param)
        {
            ...
            ComponentContents contents = (ComponentContents)param;
        ...
        private void AddSOPSectionLog(int nActionStepID, Sections.Section section, int nComponentHistoryID, bool isRealMode, int nStatus, int nProcessDirections, ArrayList arrSections, string strTask, DateTime time, string strDescription, bool showBoard, int nCheckedNotify1, int nCheckedNotify2, int nCheckedRun, int nCheckedComplete, int nAccessedUserID, Dictionary<int, List<HistorySectionData.DetailData>> dicDetailDatas, WorkFlow workFlow)
		{
			...
			{
                ApplyDetailDatas(nComponentHistoryID, section, dicDetailDatas);

				// 입력대기는 SOP Log 창에 표시하지 않는다.
				return;
			}

			HistoryManager.Instance.SetLastComponentHistory(nActionStepID, nComponentHistoryID);
            ...
            if (sectionState != null && FormSOP.Instance.HasControl)
			...
			{
                HistoryManager.Instance.AddSectionHistory(section, nComponentHistoryID, state, nProcessDirections, true, time, showBoard, nCheckedNotify1, nCheckedNotify2, nCheckedRun, nCheckedComplete, sectionState.DetailDatas);
				m_nInitHistoryLogCount++;
			}
			else if (type == Sections.Section.ComponentType.DECISION)
			{
				...
				HistoryManager.Instance.AddDecisionHistory((Sections.SectionDecision)section, state, nProcessDirections, nextSection, true, time, showBoard);
				m_nInitHistoryLogCount++;
			}
			else if (type == Sections.Section.ComponentType.INTERNAL)
			{
				...
				HistoryManager.Instance.AddInternalHistory((Sections.SectionInternal)section, state, nProcessDirections, nCheckedRun, nCheckedComplete, usePopupMessage, useSMS, useBroadcast, true, time, showBoard, nCheckedNotify1);
				m_nInitHistoryLogCount++;
			}
			else if (type == Sections.Section.ComponentType.EXTERNAL)
			{
				...
				HistoryManager.Instance.AddExternalHistory((Sections.SectionExternal)section, state, nProcessDirections, useSMS, useFax, true, time, showBoard, nCheckedNotify1, nCheckedNotify2, nCheckedRun, nCheckedComplete);
				m_nInitHistoryLogCount++;
			}
			else if (type == Sections.Section.ComponentType.TRANSMISSION)
			{
				...
				HistoryManager.Instance.AddTransmissionHistory((Sections.SectionTransmission)section, state, nProcessDirections, usePopupMessage, useSMS, useBroadcast, useExSMS, useExFax, true, time, showBoard, nCheckedNotify1, nCheckedNotify2, nCheckedRun, nCheckedComplete);
				m_nInitHistoryLogCount++;
			}
        ...
        private void ApplyDetailDatas(int nComponentHistoryID, Section section, Dictionary<int, List<HistorySectionData.DetailData>> dicDetailDatas)
        {
            if (!FormSOP.Instance.HasControl || !FormSOP.Instance.Initialization)
            {
                ...
                if (dicDetailDatas.TryGetValue(nComponentHistoryID, out detailDatas))
                {
                    ...
                    if (contents != null)
                    {
                        ...
                        contents.ApplyDetailDatas(nComponentHistoryID, detailDatas);
                    }
                }
            }
        }
        ...
        public void AddSOPSectionLog(int nActionStepID, ArrayList arrComponentHistoryID, ArrayList arrSections, ArrayList arrStatus, ArrayList arrProcessDirections, ArrayList arrTask, ArrayList arrTime, ArrayList arrDescription, ArrayList arrShowBoard, ArrayList arrCheckedNotify1, ArrayList arrCheckedNotify2, ArrayList arrCheckedRun, ArrayList arrCheckedComplete, ArrayList arrAccessedUserID, bool isRealMode, Dictionary<int, List<HistorySectionData.DetailData>> dicDetailDatas, WorkFlow workFlow)
		{
			...
			for (int i=0;i<nSectionCount;i++)
			{
                ...
                AddSOPSectionLog(nActionStepID, section, nComponentHistoryID, isRealMode, nStatus, nProcessDirections, arrSections, strTask, time, strDescription, showBoard, nCheckedNotify1, nCheckedNotify2, nCheckedRun, nCheckedComplete, nAccessedUserID, dicDetailDatas, workFlow);
			}
		}
		...
		private bool LoadComponentHistory(WebDBManager dbMgr, ArrayList arrPanels, ArrayList arrActionStepHistoryID, ArrayList arrActionStepID, ArrayList arrActionStepBeginTime, ArrayList arrActionStepDetectTime, ArrayList arrDisaster,ArrayList arrSensorZoneHistories, bool isRealMode, Dictionary<int, DisasterInfo> dicDisaster, Dictionary<DisasterInfo, string> dicDisasterFullPath, bool isRegular, bool isNormal)
		{
			...
			AddSOPSectionLog(panel.ActionStepID, arrComponentHistoryID, arrSections4Log, arrSectionStatus4Log, arrSectionProcessDirections4Log, arrTask, arrTime, arrDescription, arrShowBoard, arrCheckedNotify1, arrCheckedNotify2, arrCheckedRun, arrCheckedComplete, arrAccessedUserID, isRealMode, dicDetailDatas, workFlow);
			...
			for (int i = 0; i < nSectionCount; i++)
			{
                ...
                System.Diagnostics.Trace.WriteLine("Load Component Section State : " + nStatus + " Section : " + state.Section.Data.ComponentID);
				...
				if (nStatus == 2)
				{
					...
					foreach (SectionState next in arList)
					{
						if (next != null)
						{
                            ...
                            if( next.Section.GetComponentType() == Section.ComponentType.INTERNAL)
                            {
                                System.Diagnostics.Trace.WriteLine("Set Input State : " + nStatus + " Section : " + next.Section.Data.ComponentID);
