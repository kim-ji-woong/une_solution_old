...
{
    public class ClientProvider_Internal : ClientServiceProvider
    {
		public enum ClientType { ALL = 0, SDMS_CLIENT, SOP_SIMULATOR, SENSOR_SIMULATOR, UNKNOWN };

        private NetworkManager_Internal m_mgr = null;
        ...
        public ClientProvider_Internal(NetworkManager_Internal mgr)
        {
            m_mgr = mgr;
        ...
        private void ProcessInternalMessage(ArrayList arrDatas)
        {
            ...
            switch (msg)
            {
                case IntegratedManagement2.InternalMessage.RUN_SOP_SIMULATOR:
                ...
                case IntegratedManagement2.InternalMessage.CHECK_SOP_SIMULATOR:
                    SendCheckSOPSimulator(arrDatas);
                    break;
            }
        }

        private void SendCheckSOPSimulator(ArrayList arrDatas)
        {
            ArrayList arrDatas2 = new ArrayList();
            arrDatas2.Add(IntegratedManagement2.InternalMessage.REPLY_CHECK_SOP_SIMULATOR);
            arrDatas2.Add(!FormSOP.Instance.OnlySDMS);

            for (int i=1;i<arrDatas.Count;i++)
            {
                arrDatas2.Add(arrDatas[i]);
            }

            byte[] bytes = MakeBytes(TCP_ID.INTERNAL_MESSAGE, arrDatas2);
            m_mgr.Send(bytes, this);
        }

        private void ProcessAreYouThere()
        {
            byte[] bytes = MakeBytes(TCP_ID.I_AM_HERE, null);
        ...
        private void SendWhoIAm()
        {
            ...
            int nClientType = (int)TCP_CLIENT.SDMS_CLIENT;
            arrDatas.Add(nClientType);
            arrDatas.Add(FormSOP.Instance.ExecutableIndex);
