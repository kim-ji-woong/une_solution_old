...
namespace IntegratedManagement2
{
    public class ClientProvider : ClientServiceProvider
    {
        ...
        public bool SendRegisterUser(int nMemberID, string szID, string szPass, string szNickName, IntegratedManagement2.PopupDialog.Chief chief)
		{
			if (this.IsConnected == false)
			{
				return false;
            ...
            byte[] bytes = new byte[dataBytes.Length + dataBytes1.Length + dataBytes2.Length + dataBytes3.Length + dataBytes4.Length + dataBytes5.Length + dataBytes6.Length + dataBytes7.Length + 6];
