...
namespace IntegratedManagement2
{
    public class NetworkServer
    {
        ...
        public void ProcessCommand(TcpLib2.ConnectionState state, Command cmd)
        {
            switch (cmd)
            {
                case Command.RUN_SOP_SIMULATOR0:
                case Command.RUN_SOP_SIMULATOR1:
                    m_provider.RunSOPSimulator(state);
                    break;

                case Command.CHECK_SOP_SIMULATOR1_N_RUN_SOP_SIMULATOR0:
                    m_provider.CheckSOPSimulator(state);
                    break;
            }
