...
namespace ServerMonitor
{
	public class NetworkManager
	{
        ...
        public void SendStartServer(int nType)
        {
            if (m_provider == null)
                return;

            ArrayList arrDatas = new ArrayList();
            arrDatas.Add((int)nType);

            byte[] bytes = MakeBytes(TCP_ID.START_SERVER_FROM_MONITOR, arrDatas);
            Send(bytes, m_provider);

        }

        public void SendStopServer(int nType)
        {
            if (m_provider == null)
                return;
            ...
            byte[] bytes = MakeBytes(TCP_ID.STOP_SERVER_FROM_MONITOR, arrDatas);
            Send(bytes, m_provider);
        }
