...
namespace BroadRunner
{
    public partial class FormMain : Form
    {
        ...
        {
            string strSQL = "Select PropertyValue from OptionSOPSimulator where PropertyName = '" + OPTION_TAG + "' and SiteID = " + m_nSiteID.ToString();
        ...
        {
            string strSQL = "Select PropertyValue from OptionSOPSimulator where PropertyName = 'UseBroadcast' and SiteID = " + m_nSiteID.ToString();
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL, 0);
        ...
        private void btnRun_Click(object sender, EventArgs e)
        {
            ...
            string strSQL = string.Format(strFormat, strMessage, checkBoxUseSiren.Checked ? 1 : 0, 1, 1, dtNow.ToShortDateString(), dtNow.Hour, dtNow.Minute, dtNow.Second, m_nSiteID);

            if (m_dbMgr.GetResultData(strSQL, 0) == null)
                return;

            strSQL = "Select PropertyValue from OptionSOPSimulator where PropertyName = '" + OPTION_TAG + "' and SiteID = " + m_nSiteID.ToString();
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL, 0);
            ...
            {
                strSQL = "Update OptionSOPSimulator set PropertyValue = '" + strMessage + "' where PropertyName = '" + OPTION_TAG + "' and SiteID = " + m_nSiteID.ToString();
                m_dbMgr.GetResultData(strSQL, 0);
            }
            else
            {
                strSQL = "Insert into OptionSOPSimulator (PropertyName, PropertyValue, SiteID, Description) values ";
                strSQL += string.Format("('{0}', '{1}', {2}, NULL)", OPTION_TAG, strMessage, m_nSiteID);
