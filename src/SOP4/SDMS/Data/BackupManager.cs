	...
	internal class BackupManager
	{
		...
		private void Addextendedproperty(StreamWriter file)
		{
			...
			for (int i = 0; i < arrResult.Count - 4; i += 5)
			{
                ...
                ArrayList arrResult10 = m_dbMgr.GetResultData(szSubSql, 0);
                if (arrResult10 == null)
                    continue;
        ...
        private void DefaultSensorValue(StreamWriter file, StreamWriter file2, string szTableName)
        {
            ...
            ArrayList arrResult2 = new ArrayList();
            arrResult2 = m_dbMgr.GetResultData(szSQL, 0);
        ...
        private void InsertSensorValueTable(StreamWriter file, StreamWriter file2, string szTableName)
        {
            for (int i = 1; i <= 50; i++)
            {
                ...
                {
                    DBUtility.VariousData<int> id = DBUtility.WebDBManager.GetIntField(arrResult[j].ToString());
                    DBUtility.VariousData<float> value = DBUtility.WebDBManager.GetFloatField(arrResult[j + 1].ToString());
