...
namespace SDMS
{
    public class DataManager
    {
        ...
        public DataManager(DBUtility.WebDBManager dbMgr)
        {
            ...
            //LoadDisasterPreventionEquipment();
        }
        ...
        public bool LoadPOI(ISensorTooltipOwner view, bool isIndoor)
        {
        ...
        public void LoadFacilityManager()
        {
            ...
            /*string szText = "SELECT fm.id, fm.MemberID, fm.MemberType, fm.FacilityType, fm.LevelLimit, fm.UpperLimit, fm.Description FROM FacilityManager as fm " +
            ...
            szText = "select id, MemberID, MemberType, FacilityType, LevelLimit, BuildingID, UpperLimit, Description from BuildingFacilityManager WHERE SiteID = {0} " +
            ...
            szText = "select id, MemberID, MemberType, FacilityType, LevelLimit, EquipZoneID, UpperLimit, Description from EquipZoneFacilityManager WHERE SiteID = {0} " +
                     "order by FacilityType";
        ...
        private bool UseFacilityManagerType(WebDBManager dbMgr)
        {
        ...
        public void LoadDisasterPreventionEquipment()
        {
            ...
            {
                int nID = DBUtility.WebDBManager.GetIntField(arrResult[i].ToString(), -1);
                int nTypeID = DBUtility.WebDBManager.GetIntField(arrResult[i + 1].ToString(), -1);
                int nLocationID = DBUtility.WebDBManager.GetIntField(arrResult[i + 2].ToString(), -1);
                string strName = DBUtility.WebDBManager.GetStringField(arrResult[i + 3].ToString(), "");
                int nQuantity = DBUtility.WebDBManager.GetIntField(arrResult[i + 4].ToString(), -1);
                string strDescription = DBUtility.WebDBManager.GetStringField(arrResult[i + 5].ToString(), "");
        ...
        private void LoadDisasterPreventionEquipmentType()
        {
            ...
            {
                int nID = DBUtility.WebDBManager.GetIntField(arrResult[i].ToString(), -1);
                string strName = DBUtility.WebDBManager.GetStringField(arrResult[i + 1].ToString(), "");
        ...
        private void LoadDisasterPreventionEquipmentLocation()
        {
            ...
            {
                int nID = DBUtility.WebDBManager.GetIntField(arrResult[i].ToString(), -1);
                string strLocationName = DBUtility.WebDBManager.GetStringField(arrResult[i + 1].ToString(), "");
        ...
        public bool SaveDisasterPreventionEquipment()
        {
            ...
            foreach (DisasterPreventionEquipment disasterPreventionEquipment in liEquipment)
            {
                ...
                if (disasterPreventionEquipment.Type.ID < 0)
                {
                    ...
                    if (isOK == false)
                    {
                        arrResult = dbMgr.GetResultData(strGetTypeIDSQL, 0);

                        int nTypeID = -1;
                        if (arrResult == null)
                            return isSave;

                        nTypeID = DBUtility.WebDBManager.GetIntField(arrResult[0].ToString(), 0) + 1;

                        if (dbMgr.GetResultData(String.Format(strInsertTypeSQL, nTypeID, disasterPreventionEquipment.Type.Name), 0) != null)
                        {
                            DisasterPreventionEquipmentType newType = new DisasterPreventionEquipmentType();
                            newType.ID = nTypeID;
                ...
                if (disasterPreventionEquipment.Location.ID < 0)
                {
                    ...
                    if (isOK == false)
                    {
                        arrResult = dbMgr.GetResultData(strGetLocationIDSQL, 0);

                        int nLocationID = -1;
                        if (arrResult == null)
                            return isSave;

                        nLocationID = DBUtility.WebDBManager.GetIntField(arrResult[0].ToString(), 0) + 1;

                        if (dbMgr.GetResultData(String.Format(strInsertLocationSQL, nLocationID, disasterPreventionEquipment.Location.Name), 0) != null)
                        {
                            DisasterPreventionEquipmentLocation newLocation = new DisasterPreventionEquipmentLocation();
                ...
                switch (disasterPreventionEquipment.Status)
                {
                    case DisasterPreventionEquipment.STATUS.NEW:
                        ...
                        arrResult = dbMgr.GetResultData(strGetIDSQL, 0);
                    ...
                    case DisasterPreventionEquipment.STATUS.DEL:
                        ...
                        if (dbMgr.GetResultData(String.Format(strDeleteSQL, disasterPreventionEquipment.ID), 0) != null)
                        {
                            m_dicDisasterPreventionEquipment.Remove(disasterPreventionEquipment.ID);
                        }
                        else
        ...
        public string GetEditPassword()
        {
            ...
            ArrayList arrResult = FormMain.Instance.DBManager.GetResultData(szSQL, 0);
