    ...
    public class PSMManager
    {
        ...
        private Dictionary<int, PSMTank> ReadPSMTanks()
        {
            ...
            for (int i = 0; i < nResultCount - 10 ; i += 11)
            {
                ...
                DBUtility.VariousData<float> fCapacity = WebDBManager.GetFloatField(arrResult[i + 4].ToString());
                DBUtility.VariousData<float> fRemains = WebDBManager.GetFloatField(arrResult[i + 5].ToString());
