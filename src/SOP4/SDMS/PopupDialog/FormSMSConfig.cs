...
namespace SDMS
{
	...
	{
        public enum MessageType { FACILITY_FAULT = 0, DETECT_FIRE, REPORT_FIRE, DETECT_SPILL, REPORT_SPILL, RESET_SPILL, DETECT_SECURITY, REPORT_SECURITY, RESET_SECURITY };
		...
		private void LoadDB()
		{
			...
			ArrayList arrRes = dbMgr.GetResultData(query, 0);
			if (arrRes == null || arrRes.Count == 0)
            {
                query = "SELECT MAX(ID) FROM OptionSDMS";
                arrRes = FormMain.Instance.DBManager.GetResultData(query, 0);
                if (arrRes != null)
                {
		...
		private void Save(bool useSMS, MessageType type)
		{
            ...
            ArrayList arTemp = dbMgr.GetResultData(szSQL, 0);
