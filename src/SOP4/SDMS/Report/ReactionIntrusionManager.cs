        ...
        public class ReactionIntrusionManager
        {
                ...
                foreach (KeyValuePair<int, ArrayList> pair in m_dicHistoryLog)
                {
                    ...
                    if (nSensorHistoryID == nHistoryID)
                    {
                        ...
                        foreach (SensorReactionIntrusionLog srLog in log)
                        {
                            ...
                            if (srLog.ReactionType == (int)ReactionType.MALFUNCTION || srLog.ReactionType == (int)ReactionType.IGNORE_S1SVMS_STATUS || srLog.ReactionType == (int)ReactionType.IGNORE_S1ACCESS_STATUS)
                            {
                                int nCommanderID = -1;
                ...
                foreach (Zone zone in arrZoneList)
                {
                    ...
                    foreach (int nHistoryID in arrHistoryList)
                    {
                        ...
                        foreach (SensorReactionIntrusionLog log in arrLog)
                        {
                            ...
                            if (log.ReactionType == (int)ReactionType.BEGIN_S1SVMS_STATUS || log.ReactionType == (int)ReactionType.BEGIN_S1ACCESS_STATUS)
                            {
            ...
            private ArrayList GetReactionHistory(ArrayList arrSensorHistoryID)
            {
                ...
                for (int i = 0; i < nResultCount - 10; i += 11)
                {
                    ...
                    if (nReactionType == (int)SDMS.ReactionType.MALFUNCTION || nReactionType == (int)SDMS.ReactionType.NOTIFY_SECURITY || nReactionType == (int)SDMS.ReactionType.IGNORE_S1SVMS_STATUS || nReactionType == (int)SDMS.ReactionType.IGNORE_S1ACCESS_STATUS)
                    {
                        if (!m_dicHistoryMember.ContainsKey(nSensorHistoryID))
                            m_dicHistoryMember.Add(nSensorHistoryID, Param3);
                    }
                    ...
                    //문자메시지
                    if (nReactionType == (int)SDMS.ReactionType.SEND_SMS)
                    {
                        ...
                        {
                            reactionLog.ReactionType = (int)SDMS.ReactionType.SEND_MALFUNCTION_SMS;
                        }
