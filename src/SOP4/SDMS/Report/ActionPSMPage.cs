...
namespace SDMS
{
    public partial class ActionPSMPage : Form
    {
        ...
        {           
            ...
            lblAlarmType.SetBounds(lblEquipZone.Location.X + lblEquipZone.Width - 10, lblEquipZone.Location.Y, lblStatementEnd.Width, lblStatementEnd.Height);
        ...
        private void ActionPSMPage_Load(object sender, EventArgs e)
        {
            ...
            btnEndDate = (Button)arContorls[0];
            btnStartDate = (Button)arContorls[1];
        ...
        private void ActionPSMPage_Resize(object sender, System.EventArgs e)
        {
            ...
            lblActorName.SetBounds(lblResult.Location.X + lblResult.Width, lblResult.Location.Y, lblActorName.Width, lblActorName.Height);
        }
        ...
        private void RefreshReactionType(int nReactionType)
        {
            ...
            foreach (Report.ReactionLog log in m_allLogs)
            {
                ...
                {
                    if (log.Type == (int)ReactionType.BEGIN_PSM_STATUS || log.Type == (int)ReactionType.NOTIFY_PSM)
                        reactionPSMLogBindingSource.Add(log);
                }
                else if (nReactionType == (int)ReactionType.MALFUNCTION) //시스템복구 포함
                {
                    if (log.Type == (int)ReactionType.BEGIN_PSM_STATUS || log.Type == (int)ReactionType.NOTIFY_PSM || log.Type == (int)ReactionType.MALFUNCTION || log.Type == (int)ReactionType.PSM_USER_RESET)
                        reactionPSMLogBindingSource.Add(log);
                }
                else if (nReactionType == (int)ReactionType.IGNORE_PSM_DETECT) //현장복구 포함
                {
                    if (log.Type == (int)ReactionType.BEGIN_PSM_STATUS || log.Type == (int)ReactionType.NOTIFY_PSM || log.Type == (int)ReactionType.IGNORE_PSM_DETECT)
                        reactionPSMLogBindingSource.Add(log);
                }
                else if (nReactionType == (int)ReactionType.ETC) //모든 신호
                {
                    if (log.Type == (int)ReactionType.BEGIN_PSM_STATUS || log.Type == (int)ReactionType.NOTIFY_PSM || log.Type == (int)ReactionType.MALFUNCTION || log.Type == (int)ReactionType.PSM_USER_RESET || log.Type == (int)ReactionType.IGNORE_PSM_DETECT)
                        reactionPSMLogBindingSource.Add(log);
                }
            }
        ...
        private void LoadPSMReaction(int nReactionType = (int)ReactionType.IGNORE_PSM_DETECT)
        {
        ...
        public string GetReactionString(int nReactionType, ReactionPSMLog log)
        {
            ...
            {
                case (int)ReactionType.BEGIN_STATUS: strType = "상황 시작";
                    break;
                ...
                case (int)ReactionType.SEND_REPAIR_SMS: strType = "문자메시지 발송(복구)";
                    break;
                case (int)ReactionType.PSM_USER_RESET: 
                case (int)ReactionType.MALFUNCTION: strType = "시스템 복구 처리";
                    break;
                case (int)ReactionType.NOTIFY_FIRE: strType = "화재 신고";
                    break;
                case (int)ReactionType.IGNORE_FIRE: strType = "화재신호 꺼짐";
                    break;
                ...
                case (int)ReactionType.END_STATUS: strType = "상황 해제";
                    break;
                case (int)ReactionType.BEGIN_PSM_STATUS: strType = (log.Level != 0) ? String.Format("누출 탐지 - {0}단계", log.Level) : "누출 탐지";
                    break;
                case (int)ReactionType.IGNORE_PSM_DETECT: strType = "현장 복구";
                    break;
                case (int)ReactionType.CHANGE_PSM_ALARM_DEPTH: strType = String.Format("누출 단계 변경 - {0}단계", log.Level);
                    break;
                case (int)ReactionType.NOTIFY_PSM: strType = "누출 신고";
                    break;
                case (int)ReactionType.END_PSM_STATUS: strType = "상황 해제";
                    break;
                default:
                    break;
            }
        ...
        private void InitGridView()
        {
            ...
            gvMain.Columns[m_nTIME_INDEX].Width = 220;
            ...
            gvMain.Columns[m_nMATERIAL_INDEX].Width = 100;
            ...
            gvMain.Columns[m_nMANAGER_INDEX].Width = 100;
            ...
            gvMain.Columns[m_nDETECT_LOCATION_INDEX].Width = 300;
            ...
            gvMain.Columns[m_nACTION_INDEX].Width = 280;
        ...
        private bool SetGridRows(Report.ReactionPSMLog dataLog, int nRowNo, string strReactionType)
        {
            ...
            if (dataLog.Type == (int)ReactionType.MALFUNCTION
              || dataLog.Type == (int)ReactionType.PSM_USER_RESET)
            {
            ...
            if (dataLog.Type == (int)ReactionType.NOTIFY_PSM
                || dataLog.Type == (int)ReactionType.PSM_USER_RESET
