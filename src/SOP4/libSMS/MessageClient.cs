...
namespace libSMS
{
    ...
    internal class MessageClient : IMessageClient
    {
        ...
        internal MessageClient(string szServerIP, int nSiteID)
        {
    ...
    internal class MessageClientSNU : IMessageClient
    {
        ...
        private MessageBrokerSNU m_snuMsg = null;

        private string m_strTitle = "";
        private List<KeyValuePair<MessageContentMMS.ContentType, string>> m_arrContents = null;

        public MessageClientSNU(string szServerIP, int nSiteID)
        {
            m_szServerIP = szServerIP;
            m_nSiteID = nSiteID;
            m_snuMsg = new MessageBrokerSNU(m_nSiteID);
            //m_broker = new MessageBroker(szServerIP);
        }
        ...
        public bool SendSMS(List<MessageContent> arMessages)
        {
            ...
            foreach (MessageContent content in arMessages)
            {
                if (m_snuMsg != null)
                {
                    ...
                    //m_snuMsg.SendSmsMessage(content.Caller, content.Reciver, content.Message);
                }
            }
        ...
        private void AddBuffer(string strCaller, string strReceiver, string strMessage)
        {
            ...
            {
                if (m_isMMS.Data)
                    m_snuMsg.SendMMSMessage(m_strCaller, m_rcvPhoneNumbers, m_strMessage);
                else
                    m_snuMsg.SendSMSMessage(m_strCaller, m_rcvPhoneNumbers, m_strMessage);
        ...
        private void AddBuffer(string strCaller, string strReceiver, string strMessage, string strTitle, List<KeyValuePair<MessageContentMMS.ContentType, string>> contentsData)
        {
            ...
            if (m_rcvPhoneNumbers.Count >= m_msgBufCount || m_isBegin == false)
            {
                m_snuMsg.SendMMSMessage(m_strCaller, m_rcvPhoneNumbers, m_strMessage, strTitle, m_arrContents);
                m_rcvPhoneNumbers.Clear();
            }
        }
        ...
        public void EndSend()
        {
            if (m_snuMsg != null && m_isMMS != null && m_rcvPhoneNumbers.Count > 0)
            {
                if (m_isMMS.Data)
                {
                    if (m_arrContents == null)
                        ...
                        m_snuMsg.SendMMSMessage(m_strCaller, m_rcvPhoneNumbers, m_strMessage, m_strTitle, m_arrContents);
                }
                else
                    m_snuMsg.SendSMSMessage(m_strCaller, m_rcvPhoneNumbers, m_strMessage);
            }
    ...
    internal class MessageClientKPX : IMessageClient
    {
        ...
        public MessageClientKPX(string szServerIP, int nSiteID)
        {
            m_szServerIP = szServerIP;
            m_nSiteID = nSiteID;
            m_broker = new MessageBrokerKPX(szServerIP);
        }
    ...
    // 지역난방공사
    internal class MessageClientKDHC : IMessageClient
    {
        private string m_szServerIP = "";
        private int m_msgBufCount = 100;
        private int m_nSiteID = -1;
        ...
        private int m_nSMSLimit = 90;
        private bool m_isBegin = false;
        private MessageBrokerKDHC m_broker = null;

        private string m_strTitle = "";
        private List<KeyValuePair<MessageContentMMS.ContentType, string>> m_arrContents = null;

        public MessageClientKDHC(string szServerIP, int nSiteID)
        {
            m_szServerIP = szServerIP;
            m_nSiteID = nSiteID;
            m_broker = new MessageBrokerKDHC(m_nSiteID);
            //m_broker = new MessageBroker(szServerIP);
        }
    ...
    internal class MessageClientKDHCLG : IMessageClient
    {
        ...
        public MessageClientKDHCLG(string szServerIP, int nSiteID)
        {
            m_szServerIP = szServerIP;
            m_nSiteID = nSiteID;
            m_broker = new MessageBrokerKDHCLG(m_nSiteID);
            //m_broker = new MessageBroker(szServerIP);
        }
    ...
    internal class MessageClientMCS : IMessageClient
    {
        ...
        public MessageClientMCS(string szServerIP, int nSiteID)
        {
            m_szServerIP = szServerIP;
            m_nSiteID = nSiteID;
            m_broker = new MessageBrokerMCS(m_nSiteID);
            //m_broker = new MessageBroker(szServerIP);
        }
