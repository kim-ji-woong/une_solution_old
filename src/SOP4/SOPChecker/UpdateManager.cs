...
namespace SOPChecker
{
    public class UpdateManager
    {
        ...
        public static UpdateManager Instance
        {
            get
            {
                if (m_instance == null)
                {
                    ...
                    Utility m_ini = new Utility("SOPChecker.ini");

                    m_instance.m_strUpdateSrcPath = m_ini.getinivalue("SDMS","update_src");
                    m_instance.m_strUpdateTrgPath = m_ini.getinivalue("SDMS", "update_trg");
                    m_instance.m_strUpdateTempPath = m_ini.getinivalue("SDMS", "update_temp");
                }
        ...
        public void Run()
        {
            ...
            m_strUpdateSrcPath = dbMgr.LoadIni("update_src", "SDMS");
            m_strUpdateTrgPath = dbMgr.LoadIni("update_trg", "SDMS");

            if (m_strUpdateSrcPath.Length > 0 && m_strUpdateTrgPath.Length > 0)
            {
                m_threadUpdate = new Thread(UpdateThread);
