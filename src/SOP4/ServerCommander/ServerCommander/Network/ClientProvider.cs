...
namespace ServerCommander
{
    public class ClientProvider : ClientServiceProvider
    {
            ...
            try
            {
                if (ReceivedData != null)
                {
                    ...
                    if (nBytesCount > 0)
                    {
                        ...
                        m_mgr.RecvLog(ReceivedData);
                        //Debug.WriteLine(ReceivedData[0].ToString());
        ...
        private void SendWhoIam()
        {
            ...
            byte[] dataBytes = MakeBytes((int)TCP_CLIENT.SERVER_COMMANDER);
        ...
        public void SendRunSDMS()
        {
            ...
            byte[] dataBytes = MakeBytes(ServerCommandType.RUN_SDMS);

            byte[] nHeader = BitConverter.GetBytes((short)TCP_ID.SERVER_COMMAND);
            byte[] nCount = BitConverter.GetBytes(1);
        ...
        {
            byte[] bytes = new byte[12];
            byte[] dataBytes = MakeBytes(ServerCommandType.UPDATE_SYSTEM);
