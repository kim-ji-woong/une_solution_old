...
namespace SVMSEventReciver
{
    public class IOManager
    {
        ...
        public void LoadBuildings()
        {
            ...
            LocalDBManager m_dbMgr = SVMSEventReciver.Instance.DBManager;
           ...
           // string strSQL = "select Building.id, BuildingID, BuildingCode, BuildingName, BuildingGroupID, MaxFloor, MinFloor, BuildingGroup.GroupName ";
            ...
            {
                int nID = LocalDBManager.GetIntField(arrResult[i].ToString(), -1);
                string strBuildingID = LocalDBManager.GetStringField(arrResult[i + 1], "");
                string strBuildingCode = LocalDBManager.GetStringField(arrResult[i + 2], "");
                string strBuildingName = LocalDBManager.GetStringField(arrResult[i + 3], "");
                int nBuildingGroupID = LocalDBManager.GetIntField(arrResult[i + 4].ToString(), -1);
                int nMaxFloorID = LocalDBManager.GetIntField(arrResult[i + 5].ToString(), -1);
                int nMinFloorID = LocalDBManager.GetIntField(arrResult[i + 6].ToString(), -1);
                string strBuildingGroupName = LocalDBManager.GetStringField(arrResult[i + 7], "");
        ...
        public void LoadSensorZone()
        {
            ...
            {
                int nID = LocalDBManager.GetIntField(arrResult[i].ToString(), -1);
                int nType = LocalDBManager.GetIntField(arrResult[i + 1].ToString(), -1);
                int nConnected = LocalDBManager.GetIntField(arrResult[i + 2].ToString(), -1);
                int nEquipZoneID = LocalDBManager.GetIntField(arrResult[i + 3].ToString(), -1);
                int nData = LocalDBManager.GetIntField(arrResult[i + 4].ToString(), -1);
                ...
                sensorZone.ID = nID;
                sensorZone.Type = Facility.ToFacilityType(nType);
        ...
        public void LoadEquipmentZone()
        {
            ...
            {
                int nID = LocalDBManager.GetIntField(arrResult[i].ToString(), -1);
                string strEquipZoneName = LocalDBManager.GetStringField(arrResult[i + 1], "");
                string strLinkedZoneIDList = LocalDBManager.GetStringField(arrResult[i + 2], "");
                int nType = LocalDBManager.GetIntField(arrResult[i + 3].ToString(), -1);
                ...
                if (equipZoneType == EquipmentZone.EquipZoneType.NONE)
                    continue;
        ...
        public Reciver FindReciverByType(int nReciverType)
        {
            foreach (Reciver reciver in m_arReciverList)
            {
		...
		private bool LoadCurcuit(Reciver reciver)
		{
			...
			{
                int nID = LocalDBManager.GetIntField(arrResult[i].ToString(), -1);
                int nTagNo = LocalDBManager.GetIntField(arrResult[i + 1].ToString(), -1);

                string strName = LocalDBManager.GetStringField(arrResult[i + 2], "");
                int nSensorType = LocalDBManager.GetIntField(arrResult[i + 3].ToString(), -1);
				//int nEquipZone = WebDBManager.GetIntField(arrResult[i + 4].ToString(), -1);
                int nSensorZoneID = LocalDBManager.GetIntField(arrResult[i + 4].ToString(), -1);

                string strDesc = LocalDBManager.GetStringField(arrResult[i + 5], "");

                if (nSensorZoneID < 0)
                    sensorZone = null;
                else if (!d_SensorZone.TryGetValue(nSensorZoneID, out sensorZone))
                    continue;
				...
				curcuit.SensorType = nSensorType;
                curcuit.SensorZone = sensorZone;
				//curcuit.TargetZoneID = nEquipZone;
				curcuit.Name = strName;

				if (!reciver.Curcuits.ContainsKey(nTagNo))
				{
					reciver.Curcuits.Add(nTagNo, curcuit);
				}
