	...
	public class CCTVManager
	{
        ...
        public bool LoadCCTV(ISensorTooltipOwner view, bool isIndoor)
		{
			...
			for (int i = 0; i < nResultCount - 18; i += 19)
			{
                ...
                bool indoor = DBUtility.WebDBManager.GetIntField(arrResult[i + 16].ToString(), 0) == 0 ? false : true;
                int nReversePTZ = DBUtility.WebDBManager.GetIntField(arrResult[i + 17].ToString(), -1);
                string strURL = DBUtility.WebDBManager.GetStringField(arrResult[i + 18], "");
                ...
                else if (UnE.SOP.ProxySOP.Instance.SiteID == 999)
                {
                    cctv.POI.X = x;
                    cctv.POI.Y = z;
                    cctv.POI.Z = -y;
