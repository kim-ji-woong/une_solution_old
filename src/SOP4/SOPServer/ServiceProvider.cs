    ...
    public class ServiceProvider : TcpServiceProvider
    {
		...
		{            
            ...
            if (szMsg != "")
            {
                ...
                libSMS.MessageContentMMS.ContentType contentType = log.ImagePath.Length > 0 ? libSMS.MessageContentMMS.ContentType.Image : libSMS.MessageContentMMS.ContentType.None;
                
                // Send SMS
                // 메시지 전송 부분만 쓰레드로 처리해야 함. (중요), skkim 2014.12.16
                new Thread(() =>
                {
                    SMSManager.Instance.SendSMS(arrPhoneNumbers, szSendNum, szMsg, true, contentType, log.ImagePath);
        ...
        public static SMSManager.SMSMessageType GetSMSMessageTypeFromLog(SensorReactionLog log)
        {
            ...
            else if (log.Type == SensorReactionLog.ReactionType.IGNORE_S1SVMS_STATUS || log.Type == SensorReactionLog.ReactionType.IGNORE_S1ACCESS_STATUS)
            {
                smsType = SMSManager.SMSMessageType.RESET_SECURITY;
            }
            else if (log.Type == SensorReactionLog.ReactionType.BEGIN_S1SVMS_STATUS || log.Type == SensorReactionLog.ReactionType.BEGIN_S1ACCESS_STATUS)
            {
                smsType = SMSManager.SMSMessageType.DETECT_SECURITY;
            }
            else if (log.Type == SensorReactionLog.ReactionType.NOTIFY_SECURITY)
            {
		...
		public void SendSMS(SensorReactionLog log, SMSManager.SMSMessageType smsType)
		{
            ...
            if (szMsg != "")
            {
                ...
                smsLog.Param2 = log.Param2;
                smsLog.SensorHistoryID = log.SensorHistoryID;
                smsLog.ImagePath = log.ImagePath;
                smsLog.Type = SensorReactionLog.ReactionType.SEND_SMS;
                AddReactionLog(smsLog);
                ...
                new Thread(() =>
                {
                    SMSManager.Instance.SendSMS(arrPhoneNumbers, szSendNum, szMsg, false, contentType, log.ImagePath);
        ...
        private void MonitorNotifyStatusThread(object arg)
        {
			...
			while (!NetworkServer.Instance.FinishProcess)
            {
                ...
                if (_history.LastReactionLog.Type != SensorReactionLog.ReactionType.NOTIFY_FIRE ||
                    _history.LastReactionLog.Type != SensorReactionLog.ReactionType.NOTIFY_SECURITY
                    )
        ...
        public void SendStatusSensorSignal(SensorReactionLog log,ConnectionState state = null)
        {
			...
			if (nSensorZoneID != 0)
			{
                ...
                strOriginSensorTableName = FacilityEx.GetFacilityTypeTable(sensor.Type);
