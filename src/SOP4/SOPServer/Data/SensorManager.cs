	...
	public class SensorManager
	{
        ...
        private void ReadFacilityTypes()
        {
            ...
            for (int i=0;i<nResultCount-1;i+=2)
            {
                ...
                FacilityEx.SetFacilityTypeTable(nID, strTableName);
            }
        }
        ...
        public int ProcessSensorData(IFacility.FacilityType sensorType, int nSensorTagInfoID, int nSensorZoneID, int nSensorData, out int outSensorID, out int data, out bool bconnected, ref int nPrevSensorHistoryID)
        {
            ...
            // 0 ~ 19 : 화재센서 대역
            // 20 ~ 39 : PSM센서 대역
            if (nSensorData == 0)   // 이상 없음
            {
                connected = 1;
                data = 0;
            }
            else if (nSensorData == 1)  // 상황 발생
            {
                connected = 1;
                data = 1;
            }
            else if (nSensorData == 2)  // 통신 끊김
            {
                connected = 0;
                data = 0;
            ...
            //History
            string sqlInsert = "insert into SensorZoneHistory(ID, SensorID,Connected,Data,Time, SiteID) Values('"
                + Max_ID + "','" + nSensorID + "','" + connected + "','" + data + "','" + strDateTimeField + "','" + m_nSiteID + "')";
        ...
        {
            string strCondition = ((int)SensorReactionLog.ReactionType.BEGIN_STATUS).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.BEGIN_PSM_STATUS).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.NOTIFY_FIRE).ToString();
            
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.BEGIN_S1SVMS_STATUS).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.BEGIN_S1ACCESS_STATUS).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.NOTIFY_SECURITY).ToString();
        ...
        {
            string strCondition = ((int)SensorReactionLog.ReactionType.MALFUNCTION).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.IGNORE_FIRE).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.IGNORE_SOP).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.END_STATUS).ToString();
            //strCondition += ", " + ((int)SensorReactionLog.ReactionType.END_PSM_STATUS).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.IGNORE_S1SVMS_STATUS).ToString();
            strCondition += ", " + ((int)SensorReactionLog.ReactionType.IGNORE_S1ACCESS_STATUS).ToString();
        ...
        public void ReadSensorHistory(ServiceProvider provider)
        {
            ...
            foreach (TimeHistory history in arrTimeHistory)
            {
                ...
                // 방범신호에 대해 초기 로드감시 추가
                // skkim 2017-03-26
                else if (history.LastReactionLog.Type == SensorReactionLog.ReactionType.NOTIFY_SECURITY)
        ...
        private void CheckHistory(ref int nMaxReactionID, int nSensorID, SensorReactionLog log, ArrayList arrTimeHistory)
        {
            ...
            if (log.Type == SensorReactionLog.ReactionType.IGNORE_S1ACCESS_STATUS ||
            ...
            else if (log.Type == SensorReactionLog.ReactionType.END_STATUS)
            {
