...
namespace WebApplication1
{
    public partial class Main : System.Web.UI.Page
    {
        ...
        private const string DATA_DB = "SOP_1";
        private const string ALARM_DB = "AlarmSimulation";

        private const int m_nSiteID = 1;
        //private const int m_nOutPort = 10092;

        private const string SessionFireEnabled = "FireEnabled";
        private const string SessionPSMEnabled = "PSMEnabled";
        private const string SessionEarthquakeEnabled = "EarthquakeEnabled";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                int nEarthquakeCount = GetAlarmCount("지진");

                Session[SessionFireEnabled] = nFireCount == 0;
                Session[SessionPSMEnabled] = nPSMCount == 0;
                Session[SessionEarthquakeEnabled] = nEarthquakeCount == 0;*/
            ...
            btnPSM.Enabled = Session[SessionPSMEnabled] == null ? true : (bool)Session[SessionPSMEnabled];
            btnEarthquake.Enabled = Session[SessionEarthquakeEnabled] == null ? true : (bool)Session[SessionEarthquakeEnabled];
        ...
        private void InitSessions()
        {
            ...
            Session[SessionEarthquakeEnabled] = true;
            ...
            foreach (object obj in arrResult)
            {
                if (obj.ToString().Trim() == "오염")
                {
                    Session[SessionPSMEnabled] = false;
                    break;
                }
            }
            ...
            foreach (object obj in arrResult)
            {
                if (obj.ToString().Trim() == "지진")
                {
                    Session[SessionEarthquakeEnabled] = false;
                    break;
                }
            }
        }
        ...
        protected void btnPSM_Click(object sender, EventArgs e)
        {
            if (btnPSM.Enabled == false)
                return;

            btnPSM.Enabled = false;
            Session[SessionPSMEnabled] = false;

            int nMaxID = GetMaxData("RequestAlarm", ALARM_DB);

            string strSQL = "Insert into RequestAlarm (ID, AlarmCategory, AlarmParameter, SiteID) values (" + (nMaxID + 1).ToString();
            strSQL += ", '오염', '1\t암모니아', " + m_nSiteID.ToString() + ")";
            ExecuteData(strSQL, ALARM_DB);
            //GetResultData(strSQL, 0, ALARM_DB);
        }

        protected void btnEarthquake_Click(object sender, EventArgs e)
        {
            if (btnEarthquake.Enabled == false)
                return;
            ...
            string strSQL = "Insert into RequestAlarm (ID, AlarmCategory, AlarmParameter, SiteID) values (" + (nMaxID + 1).ToString();
            strSQL += ", '지진', '진도\t3', " + m_nSiteID.ToString() + ")";
            ExecuteData(strSQL, ALARM_DB);
            //GetResultData(strSQL, 0, ALARM_DB);
        }
        ...
        protected void btnStopPSM_Click(object sender, EventArgs e)
        {
            string strSQL = "Select SensorTagInfoID from AlarmBoard where SiteID = " + m_nSiteID.ToString();
            ArrayList arrResult = GetReadData(strSQL, ALARM_DB);
            //ArrayList arrResult = GetResultData(strSQL, 0, ALARM_DB);
            ...
            foreach (object obj in arrResult)
            {
                if (int.TryParse(obj.ToString().Trim(), out nSensorTagInfoID))
                {
                    string strParam = "0\t" + nSensorTagInfoID.ToString();
                    string strSQL2 = "Insert into RequestAlarm (ID, AlarmCategory, AlarmParameter, SiteID) values (" + (nMaxID + 1).ToString();
                    strSQL2 += ", '오염', '" + strParam + "', " + m_nSiteID.ToString() + ")";
                    ExecuteData(strSQL2, ALARM_DB);
                    //GetResultData(strSQL2, 0, ALARM_DB);
                }
            }
        ...
        protected void Timer1_Tick(object sender, EventArgs e)
        {
            if ((bool)Session[SessionFireEnabled] == false ||
                (bool)Session[SessionPSMEnabled] == false ||
                (bool)Session[SessionEarthquakeEnabled] == false)
            {
                string strSQL = "select AlarmCategory from RequestAlarm group by AlarmCategory, SiteID having SiteID = " + m_nSiteID.ToString();
                ArrayList arrResult = GetReadData(strSQL, ALARM_DB);
                //ArrayList arrResult = GetResultData(strSQL, 0, ALARM_DB);
                ...
                if ((bool)Session[SessionEarthquakeEnabled] == false)
                {
                    bool find = false;
