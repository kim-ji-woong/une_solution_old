...
namespace TeamEditor
{
    public class TeamTreeView : TreeView
    {
        ...
        private bool LoadExternalCompanyTeam()
        {
            ...
            //arrResults = m_dbMgr.GetResultData(strSQL, 0);
        ...
        private void TeamTreeView_DragDrop(object sender, System.Windows.Forms.DragEventArgs e)
        {
            ...
            if (m_dropType == DropDataType.REGULAR_MEMBER)
            {
                ...
                if (m_dropData is Command.CommandMoveRegularMembers)
                {
                    OnDropRegularMembers((Command.CommandMoveRegularMembers)m_dropData, dropNode);
                }
            ...
            else if (m_dropType == DropDataType.TEMPORARY_MEMBER)
            {
                ...
                if (m_dropData is Command.CommandMoveTemporaryMembers)
                {
                    OnDropTemporaryMembers((Command.CommandMoveTemporaryMembers)m_dropData, dropNode);
                }

                return;
            }
            else if (m_dropType == DropDataType.EXTERNAL_MEMBER)
            {
                if (m_dropData == null)
                    return;

                if (m_dropData is Command.CommandMoveExternalMembers)
                {
                    OnDropExternalMembers((Command.CommandMoveExternalMembers)m_dropData, dropNode);
                }
        ...
        private void OnDropExternalMembers(Command.CommandMoveExternalMembers cmd, TreeNode dropNode)
        {
            this.timer.Enabled = false;
            FormMain.Instance.OnDropExternalMembers(cmd, dropNode);
        }

        private void OnDropTemporaryMembers(Command.CommandMoveTemporaryMembers cmd, TreeNode dropNode)
        {
            // Disable scroll timer
            this.timer.Enabled = false;
        ...
        private void OnDropRegularMembers(Command.CommandMoveRegularMembers cmd, TreeNode dropNode)
        {
            // Disable scroll timer
            this.timer.Enabled = false;
