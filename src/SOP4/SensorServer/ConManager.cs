...
namespace SensorServer
{
	...
	class ConManager
	{
		...
		private ControlMonitoring.Utility m_ini = new ControlMonitoring.Utility();
		...
		public ConManager()
		{
			...
			SensorService.WriteLine("READ IP : " + m_strServerIP);
			try
			{
				string idpass = m_ini.getinivalue(strSection, "dbCon");
				SensorService.WriteLine("READ ENC : " + idpass);
				string strDec = DBUtility.AES256Cipher.AES_decrypt(idpass, key);
				SensorService.WriteLine("READ DEC : " + strDec);
				m_strServerID = strDec.Substring(0, strDec.IndexOf('|'));
				m_strServerPW = strDec.Substring(strDec.IndexOf('|') + 1);

				SensorService.WriteLine("READ ID : "+ m_strServerID);
				SensorService.WriteLine("READ PASS : " + m_strServerPW);
			}
			catch (System.Exception e)
			{
                SensorService.WriteLine(e.Message);
			}
            
			m_strConnection = GetConnectionInfo();
            SensorService.WriteLine(m_strConnection);
