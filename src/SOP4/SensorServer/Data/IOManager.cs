    ...
    public class IOManager
    {
        ...
        public Reciver FindReciver(int nReciverID)
        {
            foreach (Reciver reciver in m_arReciverList)
            {
                ...
                if (reciver.ReciverID == nReciverID)
                {
                    return reciver;
                }
            }
		...
		{
            WebDBManager m_dbMgr = NetworkServer.Instance.DBManager;
            string strSQL = "select ID,Place, IP, MacAddr, Baudrate, Mode, FlowCtrl, Multiport, Timeout, Description, ReciverID from SensorServerInfo where SiteID =" + m_nSiteID.ToString();
