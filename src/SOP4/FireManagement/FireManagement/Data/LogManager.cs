...
namespace FireManagement
{
    public class LogManager
    {
        ...
        private void WriteCheckLog(FireEquipment equip, DateTime dtNow, FireEquipmentHistory equipHistory)
        {
            ...
            // RFIDReader로 Tag를 읽지 않고 프로그램에서만 수정한 데이터는 Log에 기록하지 않는다.
            // 작업자가 현장에서 실제로 RFID Tag를 읽은것만 로그로 저장한다.
            ...
            if (!logExist)
            {
                ...
                if (m_writer == null || m_strLogFilePath != strFilePath)
                {
                    ...
                    string strColumns = string.Format("번호{0}점검시간{0}타입{0}관리번호{0}설비정보{0}설비상태{0}Zone이름{0}건물명{0}건물그룹{0}점검자의견", m_strDelimeter);
                ...
                if (equipHistory != null)
                    history.CheckersOpinion = equipHistory.CheckersOpinion;
        ...
        private void WriteEquipmentLog(EquipmentCheckHistory history)
        {
            ...
            else
            {
                ...
                if (history.Equipment.Type == FireEquipment.EquipmentType.FE)
                    ...
                    m_nFACheckCount++;
                else if (history.Equipment.Type == FireEquipment.EquipmentType.FR)
                    m_nFRCheckCount++;
            }
            ...
            string strLine = string.Format("{0}{11}{1:00}:{2:00}:{3:00}{11}{4}{11}{5}{11}{6}{11}{7}{11}{8}{11}{9}{11}{10}{11}{12}",
                num,
