...
namespace FireManagement
{
    ...
    public partial class FormMain2 : Form, ITextPictureBoxOwner, IRibbonButtonOwner
    {
        ...
        private Dictionary<FireEquipment.EquipmentType, ControlLayer> m_dicEquipmentLayer = new Dictionary<FireEquipment.EquipmentType, ControlLayer>();
        ...
        private string m_strIndoorFileType = "png";
        private string m_strIndoorFolderPath = "FEData\\DXF";
        private Control2D.ControlMode m_drawingControlMode = Control2D.ControlMode.NONE;
        ...
        {
            get { return m_frmPanel2.DrawingControl; }
        }
        ...
        {
            get { return m_strIndoorFileType; }
            set { m_strIndoorFileType = value; }
        }

        public string IndoorFolderPath
        {
            get { return m_strIndoorFolderPath; }
            set { m_strIndoorFolderPath = value; }
        }

        public Control2D.ControlMode DrawingControlMode
        {
            get { return m_drawingControlMode; }
        }
        ...
        public void SetEquipmentLayer(FireEquipment.EquipmentType type, ControlLayer layer)
        {
        ...
        private void InitMode()
        {
            ...
            m_strIndoorFileType = m_ini.getinivalue("General", "IndoorFileType", strPath);
            m_drawingControlMode = Control2D.GetControlMode(m_strIndoorFileType);
            m_strIndoorFolderPath = m_ini.getinivalue("General", "IndoorFolderPath", strPath);
        ...
        private void LoadDB()
        {
            ...
            {
                m_strDataFilePath = System.Windows.Forms.Application.StartupPath + "\\" + IOManager.TabletDataFile;

                bool isPCMode = !m_isPCMode;
                m_fileMgr.FirstRead = true;
                m_fileMgr.ImportData(m_strDataFilePath, ref isPCMode);
        ...
        {
            string strSQL = "Select PropertyName, PropertyValue from OptionFireEquipment where SiteID = " + m_nSiteID.ToString();
            ...
            for (int i=0;i<nResultCount-1;i+=2)
            {
                ...
                if (string.Compare(strPropertyName, "IndoorFileType", true) == 0)
                    m_strIndoorFileType = strPropertyValue;
                else if (string.Compare(strPropertyName, "IndoorFolderPath", true) == 0)
                    m_strIndoorFolderPath = strPropertyValue;
                else if (string.Compare(strPropertyName, "ViewerBackColor", true) == 0)
                    ReadColorString(strPropertyValue, ref m_backColor);
            }
        }

        private bool ReadColorString(string strColor, ref Color color)
        {
            string[] arrTokens = strColor.Split(',');

            int r, g, b, a;
            string strA = "255", strR = "", strG = "", strB = "";

            int nTokenCount = arrTokens.Count();

            if (nTokenCount >= 3)
            {
                strR = arrTokens[0].Trim();
                strG = arrTokens[1].Trim();
                strB = arrTokens[2].Trim();

                if (nTokenCount == 4)
                    strA = arrTokens[3].Trim();
                else if (nTokenCount > 4)
                    return false;
            }
            else
                return false;

            if (int.TryParse(strR, out r) && int.TryParse(strG, out g) && int.TryParse(strB, out b) && int.TryParse(strA, out a))
            {
        ...
        private void InitPanels()
        {
            ...
            m_frmPanel2.DrawingControl.BackColor = m_backColor;
        ...
        public void SelectCheckEquipTab()
        {
            pictureBoxNormalMode.BackgroundImage = global::FireManagement.Properties.Resources.MgrTab_normal_;
            pictureBoxEditMode.BackgroundImage = global::FireManagement.Properties.Resources.MgrTab_normal_;
            pictureBoxHistory.BackgroundImage = global::FireManagement.Properties.Resources.MgrTab_normal_;
        ...
        public float GetUnitFlag(UnitOfLength unitTrg)
        {
            UnitOfLength unitSrc = UnitOfLength.METER;

            if (m_frmPanel2 == null || DrawingControl == null)
            {
                Control2D ctrl = new Control2D(m_drawingControlMode, this);
                unitSrc = ctrl.UnitOfLength;
                ctrl.Control.Dispose();
            }
            else
                unitSrc = DrawingControl.UnitOfLength;
            /*if (DrawingControl == null)
                return 1.0f;

            UnitOfLength unitSrc = DrawingControl.UnitOfLength;*/

            if (unitSrc == UnitOfLength.INCH)
            {
                if (unitTrg == UnitOfLength.INCH)
                    return 1.0f;
                else if (unitTrg == UnitOfLength.FEET)
                    return 1.0f / 12;
                else if (unitTrg == UnitOfLength.MILLIMETER)
                    return 25.4f;
                else if (unitTrg == UnitOfLength.CENTIMETER)
                    return 2.54f;
                else if (unitTrg == UnitOfLength.METER)
                    return 0.0254f;
            }
            else if (unitSrc == UnitOfLength.FEET)
            {
                if (unitTrg == UnitOfLength.INCH)
                    return 12.0f;
                else if (unitTrg == UnitOfLength.FEET)
            ...
            else if (unitSrc == UnitOfLength.MILLIMETER)
            {
                if (unitTrg == UnitOfLength.INCH)
                    return 1.0f / 25.4f;
                else if (unitTrg == UnitOfLength.FEET)
            ...
            else if (unitSrc == UnitOfLength.CENTIMETER)
            {
                if (unitTrg == UnitOfLength.INCH)
                    return 1.0f / 2.54f;
                else if (unitTrg == UnitOfLength.FEET)
            ...
            else if (unitSrc == UnitOfLength.METER)
            {
                if (unitTrg == UnitOfLength.INCH)
                    return 1.0f / 0.0254f;
                else if (unitTrg == UnitOfLength.FEET)
        ...
        public ControlLayer GetEquipmentLayer(FireEquipment.EquipmentType type)
        {
        ...
        public FireEquipment.EquipmentType GetEquipmentLayerType(ControlLayer layer)
        {
            foreach (KeyValuePair<FireEquipment.EquipmentType, ControlLayer> pair in m_dicEquipmentLayer)
            {
                if (pair.Value == layer)
                    return pair.Key;
            }
        ...
        public void SetEquipmentLayerOnOff()
        {
            ...
            EquipmentLayerOn(FireEquipment.EquipmentType.FR, m_frmPanel2.BtnFireReceiver.IsChecked, false);

            ViewControl.LeftBar.Rearrange(m_frmPanel2.BtnFireExtinguisher.IsChecked, m_frmPanel2.BtnFirePlug.IsChecked, m_frmPanel2.BtnFireAlarm.IsChecked, m_frmPanel2.BtnFireReceiver.IsChecked);
        }
        public void EquipmentLayerOn(FireEquipment.EquipmentType type, bool turnOn, bool refresh = true)
        {
            if (m_dicEquipmentLayer.ContainsKey(type))
            {
                ControlLayer layer = m_dicEquipmentLayer[type];
                layer.Hidden = !turnOn;
        ...
        public void Save()
        {
            if (IsPCMode)
                ...
                m_pageSave.SaveToFile(false, false);

            m_ioMgr.ChangedZones.Clear();
            ...
            if (zoneCurrent != null)
            {
                m_ioMgr.MakeSameEquipments(zoneCurrent);
            }
        }
        ...
        private void FormMain2_Resize(object sender, EventArgs e)
        {
            ...
            //    FormFrame.Instance.TitlePosition = (this.Width / 2) - (FormFrame.Instance.TitleTextWidth / 2);
            //}
            FormFrame.Instance.TitlePosition = new Point((this.Width / 2) - (FormFrame.Instance.TitleTextWidth / 2), 2);
            //FormFrame.Instance.();
