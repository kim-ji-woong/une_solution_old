...
namespace FireManagement
{
    public partial class PageBackstageSave2 : Form
    {
        ...
        public void SaveToFile(bool showResultMessage, bool showFileDialog = true)
        {
            ...
            else
            {
                ...
                if (!showFileDialog || dlg.ShowDialog() == DialogResult.OK)
                {
                    string strFilePath = showFileDialog ? dlg.FileName : FormMain2.Instance.DataFilePath;
                    FormMain2.Instance.ViewControl.LeftBar.ApplyGridData();
                    string strResult = "";

                    if (FormMain2.Instance.FileManager.ExportData(strFilePath))
        ...
        private bool WriteEquipmentIDToFile(FireEquipment equip, Dictionary<object, DataGridViewRow> dicShapes, System.IO.StreamWriter writer, ref string strErrorMessage)
        {
            if (equip == null)
                return true;
        ...
        public bool WriteEquipmentIDsToFile(Dictionary<object, DataGridViewRow> dicFEShapes, Dictionary<object, DataGridViewRow> dicHDShapes, Dictionary<object, DataGridViewRow> dicFAShapes, string strFilePath, ref string strErrorMessage)
        {
            try
            {
                System.IO.StreamWriter writer = new System.IO.StreamWriter(strFilePath, false, Encoding.UTF8);

                foreach (KeyValuePair<object, DataGridViewRow> pair in dicFEShapes)
                {
                    if (!WriteEquipmentIDToFile((FireEquipment)pair.Value.Tag, dicFEShapes, writer, ref strErrorMessage))
                        return false;
                }

                foreach (KeyValuePair<object, DataGridViewRow> pair in dicHDShapes)
                {
                    if (!WriteEquipmentIDToFile((FireEquipment)pair.Value.Tag, dicHDShapes, writer, ref strErrorMessage))
                        return false;
                }

                foreach (KeyValuePair<object, DataGridViewRow> pair in dicFAShapes)
                {
        ...
        private bool CheckDuplicateEquipID(FireEquipment equip, Dictionary<object, DataGridViewRow> dicShapes, ref string strErrorMessage)
        {
            foreach (KeyValuePair<object, DataGridViewRow> pair in dicShapes)
            {
                if (pair.Value == null)
                    continue;
