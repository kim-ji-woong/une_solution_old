...
namespace SOPBulletin
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            Pane pane;
            pane = axDockingPane.FindPane(1);
            pane.MinTrackSize.SetSize(this.Width, 160);
            pane.MaxTrackSize.SetSize(this.Width, 160);
        ...
        private void TimeServerThread()
        {
            ...
            currentTime = currentTime.AddHours(FormMain.Instance.m_dbMgr.ServerTime.AddTime);
        ...
        public DateTime GetNetworkTime()
        {
            try
            {
                string ntpServer = FormMain.Instance.m_dbMgr.ServerTime.TimeServerAddress;
        ...
        {
            Pane paneProgress = axDockingPane.CreatePane(1, 500, 210, DockingDirection.DockBottomOf, null);
            paneProgress.Title = "SOP 진행현황";
            paneProgress.Options = PaneOptions.PaneNoCloseable;

            Pane paneRealTime = axDockingPane.CreatePane(0, 500, 400, DockingDirection.DockTopOf, paneProgress);
            paneRealTime.Title = "실시간 SOP 현황";
            paneRealTime.Options = PaneOptions.PaneNoCloseable;

            m_arrDocking[0] = new DockingRealTime();
            m_dockRealTime = (DockingRealTime)m_arrDocking[0];

            m_arrDocking[1] = new DockingProgress();
            m_dockProgress = (DockingProgress)m_arrDocking[1];
        ...
        private void ReadMessage(int nActionStepHistoryID)
        {
            ...
            string strSQL = string.Format("select id, SendTime, Message from Message where ActionStepHistoryID = {0} and SendTime > {1}",
            ...
            for (int i = 0; i < nResultCount - 2; i += 3)
            {
                ...
                m_dockRealTime.AddMessageRow(dtMessage, strMessage, currentActionStepData);
                FileWrite(dtMessage);
            }
        }
