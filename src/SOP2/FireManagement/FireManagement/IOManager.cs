...
namespace FireManagement
{
    public class IOManager
    {
        ...
        /*public void LoadZoneEquipments(ArrayList arrEquipments, Zone zone)
        {
            WebDBManager dbMgr = FormMain2.Instance.DBManager;
            string strSQL = "select id, RFIDTag, EquipID, RFIDTagID, DxfObjID, Description from FireEquipment where ZoneID = " + zone.ID.ToString();

            ArrayList arrResult = dbMgr.GetResultData(strSQL, 0);
            if (arrResult == null)
                return;

            Dictionary<string, FireEquipment> dicEquipment = new Dictionary<string, FireEquipment>();

            foreach (FireEquipment equip in arrEquipments)
            {
                dicEquipment[equip.DXFObjID] = equip;
            }
            ...
            for (int i = 0; i < nResultCount - 5; i += 6)
            {
                ...
                if (!dicEquipment.ContainsKey(strDxfObjID))
                    continue;

                FireEquipment equip = dicEquipment[strDxfObjID];

                equip.ID = nID;
                equip.RFIDTag = strRFIDTag == "null" ? "" : strRFIDTag;
                equip.EquipID = strEquipID == "null" ? "" : strEquipID;
                equip.RFIDTagID = strRFIDTagID == "null" ? "" : strRFIDTagID;
                equip.DXFObjID = strDxfObjID == "null" ? "" : strDxfObjID;
                equip.Zone = zone;
        ...
        /*public ArrayList LoadZone(WebDBManager dbMgr, ArrayList arrZone)
        {
            string strSQL = "SELECT ID, ZoneName, SiteID, BuildingID, FloorIndex FROM Zone";
            ...
            for (int i = 0; i < nResultCount - 4; i += 5)
            {
                ...
                string strZoneName = DBUtility.WebDBManager.GetStringField(arrResult[i + 1], "");
                int nSiteID = DBUtility.WebDBManager.GetIntField(arrResult[i + 2].ToString(), 0);
                int nBuildingID = DBUtility.WebDBManager.GetIntField(arrResult[i + 3].ToString(), 0);
                int nFloorIndex = DBUtility.WebDBManager.GetIntField(arrResult[i + 4].ToString(), 0);
            }
        ...
        {
            int nTotalEquipCount = 0, nTotalFECount = 0, nTotalHDCount = 0, nTotalFACount = 0;
            string strLog = "";

            foreach (KeyValuePair<Zone, ArrayList> pair in m_dicZoneEquipments)
            {
                int nFECount = 0, nHDCount = 0, nFACount = 0;
            ...
            nTotalEquipCount = nTotalFECount + nTotalHDCount + nTotalFACount;
            ...
            writer.WriteLine(string.Format("건물그룹{0}건물명{0}Zone이름{0}소화기개수{0}소화전개수{0}발신기개수", strDelimeter));
    ...
    /*public class Zone
    {
        ...
        public int BuildeingID
        {
            get { return m_nBuildeingID; }
            set { m_nBuildeingID = value; }
        }
