...
namespace SOPManager
{
    ...
    {
        ArrayList m_arrCategory = new ArrayList();
        ArrayList m_arrSub = new ArrayList();
        ArrayList m_arrDetail = new ArrayList();
        ArrayList m_arrActionStep = new ArrayList();

        int m_nRegular = 0;     // 0이면 비등록 모드
        bool m_isNormal = true; // 평일 모드 인가?
        bool m_isNormalOrigin = true;   // 초기값 저장

        Sections.SectionTransSOP m_section = null;

        private bool m_bLeftMouseDown = false;
        private Point m_ptMove = new Point();

        public PopupTransSOP(Sections.SectionTransSOP section)
        {
            InitializeComponent();
            ...
            bool isRegular = FormMain.Instance.GetPageDisaster().IsRegularMode();
            m_isNormal = FormMain.Instance.GetPageDisaster().IsWeekMode();
        ...
        private void GetCurrentVersion(Sections.Section section)
        {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            FormMain.Instance.GetPageLevel().GetPropertiesTransSOP().FullPath = treeView.SelectedNode.FullPath;
            FormMain.Instance.GetPageLevel().GetPropertiesTransSOP().SetSectionTitle(treeView.SelectedNode, strValue);
