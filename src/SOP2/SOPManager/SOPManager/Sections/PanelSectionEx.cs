    ...
    public partial class PanelSectionEx : PanelSection
    {
        ...
        protected override void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            if (m_arrDragDropDrawing != null)
                e.Graphics.DrawPolygon(bBoundPen, m_arrDragDropDrawing);
        ...
        protected Point GlobalToScreen(PointF pt)
        {
        ...
        private void PopupSortPanel(int x, int y)
        {
            ...
            bool enableDelete = FormMain.Instance.GetPageLevel().GetBarPage().IsEnableDelete(nCurrentIndex);
        ...
        protected override void OnMouseDown(object sender, MouseEventArgs e)
        {
            ...
            else if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
				...
				{
					groupContextMenuStrip.Items[2].Enabled = true;
					groupContextMenuStrip.Items[3].Enabled = false;
				...
				else
				{
					...
					if (m_arrDragDropDrawing != null && m_sectionDragDropType != Section.ComponentType.NONE)
					{
						...
						BarComponent form = FormMain.Instance.GetPageLevel().GetBarComponent();
						form.ClearSelection();
					}
        ...
        private bool LinkSection(float x, float y)
        {
            ...
            if (sectionBegin.AddArrow(m_tempArrow))
            {
                if (sectionEnd.AddArrow(m_tempArrow))
                {
                    result = true;
                    m_tempArrow.CalcArrowLine();
                }
                else
                    sectionBegin.RemoveArrow(m_tempArrow);
            }
        ...
        {
            if (MessageBox.Show("현재 패널의 왼쪽에 새로운 패널을 추가합니다.\r\n이 작업은 현재 열려있는 모든 탭들에 영향을 주게 됩니다.\r\n계속하시겠습니까?", "알림", MessageBoxButtons.YesNo)
                == DialogResult.Yes)
            {
        ...
        public static ArrayList GetTabPageTeamList(TabPage tabPage)
        {
            ...
            foreach (Control ctrl in tabPage.Controls)
            {
                if (ctrl.GetType() == type)
                {
                    ...
                    nTeamData = nTeamData | panel.TeamType;
        ...
        void deleteToolStripMenuItem_Click(object sender, System.EventArgs e)
        {
                ...
                == DialogResult.Yes)
            {
                ...
                if (FormMain.Instance.GetPageLevel().GetBarPage().DeleteDataGrid(nPanelIndex))
                {
                    ...
                    //tabPage.Controls.RemoveAt(tabPage.Controls.GetChildIndex(this));
                    FormMain.Instance.GetPageLevel().PanelResize();
                }
            }
        }
			...
			if (e.ClickedItem.Text == "그룹화")
			{
				...
				{					
					...
					else
					{
						...
						MessageBox.Show(szMsg, "그룹생성오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
						ResumeRefresh();
						return;
					}
				}
