...
namespace SOPDisasterSystem
{
    public partial class FormRightSituation : Form
    {
        ...
        private ISOPInfo m_sopInfo = null;
        ...
        {
            if (m_frmMain.LayoutForm != null)
            {
                userControl.DigitText = SOPMonitoringSystem.FlexTimer.Now.ToString("HH:mm:ss");//DateTime.Now.ToString("HH:mm:ss");
                m_frmMain.LayoutForm.RedrawWindow();
                ReadSOPInfo();
            }
        }

        private bool FailSOPInfo()
        {
            labelActivity.Text = "";
            dataGridSenario.Rows[0].Cells[1].Value = "";
            dataGridSenario.Rows[1].Cells[1].Value = "";
            dataGridSenario.Rows[2].Cells[1].Value = "";
            return false;
        }

        private bool ReadSOPInfo()
        {
            if (m_sopInfo == null)
                return FailSOPInfo();

            Sections.WorkFlow work = m_sopInfo.GetCurrentWorkflow();

            if (work == null)
                return FailSOPInfo();

            string strSOPName = work.SOPName;
            
            int nIndex1 = strSOPName.IndexOf('\\');
            if (nIndex1 < 0)
                return FailSOPInfo();

            int nIndex2 = strSOPName.IndexOf('\\', nIndex1 + 1);
            if (nIndex2 < 0)
                return FailSOPInfo();

            dataGridSenario.Rows[0].Cells[1].Value = strSOPName.Substring(0, nIndex1);
            dataGridSenario.Rows[1].Cells[1].Value = strSOPName.Substring(nIndex1 + 1, nIndex2 - (nIndex1 + 1));
            dataGridSenario.Rows[2].Cells[1].Value = work.Position;

            SOPMonitoringSystem.FormMain.Instance.FrmMain3.Title = (string)dataGridSenario.Rows[1].Cells[1].Value;

            labelActivity.Text = strSOPName.Substring(nIndex2 + 1).Replace('\\', szDeli);
        ...
        {
            string[] strValue = strData.Split(szDeli);
        ...
        public void SetSOPInfo(ISOPInfo info)
        {
            m_sopInfo = info;
        }

        private void FormRightSituation_Load(object sender, EventArgs e)
        {

        }

        private void FormRightSituation_FormClosing(object sender, FormClosingEventArgs e)
        {
            timer1.Stop();
