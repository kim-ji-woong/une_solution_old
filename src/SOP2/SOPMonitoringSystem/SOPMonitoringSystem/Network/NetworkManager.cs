...
namespace SDMS
{
    public class NetworkManager
    {
        ...
        public SDMS.ClientProvider ClientProvier
        {
        ...
        {
			//FireDetectSignal signal = this.FindDetectSignal(nSensorHistoryID);
			//if( signal == null)
			//	return;

			//int nSensorID = signal.SensorID;
			//int nZoneID = signal.EquipZoneID;

            byte[] sensorHistoryBytes = ClientProvider.MakeBytes(nSensorHistoryID);
			//byte[] sensorIdBytes = ClientProvider.MakeBytes(nSensorID);
			//byte[] zoneIdBytes = ClientProvider.MakeBytes(nZoneID);
			...
			//nOffset += sensorHistoryBytes.Length;
			//System.Buffer.BlockCopy(sensorIdBytes, 0, bytes, nOffset, sensorIdBytes.Length);
			//nOffset += sensorIdBytes.Length;
			//System.Buffer.BlockCopy(zoneIdBytes, 0, bytes, nOffset, zoneIdBytes.Length); 
			Send(bytes, m_provider);
        }
		...
		public void SendSelectMission(int nActionStepHistory, int nRealMode, int nCompHistoryID, int nRow)
		{
			...
			byte[] ipBytes = ClientProvider.MakeBytes(nRow);
