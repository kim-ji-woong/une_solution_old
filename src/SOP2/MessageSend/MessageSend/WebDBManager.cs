...
namespace MessageSend
{
    public class WebDBManager
    {
        ...
        public void Loadini_ServerConnectionInfo()
        {
            ...
            m_strWebServerURL = DBUtility.RegUtil.ReadRegValue(strSection, "webserver_url");
            if (m_strWebServerURL == null || m_strWebServerURL == "")
            {
                m_strWebServerURL = m_ini.getinivalue(strSection, "webserver_url");
                if (m_strWebServerURL == null || m_strWebServerURL == "")
                    m_strWebServerURL = "http://unes.iptime.org:9805/SOP";

                DBUtility.RegUtil.WriteRegValue("Server Connection Info", "webserver_url", m_strWebServerURL);
            }
        }
