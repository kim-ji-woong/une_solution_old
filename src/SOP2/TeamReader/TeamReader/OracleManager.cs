...
namespace TeamReader
{
    public class OracleManager
    {
        ...
        /*private int GetEmptyIndex(string str, int nStartIndex)
        {
            int nLen = str.Length;
        ...
        public bool LoadTeamList(Dictionary<string, RegularTeam> dicRegularTeam, Dictionary<RegularTeam, string> dicTeamLeader)
        {
            ...
            System.IO.StreamReader reader = new System.IO.StreamReader("F:\\서버설치\\aaa.txt", Encoding.UTF8);
            char[] arrTrim = new char[] { ' ', '\t', '\r', '\n' };

            while (!reader.EndOfStream)
            {
                string strLine = reader.ReadLine();
                strLine = strLine.TrimStart(arrTrim);
                strLine = strLine.TrimEnd(arrTrim);

                int nIndex = GetEmptyIndex(strLine, 0);
                if (nIndex < 0) continue;

                string strTeamCode = strLine.Substring(0, nIndex);
                strLine = strLine.Substring(nIndex + 1).TrimStart(arrTrim);

                nIndex = GetEmptyIndex(strLine, 0);
                if (nIndex < 0) continue;

                string strUpCode = strLine.Substring(0, nIndex);
                strLine = strLine.Substring(nIndex + 1).TrimStart(arrTrim);

                nIndex = GetEmptyIndex(strLine, 0);
                if (nIndex < 0) continue;

                string strTeamName = strLine.Substring(0, nIndex);
                strLine = strLine.Substring(nIndex + 1).TrimStart(arrTrim);

                nIndex = GetEmptyIndex(strLine, 0);
                string strTeamOrder = "", strTeamLeader = "";

                if (nIndex < 0)
                {
                    strTeamOrder = strLine;
                }
                else
                {
                    strTeamOrder = strLine.Substring(0, nIndex);
                    strLine = strLine.Substring(nIndex + 1).TrimStart(arrTrim);
                    strTeamLeader = strLine.TrimEnd(arrTrim);
                }

                RegularTeam team = new RegularTeam();

                team.TeamCode = strTeamCode;
                dicTeamCode[team] = strUpCode;
                team.TeamName = strTeamName;
                dicTeamLeader[team] = strTeamLeader;
        ...
        private bool GetTeamInfoFromCSV(string strLine, out string strTeamCode, out string strParentTeamCode, out string strTeamName, out string strTeamOrder, out string strTeamLeaderCode)
        {
            ...
            int nIndex4 = strLine.IndexOf(',', nIndex3 + 1);
            if (nIndex4 < 0)
                return false;
        ...
        private bool GetMemberInfoFromCSV(string strLine, out string strMemberCode, out string strTeamCode, out string strLevelNo, out string strEMail, out string strMemberName, out string strPhoneNumber, out string strPosition, out bool isTeamLeader)
        {
            strMemberCode = strTeamCode = strLevelNo = strEMail = strMemberName = strPhoneNumber = strPosition = "";
            isTeamLeader = false;
            ...
            int nIndex5 = strLine.IndexOf(',', nIndex4 + 1);
            if (nIndex5 < 0)
                return false;

            int nIndex6 = strLine.IndexOf(',', nIndex5 + 1);
            if (nIndex6 < 0)
                return false;

            int nIndex7 = strLine.IndexOf(',', nIndex6 + 1);
            if (nIndex7 < 0)
                return false;
            ...
            strPhoneNumber = strLine.Substring(nIndex5 + 1, nIndex6 - nIndex5 - 1);
            strPosition = strLine.Substring(nIndex6 + 1, nIndex7 - nIndex6 - 1);
            string strTeamLeader = strLine.Substring(nIndex7 + 1);
        ...
        private bool LoadCompanyMemberListFromCSV(/*Out*/Dictionary<string, CompanyMember> dicCompanyMember, /*In*/Dictionary<string, RegularTeam> dicRegularTeam, Dictionary<RegularTeam, string> dicTeamLeader)
        {
            ...
            string strMemberCode, strTeamCode, strLevelNo, strEMail, strMemberName, strPhoneNumber, strPosition;
            bool isTeamLeader;
            ...
            {
                string strLine = reader.ReadLine();
                if (!GetMemberInfoFromCSV(strLine, out strMemberCode, out strTeamCode, out strLevelNo, out strEMail, out strMemberName, out strPhoneNumber, out strPosition, out isTeamLeader))
                    break;
                ...
                member.OfficePhoneNumber = "0000";
                member.PhoneNumber = AES256Cipher.AES_encrypt(strPhoneNumber, key);
