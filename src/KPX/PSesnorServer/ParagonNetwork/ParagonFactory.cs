...
namespace ParagonNetwork
{
    internal class ParagonFactory
    {
        ...
        private ParagonManager m_Main = null;
        
        public ParagonFactory(ParagonManager dm, ConfigFile file)
        {
            m_Main = dm;
            m_Conf = file;
        }
        ...
        private List<Paragon> m_List = new List<Paragon>();
        
        private List<Paragon> CreateDetector()
        {
                ...
                for (int i = 0; i < m_nUnitNum; i++)
                {
                    ...
                    if (szUnitValues != null && szUnitValues.Length > 0)
                    {
                        ...
                        if (values.Length >= 6)
                        {
                            ...
                            int nDataLength = Convert.ToInt32(values[12]);
                            int nFunction = Convert.ToInt32(values[13]);
                            string szUnits = values[14];

                            LevelMeter[] aUnits = ReadLevelMeter(nUnit, nStartUnit, szUnits);

                            Paragon detector = new Paragon(nID, nStartUnit, nEndUnit, aUnits);                            
        ...
        public List<Paragon> CreateDetectorList()
        {
