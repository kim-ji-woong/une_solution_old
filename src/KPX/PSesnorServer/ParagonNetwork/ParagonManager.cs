...
namespace ParagonNetwork
{
    ...
    public class ParagonManager : IDisposable
    {
        ...
        /// </summary>
        private List<Paragon> m_DetectorList = new List<Paragon>();
        internal List<Paragon> DetectorList
        {
        ...
        private string szConfFileName = "ParagonConfig.ini";
        ...
        public ParagonManager(string iniFilePath = "")
        {
        ...
        private void ReadConfig()
        {
            if( m_confPath == "")
            {
                ...
                if(File.Exists(m_confPath))
                {
                    ...
                    ParagonFactory factory = new ParagonFactory(this, conf);
        ...
        public bool Start()
        {            
            List<Paragon> detectors = new List<Paragon>(m_DetectorList);
            foreach (Paragon detector in detectors)
            {
            ...
            if (m_CheckThread == null || m_CheckThread.IsAlive == false)
            {
                ...
                m_CheckThread.Name = "ParagonPipeChecker";
                m_CheckThread.Start();
            }                  
        ...
        {
            Paragon detector = FindDetector(nComm);
            if (detector != null)
            {
        ...
        private Paragon FindDetector(int nID)
        {
            foreach (Paragon detector in m_DetectorList)
            {
                if (detector.ID == nID)
                {
                    return detector;
        ...
        {
            Paragon dt = FindDetector(nDetectID);
            if( dt != null)
            {
                return dt.IsOnline();
            }
