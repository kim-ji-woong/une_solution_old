...
namespace ParagonNetwork
{
    internal class Paragon
    {
        ...
        public Paragon(int nID, int nStartAddress, int EndAddress, LevelMeter[] aUnits)
        {
            m_nID = nID;
        ...
        public void CheckValue(SerialManager sm, int nTimeOut)
        {
            if (sm.IsConnected == true)
            {
                ...
                if (sm.ReciverData == true)
                {
                    ...
                    {
                        SetPressureValue(nHeader, smReadBuffer);                        
                    }
                    sm.ClearBuffer();
                    m_bOffline = false;
                    m_nFaultCount = 0;
        ...
        public void CheckValue(NetworkManager sm, int nTimeOut)
        {
            if (sm.IsConnected == true)
            {
                ...
                byte[] data = MsgHelper.MakeLongCmdReadRegisterData((byte)start[1], (byte)start[0], length[1], length[0], (byte)m_nSlaveID, (byte)m_nFunction, out nHeader);

                //byte[] data = MsgHelper.MakeData((byte)0, 0, (byte)m_nStartUnit, (byte)m_nLastUnit, (byte)m_nFunction, (byte)m_nSlaveID, out nHeader);

                System.Diagnostics.Trace.WriteLine(m_szName);
                //Logger.Instance.Write("checkvalue : " + m_szName);
                sm.SendBytes(data); // value check
        ...
        private void SetPressureValue(byte nHeader, byte[] bStatus)
        {
            try
            {
                ...
                //02 00 00 00 00 0B 01 04 08 00 00 00 00 00 00 00 00
                ...
                for (int i = 0; i < nLength; i += 4)
                {
                    ...
                    if (aUnit != null)
                    {
                        ...
                        Logger.Instance.Write(aUnit.Name + " value : " + szTemp3 + " org value : " + BitConverter.ToSingle(temp, 0));
                    }
                }
            }
            catch (Exception ex)
            {
                Logger.Instance.Write("ex : " + ex.Message);
            }
        }
