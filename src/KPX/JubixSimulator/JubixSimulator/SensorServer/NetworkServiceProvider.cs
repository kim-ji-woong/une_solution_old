...
{
    public class NetworkServiceProvider : TcpServiceProvider
    {
        ...
        public void RecvLog(byte[] bytes, ConnectionState state)
        {
            ...
            if (bytes[0] != SDMS.TCP_ID.I_AM_HERE || !m_exceptPingLog)
            {
                ...
                if (data != null)
                {
                    if (data.Type == ClientData.ClientType.KPXSensorServer)
                        strClient = "KPX";
        ...
        public override void OnAcceptConnection(ConnectionState state)
        {
            ...
            //lock (m_arrClients)
            {
                ...
                ClientData data = new ClientDataSensorServer(this, state);
                state.Tag = data;
        ...
        public void SetSimValue(int nCh, float value)
        {
            ICollection<ConnectionState> arClient = null;
            DdMonitor.Enter(m_arrClients, true);
            {
            ...
            foreach (ConnectionState state in arClient)
            {
                ...
                ClientDataSensorServer c = (ClientDataSensorServer)client;
                c.SetSimValue(nCh, value);
            }
        }

        public void SendWorkStart(int nch)
        {
            ICollection<ConnectionState> arClient = null;
            DdMonitor.Enter(m_arrClients, true);
            {
    ...
    public class ConnectionLogClient : ConnectionLog
    {
        ...
        public static bool MakeInstance()
        {
            ...
            string szAssemName = System.Reflection.Assembly.GetEntryAssembly().GetName().Name;
            SetLevel("Log4net.Client", "DEBUG");
            AddAppender("Log4net.Client", CreateFileAppender("Client", szAssemName + ".log"));                       
