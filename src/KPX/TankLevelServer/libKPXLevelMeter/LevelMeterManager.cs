...
namespace GasDetector
{
    ...
    public class LevelMeterManager : IDisposable
    {
        ...
        /// </summary>
        private List<KPXLevelMeter> m_DetectorList = new List<KPXLevelMeter>();
        internal List<KPXLevelMeter> DetectorList
        {
        ...
        private string szConfFileName = "KPXLevelMeterConfig.ini";
        ...
        private void ReadConfig()
        {
            if( m_confPath == "")
            {
                ...
                if(File.Exists(m_confPath))
                {
                    ...
                    KPXLevelMeterFactory factory = new KPXLevelMeterFactory(this, conf);
        ...
        public bool Start()
        {            
            List<KPXLevelMeter> detectors = new List<KPXLevelMeter>(m_DetectorList);
            foreach (KPXLevelMeter detector in detectors)
            {
        ...
        {
            KPXLevelMeter detector = FindDetector(nComm);
            if (detector != null)
            {
        ...
        private KPXLevelMeter FindDetector(int nID)
        {
            foreach(KPXLevelMeter detector in m_DetectorList)
            {
                if(detector.ID == nID)
                {
                    return detector;
        ...
        {
            KPXLevelMeter dt = FindDetector(nDetectID);
            if( dt != null)
            {
                return dt.IsOnline();
            }
