...
namespace TankModbusLib
{
    internal class KPXLevelMeter
    {
        ...
        public void SendValue(NetworkManager sm, int nTimeOut, int address, int value)
        {
            if (sm.IsConnected == true)
            {
                sm.ClearBuffer();
                ...
                //byte[] data = MsgHelper.MakeLongCmdReadRegisterData((byte)start[1], (byte)start[0], length[1], length[0], (byte)1, out nHeader);
                byte[] data = MsgHelper.MakeCmdData2((byte)start[1], (byte)start[0], length[1], length[0], (byte)3, (byte)address);
