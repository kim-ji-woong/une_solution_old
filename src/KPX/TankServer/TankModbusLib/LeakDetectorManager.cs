...
namespace TankModbusLib
{
    ...
    public class LeakDetectorManager : IDisposable
    {
        ...
        /// </summary>
        private List<KPXLeakDetector> m_DetectorList = new List<KPXLeakDetector>();
        internal List<KPXLeakDetector> DetectorList
        {
        ...
        private string szConfFileName = "KPXModbusConfig.ini";
        ...
        public LeakDetectorManager(string iniFilePath = "")
        {
        ...
        private void ReadConfig()
        {
            if (m_confPath == "")
            {
                ...
                if (File.Exists(m_confPath))
                {
                    ...
                    KPXLeakDetectorFactory factory = new KPXLeakDetectorFactory(this, conf);
        ...
        public bool Start()
        { 
            List<KPXLeakDetector> detectors = new List<KPXLeakDetector>(m_DetectorList);
            foreach (KPXLeakDetector detector in detectors)
            {
        ...
        {
            KPXLeakDetector detector = FindDetector(nComm);
            if (detector != null)
            {
        ...
        public float GetLevel(int nComm, int nAlarmUnit)
        {
            if (nAlarmUnit < 0)
                return -999;
        ...
        public void SetControlRegister(int nComm, int nFunction, int nChannel, int nValue)
        {
            ...
            //    datas[4] = 0x00;
            //    datas[5] = 0x01;
            //    detector.NetworkMan.SendBytes(datas);
            ...
            foreach (KPXLeakDetector detector in detectors)
            {
                if (detector.ID != nComm) continue;

                NetworkManager sm = detector.NetworkMan;
                if (sm.IsConnected == true)
                {
                    //detector.SendValue(sm, 1000, nComm, nChannel, nValue);
                }
            }
        }
        ...
        private KPXLeakDetector FindDetector(int nID)
        {
            foreach(KPXLeakDetector detector in m_DetectorList)
            {
                if(detector.ID == nID)
                {
                    return detector;
        ...
        {
            KPXLeakDetector dt = FindDetector(nDetectID);
            if( dt != null)
            {
                return dt.IsOnline();
            }
