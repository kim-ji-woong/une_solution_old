...
namespace BuildingSMS
{
    public partial class FormMessage : Form
    {
        ...
        private void SetTeamTags(Building building)
        {
            if (building.BuildingCode == Building.HotelCode)
            {
                m_strTeamTags.Add("테이크호텔_f#담당");
            }
            else if (building.BuildingCode == Building.Tower1Code)
            {
                m_strTeamTags.Add("U_Tower_f#담당");
            }
            else if (building.BuildingCode == Building.Tower2Code)
            {
                m_strTeamTags.Add("T_Tower_f#담당");
            }
            else if (building.BuildingCode == Building.RetailCode)
            {
                m_strTeamTags.Add("AK백화점_f#담당");
                m_strTeamTags.Add("어반브릭스_f#담당");
            }
        }
        ...
        {
            if (m_building.BuildingCode == Building.HotelCode)
                m_sendManager = new HotelManager(m_building, m_nFloorIndex);
            else if (m_building.BuildingCode == Building.RetailCode)
                m_sendManager = new RetailManager(m_building, m_nFloorIndex);
            else if (m_building.BuildingCode == Building.Tower1Code)
                m_sendManager = new Tower01Manager(m_building, m_nFloorIndex);
            else if (m_building.BuildingCode == Building.Tower2Code)
            ...
            textBoxMessage.Text = m_sendManager.Parsing(m_strMessage, m_nReceiverIndex - 1);
        }
        ...
        private bool GetNext(List<int> floors, out bool isLast)
        {
            ...
            {
                result = m_sendManager.GetNext(floors, m_nReceiverGroupCount, out isLast);
                m_finishCalc = isLast;
            ...
            {
                List<int> prevFloors = GetPrevFloors();
                floors.AddRange(prevFloors);

                /*if (m_dicFloorList.TryGetValue(m_nReceiverIndex - 1, out prevFloors))
                {
                    result = true;
        ...
        private List<int> GetPrevFloors()
        {
            List<int> floors = new List<int>();

            foreach (KeyValuePair<int, List<int>> pair in m_dicFloorList)
            {
                floors.AddRange(pair.Value);
            }

            return floors;
        }

        private void RemovePrev(List<int> floors, List<int> prevFloors)
        {
            foreach (int nIndex in prevFloors)
            {
                floors.Remove(nIndex);
            }
        }
