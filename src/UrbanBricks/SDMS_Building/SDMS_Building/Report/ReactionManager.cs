...
namespace SDMS_Building.Report
{
    public class ReactionManager
    {
        ...
        //             false이면 데이터 변경없음.
        public void ZoneSubmit(ArrayList arrZoneList, DateTime startDate, DateTime endDate, IFacility.FacilityType facilityType, int pageType = 1)//pageType이 1이면 탐지/처리 2이면 대응이력
        {
            LoadSOPGenUser();
            ...
            ArrayList arrManualReactionHistory = new ArrayList();
            if (facilityType != IFacility.FacilityType.Earthquake) // 지진은 수동신고가 없음
                arrManualReactionHistory = GetManualReactionHistory(arrZoneList, strBeforeDate, strNowDate, facilityType);

            //선택한 ZoneID 리스트로 EquipmentZoneID를 찾는다.
            List<EquipmentZone> arrEquipmentZoneList = new List<EquipmentZone>();
            if (facilityType != IFacility.FacilityType.Earthquake) // 지진은 Zone이 없음
                arrEquipmentZoneList = FindEquipZone(arrZoneList);

            //가져온 EquipmentZoneID 리스트로 SensorID를 찾아온다.
            Dictionary<int, ISensor> dicSensorZones = FindSensorZone(arrEquipmentZoneList, facilityType);
            ...
            //오작동이력 로그 저장
            m_arrMulFunctionList = GetMulFunctionLog(arrZoneList, strBeforeDate, strNowDate, facilityType);
            ...
            foreach (KeyValuePair<int,ArrayList> pair in m_dicHistoryLog)
            {
                ...
                if (reactionLog.SensorType == 0)
                {
                    ...
                    if (reactionLog.SensorType == 0)
                    {
                        ...
                        {
                            if (Typelog.ReactionType == (int)ReactionType.NOTIFY_SIGNAL ||
                                Typelog.ReactionType == (int)ReactionType.MALFUNCTION ||
                                Typelog.ReactionType == (int)ReactionType.IGNORE_SIGNAL)
                            {
        ...
        {
            //if (ReactionIntrusionManager.IsS1Access(log.SensorType))
            ////if (log.ReactionType == (int)ReactionType.BEGIN_S1ACCESS_STATUS)
            //{
        ...
        {
            //if (ReactionIntrusionManager.IsS1SVMS(log.SensorType))
            ////if (log.ReactionType == (int)ReactionType.BEGIN_S1SVMS_STATUS)
            //{
            //    if (log.Message.Contains("화재"))
            //        return true;
            //}
        ...
        private ArrayList GetDetectLog(ArrayList arrAllLog)
        {
            ...
            {
                int reportSensorZoneID = -1;

                if (reactionLog.ReactionType == (int)ReactionType.BEGIN_STATUS || reactionLog.ReactionType == (int)ReactionType.CHANGE_ALARM_DEPTH ||
                    (reactionLog.ReactionType == (int)ReactionType.NOTIFY_SIGNAL && (int.TryParse(reactionLog.Param2, out reportSensorZoneID) && Convert.ToInt32(reactionLog.Param2) >= SOPWebServer.Header.ManualReportDefaultID))
                 || IsS1AccessFire(reactionLog) || IsSVMSFire(reactionLog))
                {
                    ...
                    reportSensorZoneID = -1;
                    // 수동 신고
                    if (int.TryParse(reactionLog.Param2, out reportSensorZoneID) && Convert.ToInt32(reactionLog.Param2) >= SOPWebServer.Header.ManualReportDefaultID)
                    {
                        if (reactionLog.Param2 != (SOPWebServer.Header.ManualReportDefaultID + (int)IFacility.FacilityType.STRONG_WIND).ToString()) // 강풍은 Zone이 없음
                        {
                    ...
                    string szBuildingName = (zone != null && zone.Building != null) ? zone.Building.BuildingName : "";
                    ...
                    if (szBuildingName == "" && zone == null)
                        detect.BuildingName = "모든 건물";
                    else if (szBuildingName == "" && zone != null)
                    ...
                    else if (reactionLog.ReactionType == (int)ReactionType.CHANGE_ALARM_DEPTH)
                    {
                        if (reactionLog.Message.Contains("상향"))
                            detect.DetectType = "알람 단계 상향";
                        else if (reactionLog.Message.Contains("하향"))
                            detect.DetectType = "알람 단계 하향";
                        else
                            detect.DetectType = "알람 단계 상하향";
                    }
                    else//수동신고
        ...
        private ArrayList GetMulFunctionLog(ArrayList arrZoneList, string strStartDate, string strEndDate, IFacility.FacilityType facilityType)
        {
            ...
            foreach (Zone zone in arrZoneList)
            {
                ArrayList arrHistoryList = FindHistoryID(zone.ID, facilityType);
                    ...
                    foreach (SensorReactionLog log in arrLog)
                    {
                        ...
                        else if (log.ReactionType == (int)ReactionType.NOTIFY_SIGNAL)
                        {
                            ...
                            break;
                                 ...
                                 (log.ReactionType == (int)ReactionType.USER_RESET && facilityType == IFacility.FacilityType.PSM_SENSOR))
                        {
                            nMulFunctionCount++;
                            nType = log.ReactionType;
                            break;
                ...
                mulfuction.DetectType = IFacility.GetFacilityTypeString(facilityType); // GetReactionString(1);
                if (facilityType == IFacility.FacilityType.PSM_SENSOR && m_nSiteID == 201)
                    mulfuction.DetectType = Data.CommonString.POI_Gas_Kor;
        ...
        private ArrayList GetManualReactionHistory(ArrayList arrZoneList, string startDate, string endDate, IFacility.FacilityType facilityType)
        {
            //수동신고 목록을 저장 할 배열
            ...
            string strSensorZoneID = (SOPWebServer.Header.ManualReportDefaultID + (int)facilityType).ToString();

            WebDBManager webDB = FormMain.Instance.DBManager;

            StringBuilder sb = new StringBuilder();
            sb.Append("Select ID, SensorHistoryID, ReactionType, Time, Message, Param1, Param2, Param3, Param4, Param5, DetectionStatus ");
            sb.Append("  From SensorReactionHistory ");
            sb.Append(" Where SensorHistoryID in (Select SensorHistoryID ");
            sb.Append("                             From SensorReactionHistory ");
            sb.AppendFormat("                      Where param2 = '{0}' ", strSensorZoneID);
            if (facilityType != IFacility.FacilityType.STRONG_WIND) // 강풍은 Zone이 없음
                sb.AppendFormat("                    And param1 in ({0}) ", strZoneList);            
            sb.AppendFormat("                        And ReactionType = {0} ", (int)ReactionType.NOTIFY_SIGNAL);
            sb.AppendFormat("                        And Time Between '{0}' and '{1}')", startDate, endDate);
            ...
            for (int i = 0; i < nResultCount - 10; i += 11)
            {
                ...
                reactionLog.SensorType = (int)facilityType;
                ...
                if (ContainsLog(arrLogs, reactionLog.ID) == false)
                {
        ...
        private ArrayList GetReactionHistory(ArrayList arrSensorHistoryID)
        {
            ...
            for (int i = 0; i < nResultCount - 10; i += 11)
            {
                ...
                if (nReactionType == (int)libSensorProcess.ReactionType.MALFUNCTION || nReactionType == (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL || nReactionType == (int)libSensorProcess.ReactionType.IGNORE_SIGNAL)
                {
                    if (!m_dicHistoryMember.ContainsKey(nSensorHistoryID))
                        m_dicHistoryMember.Add(nSensorHistoryID, Param3);
                }
        ...
        private bool ContainsLog(ArrayList logs, int nSensorReactionID)
        {
            foreach (SensorReactionLog log in logs)
            {
                if (log.ID == nSensorReactionID)
                    return true;
            }
        ...
        private ArrayList GetSensorZoneHistoryID(Dictionary<int, ISensor> dicSensorZones, string startDate, string endDate)
        {
            ...
            string strSQL = "select id,SensorID from SensorZoneHistory where Time Between '" + startDate + "' and '" + endDate + "'";
            ArrayList arrResult = webDB.GetResultData(strSQL);
            if (arrResult == null)
                return null;
        ...
        private ArrayList FindHistoryID(int nZoneID, IFacility.FacilityType facilityType)
        {
        ...
        private List<EquipmentZone> FindEquipZone(ArrayList arrZoneList)
        {
            ...
            arrEquipZoneList= arrEquipZoneList.Distinct().ToList();
        ...
        // Key : SensorZone ID
        private Dictionary<int, ISensor> FindSensorZone(List<EquipmentZone> arrEquipZoneList, IFacility.FacilityType facilityType)
        {
            Dictionary<int, ISensor> dicSensorZones = new Dictionary<int, ISensor>();
            if (arrEquipZoneList == null)
                return null;

            if (facilityType == IFacility.FacilityType.Earthquake || facilityType == IFacility.FacilityType.STRONG_WIND)
            {
                List<ISensor> arSensors = SensorZoneManager.Instance.FindZoneInSensor(-1, facilityType);
                if (arSensors == null)
                    return null;
            ...
            else
            {
                ...
                {
                    List<ISensor> arSensors = arSensors = SensorManager.Instance.FindZoneInSensor(equip.ID, facilityType);
    ...
    class ReactionLog : IComparable
    {
        ...
        public override string ToString()
        {
            ...
            if (nReactionType == (int)ReactionType.NOTIFY_SIGNAL)
            {
                ...
                strReactionType = facilityType.ToString() + " 발생";
            }
            ...
            else if (nReactionType == (int)ReactionType.IGNORE_S1ACCESS_STATUS)
            ...
            {
                int nfacilityType;
                if (int.TryParse(facilityType, out nfacilityType) && (Convert.ToInt32(facilityType) == (int)IFacility.FacilityType.Fire_S1 || Convert.ToInt32(facilityType) == (int)IFacility.FacilityType.FireF1_S1))
                {
                    //화재뿐이라 화재라는 단어가 필요없음
                    ...
                    + "   [ " + SOPServer.EventTypeString.GetEventTypeDetectString(Convert.ToInt32(facilityType)).Replace("화재","") + " 수동 신고 ] " + strReactionType;
                }
                else
                { 
                    return String.Format("{0:0000}-{1:00}-{2:00} {3} {4:00}:{5:00}", time.Year, time.Month, time.Day, time.Hour < 12 ? "오전" : "오후", time.Hour > 12 ? time.Hour - 12 : time.Hour, time.Minute)
            ...
            else
            {
                ...
                if (int.TryParse(facilityType, out nfacilityType) && (Convert.ToInt32(facilityType) == (int)IFacility.FacilityType.Fire_S1 || Convert.ToInt32(facilityType) == (int)IFacility.FacilityType.FireF1_S1))
                {
                    //화재뿐이라 화재라는 단어가 필요없음
                    return String.Format("{0:0000}-{1:00}-{2:00} {3} {4:00}:{5:00}", time.Year, time.Month, time.Day, time.Hour < 12 ? "오전" : "오후", time.Hour > 12 ? time.Hour - 12 : time.Hour, time.Minute)
                        + "   [ " + SOPServer.EventTypeString.GetEventTypeDetectString(Convert.ToInt32(facilityType)).Replace("화재", "") + " ] " + strReactionType;
                }
                else
                {
                    return String.Format("{0:0000}-{1:00}-{2:00} {3} {4:00}:{5:00}", time.Year, time.Month, time.Day, time.Hour < 12 ? "오전" : "오후", time.Hour > 12 ? time.Hour - 12 : time.Hour, time.Minute)
