...
namespace SDMS_Building.Data
{
    public class PSMManager
    {
        ...
        // OrgSensorID, Connected, DeActivate
        private ArrayList m_psmSensorStatus = null;        
        // Key : Tank ID
        ...
        private Dictionary<int, PSMSensor> ReadPSMSensors()
        {
            ...
            sb.Append("Select OrgSensorID, Connected, DeActivate ");
            sb.Append("  From SensorTagInfo as sti Inner join SensorZone as sz ON sti.SensorZoneID = sz.ID ");
            sb.Append(" Where Type = 11 ");
            sb.AppendFormat(" And OrgSensorID in ({0}) ", string.Join(",", nIDs));
            
            ArrayList arrResult2 = dbMgr.GetResultData(sb.ToString());
            if (arrResult2 != null && arrResult2.Count > 0)
            {
                m_psmSensorStatus = new ArrayList();

                for (int i = 0; i < arrResult2.Count; i += 3)
                {
                    int nID = WebDBManager.GetIntField(arrResult2[i].ToString(), -1);
                    int nConnected = WebDBManager.GetIntField(arrResult2[i + 1].ToString(), -1);
                    string strDeActivate = WebDBManager.GetStringField(arrResult2[i + 2]);

                    m_psmSensorStatus.Add(nID);
                    m_psmSensorStatus.Add((nConnected == 1) ? true : false);
                    m_psmSensorStatus.Add((strDeActivate == "Y") ? true : false);
                }
            }
        ...
        public void LoadPSMSensorPOI(WebDBManager dbMgr, int nZoneID, bool isIndoor, UnE.Sensor.ISensorTooltipOwner view, Dictionary<int, POI> dicPOIs)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("Select s.ID, SensorName, X, Y, Z, sz.ID as sensorZoneID ");
            sb.Append("  From PSMSensor as s INNER JOIN SensorZone as sz ON s.ID = sz.OrgSensorID ");
            sb.AppendFormat(" Where Zone = {0} ", nZoneID);
            sb.AppendFormat("   And Type = {0} ", (int)IFacility.FacilityType.PSM_SENSOR);
            ...
            if (nResultCount > 0)
            {
                ...
                string strPOIType = Data.CommonString.POI_Gas;
                ...
                for (int i = 0; i < nResultCount - 5; i += 6)
                {
                    ...
                    PSMSensorForPOI sensor = new PSMSensorForPOI();
                    ...
                    dicPOIs[sensorZoneId.Data] = sensor.POI;
                ...
                string strFilePath = System.IO.Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) + "\\AddGasPOI.txt";
                view.AddPOIFile(strPOIType, strFilePath, pois);

                strFilePath = System.IO.Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) + "\\ShowGasPOI.txt";
                view.ShowIconPOIFile(strPOIType, strFilePath, poiIDs, poiTypes, poiVisibles);
            }
        }

        public bool GetSensorDeActivate(int nID, ref bool connected, ref bool deActivate)
        {
            if (m_psmSensorStatus == null || m_psmSensorStatus.Count == 0)
                return false;

            for (int i = 0; i < m_psmSensorStatus.Count; i += 3)
            {
                int nID2 = (int)m_psmSensorStatus[i];
                if (nID == nID2)
                {
                    connected = (bool)m_psmSensorStatus[i + 1];
                    deActivate = (bool)m_psmSensorStatus[i + 2];
                    return true;
                }
            }
        ...
        private Dictionary<int, SensorSchedule> LoadSensorStatusData()
        {
            ...
            List<int> localOffSensorIDList = ReadLocalOffSensorIDList();
