...
namespace SOPManager
{
	public class PropertiesLevel
	{
        ...
        private string m_szWeekType = "주간";

        [Category("\t일반")]
        [Browsable(true)]
        [DisplayName("주간/야간")]
        [Description("주간/야간을 선택합니다.")]
        [TypeConverter(typeof(WeekTypeConverter))]
        [ReadOnly(false)]
        public string WeekType
        {
            get
			{
				...
				{					
					...
					if (bWeek)
						m_szWeekType = WeekTypes.TypeList[0];
					else
                        m_szWeekType = WeekTypes.TypeList[1];
			...
			set
			{
				if (m_szWeekType != value)
				{
                    bool bWeek = SopDocManager.Instance.WeekMode;
					SopDocManager.Instance.WeekMode = !SopDocManager.Instance.WeekMode;
