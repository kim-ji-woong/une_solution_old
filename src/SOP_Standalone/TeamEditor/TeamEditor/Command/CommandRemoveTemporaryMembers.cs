...
namespace TeamEditor.Command
{
    public class CommandRemoveTemporaryMembers : CommandEx
    {
        ...
        private List<string> RemoveTemporaryMemberList(DBUtility.WebDBManager dbMgr, string strTemporaryMemberIDs)
        {
            ...
            for (int i = 0; i < nResultCount - 8; i += 9)
            {
                ...
                string strDisplayMember = DBUtility.WebDBManager.GetStringField(arrResult[i + 1], "");
                ...
                int nTeamLeader = DBUtility.WebDBManager.GetIntField(arrResult[i + 5].ToString(), 0) == 0 ? 0 : 1;
        ...
        public List<string> RemoveDB(DBUtility.WebDBManager dbMgr, string strTemporaryMemberIDs)
        {
            m_rollbackSQLs.Clear();
