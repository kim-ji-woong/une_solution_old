...
namespace SDMSServer
{
    public class ClientDataSOPSimulator : ClientData, ControlMonitoring.IControlClientOwner
    {
        ...
        {
            m_provider = provider;
            ClientType = TCP_CLIENT.SOP_SIMULATOR;
        ...
        protected override bool ProcessFirstConnection(ConnectionState state)
        {
            ...
            m_provider.SendCurrentSelectMission();
            return true;
        }
        ...
        private void SendCurrentFireSensorSignal(ConnectionState state)
        {
            ...
            for (int i = 0; i < nHistoryCount;i++ )
            {
                ...
                m_provider.SendStatusSensorSignal(history.LastReactionLog, state);
                break;
            }
        }
        ...
        private void ProcessSOPSimulatorCommand(ArrayList arrDatas, byte[] bytes)
        {
            ...
            {
                m_provider.SendDataToOther(bytes, this, false, TCP_CLIENT.SOP_SIMULATOR);
            }
        }

        private void ProcessUpdateToWorkingMemberData(byte[] bytes)
        {
            try
            {
                m_provider.SendDataToOther(bytes, this, false, TCP_CLIENT.SOP_SIMULATOR);
        ...
        {
            this.m_provider.RememberSelectMission(bytes);
        }
        ...
        private void ProcessRequestControl()
        {
            ...
            else if (conMgr.ControlClient != this)
            {
                ...
                {
                    ClientDataSOPSimulator client = (ClientDataSOPSimulator)conMgr.ControlClient;
                    SendRequestControl(client.m_state, this.GetControlClient(), TCP_ID.REQUEST_CONTROL);
                }
            }
        }

        // 제어권 요청취소
        private void ProcessCancelRequestControl()
        {
            ControlManager conMgr = ControlManager.Instance;
            ...
            {
                ClientDataSOPSimulator client = (ClientDataSOPSimulator)conMgr.ControlClient;
                SendRequestControl(client.m_state, this.GetControlClient(), TCP_ID.CANCEL_REQUEST_CONTROL);
            }
        }

        private void SendRequestControl(ConnectionState state, ControlClient data, byte header)
        {
            ...
            byte[] userIDBytes = TcpHelper.MakeBytes(strUserID);
            byte[] userNameBytes = TcpHelper.MakeBytes(data.UserName);
            byte[] userNickNameBytes = TcpHelper.MakeBytes(strUserNickName);

            string strIP = ((System.Net.IPEndPoint)state.RemoteEndPoint).Address.ToString();
            byte[] ipBytes = TcpHelper.MakeBytes(strIP);
            ...
            TcpHelper.CopyBytes(bytes, ref nIndex, chunkCountBytes);
            TcpHelper.CopyBytes(bytes, ref nIndex, userIDBytes);
            TcpHelper.CopyBytes(bytes, ref nIndex, userNameBytes);
            TcpHelper.CopyBytes(bytes, ref nIndex, userNickNameBytes);
            TcpHelper.CopyBytes(bytes, ref nIndex, ipBytes);
        ...
        protected override bool ProcessIAmHere(ArrayList arrDatas)
        {
            ...
            if (hasControl)
            {
                if (ControlManager.Instance.ControlClient != null && ControlManager.Instance.ControlClient != this)
                {
