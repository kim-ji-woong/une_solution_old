        ...
        private void PostProcessSensorData(int nHistoryID, int nPrevSensorHistoryID, int nSensorID, int nData, int nEventType, bool bConnected)
        {
            ...
            if (nData == 1 && nHistoryID != -1)
            {
                if (!m_provider.CheckSituation(nHistoryID))
                {
                    ...
                    // 2013-12-18
                    if (m_bUseBroadcast == true || log.Type == libSensorProcess.ReactionType.BEGIN_STATUS)
                ...
                if (nPrevSensorHistoryID > 0)
                {
                    ...
                    if (history != null && history.LastReactionLog != null/* && history.LastReactionLog.Type == SensorReactionLog.ReactionType.BEGIN_STATUS*/)
                    {
                        ...
                        if (history.LastReactionLog.Type == libSensorProcess.ReactionType.BEGIN_STATUS ||
                            history.LastReactionLog.Type == libSensorProcess.ReactionType.BEGIN_S1ACCESS_STATUS ||
                            history.LastReactionLog.Type == libSensorProcess.ReactionType.BEGIN_S1SVMS_STATUS ||
                            history.LastReactionLog.Type == libSensorProcess.ReactionType.BEGIN_SECOM_STATUS ||
                            history.LastReactionLog.Type == libSensorProcess.ReactionType.NOTIFY_FIRE ||
                            history.LastReactionLog.Type == libSensorProcess.ReactionType.NOTIFY_SECURITY
                            )
                        {
                            bool notifyFire = history.LastReactionLog.Type == libSensorProcess.ReactionType.NOTIFY_FIRE;
                            bool notifySecurity = history.LastReactionLog.Type == libSensorProcess.ReactionType.NOTIFY_SECURITY;
