...
namespace SDMSServer
{
    public class ClientDataSDMS : ClientData
    {
        ...
        public static void RunBroadcast(SensorReactionLog log, ServiceProvider provider, BroadcastManager.SituationType type)
        {
            ...
            if (BroadcastManager.Instance.IsEnabled(type) == true)
            {
                ...
                if (bResult)
                {
                    ...
                    smsLog.Type = libSensorProcess.ReactionType.RUN_BROADCAST;
        ...
        public static void ClearPSMSensorAlarm(PSMSensor sensor, int nSOPGenUserID)
        {
            ...
            SensorZone sensorZone = NetworkServer.Instance.IOManager.GetSensorZone(sensor.ID, IFacility.FacilityType.PSM_SENSOR, equipZone);
        ...
        private void ProcessMalfunction(byte[] bytes, ArrayList arArgs)
        {
            ...
            if (int.TryParse(log.Param2, out nSensorZoneID))
            {
                ...
                if (sensor != null && sensorZoneGroup != null)
                {
                    if (sensorZoneGroup.SensorDatas.ContainsKey(sensor) && sensorZoneGroup.SensorDatas.Count > 1)        //같은 SensorZoneGroup이 있는지 체크
                    {
                        foreach (KeyValuePair<int, int> pair in SensorManager.Instance.DicSensorHistory)
                        {
                            int sensorZoneId = pair.Value;
                            foreach (KeyValuePair<SensorZone, Object> keyValue in sensorZoneGroup.SensorDatas)
                            {
                                int storedSensorID = keyValue.Key.ID;
                                if (storedSensorID == sensorZoneId)         //history에 남아있는 
                                {
                                    SensorReactionLog sameEquipzoneLog = ReadMalfunctionReport(pair.Key, sensorZoneId, log.Param3, log.DescriptionText);
                                    ProcessMalfunction(sameEquipzoneLog);
                                }
                            }                        
                        }
        ...
        public SensorReactionLog ReadUserReset(byte[] bytes)
        {
            ...
            log.Type = libSensorProcess.ReactionType.PSM_USER_RESET;
            log.SensorHistoryID = nSensorHistoryID;
        ...
        public SensorReactionLog ReadMalfunctionReport(int sensorHistoryID, int sensorZoneID, string sopGenUser, string strDescriptionText)
        {
            SensorReactionLog log = new SensorReactionLog();

            log.SensorHistoryID = sensorHistoryID;

            log.Param2 = sensorZoneID.ToString();

            IFacility.FacilityType sensorType = IFacility.FacilityType.FIRE_SENSOR;
            string szTypeMsg = "화재";
            try
            {
                SensorZone sz = NetworkServer.Instance.IOManager.GetSensorZone(sensorZoneID);
                if (sz != null)
                {
                    sensorType = sz.Type;
            ...
            log.Param3 = sopGenUser;

            if (strDescriptionText != null && strDescriptionText.Length > 0)
                log.DescriptionText = strDescriptionText;

            int nEquipZoneID = SensorManager.Instance.GetEquipmentZoneID(sensorZoneID);
        ...
        private void ProcessManualFireDetect(SensorReactionLog log)
        {
            ...
            string sqlInsert = "insert into SensorZoneHistory(ID, SensorID,Connected,Data,Time, param1, param2, SiteID ) Values('"
                + nHistoryID + "','" + 0 + "','" + 1 + "','" + 1 + "','" + strDateTimeField + "','" + log.Param1 + "','" + strParam2 + "','" + m_nSiteID + "')";
        ...
        private SensorReactionLog ReadSpillReport(byte[] bytes, out int nSOPGenUserID)
        {
            ...
            log.Type = libSensorProcess.ReactionType.NOTIFY_PSM;
            return log;
        }
        ...
        private SensorReactionLog ReadFireReport(byte[] bytes, out int nSOPGenUserID)
        {
            ...
            log.Type = libSensorProcess.ReactionType.NOTIFY_FIRE;
            return log;
        }
