...
namespace UnE.Control.CCTVControl.IDIS_NVR
{
    public partial class IdisNvrSet
    {
        ...
        public void feature_control_PTZ_set_enable(bool enable)
        {
            ...
            {
                m_owner.BeginInvoke((MethodInvoker)delegate() { feature_control_PTZ_set_enable(enable); });
                return;
            }
        ...
        public void feature_control_PTZ_screen_changed_pane(int pane)
        {
            if (m_owner.InvokeRequired)
            {
                m_owner.BeginInvoke((MethodInvoker)delegate() { feature_control_PTZ_screen_changed_pane(pane); });
                return;
            }
