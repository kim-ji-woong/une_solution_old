...
namespace SecomEventReceiver
{
    public class ClientDataS1SensorTester : ClientData
    {
        ...
        private int GetSensorTagInfoID(DBUtility.WebDBManager dbMgr, int nSensorServerID, int nTagNo, ref int nSensorType)
        {
            ...
            DBUtility.VariousData<int> sensorType = DBUtility.WebDBManager.GetIntField(arrResult[1].ToString());
            ...
            /*if (sensorType != null)
            {
                if (sensorType.Data == (int)IFacility.FacilityType.PSM_SENSOR)
                    nSensorType = sensorType.Data;
                else if (sensorType.Data == (int)IFacility.FacilityType.FIRE_SENSOR || sensorType.Data == (int)IFacility.FacilityType.SecomFire ||
                ...
                else if (sensorType.Data == (int)IFacility.FacilityType.SecomExternalAlarmBell || sensorType.Data == (int)IFacility.FacilityType.SecomWomenAlarmBell)
                    nSensorType = (int)IFacility.FacilityType.Security_Sensor;
            }*/
        ...
        private void ProcessSensorData(byte[] bytes, ArrayList arDatas, int bActivate, bool bPSM = false)
        {
            ...
            if (reciver != null)
            {
                ...
                {
                    curcuit = reciver.Circuits[nCircuit];
                }
        ...
        private void SendSensorData(Circuit circuit, int nData, bool bPSM = false)
        {
            ...
            int nSensorZoneID = circuit.SensorZoneID;
            
            int nTagNum = circuit.TagNum;
            int nSensorType = (int)circuit.SensorType;
            //if(nSensorType == 6 && nSensorType == 9)
            {
                NetworkManager.Instance.SendSensorData(nSensorZoneID, circuit.ID, nSensorType, nData, true);
            }
