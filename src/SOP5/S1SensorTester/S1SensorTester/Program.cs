...
namespace SensorTester
{
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            if (args.Count() > 2)
            {
                string strPos = args[2];
                string[] arrTokens = strPos.Split(',');

                if (arrTokens.Count() == 2)
                {
                    string strX = arrTokens[0].Trim();
                    string strY = arrTokens[1].Trim();
            ...
            Application.Run(new FormMain(strServerAddr, strTitle, ptStart));
        }
