...
namespace SOPManager
{
    ...
    public class IOManager : IDisposable
    {
        ...
        public bool Save(FormMain frm, WebDBManager dbMgr, string strVersionName, int nVersionID, int nSOPGenUserID, string strDescription, ref VersionInfo rVersion, out int nDisasterID)
        {
            ...
            nDisasterID = AddDisaster(dbMgr, nVersionID);
            if (nDisasterID < 0)
            {
                // Rollback
				dbMgr.BatchRollback();
            ...
            Dictionary<System.Windows.Forms.TabPage, int> dicActionStepIDs = AddActionSteps(frm, dbMgr, nDisasterID, strUserDefinedConfigName);
            ...
            DBUtility.VariousData<int> _teamID = null, _teamType = null;
            
            if (GetStepMemberTeamData(dbMgr, 1, out _teamID, out _teamType))
            {
        ...
        private void GetComponentMaxID(WebDBManager dbMgr, string strComponentTableName, ref int nComponentID, bool transaction)
        {
            if (nComponentID < 0)
            {
                ...
                ArrayList arrResult = transaction ? dbMgr.GetBatchData(strSQL) : dbMgr.GetResultData(strSQL, 0);//dbMgr.GetResultData(strSQL, transaction ? 1 : 0);
		...
		private bool AddGroupComponent(WebDBManager dbMgr, int nStepMemberID, Sections.SectionGroup group, int nGroupID, Dictionary<Sections.Section, int> dicComponentID)
		{
			...
			if (szSQL == "")
				return false;
			ArrayList arResult = dbMgr.GetBatchData(szSQL);
			if (arResult == null)
				return false;
			return true;
		}

        private bool GetStepMemberTeamData(WebDBManager dbMgr, int nTransaction, out DBUtility.VariousData<int> nTeamID, out DBUtility.VariousData<int> nTeamType)
        {
        ...
        public int AddUserDefinedTeam(WebDBManager dbMgr, string strTeamName, bool transaction)
        {
            ...
            return dbMgr.GetResultData(strSQL, 0) == null ? -1 : nTeamID;
		...
		public bool DeleteUserDefinedTeam(WebDBManager dbMgr, string strTeamName, bool transaction)
		{
            ...
            ArrayList arResult = dbMgr.GetResultData(strSQL3, 0);
            ...
            if (DeleteActionStepUsingTeam(dbMgr, nID, 3) == false)
                return false;

            // Site별로 사용할 수 있도록 수정 , Edit by skkim 2015.01.09
			string strSQL = string.Format("delete from UserDefinedTeam where TeamName = '{0}' and SiteID = {1}", ChangeSpecialCharacter(strTeamName), m_nSiteID);
			...
			arResult = dbMgr.GetResultData(strSQL2, 0);
        ...
        public static bool DeleteActionStepUsingTeam(WebDBManager dbMgr, int nTeamID, int nTeamType)
        {
            string strSQL = string.Format("delete from ActionStepUsingTeam where TeamID = {0} and TeamType = {1}", nTeamID, nTeamType);
            return dbMgr.GetResultData(strSQL, 0) != null;
        }
        ...
        //             저장에 실패하면 null을 리턴
        public Dictionary<System.Windows.Forms.TabPage, int> AddActionSteps(FormMain frm, WebDBManager dbMgr, int nDisasterID, string strUserDefinedConfigName)
        {
			FormPageSOP pageLevel = frm.GetPageLevel();
            ...
            if (strUserDefinedConfigName != null || strUserDefinedConfigName.Length > 0)
            {
                ...
                else
                {
                    ...
                    {
                        int nID = AddUserDefinedConfig(dbMgr, config, variables);
        ...
        private int AddUserDefinedConfig(WebDBManager dbMgr, ConfigData config, List<SOPParameter> variables)
        {
            if (config == null)
                return -1;
        ...
        //             이 값이 0보다 작으면 실패
        private int AddDisaster(WebDBManager dbMgr, int nVersionID)
        {            
        ...
        private int SaveVersion(FormMain frm, WebDBManager dbMgr, string strVersionName, int nVersionID, int nSOPGenUserID, string strDescription, ref VersionInfo rVersion)
        {
            ...
            ArrayList arResult = dbMgr.GetBatchData(strSQL);

            if (arResult == null)// || arResult.Count == 0)
                return -1;


            string szTemp = "select * from Version where ID = " + nVersionID;

            ArrayList arResult2 = dbMgr.GetBatchData(szTemp);

            if (arResult2 == null || arResult2.Count == 0)
                return -1;

            int nRestID = WebDBManager.GetIntField(arResult2[0].ToString(), 0);
		...
		public bool IsMonitoringSOPVersion(WebDBManager dbMgr, int nVersionID, bool bTransaction )
		{
			if (nVersionID < 0)
				return false;
            ...
            ArrayList arrResult = bTransaction ? dbMgr.GetBatchData(strSQL) : dbMgr.GetResultData(strSQL, 0);//dbMgr.GetResultData(strSQL, bTransaction == true ? 1 : 0);
			if (arrResult == null)
				return false;
			if (arrResult.Count == 0)
				return false;
        ...
        // 기존 버전을 삭제
        public bool DeleteSOPVersion(WebDBManager dbMgr, int nVersionID, bool deleteVersion, bool transaction, bool noCommit = false)
        {
            ...
            if (IsMonitoringSOPVersion(dbMgr, nVersionID, false))
			{
				return false;
			}
            ...
            if (deleteVersion)
            {
                if (!DeleteVersion(dbMgr, nVersionID, transaction))
                {
                    if (noCommit == false)
                        dbMgr.BatchRollback();
                    return false;
        ...
        private bool DeleteActionStepHistory(WebDBManager dbMgr, string strActionStepIDs, bool transaction)
        {
            ...
            arrResult = transaction ? dbMgr.GetBatchData(strSQL) : dbMgr.GetResultData(strSQL, 0);//dbMgr.GetResultData(strSQL, transaction ? 1 : 0);
            ...
            {
                strSQL = string.Format("delete from ComponentHistoryDetail where ComponentHistoryID in ({0})", strComponentHistoryIDs);
