    ...
    namespace Report
    {
        public class ReactionIntrusionManager
        {
                ...
                foreach (KeyValuePair<int, ArrayList> pair in m_dicHistoryLog)
                {
                    ...
                    if (nSensorHistoryID == nHistoryID)
                    {
                        ...
                        foreach (SensorReactionIntrusionLog srLog in log)
                        {
                            ...
                            if (srLog.ReactionType == (int)ReactionType.MALFUNCTION || srLog.ReactionType == (int)ReactionType.IGNORE_S1SVMS_STATUS || srLog.ReactionType == (int)ReactionType.IGNORE_S1ACCESS_STATUS || srLog.ReactionType == (int)libSensorProcess.ReactionType.IGNORE_SECOM_STATUS)
                            {
                                int nCommanderID = -1;
            ...
            private ArrayList GetDetectLog(ArrayList arrAllLog)
            {
                ...
                {
                    if (reactionLog.ReactionType == (int)ReactionType.BEGIN_S1SVMS_STATUS  
                     ...
                     || (reactionLog.ReactionType == (int)ReactionType.NOTIFY_SECURITY && reactionLog.Param2 == "0"))
                    {
                        ...
                        if (reactionLog.ReactionType == (int)ReactionType.NOTIFY_SECURITY)
                        {
                            //EquipZone구하기
                    ...
                    foreach (int nHistoryID in arrHistoryList)
                    {
                        ...
                        foreach (SensorReactionIntrusionLog log in arrLog)
                        {
                            ...
                            if ((log.ReactionType == (int)ReactionType.BEGIN_S1SVMS_STATUS ||
                                log.ReactionType == (int)ReactionType.BEGIN_S1ACCESS_STATUS) && log.Param3 != "null") 
                                beginDetectType = Convert.ToInt32(log.Param3);
                            if ((log.ReactionType == (int)ReactionType.BEGIN_SECOM_STATUS) && log.Param3 != "null")
                                beginDetectType = Convert.ToInt32(log.Param3);
                            if (log.ReactionType == (int)ReactionType.BEGIN_S1SVMS_STATUS || log.ReactionType == (int)ReactionType.BEGIN_S1ACCESS_STATUS || log.ReactionType == (int)ReactionType.BEGIN_SECOM_STATUS)
                            {
                            ...
                            {
                                nFireCount++;
                                nType = (int)ReactionType.BEGIN_SECOM_STATUS;
                            ...
                            {
                                nFireCount++;
                                nType = (int)ReactionType.NOTIFY_SECURITY;
                            ...
                            // 처리되지 않음.
                            else if (log.ReactionType == (int)ReactionType.IGNORE_S1SVMS_STATUS)
                            {
                                nType = (int)ReactionType.IGNORE_S1SVMS_STATUS;

                                break;
                            }
                            else if (log.ReactionType == (int)ReactionType.IGNORE_S1ACCESS_STATUS)
                            {
                                nType = (int)ReactionType.IGNORE_S1ACCESS_STATUS;

                                break;
                            }
                            else if (log.ReactionType == (int)ReactionType.IGNORE_SECOM_STATUS)
                            {
                                nType = (int)ReactionType.IGNORE_SECOM_STATUS;
            ...
            private ArrayList GetManualReactionHistory(ArrayList arrZoneList, string startDate, string endDate)
            {
                ...
                string strSQL = "select ID,SensorHistoryID,ReactionType, Time, Message, Param1, Param2, Param3, Param4, Param5, DetectionStatus From SensorReactionHistory where SensorHistoryID in "
                         + "(select SensorHistoryID from SensorReactionHistory where param1 in(" + strZoneList + ") And ReactionType = " + (int)ReactionType.NOTIFY_SECURITY + " AND Param2 = 0 And Time Between '" + startDate + "' and '" + endDate + "')";
            ...
            private ArrayList GetReactionHistory(ArrayList arrSensorHistoryID)
            {
                ...
                for (int i = 0; i < nResultCount - 10; i += 11)
                {
                    ...
                    if (nReactionType == (int)libSensorProcess.ReactionType.MALFUNCTION || nReactionType == (int)libSensorProcess.ReactionType.NOTIFY_SECURITY || nReactionType == (int)libSensorProcess.ReactionType.IGNORE_S1SVMS_STATUS || nReactionType == (int)libSensorProcess.ReactionType.IGNORE_S1ACCESS_STATUS || nReactionType == (int)libSensorProcess.ReactionType.IGNORE_SECOM_STATUS)
                    {
                        if (!m_dicHistoryMember.ContainsKey(nSensorHistoryID))
                            m_dicHistoryMember.Add(nSensorHistoryID, Param3);
                    }
        ...
        class ReactionIntrusionLog : IComparable
        {
            private int nHistoryID = -1;
            public int HistoryID
            {
