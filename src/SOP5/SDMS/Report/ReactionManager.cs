    ...
    namespace Report
    {
        public class ReactionManager
        {
                ...
                foreach (KeyValuePair<int,ArrayList> pair in m_dicHistoryLog)
                {
                    ...
                    if (reactionLog.SensorType == 0)
                    {
                        ...
                        if (reactionLog.SensorType == 0)
                        {
                            ...
                            {
                                if (Typelog.ReactionType == (int)ReactionType.NOTIFY_FIRE ||
                                    Typelog.ReactionType == (int)ReactionType.MALFUNCTION ||
                                    Typelog.ReactionType == (int)ReactionType.IGNORE_FIRE)
                                {
                    ...
                    if (nSensorHistoryID == nHistoryID)
                    {
                        ...
                        foreach (SensorReactionLog srLog in log)
                        {
                            ...
                            if (srLog.ReactionType == (int)ReactionType.MALFUNCTION
                                || srLog.ReactionType == (int)ReactionType.NOTIFY_FIRE
                                || srLog.ReactionType == (int)ReactionType.IGNORE_FIRE)
                            {
                                int nCommanderID = -1;
            ...
            private ArrayList GetDetectLog(ArrayList arrAllLog)
            {
                ...
                {
                    if (reactionLog.ReactionType == (int)ReactionType.BEGIN_STATUS || (reactionLog.ReactionType == (int)ReactionType.NOTIFY_FIRE && reactionLog.Param2 == "0")
                ...
                foreach (Zone zone in arrZoneList)
                {
                    ...
                    foreach (int nHistoryID in arrHistoryList)
                    {
                        ...
                        foreach (SensorReactionLog log in arrLog)
                        {
                            ...
                            // 처리되지 않음.
                            else if (log.ReactionType == (int)ReactionType.IGNORE_FIRE)
                            {
                                nType = log.ReactionType;
            ...
            private ArrayList GetManualReactionHistory(ArrayList arrZoneList, string startDate, string endDate)
            {
                ...
                for (int i = 0; i < nResultCount - 10; i += 11)
                {
                    ...
                    if  (nReactionType == (int)ReactionType.MALFUNCTION || 
                         nReactionType == (int)ReactionType.NOTIFY_FIRE ||
                         nReactionType == (int)ReactionType.IGNORE_FIRE)
                    {
        ...
        class ReactionLog : IComparable
        {
            ...
            public override string ToString()
            {
                ...
                if (nReactionType == (int)ReactionType.NOTIFY_FIRE)
                    strReactionType = "화재 발생";
                else if (nReactionType == (int)ReactionType.MALFUNCTION)
                    strReactionType = "오작동 처리";
                else if (nReactionType == (int)ReactionType.IGNORE_FIRE)
