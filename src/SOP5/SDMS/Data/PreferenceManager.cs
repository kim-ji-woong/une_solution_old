namespace SDMS
{
	public class PreferenceManager
	{
        ...
        private void SaveSignalState()
        {
            ...
            string szValue1 = DBUtility.RegUtil.ReadRegValue("SDMS", "ReciveFireSignal", nSiteID);
            if (szValue1.ToLower() == "false")
            {
                bReciveFireSignal = false;
            }

            string szValue2 = DBUtility.RegUtil.ReadRegValue("SDMS", "RecivePSMSignal", nSiteID);
            if (szValue2.ToLower() == "false")
            {
                bRecivePSMSignal = false;
            }

            string szValue3 = DBUtility.RegUtil.ReadRegValue("SDMS", "ReciveSecuritySignal", nSiteID);
            ...
            DBUtility.RegUtil.WriteRegValue("SDMS", "ReciveFireSignal", m_bReciveFireSignal.ToString(), nSiteID);
            DBUtility.RegUtil.WriteRegValue("SDMS", "RecivePSMSignal", m_bRecivePSMSignal.ToString(), nSiteID);
            DBUtility.RegUtil.WriteRegValue("SDMS", "ReciveSecuritySignal", m_bReciveSecuritySignal.ToString(), nSiteID);

            if (bRequestReactionLogList == true)
            {
                NetworkManager.Instance.SendRequestReactionLogList();
            }
