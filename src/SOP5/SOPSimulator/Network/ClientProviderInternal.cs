...
namespace SOPMonitoringSystem
{
    public class ClientProviderInternal : ClientServiceProvider
    {
            ...
            try
            {
                if (bytes != null)
                {
                    ...
                    if (nBytesCount > 0)
                    {
                        ...
                        short nHeader;
                        ArrayList arrDatas = ClientProvider.ReadBytes(m_arrReceived, out nHeader);
        ...
        private void ProcessAreYouThere(ArrayList arrDatas)
        {
            ...
            byte[] bytes = ClientProvider.MakeBytes(TCP_ID.I_AM_HERE, arrDatas);

            m_mgr.Send(bytes, this, m_strTag);
        }
        ...
        private void SendWhoIAm()
        {
            ...
            byte[] bytes = ClientProvider.MakeBytes(TCP_ID.WHO_I_AM, arrDatas);
        ...
        {
            byte[] bytes = ClientProvider.MakeBytes(header, arrDatas);
