...
namespace SOPMonitoringSystem
{
    public class ClientProvider : ClientServiceProvider
    {
        ...
        private bool OnReceive(byte[] bytes)
        {
            if (bytes != null)
            {
                ...
                if (nBytesCount > 0)
                {
                    ...
                    {
                        ProcessCancelRequestControl(m_arrReceived);
                    }
        ...
        {
            // SDMS 지진 이벤트를 발생시킨다.
            SDMSMessenger.Instance.ShowBuildingCollapse("yhz85", "14호기 기계공작실");
        ...
        /*private VersionInfo GetActionStep(string strSOPPath, ArrayList actionSteps)
        {
            ...
            string strSQL = "SELECT d.VersionID, d.ID, v.VersionName from DisasterCategory as dc, SubDisasterCategory as sdc, Disaster as d, Version as v ";
            strSQL += "where d.SubDisasterID = sdc.ID and sdc.DisasterID = dc.ID and d.VersionID = v.ID and v.isRegular = 1 and v.isNormal = 1 and d.DisasterName = '" + tokens[0].Trim() + "' ";
            strSQL += "and sdc.SubCategoryName = '" + tokens[1].Trim() + "' and dc.CategoryName = '" + tokens[2].Trim() + "' order by d.VersionID";
            ...
            VariousData<int> versionID = WebDBManager.GetIntField(arrResult[0].ToString());
            VariousData<int> disasterID = WebDBManager.GetIntField(arrResult[1].ToString());
            string strVersionName = WebDBManager.GetStringField(arrResult[2]);

            if (versionID == null || disasterID == null || strVersionName == null)
                return null;

            strSQL = "SELECT ID, StepName from ActionStep where DisasterID = " + disasterID.Data.ToString();
            ...
            for (int i=0;i<nResultCount-1;i+=2)
            {
                ...
                if (actionStepID == null || strStepName == null)
                    continue;
        ...
        private void ProcessCancelRequestControl(byte[] bytes)
        {
            string strUserID;
            string strUserName, strUserNickName, strIP;
            int nIndex = 6;
        ...
        private void LoseControl()
        {
            ...
            FormSOP.Instance.Invoke((MethodInvoker)delegate
            {
                try
                {
                     ...
                     SupervisorSOPClose.SupervisorSOPLostControlAuthority();
