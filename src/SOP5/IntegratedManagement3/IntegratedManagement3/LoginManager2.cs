...
namespace IntegratedManagement3
{
	public class LoginManager
	{
		...
		public void OnAcceptLogin(int nUserID, string szUserName, string szNickName)
		{
            ...
            IntegratedManagement3.PopupDialog.Chief m_Chief = new IntegratedManagement3.PopupDialog.Chief();
            ...
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL + strSQLAdd, 0);
            if (arrResult != null && arrResult.Count != 0)
            {
                m_Chief.DayLight_Day = true;

                m_Chief.SOPTYPE = (IntegratedManagement3.PopupDialog.SOPTeamType)WebDBManager.GetIntField(arrResult[0].ToString(), -1);
            ...
            arrResult.Clear();
            arrResult = m_dbMgr.GetResultData(strSQL + strSQLAdd, 0);
            if (arrResult != null && arrResult.Count != 0)
            {
            ...
            System.Diagnostics.Trace.WriteLine("OnAcceptLogin");
        ...
        public bool JoinUser(int nID, string szMemberID, string szPass, string szNickName, IntegratedManagement3.PopupDialog.Chief chief)
        {
        ...
        public void OnChangeSOPGenUserCommander(int nResult)
        {
            if (m_frmMain != null)
            {
        ...
        public bool LogIn(string strID, string strEncrypt, bool isEncryptPass)
        {
            if (isEncryptPass)
            {
                ...
                DBUtility.RegUtil.WriteRegValue("Update Info", "LastUser", strID, m_frmMain.SiteID);
                DBUtility.RegUtil.WriteRegValue("Update Info", "LastEncr", strEncrypt, m_frmMain.SiteID);
        ...
        public bool ChangeSOPGenCommander(IntegratedManagement3.PopupDialog.Chief pchief)
        {
		...
		public int GetMemberID(string strMemberID, string strMemberName, ref string strGenUserID)
		{
			...
			ArrayList arrResult3 = m_dbMgr.GetResultData(strSQL, 0);
