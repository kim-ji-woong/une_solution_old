...
namespace PSMSensorServer
{
	public class NetworkClient
	{
		...
		public bool SendSensorData(int nSensorZoneID, int nSensorTagInfoID, int nSensorType, int nData, string szBuilding, string szTag, bool bPSM = false, bool bTest = false, int windDirection = -1, int windSpeed = -1)
		{
			...
			byte[] bytes = new byte[60];        //42에서 60으로 변경. (wind 관련 추가 전송)
            ...
            byte[] wDircetionBytes = SOPClientProvider.MakeBytes((int)windDirection);       //wind direcction ==> 0 : N, 1 : NE, E : 2, 3 : SE, 4 : S, 5 : SW, 6 : W, 7 : NW 
            byte[] wSpeedBytes = SOPClientProvider.MakeBytes((int)windSpeed);               //windspeed ===> 1 : 2.7미만, 2 : 2.7이상
            ...
            CopyBytes(bytes, ref nIndex, wDircetionBytes);          //암모니아 확산 모델링을 위한 바이트 전송, -1이면 암모니아 아님
            CopyBytes(bytes, ref nIndex, wSpeedBytes);              //상등
        ...
        public bool SendFireSensorData(int sensorType, int sensorTagID, int sensorZoneID)
        {
           ...
           // byte[] bytes = new byte[60];        
            ...
            byte[] bytes = SOPClientProvider.MakeBytes(TCP_ID.SENSOR_DATA, arrDatas);
            m_msgQueue.Add(new QueueData_SensorData(bytes, sensorZoneID, sensorType));
            return true;
