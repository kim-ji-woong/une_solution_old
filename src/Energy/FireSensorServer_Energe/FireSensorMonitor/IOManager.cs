...
namespace SensorMonitor
{
    public class IOManager
    {
        ...
        public void LoadBuildings()
        {
            ...
            WebDBManager m_dbMgr = SOPMonitor.Instance.DbMgr;
...
//" FROM SensorZone as sz, EquipmentZone as ez WHERE sz.EquipZoneID = ez.ID and ez.SiteID = {0}";
		...
		public void LoadReciverList()
		{
            ...
            string strSQL =
                "SELECT ID,Place, IP, MacAddr, Baudrate, Mode, FlowCtrl, Multiport, Timeout, Description FROM SensorServerInfo " 
            + " WHERE ReciverType = 1 AND SiteID =" + m_nSiteID.ToString();
		...
		private bool LoadCurcuit(Reciver reciver)
		{
            ...
            string strSQL = string.Format("select ID, TagNo, TagID, SensorName, SensorType, SensorZoneID, Description from SensorTagInfo where SensorServerID = {0}", reciver.ID);
			//if( m_dbMgr.DatabaseName == "SOP3")
            //    strSQL = string.Format("select ID, TagNo, SensorName, SensorType, EquipZoneID, Description from SensorTagInfo where SensorServerID = {0}", reciver.ID);
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL, 0);
			...
			for (int i = 0; i < nResultCount - 5; i += 7)
			{
				int nID = WebDBManager.GetIntField(arrResult[i].ToString(), -1);
                int nTagNo = WebDBManager.GetIntField(arrResult[i+1].ToString(), -1);
                string strTagID = WebDBManager.GetStringField(arrResult[i + 2].ToString(), "");
				...
				int nEquipZone = WebDBManager.GetIntField(arrResult[i + 5].ToString(), -1);

				string strDesc = WebDBManager.GetStringField(arrResult[i + 6], "");

				Curcuit curcuit = new Curcuit();

				curcuit.ID = nID;
                curcuit.TagNum = nTagNo;
                curcuit.TagID = strTagID;
				curcuit.ReciverID = reciver.ID;
				curcuit.SensorType = nSensorType;
				curcuit.TargetZoneID = nEquipZone;
				curcuit.Name = strName;

                if (!reciver.Curcuits.ContainsKey(strTagID))
				{
                    reciver.Curcuits.Add(strTagID, curcuit);
				}
