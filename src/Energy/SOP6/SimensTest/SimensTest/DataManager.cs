...
namespace SensorTester
{
    public class DataManager
    {
        ...
        private Dictionary<int, string> LoadPSMSensorName()
        {
            Dictionary<int, string> dicPSMSensorNames = new Dictionary<int, string>();

            string strSQL = "select ps.SensorName, sz.ID from PSMSensor as ps, SensorZone sz where ps.ID = sz.OrgSensorID and sz.Type = " + ((int)SensorTag.SensorType.PSM센서).ToString();
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL);

            if (arrResult == null)
                return dicPSMSensorNames;

            int nResultCount = arrResult.Count;

            for (int i=0;i<nResultCount-1;i++)
            {
                ...
                if (strSensorName == null || sensorZoneID == null)
                    continue;

                dicPSMSensorNames[sensorZoneID.Data] = strSensorName;
            }
        ...
        {
            Dictionary<int, string> dicPSMSensorNames = LoadPSMSensorName();
            ...
            for (int i = 0; i < nResultCount - 5; i += 6)
            {
                ...
                tag.SensorDisplayName = strSensorName;
                ...
                {
                    string strPSMSensorName = "";

                    if (dicPSMSensorNames.TryGetValue(nSensorZoneID, out strPSMSensorName))
                        tag.SensorDisplayName = strPSMSensorName;
                }
            }
        ...
        {
            string strSQL = "select Building.ID, BuildingName, BuildingGroupID, BroadCastingText from Building, BuildingGroup where Building.BuildingGroupID = BuildingGroup.ID and BuildingGroup.SiteID = " + m_nSiteID.ToString();
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL);
            ...
            for (int i = 0; i < nResultCount - 3; i += 4)
            {
                ...
                if (nID < 0 || nBuildingGroupID < 0 || strBroadcastingName == "null" || strBroadcastingName.Length == 0)
                    continue;
                ...
                building.ID = nID;
                building.Name = strBroadcastingName;
        ...
        {
            string strSQL = "select ID, BuildingID, BroadCastName from Zone where SiteID = " + m_nSiteID.ToString();
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL);
        ...
        public void MakeSensorTagTree(TreeView tree, string strSearchWord)
        {
            ...
            if (String.IsNullOrWhiteSpace(strSearchWord) == false)
            {
                ...
                foreach (SensorTag item in from items in m_dicSensorTags.Values.AsEnumerable()
                                           where items.SensorDisplayName.Contains(strSearchWord)
        ...
        private void AddNode(TreeView tree, SensorTag tag)
        {
            ...
            foreach (Zone zone in equipZone.LinkedZones)
            {
                if (zone.IsOutdoor)
                {
                    TreeNode root = GetOutdoorZoneRootNode(tree);
                    TreeNode zoneNode = GetZoneNode(root.Nodes, zone);
                    AddSensorTagNode(zoneNode, tag);
                }
                else if (zone.Building != null)
                {
                    TreeNode root = GetBuildingGroupNode(tree, zone.Building.BuildingGroup);
                    TreeNode buildingNode = GetBuildingNode(root.Nodes, zone.Building);
                    TreeNode zoneNode = GetZoneNode(root.Nodes, zone);
                    AddSensorTagNode(zoneNode, tag);
                }
            }
        ...
        private TreeNode GetBuildingGroupNode(TreeView tree, BuildingGroup group)
        {
        ...
        {
            TreeNode tagNode = node.Nodes.Add(tag.SensorDisplayName);
            //TreeNode tagNode = node.Nodes.Add(tag.SensorName);
            tagNode.Tag = tag;
            return tagNode;
        }

        private TreeNode GetOutdoorZoneRootNode(TreeView tree)
        {
            foreach (TreeNode node in tree.Nodes)
            {
                if (node.Tag == null)
