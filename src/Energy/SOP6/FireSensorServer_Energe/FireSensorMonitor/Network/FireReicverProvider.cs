...
namespace SensorMonitor
{
    internal class FireReciverProvider : ClientServiceProvider
    {
        ...
        private Dictionary<string, Curcuit> m_nCircuits = null;
                
        // 이전에 처리되고 남은 수신데이터
        // 다음번 처리시 앞에 붙는다.
        private byte[] m_extraBuffer = null;

        public FireReciverProvider(NetworkWebManager mgr, Reciver reciver)
        {
            ...
            try
            {
                ...
                if (IsConnected == false)
                {
                    try
                    {
                        ...
                        System.Diagnostics.Trace.WriteLine("Connnect [ " + m_szIP + " ], " + m_Reciver.ID + " : " + bResult);

                        if (bResult== true)
                        {
                            m_nPingCount = 0;
                            //SendNACK();
                            SendData(BitConverter.GetBytes(m_nReciverNum)[0]);
                        }

                        if (m_mgr != null)
                            m_mgr.SendConnectionState(m_Reciver.ID, true);
        ...
        private void ProcessRecivedData(byte[] data)
        {
            ...
            foreach (byte[] cmd in arDatas)
            {
                ...
                if (!IsPoll(cmd))
                {
                    ...
                    // AA(동), BB(계단), CC(층) 정보를 가져옴 (6자리)
                    string strCurcuit = GetCurcuit(cmd);

                    // 해당 데이터를 처리한다.
                    ProcessSensorData(cmd, strCurcuit);
                    SendACK();
                }
                // Polling data 일 경우 (현재 사용하지 않음)
                else
                {
        ...
        // AA(동), BB(계단), CC(층) 정보를 가져옴 (6자리)
        private string GetCurcuit(byte[] data)
        {
            //02 30 30 2D 30 30 2D 30 30 65 34 03
            if (data.Length < 9)
                return "";
            ...
            for (int i = 1; i < data.Length-3; i += 3)
            {
                sb.Append((char)data[i]);
                sb.Append((char)data[i+1]);
            }
            string szTag = sb.ToString();
        ...
        private bool IsPoll(byte[] data)
        {
            ...
            if (data.Length <= nSTX + 8)
                return false;

            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < 8; ++i)
                sb.Append((char)data[nSTX + i]);
        ...
        private void ProcessSensorData(byte[] bytes, string strCurcuit)
        {
            if (strCurcuit == "")
                return;
            ...
            if (m_nCircuits.ContainsKey(strCurcuit))
            {
                curcuit = m_nCircuits[strCurcuit];
            }

            // 회로번호가 없는 경우
            if (curcuit == null)
            {
                logger.Info("없는 회로 번호 : " + strCurcuit);
                return;
            }
            ...
            {
                logger.Info("처리할 수없는 데이터 유형 : " + strCurcuit);
                return;
            }
        ...
        {
            string strCurcuit = curcuit.TagID;

            logger.Info("[SOP서버로 회로 이름 " + curcuit.Name + " 에 대해 " + nData.ToString() + " 값 전송, 회로" + strCurcuit + "]");
            Debug.WriteLine("[SOP서버로 회로 이름 " + curcuit.Name + " 에 대해 " + nData.ToString() + " 값 전송, 회로" + strCurcuit + "]");
        ...
        {
            int nData = 0;
            foreach (KeyValuePair<string, Curcuit> pair in m_nCircuits)
            {
