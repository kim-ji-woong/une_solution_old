...
namespace PSMSensorServer
{
    public partial class FormWeather : Form
    {
        ...
        public FormWeather(LocalDBManager dbMgr)
        {
            m_dbMgr = dbMgr;
            InitializeComponent();
        }
        ...
        private void ReadWeatherInfo()
        {
            ...
            if( arResult != null && arResult.Count > 0)
            {
                ...
                {
                    string szDate = LocalDBManager.GetStringField(arResult[i]);
                    ...
                    float fWind = LocalDBManager.GetFloatField(arResult[i + 1].ToString(), -9999);
                    float fWSpeed = LocalDBManager.GetFloatField(arResult[i + 2].ToString(), -9999);
                    float fTemp = LocalDBManager.GetFloatField(arResult[i + 3].ToString(), -9999);
