...
UBML파일 또는 3ds 파일을 로드하여 3차원 가시화 및 사용자 인터렉션을 수행하는 프로그램


이 파일에는 UBMLViewer 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
UBMLViewer.h
	...
	헤더(Resource.h 포함)가 들어 있고 CUBMLViewerApp 응용 프로그램
	클래스를 선언합니다.

UBMLViewer.cpp
	응용 프로그램 클래스 CUBMLViewerApp이(가) 들어 있는 기본 응용 프로그램
	소스 파일입니다.
...
res\UBMLViewer.ico
	...
	주 리소스 파일인 UBMLViewer.rc에 의해 포함됩니다.
...
UBMLViewerDoc.h, UBMLViewerDoc.cpp - 문서
	이 파일에는 CUBMLViewerDoc 클래스가 들어 있습니다. 이 파일을 편집하여
	특수한 문서 데이터를 추가하고 CUBMLViewerDoc::Serialize를 통해 파일
	저장 및 로드를 구현합니다.
	이 문서에는 다음 문자열이 포함됩니다.
		파일 확장명:      ubml
		파일 형식 ID:        UBMLViewer.Document
		주 프레임 캡션:  UBMLViewer
		문서 형식 이름:       UBMLViewer
		필터 이름:         UBMLViewer Files (*.ubml)
		파일의 새 약식 이름: UBMLViewer
		파일 형식의 긴 이름: UBMLViewer.Document

UBMLViewerView.h, UBMLViewerView.cpp - 문서 뷰
	이 파일에는 CUBMLViewerView 클래스가 들어 있습니다.
	CUBMLViewerView 개체는 CUBMLViewerDoc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
	이 파일은 미리 컴파일된 헤더 파일(PCH)인 UBMLViewer.pch와
