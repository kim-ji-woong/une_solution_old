...
class Paginate extends Component {
    ...
    onClickLast = () => {
        const pageNumNext = this.state.allPage;
        this.setState({ page: pageNumNext });
        this.props.onChange(pageNumNext);
        return;
    }
    ...
    render() {
        ...
        if (this.state.page !== 1) {
            paging.push(<a key={Math.random()} className="arrow pprev" onClick={this.onClickFirst} ></a>);
            paging.push(<a key={Math.random()} className="arrow prev" onClick={this.onClickPre} ></a>);
        } else {
            paging.push(<a key={Math.random()} className="arrow pprev" ></a>);
            paging.push(<a key={Math.random()} className="arrow prev" ></a>);
        }

        for (let i = first; i <= last; i++) {
            if (i === this.state.page) {
                paging.push(<a key={Math.random()} className="active"> {i} </a>);
            } else if (i !== this.state.page) {
                paging.push(<a key={Math.random()} onClick={() => this.onClickPage(i)}> {i} </a>);
            }
        }

        if (this.state.page !== this.state.allPage) {
            paging.push(<a key={Math.random()} className="arrow next" onClick={this.onClickNext}></a>);
            paging.push(<a key={Math.random()} className="arrow nnext" onClick={this.onClickLast}></a>);
        } else {
            paging.push(<a key={Math.random()} className="arrow next" ></a>);
            paging.push(<a key={Math.random()} className="arrow nnext" ></a>);
        }
