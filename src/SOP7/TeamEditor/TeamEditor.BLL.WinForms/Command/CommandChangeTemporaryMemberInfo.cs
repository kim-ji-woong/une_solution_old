...
namespace TeamEditor.BLL.WinForms.Command
{
    public class CommandChangeTemporaryMemberInfo : CommandEx
    {
        ...
        public CommandChangeTemporaryMemberInfo(IDataManager dataManager, MergedDataGridView grid, TemporaryMember member)
        {
        ...
        public override void Do()
        {
            ...
            if (m_isNewMember)
            {
                ...
                {
                    members = GetManager.GetTemporaryNormalMembers((TemporaryNormalTeam)m_team);

                    if (members == null)
                    {
                        members = new List<TemporaryMember>();
                        SetManager.SetTemporaryNormalMembers((TemporaryNormalTeam)m_team, members);
                    }
                }
                else if (m_team is TemporaryEmergencyTeam)
                {
                    members = GetManager.GetTemporaryEmergencyMembers((TemporaryEmergencyTeam)m_team);

                    if (members == null)
                    {
                        members = new List<TemporaryMember>();
                        SetManager.SetTemporaryEmergencyMembers((TemporaryEmergencyTeam)m_team, members);
                    }
                }
        ...
        private void UpdateDB(WebDBManager dbMgr)
        {
            ...
            // TemporaryMember >> 변환 필요. 테이블 컬럼 항목이 필요.
            // companyLink = ReadTemporaryMember(m_member);
            m_dataManager.GetUpdateManager().UpdateCompanyLink(companyLink, out strErrorMessage);

        }

        private CompanyLink ReadTemporaryMember(TemporaryMember member)
        {
        ...
        private void AddDB(WebDBManager dbMgr)
        {
            if (m_rollbackSQLs == null)
            {
                ...
                string strErrorMessage;
                GetManager getManager = new GetManager(m_dataManager);
                int nID = getManager.GetMaxTemporaryCompanyLinkID();
                ...
                CompanyLink companyLink = new CompanyLink();
                // companyLink 데이터 값 입력 필요 >> 컬럼 지정
                if (m_dataManager.GetCreateManager().AddCompanyLink(companyLink) == false)
                {
                    dbMgr.BatchRollback();
                    return;
                }
