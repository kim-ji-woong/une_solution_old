...
using TeamEditor.BLL.WinForms.Command;
...
namespace TeamEditor
{
    public class TeamGrid : MergedDataGridView
    {
        ...
        private void RemoveTemporaryMembers()
        {
            ...
            string strMembers = "";
            List<CommandMoveTemporaryMembers.TemporaryMemberNIndex> memberIndeces = new List<CommandMoveTemporaryMembers.TemporaryMemberNIndex>();
            ...
            foreach (int nRowIndex in rowIndeces)
            {
                ...
                CommandMoveTemporaryMembers.TemporaryMemberNIndex memberIndex = new CommandMoveTemporaryMembers.TemporaryMemberNIndex(member, nRowIndex);
            ...
            {
                CommandRemoveTemporaryMembers cmd = new CommandRemoveTemporaryMembers(m_teamCurrent, memberIndeces, this);
        ...
        private void RemoveExternalMember()
        {
            ...
            //string strMembers = "";
            //List<CommandRemoveExternalCompanyMembers.ExternalMemberNIndex> memberIndeces = new List<CommandRemoveExternalCompanyMembers.ExternalMemberNIndex>();
            ...
            //    CommandRemoveExternalCompanyMembers.ExternalMemberNIndex memberIndex = new CommandRemoveExternalCompanyMembers.ExternalMemberNIndex(member, nRowIndex);
            //    memberIndeces.Add(memberIndex);
        ...
        private void RemoveCompanyMembers()
        {
            ...
            string strMembers = "";
            List<CommandMoveRegularMembers.CompanyMemberNIndex> memberIndeces = new List<CommandMoveRegularMembers.CompanyMemberNIndex>();
            ...
            foreach (int nRowIndex in rowIndeces)
            {
                ...
                CommandMoveRegularMembers.CompanyMemberNIndex memberIndex = new CommandMoveRegularMembers.CompanyMemberNIndex(member, nRowIndex);
                memberIndeces.Add(memberIndex);
            ...
            if (_result == DialogResult.Yes)
            {
                CommandRemoveRegularMembers cmd = new CommandRemoveRegularMembers((RegularTeam)m_teamCurrent, memberIndeces, this);
                FormMain.Instance.AddCommand(cmd);
            } 
        }
        ...
        private void RemoveUserDefinedTeams()
        {
            ...
            //if (_result == DialogResult.Yes)
            //{
            //    CommandRemoveUserDefinedTeam cmd = new CommandRemoveUserDefinedTeam(teams, this);
            //    FormMain.Instance.AddCommand(cmd);
            //}
        }
        ...
        private void TeamGrid_MouseDown(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            if (m_gridType == GridType.RegularMember)
            {
                ...
                CommandMoveRegularMembers cmd = MakeMoveRegularMemberCommand(row);
            ...
            else if (m_gridType == GridType.TemporaryNormal || m_gridType == GridType.TemporaryEmergency)
            {
                ...
                CommandMoveTemporaryMembers cmd = MakeMoveTemporaryMemberCommand(row);
                LinkedTree.DropData = cmd;
                dropType = TeamTreeView.DropDataType.TEMPORARY_MEMBER;
            }           
            else
        ...
        private void TeamGrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (this.m_gridType == GridType.TemporaryEmergency || this.m_gridType == GridType.TemporaryNormal)
            {
                if (this.Rows[e.RowIndex].Tag is null || (this.Rows[e.RowIndex].Tag is TemporaryMember) == false)
                    return;

                TemporaryMember member = (TemporaryMember)this.Rows[e.RowIndex].Tag;

                if (e.ColumnIndex == IncludeChildTeamImageIndex || e.ColumnIndex == IncludeChildTeamIndex)
                {
                    if (member.TemporaryMemberType == TemporaryMember.MemberType.RegularTeam || member.TemporaryMemberType == TemporaryMember.MemberType.RegularTeam)
                    {
                        try
                        {
        ...
        private void OnFinishRegularMemberCellEdit(DataGridViewCell cell, ComboBox cbo)
        {
            ...
            CommandChangeRegularMemberInfo cmd = null;
            ...
            if (cell.ColumnIndex == SubLevelIndex)
            {
                ...
                CommandChangeRegularMemberInfo.InfoType type = CommandChangeRegularMemberInfo.ToInfoType(cell.ColumnIndex);

                cmd = new CommandChangeRegularMemberInfo(this, member);
                cmd.DataType = type;
                cmd.IsNewMember = isNewMember;

                if (type == CommandChangeRegularMemberInfo.InfoType.Unknown)
                    return;
        ...
        private void OnFinishExternalMemberCellEdit(DataGridViewCell cell, ComboBox cbo)
        {
            ...
            //CommandChangeExternalMemberInfo cmd = null;
            ...
            //    CommandChangeExternalMemberInfo.InfoType type = CommandChangeExternalMemberInfo.ToInfoType(cell.ColumnIndex);
        ...
        private CommandMoveTemporaryMembers MakeMoveTemporaryMemberCommand(DataGridViewRow pRow)
        {
            List<CommandMoveTemporaryMembers.TemporaryMemberNIndex> memberIndeces = new List<CommandMoveTemporaryMembers.TemporaryMemberNIndex>();
            ...
            if (_row.Tag != null && _row.Tag is TemporaryMember)
            {
                ...
                memberIndeces.Add(new CommandMoveTemporaryMembers.TemporaryMemberNIndex(member, _row.Index));
            }

            CommandMoveTemporaryMembers cmd = new CommandMoveTemporaryMembers(m_teamCurrent, null, memberIndeces, this, m_linkedTree);
            return cmd;
        }

        private CommandMoveTemporaryMembers MakeMoveTemporaryMembersCommand(out string strMemberNames)
        {
            strMemberNames = "";
            ...
            foreach (KeyValuePair<int, int> pair in dicRowIndeces)
            {
                ...
                {
                    TemporaryMember member = (TemporaryMember)row.Tag;
                    memberIndeces.Add(new CommandMoveTemporaryMembers.TemporaryMemberNIndex(member, pair.Value));
        ...
        private CommandMoveRegularMembers MakeMoveRegularMemberCommand(DataGridViewRow pRow)
        {
            List<CommandMoveRegularMembers.CompanyMemberNIndex> memberIndeces = new List<CommandMoveRegularMembers.CompanyMemberNIndex>();
            DataGridViewRow _row = pRow;
            ...
            if (_row.Tag != null && _row.Tag is CompanyMember)
            {
                ...
                memberIndeces.Add(new CommandMoveRegularMembers.CompanyMemberNIndex(member, _row.Index));
            }

            CommandMoveRegularMembers cmd = new CommandMoveRegularMembers((RegularTeam)m_teamCurrentRow, null, memberIndeces, this, m_linkedTree);
            return cmd;
        }

        private CommandMoveRegularMembers MakeMoveRegularMembersCommand(out string strMemberNames)
        {
            strMemberNames = "";
            ...
            foreach (KeyValuePair<int, int> pair in dicRowIndeces)
            {
                ...
                if (row.Tag != null && row.Tag is CompanyMember)
                {
                    CompanyMember member = (CompanyMember)row.Tag;
                    //members.Add(member);
                    memberIndeces.Add(new CommandMoveRegularMembers.CompanyMemberNIndex(member, pair.Value));
        ...
        private CommandMoveRegularMembers MakeMoveRegularMembersCommand2(DataGridViewCell cell)
        { 
            Dictionary<int, int> dicRowIndeces = new Dictionary<int, int>();
            ...
            CommandMoveRegularMembers cmd = new CommandMoveRegularMembers((RegularTeam)cell.Tag, null, memberIndeces, this, m_linkedTree);
            return cmd;
        }
        ...
        { 
            ...
            //if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (frm.SelectedTeam is RegularTeam)
                {
                    ...
                    {
                        string strMemberNames;
                        CommandMoveRegularMembers cmd = MakeMoveRegularMembersCommand(out strMemberNames);
        ...
        private void tsMoveTemporaryMember_Click(object sender, EventArgs e)
        {
            ...
            //if (frm.ShowDialog() == DialogResult.OK)
            {
                ...
                if (IsTemporaryTeam((Team)frm.SelectedTeam, out isNormal))
                {
                    ...
                    {
                        string strMemberNames;
                        CommandMoveTemporaryMembers cmd = MakeMoveTemporaryMembersCommand(out strMemberNames);
        ...
        private void TemporaryMemberCellEndEdit(DataGridViewRow row, DataGridViewCell cell, bool isNormal)
        {
            ...
            CommandChangeTemporaryMemberInfo.InfoType type = CommandChangeTemporaryMemberInfo.ToInfoType(cell.ColumnIndex);

            if (type == CommandChangeTemporaryMemberInfo.InfoType.Unknown)
                return;

            CommandChangeTemporaryMemberInfo cmd = new CommandChangeTemporaryMemberInfo(this, member);
        ...
        private bool GetTemporaryCellData(DataGridViewCell cell, out object cellData)
        {
            ...
            if (cell.ColumnIndex == (int)CommandChangeTemporaryMemberInfo.InfoType.Member)
            {
                if (cell.Tag == null)
                    return true;
                else
            ...
            else if (cell.ColumnIndex == (int)CommandChangeTemporaryMemberInfo.InfoType.Position)
            {
            ...
            else if (cell.ColumnIndex == (int)CommandChangeTemporaryMemberInfo.InfoType.MemberCount)
            {
                if (cell.Value == null)
                    return true;
            ...
            else if (cell.ColumnIndex == (int)CommandChangeTemporaryMemberInfo.InfoType.IncludeChildTeams)
            {
                cellData = cell.Value;
                return true;
            }
        ...
        private void RegularMemberCellEndEdit(DataGridViewRow row, DataGridViewCell cell)
        {
            ...
            if (!isNewMember && cell.ColumnIndex == TeamNameIndex) // 기존 직원 팀이동은 팀이동 Command 사용
            {
                ...
                { 
                    CommandMoveRegularMembers cmd = MakeMoveRegularMembersCommand2(cell);
            ...
            {
                CommandChangeRegularMemberInfo cmd = new CommandChangeRegularMemberInfo(this, member);
                cmd.DataType = type;
                cmd.IsNewMember = isNewMember;
        ...
        private void UserDefinedTeamCellEndEdit(DataGridViewRow row, DataGridViewCell cell)
        {
            ...
            //CommandChangeUserDefinedTeamInfo.InfoType type = Command.CommandChangeUserDefinedTeamInfo.ToInfoType(cell.ColumnIndex);
        ...
        public CommandChangeTemporaryMemberInfo GetTemporaryMemberChangingCommand(object dataTeam, object dataMember, CommandChangeTemporaryMemberInfo.InfoType type)
        {
            if (m_teamCurrent == null)
                return null;
