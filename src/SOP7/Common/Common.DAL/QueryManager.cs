...
namespace Common.DAL
{
    public class QueryManager
    {
        ...
        protected string ShelterBoundaryToString(List<Polygon> boundaries)
        {
            string strBoundaries = "";

            foreach (Polygon polygon in boundaries)
            {
                string strBoundary = PolygonToString(polygon);

                if (strBoundary.Length > 0)
                {
                    if (strBoundaries.Length == 0)
                        strBoundaries = strBoundary;
                    else
                        strBoundaries += "#" + strBoundary;
                }
            }

            return strBoundaries;
        }
        ...
        private string PolygonToString(Polygon polygon)
        {
            ...
            {
                if (strBoundary.Length == 0)
                    strBoundary = string.Format("{0:F1},{1:F1}", vertex.x, vertex.y);
                else
                    strBoundary += ", " + string.Format("{0:F1},{1:F1}", vertex.x, vertex.y);
            }
        ...
        protected List<Polygon> StringToShelterBoundary(string strBoundaries)
        {
            string[] tokens = strBoundaries.Split('#');
            List<Polygon> polygons = new List<Polygon>();

            foreach (string strToken in tokens)
            {
                Polygon polygon = StringToPolygon(strToken.Trim());
        ...
        private Polygon StringToPolygon(string strBoundary)
        {
            string[] tokens = strBoundary.Split(',');
            int nTokenCount = tokens.Length;

            if (nTokenCount == 0 || nTokenCount % 2 == 1)
                return null;
            ...
            {
                if (double.TryParse(tokens[i].Trim(), out x) && double.TryParse(tokens[i + 1].Trim(), out y))
                {
                    polygon.AddVertex(new Vertex2D(x, y));
                }
                else
                    return null;
            }
