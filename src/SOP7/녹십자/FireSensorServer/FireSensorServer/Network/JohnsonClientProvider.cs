...
namespace FireSensorServer.Network
{
    ...
    public class JohnsonClientProvider : ClientServiceProvider
    {
        ...
        private const string ClientType = "Johnson";

        private JohnsonManager m_mgr = null;
        ...
        public JohnsonClientProvider(JohnsonManager mgr, Logger logger)
        {
            m_mgr = mgr;
            m_logger = logger;
        ...
        {
            if (m_logger != null)
                m_logger.Write("[" + NetworkManager.GetClientTypeString(NetworkManager.ClientType.Johnson) + "] close Connection");
        }
        ...
        private void ProcessData(byte[] bytes, int nBeginIndex, int nEndIndex)
        {
            try
            {
                ...
                string strLog = string.Format("[{10}] Receiver({0}), RealyTeam({1}), Loop({2}), RelayID({3}), Tag({4}), On({5}), Area : {6}, Device : {7}, Running : {8}, Type : {9}",
                    nReceiverID, nRelayTeam, nLoopID, nRelayID, nTagID, isOn, strArea, strDevice, strRunning, (int)lType, ClientType);
                ...
                if ((m_nMuxType == MUXTYPE_1 && lType == LOG_TYPE_OP && (strDevice.Contains("화재 복구") || strDevice.Contains("화재복구"))) ||
                    (m_nMuxType == MUXTYPE_2 && lType == LOG_TYPE_RECOVERTY))
                {
                    ProcessAllClear();
                }
            ...
            {
                m_logger.Write("[" + ClientType + "] [ERROR] ClientProvider.cs > void ProcessData(byte[], int, int) :" + ex.Message);
            }
        }
        ...
        {
            int tagNo = SensorManager.GetSensorTagNumber(nReceiverID, nRelayTeam, nLoopID, nRelayID, nTagID);
            NetworkManager.Instance.SendSensorData(tagNo, NetworkManager.EventType.DetectSensor, NetworkManager.ClientType.Johnson, DateTime.Now, strArea, strRunning, "");
            /*TagInfo tag = SensorManager.Instance.GetTagInfo(NetworkManager.Instance.GetSensorServerID(NetworkManager.ClientType.Johnson), nReceiverID, nRelayTeam, nLoopID, nRelayID, nTagID);

            if (tag == null)
                return;

            NetworkWebClient.Instance.SendSensorData(tag.ID, tag.SensorZoneID, 1);*/
        }

        public void ProcessFireFromTagNo(int nSensorTagNo, bool isOn)
        {
            NetworkManager.Instance.SendSensorData(nSensorTagNo, NetworkManager.EventType.DetectSensor, NetworkManager.ClientType.Johnson, DateTime.Now, "", "", "");
            /*TagInfo tag = SensorManager.Instance.GetTagInfo(NetworkManager.Instance.GetSensorServerID(NetworkManager.ClientType.Johnson), nSensorTagNo);

            if (tag == null)
                return;

            NetworkWebClient.Instance.SendSensorDataAsync(tag.ID, tag.SensorZoneID, 1);*/
        }
        ...
        {
            int tagNo = SensorManager.GetSensorTagNumber(nReceiverID, nRelayTeam, nLoopID, nRelayID, nTagID);
            NetworkManager.Instance.SendSensorData(tagNo, NetworkManager.EventType.ClearSensor, NetworkManager.ClientType.Johnson, DateTime.Now, strArea, strRunning, "");
            /*TagInfo tag = SensorManager.Instance.GetTagInfo(NetworkManager.Instance.GetSensorServerID(NetworkManager.ClientType.Johnson), nReceiverID, nRelayTeam, nLoopID, nRelayID, nTagID);

            if (tag == null)
                return;

            NetworkWebClient.Instance.SendSensorDataAsync(tag.ID, tag.SensorZoneID, 0);*/
        }

        public void ProcessClearFromTagNo(int nSensorTagNo, bool isOn)
        {
            NetworkManager.Instance.SendSensorData(nSensorTagNo, NetworkManager.EventType.ClearSensor, NetworkManager.ClientType.Johnson, DateTime.Now, "", "", "");
            /*TagInfo tag = SensorManager.Instance.GetTagInfo(NetworkManager.Instance.GetSensorServerID(NetworkManager.ClientType.Johnson), nSensorTagNo);
        ...
        {
            // 전체복구 처리해야 함
            NetworkManager.Instance.SendSensorData(0, NetworkManager.EventType.ClearAll, NetworkManager.ClientType.Johnson, DateTime.Now, "", "", "");
            //NetworkWebClient.Instance.SendAllClearAsync();
        }
        ...
        private void WriteBinaryLog(byte[] bytes, int nIndex, int len)
        {
            ...
            strLog = string.Format("[{2}] Recv from Server\r\nBytes Length : {0}\r\n{1}", len, strLog, NetworkManager.GetClientTypeString(NetworkManager.ClientType.Johnson));
            m_logger.Write(strLog);
        }
