...
import $ from 'jquery';
import imgRectChecked from '../../../Common/img/common/checkbox_rect.png';
...
class TreeNode extends Component {
	...
	onBlur(name) {
		if (name !== this.props.teamTreeData.TeamName) {			
	...
	render() {		
		...
		if (selectedTeam !== null && selectedTeam !== undefined &&
			...
			if (this.props.isEditMode === true)
				selectNodeClass = team.selected + " " + team.wordIength;
			else
				selectNodeClass = team.select;
        }
			...
			<>
				{
					(this.props.editNodeID === this.props.teamTreeData.ID || this.state.nodeEdit) ?
						<a><input type="text" defaultValue={this.props.teamTreeData.TeamName} onBlur={(e) => this.onBlur(e.target.value)} onKeyPress={this.handleKeyPress} autoFocus /></a>
						:
						<a className={selectNodeClass} id={team.wordIength} onClick={(e) => this.onTreeNodeChanged(e)} onContextMenu={(e) => this.onTreeNodeChanged(e)}>
							{this.props.teamTreeData.TeamName}
							<span className={team.editArea}>
								<span className={team.treeEdit} onClick={this.props.editTeam}></span>
								<span className={team.treeMinus} onClick={this.props.removeTeam}></span>
								<span className={team.treePlus} onClick={this.props.addTeam}></span>
							</span>
						</a>
				}
			</>
					...
					(this.props.editNodeID === this.props.teamTreeData.ID || this.state.nodeEdit) ?
                        <>				
							<h5 style={{ marginRight: "10px" }}>
								<input type="text" defaultValue={this.props.teamTreeData.TeamName} onBlur={(e) => this.onBlur(e.target.value)} onKeyPress={this.handleKeyPress} autoFocus style={{ width: "150px" }} />
							</h5>
                        </>
						:
                        <>
							{/* <h5 style={{ marginRight: "10px" }} onClick={this.onTreeNodeChanged} onContextMenu={(e) => this.onTreeNodeChanged(e)}>{this.props.teamTreeData.TeamName}</h5> */}
							<h5 className={selectNodeClass} style={{ marginRight: "10px" }} onClick={this.onTreeNodeChanged} onContextMenu={(e) => this.onTreeNodeChanged(e)}>{this.props.teamTreeData.TeamName}
								<span className={team.editArea}>
									<span className={team.treeEdit} onClick={this.props.editTeam}></span>
									<span className={team.treeMinus} onClick={this.props.removeTeam}></span>
									<span className={team.treePlus} onClick={this.props.addTeam}></span>
		...
		if (this.props.teamTreeData.Children === null || this.props.teamTreeData.Children === undefined)
			console.log(this.props.teamTreeData.TeamName + ': NULL')
		else
			console.log(this.props.teamTreeData.TeamName + ':' + this.props.teamTreeData)
		return (	
			<>
				{output}
				{checkBox}
