...
namespace SOPWebServer2.Controllers
{
    ...
    [ApiController]
    public class PSMSensorController : ControllerBase
    {
        private SOPWebServer.BLL.MainManager m_mainManager = null;

        public PSMSensorController(SDMS.IDAL.IDataManager sdmsDataManager, Common.IDAL.IDataManager commonDataManager, TeamEditor.IDAL.IDataManager teamDataManager)
        {
        ...
        public IActionResult Post(SensorParameter param)
        {
            ...
            Result result = m_mainManager.SensorManager.OnReceive(Facility.FacilityType.PSM_SENSOR, param.Header, param.ClientInfo, arrDatas);
            return Ok(result);
        }
    }
}
