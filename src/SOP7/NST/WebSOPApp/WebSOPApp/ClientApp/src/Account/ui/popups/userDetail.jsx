...
class UserDetail extends Component {
    ...
    initPopupState() {
        var popup = document.getElementsByClassName(styles.userDetailBox)[0];
    ...
    displayInfo = () => {
        ...
        if (menu === UserDetail.Info) {
            displayInfo.push(<table>
                <tr>
                    <td>이름</td>
                    <td className={styles.userTd}>조길동</td>
                </tr>
                <tr>
                    <td>부서</td>
                    <td className={styles.userTd}>시설팀</td>
                </tr>
                <tr>
                    <td>권한</td>
                    <td className={styles.userTd}>일반관리자</td>
                </tr>
                <tr>
                    <td>연락처</td>
                    <td className={styles.userTd}>010-123-1234</td>
                </tr>
            </table>);
        } else {
            displayInfo.push(<table>
                ...
                <tr>
                    <td>연락처</td>
                    <td>
                        <input type="text" id="" alt="" placeholder="010" />-
    ...
    displayBtn = () => {
        ...
        if (menu === UserDetail.Info) {
            displayBtn.push(
                <div className={styles.detailConfirm}>
                    <span onClick={() => this.onClickChangeMode(UserDetail.Modifity)}>수정</span>
                    <span onClick={this.onClose}>목록</span>
                </div>);
        } else {
            displayBtn.push(
                <div className={styles.detailConfirm}>
                    <span onClick={() => this.onClickChangeMode(UserDetail.Info)}>저장</span>
                    <span onClick={this.onClose}>목록</span>
                </div>);
        }
    ...
    render() {
        ...
        const displayBtn = this.displayBtn();

        return (
            <div className={styles.userDetailBox}>
                <div className={styles.userDetailTitle} onMouseDown={(e) => this.popupDragMousePress(e)}>사용자정보 상세
                    <div className={styles.popupBoxDetailX}><img src={imgClose} onClick={this.onClose} alt="닫기" /></div>
                </div>
