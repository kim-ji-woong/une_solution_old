	...
	getCheckBox() {
		if (this.props.useCheckBox === TreeNode.CheckBox_NormalUse) {
			if (this.props.teamTreeData.checked === TreeNode.CHECKED_ALL) {
				return <input type="checkbox" checked={true} onChange={this.onChecked} />;
			}
			else {
				return <input type="checkbox" checked={false} onChange={this.onChecked} />;
			}
		}
			...
			if (this.props.teamTreeData.checked === TreeNode.CHECKED_ALL) {
				...
				return <input type="checkbox" checked={false} onChange={this.onChecked} style={{ background: `#fff url(${imgRectChecked}) no-repeat center center` }} />;
			}
			else {
				return <input type="checkbox" checked={false} onChange={this.onChecked} />;
            }
			...
			<>
				{
					(this.props.editNodeID === this.props.teamTreeData.ID || this.state.nodeEdit) ?
						<a>
							<input type="text" defaultValue={this.props.teamTreeData.Name} onBlur={(e) => this.onBlur(e.target.value)} onKeyPress={this.handleKeyPress} autoFocus />
						</a>
						:
						<a onClick={(e) => this.onTreeNodeChanged(e)} onContextMenu={(e) => this.onTreeNodeChanged(e)}>
