										...
										<ul className={NewSOPOptions.cssStyles.speLst}>
											{
												this.state.subDisasterCategories.map(sdc => (
													<li key={"sdc_" + sdc.subDisasterCategory.subCategoryName}>
														<input type="radio" name="speType" id={NewSOPOptions.cssStyles.speType01} onChange={() => this.onSelectSubDisasterCategory(null, sdc)}/>
														<label onClick={(event) => this.onSelectSubDisasterCategory(event, sdc)}>{sdc.subDisasterCategory.subCategoryName}</label>
													</li>
                                                ))
											}
											<li>
												<input ref={this.refNewSDCRadio} type="radio" name="speType" id={NewSOPOptions.cssStyles.speType01} onChange={() => this.onSelectNewSubDisasterCategory()} />
												<input ref={this.refNewSDC} type="text" className={bodyStyles.fullText} placeholder={SopManagerResource.ID.placeHolders.newSubDisasterCategory} />
											</li>
										</ul>
									</div>
								</div>
										...
										<ul className={NewSOPOptions.cssStyles.speIpt}>
											{
												this.state.disasterDatas.map(disasterData => (
													<li key={"disaster_" + disasterData.disasterName}>
														<input type="radio" name="speIpt" id={NewSOPOptions.cssStyles.speIpt01} onChange={() => this.onSelectDisaster(null, disasterData)}/>
														<label onClick={(event) => this.onSelectDisaster(event, disasterData)}>{disasterData.disasterName}</label>
													</li>
													))
                                            }
											<li>
												<input ref={this.refNewDisasterRadio} type="radio" name="speIpt" id={NewSOPOptions.cssStyles.speIpt01} onChange={() => this.onSelectNewDisaster()} />
												<input ref={this.refNewDisaster} type="text" className={bodyStyles.fullText} placeholder={SopManagerResource.ID.placeHolders.newSOP} />
											</li>
										</ul>
									</div>
								</div>
