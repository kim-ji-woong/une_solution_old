...
namespace SDMS.BLL.Excel.Reader
{
    ...
    public class RegularMemberReader : ExcelReader
    {
        ...
        public RegularMemberReader(string strFilePath, SDMS.IDAL.IDataManager dataManager)
        ...
        private bool SetTempRegularTeam(string strMemberIDs, RollbackManager rollback)
        {
            ...
            if (tempTeam == null)
            {
                ...
                if (m_teamDataManager.GetCreateManager().AddRegular(tempTeam, out strErrorMessage) == false)
                    return false;
                else
                {
            ...
            foreach (KeyValuePair<RegularTeam, List<RegularMember>> pair in dicTeamMembers)
            {
                foreach (RegularMember member in pair.Value)
                {
                    if (member.ID <= 0)
                    {
                        ...
                        if (m_teamDataManager.GetCreateManager().AddRegularMember(member, out strErrorMessage) == false)
                            return false;
                        else
                ...
                foreach (RegularTeam team in teams)
                {
                    if (team.ID <= 0)
                    {
                        if (team.ParentTeam == null || (team.ParentTeam != null && team.ParentTeam.ID > 0))
                        {
                            ...
                            if (m_teamDataManager.GetCreateManager().AddRegular(team, out strErrorMessage) == false)
                                return false;
                            else
