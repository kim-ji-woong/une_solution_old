...
namespace SDMS.BLL.Excel.Writer
{
    public abstract class ExcelWriter
    {
        ...
        public static ExcelWriter MakeInstance(DataMode mode, IDataManager dataManager)
        {
            if (mode == DataMode.FacilityInfo)
                return new FacilityInfoWriter(dataManager);
            else if (mode == DataMode.BuildingData)
                return new BuildingWriter(dataManager);
            else if (mode == DataMode.BuildingGroupData)
                return new BuildingGroupWriter(dataManager);
            else if (mode == DataMode.RegularTeamData)
                return new RegularMemberWriter(dataManager);
