...
namespace SOPManager
{
	public partial class FormPageSOP : Form, Sections.ISectionListener
	{
        ...
        private BarComponentDetail m_barComponentDetail = null;
        ...
        {
            m_barComponentDetail = new BarComponentDetail();
            //m_barComponent = new BarComponent();
            m_barLevelTree = new BarLevelTree();
            m_barPage = new BarPage();

            m_barComponentDetail.TopLevel = false;
            m_barComponentDetail.Dock = DockStyle.Fill;
            m_barComponentDetail.Visible = true;
            panelComponent.Controls.Add(m_barComponentDetail);
        ...
        public BarComponentDetail GetBarComponentDetail()
        {
            return m_barComponentDetail;
        }
        ...
        {
			ActionStepTabPage tabPage = new ActionStepTabPage(tabControl);
...
//#endif
		...
		public void AddUsingTeam(ArrayList arTeamMemeber)
		{
			 ...
			 for (int i = 0; i < nTeamCount; i++)
			 {
				 ...
				 AddUsingTeam(data.ID, data.TeamName, nIdx++);
			 }
		}

        private void RemoveUsingTeam(int nTeamID, string szTeamName)
        {
            bool bFindData = false;
            StepMemberData findData = new StepMemberData(szTeamName);
            foreach (StepMemberData team in m_arUsingTeam)
            {
                if (team.ID == nTeamID && team.TeamName == szTeamName)
                {
                    findData = team;
                    bFindData = true;
                    break;
        ...
        private void AddUsingTeam(int nTeamID, string szTeamName, int nIdx = -1)
		{
            foreach(StepMemberData data in m_arUsingTeam)
            {
                if(data.ID == nTeamID)
                {
                    if (data.TeamName == szTeamName)
                    {
                        return;
                    }
                }
            ...
            StepMemberData teamData = new StepMemberData(szTeamName);
			teamData.ID = nTeamID;
		...
		public void AddPane(List<SelectTeamInfo> arSelectedTeamList)
        {
            ...
            {				
				...
				AddUsingTeam(teamInfo.TeamID, teamInfo.TeamName);				
            }
        }
        ...
        public void RemoveTabPage(ActionStepTabPage tabPage = null, bool needConfirm = true)
        {
			...
			if (!remove)
			{
				...
				remove = UnE.Utility.UMessageBox.Show(szName, "삭제", MessageBoxButtons.OKCancel, MessageBoxIcon.Asterisk) == DialogResult.OK;
			}
		...
		{
            ActionStepTabPage page = GetCurrentTabPage();
            ...
            {
                if (ctrl is Sections.PanelSectionEx)
                    return (Sections.PanelSectionEx)ctrl;
            }
        ...
        public void DeletePanel(int nControlIndex)
        {
			...
			if (panel != null)
			{
				RemoveUsingTeam(panel.TeamID, panel.TeamName);
			}
	   ...
	   panel.TeamName);
				...
				if (UnE.Utility.UMessageBox.Show(strMsg, "알림", MessageBoxButtons.YesNo)
					== DialogResult.Yes)
				{
					...
					if (panel != null)
					{
						...
						RemoveUsingTeam(teamID, teamName);	
					}					
		...
		private int m_nMaxSplitPos = 320;
		private int m_nMinSplitPos = 40;
		private int m_nCurSplitPos = 320;
