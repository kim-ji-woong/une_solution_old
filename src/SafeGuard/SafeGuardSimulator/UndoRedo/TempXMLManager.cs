...
namespace SOPManager
{
    ...
    public class TempXMLManager
    {
        ...
        private bool LoadTabPages(FormMain frm)
        {
            ...
            //<int, Sections.ExternalTeamData> dicExternal = IOManager.ReadExternalTeamList(FormMain.Instance.DBManager);
            ...
            foreach (ActionStep actionStep in m_arrActionSteps)
            {
                ...
                if (!AddPanels(tabPage, actionStep, pageLevel))
                    return false;
            }
        ...
        private StepMember FindStepMember(ActionStep actionStep, int nTeamID,string strTeamName)
        {
            foreach (StepMember stepMember in actionStep.StepMemberList)
            {
                if (stepMember.ID == nTeamID && stepMember.TeamName == strTeamName)
                    return stepMember;
            }
		...
		private bool AddPanels(TabPage tabPage, ActionStep actionStep, FormPageSOP pageLevel)
        {
            ...
            foreach (Sections.PanelSectionEx panel in arrPanels)
            {
                ...
                AddComponents(panel, stepMember, nStepMemberIndex, dicSections, dicLinkSections, ref dicCompSection);                 
        ...
        private bool AddComponents(Sections.PanelSectionEx panel, StepMember stepMember, long nStepMemberIndex, Dictionary<long, Sections.Section> dicSections, Dictionary<PropertyLink, Sections.SectionLink> dicLinkSections, ref Dictionary<int, Sections.Section> dicCompSection)
        {
            foreach (Component component in stepMember.ComponentList)
            {
                ComponentProperty property = component.Property;
				Sections.Section section = ToSection(panel, component.X, component.Y, property, dicLinkSections, dicCompSection);
        ...
        private Sections.Section ToSection(Sections.PanelSectionEx panel, float x, float y, ComponentProperty property, 
            Dictionary<PropertyLink, Sections.SectionLink> dicLinkSections, 
            Dictionary<int, Sections.Section> dicCompSection)
        {
            if (property.Type == Sections.Section.ComponentType.PROCESS)
            {
                return ToSectionProcess(panel,x, y, (PropertyProcess)property);
            }            
