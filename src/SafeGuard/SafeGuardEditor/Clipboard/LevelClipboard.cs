...
namespace SOPManager
{
	public class LevelClipboard
	{
		...
		public bool PasteTab(ActionStepTabPage page)
		{
			...
			{
				UnE.Utility.UMessageBox.Show(FormMain.Instance, "같은 탭끼리는 복사할 수 없습니다.", "복사 오류", MessageBoxButtons.OK, MessageBoxIcon.Error);
				return false;
			}

			string szMessage = string.Format("기존 [{0}] 단계에 포함된 모든 컴포넌트가 삭제되며, [{1}] 단계의 컴포넌트가 복사됩니다.\n계속하시겠습니까?",  page.Text, m_tabPageCopySrc.Text);

			if (UnE.Utility.UMessageBox.Show(FormMain.Instance, szMessage, "붙여넣기", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
