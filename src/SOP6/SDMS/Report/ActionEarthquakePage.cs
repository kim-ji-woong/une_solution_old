...
namespace SDMS
{
    public partial class ActionEarthquakePage : FormReportBase
    {
        private enum RefreshType { NO_REFRESH = 0, REFRESH_ALL, REFRESH_REACTION_TYPE };

        private Report.ReactionEarthquakeManager m_ActionMgr = null;
        ...
        private List<Report.ReactionEarthquakeLog> m_allLogs = new List<Report.ReactionEarthquakeLog>();
        ...
        public ActionEarthquakePage(Report.ReactionEarthquakeManager reactionMgr)
        {
            InitializeComponent();
            ...
            this.reactionLogBindingSource.DataSource = typeof(SDMS.Report.ReactionEarthquakeLog);
		...
		{
            ArrayList arContorls = FormMain.Instance.GetActionEarthquakeContorl();
            ...
            if (react_cboSearchType.Items.Count > 0)
                react_cboSearchType.SelectedIndex = 0;
        ...
        private void SetGridViewSize()
        {
            ...
            if (m_dataGridView.Columns.Count >= 3)
            {
                m_dataGridView.Columns[0].Width = (int)(160 * sizePer);
                m_dataGridView.Columns[1].Width = (int)(500 * sizePer);
                m_dataGridView.Columns[2].Width = (int)(500 * sizePer);
            }
        ...
        public void Function_DataGrid(int ZoneHistoryID, int nReactionType)
        {
			...
			foreach (Report.ReactionEarthquakeLog data in arrSensorReactionHistory)
            {
                //찾은 검색결과를 DataGrid로 출력
            ...
            label6.SetBounds(label2.Location.X + label2.Width, label2.Location.Y, label6.Width, label6.Height);
            lblSelectDate.Text = strDate + "에 발생한 지진 탐지 대응 이력입니다.";
            lblSelectDate.SetBounds(label6.Location.X + label6.Width + 5, label6.Location.Y, lblSelectDate.Width, lblSelectDate.Height);
        ...
        private string GetMemo(Report.ReactionEarthquakeLog log)
        {
        ...
        private bool SetGridRows(Report.ReactionEarthquakeLog data, int nRow, int nCount, string strReactionType)
		{
			if (data == null)
				return false;
			if (nRow < 0)
			...
			string[] rows = { "", "", strManagerName, strType };
        ...
        public string GetReactionString(int nReactionType)
        {
            ...
            switch (nReactionType)
            {
                ...
                case (int)libSensorProcess.ReactionType.BEGIN_STATUS: strType = "지진 탐지";
                    break;
                ...
                case (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL: strType = "화재 신고";
                    break;
                case (int)libSensorProcess.ReactionType.IGNORE_SIGNAL: strType = "화재신호 꺼짐";
                    break;
        ...
        public void cboFireSelect_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            Report.ReactionEarthquakeLog data = (Report.ReactionEarthquakeLog)cboFireSelect.SelectedItem;
        ...
        private void RefreshReactionType(int nReactionType)
        {
            ...
            foreach (Report.ReactionEarthquakeLog log in m_allLogs)
            {
                //화재신고만   
                if (nReactionType == 22)
                {
        ...
        public void Function_cboFireSelect(int nReactionType = 23)
        {
            ...
            //설정한 기간, ZoneList를 ZoneSubmit함수에 넘겨줌
            m_ActionMgr.Submit(dtStart, dtEnd, 2);

            //HistorySubmit를 통해 화재탐지나 수동신고 된 ReactionLog를 HistoryID로 찾아와서 Combobox에 넣음
            ArrayList arrComboData = m_ActionMgr.HistorySubmit(dtStart, dtEnd);

            //cboFireSelect.Items.Clear();
            foreach (Report.ReactionEarthquakeLog log in arrComboData)
            {
                //화재신고만   
                if (nReactionType == 22)
                {
                    if (log.Type == 0 || log.Type == 22 || log.Type == 64)
                        reactionLogBindingSource.Add(log);
        ...
        {
            foreach (Report.ReactionEarthquakeLog log in cboFireSelect.Items)
            {
        ...
        private void SetManualID()
        {
            ...
            m_manualManager.SetID(this, "SDMS_Report_Action_Fire");
            m_manualManager.SetID(label1, "SDMS_Report_Action_Fire");
            m_manualManager.SetID(btnSaveHWP, "Action_Fire_ExportReport");
            m_manualManager.SetID(lblSelectDate, "Action_Fire_Grid");
            m_manualManager.SetID(m_dataGridView, "Action_Fire_Grid");
            m_manualManager.SetID(textBoxMemo, "Action_Fire_Grid");
