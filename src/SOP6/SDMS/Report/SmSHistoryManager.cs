    ...
    internal class SmSHistoryManager
    {
        ...
        public void ZoneSubmit(ArrayList arEquipZoneList, ArrayList arZoneList, DateTime dtStart, DateTime dtEnd, UnE.Sensor.IFacility.FacilityType type)
        {
            ...
            for (int i = 0; i < arResult.Count - 9; i += 10)
            {
                ...
                data.ID = WebDBManager.GetIntField(arResult[i].ToString(), -1);
                data.SensorHistoryID = WebDBManager.GetIntField(arResult[i + 1].ToString(), -1);
                data.ReactionHistoryID = WebDBManager.GetIntField(arResult[i + 2].ToString(), -1);

                data.CompanyMemberList = MakeArrayList(WebDBManager.GetStringField(arResult[i + 3]));
                data.ExteanlMemberList = MakeArrayList(WebDBManager.GetStringField(arResult[i + 4]));
                data.Message = WebDBManager.GetStringField(arResult[i + 5]);

                data.IsAuto = (WebDBManager.GetIntField(arResult[i + 6].ToString(), -1) == 1);
                data.Time = WebDBManager.GetDateTimeField(arResult[i + 7].ToString(), DateTime.Now);
                int nEquipZoneID = WebDBManager.GetIntField(arResult[i + 8].ToString(), -1);
            ...
            arResult = m_DBMgr.GetResultData(strSQL);
            if (arResult == null)
                return;

            for (int i = 0; i < arResult.Count; i++)
            {
                int nSensorZoneHistoryID = WebDBManager.GetIntField(arResult[i].ToString(), -1);
