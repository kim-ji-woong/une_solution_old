...
namespace SDMS
{
    public partial class ActionIntrusionPage : FormReportBase
    {
        ...
        public void Function_DataGrid(int ZoneHistoryID, int nReactionType)
        {
            ...
            {
                //찾은 검색결과를 DataGrid로 출력
                if (!SetGridRows(data, count, nCount, GetReactionString(nReactionType, data.SensorType)))
                    continue;
        ...
        private bool SetGridRows(Report.ReactionIntrusionLog data, int nRow, int nCount, string strReactionType)
		{
			...
			string strType = GetReactionString(ReactionType, data.SensorType);
            ...
            if (ReactionType == (int)libSensorProcess.ReactionType.BEGIN_STATUS)
            {
                ...
                if (detectStr.Length > 0)
                {
                    ...
                    if (int.TryParse(log.Param3, out nSensorType) && nSensorType == log.SensorType)
                        strType = detectStr;
                    else
                    {
                        if (IsS1SVMSType(log.SensorType))
                            strType = "S1SVMS " + detectStr + " 탐지";
                        else if (IsS1AccessType(log.SensorType))
                            strType = "S1ACCESS " + detectStr + " 탐지";
                        else if (IsSecomType(log.SensorType))
            ...
            else if (ReactionType == (int)libSensorProcess.ReactionType.END_STATUS)
            {
                if (log.SensorType == (int)IFacility.FacilityType.ExternalAlarmBell || log.SensorType == (int)IFacility.FacilityType.SecomExternalAlarmBell)
                    strType = "외부비상벨 종료";
                else if (log.SensorType == (int)IFacility.FacilityType.SecomWomenAlarmBell)
                    strType = "여자화장실 비상벨 종료";
            }
            else if (ReactionType == (int)libSensorProcess.ReactionType.IGNORE_SIGNAL)
            {
                if (log.SensorType == (int)IFacility.FacilityType.ExternalAlarmBell || log.SensorType == (int)IFacility.FacilityType.SecomExternalAlarmBell)
                    strType = "외부비상벨 꺼짐";
                else if (log.SensorType == (int)IFacility.FacilityType.SecomWomenAlarmBell)
            ...
            if (strReactionType == "S1ACCESS 꺼짐" && log.SensorType == (int)IFacility.FacilityType.ExternalAlarmBell) label2.Text = "외부비상벨 꺼짐";
            else if (strReactionType == "S1ACCESS 종료" && log.SensorType == (int)IFacility.FacilityType.ExternalAlarmBell) label2.Text = "외부비상벨 종료";
            else if (strReactionType == "SECOM 꺼짐" && log.SensorType == (int)IFacility.FacilityType.SecomExternalAlarmBell) label2.Text = "외부비상벨 꺼짐";
            else if (strReactionType == "SECOM 종료" && log.SensorType == (int)IFacility.FacilityType.SecomExternalAlarmBell) label2.Text = "외부비상벨 종료";
            else if (strReactionType == "SECOM 꺼짐" && log.SensorType == (int)IFacility.FacilityType.SecomWomenAlarmBell) label2.Text = "여자화장실 비상벨 꺼짐";
            else if (strReactionType == "SECOM 종료" && log.SensorType == (int)IFacility.FacilityType.SecomWomenAlarmBell) label2.Text = "여자화장실 비상벨 종료"; 
        ...
        private bool IsS1SVMSType(int nSensorType)
        {
            return nSensorType >= (int)IFacility.FacilityType.Intrusion_S1 && nSensorType <= (int)IFacility.FacilityType.EmergencyBell_S1;
        }

        private bool IsS1AccessType(int nSensorType)
        {
            return nSensorType >= (int)IFacility.FacilityType.GeneralIntrusionT1_S1 && nSensorType <= (int)IFacility.FacilityType.SynthesisAlertAbnormalityU8_S1;
        }

        private bool IsSecomType(int nSensorType)
        {
            return nSensorType >= (int)IFacility.FacilityType.SecomExternalAlarmBell && nSensorType <= (int)IFacility.FacilityType.SecomWomenAlarmBell;
        }
        ...
        public string GetReactionString(int nReactionType, int nSensorType)
        {
            string strType = "";
            ...
            switch (nReactionType)
            {
                ...
                case (int)libSensorProcess.ReactionType.BEGIN_STATUS:
                    if (IsS1SVMSType(nSensorType))
                        strType = "S1SVMS 탐지";
                    else if (IsS1AccessType(nSensorType))
                        strType = "S1ACCESS 탐지";
                    else if (IsSecomType(nSensorType))
                        strType = "SECOM 탐지";
                    else
                        strType = "방범 탐지";
                    break;
                ...
                case (int)libSensorProcess.ReactionType.USER_RESET:
                    strType = "신호복구";
                    break;
                case (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL: strType = "화재 신고";
                    break;
        ...
        private void SetItemsClear()
        {
            ...
            if(nTemp == 0)
            {
                ...
                Function_cboActionIntrusionSelect((int)libSensorProcess.ReactionType.NOTIFY_SIGNAL);
            }
            else if(nTemp == 1)
            {
            ...
            else if(nTemp == 2)
            {
                ...
                Function_cboActionIntrusionSelect((int)libSensorProcess.ReactionType.IGNORE_SIGNAL);
            }
        ...
        public void DetectIntrusionPageDoubleClick(int nSensorZoneHistoryID = 0)
        {
            ...
            {
                nReactionType = (int)libSensorProcess.ReactionType.MALFUNCTION;
                Function_cboActionIntrusionSelect(nReactionType);
            }
            else if (react_cboSearchType.SelectedIndex == 2)
            {
        ...
        private void RefreshReactionType(int nReactionType)
        {
            ...
            foreach (Report.ReactionIntrusionLog log in m_allLogs)
            {
                ...
                {
                    if (log.Type == (int)libSensorProcess.ReactionType.BEGIN_STATUS || log.Type == (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL || log.Type == (int)libSensorProcess.ReactionType.MALFUNCTION || log.Type == (int)libSensorProcess.ReactionType.USER_RESET)
                        reactionLogBindingSource.Add(log);
                }
                else if (nReactionType == (int)libSensorProcess.ReactionType.IGNORE_SIGNAL)
                {
                    if (log.Type == (int)libSensorProcess.ReactionType.BEGIN_STATUS || log.Type == (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL || log.Type == (int)libSensorProcess.ReactionType.MALFUNCTION || log.Type == (int)libSensorProcess.ReactionType.USER_RESET || log.Type == (int)libSensorProcess.ReactionType.IGNORE_SIGNAL)
                        reactionLogBindingSource.Add(log);
                }
            }
        ...
        public void Function_cboActionIntrusionSelect(int nReactionType = (int)libSensorProcess.ReactionType.IGNORE_SIGNAL)
        {
            ...
            DBUtility2.WebDBManager dbMgr = FormMain.Instance.DBManager;
