...
namespace SDMS
{
    public partial class FormReport : Form
    {
        ...
        #region 지진
        private Report.ReactionEarthquakeManager m_detectEarthquakeMgr = new Report.ReactionEarthquakeManager();
        private Report.ReactionEarthquakeManager m_actionEarthquakeMgr = new Report.ReactionEarthquakeManager();

        private DetectEarthquakePage m_detectEarthquakePage = null;
        private ActionEarthquakePage m_ActionEarthquakePage = null;
        #endregion

        #region 온도/습도
        private Report.ReactionTHManager m_detectTHMgr = new Report.ReactionTHManager();
        private Report.ReactionTHManager m_actionTHMgr = new Report.ReactionTHManager();

        private Report.ParetoTHPage m_paretoTHPage = null;
        private Report.DetectTHPage m_detectTHPage = null;
        private Report.ActionTHPage m_actionTHPage = null;

        private bool m_useTH = false;
        #endregion
        ...
        public FormReport()
        {
            ...
            m_useTH = FormMain.Instance.UseTH;
            ...
            {
                m_detectEarthquakePage = new DetectEarthquakePage(m_detectEarthquakeMgr);
                m_ActionEarthquakePage = new ActionEarthquakePage(m_detectEarthquakeMgr);
            }

            if (m_useTH)
            {
                m_detectTHPage = new Report.DetectTHPage(m_detectTHMgr);
                m_paretoTHPage = new Report.ParetoTHPage(UnE.Sensor.IFacility.FacilityType.TEMPERATURE_HUMIDITY, m_detectTHMgr, m_detectTHPage);
                m_actionTHPage = new Report.ActionTHPage(m_actionTHMgr);
            }
            ...
            m_ActionPanel.VisibleChanged += new EventHandler(m_ActionPage.ParentPanel_VisibleChanged);
            m_ActionPage.Show();
            ...
            if (UnE.SOP.ProxySOP.Instance.UsePSM)
            {
                ...
                m_ActionPSMPanel.VisibleChanged += new EventHandler(m_ActionPSMPage.ParentPanel_VisibleChanged);
            ...
            if (UnE.SOP.ProxySOP.Instance.UseIntrusion)
            {
                ...
                m_ActionIntrusionPanel.VisibleChanged += new EventHandler(m_ActionIntrusionPage.ParentPanel_VisibleChanged);
            ...
            if (UnE.SOP.ProxySOP.Instance.UseEarthquake)
            {
                m_detectEarthquakePage.Location = new Point(0, 0);
                m_detectEarthquakePage.TopLevel = false;
                m_detectEarthquakePage.Parent = m_detectEarthquakePanel;
                m_detectEarthquakePage.Dock = DockStyle.Fill;
                m_detectEarthquakePanel.Controls.Add(m_detectEarthquakePage);
                m_detectEarthquakePage.Show();

                m_ActionEarthquakePage.Location = new Point(0, 0);
                m_ActionEarthquakePage.TopLevel = false;
                m_ActionEarthquakePage.Parent = m_NotOperationEarthquakePanel;
                m_ActionEarthquakePage.Dock = DockStyle.Fill;
                m_ActionEarthquakePanel.Controls.Add(m_ActionEarthquakePage);
                m_ActionEarthquakePage.Show();
            }

            if (m_useTH)
            {
                m_paretoTHPage.Location = new Point(0, 0);
                m_paretoTHPage.TopLevel = false;
                m_paretoTHPage.Parent = m_paretoTHPanel;
                m_paretoTHPage.Dock = DockStyle.Fill;
                m_paretoTHPanel.Controls.Add(m_paretoTHPage);
                m_paretoTHPage.Show();

                m_detectTHPage.Location = new Point(0, 0);
                m_detectTHPage.TopLevel = false;
                m_detectTHPage.Parent = m_detectTHPanel;
                m_detectTHPage.Dock = DockStyle.Fill;
                m_detectTHPanel.Controls.Add(m_detectTHPage);
                m_detectTHPage.Show();

                m_actionTHPage.Location = new Point(0, 0);
                m_actionTHPage.TopLevel = false;
                m_actionTHPage.Parent = m_ActionPSMPanel;
                m_actionTHPage.Dock = DockStyle.Fill;
                m_actionTHPanel.Controls.Add(m_actionTHPage);
                m_actionTHPanel.VisibleChanged += new EventHandler(m_actionTHPage.ParentPanel_VisibleChanged);
                m_actionTHPage.Show();
            }

            //SetVisibleHWPExport(); // 대용량 속도 문제로 개선한 Hml 작성 방식으로 변경 후에는 한글 설치 여부에 관계없이 파일을 만들 수 있다.
            SetPanelVisible();
        }
        ...
        private void ShowPanel(ReportMode mode)
        {
            ...
            switch (mode)
            {
                ...
                case ReportMode.DetectEarthquake:
                    m_detectEarthquakePanel.Visible = true;
                    break;
                    
                case ReportMode.ActionEarthquake:
                    m_ActionEarthquakePanel.Visible = true;
                    break;

                case ReportMode.DetectTHAnalyze:
                    m_paretoTHPanel.Visible = true;
                    break;

                case ReportMode.DetectTH:
                    m_detectTHPanel.Visible = true;
                    break;

                case ReportMode.ActionTH:
                    m_actionTHPanel.Visible = true;
                    break;
            }

            //if (selectPanel != null && selectPanel.Controls.Count > 0)
            //{
            //    SDMS.Report.FormReportBase frmReport = selectPanel.Controls[0] as SDMS.Report.FormReportBase;
            //    if (frmReport == null)
            //        return;

            //    frmReport.SetChildCtrlResize(selectPanel.Controls[0], FormMain.Instance.nContentSize.Width, FormMain.Instance.nContentSize.Height);
            //}
        }
        ...
        private void SetPanelVisible()
        {
            ...
            AddPanel(m_detectEarthquakePanel);
            AddPanel(m_NotOperationEarthquakePanel);
            AddPanel(m_ActionEarthquakePanel);
            AddPanel(m_paretoTHPanel);
            AddPanel(m_detectTHPanel);
            AddPanel(m_actionTHPanel);
        ...
        public void LoadReport(string strGroupName, string strBuildingName, string strFloorName, DateTime startDate, DateTime EndDate, int nSplitUnitOfMeansure, int nSplitUnitOfMeansureDetail, int nViewCount)
        {
            ...
            else if (m_nReportPage == ReportMode.DetectFire)
            {
                if (m_DetectMgr.NeedRefresh(arrSelectZoneList, startDate, EndDate, m_DetectPage.RefreshCheckData))
                {
                    ...
                    m_DetectPage.CreateBarChart(startDate, EndDate, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail);
                }
        ...
        public void LoadReportForDetectPSM(int[] nBuildingIDs, DateTime startDate, DateTime EndDate, int nSplitUnitOfMeansure, int nSplitUnitOfMeansureDetail, int nViewCount, string strLocationName)
        {
            ...
            if (m_DetectPSMMgr.NeedRefresh(arrSelectZoneList, startDate, EndDate, m_DetectPSMPage.RefreshCheckData))
            {
                ...
                //그래프그리기
                m_DetectPSMPage.CreateBarChart(startDate, EndDate, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail);
            }
        ...
        public void LoadReportForEarthquake(DateTime startDate, DateTime EndDate, int nSplitUnitOfMeansure, int nSplitUnitOfMeansureDetail, int nViewCount)
        {
            m_dtStartDate = startDate;
            m_dtEndDate = EndDate;
            m_nSplitUnitOfMeansure = nSplitUnitOfMeansure;
            ...
            if (m_nReportPage == ReportMode.DetectEarthquake)
            {
                if (m_detectEarthquakeMgr.NeedRefresh(startDate, EndDate, m_detectEarthquakePage.RefreshCheckData))
                {
                    m_detectEarthquakePage.RefreshCheckData.ViewCount = nViewCount;

                    m_detectEarthquakeMgr.DataClear();
                    m_detectEarthquakeMgr.Submit(startDate, EndDate);

                    m_detectEarthquakePage.Load_DataGrid(null);
                    //m_detectEarthquakePage.Load_DataGrid(m_paretoEarthquakePage.SensorHistories);
                    m_detectEarthquakePage.CreateBarChart(startDate, EndDate, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail, nViewCount);
                }
                else if (m_detectEarthquakePage.RefreshCheckData.ViewCount != nViewCount)
                {
                    // 다른 조건은 모두 동일한데 Graph의 최대 표기 개수만 달라진 경우
                    m_detectEarthquakePage.RefreshCheckData.ViewCount = nViewCount;
                    m_detectEarthquakePage.CreateBarChart(startDate, EndDate, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail, nViewCount);
        ...
        public void LoadReportForTH(string strGroupName, string strBuildingName, string strFloorName, DateTime startDate, DateTime EndDate, int nSplitUnitOfMeansure, int nSplitUnitOfMeansureDetail, int nViewCount)
        {
            m_dtStartDate = startDate;
            m_dtEndDate = EndDate;
            m_nSplitUnitOfMeansure = nSplitUnitOfMeansure;
            ...
            if (m_nReportPage == ReportMode.DetectTHAnalyze)
            {
                m_paretoTHPage.ComboSubmit(strGroupName, strBuildingName, strFloorName);
                m_paretoTHPage.Load_DataGrid(arrSelectZoneList, strGroupName, strBuildingName, strFloorName, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail, nViewCount);
            }
            else if (m_nReportPage == ReportMode.DetectTH)
            {
                if (m_detectTHMgr.NeedRefresh(arrSelectZoneList, startDate, EndDate, m_detectTHPage.RefreshCheckData))
                {
                    m_detectTHPage.RefreshCheckData.ViewCount = nViewCount;

                    m_detectTHMgr.DataClear();
                    m_detectTHMgr.ZoneSubmit(arrSelectZoneList, startDate, EndDate);

                    m_detectTHPage.ComboSubmit(strGroupName, strBuildingName, strFloorName);
                    m_detectTHPage.Load_DataGrid(m_paretoPage.SensorHistories, m_paretoPage.EquipZoneHistories);
                    m_detectTHPage.CreateBarChart(startDate, EndDate, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail);
                }
                else if (m_detectTHPage.RefreshCheckData.ViewCount != nViewCount)
                {
                    // 다른 조건은 모두 동일한데 Graph의 최대 표기 개수만 달라진 경우
                    m_detectTHPage.RefreshCheckData.ViewCount = nViewCount;
                    m_detectTHPage.CreateBarChart(startDate, EndDate, nSplitUnitOfMeansure, nSplitUnitOfMeansureDetail);
            ...
            else if (m_nReportPage == ReportMode.ActionTH)
            {
                m_actionTHPage.SetLabelString(strGroupName + "  " + strBuildingName + "  " + strFloorName);
            }
        }
        ...
        public void ShowDetectEarthquakeReport()
        {
            ShowPanel(ReportMode.DetectEarthquake);
        }
        
        public void ShowActionEarthquakeReport()
        {
            ShowPanel(ReportMode.ActionEarthquake);
        }

        public void ShowDetectTHAnalyze()
        {
            ShowPanel(ReportMode.DetectTHAnalyze);
        }

        public void ShowDetectTHReport()
        {
            ShowPanel(ReportMode.DetectTH);
        }

        public void ShowActionTHReport()
        {
            ShowPanel(ReportMode.ActionTH);
        }
        ...
        private string GetStringDisaster(ReportMode mode)
        {
            ...
            if (mode == ReportMode.DetectFireAnalyze || mode == ReportMode.DetectFire || mode == ReportMode.ProcessFire || mode == ReportMode.ActionFire)
                ...
                strDisaster = "방범";
            else if (mode == ReportMode.DetectEarthquake || mode == ReportMode.ActionEarthquake)
                strDisaster = "지진";
            else if (mode == ReportMode.DetectTHAnalyze || mode == ReportMode.DetectTH || mode == ReportMode.ActionTH)
                strDisaster = "온도/습도";
        ...
        public bool SaveHWPForDetectAndNotPoeration()
        {
            ...
            if (m_nReportPage == ReportMode.DetectFire || m_nReportPage == ReportMode.DetectTH) //탐지
            {
                if (m_nReportPage == ReportMode.DetectFire)
                    strSavePath = GetHWPFilePath("화재_탐지이력_보고서", isHwpSetup);
                else if (m_nReportPage == ReportMode.DetectTH)
                    strSavePath = GetHWPFilePath("온도,습도_탐지이력_보고서", isHwpSetup);
                ...
                //if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    {
                        m_detectTHPage.ControllCapture();
                        m_detectTHPage.FileWriter();
                        m_detectTHPage.SetHwpData();
                    }
        ...
        public bool SaveHWPForAction()
        {
            ...
            else if (m_nReportPage == ReportMode.ActionEarthquake)
                strSavePath = GetHWPFilePath("지진_대응이력_보고서", isHwpSetup);
            else if (m_nReportPage == ReportMode.ActionTH)
                strSavePath = GetHWPFilePath("온도,습도_대응이력_보고서", isHwpSetup);
            ...
            //if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    m_ActionEarthquakePage.FileWriter();
                    m_ActionEarthquakePage.SetHwpData();
                }
                else if (m_nReportPage == ReportMode.ActionTH)
                {
                    m_actionTHPage.FileWriter();
                    m_actionTHPage.SetHwpData();
                }
        ...
        public bool SaveHWPForDetectEarthquake()
        {
            bool isHwpSetup = false;
            isHwpSetup = m_detectEarthquakePage.HwpCtrl.GetRegistry(ref m_strHWPPath);
            ...
            string strSavePath = GetHWPFilePath("지진_탐지이력_보고서", isHwpSetup);
            ...
            //화면캡쳐
            m_detectEarthquakePage.ControllCapture();
            m_detectEarthquakePage.FileWriter();
            m_detectEarthquakePage.SetHwpData();
        ...
        public bool SaveHWPForActionEarthquake()
        {
            bool isHwpSetup = false;
            isHwpSetup = m_ActionEarthquakePage.HwpCtrl.GetRegistry(ref m_strHWPPath);
            
            string strSavePath = GetHWPFilePath("지진_대응이력_보고서", isHwpSetup);
        ...
        public bool SaveHWPForDetectAndNotPoerationIntrusion()
        {
            ...
            if (m_nReportPage == ReportMode.DetectFire) //탐지
            {
                ...
                //if (saveFileDialog1.ShowDialog() == DialogResult.OK)
                {
                    ...
                    System.Diagnostics.ProcessStartInfo info = new System.Diagnostics.ProcessStartInfo();
                    info.Arguments = ((int)ReportMode.DetectFire).ToString() + " " + strSavePath + " " + m_strLogoFileName + " " + UnE.SOP.ProxySOP.Instance.SiteID;
                    info.CreateNoWindow = true;
                    info.FileName = Application.StartupPath + "\\HwpReport.exe";
        ...
        public void SelectActionEarthquakePage(int nSensorZoneHistoryID)
        {
            m_ActionEarthquakePage.SelectHistory(nSensorZoneHistoryID);
        }

        public void SelectActionTHePage(int nSensorZoneHistoryID)
        {
            m_actionTHPage.SelectHistory(nSensorZoneHistoryID);
        }
