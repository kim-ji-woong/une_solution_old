...
namespace SDMS
{
    public partial class DetectPage : FormReportBase
    {
        private const int NO_INDEX = 0;
        private const int TIME_STAMP_INDEX = 1;
        private const int SENSOR_TYPE_INDEX = 2;
        private const int SENSOR_NAME_INDEX = 3;
        private const int BUILDING_GROUP_INDEX = 4;
        private const int BUILDING_INDEX = 5;
        private const int FLOOR_INDEX = 6;
        private const int LOCATION_INDEX = 7;
        private const int MEMO_INDEX = 8;
        private const int STATUS_INDEX = 9;
        ...
        private Report.ReactionManager.RefreshCheckData m_checkData = new ReactionManager.RefreshCheckData();

        public Report.ReactionManager.RefreshCheckData RefreshCheckData
        {
        ...
        public void CreateLineChart(DateTime StartDate, DateTime EndDate, int nSplitUnitOfMeansure, int nViewCount, bool isLoad = false)
        {
            WebDBManager m_dbMgr = FormMain.Instance.DBManager;
            ...
            if (isToday == true)
            {
                ...
                if (totalToday < 1)
                {
                    nXCount = DateTime.Now.Hour;
                }
                // 금일 포함 2일 지정
                else if (totalToday == 1)
                {
                    if (DateTime.Now.Hour < 16)
                        nXCount = 24 + DateTime.Now.Hour;
                }
            }

            //날짜 차이가 40일 아래 일 경우 시간 단위로 출력
            if (totalToday <= 39)
            {
                string strDateFormatString = "{0:00}-{1:00} {2:00}시";

                if (nXCount == -1)
                    nXCount = 40;


                arrDays.Add(String.Format(strDateFormatString, dtBeforeDate.Month, dtBeforeDate.Day, 0));
                y_arr.Add(0);
                ...
                for (int k = 1; k < nXCount + 1; k++)
                {
                    int nCount = 0;

                    //구한 Time
                    int nAddTime = (totalTime * k) / nXCount;
                    int nRest = (totalTime * k) % nXCount;
                    ...
                    if (k == nXCount && isToday == true)
                    {
                        dtNextDate = DateTime.Now;
                        arrDays.Add(String.Format(strDateFormatString, dtNextDate.Month, dtNextDate.Day, dtNextDate.Hour));
                    }
                    else
                    {
                        arrDays.Add(String.Format(strDateFormatString, dtNextDate.Month, dtNextDate.Day, dtNextDate.Hour));
                ...
                labels = new string[nXCount + 1];
                data = new double[nXCount + 1];
                int n_count = 0;
        ...
        */
        ...
        {  
            ...
            XYChart c = new XYChart(sizeGrid.Width, (int)(cHeight * sizePer));
            ...
            if (m_nCurrentPage > 0)
            {
                ...
                {
                    liDatas.Add((from logs in m_detectMgr.DectectList.ToArray().Cast<Report.DetectLog>()
            ...
            c.yAxis().setTitle("화재 탐지 횟수", Program.prgFont, 22.50 * sizePer);
            c.yAxis().setLabelStyle(Program.prgFont, 21.5 * sizePer, 0x000000);

            c.xAxis().setLabels(m_nCurrentPage < 0 ? null : dicLabels[m_nCurrentPage]);
            c.xAxis().setLabelStyle(Program.prgFont, 21.5 * sizePer, 0x000000, dbAngle);

            c.setPlotArea(ptGrid.X + nSpace, (int)(30 * sizePer), sizeGrid.Width - nSpace * 2, (dbAngle > 0) ? (int)(390 * sizePer) + nLabelLength : (int)(470 * sizePer));
            ...
            layer.setAggregateLabelFormat("{value}회");
            layer.setAggregateLabelStyle(Program.prgFont, 22.50 * sizePer, 0x000000, 0).setAlignment(Chart.Center);

            layer.setBarWidth((int)(80 * sizePer)); // Bar의 두께 설정
        ...
        */
        ...
        private void SetupDataGrid()
        {
            ...
            dataGridView1.Columns[SENSOR_TYPE_INDEX].Width = (int)(180 * sizePer);
            dataGridView1.Columns[SENSOR_NAME_INDEX].Width = (int)(160 * sizePer);
            dataGridView1.Columns[BUILDING_GROUP_INDEX].Width = (int)(160 * sizePer);
            dataGridView1.Columns[BUILDING_INDEX].Width = (int)(220 * sizePer);
            dataGridView1.Columns[FLOOR_INDEX].Width = (int)(100 * sizePer);
            dataGridView1.Columns[LOCATION_INDEX].Width = (int)(260 * sizePer);
            dataGridView1.Columns[MEMO_INDEX].Width = (int)(240 * sizePer);
            dataGridView1.Columns[STATUS_INDEX].Width = (int)(200 * sizePer);

            //dataGridView1.Columns[NO_INDEX].Width = 90;
            //dataGridView1.Columns[TIME_STAMP_INDEX].Width = 200;
            //dataGridView1.Columns[SENSOR_TYPE_INDEX].Width = 90;
            //dataGridView1.Columns[SENSOR_NAME_INDEX].Width = 80;
            //dataGridView1.Columns[BUILDING_GROUP_INDEX].Width = 80;
            //dataGridView1.Columns[BUILDING_INDEX].Width = 110;
            //dataGridView1.Columns[FLOOR_INDEX].Width = 50;
            //dataGridView1.Columns[LOCATION_INDEX].Width = 130;
            //dataGridView1.Columns[MEMO_INDEX].Width = 120;
            //dataGridView1.Columns[STATUS_INDEX].Width = 100;
            ...
            column.DefaultCellStyle.Padding = new System.Windows.Forms.Padding(5, 0, 0, 0);
            column.SortMode = DataGridViewColumnSortMode.NotSortable;
            column.ReadOnly = true;
            dataGridView1.Columns.Add(column);
            ...
            column2.Items.AddRange(new object[] { "실제", "오동작", "테스트" });
        ...
        private void dataGridView1_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Report.DetectLog detectLog = m_dicDetectLog[Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[0].Value)];
        ...
        public void Load_DataGrid(Dictionary<SensorTagInfo, int> dicSensorHistories, Dictionary<UnE.Spatial.EquipmentZone, int> dicEquipZoneHistories)
        {
            ...
            if (m_dicDetectLog == null) m_dicDetectLog = new Dictionary<int, DetectLog>();
                                                     ...
                                                     )
            {
                ...
                // Data Binding
                DetectPageGridData data = new DetectPageGridData();
                data.No = ++count;
                data.TimeStamp = historyData.Time;
                data.SensorType = strType;
                data.SensorName = strSensorName;
                data.BuildingGroup = szGroupName;
                data.Building = szBuildingName;
        ...
        private void SaveHwpCrtl()
        {
            ...
            for (int index = 0; index < dataGridView1.RowCount; index++)
            {
                ...
                SaveArr.Add(row.Cells[BUILDING_INDEX].Value.ToString());
                SaveArr.Add(row.Cells[FLOOR_INDEX].Value.ToString());
        ...
        private void comboBox_Leave(object sender, EventArgs e)
        {
            ...
            Report.DetectLog detectLog = m_dicDetectLog[Convert.ToInt32(dataGridView1.Rows[cell.RowIndex].Cells[0].Value)];
        ...
        {
            Report.DetectLog log = (Report.DetectLog)popupMenu.Tag;
        ...
        private void dataGridView1_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (e.Button == System.Windows.Forms.MouseButtons.Left)
            {
                if (e.ColumnIndex >= 0 && e.ColumnIndex < STATUS_INDEX && e.RowIndex >= 0)
                {
                    ...
                    if (m_dicDetectLog.TryGetValue(nLogIndex, out detectLog))
                    { 
                        FormMain.Instance.SelectActionPage(detectLog.HistoryID);
                        int nSpace3 = 26;
                        if (FormMain.Instance.Resolution == Resolution.FullHD)
                        {
                        ...
                        FormMain.Instance.FromDetectPageToActionPage();
                    }
                }
            }
        }
        ...
        private void SetManualID()
        {
            ...
            m_manualManager.SetID(this, "SDMS_Report_Detect_Fire");
            m_manualManager.SetID(label2, "SDMS_Report_Detect_Fire");
            m_manualManager.SetID(btnSaveHWP, "Detect_Fire_ExportReport");
            m_manualManager.SetID(panelChart, "Detect_Fire_Graph");
            m_manualManager.SetID(btnPreviousIndex, "Detect_Fire_Graph");
            m_manualManager.SetID(btnNextIndex, "Detect_Fire_Graph");
            m_manualManager.SetID(lblTotalPage, "Detect_Fire_Graph");
            m_manualManager.SetID(dataGridView1, "Detect_Fire_Grid");
