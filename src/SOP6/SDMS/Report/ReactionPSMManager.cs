    ...
    namespace Report
    {
        public class ReactionPSMManager
        {
            ...
            private ArrayList GetDetectLog(ArrayList arrAllLog)
            {
                ...
                foreach (SensorReactionPSMLog reactionLog in arrAllLog)
                {
                        ...
                        (reactionLog.ReactionType == (int)ReactionType.NOTIFY_SIGNAL && reactionLog.Param2 == "0"))
                    {
                        ...
                        {
                            case (int)ReactionType.BEGIN_STATUS:
                            //case (int)ReactionType.BEGIN_PSM_STATUS:
                            case (int)ReactionType.IGNORE_SIGNAL:
                            case (int)ReactionType.NOTIFY_SIGNAL:
                            case (int)ReactionType.CHANGE_ALARM_DEPTH:
                            case (int)ReactionType.END_STATUS:
                ...
                foreach (Zone zone in arrZoneList)
                {
                    ...
                    foreach (EquipmentZone equipZone in ZoneManager.Instance.GetEquipmentZoneList(zone))
                    {
                        ...
                        foreach (int nHistoryID in arrHistoryList)
                        {
                            ...
                            int nType = (int)ReactionType.BEGIN_STATUS;
                            ...
                            foreach (SensorReactionPSMLog log in arrLog)
                            {
                                ...
                                else if (log.ReactionType == (int)ReactionType.MALFUNCTION || log.ReactionType == (int)ReactionType.USER_RESET)
                                {
                                    nMulFunctionCount++;
                                    nType = (int)ReactionType.USER_RESET;

                                    break;
                                }
                                else if (log.ReactionType == (int)ReactionType.IGNORE_SIGNAL || log.ReactionType == (int)ReactionType.END_STATUS)
                                {
            ...
            private ArrayList GetManualReactionHistory(ArrayList arrZoneList, string startDate, string endDate)
            {
                ...
                int nSensorZoneID = SOPWebServer.Header.ManualReportDefaultID + (int)IFacility.FacilityType.PSM_SENSOR;
                ...
                strSQL += "(select SensorHistoryID from SensorReactionHistory where param1 in(" + strZoneList + ") And ReactionType = " + (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL + " And Param2 = " + nSensorZoneID + " And Time Between '" + startDate + "' and '" + endDate + "')";
                ...
                for (int i = 0; i < nResultCount - 11; i += 12)
                {
                    ...
                    if (nReactionType == (int)ReactionType.IGNORE_SIGNAL || nReactionType == (int)ReactionType.NOTIFY_SIGNAL || nReactionType == (int)ReactionType.USER_RESET || nReactionType == (int)ReactionType.MALFUNCTION)
                    {
            ...
            private ArrayList GetReactionHistory(ArrayList arrSensorHistoryID)
            {
                ...
                strSQL += "and (szh.SensorID = sz.ID and sz.Type = " + (int)IFacility.FacilityType.PSM_SENSOR + ") ";
            ...
            private ArrayList GetSensorZoneHistoryID(Dictionary<int, ISensor> dicSensorZones, string startDate, string endDate)
            {
                ...
                string strSQL = string.Format("select id,SensorID from SensorZoneHistory where Time Between '" + startDate + "' and '" + endDate + "' and ( Data IN ({0}, {1}, {2}))",
                    (int)libSensorProcess.ReactionType.MALFUNCTION, (int)libSensorProcess.ReactionType.NOTIFY_SIGNAL, (int)libSensorProcess.ReactionType.IGNORE_SIGNAL);
        ...
        public class ReactionPSMLog : IComparable
        {
            ...
            public override string ToString()
            {
                ...
                else if (nReactionType == (int)ReactionType.MALFUNCTION || nReactionType == (int)ReactionType.USER_RESET)
                    strReactionType = "시스템 복구 처리";
                else if (nReactionType == (int)ReactionType.IGNORE_SIGNAL || nReactionType == (int)ReactionType.END_STATUS)
