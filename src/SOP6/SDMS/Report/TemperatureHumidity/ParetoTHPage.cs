...
{
    public partial class ParetoTHPage : FormReportBase, SDMS.Data.IParetoPage
    {
        ...
        private const string SensorChartTag = "탐지횟수";
        ...
        private DetectTHPage m_detectPage = null;
        ...
        public ParetoTHPage(UnE.Sensor.IFacility.FacilityType type, Report.ReactionTHManager detectMgr, DetectTHPage detectPage)
        {
        ...
        private void SetSize()
        {
            ...
            foreach (DataGridViewTextBoxColumn col in dataGridViewSensor.Columns)
            {
                if (col.Name == "colNo")
                    ...
                    col.Width = (int)(580 * sizePer);
                else if (col.Name == "colFloor")
                    col.Width = (int)(200 * sizePer);
                else if (col.Name == "colPercent")
                    col.Width = (int)(280 * sizePer);
        ...
        private bool GetDataFromDetectPage(ArrayList arrSelectZoneList, string strGroupName, string strBuildingName, string strFloorName, int nSplitUnitOfMeasure, int nSplitUnitOfMeasureDetail, DateTime startDate, DateTime endDate, int nViewCount)
        {
            ...
            if (m_detectMgr.NeedRefresh(arrSelectZoneList, startDate, endDate, m_detectPage.RefreshCheckData))
            {
                ...
                m_detectPage.CreateBarChart(startDate, endDate, nSplitUnitOfMeasure, nSplitUnitOfMeasureDetail);
            }
        ...
        private void RefreshSensorGrid(out double[] values, out string[] labels)
        {
            ...
            for (int i = nHistoryCount - 1, j = 0; i >= 0; i--, j++)
            {
                ...
                row.Cells[(int)SensorGrid.NO_INDEX].Value = row.Index + 1;
                row.Cells[(int)SensorGrid.SENSOR_NAME_INDEX].Value = history.Sensor.TagName;
                row.Cells[(int)SensorGrid.FLOOR_INDEX].Value = GetFloorName(history);
                row.Cells[(int)SensorGrid.LOCATION_INDEX].Value = GetLocationName(history);
                row.Cells[(int)SensorGrid.HISTORY_COUNT_INDEX].Value = history.HistoryCount;
                row.Cells[(int)SensorGrid.PERCENT_INDEX].Value = GetHistoryPercent(nTotalHistoryCount, history.HistoryCount);
        ...
        private void EmptyChart(WinChartViewer chartViewer, DataGridView grid, string strChartTag)
        {
            ...
            // Add a title to the primary y-axis
            //c.yAxis().setTitle("Frequency", Program.prgFont, 22.50 * sizePer, 0xffffff);
        ...
        private void btnPreviousIndex_Click(object sender, EventArgs e)
        {
            ...
            PageData pageData = m_pageSensor;
        ...
        private void ParetoPageTH_Load(object sender, EventArgs e)
        {
        ...
        private void ParetoPageTH_Resize(object sender, EventArgs e)
        {
            if (Width == 0 || Height == 0)
                return;
        ...
        private void SaveHwpCrtl(DataGridView grid, List<string> lines, string strTag)
        {
            ...
            for (int i = 0; i < nRowCount; i++)
            {
                ...
                {                    
                    ...
                    if (UnE.SOP.ProxySOP.Instance.SiteID == 200) // 신한은행은 빌딩버전이기때문에 건물에 들어갈 컬럼을 채워줘야함
                    {
                        if (j == 2)
                            lines.Add("데이터센터"); 
                    }
        ...
        {
            return "온도,습도_탐지분석_보고서";
        }

        public SDMS.Data.ReportMode GetReportMode()
        {
            return Data.ReportMode.DetectTHAnalyze;
        }
        ...
        private void SetManualID()
        {
            ...
            m_manualManager.SetID(this, "SDMS_Report_Pareto_TH");
            m_manualManager.SetID(lblTitle, "SDMS_Report_Pareto_TH");
            m_manualManager.SetID(btnSaveHWP, "Pareto_TH_ExportReport");
            m_manualManager.SetID(winChartViewerSensor, "Pareto_TH_Graph");
            m_manualManager.SetID(lblTotalPageSensor, "Pareto_TH_Graph");
            m_manualManager.SetID(dataGridViewSensor, "Pareto_TH_Grid");
