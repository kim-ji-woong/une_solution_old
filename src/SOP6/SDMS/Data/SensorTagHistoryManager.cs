...
namespace SDMS
{
    public class SensorTagHistoryManager
    {
        ...
        // Key : 상위 4바이트(SensorServer ID), 하위 4바이트(TagID)
        private Dictionary<long, SensorTagInfo> m_dicTHSensorTags = new Dictionary<long, SensorTagInfo>();
        // 각 센서들의 마지막 상태값
        // Key : 상위 4바이트(SensorServer ID), 하위 4바이트(TagID)
        ...
        private List<SensorTagHistory> m_sensorTHHistories = new List<SensorTagHistory>();
        ...
        private Dictionary<long, SensorTagInfo> GetSensorTagDictionary(UnE.Sensor.IFacility.FacilityType type)
        {
            if (type == UnE.Sensor.IFacility.FacilityType.FIRE_SENSOR ||
                ...
                type == UnE.Sensor.IFacility.FacilityType.Fire_S1 || type == UnE.Sensor.IFacility.FacilityType.FireF1_S1 || type == UnE.Sensor.IFacility.FacilityType.SecomFire)

            {
                ...
                || type == UnE.Sensor.IFacility.FacilityType.SecomExternalAlarmBell || type == UnE.Sensor.IFacility.FacilityType.SecomWomenAlarmBell
            ...
            else if (type == IFacility.FacilityType.TEMPERATURE_HUMIDITY)
                return m_dicTHSensorTags;
        ...
        public void ProcessDeleteSensorTagHistory(DateTime dtLimit)
        {
            ...
            int nIndexTH = FindSensorHistoryIndex(ref dtLimit, true, m_sensorTHHistories, UnE.Sensor.IFacility.FacilityType.TEMPERATURE_HUMIDITY);
            ...
            lock (m_lockObject)
            {
                ...
                for (int i = nIndexTH - 1; i >= 0; i--)
                {
                    m_sensorTHHistories.RemoveAt(i);
                }
            }
        }
        ...
        private bool LoadSensorTagHistories(WebDBManager dbMgr, int nHistoryType, ref int nLastReadHistoryID)
        {
            ...
            if (nHistoryType == (int)UnE.Sensor.IFacility.FacilityType.Intrusion_S1)
            {
                string strTypeIDs = (int)IFacility.FacilityType.Intrusion_S1 + "," + (int)IFacility.FacilityType.Loiter_S1 + ","
                    ...
                    + (int)IFacility.FacilityType.SecomExternalAlarmBell + "," + (int)IFacility.FacilityType.SecomWomenAlarmBell;
            ...
            else if (nHistoryType == (int)UnE.Sensor.IFacility.FacilityType.FIRE_SENSOR)
            {
                ...
                strTypeIDs += ", " + ((int)UnE.Sensor.IFacility.FacilityType.SecomFire).ToString();
                strSQL = "Select sth.ID, st.SensorServerID, st.TagNo, sth.TimeStamp, sth.value, sth.HistoryType from SensorTagHistory as sth, SensorTagInfo as st ";
        ...
        private int FindHistoryIndex(ref DateTime timeStamp, bool isBeign, int nIndex, int nRangeBegin, int nRangeEnd, List<SensorTagHistory> sensorTagHistories, UnE.Sensor.IFacility.FacilityType type)
        {
            ...
            else if (type == UnE.Sensor.IFacility.FacilityType.TEMPERATURE_HUMIDITY)
                prevHistories = m_sensorTHHistories;
            else
                return -1;
