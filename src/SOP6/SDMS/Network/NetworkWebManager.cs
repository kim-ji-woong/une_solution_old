...
namespace SDMS
{
    public class NetworkWebManager : IPostMan
    {
        ...
        private bool m_shutdownThread2 = false;
        ...
        public static NetworkWebManager Instance
        {
            ...
            {
                if (m_manager == null)
                    m_manager = new SDMS.NetworkWebManager();
                return m_manager;
            }
        }
        ...
        public NetworkWebManager()
        {
            ...
            t = new Thread(new ThreadStart(ProcessMessageQueue));
            t.Name = "SDMS.ProcessQueue";
            t.Start();
        }

        private void GetServerIP()
        {
            string strServerURL = RegUtil.ReadRegValue("Server Connection Info", "webserver_url2", m_dbMgr.SiteID);
            if (strServerURL == null || strServerURL == "")
                strServerURL = FormMain.Instance.DBManager.WebServerURL;

            string strURL = strServerURL.Replace("http://", "");
        ...
        private System.Collections.Concurrent.ConcurrentQueue<MessageQueue> m_queues = new System.Collections.Concurrent.ConcurrentQueue<MessageQueue>();
        ...
        private void ProcessMessageQueue()
        {
            while (!m_shutdownThread2)
            {
                ...
                MessageQueue queue;
                if (m_queues.TryDequeue(out queue) == false)
                    continue;

                int header = queue.nHeader;
                byte[] messages = queue.Bytes;
                ...
                Trace.WriteLine("Process Queue Header : " + header);
        ...
        public void OnMessage(int header, byte[] messages)
        {
            ...
            queue.nHeader = header;
            queue.Bytes = messages;

            m_queues.Enqueue(queue);

            Trace.WriteLine("Register Queue Header : " + header);
        ...
        public void SendChangeEquipZoneCCTV(int nEquipZoneID)
        {
            ...
            SendChangedConfig(SOPWebServer.ClientType.SDMS, SOP.SDMSConfig.GetPropertyName(SOP.SDMSConfig.ConfigType.EQUIPZONE_CCTV), nEquipZoneID.ToString());
        }
        ...
        public void SendUpdateFacilityZone(ISensor sz, int nOrgEquipZoneID)
        {
            ...
            foreach (SensorZoneUpdateData data in arDatas)
            {
            ...
            byte[] bytes = SOPWebServer.BinaryHelper.MakeBytes(arrDatas);
            SendMessage(SOPWebServer.Header.EDIT_SENSOR_ZONE, bytes);            
        }
        ...
        public void SendRequestReactionLogList()
        {
            ...
            SendData(SOPWebServer.Header.REQUEST_SENSOR_REACTION_HISTORY_DATA_LIST);
        }
        ...
        public bool SendRequestRestore()
        {
            ...
            lock (this)
            {
                SendData(SOPWebServer.Header.REQUEST_RESTORE);
            }
        ...
        public void SendPSMSensorStatus(int nSensorID, byte status, long beginWorkTime, long endWorkTime)
        {
            ...
            byte[] bytes = SOPWebServer.BinaryHelper.MakeBytes(arrDatas);
            SendMessage(SOPWebServer.Header.SDMS_COMMAND, bytes);
        }
        ...
        changeAction historyAction = null;
        ...
        public void SendTagChangeDeactivationInfo(Dictionary<int, string> dicCangedData, System.Action callback)
        {
            ...
            historyAction = new changeAction(callback);
            Change += historyAction;
