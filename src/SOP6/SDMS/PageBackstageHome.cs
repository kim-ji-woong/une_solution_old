...
namespace SDMS
{
    public partial class PageBackstageHome : Form, IChangedDataManager, IBaseViewOwner, IFormContentOwner
	{
        ...
        // Translucent를 사용한 팝업창이 너무 크게 나타나는 오류를 확인하기 위한 임시 로그
        private static System.IO.StreamWriter m_writer = new System.IO.StreamWriter(".\\TranslucentForm.log", false, System.Text.Encoding.UTF8);

        public static void WriteTempLog(string strLog)
        {
		...
		public static void ShowTranslucentForm(Form targetForm, int x, int y, int width, int height, int nCommandID)
		{
            ...
            WriteTempLog(string.Format("PageBackstageHome.ShowTranslucentForm, Form Name : {0}, Location : ({1}, {2}), Size : ({3}, {4})", targetForm.Name, targetForm.Location.X, targetForm.Location.Y, targetForm.Size.Width, targetForm.Size.Height));
        ...
        public Dictionary<ContentOwnerTab, ContentOwnerTabRightDockingMode> DicTabDockingMode
        {
            get { return m_dicTabDockingMode; }
            set { m_dicTabDockingMode = value; }
        }
        ...
        private ContentOwnerTabRightDockingMode ShowDockingPane(ContentOwnerTab tab)
        { 
            ...
            if (m_dicTabDockingMode.TryGetValue(tab, out mode))
            {
                ...
                //}
                //else 
                if (mode == ContentOwnerTabRightDockingMode.SHOW_PSM)
                {
		...
		private void PageBackstageHome_Load(object sender, EventArgs e)
		{
            ...
            m_dockFormLocation.SetSplitDistance(DockingFormLocation.Mode.CCTV, PopupDialog.FormCCTVList.nOrgFrmWidth);
        ...
        private IFormContent CreateContentFormBySiteID(int nSiteID)
        {
            ...
            {
                UnE.View.Content.FormContentUnity form = new UnE.View.Content.FormContentUnity(this);
            ...
            else if (nSiteID == 999 || nSiteID == 102)
            {
                FormContent2DOnly form = new FormContent2DOnly(this);
		...
		public void OnChangeLayer(int nID)
		{
			switch (nID)
			{
				...
				case ID.ID_LAYER_CCTVLOW:
                    libSensorProcess.FireDetectProcess.ShowFireDetectTooltipCCTV = !libSensorProcess.FireDetectProcess.ShowFireDetectTooltipCCTV;
        ...
        {
            ContentOwnerTabRightDockingMode mode = ShowDockingPane(mCurrentTab);

            ShowLayer(ID.ID_LAYER_BUILDING_TEXT, m_bShowBuildingTextLayer);

            FormMain.Instance.OnReadyDataLoad();

            FormMain.Instance.LoadActiveFormInDockingMode(mode);
        }
		...
		public void OnChangeLayer(int nID, bool visible)
		{
			switch (nID)
			{
				...
				case ID.ID_LAYER_CCTVLOW:
                    libSensorProcess.FireDetectProcess.ShowFireDetectTooltipCCTV = visible;
        ...
        public void OnClickToolBarButton(ImageButton btn)
		{
			...
			switch (nID)
			{
                ...
                case ID.ID_VIEW_CAMPUS:
                    m_ContentForm.ChangeCampus();
                    break;
				default:
					break;
			};
        ...
        public void ShowSituationCCTV(bool bSituation, ArrayList arrCCTVs)
        {
            if (UnE.SOP.ProxySOP.Instance.ShowCCTVForm == false)
            {
                // 내부CCTV는 사용하지 않음 comment by skkim 2016-10-14
            ...
            else
            {
                ...
                if (bSituation == true)
                {
                    ...
                    if(FormMain.Instance.CurrentSensorDetectProcess != null)
                    {
                        ...
                        {
                            // CCTV 개수가 5개 이상이면 실내 이미지는 표시하지 않는다.
                            if (arrCCTVs.Count <= 4)
                            {
                                string szPath2 = "";
        ...
        public ArrayList DownloadPSMImage(int nSensorID)
        {
            ...
            ArrayList arResult = FormMain.Instance.DBManager.GetResultData(szSQL);
            ...
            string strImgTitle1 = WebDBManager.GetStringField(arResult[3], "");
            string strImgTitle2 = WebDBManager.GetStringField(arResult[4], "");
            string strImgTitle3 = WebDBManager.GetStringField(arResult[5], "");
           ...
           for( int i = 0; i < 3 ; i++)
           {
                string strImageFileName = WebDBManager.GetStringField(arResult[i]);
        ...
        /*
         * 신호가 수신된 경우 기본으로 설정되어 있는 preset으로 이동 
         */
        public void SetTargetCCTVPreset(int EquipmentZoneID)
        {
            if (FormMain.Instance.CCTVPipe == null)
                return;

            FormMain.Instance.CCTVPipe.Send("GoPresetInvokeAlarm(" + EquipmentZoneID + ")");
        }
		...
		{
            bool bSituation = (nSituation == 0) ? false : true;
            FormMain.Instance.ShowLeftThumbnail(bSituation, arrCCTVs);
            ...
            if (UnE.SOP.ProxySOP.Instance.ShowCCTVForm == true)
            {
                ...
                if (arrCCTVs.Count == 4)
                    FormMain.Instance.CCTVPipe.Send("SetCCTV(" + strCCTVIDs + ")");
                else if (arrCCTVs.Count == 5)
                    FormMain.Instance.CCTVPipe.Send("SetCCTV5(" + strCCTVIDs + ")");
		...
		public ArrayList GetEquipZoneCCTVList(EquipmentZone equipZone)
		{
			...
			if (arrCCTVs == null)
			{
                ...
                {
                    ArrayList arrResult = CCTVManager.Instance.AutoPopupCCTV((Zone)equipZone.LinkedZoneList[0]);

                    // 자동탐지로 탐색한 CCTV는 4개까지만 사용한다.
                    if (arrResult != null)
                    {
                        while (arrResult.Count > 4)
                            arrResult.RemoveAt(4);
                    }
			...
			else
			{
                ...
                // CCTV5가 있으면 실내이미지 대신 사용한다.
                if (arrCCTVs.Length > 4 && arrCCTVs[4] != null)
                    arr.Add(arrCCTVs[4]);
			}
		...
		private void PostChangedData()
		{
			...
			{
                SDMS.NetworkWebManager.Instance.SendChangeFacilityManager();
			}

			if (m_isChangedEquipZoneCCTV)
			{
				int nID = FormMain.Instance.CurrentEquipZone.ID;
                SDMS.NetworkWebManager.Instance.SendChangeEquipZoneCCTV(nID);
			}
		...
		{
            FormMain.Instance.SetImageLeftAdminBtn(m_nTranslucentCommandID, false);
        ...
        private void ShowCCTVList()
        {
            ...
            m_frmCCTVList.SetSize(this.Height - m_dockFormLocation.panelTitle.Height); 
        ...
        private void ShowPSMList()
        {
            ...
            m_frmPSMList.SetSize(this.Height - m_dockFormLocation.panelTitle.Height);

            if (m_frmCCTVList != null && m_frmCCTVList.IsDisposed == false)
            {
        ...
        public void SetEarthquakeDetect(int nIntensity, float fMagnitude, string strPosition, bool isRealMode)
        {
            ...
            FormMain.Instance.SetVisibleDisasterPanel(true);
        }

        public void OnFinishEarthquake()
        {
            FormMain.Instance.SetEarthquakeDetect(0, 0.0f, "", true);
            FormMain.Instance.SetVisibleDisasterPanel(false);
        }
