...
{
    public partial class FormPSMList : PopupFormBase, IChildControl
    {
        ...
        {
            /*Label[] checkItems = { chkETCText, chk12Text, chk34Text, chk56Text, chkWaterText };   
            for( int i = 0 ; i < 5; i++)
            {
            ...
            Control[] checkItems2 = { btnChk12, chk12Text, btnChk34, chk34Text, btnChk56, chk56Text, btnChkWater, chkWaterText, btnChkETC, chkETCText };   
            for (int i = 1; i < 10; i++)
            {
                int x = checkItems2[i - 1].Location.X;
                int empty = (checkItems2[i] is ImageButton) ? 10 : 3;
                int width = checkItems2[i - 1].Size.Width + empty;
            ...
            lock (LockSensorData)
            {
                ...
                if (arrResult != null)
                {
                    ...
                    for (int i = 0; i < arrResult.Count; i += 12)
                    {
                        ...
                        VariousData<float> fCapacity = WebDBManager.GetFloatField(arrResult[i + 6].ToString());
                        VariousData<float> fRemains = WebDBManager.GetFloatField(arrResult[i + 7].ToString());
                            ...
                            (nMaterialID == currentMaterialItem.ID || currentMaterialItem.ID == -1))
                        {
                            ...
                            row.Cells[TANK_CCTV_INDEX].Value = "";
        ...
        private string GetTankRemainString(VariousData<float> fCapacity, VariousData<float> fRemains, string strUnitName)
        {
            string strRemains = "";
            ...
            lock (LockSensorData)
            {
                ...
                for (int i = 0; i < nResultCount - 11; i += 12)
                {
                        ...
                        (currentStatusItem.IsSame(sensor.SensorStatus) == true))
                    {
                        ...
                        row.Cells[SENSOR_CCTV_INDEX].Value = "";
        ...
        private void PopSensorState(UnE.PSM.PSMSensor sensor, int nSensorNo, string strLocationName, string strMaterialName)
        {
            ...
            {
                m_frmSensor.SetChildCtrlResize(m_frmSensor, 354, 137);
        ...
        private void PopTankDetail(UnE.PSM.PSMTank tank)
        {
            ...
            {
                m_frmTank.SetChildCtrlResize(m_frmTank, 459, 323);
                m_frmTank.SetLocation();
                m_frmTank.Show(this);
            }
        }
        ...
        {            
            ...
            {
                m_frmDepartment.SetChildCtrlResize(m_frmDepartment, 333, 236);
        ...
        private void PopSensorLifeTime(UnE.PSM.PSMSensor sensor, int nSensorNo)
        {
            ...
            {
                m_frmSensorLifeTime.SetChildCtrlResize(m_frmSensorLifeTime, 317, 254);
        ...
        private void UpdateTanks(Dictionary<UnE.PSM.PSMSensor, VariousData<int>> dicSensorIDRow)
        {
            ...
            {
                int nID = WebDBManager.GetIntField(arrResult[i].ToString(), -1);
                VariousData<float> fRemains = WebDBManager.GetFloatField(arrResult[i + 1].ToString());
                VariousData<float> fCapacity = WebDBManager.GetFloatField(arrResult[i + 2].ToString());
        ...
        private void btnShowSensorManual_Click(object sender, EventArgs e)
        {
            ...
            string strPath = System.IO.Path.GetDirectoryName(Application.ExecutablePath) + "\\" + WebDBManager.GetStringField(arrResult[0]);
        ...
        private void PopSensorAlarm(UnE.PSM.PSMSensor sensor, int nSensorNo)
        {
            ...
            {
                m_frmSensorAlarm.SetChildCtrlResize(this, 566, 285);
        ...
        {
            SetChildCtrlResize(this, 652, height);
            FormMain.Instance.CustomizeGridView(gvPSMSensor);
            FormMain.Instance.CustomizeGridView(gvPSMTank);
        }

        private Image img = SDMS.Properties.Resources.PSMList_Show_Default;
        private void gvPSMTank_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex < 0)
                return;
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                e.Graphics.DrawImage(img, new Rectangle(e.CellBounds.X, e.CellBounds.Y, e.CellBounds.Width, e.CellBounds.Height));
                e.Handled = true;
            }
        }

        private bool chk12 = true;
        private bool chk34 = true;
        private bool chk56 = true;
        private bool chkWater = true;
        private bool chkETC = true;

        #region 체크박스
        private Image imgChecked = SDMS.Properties.Resources.CheckBox_Click;
        private Image imgUnChecked = SDMS.Properties.Resources.CheckBox_Default;

        private void btnChk12_Click(object sender, EventArgs e)
        {
            if (chk12)
            {
                btnChk12.ImageNormal = imgUnChecked;
                btnChk12.ImageMouseOver = imgUnChecked;
                btnChk12.ImageClicked = imgChecked;
                chk12 = false;
            }
            else
            {
                btnChk12.ImageNormal = imgChecked;
                btnChk12.ImageMouseOver = imgChecked;
                btnChk12.ImageClicked = imgUnChecked;
                chk12 = true;
            }
        }

        private void btnChk34_Click(object sender, EventArgs e)
        {
            if (chk34)
            {
                btnChk34.ImageNormal = imgUnChecked;
                btnChk34.ImageMouseOver = imgUnChecked;
                btnChk34.ImageClicked = imgChecked;
                chk34 = false;
            }
            else
            {
                btnChk34.ImageNormal = imgChecked;
                btnChk34.ImageMouseOver = imgChecked;
                btnChk34.ImageClicked = imgUnChecked;
                chk34 = true;
            }
        }

        private void btnChk56_Click(object sender, EventArgs e)
        {
            if (chk56)
            {
                btnChk56.ImageNormal = imgUnChecked;
                btnChk56.ImageMouseOver = imgUnChecked;
                btnChk56.ImageClicked = imgChecked;
                chk56 = false;
            }
            else
            {
                btnChk56.ImageNormal = imgChecked;
                btnChk56.ImageMouseOver = imgChecked;
                btnChk56.ImageClicked = imgUnChecked;
                chk56 = true;
            }
        }

        private void btnChkWater_Click(object sender, EventArgs e)
        {
            if (chkWater)
            {
                btnChkWater.ImageNormal = imgUnChecked;
                btnChkWater.ImageMouseOver = imgUnChecked;
                btnChkWater.ImageClicked = imgChecked;
                chkWater = false;
            }
            else
            {
                btnChkWater.ImageNormal = imgChecked;
                btnChkWater.ImageMouseOver = imgChecked;
                btnChkWater.ImageClicked = imgUnChecked;
                chkWater = true;
            }
        }

        private void btnChkETC_Click(object sender, EventArgs e)
        {
            if (chkETC)
            {
                btnChkETC.ImageNormal = imgUnChecked;
                btnChkETC.ImageMouseOver = imgUnChecked;
                btnChkETC.ImageClicked = imgChecked;
                chkETC = false;
            }
            else
            {
                btnChkETC.ImageNormal = imgChecked;
                btnChkETC.ImageMouseOver = imgChecked;
                btnChkETC.ImageClicked = imgUnChecked;
                chkETC = true;
            }
        } 
        #endregion

        private void gvPSMSensor_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.RowIndex < 0)
                return;
        ...
        private void SetManualID()
        {
            ...
            m_manualManager.SetID(this, "PSMList");
            m_manualManager.SetID(gvPSMSensor, "PSMList_ShowSensorList");
            m_manualManager.SetID(gvPSMTank, "PSMList_ShowTankList");
