...
namespace SDMS
{
    public partial class FormSensorMgrList : PopupFormBase
    {
        ...
        public FormSensorMgrList()
        {
            ...
            for (int i = 0; i < gvSensorMgrList.ColumnCount; i++)
            {
                gvSensorMgrList.Columns[i].SortMode = DataGridViewColumnSortMode.NotSortable;
            }
        }
        ...
        { 
            ...
            if (sensorType > SensorType.ALL)
            {
                ...
                if (sensorType == SensorType.DETECT_PROTECT)
                {
                    ...
                    if (m_nSiteID == 102)
                    {
                        AddConditionString(ref strTypeCondition, "Type = " + ((int)IFacility.FacilityType.CustomerEmergencyC1_S1).ToString(), false);       //여자 화장실 센서
                        AddConditionString(ref strTypeCondition, "Type = " + ((int)IFacility.FacilityType.CustomerEmergencyC2_S1).ToString(), false);       //여자 화장실 센서
                    }
                }
                ...
                else
                {
                    ...
                    if (type == IFacility.FacilityType.FIRE_SENSOR)
                    {
                        ...
                        if (m_nSiteID == 102)
                        {
                            AddConditionString(ref strTypeCondition, "Type = " + ((int)IFacility.FacilityType.FireF1_S1).ToString(), false);
                        }
                    }
                }
            ...
            for (int i = 0; i < nResultCount - 8; i += 9)
            {
                ...
                int nTagID = WebDBManager.GetIntField(arrResult[i + 6].ToString(), -1);
                string szSensorName = WebDBManager.GetStringField(arrResult[i + 7], "");
                string deActivate = WebDBManager.GetStringField(arrResult[i + 8], "");
                if (nID == 0)
                    continue;
        ...
        private void store_btnClick(object sender, EventArgs e)
        {
            ...
            if (storedDic.Count > 0)
                NetworkWebManager.Instance.SendTagChangeDeactivationInfo(storedDic, changeListCallBack);  
        ...
        {
            NetworkWebManager.Instance.removeChangeEventCallback();
