...
{ 
	...
	{
        private Point m_ptPrevLocation = new Point(0, 0);
        ...
        public void ViewCCTV()
        {
            FormMain.Instance.PageHome.Invoke((MethodInvoker)delegate
            {
                ...
                {
                    EquipmentZone equipZone = ZoneManager.Instance.GetEquipZone(mCurrentData.Sensor.ZoneID);
		...
		private int GetSensorHistoryID(int nSensorID)
		{
            foreach (KeyValuePair<string, ProcessIF> pair in ProcessManager.Instance.CurrentDetectProcess)
            {
                ...
                return pair.Value.SensorHistoryID;
            }
        ...
        public bool ReportSpill()
        {
            ...
            return SDMS.NetworkWebManager.Instance.SendMessage(1, SOPWebServer.Header.NOTIFY_DISASTER, nHistoryID, ZoneID, nSensorID, nSOPGenUserID);
        }
        ...
        public bool ReportSecurity()
        {
            ...
            int nSOPGenUserID = FormMain.Instance.SOPGenUserID;
            return SDMS.NetworkWebManager.Instance.SendMessage(1, SOPWebServer.Header.NOTIFY_DISASTER, nHistoryID, EquipZoneID, nSensorID, nSOPGenUserID);
        }
		...
		public void ReportFireTranning()
		{
            ...
            //int nHistoryID = GetSensorHistoryID(nSensorID);
            //NetworkManager.Instance.SendMessage(SOPWebServer.Header.FIRE_DETECT_TRAINNING, nHistoryID, ZoneID, nSensorID);
        ...
        public bool BuzzerToggle(bool bOn)
        {
            ...
            if (process != null)
            {
                int nSOPGenUserID = FormMain.Instance.SOPGenUserID;
                // 서버로 오작동 신고를 수행한다.
                return SDMS.NetworkWebManager.Instance.SendMessage(1, SOPWebServer.Header.PSM_BUZZER_STOP, bOn == true ? 1 : 0, nSensorID, nSOPGenUserID);
            }
            return false;
        }
        ...
        public bool ReportPSMReset(string strDescriptionText)
        {
			...
			if (process != null)
			{
				int nSOPGenUserID = FormMain.Instance.SOPGenUserID;
				// 서버로 오작동 신고를 수행한다.
                return SDMS.NetworkWebManager.Instance.SendMessage(1, SOPWebServer.Header.SENSOR_USER_RESET, process.SensorHistoryID, nSensorID, nSOPGenUserID, strDescriptionText);
			}
			return false;
        }
		...
		public bool ReportAbnormal(string strDescriptionText)
		{
			...
			if (process != null)
			{
				int nSOPGenUserID = FormMain.Instance.SOPGenUserID;
				// 서버로 오작동 신고를 수행한다.
                return SDMS.NetworkWebManager.Instance.SendMessage(1, SOPWebServer.Header.SENSOR_MALFUNCTION, process.SensorHistoryID, nSensorID, nSOPGenUserID, strDescriptionText);
			}
			return false;
		}
        ...
        public bool ReportEarthquakeReset(string strDescriptionText)
        {
            ISensor m_Sensor = mCurrentData.Sensor;
            int nSensorID = m_Sensor.ID;
            ProcessIF process = ProcessManager.Instance.GetProcess(nSensorID);
        ...
        public bool ReportTemperatureHumidityReset(string strDescriptionText)
        {
            ISensor m_Sensor = mCurrentData.Sensor;
            int nSensorID = m_Sensor.ID;
            ProcessIF process = ProcessManager.Instance.GetProcess(nSensorID);
        ...
        private void BtnReportFire_Click(object sender, EventArgs e)
		{
            ...
            if( m_dlgType == ProcessType.PSMAlarm)
            {
                szPopupMessage = "전직원에게 유해화학물질 누출을 전파합니다.\r\n유해화학물질 누출발생으로 신고하시겠습니까?";
            }
            else if(m_dlgType == ProcessType.SecurityAlarm)
            {
            ...
            if (m_dlgType == ProcessType.PSMAlarm)
            {
                if (ReportSpill())
                {
                    ...
                    {
                        ISensor m_Sensor = mCurrentData.Sensor;
                        Zone zone = ZoneManager.Instance.GetZone(m_Sensor.ZoneID);
                        if (zone != null)
                        {
            ...
            else if(m_dlgType == ProcessType.FireAlarm)
            {
                if (ReportFire())
                {
                    FireDetectProcess.SoundPlayer.Stop();
		...
		private void BtnReportMalfunction_Click(object sender, EventArgs e)
		{
            ...
            else if (m_dlgType == ProcessType.EarthquakeAlarm)
            {
                string strDescription = "신호복구시 남길 메모가 있으면 아래에 입력해 주세요.";
                string strDescriptionText = "";

                if (IMessageBox.InputMessageBox.Show("지진탐지 신호를 복구하시겠습니까?", "확인", MessageBoxButtons.YesNo, strDescription, ref strDescriptionText) == System.Windows.Forms.DialogResult.No)
                    return;

                if (FireDetectProcess.SoundPlayer.SoundLocation != null)
                    FireDetectProcess.SoundPlayer.Stop();

                if (ReportEarthquakeReset(strDescriptionText))
                {
                    FireDetectProcess.SoundPlayer.Stop();
            ...
            else if (m_dlgType == ProcessType.TemperatureHumidityAlarm)
            {
                string strDescription = "신호복구시 남길 메모가 있으면 아래에 입력해 주세요.";
                string strDescriptionText = "";

                if (IMessageBox.InputMessageBox.Show("온도/습도 알람을 복구하시겠습니까?", "확인", MessageBoxButtons.YesNo, strDescription, ref strDescriptionText) == System.Windows.Forms.DialogResult.No)
                    return;

                if (FireDetectProcess.SoundPlayer.SoundLocation != null)
                    FireDetectProcess.SoundPlayer.Stop();

                if (ReportTemperatureHumidityReset(strDescriptionText))
                {
                    FireDetectProcess.SoundPlayer.Stop();
        ...
        private ProcessType m_dlgType = ProcessType.FireAlarm;
        ...
        public SeletCaseData CurrentData
		{
			...
			set
			{
                ...
                {
                    m_dlgType = ProcessType.PSMAlarm;
                    // 광교 실내공기질
                    if (UnE.SOP.ProxySOP.Instance.SiteID == 3 && (mCurrentData.Sensor.Description == "산소" || mCurrentData.Sensor.Description == "이산화탄소" || mCurrentData.Sensor.Description == "일산화탄소" || mCurrentData.Sensor.Description == "메탄"))
                    {
                        strTitle = string.Format("밀폐공간 [{0}] 신호 수신", mCurrentData.Sensor.Description);
                    }
                    else
                ...
                {
                    m_dlgType = ProcessType.SecurityAlarm;
                ...
                {
                    m_dlgType = ProcessType.FireAlarm ;
                ...
                else if (mCurrentData.ProcessType == ProcessType.EarthquakeAlarm)
                {
                    m_dlgType = ProcessType.EarthquakeAlarm;
                    strTitle = "지진 신호 수신";

                    mBtnReportFire.ImageNormal = global::SDMS.Properties.Resources.DlgSelectCase_ReportEarthquake_Default;
                    mBtnReportFire.ImageMouseOver = global::SDMS.Properties.Resources.DlgSelectCase_ReportEarthquake_Click;
                    mBtnReportFire.ImageClicked = global::SDMS.Properties.Resources.DlgSelectCase_ReportEarthquake_Click;

                    // 지진신호는 따로 전파 기능을 수행하지 않는다.
                    mBtnReportFire.Visible = false;
                    mBtnViewCCTV.Visible = false;
                ...
                else if (mCurrentData.ProcessType == ProcessType.TemperatureHumidityAlarm)
                {
                    m_dlgType = ProcessType.TemperatureHumidityAlarm;
                    strTitle = "온도/습도 알람 발생";

                    mBtnReportFire.ImageNormal = global::SDMS.Properties.Resources.DlgSelectCase_ReportEarthquake_Default;
                    mBtnReportFire.ImageMouseOver = global::SDMS.Properties.Resources.DlgSelectCase_ReportEarthquake_Click;
                    mBtnReportFire.ImageClicked = global::SDMS.Properties.Resources.DlgSelectCase_ReportEarthquake_Click;

                    // 온도/습도 신호는 따로 전파 기능을 수행하지 않는다.
                    mBtnReportFire.Visible = false;
                    mBtnViewCCTV.Visible = true;
        ...
        private bool GetFireAlarmDepartmentInfo(ref string strDepartment, ref string strPhoneNumber)
        {
            ...
            {
                string strPropertyName = DBUtility2.WebDBManager.GetStringField(arrResult[i]);
                string strPropertyValue = DBUtility2.WebDBManager.GetStringField(arrResult[i + 1]);
        ...
        private void ReadPosition()
        {
            ...
            string strX = RegUtil.ReadRegValue("SDMS", "DlgSelectPosX", nSiteID);
            string strY = RegUtil.ReadRegValue("SDMS", "DlgSelectPosY", nSiteID);
        ...
        private void SavePosition()
        {
            ...
            // 모니터간의 우선순위에 따라 좌표값이 0보다 작을수 있다.
            // [2019/10/30] 김지웅
            /*if (ptPosition.X < 0)
                ptPosition.X = 0;
            if (ptPosition.Y < 0)
            ...
            RegUtil.WriteRegValue("SDMS", "DlgSelectPosX", ptPosition.X.ToString(), nSiteID);
            RegUtil.WriteRegValue("SDMS", "DlgSelectPosY", ptPosition.Y.ToString(), nSiteID);
        ...
        public void SetManualID()
        {
            ...
            {
                m_manualManager.SetID(lblTitle, "SDMS_Alarm_EarthquakeDetect_DlgSelectCase");
                m_manualManager.SetID(labelHeader, "SDMS_Alarm_EarthquakeDetect_DlgSelectCase");
                m_manualManager.SetID(mBtnViewCCTV, "SDMS_CCTV_OnEarthquake");
                m_manualManager.SetID(mBtnReportFire, "SDMS_Alarm_EarthquakeReport");
                m_manualManager.SetID(mBtnReportMalfunction, "SDMS_Alarm_EarthquakeDetect_DlgSelectCase");
                m_manualManager.SetID(btnSound, "SDMS_Alarm_EarthquakeDetect_DlgSelectCase");
                m_manualManager.SetID(btnStartSOP, "SDMS_Alarm_EarthquakeDetect_DlgSelectCase");
            }
        ...
        private void DlgSelectCase_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                m_ptPrevLocation = this.Location;
            }
        }

        private void DlgSelectCase_MouseUp(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                // 사용자가 마우스로 알람창을 옮겨놓았을 경우 알람창의 위치정보를 Registry에 기억시킨다.
                if (m_ptPrevLocation.X != this.Location.X || m_ptPrevLocation.Y != this.Location.Y)
                {
                    SavePosition();
                }
            }
