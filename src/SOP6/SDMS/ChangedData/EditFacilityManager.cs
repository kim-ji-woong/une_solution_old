...
namespace SDMS
{
	public class EditFacilityManager : ChangedData
	{
        ...
        {
            get { return m_isDetect; }
            set { m_isDetect = value; }
        }
		...
		private bool InsertFacilityManager(WebDBManager dbMgr)
		{
            string strTableName = m_isDetect ? "FacilityManager" : "FacilityManagerReport";

			ArrayList arrResult = dbMgr.GetResultData("Select max(id) from " + strTableName);
			if (arrResult == null)
				return false;

			int nID = arrResult.Count == 0 ? 1 : WebDBManager.GetIntField(arrResult[0].ToString(), 0) + 1;

			string strFormat = "Insert into {8} (ID, MemberID, MemberType, FacilityType, LevelLimit, UpperLimit, Description, SiteID) values ";
			strFormat += "({0}, {1}, {2}, {3}, {4}, {5}, '{6}', {7})";

            string strSQL = string.Format(strFormat, nID, m_mgr.MemberID, m_mgr.MemberType, m_facilityType.Data, m_mgr.LevelLimit, m_mgr.UpperLimit, m_mgr.Description, m_nSiteID, strTableName);
            ...
            if (m_isDetect)
                ...
                FormMain.Instance.DataManager.AddFacilityManagerReport(m_mgr, m_mgr.Type);

			return true;
		}

		private bool InsertEquipZoneFacilityManager(WebDBManager dbMgr, int nEquipZone)
		{
            string strTableName = m_isDetect ? "EquipZoneFacilityManager" : "EquipZoneFacilityManagerReport";
			...
			string strFormat = "Insert into {9} (ID, MemberID, MemberType, FacilityType, LevelLimit, EquipZoneID, UpperLimit, Description, SiteID) values ";
			strFormat += "({0}, {1}, {2}, {3}, {4}, {5}, {6}, '{7}', {8})";

			string strSQL = string.Format(strFormat, nID, m_mgr.MemberID, m_mgr.MemberType, m_facilityType.Data, m_mgr.LevelLimit, nEquipZone, m_mgr.UpperLimit, m_mgr.Description, m_nSiteID, strTableName);
            ...
            if (m_isDetect)
                ...
                FormMain.Instance.DataManager.AddEquipZoneFacilityManagerReport(m_mgr, m_mgr.EquipZone, m_mgr.Type);

			return true;
		}

		private bool InsertBuildingFacilityManager(WebDBManager dbMgr, int nBuildingID)
		{
            string strTableName = m_isDetect ? "BuildingFacilityManager" : "BuildingFacilityManagerReport";
			...
			string strFormat = "Insert into {9} (ID, MemberID, MemberType, FacilityType, LevelLimit, BuildingID, UpperLimit, Description, SiteID) values ";
			strFormat += "({0}, {1}, {2}, {3}, {4}, {5}, {6}, '{7}', {8})";

			string strSQL = string.Format(strFormat, nID, m_mgr.MemberID, m_mgr.MemberType, m_facilityType.Data, m_mgr.LevelLimit, nBuildingID, m_mgr.UpperLimit, m_mgr.Description, m_nSiteID, strTableName);
            ...
            if (m_mgr.Building != null)
            {
                if (m_isDetect)
                    ...
                    FormMain.Instance.DataManager.AddBuildingFacilityManagerReport(m_mgr, m_mgr.Building, m_mgr.Type);
            }
            else// if (m_mgr.Zone != null)
            {
                if (m_isDetect)
                    ...
                    FormMain.Instance.DataManager.AddOutdoorFacilityManagerReport(m_mgr, m_mgr.Zone, m_mgr.Type);
            }
		...
		public override void AddToManager(IChangedDataManager mgr)
		{
			...
			foreach (ChangedData data in arrDatas)
			{
				if (data.GetType() == type)
				{
					...
					if (facility.IsDetect == this.IsDetect &&
                        facility.Manager.Building == this.Manager.Building &&
