...
namespace SVMSEventReciver
{
    ...
    public class CameraManager
    {
        ...
        public CameraManager(int nSiteID, WebDBManager dbMgr)
        {
            m_nSiteID = nSiteID;
            m_dbMgr = dbMgr;
        }
        ...
        public CCTV LoadCCTV(int nID)
        {
            ...
            sb.Append(",cv.HTTPPort, cv.Type,cv.Stream,cv.Channel,cv.UserID,cv.Password,cv.URL  , cv.ID ");
        ...
        private SortedList<int, CCTV> m_CameraList = new SortedList<int, CCTV>();
        private SortedList<string, CCTV> m_IPCamearList = new SortedList<string, CCTV>();
        ...
        public void LoadCCTV()
        {
            ...
            int nResultCount = arrResult.Count;
            int i = 0;
            for( i = 0 ; i < nResultCount - 16 ; i+=17)
            {
                ...
                int nReversPtz = WebDBManager.GetIntField(arrResult[i + 16].ToString(), 0);
                ...
                if (nID >= 0)
                    m_CameraList.Add(nID, cctv);


                if (!m_IPCamearList.ContainsKey(strIPAddr))
                    m_IPCamearList.Add(strIPAddr, cctv);
        ...
        internal CCTV GetCCTV(string szCameraIP)
        {
            if( m_IPCamearList.ContainsKey(szCameraIP))
            {
                return m_IPCamearList[szCameraIP];
            }
            return null;
        }
    }
