    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                if (ParseArguments(args, ref nSiteID, ref nCCTVID, ref szTitle, ref hWnd, ref nPositionIndex, ref EquipZoneCCTVMode,ref EquipZoneID, ref useTitle, ref strFilePath))
                {
                    CCTVMessageFilter m_msgFilter = new CCTVMessageFilter();                    

                    UnE.CCTV.CCTVLoader loader = new UnE.CCTV.CCTVLoader(nSiteID);
                    UnE.CCTV.CCTV cctv = strFilePath == null ? loader.LoadCCTV(nCCTVID) : loader.LoadCCTV(strFilePath);
                    string strDefaultPreset = loader.GetDefaultPreset(EquipZoneID, nPositionIndex);                                        

                    if (cctv != null && hWnd != IntPtr.Zero)
                    {
                        Form form = LoadForm(cctv, hWnd, nPositionIndex, EquipZoneCCTVMode, EquipZoneID, strDefaultPreset, useTitle);
        ...
        static Form LoadForm(UnE.CCTV.CCTV cctv, IntPtr hWnd, int nPositionIndex, bool nEquipZoneCCTVMode, int nEquipZoneID,string nDefaultPReset, bool useTitle)
        {
            Form frm = new UnE.CCTV.BigCCTVCtrlOwner(cctv, hWnd, nPositionIndex, nEquipZoneCCTVMode, nEquipZoneID, nDefaultPReset, useTitle);
            return frm;
        }
        ...
        static bool ParseArguments(string[] args, ref int nSiteID, ref int nCCTVID, ref string szTitle, ref IntPtr hWnd, ref int nPositionIndex, ref bool nEquipZoneCCTVMode, ref int nEquipZoneID, ref bool useTitle, ref string strFilePath)
        {
            if (args.Length < 5)
                return false;
            ...
            try
            {
                ...
                if (args.Length > 7)
                {
                    ...
                    if (int.TryParse(args[7].Trim(), out nUse))
                        useTitle = nUse == 1;
                }

                // DB를 읽지않고 CCTV 정보를 파일에서 읽어야 할 경우
                if (args.Length > 8)
                {
                    strFilePath = args[8].Trim();
                }
