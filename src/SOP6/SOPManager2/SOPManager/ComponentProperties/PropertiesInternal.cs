...
namespace SOPManager
{
	public class PropertiesInternal : SectionPropertiesBase
	{
        ...
        private string m_strSirenConfig = "";
        ...
        [Browsable(true)]
        [DisplayName("사용여부")]
        [Description("방송시 사이렌 사용여부를 설정합니다.")]
        [TypeConverter(typeof(UsingTypeConverter))]
        [RefreshProperties(System.ComponentModel.RefreshProperties.All)]
        [ReadOnly(false)]
        public string SirenConfig
        {
            get
            {
                if (mSection != null)
                {
                    Sections.SectionDataInternal data = (Sections.SectionDataInternal)mSection.Data;
                    if (data.UseSiren == false)
                    {
                        m_strSirenConfig = UsingType.TypeList[0];
                    }
                    else
                    {
                        m_strSirenConfig = UsingType.TypeList[1];
                    }

                }
                return m_strSirenConfig;
            }
            set
            {
                if (m_strSirenConfig != value)
                {
                    if (mSection != null)
                    {
                        Sections.SectionDataInternal data = (Sections.SectionDataInternal)mSection.Data;
