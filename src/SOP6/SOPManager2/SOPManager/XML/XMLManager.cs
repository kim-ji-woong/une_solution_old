...
namespace SOPManager
{
    ...
    public class XMLManager
    {
        ...
        private static string XML_VERSION = "V1.5";
        ///////////////////////////////////////

        private ArrayList m_arrActionSteps = new ArrayList();
        private string m_strUserDefinedConfigName = "";

        // ActionStep과 UserDefinedConfig 가운데 XML 문서상 먼저 나타나는 쪽에 대한 Dictionary를 만든다.
        // 문서상 나중에 나타나는 쪽은 먼저 만들어진 Dictionary를 이용하여 최종적으로 ActionStep의 UserDefinedConfig 값을 채우면 된다.
        private Dictionary<ActionStep, string> m_dicActionStepUserDefinedConfigName = null;
        private Dictionary<string, ConfigData> m_dicUserDefinedConfig = null;
        ...
        private bool LoadScreen(FormMain frm)
        {
			...
			frm.GetPageLevel().GetPropertiesLevel().SetTitleText(strFullPath);
            ...
            if (LoadTabPages(frm))
            {
				...
				//pageDisaster.SelectedCategory = m_strCategoryName;
				//pageDisaster.SelectedSubCategory = m_strSubCategoryName;
				// pageDisaster.SelectedDetailCategory = m_strDisasterName;

				//pageDisaster.SetRegularMode(m_isRegular);
				//pageDisaster.SetWeekMode(m_isNormal);
        ...
        private bool GetTeamInfo(ActionStep actionStep, out string strTeamName, out int nTeamID, out Sections.SOPTeam.SOPTeamType nTeamType)
        {
            strTeamName = "";
            nTeamID = -1;
            nTeamType = Sections.SOPTeam.SOPTeamType.None;
        ...
        private void LoadBarPage(FormMain frm, ArrayList arrActionSteps)
        {
            ...
            int nTeamID;
            Sections.SOPTeam.SOPTeamType nTeamType;
            string strTeamName;
            ...
            {
                GetTeamInfo(actionStep, out strTeamName, out nTeamID, out nTeamType);
        ...
        private bool LoadTabPages(FormMain frm)
        {
            ...
            ActionStep currentActionStep = null;
            ...
            if (currentActionStep != null)
                pageLevel.GetBarConfig().SetConfig(currentActionStep.UserDefinedConfig);
        ...
        private bool AddPanels(TabPage tabPage, ActionStep actionStep, FormPageSOP pageLevel, ref Dictionary<int, string> dicNormal, ref Dictionary<int, string> dicEmergency, ref Dictionary<int, string> dicUserDefined, ref Dictionary<int, Sections.ExternalTeamData> dicExternal, ref Dictionary<int, string> dicRegular, ref Dictionary<int, string> dicControlRoom)
        {
			...
			pageLevel.AddUsingTeam(arrStepMemberDatas);
            ...
            foreach (Sections.PanelSectionEx panel in arrPanels)
            {
				...
				if (!AddComponents(panel, stepMember, nStepMemberIndex, dicSections, dicLinkSections, ref dicNormal, ref dicEmergency, ref dicUserDefined, ref dicExternal, ref dicRegular, ref dicCompSection, ref dicControlRoom))
                    return false;

                if (!AddArrows(stepMember, (long)nStepMemberIndex, dicSections))
                    return false;
				
	
				if(stepMember.Viewport!= null)
				{
        ...
        private Sections.Section ToSection(Sections.PanelSectionEx panel, float x, float y, ComponentProperty property, Dictionary<PropertyLink, Sections.SectionLink> dicLinkSections, ref Dictionary<int, string> dicNormal, ref Dictionary<int, string> dicEmergency, ref Dictionary<int, string> dicUserDefined, ref Dictionary<int, Sections.ExternalTeamData> dicExternal, ref Dictionary<int, string> dicRegular, Dictionary<int, Sections.Section> dicCompSection, ref Dictionary<int, string> dicControlRoom)
        {
            ...
            {
                return ToSectionDecision(panel, x, y, (PropertyDecision)property);
        ...
        private Sections.SectionDecision ToSectionDecision(Sections.PanelSectionEx panel, float x, float y, PropertyDecision property)
        {
            ...
            data.Expression = property.AutoRunScript;

            foreach (string strType in property.VariableTypes)
            {
                int nIndex1 = strType.LastIndexOf('(');
                int nIndex2 = strType.LastIndexOf(')');

                if (nIndex1 >= 0 && nIndex2 > nIndex1)
                {
                    string strVariable = strType.Substring(0, nIndex1);
                    string strVariableType = strType.Substring(nIndex1 + 1, nIndex2 - nIndex1 - 1);
                    data.VariableTypes[strVariable] = Sections.SectionDataDecision.ToVariableType(strVariableType);
                }
            }
        ...
        private Sections.SectionInternal ToSectionInternal(Sections.PanelSectionEx panel, float x, float y, PropertyInternal property, ref Dictionary<int, string> dicNormal, ref Dictionary<int, string> dicEmergency, ref Dictionary<int, string> dicUserDefined, ref Dictionary<int, Sections.ExternalTeamData> dicExternal, ref Dictionary<int, string> dicRegular, ref Dictionary<int, string> dicControlRoom)
        {
            ...
            data.UseSiren = property.UseSiren;
            data.BroadcastMessage = property.BroadcastMessage;
            data.AutoRun = property.AutoRun;
        ...
        private int GetLinkedActionStepID(PropertyTransSOP property)
        {
            ...
            // Site별로 사용할 수 있도록 Query 수정 , Edit by skkim 2015.01.09
            string strFormat = "SELECT step.ID, step.DisasterID FROM ActionStep as step, Disaster as dis, SubDisasterCategory as sdc, DisasterCategory as dc ";
            strFormat += " WHERE dc.SiteID = {4} AND dc.CategoryName = '{3}' AND dc.ID = sdc.DisasterID AND  sdc.SubCategoryName = '{2}' ";
            strFormat += " AND sdc.ID = dis.SubDisasterID AND dis.DisasterName = '{1}' AND step.DisasterID = dis.ID AND step.StepName = '{0}'";
		...
		private bool Load(FormMain frm, XmlTextReader reader)
		{
            ...
            m_dicActionStepUserDefinedConfigName = null;
            m_dicUserDefinedConfig = null;
            ...
            while (reader.Read())
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        else if (string.Compare(reader.Name, "UserDefinedConfig", true) == 0)
                        {
                            if (!ReadUserDefinedConfig(reader))
                                return false;
        ...
        private bool ReadUserDefinedConfig(XmlTextReader reader)
        {
            m_dicUserDefinedConfig = new Dictionary<string, ConfigData>();
            bool stop = false;
            ...
            while (reader.Read())
            {
                ...
                {
                    case XmlNodeType.Element:
                        if (string.Compare(reader.Name, "ConfigData", true) == 0)
                        {
                            ConfigData data = ReadConfigData(reader);

                            if (data == null)
                                return false;

                            m_dicUserDefinedConfig[data.Text] = data;
                        }
                        else
                            PassElement(reader);
            ...
            if (m_dicActionStepUserDefinedConfigName != null)
            {
                foreach (KeyValuePair<ActionStep, string> pair in m_dicActionStepUserDefinedConfigName)
                {
                    ConfigData data = null;

                    if (m_dicUserDefinedConfig.TryGetValue(pair.Value, out data) == false)
                    {
                        m_strErrorMessage = pair.Value + "는 정의되지 않은 설정 이름입니다.";
                        return false;
                    }
                    else
                        pair.Key.UserDefinedConfig = data;
                }
            }
        ...
        private ConfigData ReadConfigData(XmlTextReader reader)
        {
            bool stop = false;
            ...
            while (reader.Read())
            {
                ...
                {
                    case XmlNodeType.Element:
                        if (string.Compare(reader.Name, "ConfigName", true) == 0)
                        {
                            if (!ReadText(reader, ref strConfigName))
                                return null;
                        }
                        else if (string.Compare(reader.Name, "Variables", true) == 0)
                        {
                            if (!ReadVariables(reader, parameters))
                                return null;
                        }
                        else
                            PassElement(reader);
            ...
            if (strConfigName == null || strConfigName.Length == 0)
                return null;

            ConfigData data = new ConfigData(ConfigData.ConfigType.FILE);
            data.Text = strConfigName;
            data.Variables.AddRange(parameters);

            return data;
        }

        private bool ReadVariables(XmlTextReader reader, List<SOPParameter> parameters)
        {
            bool stop = false;
            ...
            while (reader.Read())
            {
                ...
                {
                    case XmlNodeType.Element:
                        if (string.Compare(reader.Name, "Variable", true) == 0)
                        {
                            SOPParameter param = ReadVariable(reader);
        ...
        private SOPParameter ReadVariable(XmlTextReader reader)
        {
            bool stop = false;

            string strVariableName = null, strDescription = "";
            Sections.SectionDataDecision.VariableType variableType = Sections.SectionDataDecision.VariableType.UNKNOWN;
            ...
            while (reader.Read())
            {
                ...
                {
                    case XmlNodeType.Element:
                        if (string.Compare(reader.Name, "VariableName", true) == 0)
                        {
                            if (!ReadText(reader, ref strVariableName))
                                return null;
                        }
                        else if (string.Compare(reader.Name, "VariableType", true) == 0)
                        {
                            int nType = 0;

                            if (!ReadInt(reader, ref nType, "VariableType은", "VariableType이"))
                                return null;

                            variableType = Sections.SectionDataDecision.ToVariableType(nType);
                        }
                        else if (string.Compare(reader.Name, "Description", true) == 0)
                        {
                            if (!ReadText(reader, ref strDescription))
                                return null;
                        }
                        else
                            PassElement(reader);
            ...
            if (strVariableName == null || strDescription == null)
                return null;
        ...
        {
            m_dicActionStepUserDefinedConfigName = new Dictionary<ActionStep, string>();
            bool stop = false;
        ...
        private bool ReadActionStep(XmlTextReader reader)
        {
            ...
            string strStepName = "", strUserDefinedConfig = "";
            ...
            while (reader.Read())
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        {
                            if (!ReadActionStepUserDefinedConfig(reader, ref strUserDefinedConfig))
                                return false;

                            // XML 문서에서 UserDefinedConfig를 먼저 읽었다면...
                            if (m_dicUserDefinedConfig != null)
                            {
                                ConfigData data = null;

                                if (m_dicUserDefinedConfig.TryGetValue(strUserDefinedConfig, out data))
                                    actionStep.UserDefinedConfig = data;
                            }
                            else
                                m_dicActionStepUserDefinedConfigName[actionStep] = strUserDefinedConfig;
                        }
                        else
                            PassElement(reader);
        ...
        private bool ReadActionStepUserDefinedConfig(XmlTextReader reader, ref string strUserDefinedConfig)
        {
            if (reader.IsEmptyElement)
                return false;

            return ReadText(reader, ref strUserDefinedConfig);
        }
            ...
            while (reader.Read())
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        {
                            if (!ReadArrow(reader, stepMember.ArrowList))
                                return false;
                        }
                        else
                            PassElement(reader);
        ...
        private PropertyDecision ReadDecisionProperty(XmlTextReader reader)
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    case XmlNodeType.Element:
                        if (string.Compare(reader.Name, "autoRunScript", true) == 0)
                        {
                            string strScript = "";
                            if (ReadText(reader, ref strScript))
                            {
                                property.AutoRunScript = strScript;
                            }
                        }
                        else if (string.Compare(reader.Name, "autoRunScriptVariableTypes", true) == 0)
                        {
                            string strVariableTypes = "";
                            if (ReadText(reader, ref strVariableTypes))
                            {
                                string[] types = strVariableTypes.Split(';');

                                foreach (string strType in types)
                                {
                                    property.VariableTypes.Add(strType);
                                }
                            }
                        }
                        else
        ...
        private PropertyInternal ReadInternalProperty(XmlTextReader reader)
        {
            ...
            bool usePopupMessage = false, useSMS = false, useBroadcast = false, useSiren = false;
            ...
            while (reader.Read())
            {
                switch (reader.NodeType)
                {
                    case XmlNodeType.Element:
                        ...
                        else if (string.Compare(reader.Name, "useSiren", true) == 0)
                        {
                            if (!ReadUseSiren(reader, ref useSiren))
                                return null;

                            //readUseSiren = true;
                            property.UseSiren = useSiren;
                        }
                        ...
                        else if (string.Compare(reader.Name, "autoRun", true) == 0)
                        {
                            bool autoRun = false;
                            if (!ReadBoolean(reader, ref autoRun, "자동실행 여부가", "자동실행 여부는"))
                                return null;

                            property.AutoRun = autoRun;
                        }
                        else
                            PassElement(reader);
                        break;
        ...
        private bool ReadUseSiren(XmlTextReader reader, ref bool useSiren)
        {
            if (reader.IsEmptyElement)
                return false;
            return ReadBoolean(reader, ref useSiren, "useSiren이", "useSiren은");
        }
		...
		private bool Save(XmlTextWriter writer, FormMain frm, string strVersionName, string strDescription = null)
		{
            ...
            if (IOManager.CheckDecisionExpression(frm, out m_strUserDefinedConfigName) == false)
                return false;
        ...
        private bool MakeBody(XmlTextWriter writer, FormMain frm)
        {
            ...
            if (m_strUserDefinedConfigName != null && m_strUserDefinedConfigName.Length > 0)
            {
                ConfigData config = null;
                List<SOPParameter> parameters = frm.GetPageLevel().GetBarConfig().GetCurrentVariables(out config);

                if (config != null && parameters != null)
                    MakeUserDefinedConfig(writer, parameters);
                else
                    m_strUserDefinedConfigName = "";
            }
        ...
        private void MakeUserDefinedConfig(XmlTextWriter writer, List<SOPParameter> parameters)
        {
            writer.WriteStartElement("UserDefinedConfig");
            writer.WriteStartElement("ConfigData");

            writer.WriteStartElement("ConfigName");
            writer.WriteString(m_strUserDefinedConfigName);
            writer.WriteFullEndElement();
            ...
            foreach (SOPParameter param in parameters)
            {
                ...
                writer.WriteString(param.VariableName);
                writer.WriteFullEndElement();

                writer.WriteStartElement("VariableType");
                writer.WriteString(((int)param.Type).ToString());
                writer.WriteFullEndElement();

                writer.WriteStartElement("Description");
                writer.WriteString(param.Description == null ? "" : param.Description);
                writer.WriteFullEndElement();

                writer.WriteFullEndElement();   // Variable
            }

            writer.WriteFullEndElement();   // Variables

            writer.WriteFullEndElement();   // ConfigData
            writer.WriteFullEndElement();   // UserDefinedConfig
        }
        ...
        private bool MakeActionStep(XmlTextWriter writer, ActionStepTabPage tabPage, FormPageSOP pageLevel, Dictionary<TabPage, long> dicActionSteps)
        {
			...
			Data_ActionStep opt = tabPage.Data;
        ...
        private bool MakeStepMember(XmlTextWriter writer, Sections.PanelSectionEx panel, Dictionary<Sections.Section, long> dicSectionInfo, int nStepMemberID)
        {
            ...
            int nTeamType = (int)panel.TeamType;
            writer.WriteString(nTeamType.ToString());
            writer.WriteEndAttribute();
        ...
        private bool MakeDecisionProperty(XmlTextWriter writer, Sections.SectionDecision section)
        {
            ...
            if (data.Expression != null && data.Expression.Length > 0)
            {
                ...
                {
                    string strTypes = pair.Key + "(" + pair.Value.ToString() + ")";

                    if (strVariableTypes.Length == 0)
                        strVariableTypes = strTypes;
                    else
                        strVariableTypes += ";" + strTypes;
                }
                ...
                if (strVariableTypes.Length > 0)
                {
                    ...
                    writer.WriteString(strVariableTypes);
                    writer.WriteFullEndElement();
                }
            }
        ...
        private bool MakeProcessProperty(XmlTextWriter writer, Sections.SectionProcess section)
        {
            ...
            writer.WriteString(data.AutoRun ? "1" : "0");
            writer.WriteFullEndElement();
        ...
        private bool MakeTransSOPProperty(XmlTextWriter writer, Sections.SectionTransSOP section)
        {
			...
			string strStepName = WebDBManager.GetStringField(arrResult[3], "");
			int nParentStepID = WebDBManager.GetIntField(arrResult[4].ToString(), -1);
        ...
        private bool MakeInternalProperty(XmlTextWriter writer, Sections.SectionInternal section)
        {
            ...
            writer.WriteString(data.UseSiren ? "1" : "0");
            writer.WriteFullEndElement();
        ...
        {
            private string m_strAutoRunScript = "";
            private List<string> m_variableTypes = new List<string>();
