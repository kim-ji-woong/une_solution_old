...
namespace SOPManager
{
    ...
    public class Data_ActionStep
    {
        ...
        public static void ReadStandardActionStepName(DBUtility2.WebDBManager dbMgr)
        {
            string strSQL = "Select PropertyValue from OptionSOPSimulator where SiteID = " + dbMgr.SiteID.ToString();
            System.Collections.ArrayList arrResult = dbMgr.GetResultData(strSQL);

            if (arrResult == null || arrResult.Count == 0)
                return;

            string strNames = DBUtility2.WebDBManager.GetStringField(arrResult[0]);

            if (strNames == null)
                return;

            string[] names = strNames.Split(',');
            int nStandardCount = StandardActionStepName.Count();

            if (names.Count() < nStandardCount)
                return;

            for (int i = 0; i < nStandardCount; i++)
            {
                StandardActionStepName[i] = names[i].Trim();
            }

            FormMain.Instance.SopTree.SetStandardActionStepNames();
        }
    }
