...
namespace Sections
{
    public partial class PanelSectionEx : PanelSection
    {
        ...
        private SOPTeam.SOPTeamType m_nTeamType = SOPTeam.SOPTeamType.Normal;
        ...
        private Point m_pPopupPosition;
        private PopupMission m_MissionForm = null;
        private UnE.GUI.DialogFormFrameRibbon m_MissionFrame = null;

        private Section m_sectionEdited = null;
        private Dictionary<SectionProcess, SectionCommander> m_dicProcessCommanders = new Dictionary<SectionProcess, SectionCommander>();

        double WindowRateWidth = 1d;
        double WindowRateHeight = 1d;

        Font _groupContextMenuStrip;
        Font _editContextMenuStrip;
        Font _editTextContextMenuStrip;
        Font _endPointContextMenuStrip;
        Font _popupContextMenuStrip;
        Font _arrowPopupContextMenuStrip;
        Font _panelContextMenuStrip;
        Font _linkContextMenuStrip;
        ...
        public PanelSectionEx()
        {
            ...
            _groupContextMenuStrip = groupContextMenuStrip.Font;
            _editContextMenuStrip = editContextMenuStrip.Font;
            _editTextContextMenuStrip = editTextContextMenuStrip.Font;
            _endPointContextMenuStrip = endPointContextMenuStrip.Font;
            _popupContextMenuStrip = popupContextMenuStrip.Font;
            _arrowPopupContextMenuStrip = arrowPopupContextMenuStrip.Font;
            _panelContextMenuStrip = panelContextMenuStrip.Font;
            _linkContextMenuStrip = linkContextMenuStrip.Font;
        ...
        public void Instance_event_WinRateChanged()
        {
            Double[] dWindowRate = FormMain.Instance.GetCurWindowRate();
            WindowRateWidth = dWindowRate[0];
            WindowRateHeight = dWindowRate[1];

             groupContextMenuStrip.Font = _groupContextMenuStrip;
             editContextMenuStrip.Font = _editContextMenuStrip;
             editTextContextMenuStrip.Font = _editTextContextMenuStrip;
             endPointContextMenuStrip.Font = _endPointContextMenuStrip;
             popupContextMenuStrip.Font = _popupContextMenuStrip;
             arrowPopupContextMenuStrip.Font = _arrowPopupContextMenuStrip;
             panelContextMenuStrip.Font = _panelContextMenuStrip;
             linkContextMenuStrip.Font = _linkContextMenuStrip;
        ...
        {
            FormMain.Instance.UpdateWindowRate(groupContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(editContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(editTextContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(endPointContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(popupContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(arrowPopupContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(panelContextMenuStrip, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(linkContextMenuStrip, WindowRateWidth, WindowRateHeight);
        }
        ...
        protected override void OnPaint(object sender, PaintEventArgs e)        
        {
			...
			if(m_bCurrentPanel == true)
            {
				//int mMinX  = this.Size.Width;
				//int mMinY  = this.Size.Height;                
				//using(SolidBrush brush = new SolidBrush(Color.OrangeRed))
				//{
				//	g.FillEllipse(brush, 10 , mMinY - 30, 20, 20);
				//}
		...
		public virtual void SetViewport(PointF ptOrgin, PointF ptTrans, PointF ptScale)
		{
			m_ptOrigin = ptOrgin;
			m_fTranX = ptTrans.X;
			m_fTranY = ptTrans.Y;

			m_fCurScale = ptScale.X;
			m_fPrevScale = ptScale.Y;
		}

		public virtual void GetViewport(out PointF ptOrgin, out PointF ptTrans, out PointF ptScale)
		{			
			ptOrgin = m_ptOrigin;
			ptTrans = new PointF();
			ptTrans.X = m_fTranX;
			...
			ptScale = new PointF();
			ptScale.X = m_fCurScale;
			ptScale.Y = m_fPrevScale;
		}
        ...
        public void SelectSection(Section section)
        {
            ...
            if (m_listener != null && section != null)
                m_listener.OnSelectedSection(section);

            m_sectionSelected = section;

            if (m_sectionSelected != null)
                m_sectionSelected.Select(true);
        ...
        //private bool m_bDragArrowKnot = false;
        ...
        protected override void OnMouseDown(object sender, MouseEventArgs e)
        {
            ...
            else if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
				...
				if (SelectedSectionList.Count > 0)
				{
                    ...
                    if (SelectedSectionList.Count > 1)
                        popupContextMenuStrip2.Show(this, e.X, e.Y);
                    else
				...
				else
				{
					...
					{
						FormPageSOP form = (FormPageSOP)(this.Parent.Parent.Parent.Parent);
						...
						else
						{
                            ...
                            if (bGroupMember != true)
                            {
                                if( m_sectionSelected.GetComponentType() == Section.ComponentType.PROCESS ||
									...
									m_sectionSelected.GetComponentType() == Section.ComponentType.ANNOTATION)
                                {
                                    // 내용 편집
                                ...
                                else if(m_sectionSelected.GetComponentType() == Section.ComponentType.ENDPOINT)
                                {
									...
									m_sectionSelected.GetComponentType() == Section.ComponentType.TRANSMISSION )
								{
                                    editContextMenuStrip.Show(this, e.X, e.Y);
								}
                                else
            ...
            else if (MovingArrow(e))
            {
                ...
                #region MovingKnot
                //int nCount = m_arrowSelected.KnotCount;

                //int nKnot = m_arrowSelected.SelectKnotPoint;
                // // Moving KnotPoint
                //if( nKnot == 0 || nKnot ==( nCount - 1))                    
                //{
        ...
        private bool LinkSection(float x, float y)
        {
            ...
            if (m_tempArrow.BeginLink != m_tempArrow.EndLink)
            {
        ...
        protected override void OnSectionBeginMove()
        {
            UndoRedoManager.Instance.SaveSnapshot("위치이동");
            base.OnSectionBeginMove();
        ...
        protected override void OnMouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (m_bDragSelectMode == true)
            {
				...
				else
				{
					if (m_listener != null)
					{						
						m_listener.OnSelectedSectionList(null);
					}
				}
                Refresh();
            }
            ...
            #region MovingKnot
            //if ( m_bDragArrowKnot == true)
            //{
            //    m_bDragArrowKnot = false;
            //    Refresh();
            //}
            #endregion
        }
		...
		private void Translate(int prevX, int prevY, int x, int y, bool bRefresh)
		{
			m_ptOrigin.X += (x - prevX);
			m_ptOrigin.Y += (y - prevY);
        ...
        protected override void OnMouseMove(object sender, MouseEventArgs e)
        {
            ...
            //    {
            //        int nKnot = m_arrowSelected.SelectKnotPoint;
            //        PointF pt = m_arrowSelected.GetKnotPoint(nKnot);
            //        if (!pt.IsEmpty)
            //        {
            //            PointF pt1 = ScreenToGlobal(new Point(e.X, e.Y));
            //            PointF pt2 = ScreenToGlobal(m_ptDragCurrent);
            //            float dx = pt2.X - pt1.X;
            //            float dy = pt2.Y - pt1.Y;
            //            m_arrowSelected.MoveKnotPoint(nKnot, pt.X - dx, pt.Y - dy);
        ...
        public SOPTeam.SOPTeamType TeamType
        {
        ...
        void toFrontToolStripMenuItem_Click(object sender, System.EventArgs e)
        {
            TabPage tabPage = (TabPage)this.Parent;
            if (tabPage == null)
                return;

            int nCtrlCount = tabPage.Controls.Count;
            Type type = typeof(PanelSectionEx);

            PanelSectionEx panelNext = null;
            int nCurrentIndex = -1, nNextIndex = -1;

            for (int i = 0; i < nCtrlCount; i++)
            {
                Control ctrl = tabPage.Controls[i];
            ...
            if (nCurrentIndex >= 0 && nNextIndex >= 0)
            {
                FormMain.Instance.GetPageLevel().GetBarPage().Reorder(nNextIndex, nCurrentIndex);
                FormMain.Instance.GetPageLevel().ReorderPanel(panelNext.TeamID, panelNext.TeamType, nCurrentIndex);
                FormMain.Instance.GetPageLevel().ReorderPanel(this.TeamID, this.TeamType, nNextIndex);
                //tabPage.Controls.SetChildIndex(panelNext, nCurrentIndex);
                //tabPage.Controls.SetChildIndex(this, nNextIndex);
                FormMain.Instance.GetPageLevel().PanelResize();
            }
        }


		void pasteSectionsMenuItem_Click(object sender, System.EventArgs e)
		{
			Point pt = panelContextMenuStrip.Bounds.Location;
			PointF ptGlobal = ScreenToGlobal(PointToClient(pt));
			SectionClipboardEx.Instance.Paste(this, ptGlobal);
			
			BeginInvoke( new Action(()=> Refresh()));
		}

        void toBackToolStripMenuItem_Click(object sender, System.EventArgs e)
        {
            TabPage tabPage = (TabPage)this.Parent;
            if (tabPage == null)
                return;
            ...
            PanelSectionEx panelPrev = null;
            int nCurrentIndex = -1, nPrevIndex = -1;
            ...
            for (int i = 0; i < nCtrlCount; i++)
            {
                ...
                if (ctrl.GetType() == type)
                {
                    ...
                    {
                        panelPrev = (PanelSectionEx)ctrl;
                        nPrevIndex = i;
                    }
                }
            }

            if (nCurrentIndex >= 0 && nPrevIndex >= 0)
            {
                FormMain.Instance.GetPageLevel().GetBarPage().Reorder(nCurrentIndex, nPrevIndex);
                FormMain.Instance.GetPageLevel().ReorderPanel(this.TeamID, this.TeamType, nPrevIndex);
                FormMain.Instance.GetPageLevel().ReorderPanel(panelPrev.TeamID, panelPrev.TeamType, nCurrentIndex);
                //tabPage.Controls.SetChildIndex(this, nPrevIndex);
                //tabPage.Controls.SetChildIndex(panelPrev, nCurrentIndex);
                FormMain.Instance.GetPageLevel().PanelResize();
            }
        }
        ...
        public void insertFrontToolStripMenuItem_Click(object sender, System.EventArgs e)
        {

			string szMsg = "현재 패널의 오른쪽에 새로운 패널을 추가합니다.\r\n이 작업은 현재 열려있는 모든 탭들에 영향을 주게 됩니다.\r\n계속하시겠습니까?";
			if (UnE.Utility.UMessageBoxRibbon.Show(this, szMsg, "알림", MessageBoxButtons.YesNo, MessageBoxIcon.Information)
                == DialogResult.Yes)
            {
        ...
        public void insertBackToolStripMenuItem_Click(object sender, System.EventArgs e)
        {
			string szMsg = "현재 패널의 왼쪽에 새로운 패널을 추가합니다.\r\n이 작업은 현재 열려있는 모든 탭들에 영향을 주게 됩니다.\r\n계속하시겠습니까?";
			if (UnE.Utility.UMessageBoxRibbon.Show(this, szMsg, "알림", MessageBoxButtons.YesNo, MessageBoxIcon.Information)
				== DialogResult.Yes)
			{
        ...
        public bool ShowPanelSelection(bool isRightSide)
        {
            TabPage tabPage = (TabPage)this.Parent;
            ArrayList arrOldTeams = GetTabPageTeamList(tabPage);
            if (arrOldTeams == null)
                return false;

            PopupSelectTeam3 frm = new PopupSelectTeam3(this.TeamType, arrOldTeams);
            frm.Text = "새로운 패널 생성";
			...
			if (frame.ShowDialog(this) == DialogResult.OK)
            {
                ...
                int nCurrentIndex = tabPage.Controls.GetChildIndex(this);


				UndoRedoManager.Instance.SaveSnapshot("새로운 패널 추가");

                if (isRightSide)
                {
                    FormMain.Instance.GetPageLevel().AddPanel(data, nCurrentIndex + 1);
                    FormMain.Instance.GetPageLevel().GetBarPage().InsertDataGrid(nCurrentIndex + 1, data);
					
                }
                else
                {
                    FormMain.Instance.GetPageLevel().AddPanel(data, nCurrentIndex);
                    FormMain.Instance.GetPageLevel().GetBarPage().InsertDataGrid(nCurrentIndex, data);
                }
	   ...
	   this.TeamName);
			...
			if (UnE.Utility.UMessageBoxRibbon.Show(strMsg, "알림", MessageBoxButtons.YesNo) == DialogResult.Yes)
			{           
                TabPage tabPage = (TabPage)this.Parent;

                int nPanelIndex = GetTabPagePanelIndex(this, tabPage);
                if (nPanelIndex < 0)
                    return;

				UndoRedoManager.Instance.SaveSnapshot("패널 삭제");

                if (FormMain.Instance.GetPageLevel().GetBarPage().DeleteDataGrid(nPanelIndex))
                {
                    FormMain.Instance.GetPageLevel().DeletePanel(tabPage.Controls.GetChildIndex(this));                  
                    FormMain.Instance.GetPageLevel().PanelResize();
                }
            }
        }
			...
			if (e.ClickedItem.Text == "그룹화")
			{
				...
				{					
					...
					else
					{
						...
						UnE.Utility.UMessageBoxRibbon.Show(szMsg, "그룹생성오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
						ResumeRefresh();
						return;
					}
				}
        ...
        {            
            ...
            section.MovingStartPosition = pt;
        }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            if (!base.ProcessCmdKey(ref msg, keyData))
            {
                ...
                else if( keyData.Equals(Keys.Control | Keys.Z))
                {
                    ...
                    FormMain.Instance.GetPageLevel().SetCurrentPanel(this);
                    FormMain.Instance.GetPageLevel().AutoClickPanel();
                   // Refresh();
                    return true;
                }
		...
		private bool m_bCurrentPanel = false;
		public bool CurrentPanel
		{
			get { return m_bCurrentPanel; }
			set { m_bCurrentPanel = value; }
		}
		...
		private bool CalcSectionBound(out RectangleF rect)
		{
			...
			{
				PointF pt = section.Position;
				PointF pt2 = new PointF(pt.X + section.RectSize.Width, pt.Y + section.RectSize.Height);
				if( pt.X > fMaxX)
				{
					fMaxX = pt.X;
				}
			...
			float width = fMaxX - fMinX;
			float height = fMaxY - fMinY;


			if (m_arrSection.Count == 0)
			{
				rect = new RectangleF(0, 0, 1, 1);
				return false;
			}

			if (m_arrSection.Count == 1)
			{
				Section section = (Section)m_arrSection[0];
				rect = new RectangleF(fMinX, fMinY, section.RectSize.Width, section.RectSize.Height);
				return true;
			}
			...
			rect = new RectangleF(fMinX, fMinY, width, height);
			return true;
		}

		private int CalcRatio(RectangleF rect, Size size,out float rate)
		{
			rate = 1.0f;
			if( size.Width == 0 || size.Height == 0)
				return -1;

			float fWidth = (float)size.Width;
			float fHeight = (float)size.Height;

			float rateY = (fHeight / rect.Height);
			float rateX = (fWidth / rect.Width);
		...
		public void FitView()
		{
			...
			// Zoom을 위한 Boundary를 계산한다.
			RectangleF rect;
			bool bResult = CalcSectionBound(out rect);
			if (bResult == false)
				return;

			float fZoom = 1.0f;
			int nResult = CalcRatio(rect, size, out fZoom);


			// Zoom Center를 Boundary의 Center로 지정한다.
			float ptx = rect.X + rect.Width / 2.0f;
			float pty = rect.Y + rect.Height / 2.0f;
			
			// Center점의 화면좌표를 구한다.
			int x = (int)(ptx * m_fCurScale + m_fTranX);
			int y = (int)(pty * m_fCurScale + m_fTranY);

			// Zoom
			Zoom(x, y, fZoom, false);


			// Zoom으로 바뀌었으므로 Translation을 위한 Boundary를 다시계산한다.
			fZoom = 1.0f;
			nResult = CalcRatio(rect, size, out fZoom);

			// Center점의 화면좌표를 구한다.
			ptx = rect.X + rect.Width / 2.0f;
			pty = rect.Y + rect.Height / 2.0f;
			
			x = (int)(ptx * m_fCurScale + m_fTranX);
			y = (int)(pty * m_fCurScale + m_fTranY);

			// 화면의 Center점을 구한다.
			int scCenterX = this.Width / 2;
			int scCenterY = this.Height / 2;

			// Boundary의 Center를 화면 중심으로 이동시킨다.
			// FitView를 위한 Translation에서는 m_nMoveCenter에 대해 고려할 필요 없다.
			Translate(x, y, scCenterX , scCenterY, false);
		...
		{			
			// 뷰포트를 초기화한다.
			SetViewport(new PointF(0.0f, 0.0f), new PointF(0.0f, 0.0f), new PointF(1.0f, 1.0f));


			// 중심점이 창 중심에 있도록 창의 갯수에따라 X방향으로 이동시켜준다.
			Size size = this.Parent.Size;
			int nPane = (int)((float)size.Width / (float)this.Width + 0.5f) - 1;

			if (nPane <= 0)
				nPane = 1;
			int width = (size.Width - this.Width) / (2 * nPane);

			// 중심점위치를 구하는 작업이므로 MoveCenter에 대해 고려할 필요없다.
			Translate(0, 0, -width, 0, false);			
		}

		
		
		public void MoveCenter(int orgWidth , int newWidth)
		{
			// 크기변화에 따른 값을 X방향으로 이동시켜준다.
			int temp = (orgWidth - newWidth) / 2;
			Translate(0, 0, -temp, 0, false);
			m_nMoveCenter += temp;
		}

		// 크기변화에 따른 X축 이동값
		private int m_nMoveCenter = 0;

		// HomeView에 사용되는 Orgion
		private PointF m_SaveOrgion = new PointF();
		// HomeView에 사용되는 TranslationX
		private float m_SaveTransX = 0.0f;
		// HomeView에 사용되는 TranslationY
		private float m_SaveTransY = 0.0f;
		// HomeView에 사용되는 TranslationY
		private float m_SaveScale = 1.0f;
		private bool m_bHomeViewSaved = false;

        // 홈뷰 저장하기
		public void SaveHomeView()
		{
			m_SaveOrgion = m_ptOrigin;
			m_SaveOrgion.X += m_nMoveCenter;
			m_SaveTransX = m_fTranX + m_nMoveCenter;
			m_SaveTransY = m_fTranY;
			m_SaveScale = m_fCurScale;
			m_bHomeViewSaved = true;
		}

        // 홈뷰로 이동
		public bool HomeView()
		{
			if (m_bHomeViewSaved == false)
				return false;

			m_ptOrigin = m_SaveOrgion;
			m_fTranX = m_SaveTransX - m_nMoveCenter;
			m_ptOrigin.X -= m_nMoveCenter;
			m_fTranY = m_SaveTransY;
			m_fCurScale = m_SaveScale;
			return true;
		}
        ...
        public void ShowEdit(Section section)
        {
            if (section != null)
            {
                Section.ComponentType type = m_sectionSelected.GetComponentType();
                ClosePrevDialogAndShowFrame(type);
            }
            else
            {
                Section.ComponentType type = Section.ComponentType.NONE;
                ClosePrevDialogAndShowFrame(type);
            }
            ...
            else if (section.GetComponentType() == Section.ComponentType.ANNOTATION)
            {
                PopupNoteEdit();
            }
            else if (section.GetComponentType() == Section.ComponentType.ENDPOINT)
            {
                PopupEndPointEdit();
            }
            else if (section.GetComponentType() == Section.ComponentType.DECISION)
            {
                PopupDecisionEdit((SectionDecision)section);
            }
            else
            {
                using (PopupNote form = new PopupNote())
        ...
        private bool IsDialogFrameVisible()
        {
            if (m_MissionFrame != null && m_MissionFrame.Visible == true)
                return true;

            if (m_InternalFrame != null && m_InternalFrame.Visible == true)
                return true;

            if (m_AnnotationFrame != null && m_AnnotationFrame.Visible == true)
                return true;

            if (m_EndpointFrame != null && m_EndpointFrame.Visible == true)
                return true;

            if (m_decisionFrame != null && m_decisionFrame.Visible == true)
                return true;
        ...
        private Section.ComponentType m_CurrentEditType = Section.ComponentType.NONE;
        private void ClosePrevDialogAndShowFrame(Section.ComponentType current)
        {
            if (m_sectionSelected == null || IsDialogFrameVisible() == false)
                return;
                        
            SavePrevSectionData(m_sectionEdited, m_sectionSelected);

            if (m_sectionSelected == null)
                return;

            if (current == Section.ComponentType.PROCESS)
            {
                if (m_AnnotationFrame != null && m_AnnotationFrame.Visible == true)
                {
                    m_AnnotationFrame.Close();
                }
                ...
                if (m_EndpointFrame != null && m_EndpointFrame.Visible == true)
                {
                    m_EndpointFrame.Close();
                }

                if (m_decisionFrame != null && m_decisionFrame.Visible == true)
                {
                    m_decisionFrame.Close();
                }
            }
            else if (current == Section.ComponentType.INTERNAL)
            {
                if (m_AnnotationFrame != null && m_AnnotationFrame.Visible == true)
                {
                    m_AnnotationFrame.Close();
            ...
            else if (current == Section.ComponentType.ANNOTATION)
            {
                if (m_InternalFrame != null && m_InternalFrame.Visible == true)
                {
                    m_InternalFrame.Close();
            ...
            else if (current == Section.ComponentType.ENDPOINT)
            {
                if (m_InternalFrame != null && m_InternalFrame.Visible == true)
                {
                    m_InternalFrame.Close();
            ...
            else if (current == Section.ComponentType.DECISION)
            {
                if (m_InternalFrame != null && m_InternalFrame.Visible == true)
                {
                    m_InternalFrame.Close();
            ...
            if (current != Section.ComponentType.NONE)
            {
                if (m_sectionSelected.GetComponentType() == Section.ComponentType.PROCESS)
                {
                    PopupMissionEdit();
                ...
                else if (m_sectionSelected.GetComponentType() == Section.ComponentType.DECISION)
                {
                    PopupDecisionEdit((SectionDecision)m_sectionSelected);
                }
            }            

            m_CurrentEditType = current;
        }

        private void SavePrevSectionData(Section sectionPrev, Section sectionNow)
        {
            if( sectionPrev == null)
                return;

            if (sectionPrev == sectionNow)
                return;

            m_listener.OnSelectedSection(null);
            m_listener.OnSelectedSection(m_sectionEdited);

            if (sectionPrev.GetComponentType() == Section.ComponentType.PROCESS)
            {
                if (m_MissionFrame != null && m_MissionFrame.Visible == true)
                {
                    m_MissionForm.SaveData();
                }
            }
            else if (sectionPrev.GetComponentType() == Section.ComponentType.INTERNAL)
            {
                if (m_InternalFrame != null && m_InternalFrame.Visible == true)
                {
                    m_InternalForm.SaveData();
                }
            }
            else if (sectionPrev.GetComponentType() == Section.ComponentType.ANNOTATION)
            {
                if (m_AnnotationFrame != null && m_AnnotationFrame.Visible == true)
                {
                    m_AnnotationForm.SaveData();
                }
            }
            else if (sectionPrev.GetComponentType() == Section.ComponentType.ENDPOINT)
            {
                if (m_EndpointFrame != null && m_EndpointFrame.Visible == true)
                {
                    m_EndpointForm.SaveData();                   
                }
            }
            else if (sectionPrev.GetComponentType() == Section.ComponentType.DECISION)
            {
                if (m_decisionFrame != null && m_decisionFrame.Visible == true)
                {
                    if (m_decisionForm.SaveData() == false)
                    {
                        if (m_sectionSelected != null)
                        {
                            m_sectionSelected.Select(false);
        ...
        private PopupNote m_EndpointForm = null;
        private UnE.GUI.DialogFormFrameRibbon m_EndpointFrame = null;

        void m_Frame_LocationChanged(object sender, EventArgs e)
        {
            Form frm = sender as Form;
            if (frm == null) return;

            m_pPopupPosition = frm.Location;
        }
             
        private void PopupEndPointEdit()
        {
            if (m_EndpointForm == null || m_EndpointForm.IsDisposed == true || m_EndpointForm.IsHandleCreated == false)
            {
                m_EndpointForm = new PopupNote();
                m_EndpointForm.StartPosition = FormStartPosition.Manual;
                m_EndpointForm.TopMost = true;
                m_EndpointFrame = new UnE.GUI.DialogFormFrameRibbon(m_EndpointForm);
                //m_EndpointFrame.LocationChanged += (s, eventArg) => { m_pPopupPosition = m_EndpointFrame.Location; };
                m_EndpointFrame.LocationChanged += m_Frame_LocationChanged;
            }

            FormProperties formProperties = FormMain.Instance.GetPageLevel().FormProperties;
            if (formProperties != null)
            {
                SectionPropertiesBase prop = formProperties.GetCurrentProperty();

                if (prop != null && prop is PropertiesEndPoint)
                {

                    SavePrevSectionData(m_sectionEdited, m_sectionSelected);

                    m_sectionEdited = m_sectionSelected;

                    m_EndpointForm.PropertiesEndPoint = (PropertiesEndPoint)prop;
                    if (m_EndpointFrame.Visible == false)
                    {
                        if (m_pPopupPosition.IsEmpty == true)
                        {
                            m_pPopupPosition = FormMain.Instance.PointToScreen(new Point(10, 10));
                        } 
                        
                        m_EndpointFrame.StartPosition = FormStartPosition.Manual;
                        m_EndpointFrame.Location = m_pPopupPosition;
                        
                        m_EndpointFrame.Show(this);
                    }
                }
            }
        }

        private SOPManager.Popup.PopupNoteDecision2 m_decisionForm = null;
        private UnE.GUI.DialogFormFrameRibbon m_decisionFrame = null;

        private void PopupDecisionEdit(SectionDecision section)
        {
            if (m_decisionForm == null || m_decisionForm.IsDisposed == true || m_decisionForm.IsHandleCreated == false)
            {
                BarLevelTree tree = FormMain.Instance.GetPageLevel().GetBarLevelTree();
                m_decisionForm = new SOPManager.Popup.PopupNoteDecision2(tree.GetCurrentCategoryName(), tree.GetCurrentSubCategoryName());
                m_decisionForm.StartPosition = FormStartPosition.Manual;
                m_decisionForm.TopMost = true;
                m_decisionFrame = new UnE.GUI.DialogFormFrameRibbon(m_decisionForm);
                //m_decisionFrame.LocationChanged += (s, eventArg) => { m_pPopupPosition = m_decisionFrame.Location; };
                m_decisionFrame.LocationChanged += m_Frame_LocationChanged;
            }
            ...
            if (formProperties != null)
            {
                ...
                if (prop != null && prop is PropertiesDecision)
                {
                    SavePrevSectionData(m_sectionEdited, m_sectionSelected);

                    m_sectionEdited = m_sectionSelected;
                    m_decisionForm.PropertiesDecision = (PropertiesDecision)prop;

                    if (m_decisionFrame.Visible == false)
                    {
                        if (m_pPopupPosition.IsEmpty == true)
                        {
                            m_pPopupPosition = FormMain.Instance.PointToScreen(new Point(10, 10));
                        ...
                        m_decisionFrame.StartPosition = FormStartPosition.Manual;
                        m_decisionFrame.Location = m_pPopupPosition;

                        m_decisionFrame.Show(this);
                    }
                }
            }
        }

        private PopupNote m_AnnotationForm = null;
        private UnE.GUI.DialogFormFrameRibbon m_AnnotationFrame = null;

        private void PopupNoteEdit()
        {
            if (m_AnnotationForm == null || m_AnnotationForm.IsDisposed == true || m_AnnotationForm.IsHandleCreated == false)
            {
                m_AnnotationForm = new PopupNote();
                m_AnnotationForm.TopMost = true;
                m_AnnotationForm.StartPosition = FormStartPosition.Manual;
                m_AnnotationFrame = new UnE.GUI.DialogFormFrameRibbon(m_AnnotationForm);
                m_AnnotationFrame.Sizable = true;
                //m_AnnotationFrame.LocationChanged += (s, eventArg) => { m_pPopupPosition = m_AnnotationFrame.Location; };
                m_AnnotationFrame.LocationChanged += m_Frame_LocationChanged;
            }
            ...
            if (formProperties != null)
            {
                ...
                if (prop != null && prop is PropertiesAnnotation)
                {
                    SavePrevSectionData(m_sectionEdited, m_sectionSelected);

                    m_sectionEdited = m_sectionSelected;

                    m_AnnotationForm.PropertiesAnnotation = (PropertiesAnnotation)prop;
                    if (m_AnnotationFrame.Visible == false)
                    {
                        if (m_pPopupPosition.IsEmpty == true)
                        {
                            m_pPopupPosition = FormMain.Instance.PointToScreen(new Point(10, 10));
                        }
                        m_AnnotationFrame.StartPosition = FormStartPosition.Manual;
                        m_AnnotationFrame.Location = m_pPopupPosition;                        
                        m_AnnotationFrame.Show(this);
                    }
                }
            }
        }

        private PopupBroadcastMessage m_InternalForm = null;
        private UnE.GUI.DialogFormFrameRibbon m_InternalFrame = null;

        private void PopupInternalEdit()
        {
            if (m_InternalForm == null || m_InternalForm.IsDisposed == true || m_InternalForm.IsHandleCreated == false)
            {
                m_InternalForm = new PopupBroadcastMessage();
                m_InternalForm.TopMost = true;
                m_InternalForm.StartPosition = FormStartPosition.Manual;
                m_InternalFrame = new UnE.GUI.DialogFormFrameRibbon(m_InternalForm);
                m_InternalFrame.Sizable = false;
                //m_InternalFrame.LocationChanged += (s, eventArg) => { m_pPopupPosition = m_InternalForm.Location; };
                m_InternalFrame.LocationChanged += m_Frame_LocationChanged;
            }
            ...
            if (formProperties != null)
            {
                ...
                if (prop != null && prop is PropertiesInternal)
                {
                    SavePrevSectionData(m_sectionEdited, m_sectionSelected);

                    m_sectionEdited = m_sectionSelected;

                    m_InternalForm.PropertiesInternal = (PropertiesInternal)prop;
                    if (m_InternalFrame.Visible == false)
                    {
                        if (m_pPopupPosition.IsEmpty == true)
                        {
                            m_pPopupPosition = FormMain.Instance.PointToScreen(new Point(10, 10));
                        ...
                        m_InternalFrame.StartPosition = FormStartPosition.Manual;
                        m_InternalFrame.Location = m_pPopupPosition;                        
                        m_InternalFrame.Show(this);
                    }
                }
            }
        }

        private void PopupMissionEdit()
        {
            if (m_MissionForm == null || m_MissionForm.IsDisposed == true || m_MissionForm.IsHandleCreated == false)
            {
                m_MissionForm = new PopupMission();
                m_MissionForm.TopMost = true;
                m_MissionForm.StartPosition = FormStartPosition.Manual;
                m_MissionFrame = new UnE.GUI.DialogFormFrameRibbon(m_MissionForm);
                m_MissionFrame.Sizable = true;
                //m_MissionFrame.LocationChanged += (s, eventArg) => { m_pPopupPosition = m_MissionFrame.Location; };
                m_MissionFrame.LocationChanged += m_Frame_LocationChanged;
            }
            ...
            if (formProperties != null)
            {
                ...
                if (prop != null && prop is PropertiesProcess)
                {
                    SavePrevSectionData(m_sectionEdited, m_sectionSelected);

                    m_sectionEdited = m_sectionSelected;

                    m_MissionForm.PropertiesProcess = (PropertiesProcess)prop;                    

                    if (m_MissionFrame.Visible == false)
                    {
                        if (m_pPopupPosition.IsEmpty == true)
                        {
                             m_pPopupPosition = FormMain.Instance.PointToScreen(new Point(10, 10));
                        ...
                        m_MissionFrame.StartPosition = FormStartPosition.Manual;
                        m_MissionFrame.Location = m_pPopupPosition;                        
                        m_MissionFrame.Show(this);
                    }
                }
            }
        }

        private void PanelSectionEx_Click(object sender, EventArgs e)
        {
            if (m_sectionSelected != null)
            {
                Section.ComponentType type = m_sectionSelected.GetComponentType();
        ...
        public void SetProcessCommander(SectionProcess section, SectionCommander commander)
        {
            m_dicProcessCommanders[section] = commander;
        }

        public void RemoveProcessCommander(SectionProcess section)
        {
            m_dicProcessCommanders.Remove(section);
        }

        public SectionCommander GetProcessCommander(SectionProcess section)
        {
            SectionCommander commander = null;

            if (!m_dicProcessCommanders.TryGetValue(section, out commander))
                return null;

            return commander;
        }

        private void popupContextMenuStrip2_Opening(object sender, CancelEventArgs e)
        {

        }

        private void toolStripMenuAlignWidth_Click(object sender, EventArgs e)
        {
            Section target = null;
            float fWidth = 0;

            if (sender == toolStripMenuAlignBigWidth)
            {
                foreach (Section section in SelectedSectionList)
                {
                    float width = section.Shape.GetSize(true);
            ...
            else if (sender == toolStripMenuAlignSmallWidth)
            {
                foreach (Section section in SelectedSectionList)
                {
                    float width = section.Shape.GetSize(true);
            ...
            ChangeSectionSize(true, target, fWidth);
        }

        private void toolStripMenuAlignHeight_Click(object sender, EventArgs e)
        {
            Section target = null;
            float fHeight = 0;

            if (sender == toolStripMenuAlignBigHeight)
            {
                foreach (Section section in SelectedSectionList)
                {
                    float height = section.Shape.GetSize(false);
            ...
            else if (sender == toolStripMenuAlignSmallHeight)
            {
                foreach (Section section in SelectedSectionList)
                {
                    float height = section.Shape.GetSize(false);
            ...
            ChangeSectionSize(false, target, fHeight);
        }

        private void ChangeSectionSize(bool isHorz, Section target, float fSize)
        {
            UndoRedoManager.Instance.SaveSnapshot("크기 변경");
            ...
            if (target != null)
            {
                foreach (Section section in SelectedSectionList)
                {
                    ...
                    if (isHorz)
                        section.RectSize = new SizeF(fSize, section.Shape.GetSize(false));
                    else
                        section.RectSize = new SizeF(section.Shape.GetSize(true), fSize);
                }
