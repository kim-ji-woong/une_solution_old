...
namespace SOPManager
{
	public partial class FormOpenDB : Form
	{
        private class LevelControlData
        {
            ...
            public LevelControlData(bool isChecked, List<int> ids)
            {
        ...
        // Key : Level ID
        // Value : Disaster ID List
        private Dictionary<int, List<int>> m_dicLevelDisasters = new Dictionary<int, List<int>>();
        // Key : Disaster ID
        // Value : Level ID
        private Dictionary<int, int> m_dicDisasterLevel = new Dictionary<int, int>();
        private string m_strVisibleDisasterIDs = null;
        ...
        private void ReadSOPGenUserLevelOptions()
        {
            if (FormMain.Instance.LevelDisasterOption != LevelDisasterOption.Use)
                panelLevelDisaster.Visible = false;
            else
            {
                ShowLevelDisasterItems();
            }
        }

        private void ShowLevelDisasterItems()
        {
            if (FormMain.Instance.LevelItems.Count == 0)
            {
                panelLevelDisaster.Visible = false;
                return;
            }
            ...
            int nPictureBoxPos = 0, nLabelPos = 0;
            int nItemCount = 2;

            foreach (KeyValuePair<string, List<int>> pair in FormMain.Instance.LevelItems)
            {
                if (first)
                {
                    first = false;

                    labelFirstLevelItem.Text = pair.Key;
                    labelFirstLevelItem.Tag = pbFirstLevelItem;
                    pbFirstLevelItem.Tag = new LevelControlData(true, pair.Value);
                    pbFirstLevelItem.Click += pbLevelItem_Click;
                    labelFirstLevelItem.Click += labelLevelItem_Click;

                    int nRightPos = labelFirstLevelItem.Location.X + labelFirstLevelItem.Size.Width;

                    nPictureBoxPos = pbFirstLevelItem.Location.X + nRightPos;
                    nLabelPos = nPictureBoxPos + labelFirstLevelItem.Location.X - pbFirstLevelItem.Location.X;

                    pbPrev = pbFirstLevelItem;
                    labelPrev = labelFirstLevelItem;
                }
                else
                {
                    PictureBox pb = new PictureBox();

                    pb.BackColor = pbPrev.BackColor;
                    pb.BackgroundImage = pbPrev.BackgroundImage;
                    pb.BackgroundImageLayout = pbPrev.BackgroundImageLayout;
                    pb.Location = new System.Drawing.Point(nPictureBoxPos, pbPrev.Location.Y);
                    pb.Name = "pbLevelItem" + nItemCount.ToString();
                    pb.Size = pbPrev.Size;
                    pb.SizeMode = pbPrev.SizeMode;
                    pb.TabIndex = pbPrev.TabIndex + 1;
                    pb.TabStop = pbPrev.TabStop;
                    pb.Tag = new LevelControlData(true, pair.Value);
                    pb.Click += pbLevelItem_Click;

                    pbPrev.Parent.Controls.Add(pb);

                    Label label = new Label();

                    label.AutoSize = labelPrev.AutoSize;
                    label.BackColor = labelPrev.BackColor;
                    label.Font = labelPrev.Font;
                    label.ForeColor = labelPrev.ForeColor;
                    label.Location = new System.Drawing.Point(nLabelPos, labelPrev.Location.Y);
                    label.Margin = labelPrev.Margin;
                    label.Name = "labelLevelItem" + nItemCount.ToString();
                    label.TabIndex = pb.TabIndex + 1;
                    label.Text = pair.Key;
                    label.Tag = pb;
                    label.Click += labelLevelItem_Click;

                    int nRightPos = label.Location.X + label.Size.Width;

                    nPictureBoxPos = pbFirstLevelItem.Location.X + nRightPos;
                    nLabelPos = nPictureBoxPos + label.Location.X - pb.Location.X;

                    pbPrev.Parent.Controls.Add(label);
            ...
            ReadLevelDisasterDatas();
            panelLevelDisaster.Visible = true;
        }

        private void ReadLevelDisasterDatas()
        {
            string strSQL = "Select LevelID, DisasterID from SOPGenLevelDisaster where SiteID = " + m_dbMgr.SiteID.ToString();
            ...
            for (int i = 0; i < nResultCount - 1; i += 2)
            {
                ...
                if (m_dicLevelDisasters.TryGetValue(levelID.Data, out ids) == false)
                {
                    ids = new List<int>();
                    m_dicLevelDisasters[levelID.Data] = ids;
                }

                ids.Add(disasterID.Data);
            }
        }

        private void labelLevelItem_Click(object sender, EventArgs e)
        {
            Label label = (Label)sender;
            pbLevelItem_Click((PictureBox)label.Tag, null);
        }

        private void pbLevelItem_Click(object sender, EventArgs e)
        {
            ...
            // 중복을 막기 위하여 Dictionary
            Dictionary<int, int> dicDisasterIDs = new Dictionary<int, int>();
            List<int> disasterIDs;
            bool allChecked = true;

            foreach (Control ctrl in panelLevelDisaster.Controls)
            {
                if (ctrl is PictureBox)
                {
                    LevelControlData _data = (LevelControlData)ctrl.Tag;

                    if (_data.IsChecked)
                    {
                        foreach (int nLevelID in _data.LevelIDs)
                        {
                            if (m_dicLevelDisasters.TryGetValue(nLevelID, out disasterIDs))
                            {
                                foreach (int nDisasterID in disasterIDs)
                                {
                                    dicDisasterIDs[nDisasterID] = nDisasterID;
                                }
                            }
                        }
                    }
            ...
            if (allChecked)
                m_strVisibleDisasterIDs = null;
            else
            {
                disasterIDs = dicDisasterIDs.Keys.ToList();
                m_strVisibleDisasterIDs = "";

                foreach (int nDisasterID in disasterIDs)
                {
                    if (m_strVisibleDisasterIDs.Length == 0)
                        m_strVisibleDisasterIDs = nDisasterID.ToString();
                    else
                        m_strVisibleDisasterIDs += "," + nDisasterID.ToString();
                }
            }
            ...
            // Scroll바가 Tree의 제일 위를 향하도록 한다.
            if (treeViewSOP.Nodes.Count > 0)
                treeViewSOP.Nodes[0].EnsureVisible();
        }
        ...
        private void OnClickDeleteSOP()
        {
            ...
            if (m_selectedVersion != null)
            {
				...
				if(mgr.IsMonitoringSOPVersion(m_dbMgr, nVersionID, null))
				{
				...
				if (UnE.Utility.UMessageBoxRibbon.Show(this, szText, "경고", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    //if (m_dbMgr.BeginBatch())
                    {
                        ...
                        if (mgr.DeleteSOPVersion(m_dbMgr, m_selectedVersion.VersionID, true, rollback))
                        {
                            #region 열려있는 버전이면 닫기
        ...
        private void LoadVersion()
        {
            ...
            for (int i = 0; i < nCount - 9; i += 10)
            {
                ...
                string strMemberName = GetCompanyMemberName(nCompanyMemberID, strUserID);
        ...
        private string GetCompanyMemberName(int nCompanyMemberID, string strUserID)
        {
        ...
        private void LoadSOP()
        {
            ...
            if (m_strVisibleDisasterIDs == null)
                szText += " ORDER BY DisasterName";
            else
                szText += "and d.ID in (" + m_strVisibleDisasterIDs + ") ORDER BY DisasterName";
            ...
            foreach (KeyValuePair<int, DisasterInfo> pair in dicDisaster)
            {
                pair.Value.ActionSteps.Sort();
            }
    ...
    public class ActionStepInfo : IComparable
    {
        ...
        {
            ActionStepInfo actionStep = (ActionStepInfo)obj;

            int nActionStepIndex1 = GetActionStepIndex();
            int nActionStepIndex2 = actionStep.GetActionStepIndex();

            if (nActionStepIndex1 > nActionStepIndex2)
                return 1;
            else if (nActionStepIndex1 < nActionStepIndex2)
                return -1;
            //else
            return 0;
        }

        private int GetActionStepIndex()
        {
            int nCount = UnE.SOP.Sections.SectionTabControl.StandardActionStepNames.Count();

            for (int i=0;i<nCount;i++)
            {
                if (m_strActionStepName == UnE.SOP.Sections.SectionTabControl.StandardActionStepNames[i])
                    return i;
            }
