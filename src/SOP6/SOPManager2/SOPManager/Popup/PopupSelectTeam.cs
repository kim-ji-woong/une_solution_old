...
namespace SOPManager
{
    public partial class PopupSelectTeam : Form
    {
		...
		private string GetOriginalTeam(ref string strTeamType)
		{
			string strValue = "";
			int nCount = 0;
            ...
            if (mType == Section.ComponentType.PROCESS)
            {
                ...
                {
                    if (strTeamType.Length == 0)
                        strTeamType = team.TeamType.ToString();
                    else
                        strTeamType = "," + team.TeamType.ToString();
        ...
        private string GetSelectedTeam(ref string strTeamType)
        {
            string strValue = "";
            int nRow = 0;
            ...
            foreach (DataGridViewRow row in dataGridView.Rows)
            {
                if (row.Cells[0].Tag != null && row.Cells[0].Tag is SOPTeam)
                {
                    SOPTeam team = (SOPTeam)row.Cells[0].Tag;
        ...
        private void SelectTeam()
        {
            ...
            string strOrgTeamType = "", strTeamType = "";

            if (!m_bSelectedTeamUpdateUserDefine)
            {
                strOrgValue = GetOriginalTeam(ref strOrgTeamType);
                strValue = GetSelectedTeam(ref strTeamType);
            }

            SaveUserDefinedList();

            if (strOrgValue != strValue || strOrgTeamType != strTeamType || m_bSelectedTeamUpdateUserDefine)
            {
                ApplySelectedTeam();
