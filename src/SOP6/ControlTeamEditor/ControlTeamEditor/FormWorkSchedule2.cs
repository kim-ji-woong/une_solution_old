...
namespace ControlTeamEditor
{
    ...
    {
        private class ControlTeamUI
        {
            private ComboBoxDataGridView grid = null;
            private Point gridLocation = new Point();
            private Label title = null;
            private Label memberInfo = null;
            private Panel panel1 = null, panel2 = null, panel3 = null, panel4 = null, panel5 = null;
            private bool visible = true;
            private DataControlTeam m_currentTeam = null;
            private int m_nTeamIndex = -1;

            public ComboBoxDataGridView Grid
            {
                get { return grid; }
                set { grid = value; }
            }
            ...
            public DataControlTeam CurrentTeam
            {
                get { return m_currentTeam; }
                set { m_currentTeam = value; }
            }

            public int TeamIndex
            {
                get { return m_nTeamIndex; }
                set { m_nTeamIndex = value; }
            }
            ...
            public ControlTeamUI()
            {
            }

            public ControlTeamUI(ComboBoxDataGridView grid, Label title, Label memberInfo, Panel panel1, Panel panel2, Panel panel3, Panel panel4, Panel panel5)
            {
                this.grid = grid;
                this.title = title;
                ...
                this.panel4 = panel4;
                this.panel5 = panel5;
                this.gridLocation = grid.Location;
            }
        ...
        private List<ControlTeamUI> m_controlTeamUIs = new List<ControlTeamUI>();
        ...
        public FormWorkSchedule2(int nSiteID)
        {
            ...
            m_controlTeamUIs.Add(new ControlTeamUI(dataGridControlRoomScheduleA, labelControlRoomA, labelMemberInfoA, panel1, panel2, panel3, panel4, panel5));
            m_controlTeamUIs.Add(new ControlTeamUI(dataGridControlRoomScheduleB, labelControlRoomB, labelMemberInfoB, panel15, panel14, panel13, panel12, panel11));
            m_controlTeamUIs.Add(new ControlTeamUI(dataGridControlRoomScheduleC, labelControlRoomC, labelMemberInfoC, panel10, panel9, panel8, panel7, panel6));
            m_controlTeamUIs.Add(new ControlTeamUI(dataGridControlRoomScheduleD, labelControlRoomD, labelMemberInfoD, panel20, panel19, panel18, panel17, panel16));
        ...
        private void SetControlRoom()
        {
            ...
            m_gridControlRooms.Add(dataGridControlRoomScheduleE);
            m_gridControlRooms.Add(dataGridControlRoomScheduleF);
            m_gridControlRooms.Add(dataGridControlRoomScheduleG);
            m_gridControlRooms.Add(dataGridControlRoomScheduleH);
            ...
            dataGridControlRoomScheduleE.RowHeadersWidth = dataGridControlRoomScheduleF.RowHeadersWidth = dataGridControlRoomScheduleG.RowHeadersWidth = dataGridControlRoomScheduleH.RowHeadersWidth = 200;
            ...
            foreach (ControlTeamUI ui in m_controlTeamUIs)
            {
                ui.Visible = false;
            }
            ...
            if (nTeamCount <= m_controlTeamUIs.Count)
            {
                pbBack.Visible = pbGo.Visible = false;
            }
            ...
            for (int i = 0; i < nTeamCount && i < nGridCount; i++)
            {
                ...
                if (i < m_controlTeamUIs.Count)
                    SetControlTeam(m_controlTeamUIs[i], team, i);
                else
                {
                    m_gridControlRooms[i].Tag = team;
                    //m_gridControlRooms.Add(CloneDataGridView(dataGridControlRoomScheduleA));
                }
            ...
            if (nGridCount > nTeamCount)
            {
                for (int i=nGridCount-1;i>=nTeamCount;i--)
                {
                    m_gridControlRooms.RemoveAt(i);
                }
            }
        ...
        /*private DataGridView CloneDataGridView(DataGridView grid)
        {
            ...
            {
                gridView.Rows.Add((DataGridViewRow)row.Clone());
            }

            gridView.AllowUserToAddRows = grid.AllowUserToAddRows;
            gridView.AllowUserToDeleteRows = grid.AllowUserToDeleteRows;
            gridView.AllowUserToResizeColumns = grid.AllowUserToResizeColumns;
            gridView.AllowUserToResizeRows = grid.AllowUserToResizeRows;
            gridView.BackgroundColor = grid.BackgroundColor;
            ...
            gridView.MultiSelect = grid.MultiSelect;
            gridView.ReadOnly = grid.ReadOnly;
            gridView.RowHeadersDefaultCellStyle = grid.RowHeadersDefaultCellStyle;
            gridView.RowHeadersWidthSizeMode = grid.RowHeadersWidthSizeMode;
            gridView.ScrollBars = grid.ScrollBars;
            gridView.SelectionMode = grid.SelectionMode;
            gridView.Size = grid.Size;

            gridView.CellClick += dataGridControlRoomSchedule_CellClick;
            gridView.CellDoubleClick += dataGridControlRoomSchedule_CellDoubleClick;
            gridView.CellEnter += dataGridControlRoomSchedule_CellEnter;
            gridView.CellPainting += dataGrid_CellPainting;
            gridView.CellValueChanged += dataGridControlRoomSchedule_CellValueChanged;
            gridView.KeyDown += dataGrid_KeyDown;

            return gridView;
        }*/

        private void SetControlTeam(ControlTeamUI ui, DataControlTeam team, int nIndex)
        {
            ui.CurrentTeam = team;
            ui.TeamIndex = nIndex;
            ui.Title.Text = m_controlRoomType.RoomType + " " + team.TeamName;

            /*int nGridIndex = m_controlTeamUIs.IndexOf(ui);

            if (nGridIndex >= 0)
            {
                ComboBoxDataGridView comboGrid = m_gridControlRooms[nGridIndex];
                comboGrid.Tag = team;
            }*/

            ComboBoxDataGridView grid = m_gridControlRooms[nIndex];
            grid.Tag = team;

            if (ui.Grid != grid)
            {
                int nCurrentIndex = m_gridControlRooms.IndexOf(ui.Grid);
                int nNextIndex = m_gridControlRooms.IndexOf(grid);

                ControlTeamUI firstUI = m_controlTeamUIs[0];
                ControlTeamUI lastUI = m_controlTeamUIs[m_controlTeamUIs.Count - 1];
                
                if (ui == firstUI)
                {
                    if (nCurrentIndex < nNextIndex)
                        ui.Grid.Hide();
                    else
                    {
                        if (ui == lastUI)
                ...
                ui.Grid = grid;
                grid.Location = ui.GridLocation;
                grid.Show();

                m_dicGridViewLabel[ui.Grid] = ui.MemberInfo;
            }
        ...
        private void dataGridControlRoomSchedule_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (row.Tag == null)
            {
                ...
                dataGridControlRoomScheduleE.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = cell.Value;
                dataGridControlRoomScheduleF.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = cell.Value;
                dataGridControlRoomScheduleG.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = cell.Value;
                dataGridControlRoomScheduleH.Rows[e.RowIndex].Cells[e.ColumnIndex].Value = cell.Value;
        ...
        private void pbBack_Click(object sender, EventArgs e)
        {
            ...
            foreach (ControlTeamUI ui in m_controlTeamUIs)
            {
                int nIndex = ui.TeamIndex - 1;

                if (nIndex < 0 || nIndex >= teams.Count)
                    continue;

                DataControlTeam team = teams[nIndex];
                SetControlTeam(ui, team, nIndex);
            }

            ControlTeamUI firstUI = m_controlTeamUIs[0];
            pbBack.Enabled = firstUI.TeamIndex > 0;
            pbGo.Enabled = true;
            pbBack.Refresh();
            pbGo.Refresh();
        }
        ...
        private void pbGo_Click(object sender, EventArgs e)
        {
            ...
            ControlTeamUI lastUI = null;

            foreach (ControlTeamUI ui in m_controlTeamUIs)
            {
                int nIndex = ui.TeamIndex + 1;

                if (nIndex >= teams.Count)
                    continue;
            ...
            if (lastUI != null)
            {
                pbGo.Enabled = lastUI.TeamIndex < teams.Count - 1;
                pbGo.Refresh();
            }
