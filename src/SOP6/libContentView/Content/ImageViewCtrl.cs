...
namespace UnE.View.Content
{
    public partial class ImageViewCtrl : Panel, ISensorTooltipOwner, IBaseView
	{
        ...
        // 충남대 전용
        private List<ImageOption> m_BaseImagesOnlySite102 = new List<ImageOption>(); // 충남대 보운캠퍼스 전용
        private Size m_nLod1ImageSizeOnlySite102 = new Size(); // 충남대 보운캠퍼스 전용
        private bool m_bShowEquipmentZoneOnlySite102 = false; // 재난이 보운캠퍼스인지 여부 보운캠퍼스일때 false (충남대만 사용)
        private Matrix rememberCampus = null; // 충남대 2D View 전환할때 마지막 위치 기억
        private float rememberScaleCampus = -1; // 충남대 2D View 전환할때 마지막 위치 기억
        private int rememberLodLevelCampus = -1; // 충남대 2D View 전환할때 마지막 위치 기억

        private bool m_bIsChangeCampus = false; // false 가 기본, true는 충남대 보운캠퍼스일때
        public bool IsCampus
        {
            get { return m_bIsChangeCampus; }
        }
        ...
        { 
            DBUtility2.Utility utill = new DBUtility2.Utility();
        ...
        public void InitImage(string siteID)
        {
            DBUtility2.Utility utill = new DBUtility2.Utility();
            string path = utill.getinivalue("SDMS", "2d_image_path") + siteID;
            ...
            for (int i = 0; i < dirs.Length; i++)
            {
                ...
                if (!dirName.Contains("LOD"))
                    continue;

                string dirCutName = dirName.Replace("LOD", "");
                int nLodLevel = -1;
                if (!int.TryParse(dirCutName, out nLodLevel))
                    continue;

                m_nMaxLodLevel = System.Math.Max(m_nMaxLodLevel, nLodLevel);

                string filePath = path + @"\" + dirName;
                DirectoryInfo dir2 = new DirectoryInfo(filePath);

                foreach (FileInfo file in dir2.GetFiles())
                {
                    string strExtension = file.Extension;
                    if (strExtension.ToUpper() != ".PNG" && strExtension.ToUpper() != ".JPG")
                        continue;
                    ...
                    int nRow = CheckNumber(fileNames[0]);
                    int nColumn = CheckNumber(fileNames[1]);
                    ...
                    imgOption.ImagePath = file.FullName;
                    ...
                    if (siteID == 102 + "-1") //충남대 보운캠퍼스
                    {
                        m_BaseImagesOnlySite102.Add(imgOption);

                        if (nLodLevel == 1 && imgOption.RowIndex == 1)
                            m_nLod1ImageSizeOnlySite102.Width += imgOption.Width;
                        if (nLodLevel == 1 && imgOption.ColumnIndex == 1)
                            m_nLod1ImageSizeOnlySite102.Height += imgOption.Height;
                    }
                    else
                    {
                        ...
                        if (nLodLevel == 1 && imgOption.RowIndex == 1)
                            m_nLod1ImageSize.Width += imgOption.Width;
                        if (nLodLevel == 1 && imgOption.ColumnIndex == 1)
                            m_nLod1ImageSize.Height += imgOption.Height;
                    }
                }
            }
        ...
        private void ResetImageSize()
        {
            ...
            List<ImageOption> options = null;
            if (m_bIsChangeCampus)
                options = m_BaseImagesOnlySite102;
            else
                options = m_BaseImages;

            foreach (ImageOption imgOption in options)
            {
        ...
        {            
            ...
            { 
                ...
                foreach (ImageOption imgOption in options)
                {
                    ...
                    {
                        drawX = mRectImage.X;
                        drawY += prevHeight - 1; // -1:이미지 경계 없애기 위해서
                        horizImgCnt = -1;
                    }
                    else if (prevRow > 0)
                    {
                        drawX += prevWidth - 1; // -1:이미지 경계 없애기 위해서
                        horizImgCnt++;
                    }
                    
                    prevRow = imgOption.RowIndex;
                    prevWidth = imgOption.Width;
                    prevHeight = imgOption.Height;

                    // 화면에서 보이지 않는 영역은 그리지 않음
                    double tempX = (drawX * mTransform.Elements[0]) + mTransform.Elements[4];
                    double tempY = (drawY * mTransform.Elements[3]) + mTransform.Elements[5];

                    Point pt = new Point((int)tempX, (int)tempY);
                    if (pt.X + (imgOption.Width * mTransform.Elements[0]) >= 0 && pt.X < this.Width)
                    {
                        if (pt.Y + (imgOption.Height * mTransform.Elements[3]) >= 0 && pt.Y < this.Height)
                        {
                    ...
                    Image img = Image.FromFile(imgOption.ImagePath);
            ...
            {
                // 충남대라면 재난위치와 캠퍼스가 일치할때만 그린다
                if (UnE.SOP.ProxySOP.Instance.SiteID != 102 || (UnE.SOP.ProxySOP.Instance.SiteID == 102 &&
                                                               (m_bShowEquipmentZoneOnlySite102 && m_bIsChangeCampus) ||
                                                               (!m_bShowEquipmentZoneOnlySite102 && !m_bIsChangeCampus)))
                {
                    g.ResetTransform();
                    List<Point> showPTs = new List<Point>();

                    for (int i = 0; i < m_showPolygonList.Count; i++)
                    {
                        Point polygonPT = ConvertPT(m_showPolygonList[i].x, m_showPolygonList[i].y);

                        showPTs.Add(polygonPT);
                    }
                    GraphicsPath path = new GraphicsPath();
                    path.AddLines(showPTs.ToArray());
            ...
            {                
                ...
                {                    
                    for (int i = 0; i < m_arrBuildingGroupText.Count; i += 3)
                    {
                        string strBuildingName = m_arrBuildingGroupText[i].ToString();
                        float x = (float)m_arrBuildingGroupText[i + 1];
                        float y = (float)m_arrBuildingGroupText[i + 2];

                        Point beginPT = ConvertPT(x, y);

                        if (!m_bIsChangeCampus && strBuildingName == "보운캠퍼스")
                            continue;
                        else if (m_bIsChangeCampus && strBuildingName == "대덕캠퍼스")
                            continue;

                        Font font = null;
                        if (strBuildingName == "보운캠퍼스")
                            font = new System.Drawing.Font(this.prgFont, 30F * mTransform.Elements[0], System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
                        else
                            font = new System.Drawing.Font(this.prgFont, 52F * mTransform.Elements[0], System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(129)));

                        SizeF fontSize = g.MeasureString(strBuildingName, font);
                        
                        RectangleF rect2 = new RectangleF(mTransform.Elements[4] + 1, beginPT.Y - 5, mSizeImage.Width * mTransform.Elements[0] - horizImgCnt, fontSize.Height + 10);

                        g.FillRectangle(brushWhite2, rect2);
                        g.DrawString(strBuildingName, font, brushBlack, beginPT.X, beginPT.Y);
                    }
                    
                    for (int i = 0; i < m_arrBuildingText.Count; i += 3)
                    {
                        string strBuildingName = m_arrBuildingText[i].ToString();
                        float x = (float)m_arrBuildingText[i + 1];
                        float y = (float)m_arrBuildingText[i + 2];

                        Point beginPT =  ConvertPT(x, y);

                        Building building = ZoneManager.Instance.FindBuilding(strBuildingName);
                        if (building != null)
                        {
                            if (!m_bIsChangeCampus && building.BuildingGroup.BuildingGroupName == "보운캠퍼스")
                                continue;
                            else if (m_bIsChangeCampus && building.BuildingGroup.BuildingGroupName == "대덕캠퍼스")
                                continue;
                        }

                        Font font = null;

                        if (building.BuildingGroup.BuildingGroupName == "보운캠퍼스")
                            font = new System.Drawing.Font(this.prgFont, 11F * mTransform.Elements[0], System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
                        else
                            font = new System.Drawing.Font(this.prgFont, 13F * mTransform.Elements[0], System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
                        
                        SizeF fontSize = g.MeasureString(strBuildingName, font);
                        RectangleF rect = new RectangleF(beginPT.X - 5, beginPT.Y - 5, fontSize.Width + 10, fontSize.Height + 10);

                        g.FillRectangle(brushBlack2, rect);
                        g.DrawString(strBuildingName, font, brushWhite, beginPT.X, beginPT.Y);
                    }
                }
                else
                {
        ...
        private Brush brushBlack2 = new SolidBrush(Color.FromArgb(153, 0x00, 0x00, 0x00));
        private Brush brushWhite2 = new SolidBrush(Color.FromArgb(153, 0xff, 0xff, 0xff));
        private Brush brushBlue = new SolidBrush(Color.Blue);

        public string prgFont = "나눔스퀘어";
        ...
        private Point ConvertPT(float x, float y)
        {
            Size size;
            if (m_bIsChangeCampus)
                size = m_nLod1ImageSizeOnlySite102;
            else
                size = m_nLod1ImageSize;
                
            // LOD 1단계일때 지점(%) : a
            double xPer = x / size.Width * 100;
            double yPer = y / size.Height * 100;
        ...
        {
            int nPOICount = poiIDs.Count;

            for (int i=0;i<nPOICount;i++)
            {
                int nID = poiIDs[i];
                string strType = poiTypes[i];
                bool visible = poiVisibles[i];
                ShowIconPOI(nID, strType, visible);
            }
        }
        ...
        #region 신호 발생 전 위치, 스케일 - 신호 해제 후 적용
        private Matrix m_rememberMatrix = null;
        private float m_rememberScale = -1.0f;
        private int m_rememberLodLevel = -1; 
        #endregion
        ...
        { 
            ...
            if (UnE.SOP.ProxySOP.Instance.SiteID == 102)
            {
                ...
                {
                    if (zone.Building.BuildingGroup.BuildingGroupName == "보운캠퍼스")
                        m_bShowEquipmentZoneOnlySite102 = true;
                    else
                        m_bShowEquipmentZoneOnlySite102 = false;
                }
            }
            ShowPolygon();
        }
        ...
        private void ShowPolygon()
        {
            if (m_showPolygonList.Count > 0 && m_bShowEquipmentZone)
            {
                ...
                m_rememberMatrix = new Matrix(ele0, ele1, ele2, ele3, ele4, ele5);
                m_rememberScale = m_nCurScale;
                m_rememberLodLevel = m_nCurLodLevel;
                ...
                for (float i = m_nMinScale; i < m_nMaxScale; i += m_nScaleGap)
                {
                    ...
                    //Size tempScreenSize = new Size(m, height);
                    Point tempMinPt = PercentLodLevel1(minX, minY, mSizeImage, i);
                    Point tempMaxPt = PercentLodLevel1(maxX, maxY, mSizeImage, i);
        ...
        public void HideAllEquipmentZone()
        {
            ...
            if (m_rememberMatrix != null)
            {
                //if (m_bShowEquipmentZoneOnlySite102 != m_bIsChangeCampus)
                //{
                //    SetChangeCampus();
                //}


                float ele0 = m_rememberMatrix.Elements[0];
                float ele1 = m_rememberMatrix.Elements[1];
                float ele2 = m_rememberMatrix.Elements[2];
                float ele3 = m_rememberMatrix.Elements[3];
                float ele4 = m_rememberMatrix.Elements[4];
                float ele5 = m_rememberMatrix.Elements[5];

                mTransform = new Matrix(ele0, ele1, ele2, ele3, ele4, ele5);
                m_rememberMatrix = null;

                //m_nCurLodLevel = m_nMinLodLevel;

                if (m_rememberScale > 0)
                {
                    m_nCurScale = m_rememberScale;
                    m_rememberScale = -1.0f;
                }
                if (m_rememberLodLevel > 0)
                {
                    m_nCurLodLevel = m_rememberLodLevel;
                    m_rememberLodLevel = -1;
                }
        ...
        /// <returns></returns>
        private Point PercentLodLevel1(float x, float y, Size tempSize, float scale)
        {
            Size size;
            if (m_bIsChangeCampus)
                size = m_nLod1ImageSizeOnlySite102;
        ...
        public bool GetVisibleCompass()
        {
            string strQuery = "Select PropertyValue From OptionSDMS Where PropertyName = 'VisibleCompass'";
            UnE.View.Content.IFormContentOwner owner = UnE.View.Content.ViewUtils.GetContentViewOwner();
            DBUtility2.WebDBManager dbMgr = owner.DBManager;
            ArrayList arrResult = dbMgr.GetResultData(strQuery);
            if (arrResult != null || arrResult.Count > 0)
            {
                string strResult = DBUtility2.WebDBManager.GetStringField(arrResult[0].ToString(), "");
                if (strResult == "0" || strResult.ToUpper() == "FALSE")
                    return false;
            }
        ...
        public void SetChangeCampus()
        {
            if (UnE.SOP.ProxySOP.Instance.SiteID != 102)
                return;

            m_bIsChangeCampus = !m_bIsChangeCampus;

            Matrix temp = mTransform;
            float tempScale = m_nCurScale;
            int tempLodLevel = m_nCurLodLevel;
            if (rememberCampus == null)
            {
                // 충남대학교 보운캠퍼스 초기 값
                rememberCampus = new Matrix(1.125f, 0, 0, 1.125f, 174.617844f, -9.515579f);
                rememberScaleCampus = 1.125f;
                rememberLodLevelCampus = 1;
            }

            if (rememberCampus != null)
            {                
                mTransform = rememberCampus;
                mTransform = new Matrix(rememberCampus.Elements[0],
                                        rememberCampus.Elements[1],
                                        rememberCampus.Elements[2],
                                        rememberCampus.Elements[3],
                                        rememberCampus.Elements[4],
                                        rememberCampus.Elements[5]);

                m_nCurScale = rememberScaleCampus;
                m_nCurLodLevel = rememberLodLevelCampus;
            }

            rememberCampus = temp;
            rememberScaleCampus = tempScale;
            rememberLodLevelCampus = tempLodLevel;
