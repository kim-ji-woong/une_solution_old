...
namespace SDMS
{
	public class EditPumpPressuerSensor : ChangedData, IDisposable
	{
		private PumpPressureSensor m_Sensor = null;

		public EditPumpPressuerSensor(PumpPressureSensor sensor)
		{
			m_Sensor = sensor;
		...
		private bool Insert(WebDBManager dbMgr)
		{
			...
			string strSQL = "Select max(id) from PumpPressureSensor";
			...
			strSQL = string.Format("Insert into PumpPressureSensor (ID, Name, PositionName, X, Y, Z, ZoneID, IsIndoor, Description) values ({0}, '{1}', '{2}', {3}, {4}, {5}, {6}, {7}, NULL)",
                nID, "PumpPressureSensor", zone.DisplayText, poi.X, poi.Y, poi.Z, zone.ID, poi.IsIndoor ? 1 : 0);
			...
			nSensorZoneID = WebDBManager.GetIntField(arrResult[0].ToString(), 0) + 1;
			...
			strSQL = string.Format("Insert into SensorZone (ID, Type, Connected, EquipZoneID, Data, Description, OrgSensorID, Zone) values ({0}, {1}, {2}, {3}, {4}, '{5}', {6}, {7})",
			   nSensorZoneID, (int)IFacility.FacilityType.PRESSURE_SENSOR, "NULL", m_Sensor.EquipZoneID, "NULL", m_Sensor.Description, nID, zone.ID);
			...
			{
				poi.UpdateDBData();
				if (SensorManager.Instance.DicPressureSensor.ContainsKey(nID))
					SensorManager.Instance.DicPressureSensor.Remove(nID);

                List<ISensor> sensors = null;

                if (SensorManager.Instance.DicPressureSensor.TryGetValue(nID, out sensors) == false)
                {
                    sensors = new List<ISensor>();
                    SensorManager.Instance.DicPressureSensor[nID] = sensors;
                    sensors.Add(m_Sensor);
                }
                else
                {
                    bool exist = false;
				...
				//SensorManager.Instance.DicPressureSensor.Add(nID, m_Sensor);
		...
		override public bool Update(WebDBManager dbMgr)
		{
			...
			if (IsDeleting)
			{
				string strSQL = string.Format("Delete from PumpPressureSensor where ID = {0}", m_Sensor.OrgSensorID);
				...
				SensorManager.Instance.DicPressureSensor.Remove(m_Sensor.ID);
			...
			string strSQL2 = string.Format("Update PumpPressureSensor set {0} where id = {1}", strField, m_Sensor.OrgSensorID);
		...
		override public void AddToManager(IChangedDataManager mgr)
		{
			...
			Type type = typeof(EditPumpPressuerSensor);

			foreach (ChangedData data in arrDatas)
			{
				if (data.GetType() == type)
				{
					EditPumpPressuerSensor sensor = (EditPumpPressuerSensor)data;
