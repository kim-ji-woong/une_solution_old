...
namespace SDMS
{
	public class EditFireSensor : ChangedData, IDisposable
	{
		private FireSensor m_Sensor = null;
		...
		public EditFireSensor(FireSensor sensor)
		{
		...
		private bool Insert(WebDBManager dbMgr)
		{
			...
			strSQL = string.Format("Insert into FireSensor (ID, Name, PositionName, X, Y, Z, ZoneID, IsIndoor, Description) values ({0}, '{1}', '{2}', {3}, {4}, {5}, {6}, {7}, NULL)",
                nID, "FireSensor", zone.DisplayText, poi.X, poi.Y, poi.Z, zone.ID, poi.IsIndoor ? 1 : 0);
			...
			strSQL = string.Format("Insert into SensorZone (ID, Type, Connected, EquipZoneID, Data, Description, OrgSensorID, Zone) values ({0}, {1}, {2}, {3}, {4}, '{5}', {6}, {7})",
			   nSensorZoneID, (int)IFacility.FacilityType.FIRE_SENSOR, "NULL", m_Sensor.EquipZoneID, "NULL", m_Sensor.Description, nID, zone.ID);
			...
			if (dbMgr.GetResultData(strSQL) != null)
			{
                ...
                if (SensorManager.Instance.DicFireSensor.TryGetValue(nID, out sensors) == false)
                {
                    sensors = new List<ISensor>();
                    SensorManager.Instance.DicFireSensor[nID] = sensors;
		...
		override public bool Update(WebDBManager dbMgr)
		{
			...
			string strSQL2 = string.Format("Update FireSensor set {0} where id = {1}", strField, m_Sensor.OrgSensorID);
		...
		override public void AddToManager(IChangedDataManager mgr)
		{
			...
			Type type = typeof(EditFireSensor);

			foreach (ChangedData data in arrDatas)
			{
				if (data.GetType() == type)
				{
					EditFireSensor sensor = (EditFireSensor)data;
