...
namespace SDMS
{
    public class CCTVManager
	{
        ...
        public Dictionary<int, CCTV> DicCCTVs
        {
            get { return m_dicCCTVs; }
        }
		...
		public bool LoadEquipZoneCCTV(int nEquipZone = -1)
		{
			...
			for (int i = 0; i < nResultCount - (2 + m_nScrCCTV) ; i += (3+m_nScrCCTV))
			{
                ...
                {
                    nCCTV[j] = WebDBManager.GetIntField(arrResult[i + 2 + j].ToString(), -1);
                }
        ...
        public void AddCCTV(CCTV cctv, int nZoneID, float x, float y, float z, bool isIndoor)
        {
            ...
            int nLayerID = cctv.POI.Facility.GetLayerID();

            UnE.View.Content.IFormContent formContent = UnE.View.Content.ViewUtils.GetContentView();
            formContent.Layers.GetLayer(nLayerID).Add(cctv.POI.ID);
        ...
        public void DeleteCCTV(CCTV cctv)
        {
            ...
            if (m_dicCCTVs.TryGetValue(cctv.ID, out _cctv))
            {
                m_IndoorCCTVList.Remove(_cctv);
                m_OutdoorCCTVList.Remove(_cctv);
                m_dicCCTVs.Remove(cctv.ID);
            }
        }

        // 로딩할 CCTV 정보를 하나의 파일에 담아 한꺼번에 Unity에 전달한다.
        public bool LoadCCTVFile(ISensorTooltipOwner view, bool isIndoor, int zoneID, bool visible)
        {
            if (isIndoor)
                m_IndoorCCTVList.Clear();
            else
                m_OutdoorCCTVList.Clear();
            ...
            sb.Append(",cv.HTTPPort, cv.Type,cv.Stream,cv.Channel,cv.UserID,cv.Password,cv.URL, cv.IsIndoor, cv.ReversePTZ, cv.URL ");
            sb.AppendFormat(" FROM CCTV as cv JOIN Zone as z ON z.ID = cv.ZoneID and cv.ZoneID = {0}", zoneID);
            sb.AppendFormat(" WHERE z.SiteID = {0} AND IsIndoor = {1} ORDER BY cv.Id", m_nSiteID, (isIndoor ? "1" : "0"), zoneID);
            ...
            for (int i = 0; i < nResultCount - 18; i += 19)
            {
                ...
                string strIPAddr = WebDBManager.GetStringField(arrResult[i + 2], "");
                int nPort = WebDBManager.GetIntField(arrResult[i + 3].ToString(), -1);
                float x = WebDBManager.GetFloatField(arrResult[i + 4].ToString(), 0.0f);
                float y = WebDBManager.GetFloatField(arrResult[i + 5].ToString(), 0.0f);
                float z = WebDBManager.GetFloatField(arrResult[i + 6].ToString(), 0.0f);
                int nZoneID = WebDBManager.GetIntField(arrResult[i + 7].ToString(), -1);
                int nLOD = WebDBManager.GetIntField(arrResult[i + 8].ToString(), -1);

                int nHttpPort = WebDBManager.GetIntField(arrResult[i + 9].ToString(), 80);
                string szType = WebDBManager.GetStringField(arrResult[i + 10].ToString(), "");
                int nStream = WebDBManager.GetIntField(arrResult[i + 11].ToString(), 0);
                int nChannel = WebDBManager.GetIntField(arrResult[i + 12].ToString(), 0);
                string szUserName = WebDBManager.GetStringField(arrResult[i + 13], "guest");
                string szPassword = WebDBManager.GetStringField(arrResult[i + 14], "");
                string szURL = WebDBManager.GetStringField(arrResult[i + 15], "");
                bool indoor = WebDBManager.GetIntField(arrResult[i + 16].ToString(), 0) == 0 ? false : true;
                int nReversePTZ = WebDBManager.GetIntField(arrResult[i + 17].ToString(), -1);
                string strURL = WebDBManager.GetStringField(arrResult[i + 18], "");
                ...
                /*if (indoor == isIndoor)
                {
                    string szMsg = string.Format("CCTV: {0},{1},{2},{3}", strCameraName, x, y, z);
                ...
                {
                    if (cctv.LODType == CCTV.LOD.LOW)
                        view.ShowIconPOI(cctv.POI.ID, cctv.IconPath, false);
                }*/

                pois.Add(cctv.POI);
                poiIDs.Add(cctv.POI.ID);
                poiTypes.Add(cctv.IconPath);
                poiVisibles.Add(visible);
        ...
        public bool LoadCCTV(ISensorTooltipOwner view, bool isIndoor)
		{
            UnE.View.Content.IFormContentOwner owner = UnE.View.Content.ViewUtils.GetContentViewOwner();
			WebDBManager dbMgr = owner.DBManager;
            ...
            if (m_nSiteID == 1)
                sb.AppendFormat(" WHERE z.SiteID = {0} ORDER BY cv.Id", m_nSiteID);
            else if(m_nSiteID == 2)
                sb.AppendFormat(" WHERE z.SiteID = {0} AND IsIndoor = {1} ORDER BY cv.Id", m_nSiteID, (isIndoor ? "1" : "0"));
			...
			for (int i = 0; i < nResultCount - 18; i += 19)
			{
                ...
                else if (UnE.SOP.ProxySOP.Instance.SiteID == 999 || UnE.SOP.ProxySOP.Instance.SiteID == 102)
                {
                    cctv.POI.X = x;
                    cctv.POI.Y = z;
                    cctv.POI.Z = -y;
        ...
        public bool LoadCCTV2(ISensorTooltipOwner view, bool isIndoor)
        {
            UnE.View.Content.IFormContentOwner owner = UnE.View.Content.ViewUtils.GetContentViewOwner();
            WebDBManager dbMgr = owner.DBManager;
		...
		public CCTV[] GetCCTVArray(EquipmentZone equipZone)
		{
            ...
            LoadEquipZoneCCTV(equipZone.ID);
