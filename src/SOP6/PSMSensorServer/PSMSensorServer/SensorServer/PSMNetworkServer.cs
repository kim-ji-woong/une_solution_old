...
namespace PSMSensorServer
{
    public class PSMNetworkServer
    {
        ...
        public static string LoadIni(string strKey, string strSection)
        {
            Utility ini = new Utility();
            return ini.getinivalue(strSection, strKey);
        }

        private int GetPort()
        {
            Utility ini = new Utility();
            string strPort = ini.getinivalue("Server Connection Info", "sensor_port");

            int nPort = 0;

            if (strPort.Length > 0)
            {
                int.TryParse(strPort, out nPort);
            }
            else
            {
                nPort = 19000;
        ...
        public PSMNetworkServer(WebDBManager dbMgr = null)
        {
            ...
            m_ioMgr = new IOManager(m_dbMgr.SiteID);
        ...
        private void WritePortToDB()
        {
            ...
            {
                strSQL = string.Format("Insert into SensorServerPort (Name, Port, SiteID) values ('{0}', {1}, {2})", "PSMSensor", m_nPort, m_dbMgr.SiteID);
                m_dbMgr.GetResultData(strSQL);
            }
            else
            {
                strSQL = string.Format("Update SensorServerPort Set Port = {0} WHERE SiteID = {1} AND Name='{2}'", m_nPort, m_dbMgr.SiteID, "PSMSensor");
                m_dbMgr.GetResultData(strSQL);
            }
        }		
    }
