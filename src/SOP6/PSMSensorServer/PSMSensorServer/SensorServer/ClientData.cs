...
namespace PSMSensorServer
{
    public abstract class ClientData
    {
        ...
        public virtual bool OnReceiveData(ConnectionState state, byte[] bytes, bool checkValidation = true)
        {
            ...
            else if (nHeader == TCP_ID.ARE_YOU_THERE)
            {
                return ProcessAreYouThrere(state);
            }
        ...
        private static byte[] I_AM_HERE_BYTES = null;

        private static byte[] GetIAmHere()
        {
            if (I_AM_HERE_BYTES == null)
            {
                byte[] bytes = TcpHelper.MakeBytes(TCP_ID.I_AM_HERE, new ArrayList());
                int nLength = bytes.Length;

                byte[] len = BitConverter.GetBytes(nLength);
                I_AM_HERE_BYTES = new byte[nLength + len.Length];
                
                Array.Copy(len, I_AM_HERE_BYTES, len.Length);
                Array.Copy(bytes, 0, I_AM_HERE_BYTES, len.Length, nLength);
            }

            return I_AM_HERE_BYTES;
        }

        private bool ProcessAreYouThrere(ConnectionState state)
        {
            byte[] bytes = GetIAmHere();
            return this.ServiceProvider.Send(bytes, 0, bytes.Length, state);
        }
        ...
        protected int GetHeader(TcpLib2.ConnectionState state, byte[] bytes, out byte[] curReceivedData, bool checkValidation, out ArrayList arrDatas)
        {
            ...
            if (nBytesCount > 0)
            {
                ...
                if (checkValidation)
                {
                    // Header만 있는 메시지 일수 있다.
                    if (bytes.Length == 6)
                    {
                        short sHeader = BitConverter.ToInt16(bytes, 0);
                        int nDataCount = BitConverter.ToInt32(bytes, 2);

                        if (sHeader > 0 && nDataCount == 0)
                        {
