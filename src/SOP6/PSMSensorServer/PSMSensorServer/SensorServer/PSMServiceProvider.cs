...
{
    public class PSMServiceProvider : TcpServiceProvider
    {
        ...
        public void RecvLog(byte[] bytes, ConnectionState state)
        {
            ...
            if (bytes[0] != SDMS.TCP_ID.I_AM_HERE || !m_exceptPingLog)
            {
                ...
                if (data != null)
                {
                    if (data.Type == ClientData.ClientType.GIMENS)
                        strClient = "SIMENS";
        ...
        public override void OnAcceptConnection(ConnectionState state)
        {
            ...
            //lock (m_arrClients)
            {
                ...
                ClientData data = new ClientDataPSMTester(this, state);
                state.Tag = data;
                if (m_arrClients.TryAdd(state, data))
                {
                    //SendMessage(TCP_ID.WHO_ARE_YOU, state);
                    PSMNetworkServer.Instance.AddClient(state);
                }
            }
        }
        ...
        private void _OnDropConnection(ConnectionState state, bool noLock)
        {
            ...
            if (noLock)
            {
                ...
                {
                    data.Close();
                    PSMNetworkServer.Instance.RemoveClient(state);
                }
            }
            else
            {
    ...
    public class ConnectionLogClient : ConnectionLog
    {
        ...
        private const string LOG_FILE = "SensorData.log";
        private const string LOG_FILE2 = "SensorServer.log";
        ...
        public static bool MakeInstance()
        {
            ...
            AddAppender("Log4net.Client", CreateFileAppender("Client", LOG_FILE));                       
            m_instance2.logger = log4net.LogManager.GetLogger("Log4net.Client");

            // 작성된지 한달이 경과한 로그는 삭제한다.
            RemoveLog();
        ...
        {
            DateTime dtLimit = DateTime.Now.AddMonths(-1);
            string strLimit = string.Format("{0}-{1:00}-{2:00}", dtLimit.Year, dtLimit.Month, dtLimit.Day);

            RemoveLog(strLimit, LOG_FILE);
            RemoveLog(strLimit, LOG_FILE2);
        }
        ...
        private static string GetCurrentPath()
        {
            ...
            if (nIndex1 > nIndex2)
                return strPath.Substring(0, nIndex1);
            else if (nIndex2 > nIndex1)
                return strPath.Substring(0, nIndex2);

            return ".";
        }

        private static void RemoveLog(string strLimit, string strLogFile)
        {
            string strPath = GetCurrentPath();
            string[] files = Directory.GetFiles(strPath, strLogFile + "*");

            foreach (string strFilePath in files)
            {
                int nIndex = strFilePath.IndexOf(strLogFile);

                if (nIndex >= 0)
                {
                    int nTargetIndex = nIndex + strLogFile.Length + 1;

                    if (strFilePath.Length > nTargetIndex)
                    {
                        string strDate = strFilePath.Substring(nTargetIndex);

                        if (strDate.CompareTo(strLimit) < 0)
