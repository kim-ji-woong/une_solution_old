...
namespace SDMSAgent.SDMS
{
    public class UpdateManager
    {
        ...
        private void AddFileList(List<string> fileList, string strExceptFileName, string strFolderPath, string strParent = "")
        {
            int nFolderLength = strFolderPath.Length;
            string[] arrFiles = Directory.GetFiles(strFolderPath);

            foreach (string strFile in arrFiles)
            {
                string strFileName = strFile.Substring(nFolderLength + 1);

                if (string.Compare(strExceptFileName, strFileName, true) == 0)
                    continue;

                fileList.Add(strParent + strFileName);
            }
            ...
            {
                string strParentFolder = strParent + strFolder.Substring(nFolderLength + 1) + "\\";
                AddFileList(fileList, strExceptFileName, strFolder, strParentFolder);
            }
        }

        private bool CopyFiles(List<string> fileList, string strSrcFolderPath, string strTrgFolderPath, string strTargetFileName)
        {
            try
            {
                if (Directory.Exists(strTrgFolderPath) == false)
                    Directory.CreateDirectory(strTrgFolderPath);

                string strFileListPath = strTrgFolderPath + "\\" + strTargetFileName;
                StreamWriter writer = new StreamWriter(strFileListPath, false, Encoding.UTF8);

                foreach (string strFile in fileList)
                {
                    if (string.Compare(strFile, strTargetFileName, true) == 0)
                        continue;
                    ...
                    if (nIndex >= 0)
                    {
                        string strFolderName = strFile.Substring(0, nIndex);

                        if (Directory.Exists(strTrgFolderPath + "\\" + strFolderName) == false)
                            Directory.CreateDirectory(strTrgFolderPath + "\\" + strFolderName);
                    }

                    File.Copy(strSrcFolderPath + "\\" + strFile, strTrgFolderPath + "\\" + strFile, true);
            ...
            {
                System.Diagnostics.Trace.WriteLine("CopyFiles Error : " + e.Message);
            }
        ...
        private string MakeFileList(string strPath, out string strTargetFolderName)
        {
            ...
            string strUpdateXML = "update.xml";
            AddFileList(fileList, strUpdateXML, strPath);
            ...
            foreach (string strFile in arrFiles)
            {
                ...
                if (nIndex >= 0)
                {
                    ...
                    if (string.Compare(_strFile, strUpdateXML, true) == 0)
                    {
                        ...
                        {
                            if (CopyFiles(fileList, strPath, m_strUpdateTrgPath + "\\" + strTargetFolderName, strTargetFileName))
                            {
                                File.Copy(strFile, m_strUpdateTrgPath + "\\" + strUpdateXML, true);
        ...
        /*private string MakeFileList(string strFolderPath, string strTargetFileName)
        {
            ...
            if (nIndex >= 0)
            {
                string strFolder = strFileListPath.Substring(0, nIndex);
