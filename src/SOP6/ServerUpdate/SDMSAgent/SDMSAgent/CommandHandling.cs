    ...
    { 
        ...
        public bool Execute(CommandItem commandItem)
        {
            ...
            try
            {
                ...
                {
                    if (FormMain.Instance.ServerUploadPath.Length > 0 && commandItem.UpdateName.Length > 0)
                    {
                        File.Copy(FormMain.Instance.ServerUploadPath + "\\" + commandItem.UpdateName, FormMain.Instance.SdmsUpdateSrc + "\\" + commandItem.UpdateName, true);
                        File.Delete(FormMain.Instance.ServerUploadPath + "\\" + commandItem.UpdateName);
                ...
                else if (type == CommandType.IIS_RESET)
                {
        ...
        private bool ProcessKill(string strProcName)
        {
            if (strProcName.Contains(@"\"))
            {
                ...
                if (GetFolderNFile(strProcName, ref strFolderPath, ref strFileName2))
                {
            ...
            {
                process[0].Kill();
                FormMain.WriteLog("[INFO] ProcessKill " + strProcName);
        ...
        private bool ProcessStart(string strProcName)
        {
            ...
            if (GetFolderNFile(strProcName, ref strFolderPath, ref strFileName))
            {
                ...
                try
                {
                    ...
                    FormMain.WriteLog("[INFO] ProcessStart : " + strFolderPath + strFileName);
                }
                catch (Exception ex)
                {
                    System.Diagnostics.Trace.WriteLine(ex.Message);
                    FormMain.WriteLog("[ERROR] ProcessStart : " + ex.Message);
                    return false;
                } 
            ...
            {
                FormMain.WriteLog("[ERROR] ProcessStart : 경로 확인");
                return false;
            }
        }
        private bool ServiceStop(string strServiceName)
        {
            ...
            {
                string strFolderPath = "", strFileName2 = "";
                if (GetFolderNFile(strServiceName, ref strFolderPath, ref strFileName2))
                {
            ...
            if (SOPChecker.ServiceManager.IsRunningSerivce(strServiceName))
            {
                ...
                FormMain.WriteLog("[INFO] ServiceStop : " + strServiceName);
            }
        ...
        private bool ServiceStart(string strServiceName)
        {
            try
            {
                ...
                FormMain.WriteLog("[INFO] ServiceStart : " + strServiceName);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Trace.WriteLine(ex.Message);
                FormMain.WriteLog("[ERROR] ServiceStart : " + ex.Message);
                return false;
            }
        ...
        private bool UpdateNReboot(CommandItem commandItem)
        {
            ...
            if (UploadSelf(ref strFileName, ref strFolder) == false)
                return false;
        ...
        {
            string strDownloadURL = FormMain.Instance.ServerUploadPath;
        ...
        /// 서버에 업로드
        /// </summary> 
        private void UploadFile(CommandItem commandItem)
        {
            try
            {
                int nIndex = commandItem.SearchPath.LastIndexOf('\\');
                string strFolder = commandItem.SearchPath.Substring(0, nIndex + 1);

                string strFileName = commandItem.SearchPath.Substring(nIndex + 1);

                FileAttributes attr = File.GetAttributes(commandItem.SearchPath);
                if ((attr & FileAttributes.Directory) == FileAttributes.Directory)
                {
                    CopyFolder(commandItem.SearchPath, commandItem.UpdateName + "\\" + strFileName);
                }
                else
                {
                    File.Copy(commandItem.SearchPath, MakePath(commandItem.UpdateName, strFileName, false));
                }
                
                FormMain.WriteLog("[INFO] UploadFile : " + commandItem.UpdateName);
            }
            catch (Exception ex)
            {
                FormMain.WriteLog("[ERROR] UploadFile : " + ex.Message);
            }
        }
        ...
        private void GetFileList(string searchPath)
        {
            ...
            {
                FormMain.WriteLog("[ERROR] GetFileList : " + ex.ToString());
            }    
        }
        ...
        private void WriteDirectory(string path)
        {
            ...
            {
                FormMain.WriteLog("[ERROR] WriteDirectory / " + path + " : " + ex.ToString());
            }
        }
        ...
        {
            string[] arrFiles = Directory.GetFiles(FormMain.Instance.ServerUploadPath);
        ...
        public void DeleteFile(string strFilePath)
        {
            try
            {
                ...
                string strFilePath2 = MakePath(FormMain.Instance.LocalDownloadPath, strFileName, false);
            ...
            if (commandItem.SearchPath.Contains("@"))
            {
                ...
                if (arr.Length == 2)
                {
                    ...
                    if (dirInfo.Attributes.ToString() == "Directory") // 폴더
                    {
                        ...
                        if (nIndex < 0)
                            return;
                        string strFolderName = strSourceFileName.Substring(nIndex + 1);

                        string strDestFolderName = MakePath(strDestFileName, strFolderName, false);
                        CopyFolder(strSourceFileName, strDestFolderName);
                        isDirectory = true;
                    }
                    else 
                    {
        ...
        public void IISReset()
        {
            try
            {
                DirectoryEntry appPool = GetDirectoryEntry("UnEService");
        ...
        private DirectoryEntry GetDirectoryEntry(string pool)
        {
            string serverName = "localhost";// "192.168.0.211";
            string iisPath = String.Format("IIS://{0}/W3SVC/APPPOOLS{1}", serverName, pool);
            
            return new DirectoryEntry(iisPath);
        }
    }
