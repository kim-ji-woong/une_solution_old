...
namespace TeamEditor
{
    public static class DataManager
    {
        ...
        public static TemporaryNormalTeam RemoveTemporaryNormalTeam(int nTeamID)
        {
            TemporaryNormalTeam team = null;

            if (m_dicTemporaryNormalTeams.TryGetValue(nTeamID, out team) == false)
                return team;

            m_dicTemporaryNormalMembers.Remove(team);
            m_dicTemporaryNormalTeams.Remove(nTeamID);
            return team;
        }

        public static TemporaryEmergencyTeam RemoveTemporaryEmergencyTeam(int nTeamID)
        {
            TemporaryEmergencyTeam team = null;

            if (m_dicTemporaryEmergencyTeams.TryGetValue(nTeamID, out team) == false)
                return team;

            m_dicTemporaryEmergencyMembers.Remove(team);
            m_dicTemporaryEmergencyTeams.Remove(nTeamID);
            return team;
        }
        ...
        public static CompanyMember LoadCompanyMember(WebDBManager dbMgr, int nTransaction, int nCompanyMemberID)
        {
            ...
            ArrayList arrResults = nTransaction != 0 ? dbMgr.GetBatchData(strSQL) : dbMgr.GetResultData(strSQL);// dbMgr.GetResultData(strSQL, nTransaction);
        ...
        private static bool LoadJobSubLevel(WebDBManager dbMgr, int nTransaction)
        {
            ...
            ArrayList arrResult = nTransaction != 0 ? dbMgr.GetBatchData(strSQL) : dbMgr.GetResultData(strSQL);//dbMgr.GetResultData(strSQL, nTransaction);
        ...
        public static bool LoadNormalMember(WebDBManager dbMgr)
        {
            ...
            for (int i = 0; i < nResultCount - 8; i += 9)
            {
                ...
                {
                    if (memberType == TemporaryMember.MemberType.RegularTeam || memberType == TemporaryMember.MemberType.ExternalTeam)
        ...
        public static TemporaryNormalTeam GetTemporaryNormalTeam(string strTeamName, TemporaryNormalTeam teamParent)
        {
            if (teamParent == null)
            {
                foreach (TemporaryNormalTeam team in m_rootNormalTeams)
                {
                    if (team.TeamName == strTeamName)
                        return team;
                }
            ...
            foreach (KeyValuePair<int, TemporaryNormalTeam> pair in m_dicTemporaryNormalTeams)
            {
                if (pair.Value.ParentTeam == teamParent && pair.Value.TeamName == strTeamName)
                    return pair.Value;
            }
        ...
        public static TemporaryEmergencyTeam GetTemporaryEmergencyTeam(string strTeamName, TemporaryEmergencyTeam teamParent)
        {
            if (teamParent == null)
            {
                foreach (TemporaryEmergencyTeam team in m_rootEmergencyTeams)
                {
                    if (team.TeamName == strTeamName)
                        return team;
                }
        ...
        public static int GetMaxID(WebDBManager dbMgr, string strTableName, int nTransaction)
        {
        ...
        public static int GetJobSubLevel(WebDBManager dbMgr, int nTransaction, CompanyMember.JobLevelSubInfo subLevel)
        {
            if (subLevel == null)
                return -1;
        ...
        public static int GetJobSubPosition(WebDBManager dbMgr, int nTransaction, CompanyMember.JobPositionSubInfo subPosition)
        {
            if (subPosition == null)
                return -1;
        ...
        public static int GetGroupPosition(WebDBManager dbMgr, int nTransaction, CompanyMember.JobGroupPosition groupPosition)
        {
