...
namespace TeamEditor
{
    ...
    {
        // 사번, 성명, 소속명, 직군명, 직군상세, 직급명, 휴대전화, 근무지전화, 직위명
        ...
        protected Tree<RegularTeam> m_teamTree = new Tree<RegularTeam>();
        // 팀별 직원들
        protected Dictionary<RegularTeam, List<CompanyMember>> m_dicRegularMembers = new Dictionary<RegularTeam, List<CompanyMember>>();
        // 삭제될 이전 팀들의 목록
        ...
        protected List<Team> m_newTemporaryNormalTeams = null;
        protected List<Team> m_newTemporaryEmergencyTeams = null;
        protected List<Team> m_removingOldTemporaryNormalTeams = null;
        protected List<Team> m_removingOldTemporaryEmergencyTeams = null;

        // 추가될 주간 및 평일 자위소방대원 목록
        protected Dictionary<Team, List<TemporaryMember>> m_dicNewTemporaryNormalMembers = null;
        // 추가될 야간 및 휴일 자위소방대원 목록
        protected Dictionary<Team, List<TemporaryMember>> m_dicNewTemporaryEmergencyMembers = null;
        // 삭제될 주간 및 평일 자위소방대원 목록
        protected Dictionary<Team, List<TemporaryMember>> m_dicRemovingOldTemporaryNormalMembers = null;
        // 삭제될 야간 및 휴일 자위소방대원 목록
        protected Dictionary<Team, List<TemporaryMember>> m_dicRemovingOldTemporaryEmergencyMembers = null;
        ...
        public bool OpenFile(string strPath)
        {
            ...
            bool opened = false, updateAll = true;

            RegularMemberReader reader = null;

            if (strExt == "txt" || strExt == "csv")
            {
                char delimeter = strExt == "txt" ? '\t' : ',';

                Import.RegularMemberTextReader _reader = new Import.RegularMemberTextReader();
                opened = _reader.OpenTextFile(strPath, delimeter);

                if (opened)
                {
                    updateAll = _reader.UpdateAll;
                }
            ...
            {
                RegularMemberExcelReader _reader = new RegularMemberExcelReader();
                opened = _reader.OpenExcelFile(strPath);
            ...
            if (reader != null)
            {
                m_teamTree = reader.m_teamTree;
                m_dicRegularMembers = reader.m_dicRegularMembers;
                m_removingOldTeams = reader.m_removingOldTeams;
                m_removingOldCompanyMembers = reader.m_removingOldCompanyMembers;
                m_notRemovingOldCompanyMembers = reader.m_notRemovingOldCompanyMembers;

                m_newTemporaryNormalTeams = reader.m_newTemporaryNormalTeams;
                m_newTemporaryEmergencyTeams = reader.m_newTemporaryEmergencyTeams;
                m_removingOldTemporaryNormalTeams = reader.m_removingOldTemporaryNormalTeams;
                m_removingOldTemporaryEmergencyTeams = reader.m_removingOldTemporaryEmergencyTeams;
                m_dicNewTemporaryNormalMembers = reader.m_dicNewTemporaryNormalMembers;
                m_dicNewTemporaryEmergencyMembers = reader.m_dicNewTemporaryEmergencyMembers;
                m_dicRemovingOldTemporaryNormalMembers = reader.m_dicRemovingOldTemporaryNormalMembers;
                m_dicRemovingOldTemporaryEmergencyMembers = reader.m_dicRemovingOldTemporaryEmergencyMembers;
            }
            ...
            {
                Command.CommandUpdateRegularMembers command = new Command.CommandUpdateRegularMembers();
                command.RegularMembers = this.m_dicRegularMembers;
                command.NewTemporaryNormalTeams = this.m_newTemporaryNormalTeams;
                command.NewTemporaryEmergencyTeams = this.m_newTemporaryEmergencyTeams;
                command.RemovingOldTemporaryNormalTeams = this.m_removingOldTemporaryNormalTeams;
                command.RemovingOldTemporaryEmergencyTeams = this.m_removingOldTemporaryEmergencyTeams;
                command.NewTemporaryNormalMembers = this.m_dicNewTemporaryNormalMembers;
                command.NewTemporaryEmergencyMembers = this.m_dicNewTemporaryEmergencyMembers;
                command.RemovingOldTemporaryNormalMembers = this.m_dicRemovingOldTemporaryNormalMembers;
                command.RemovingOldTemporaryEmergencyMembers = this.m_dicRemovingOldTemporaryEmergencyMembers;
        ...
        private void CompareRegularMembers(List<Tree<RegularTeam>.Node> newNodes, TreeNodeCollection oldNodes, List<RegularTeam> removingOldTeams, List<CompanyMember> removingOldCompanyMembers, List<CompanyMember> notRemovingOldCompanyMembers)
        {
            foreach (TreeNode oldNode in oldNodes)
            {
                if (oldNode.Tag != null && (oldNode.Tag is RegularTeam))
                {
                    ...
                    if (members != null)
                    {
                        removingOldCompanyMembers.AddRange(members);
                    }

                    CompareRegularMembers(newNodes, oldNode.Nodes, removingOldTeams, removingOldCompanyMembers, notRemovingOldCompanyMembers);
                }
            }
            /*foreach (TreeNode oldNode in oldNodes)
            {
        ...
        protected bool ReadRegularMember(string[] tokens, Dictionary<COLUMN_HEADER, int> dicIndices, int nIndexCount, Dictionary<JOB_POSITION, int> dicJobPositionID, Dictionary<string, string> dicMemberID, Dictionary<string, string> dicPhoneNumber)
        {
            ...
            {
                // 사번 중복
                if (dicMemberID.ContainsKey(strMemberID))
                    strMemberID = "";
                else
                    dicMemberID[strMemberID] = strMemberID;
            }
            ...
            if (strMobilePhoneNumber != null)
            {
                ...
                {
                    // 휴대전화번호 중복
                    if (dicPhoneNumber.ContainsKey(strMobilePhoneNumber))
                        strMobilePhoneNumber = "";
                    else
                        dicPhoneNumber[strMobilePhoneNumber] = strMobilePhoneNumber;
                }
            }
            ...
            if (team != null)
            {
                ...
                member.Name = strName;
                member.PositionID = GetJobPositionID(strTeamPath, strJobPosition, dicJobPositionID, strTeamPosition, ref subPosition);
        ...
        // 같은 부서에 동일한 이름을 가졌으면서 휴대전화번호와 사번이 모두 비어있는 다른 직원이 있는지 검사한다.
        protected bool CheckDuplicateMember(CompanyMember member)
        {
            if (member.MemberID.Length > 0 || member.PhoneNumber.Length > 0)
                return true;

            List<CompanyMember> members = null;

            if (m_dicRegularMembers.TryGetValue(member.Team, out members) == false)
                return true;

            foreach (CompanyMember other in members)
            {
                if (other.Name == member.Name && other.MemberID.Length == 0 && other.PhoneNumber.Length == 0)
                    return false;
            }
        ...
        protected RegularTeam GetRegularTeam(string strTeamPath)
        {
            ...
            for (int i=0;i<nTeamCount;i++)
            {
                ...
                if (i > 0 && nodeParent == null)
                    continue;

                TreeNode teamNode = _FindRegularTeamNode(strTeamName, nodeParent);

                if (node == null)// || teamNode == null)
                    return null;

                if (teamNode != null)
                {
                    nodeParent = teamNode;
                    RegularTeam team = (RegularTeam)teamNode.Tag;
                    node.Data.TeamID = team.TeamID;
                }
            }
        ...
        private TreeNode _FindRegularTeamNode(string strTeamName, TreeNode nodeParent)
        {
            if (nodeParent == null)
            {
                foreach (TreeNode node in FormMain.Instance.RegularTeamTree.Nodes)
                {
                    if (node.Text == strTeamName)
                        return node;
                }
            }
            else
        ...
        protected CompanyMember.JobGroupPosition GetJobGroup(string strJobGroup)
        {
        ...
        protected int GetJobLevel(string strJobLevel, ref CompanyMember.JobLevelSubInfo subJob)
        {
            if (strJobLevel == null || strJobLevel.Length == 0)
                return 0;

            int nIndex3 = strJobLevel.IndexOf('(');

            int nLevelID = 0;
            int nIndex = strJobLevel.IndexOf("직급");

            if (nIndex < 0 || (nIndex3 >= 0 && nIndex > nIndex3))
            {
                nIndex = strJobLevel.IndexOf("급");

                if (nIndex3 >= 0 && nIndex > nIndex3)
                    nIndex = -1;
            }
            ...
            {
                int nIndex4 = strJobLevel.IndexOf(')');

                if (nIndex4 > nIndex3)
                {
                    string strSubLevel = strJobLevel.Substring(nIndex3 + 1, nIndex4 - nIndex3 - 1);
                    subJob = CompanyMember.JobLevelSubInfo.GetJobSubLevel(strSubLevel);
        ...
        protected string GetMobilePhoneNumber(string strPhoneNumber)
        {
        ...
        // Key : Column Index
        // Value : 해당 Column에 대한 실제 File 내 Column Index
        protected Dictionary<COLUMN_HEADER, int> FindColumnHeader(string[] tokens, int nIndexCount)
        {
            ...
            for (int i=0;i<nTokenCount;i++)
            {
                ...
                if (strToken == "사번")
                    ...
                    dicIndeces[COLUMN_HEADER.TEAM_POSITION] = i;
            }

            // 직위는 양식파일에 없어도 된다.
            if (dicIndeces.ContainsKey(COLUMN_HEADER.TEAM_POSITION) == false)
                dicIndeces[COLUMN_HEADER.TEAM_POSITION] = -1;
        ...
        protected Encoding GetEncoding(string strPath)
        {
