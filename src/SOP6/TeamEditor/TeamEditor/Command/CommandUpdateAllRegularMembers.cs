    ...
    public class CommandUpdateAllRegularMembers : CommandEx
    {
        ...
        public override void SaveDB(WebDBManager dbMgr, bool dir)
        {
            ...
            if (dir)
            {
                ...
                if (RemoveDBCompanyMembers(dbMgr) == false)
                    goto ROLLBACK;
                if (RemoveDBRegularTeams(dbMgr, out siteTeamIDIsNull) == false)
                    goto ROLLBACK;
                ...
                if (AddDBRegularTeams(dbMgr, dicRegularTeamPathID, siteTeamIDIsNull) == false)
                    goto ROLLBACK;
        ...
        private bool AddDBRegularMembers(WebDBManager dbMgr, Dictionary<CompanyMember, RegularTeam> dicAddedMembers, Dictionary<CompanyMember, int> dicMemberIDs, Dictionary<RegularTeam, int> dicRegularTeamIDs)
        {
            ...
            foreach (KeyValuePair<CompanyMember, RegularTeam> pair in dicAddedMembers)
            {
                ...
                strBatchQuerySQLs += strSQL + ";";

                //if (dbMgr.GetBatchData(strSQL) == null)
                //    return false;
            }

            if (strBatchQuerySQLs.Length > 0)
            {                
                if (dbMgr.GetBatchData(strBatchQuerySQLs) == null)
                    return false;
            }
        ...
        private bool AddDBRegularTeams(WebDBManager dbMgr, Dictionary<string, int> dicRegularTeamID, bool updateSiteTeamID)
        {
            ...
            if (AddDBRegularTeams(dbMgr, m_teamTree.RootNode, ref nTeamID, strInsertFormat, ref strInsertIDs, dicRegularTeamID, updateSiteTeamID) == false)
                return false;
        ...
        // Value : Team ID
        private bool AddDBRegularTeams(WebDBManager dbMgr, Tree<RegularTeam>.Node node, ref int nTeamID, string strInsertFormat, ref string strInsertIDs, Dictionary<string, int> dicRegularTeamID, bool updateSiteTeamID)
        {
            if (node.Data == null)
                return true;
            ...
            /*if (node.Data.TeamID > 0)
            {
                ...
                {
                    if (AddDBRegularTeams(dbMgr, child, ref nTeamID, strInsertFormat, ref strInsertIDs, dicRegularTeamID, updateSiteTeamID) == false)
                        return false;
                }
            ...
            else
            {
                if (updateSiteTeamID)
                {
                    string strSQL = string.Format(strInsertFormat, nTeamID++, node.Data.TeamName, "NULL");
                    ...
                    string strSQL2 = string.Format("Update Site set SiteName = '{0}', TeamID = {1} where ID = {2}", node.Data.TeamName, nTeamID - 1, FormMain.Instance.SiteID);
                ...
                else
                {
                    ...
                    if (nOldRootTeamID > 0)
                    {
                        ...
                        if (dbMgr.GetBatchData(strSQL2) == null)
                            ...
                            node.Data.TeamID = nOldRootTeamID;
                    }
                    else
                    {
                        string strSQL2 = string.Format(strInsertFormat, nTeamID++, node.Data.TeamName, strParentTeamID);
                    ...
                    string strSQL3 = string.Format("Update Site set SiteName = '{0}', TeamID = {1} where ID = {2}", node.Data.TeamName, node.Data.TeamID, FormMain.Instance.SiteID);

                    if (dbMgr.GetBatchData(strSQL3) == null)
                        return false;
        ...
        private bool AddNUpdateDBCompanyMembers(WebDBManager dbMgr, Dictionary<CompanyMember, RegularTeam> dicAddedMembers, Dictionary<CompanyMember, int> dicMemberIDs, Dictionary<RegularTeam, int> dicRegularTeamIDs, Dictionary<string, int> dicRegularTeamPathID)
        {
            ...
            string strIDs = "";
            Dictionary<string, int> dicSubJobLevelIDs = new Dictionary<string, int>();
                ...
                foreach (CompanyMember member in pair.Value)
                {
                    ...
                    {                        
                        ...
                        string strSQL = string.Format(strInsertFormat, nMemberID++, member.Name, member.LevelID,
                            ...
                            GetSubLevelID(dbMgr, member, dicSubJobLevelIDs));
        ...
        private string GetSubLevelID(WebDBManager dbMgr, CompanyMember member, Dictionary<string, int> dicSubJobLevelIDs)
        {
            ...
            if (dicSubJobLevelIDs.TryGetValue(member.SubJobLevel.Name, out nSubJobLevelID))
            {
                member.SubJobLevel.ID = nSubJobLevelID;
                return nSubJobLevelID.ToString();
            }
            ...
            dicSubJobLevelIDs[member.SubJobLevel.Name] = nID;
        ...
        private bool GetSiteTeamID(WebDBManager dbMgr, out int nSiteTeamID)
        {
            nSiteTeamID = -1;

            string strSQL = "Select TeamID from Site where ID = " + dbMgr.SiteID.ToString();
            ArrayList arrResult = dbMgr.GetBatchData(strSQL);

            if (arrResult == null || arrResult.Count == 0)
                return false;

            VariousData<int> siteID = WebDBManager.GetIntField(arrResult[0].ToString());

            if (siteID == null)
                nSiteTeamID = -1;
            else
                nSiteTeamID = siteID.Data;

            return true;
        }

        private bool UpdateSiteTeamIDNull(WebDBManager dbMgr)
        {
            string strSQL = "Update Site set TeamID = NULL where ID = " + dbMgr.SiteID.ToString();
            return dbMgr.GetBatchData(strSQL) != null;
        }

        private bool RemoveDBRegularTeams(WebDBManager dbMgr, out bool siteTeamIDIsNull)
        {
            siteTeamIDIsNull = false;
            int nSiteTeamID;

            if (GetSiteTeamID(dbMgr, out nSiteTeamID) == false)
                return false;
            ...
            foreach (RegularTeam team in m_removingOldTeams)
            {
                ...
                if (nSiteTeamID == team.TeamID)
                {
                    if (UpdateSiteTeamIDNull(dbMgr))
                        siteTeamIDIsNull = true;
                    else
                        return false;
                }
            }
            ...
            //string strSQL2 = "Delete from RegularTeam where ID in (" + strTeamIDs + ")";
            bool result = dbMgr.GetBatchData(strSQL2) != null;

            if (result == false)
                System.Diagnostics.Trace.WriteLine(dbMgr.LastErrorMessage);
        ...
        {
            LoadNewTeams(m_teamTree.RootNode, null);

            foreach (KeyValuePair<RegularTeam, List<CompanyMember>> pair in m_dicRegularMembers)
            {
                List<CompanyMember> members = null;

                // 모두 삭제하고 새로 만들기 때문에 모든 TeamID는 -로 둔다.
                if (pair.Key.TeamID > 0)
                    pair.Key.TeamID = -pair.Key.TeamID;
        ...
        private void LoadNewTeams(Tree<RegularTeam>.Node node, RegularTeam teamParent)
        {
            if (node == null)
                return;
