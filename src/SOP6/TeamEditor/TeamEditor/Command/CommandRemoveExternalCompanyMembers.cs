...
namespace TeamEditor.Command
{
    public class CommandRemoveExternalCompanyMembers : CommandEx
    {
        ...
        public List<string> RemoveDB(WebDBManager dbMgr, string strExternalMemberIDs)
        {
            m_rollbackSQLs.Clear();
        ...
        private List<string> RemoveExternalCompanyMembers(WebDBManager dbMgr, string strExternalMemberIDs)
        {
        ...
        // RollBack을 위하여 삭제한 데이터들의 Insert 구문 리스트를 반환한다.
        private List<string> RemoveExternalMemberList(WebDBManager dbMgr, string strExternalMemberIDs)
        {
            ...
            for (int i = 0; i < nResultCount - 3; i += 4)
            {
                ...
                int nPositionID = WebDBManager.GetIntField(arrResult[i + 3].ToString(), -1);
