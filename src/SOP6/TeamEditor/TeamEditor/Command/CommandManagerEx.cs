...
namespace TeamEditor.Command
{
    public class CommandManagerEx : UnE.Command.CommandManager
    {
        ...
        public CommandManagerEx(UnE.GUI.RibbonButton btnUndo, UnE.GUI.RibbonButton btnRedo, UnE.GUI.RibbonButton btnSave, UnE.GUI.RibbonButton btnEdit, WebDBManager dbMgr)
        ...
        private void SaveLastAccessedMemberTime(DateTime time)
        {
            ...
            {
                int nID = DataManager.GetMaxID(m_dbMgr, "OptionSOPSimulator", 0) + 1;

                string strFormat = "Insert into OptionSOPSimulator (ID, PropertyName, PropertyValue, SiteID, Description) values ";
                strFormat += "({0}, 'LastAccessedMemberTime', {1}, {2}, '마지막으로 인사정보(외부, 사용자정의 포함)가 수정된 시간')";

                strSQL = string.Format(strFormat, nID, strTime, FormMain.Instance.SiteID.ToString());
                m_dbMgr.GetResultData(strSQL);
            }
        }
        ...
        protected bool UpdateSDMSConfig()
        {
            ...
            {
                int nID = DataManager.GetMaxID(m_dbMgr, strTableName, 0) + 1;

                strSQL = string.Format("Insert into {0} (ID, PropertyName, PropertyValue, Description, SiteID) values ({1}, '{2}', '{3}', NULL, {4})",
                    strTableName, nID, strPropertyName, m_nSDMSConfig, FormMain.Instance.SiteID);
