...
namespace TeamEditor
{
    public class RegularMemberExcelReader : RegularMemberReader
    {
        ...
        private bool ReadWorkSheet(Excel.Worksheet workSheet)
        {
            ...
            Import.IRegularReader reader = null;
            ...
            for (int i = 0; i < nRowCount; i++)
            {
                ...
                if (reader == null)
                    reader = Import.RegularReaderManager.MakeExcelReader(tokens);
                else
                    reader.ReadRegularMember(tokens, m_dicRegularMembers);
            ...
            if (reader != null)
            {
                m_updateAll = reader.UpdateAll;
                m_newTemporaryNormalTeams = reader.NewTemporaryNormalTeams;
                m_newTemporaryEmergencyTeams = reader.NewTemporaryEmergencyTeams;
                m_removingOldTemporaryNormalTeams = reader.RemovingOldTemporaryNormalTeams;
                m_removingOldTemporaryEmergencyTeams = reader.RemovingOldTemporaryEmergencyTeams;
                m_dicNewTemporaryNormalMembers = reader.NewTemporaryNormalMembers;
                m_dicNewTemporaryEmergencyMembers = reader.NewTemporaryEmergencyMembers;
                m_dicRemovingOldTemporaryNormalMembers = reader.RemovingOldTemporaryNormalMembers;
                m_dicRemovingOldTemporaryEmergencyMembers = reader.RemovingOldTemporaryEmergencyMembers;

                if (reader is RegularMemberReader)
                {
                    m_teamTree = ((RegularMemberReader)reader).TeamTree;
                }
            }
            else
                return false;
