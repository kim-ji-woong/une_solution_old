...
namespace SOPBulletin
{
    public class HistoryManager2
    {
        ...
        private static void FindLastActionStepHistory(DockingRealTime2 realTime)
        {
            ...
            string strSQL = "Select ID, MemberID, UserID, NickName from SOPGenUser where SiteID = " + FormMain2.Instance.DBManager.SiteID.ToString();
            ArrayList arrResult = FormMain2.Instance.DBManager.GetResultData(strSQL);
            ...
            if (strIDs.Length != 0)
            {
                ...
                arrResult = FormMain2.Instance.DBManager.GetResultData(strSQL);
            ...
            arrResult = FormMain2.Instance.DBManager.GetResultData(strSQL, 1);
        ...
        private static void LoadWorkingTime(WebDBManager dbMgr)
        {
            ...
            string strSQL = string.Format("Select PropertyName, PropertyValue from OptionSOPSimulator where (PropertyName = '{0}' or PropertyName = '{1}') and SiteID = {2}",
                strBeginTimeTag, strEndTimeTag, FormMain2.Instance.DBManager.SiteID);
            ArrayList arrResult = dbMgr.GetResultData(strSQL);
        ...
        {
            string strSQL = "Select ID, NickName from SOPGenUser where SiteID = " + FormMain2.Instance.DBManager.SiteID.ToString();
            ArrayList arrResult = dbMgr.GetResultData(strSQL);
            ...
            strSQL = "Select UserID from ControlUser where SiteID = " + FormMain2.Instance.DBManager.SiteID.ToString();
            arrResult = dbMgr.GetResultData(strSQL);
        ...
        public static void LoadComponentHistory(DockingRealTime2 realTime, WebDBManager dbMgr, string strActionStepHistoryIDs, Dictionary<int, ActionStepHistory> dicActionStepHistories, bool ignoreLastLog = false)
        {
            ...
            for (int i = 0; i < nResultCount - 9; i += 10)
            {
                ...
                SetEndpointTime(dbMgr, sectionState.Section, componentHistory, nActionStepHistoryID.Data);
            ...
            if (componentHistories != null)
            {
                ...
                foreach (ComponentHistory history in componentHistories)
                {
                    ...
                    if (dicComponentHistoryDetails.TryGetValue(history, out details))
                    {
                        ...
                        {
                            realTime.AddComponentHistory(history.ActionStepHistory, detail);
                        }
                    }
                }
            }
        ...
        private static void SetEndpointTime(WebDBManager dbMgr, Sections.Section section, ComponentHistory componentHistory, int nActionStepHistory)
        {
            if (section.GetComponentType() == Sections.Section.ComponentType.ENDPOINT)
            {
                string strSQL = "Select BeginTime, EndTime, CancelTime from ActionStepHistory where ID = " + nActionStepHistory.ToString();
                ArrayList arrResult = dbMgr.GetResultData(strSQL);

                if (arrResult == null || arrResult.Count < 3)
                    return;

                if (((Sections.SectionDataEndPoint)section.Data).IsBegin)
                {
                    VariousData<DateTime> beginTime = WebDBManager.GetDateTimeField(arrResult[0]);

                    if (beginTime != null)
                        componentHistory.Time = beginTime.Data;
                }
                else
                {
                    VariousData<DateTime> endTime = WebDBManager.GetDateTimeField(arrResult[1]);
                    VariousData<DateTime> cancelTime = WebDBManager.GetDateTimeField(arrResult[2]);

                    if (endTime != null)
                        componentHistory.Time = endTime.Data;
                    else if (cancelTime != null)
                        componentHistory.Time = cancelTime.Data;
                }
            }
        }
        ...
        public static string GetUserCommanderName(int nSOPGenUserID)
        {
            string strName;

            if (m_dicSOPGenUserCommanderNames.TryGetValue(nSOPGenUserID, out strName))
                return strName;
        ...
        public static bool GetCurrentActionStepID(out int nActionStepID, out bool isRealMode)
        {
            ...
            string strSQL = "Select ActionStepID, RealMode from CurrentActionStep where SiteID = " + FormMain2.Instance.DBManager.SiteID.ToString();
            ArrayList arrResult = FormMain2.Instance.DBManager.GetResultData(strSQL);
