...
namespace SDMS_Building
{
    ...
    public partial class FormMain : Form, IProcessOwner
    {
        ...
        private uSensorInfo uSensorInfo = null;
        private uPoiVisible m_ufrmPoiVisible = null;
        public uPoiVisible uFrmPoiVisible
        {
            get { return m_ufrmPoiVisible; }
        }
        ...
        public FormMain(int nSOPGenUserID)
        {
            ...
            eleBuilding.Enabled = false;
            eleFloor.Enabled = false;
            ...
            btnHome.Font = new System.Drawing.Font("나눔바른고딕", 12.5F, System.Drawing.FontStyle.Bold, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
            ...
            uSensorInfo = new uSensorInfo();
            uSensorInfo.Parent = pnSensorInfo;
        ...
        public void OnReadyDataLoad()
        {
            ...
            cbFloor_SelectionChanged(null, null);


            m_contentManager.ContentForm.RedrawWindow();

            uSensorInfo.Rbtn_Click(uSensorInfo.BtnFire, null);
        ...
        private void InitPosition()
        {
            ...
            uSensorInfo.Location = new Point(55, 30);
            ...
            CustomGridView(dgvSensorInfo, 10f, Color.FromArgb(0xc4, 0xd2, 0xfa), Color.FromArgb(0x29, 0x37, 0x59), Color.FromArgb(0x53, 0x65, 0x96), Color.White);
            ...
            dgvSensorInfo.ScrollBars = ScrollBars.Vertical;
        ...
        private void pnLeft_Paint(object sender, PaintEventArgs e)
        {
            ...
            Image imgSensorInfo = global::SDMS_Building.Properties.Resources.pnLeft_SensorInfo;
            g.DrawImage(imgSensorInfo, 0, 268, imgSensorInfo.Width, imgSensorInfo.Height);

            g.DrawLine(m_penLine1, 10, 130, 335, 130);
        }
        ...
        private void SetVisiblePOIIcons(bool visible)
        {
            if (!visible)
                m_ufrmPoiVisible.Visible = false;
        }
        ...
        private void SelectMonitoringTab()
        {
            ...
            SetVisiblePOIIcons(true);

            dgvSensorInfo.Columns[colSensorName.Index].ReadOnly = true;
            uPoiVisible.SetLayers(null);
            //uSensorInfo.ShowAllLayer();
        }
        ...
        private void SelectReportTab()
        {
            ...
            SetVisiblePOIIcons(false);
            SetVisiblePOIForm(false);
        ...
        private void SelectEditTab()
        {
            ...
            dgvSensorInfo.Columns[colSensorName.Index].ReadOnly = false;
            uSensorInfo.SetLayerState();
        ...
        {
            m_cbBuilding = new UEWpfControl.WpfBlueComboBox();
            ...
            m_cbFloor = new UEWpfControl.WpfBlueComboBox();
        ...
        private void cbBuilding_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            if (type == typeof(Building))
            {
                ...
                if (building.BuildingName == "외부")
                {
                    if (m_enableOutdoor == false)
                    {
                        FormMessageBox msg = new FormMessageBox("편집모드에서는 외부 화면으로 이동할 수 없습니다.", MessageBoxButtons.OK);
                ...
                else
                {
                    ...
                    {
                        //if (!floor.Floor.ToString().Contains("지하"))
                            m_cbFloor.customComboBox.Items.Add(floor.Floor);
                    }
                }

                if (m_cbFloor.customComboBox.Items.Count > 0 && !m_bZoneSelection)
                {
                    if (m_cbFloor.customComboBox.Items.Count == 1)
                        m_cbFloor.customComboBox.SelectedIndex = 0;
                    else
                    {
                        // 아무층도 선택하지 않는다.
                        m_cbFloor.customComboBox.SelectedIndex = -1;
                        /*for (int i = 0; i < m_cbFloor.customComboBox.Items.Count; i++)
                        {
                            if (m_cbFloor.customComboBox.Items[i].ToString() == "1층")
                            {
        ...
        private void cbFloor_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            if (!m_bZoneLoad)
            {
                ...
                if (GetFloorInfo(out isoutDoor, out floor) == false)
                    return;
                ...
                if (isoutDoor)
                    ...
                    strMsg = floor.Zone.Building.BuildingName + " " + floor.ToString() + "으로 ";

                bool yes = true;

                if (m_systemInput == false)
                {
                    CheckEditData();

                    FormMessageBox msg = new FormMessageBox(strMsg + "이동하시겠습니까?", MessageBoxButtons.YesNo);
                    Point pt = this.PointToScreen(pnLeft.Location);
                    msg.StartPosition = FormStartPosition.Manual;
                    msg.Location = new Point(pt.X + pnLeft.Width + 10, pt.Y + 10);
                    yes = msg.ShowDialog() == DialogResult.Yes;
                }
        ...
        public bool GetFloorInfo(out bool isoutDoor, out Floor floor)
        {
            ...
            floor = m_cbFloor.customComboBox.Items[m_cbFloor.customComboBox.SelectedIndex] as Floor;

            if (floor == null)
            {
                Building building = m_cbBuilding.customComboBox.Items[m_cbBuilding.customComboBox.SelectedIndex] as Building;
        ...
        private void LoadFloor(bool isoutDoor, Floor floor)
        {
            ...
            {
                m_contentManager.ContentForm.HideAllAlarmZones();
                m_contentManager.ContentForm.SelectScene("Outdoor_Model");
        ...
        {
            if (m_CurrentTab == ContentOwnerTab.ADMIN_TAB && m_uFormEdit != null && m_uFormEdit.HasChange())
            {
        ...
        private void btnSensorInfoLeft_Click(object sender, EventArgs e)
        {
            if (uSensorInfo.ShowIndex == 0)
                return;

            if (!uSensorInfo.TimerMove.Enabled)
            {
                uSensorInfo.ShowIndex--;
                uSensorInfo.TimerMove.Enabled = true;
            }
        }

        private void btnSensorInfoRight_Click(object sender, EventArgs e)
        {
            if ((uSensorInfo.ArrSensorInfos.Count - 1) - 2 == uSensorInfo.ShowIndex)
                return;
        ...
        private void ResizeBody()
        {
            ...
            m_ufrmPoiVisible.Location = new Point(panelBody.Width - m_ufrmPoiVisible.Width - 58, panelBody.Location.Y + 510);
        }
        ...
        public void AddSensorDectect(ProcessIF proc, bool bAddSelect = true, bool bCallSelect = true)
        {
            ...
            if (proc.DetectSensorID == SOPWebServer.Header.ManualReportDefaultID + (int)IFacility.FacilityType.PSM_SENSOR ||
        ...
        private void SetZone(EquipmentZone zone)
        {
            ...
            for (int i = 0; i < m_cbBuilding.customComboBox.Items.Count; i++)
            {
                Building building = m_cbBuilding.customComboBox.Items[i] as Building;
                if (building == null)
                    continue;
                ...
                if (building == zone.Building)
                {
                    m_cbBuilding.customComboBox.SelectedIndex = i;

                    for (int j = 0; j < m_cbFloor.customComboBox.Items.Count; j++)
                    {
                        Floor floor = m_cbFloor.customComboBox.Items[j] as Floor;
                        if (floor == null)
                            continue;

                        if (floor == zone.Floor)
                        {
                            m_cbFloor.customComboBox.SelectedIndex = j;
                            break;
                        }
                    }
        ...
        public void RemoveSensorDetect(ProcessIF process, bool bNextSelect = true)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    //if (m_optionMgr.UseEquipZoneVolume && m_optionMgr.DicBuildingGroupScene.TryGetValue(1, out strSceneName))
                    //{
                    //    m_contentManager.ContentForm.HideAllAlarmZones();
                    //    m_contentManager.ContentForm.SelectScene(strSceneName);
                    //}
                    //else
                    //{
                    //    m_contentManager.ContentForm.RestoreViewState();
                    //}
                }
            }

            if (uSensorInfo.SelectedType == IFacility.FacilityType.FIRE_SENSOR)
            {
                foreach (DataGridViewRow sensorInfoRow in dgvSensorInfo.Rows)
                {
                    SensorInfo info = sensorInfoRow.Tag as SensorInfo;
                    if (info.SensorID == process.TargetSensor.OrgSensorID)
                    {
                        if (info.ISConnected)
                            sensorInfoRow.Cells[colAlarmStatus.Index].Value = m_imgStatusConnect;
                        else
                            sensorInfoRow.Cells[colAlarmStatus.Index].Value = m_imgStatusDisConnect;
                        break;
                    }
                }
            }
        ...
        public void SetSensorInfo(List<SensorInfo> sensors, IFacility.FacilityType selectedType)
        {
            dgvSensorInfo.Rows.Clear();
            if (sensors != null)
                AddSensorInfo(sensors, selectedType);
            
            if (m_uFormEdit != null)
            {
                List<SensorInfo> addSensors = m_uFormEdit.GetAddedPOIs(selectedType);
                AddSensorInfo(addSensors, selectedType);

                List<SensorInfo> deletedSensors = m_uFormEdit.GetDeletedPOIs(selectedType);
                DeleteSensorInfo(deletedSensors, selectedType);
            }
        }

        private void AddSensorInfo(List<SensorInfo> sensors, IFacility.FacilityType selectedType)
        {
            if (sensors == null)
                return;

            if (selectedType == IFacility.FacilityType.CCTV || selectedType == IFacility.FacilityType.DOOR)
            {
                dgvSensorInfo.Columns[colSensorID.Index].Visible = true;
                dgvSensorInfo.Columns[colAlarmStatus.Index].Visible = false;
                dgvSensorInfo.Columns[colDeActivate.Index].Visible = false;

                dgvSensorInfo.Columns[colSensorID.Index].Width = 50;
                dgvSensorInfo.Columns[colSensorName.Index].Width = 300;
            }
            else
            {
                dgvSensorInfo.Columns[colSensorID.Index].Visible = false;
                dgvSensorInfo.Columns[colAlarmStatus.Index].Visible = true;
                dgvSensorInfo.Columns[colDeActivate.Index].Visible = true;

                dgvSensorInfo.Columns[colSensorName.Index].Width = 230;
            }

            //int nSelectedRowIndex = -1;

            foreach (SensorInfo sensors2 in sensors)
            {
                Image status = null;
                bool isAlarm = false;
                foreach (ProcessIF proc in m_SensorDectects)
                {
                    if ((selectedType == IFacility.FacilityType.FIRE_SENSOR && proc.ProcessType == ProcessType.FireAlarm) ||
                        (selectedType == IFacility.FacilityType.PSM_SENSOR && proc.ProcessType == ProcessType.PSMAlarm) ||
                        (selectedType == IFacility.FacilityType.DOOR && proc.ProcessType == ProcessType.DoorAlarm))
                    {
                        if (proc.TargetSensor.OrgSensorID == sensors2.SensorID)
                        {
                            status = m_imgStatusAlarm;
                            isAlarm = true;
                            break;
                        }
                    }
                ...
                if (!isAlarm)
                {
                    if (sensors2.ISConnected)
                        status = m_imgStatusConnect;
                    else
                        status = m_imgStatusDisConnect;
                }

                if (isAlarm)
                {
                    dgvSensorInfo.Rows.Insert(rowIndex, sensors2.SensorID, sensors2.SensorName, status);
                }
                else
                {
                    rowIndex = dgvSensorInfo.Rows.Add();
                    
                    dgvSensorInfo.Rows[rowIndex].Cells[colSensorID.Index].Value = sensors2.SensorID;
                    dgvSensorInfo.Rows[rowIndex].Cells[colSensorName.Index].Value = sensors2.SensorName;
                    dgvSensorInfo.Rows[rowIndex].Cells[colAlarmStatus.Index].Value = status;
                }
                dgvSensorInfo.Rows[rowIndex].Tag = sensors2;                
            }

            //if (nSelectedRowIndex >= 0)
            //    dgvSensorInfo.Rows[nSelectedRowIndex].Selected = true;
        }

        private void DeleteSensorInfo(List<SensorInfo> sensors, IFacility.FacilityType selectedType)
        {
            if (sensors == null)
                return;
            ...
            foreach (SensorInfo sensors2 in sensors)
            {
                foreach (DataGridViewRow row in dgvSensorInfo.Rows)
                {
                    SensorInfo _sensor = (SensorInfo)row.Tag;

                    if (_sensor.SensorID == sensors2.SensorID)
                    {
                        dgvSensorInfo.Rows.Remove(row);
                        break;
                    }
                }
            }
        ...
        private void dgvSensorInfo_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            if (e.ColumnIndex != 3)
                return;
            ...
            int x = e.CellBounds.X + (e.CellBounds.Width / 2) - (10 / 2) - 5;
            int y = e.CellBounds.Y + (e.CellBounds.Height / 2) - (10 / 2);

            Rectangle rect = new Rectangle(x, y, 10, 10);

            SensorInfo sensor = dgvSensorInfo.Rows[e.RowIndex].Tag as SensorInfo;
            if (sensor != null)
            {
                if (sensor.DeActivate)
                {
                    Brush br = new SolidBrush(Color.FromArgb(0x14, 0x1e, 0x38));
                    Pen pen = new Pen(br);
                ...
                {
                    Brush br = new SolidBrush(Color.FromArgb(0xb4, 0xc8, 0xe6));
                    Pen pen = new Pen(br);
        ...
        private void AddOutdoorZone()
        {
            ...
            building.BuildingName = "외부";

            m_cbBuilding.customComboBox.Items.Insert(0, building);
        }
        ...
        public void SetVisiblePOIForm(bool visible)
        {
            if (visible)
            {
        ...
        public void OnAddPOI(POI poi)
        {
            if (m_uFormEdit != null)
            {
                ...
                if (sensor != null)
                {
                    ...
                    sensors.Add(sensor);
                    AddSensorInfo(sensors, poi.Facility.Type);
                }
            }
        }
        ...
        public void OnDeletePOI(POI poi)
        {
            if (m_uFormEdit != null)
            {
                ...
                if (sensor != null)
                {
                    List<SensorInfo> sensors = new List<SensorInfo>();
                    sensors.Add(sensor);
                    DeleteSensorInfo(sensors, poi.Facility.Type);
        ...
        private void dgvSensorInfo_SelectionChanged(object sender, EventArgs e)
        {
            if (dgvSensorInfo.SelectedCells.Count == 0)
                System.Diagnostics.Trace.WriteLine("no Selected DataGridRow");
            else
            {
                SensorInfo sensor = (SensorInfo)dgvSensorInfo.SelectedCells[0].OwningRow.Tag;

                if (sensor != null && m_CurrentTab == ContentOwnerTab.ADMIN_TAB && m_uFormEdit != null)
                    m_uFormEdit.OnSelectSensor(sensor, uSensorInfo.SelectedType);
            }
        }

        private DataGridViewRow GetDataGridViewRow(SensorInfo sensor)
        {
            foreach (DataGridViewRow row in dgvSensorInfo.Rows)
            {
                if (row.Tag != null && row.Tag is SensorInfo)
                {
                    if (((SensorInfo)row.Tag).SensorID == sensor.SensorID)
                        return row;
                }
            }
        ...
        private void dgvSensorInfo_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (m_CurrentTab == ContentOwnerTab.ADMIN_TAB && m_uFormEdit != null)
                m_uFormEdit.OnChangeText(sensor, strText, uSensorInfo.SelectedType);
        ...
        private void dgvSensorInfo_CellMouseDoubleClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            SensorInfo tag = dgvSensorInfo.Rows[e.RowIndex].Tag as SensorInfo;
            if (tag == null)
                return;
            ...
            if (uSensorInfo.SelectedType == IFacility.FacilityType.FIRE_SENSOR)
            {
                ISensor sensor = tag.Sensor as ISensor;
                if (sensor == null)
                    return;

                if (!m_dataMgr.DicPOI.ContainsKey(tag.SensorID))
                    return;

                poi = m_dataMgr.DicPOI[tag.SensorID];
                poiType = "Fire";
            }
            else if (uSensorInfo.SelectedType == IFacility.FacilityType.CCTV)
            {
                CCTV cctv = tag.Sensor as CCTV;
                if (cctv == null)
                    return;
            ...
            else if (uSensorInfo.SelectedType == IFacility.FacilityType.DOOR)
            {
                ISensor sensor = tag.Sensor as ISensor;
                if (sensor == null)
                    return;
        ...
        public void SelectedCCTV(int nCCTVID)
        {
            if (uSensorInfo.SelectedType != IFacility.FacilityType.CCTV)
                //uSensorInfo.Rbtn_Click(uSensorInfo.BtnCCTV, null);
                return;

            SelectedSensorInfoGridView(nCCTVID);
        }

        public void SelectedPOI(IFacility.FacilityType facilityType, int nID)
        {
            if (uSensorInfo.SelectedType != facilityType)
            {
                uSensorInfo.SelectType(facilityType);
            }

            SelectedSensorInfoGridView(nID);
        }

        private void SelectedSensorInfoGridView(int nID)
        {
            int selectedRowIndex = -1;
            foreach (DataGridViewRow sensorInfoRow in dgvSensorInfo.Rows)
            {
                SensorInfo info = sensorInfoRow.Tag as SensorInfo;
                if (info.SensorID == nID)
                {
                    sensorInfoRow.Selected = true;
                    selectedRowIndex = sensorInfoRow.Index;
                    break;
                }
            }
            ...
            {
                dgvSensorInfo.Rows[selectedRowIndex].Selected = true;
                dgvSensorInfo.FirstDisplayedScrollingRowIndex = selectedRowIndex;
            }
        }
        ...
        private void DisplayCloseDoor()
        {
            ...
            if (process.TargetZone.LinkedZone.ID != floor.Zone.ID)
                return;
            
            List<POI> pois = null;
            List<string> poiTypes = null;

            m_dataMgr.LoadCloseDoorPOI(process.TargetZone.LinkedZone.ID, ref pois, ref poiTypes, process.DetectTime);

            if (pois != null && poiTypes != null)
            {
                Panel4Unity panel = (Panel4Unity)m_contentManager.ContentForm.OutdoorView;
                ...
                {
                    if (poiTypes[i].Contains(Data.CommonString.AlarmTag))
                    {
                        int rowCount = dgvOpenDoor.Rows.Add();
                        dgvOpenDoor.Rows[rowCount].Cells[0].Value = (pois[i].Facility as EtcSensor).SensorName;
                    }
                }
