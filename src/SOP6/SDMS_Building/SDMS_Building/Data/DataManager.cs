...
namespace SDMS_Building.Data
{
    public class DataManager
    {
        ...
        public void LoadSensorPOI(WebDBManager dbMgr, bool clearPOI, bool isIndoor, IFacility.FacilityType type)
        {
            ...
            if (zone != null)
            {
                if (type == IFacility.FacilityType.FIRE_SENSOR)
                    ...
                    PSMManager.Instance.LoadPSMSensorPOI(dbMgr, zone.ID, isIndoor, m_ContentView, m_dicPOI);
            }
        }
        ...
        public void LoadCloseDoorPOI(int nZoneID, ref List<POI> pois, ref List<string> poiTypes, DateTime dtDetect)
        {
            ...
            List<EtcSensor> doors = m_dicEtcSensor[IFacility.FacilityType.DOOR];
                ...
                for (int j = 0; j < curDoors.Count; j++)
                {
                    ...
                    if (sensor.ID == nID)
                    {
                        ...
                        else
                        {
                            ...
                            poiTypes[j] = handler.CurrentPOIType;
                        }
                        break;
                    } 
                }
        ...
        private Dictionary<int, POI> m_dicPOI = new Dictionary<int, POI>();
        public Dictionary<int, POI> DicPOI
        {
            get { return m_dicPOI; }
            set { m_dicPOI = value; }
        }
        ...
        {            
            if (m_dicPOI.ContainsKey(sensor.ID))
            {
                POI poi = m_dicPOI[sensor.ID];
                if (poi == null)
                    return;
