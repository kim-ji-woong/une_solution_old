    ...
    public partial class FormMain : Form
    {
        ...
        private void AddAlarm(int nCCTVID)
        {
            ...
            if (cctv != null)
            {
                ...
                NetworkWebManager.Instance.SendAlarm(cctv.SensorZoneID, cctv.SensorTagInfoID);
        ...
        private void SaveTagHistory(int nHeader, int nTagID, int nSensorType, int nSensorZoneID)
        {
            ...
            {
                int nMaxID = DBUtility2.WebDBManager.GetIntField(arResult[0].ToString(), 0);
                int nID = nMaxID + 1;
                if (nTagID >= 0)
                {
        ...
        private void btnAlarmClear_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in gridAlarmLog.Rows)
            {
                ...
                {
                    gridAlarmLog.Rows.Remove(row);
                    NetworkWebManager.Instance.SendAlarmClear(cctv.SensorZoneID, cctv.SensorTagInfoID);
