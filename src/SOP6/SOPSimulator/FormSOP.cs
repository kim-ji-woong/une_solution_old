...
namespace SOPMonitoringSystem
{
		...
		, IWorkflowContainer
	{
		...
		private void FormMain_Load(object sender, EventArgs e)
		{
            ...
            HistoryManager.MakeInstance();
            SectionState.HistoryManager = HistoryManager.Instance;
        ...
        private void SetControlText(bool hasControl)
        {
            ...
            SetWorkFlowControlUserID(WorkFlowManager.Instance.RealWorkFlowList);
            SetWorkFlowControlUserID(WorkFlowManager.Instance.VirtualWorkFlowList);
        }

        private void SetWorkFlowControlUserID(SortedList workFlowList)
        {
            int nControlUser = m_hasControl ? ProxySOP.Instance.SOPGenUserID : 0;

            foreach (WorkFlow work in workFlowList.Values)
            {
                work.ControlUserID = nControlUser;
            }
        }
        ...
        private void SetRealMode(bool isReal)
        {
            m_systemInput = true;
		...
		public bool PlayWithDisasterPosition(DateTime sopTime, int nZoneID, int nSensorID, int nSensorHistoryID, bool isRealMode, string strAlarmMessage = null)
        {
            ...
            RunWorkflowWithoutEvent(sopTime, nZoneID, nSensorID, nSensorHistoryID, isRealMode, strAlarmMessage);
		...
		public void RunWorkflowWithoutEvent(DateTime sopTime, int nZoneID, int nSensorID, int nSensorHistoryID, bool isRealMode, string strAlarmMessage)
		{
            try
            {
		...
		{
			if (isControl == 0)
				m_netMgr.SendMessage(SOPWebServer.Header.RETURN_CONTROL, null);
			else
				m_netMgr.SendMessage(SOPWebServer.Header.REQUEST_CONTROL, null);
		}
		...
		public void ShowRequestControl(string strUserID, string strUserNickName, string strIP)
		{
			if (m_frmRequestControl == null)
			{
			...
			m_frmRequestControl.AddUser(strUserID, strUserNickName, strIP);
		}
		...
		//public WorkFlow RunWorkflow(bool bSendSMS = false, VariousData<DateTime> dtDetect = null, string strPosition = null, string strBroadcastPositionName = null, string strPSMMaterialName = null, VariousData<int> psmDistance = null, string strAmountSnowfall = null)
		{
            ...
            if (work != null)
            {
                ...
                SOPScenario scenario = SOPScenarioManager.Instance.AddSOPScenario(szPath.Replace("\\", szDeli.ToString()), page.ActionStepID, !page.VirtualMode, page.IsNormal, page.ActionStepHistoryID, nSensorZoneHistoryID);
		...
		public void ChangeSOP(VersionInfo vInfo, ActionStepInfo aInfo, bool isRealMode)
		{
            ...
            {
                int nSensorZoneHistoryID = work.Option.SensorZoneHistoryID;
                SOPScenarioManager.Instance.AddSOPScenario(node.FullPath.Replace("\\", szDeli.ToString()), page.ActionStepID, !page.VirtualMode, page.IsNormal, page.ActionStepHistoryID, nSensorZoneHistoryID);
            }
		}
