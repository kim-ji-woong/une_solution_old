...
namespace SOPMonitoringSystem
{
	public class SOPScenarioManager : ISOPPageContainer
	{
		...
		public SOPScenario AddSOPScenario(string strPath, int nActionStepID, bool bReal, bool isNormal, int nActionStepHistoryID, int nSensorZoneHistoryID)
		{
			bool bRegular = FormSOP.Instance.IsRegular;
			//bool bNormal = FormSOP.Instance.IsNormal;
            return AddSOPScenario(strPath, nActionStepID, bReal, bRegular, isNormal, nActionStepHistoryID, nSensorZoneHistoryID);
		}
		...
		public SOPScenario AddSOPScenario(string strPath, int nActionStepID, bool bReal, bool bRegular, bool bNormal, int nActionStepHistoryID, int nSensorZoneHistoryID)
		{
        ...
        private void AddSOPSectionLog(int nActionStepID, Sections.Section section, int nComponentHistoryID, bool isRealMode, int nStatus, int nProcessDirections, ArrayList arrSections, string strTask, DateTime time, string strDescription, bool showBoard, int nCheckedNotify1, int nCheckedNotify2, int nCheckedRun, int nCheckedComplete, int nAccessedUserID, Dictionary<int, List<HistorySectionData.DetailData>> dicDetailDatas, WorkFlow workFlow)
		{
			...
			{
                HistoryManager.Instance.AddSectionHistory(section, sectionState, nComponentHistoryID, state, nProcessDirections, true, time, showBoard, nCheckedNotify1, nCheckedNotify2, nCheckedRun, nCheckedComplete, sectionState.DetailDatas);
				m_nInitHistoryLogCount++;
			}
			else if (type == Sections.Section.ComponentType.DECISION)
			{
				...
				HistoryManager.Instance.AddDecisionHistory((Sections.SectionDecision)section, sectionState, state, nProcessDirections, nextSection, true, time, showBoard);
				m_nInitHistoryLogCount++;
			}
			else if (type == Sections.Section.ComponentType.INTERNAL)
			{
				...
				HistoryManager.Instance.AddInternalHistory((Sections.SectionInternal)section, sectionState, state, nProcessDirections, nCheckedRun, nCheckedComplete, usePopupMessage, useSMS, useBroadcast, true, time, showBoard, nCheckedNotify1);
				m_nInitHistoryLogCount++;
			}
