...
namespace UnE.SOP
{
    namespace Utility
    {
        ...
        {
            // 앞의 공백문자를 제거
            static protected string TrimStart(string str)
            {
                return str.TrimStart(new char[] { ' ', '\t', '\r', '\n' });
            }

            // 뒤의 공백문자를 제거
            static protected string TrimEnd(string str)
            {
                return str = str.TrimEnd(new char[] { ' ', '\t', '\r', '\n' });
            }
        }

        public class SOPSimulatorScript : Script
        {
            public class DataParameter
            {
                ...
                // 1이면 실제모드, 0이면 훈련모드, -1이면 무시
                private int m_nRealMode = -1;
                // 1이면 평일모드, 0이면 야간 및 휴일모드, -1이면 무시
                private int m_nNormalMode = -1;
                // 유해화학물질
                private string m_strPSMMaterialType = "";
                // 유해화학물질 누출시 대피거리(미터)
                private int m_dPSMDistance = 0;
                // 적설량(cm)
                ...
                public int RealMode
                {
                    get { return m_nRealMode; }
                    set { m_nRealMode = value; }
                }

                // 1이면 평일모드, 0이면 야간 및 휴일모드, -1이면 무시
                public int NormalMode
                {
                    get { return m_nNormalMode; }
                    set { m_nNormalMode = value; }
                }

                // 유해화학물질
                public string PSMMaterialType
                {
                    get { return m_strPSMMaterialType; }
                    set { m_strPSMMaterialType = value; }
                }

                // 유해화학물질 누출시 대피거리(미터)
                public int PSMDistance
                {
                    get { return m_dPSMDistance; }
                    set { m_dPSMDistance = value; }
                }

                // 적설량(cm)
                // "2~3 cm", "많은양의 눈" 등과 같은 다양한 형태의 적설량을 표현할 수 있도록 하기 위하여
                // 숫자 대신 문자열을 사용한다.
                ...
                public DataParameter(string strMsg, DateTime dtTime, string strPlace, string strAlarmMessage)
                {
                    m_strMsg = strMsg;
                    m_dtTime = dtTime;
                    m_strPlace = strPlace;
            ...
            //static public string Parse(string strMsg, DateTime dtTime, string strPlace, int nRealMode = -1, int nNormalMode = -1)
            {
                string strResult = "";
                int nBeginIndex = 0;
                ...
                while (nBeginIndex >= 0 && nBeginIndex < nLen)
                {
                    ...
                    //strResult += ChangeSpecialMessage(param.Message.Substring(nIndex1, nIndex2 - nIndex1 + 1), param.Time, param.Place, param.RealMode, param.NormalMode);
            ...
            //static private string ChangeSpecialMessage(string strToken, DateTime dtTime, string strPlace, int nRealMode, int nNormalMode)
            {
                ...
                else if (nPlusIndex >= 0)
                {
                    ...
                    {
                        strTag = strToken.Substring(nBeginIndex, nPlusIndex - nBeginIndex);
                        strTag = TrimEnd(strTag);
                    }
                ...
                else if (string.Compare(strTag, "real_day", true) == 0 && nColonIndex >= 0)
                {
                    ...
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.RealMode == 1 && param.NormalMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "real_night", true) == 0 && nColonIndex >= 0)
                {
                    // 실제모드 && 야간 및 휴일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.RealMode == 1 && param.NormalMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "virtual_day", true) == 0 && nColonIndex >= 0)
                {
                    // 훈련모드 && 주간모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.RealMode == 0 && param.NormalMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "virtual_night", true) == 0 && nColonIndex >= 0)
                {
                    // 훈련모드 && 야간 및 휴일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.RealMode == 0 && param.NormalMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "real", true) == 0 && nColonIndex >= 0)
                {
                    // 실제모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.RealMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "virtual", true) == 0 && nColonIndex >= 0)
                {
                    //  훈련모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.RealMode == 0))
                        return strResult;
                }
                else if (string.Compare(strTag, "day", true) == 0 && nColonIndex >= 0)
                {
                    // 평일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.NormalMode == 1))
                        return strResult;
                }
                else if (string.Compare(strTag, "night", true) == 0 && nColonIndex >= 0)
                {
                    //  야간 및 휴일모드에서만 나오는 문자열
                    if (GetModeString(ref strResult, nBeginIndex, nEndIndex, strToken, param.NormalMode == 0))
                        return strResult;
                }
                ...
                {
                    return param.PSMMaterialType == null ? "" : param.PSMMaterialType;
                }
                else if (string.Compare(strTag, "psmdistancem", true) == 0)
                {
                    return param.PSMDistance <= 0 ? "0 미터" : param.PSMDistance.ToString() + " 미터";
                }
                else if (string.Compare(strTag, "psmdistancekm", true) == 0)
                {
                    if (param.PSMDistance >= 100)
                        return param.PSMDistance <= 0 ? "0 미터" : string.Format("{0:F1} 킬로미터", param.PSMDistance / 1000.0);
                    else if (param.PSMDistance >= 10)
                        return param.PSMDistance <= 0 ? "0 미터" : string.Format("{0:F2} 킬로미터", param.PSMDistance / 1000.0);
                    //else
                        return param.PSMDistance <= 0 ? "0 미터" : string.Format("{0:F3} 킬로미터", param.PSMDistance / 1000.0);
                }
                else if (string.Compare(strTag, "snow_depth", true) == 0)
                {
                    if (GetSnowDepthString(ref strResult, nBeginIndex, nEndIndex, strToken, param.AmountSnowfall))
                        return strResult;
                }
                else if (string.Compare(strTag, "alarmmessage", true) == 0)
                {
                    if (GetAlarmMessageString(ref strResult, nBeginIndex, nEndIndex, strToken, param.AlarmMessage))
                        return strResult;
                }
            ...
            static public bool GetSOPFullModeString(ref string strResult, int nRealMode)
            {
                if (nRealMode == 1)
                    strResult = "실제상황";
                else if (nRealMode == 0)
            ...
            static public bool GetSOPModeString(ref string strResult, int nRealMode)
            {
                if (nRealMode == 1)
                    strResult = "실제";
                else if (nRealMode == 0)
            ...
            static private bool GetAlarmMessageString(ref string strResult, int nBeginIndex, int nEndIndex, string strToken, string strAlarmMessage)
            {
                int nIndex = GetNotEmptyIndex(strToken, nBeginIndex, nEndIndex - 1);
            ...
            static private bool GetTimeString(ref string strResult, int nBeginIndex, int nEndIndex, string strToken, DateTime dtTime)
            {
                ...
                string strOption = strToken.Substring(nBeginIndex, nEndIndex - nBeginIndex);
                strOption = TrimEnd(strOption);
                ...
                {
                    strOption = strToken.Substring(nBeginIndex, nIndex - nBeginIndex);
                    strOption = TrimEnd(strOption);
