...
namespace SOPMonitoringSystem
{
	namespace Process
	{
		class TaskNotifyProcess : ProcessIF
		{            	
			...
			public void MakePhoneList(SectionDataProcess data)
			{
				...
				foreach (SOPTeam teamData in arTargetTeamList)
				{
					...
					if (teamData.TeamType == 0 || teamData.TeamType == 1)   // 평일 조직 또는 야간 조직
					{
						bool bEmergency = ((teamData.TeamType == 0) ? true : false);
						arrMembers = new ArrayList();
						
						FormMain.Instance.SOPManager.GetCompanyMemberList(teamData.TeamID, bEmergency, ref arrMembers);
                   
					}
					else if (teamData.TeamType == 3)    // 사용자 정의 조직
					{
						Data_ExternalTeam team = FormMain.Instance.SOPManager.GetUserDefinedTeamMember(teamData.TeamID);
					...
					else if (teamData.TeamType == 4)    // 정규 조직
					{
						arrMembers = new ArrayList();
						FormMain.Instance.SOPManager.GetRegularCompanyMemberList(teamData.TeamID, ref arrMembers);
					}
			...
			{
				strMessage = FormMain.Instance.DBManager.SMS_ADD_TEXT + strMessage;
