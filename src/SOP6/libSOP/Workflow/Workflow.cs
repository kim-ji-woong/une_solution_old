    ...
    namespace SOP
    {
        namespace Workstate
        {
			...
			public class WorkFlow
			{
                ...
                public SectionState SelectedSectionState
                {
                    get { return m_selectedSection; }
                    set { SelectSectionState(value); }
                }
                ...
                private SectionState m_selectedSection = null;
                ...
                public void LogState(Section section, SectionState state, int nCheckedRun, int nCheckedComplete)
                {
                    ...
                    History.IHistoryManager history = WorkFlowManager.HistoryManager;
                    //History.HistoryManager history = History.HistoryManager.Instance;
                    History.HistorySectionData data = history.AddSectionHistory(section, state, state.State, state.ProcessDirections, true, state.CheckNotify1, state.CheckNotify2, state.CheckedRun, state.CheckedComplete, state.DetailDatas);
				...
				{
                    state.AccessedUserID = this.ControlUserID;
                    //state.AccessedUserID = ProxySOP.Instance.SOPGenUserID;
					...
					if (type == Section.ComponentType.ENDPOINT)
					{
                        ...
                        if (state.AccessedUserID == ProxySOP.Instance.SOPGenUserID)
                            historyData = history.AddSectionHistory(section, state, state.State, state.ProcessDirections, true, 0, 0, 0, 0, state.DetailDatas);
					}
					else if (type == Section.ComponentType.PROCESS)
					{
                        if (state.AccessedUserID == ProxySOP.Instance.SOPGenUserID)
                        {
                            ...
                            historyData = history.AddSectionHistory(section, state, state.State, state.ProcessDirections, true, nCheckedNotify1, nCheckedNotify2, state.CheckedRun, state.CheckedComplete, state.DetailDatas);
                        }
					}
					...
					{
                        if (state.AccessedUserID == ProxySOP.Instance.SOPGenUserID)
                            historyData = history.AddDecisionHistory((SectionDecision)section, state, state.State, state.ProcessDirections, DecisionNextSection, true);
					}
					else if (type == Section.ComponentType.INTERNAL)
					{
                        if (state.AccessedUserID == ProxySOP.Instance.SOPGenUserID)
                        {
                            ...
                            {
                                int nCheckedNotify1 = state.CheckNotify1;
                                historyData = history.AddInternalHistory((SectionInternal)section, state, state.State, state.ProcessDirections, nCheckedNotify1, state.CheckedRun, state.CheckedComplete, data.UsePopupMessage, data.UseMobileApp, data.UseBroadcast, true);
                            }
                        }
					}
                    ...
                    if (historyData != null)
                        ...
                        state.Time = new VariousData<DateTime>(DateTime.Now);
				}
				...
				public ArrayList FindNext(SectionState stateCurrent)
				{
					...
					Section section = stateCurrent.Section;
                ...
                public void InitSectionContents()
                {
                    foreach (SectionState state in mSectionList.Values)
                    {
                        if (state.SectionContents != null)
                        {
                            state.SectionContents.Collapsed = false;

                            if (state.Section is SectionEndPoint)
                            {
                                SectionDataEndPoint data = (SectionDataEndPoint)state.Section.Data;
                                ...
                                if (data.IsBegin)
                                {
                                    ...
                                    state.SectionContents.State = Workstate.State.DONE;
                                    state.SectionContents.EnableControl = false;
                                    continue;
                                }
                            }

                            state.SectionContents.EnableControl = true;
                        }
                    }
                }

                public void PostDoneState()
                {
                    foreach (SectionState state in mSectionList.Values)
                    {
                        if (state.SectionContents != null)
                        {
                            if (state.Section is SectionEndPoint)
                            {
                                ...
                                {
                                    // SOP가 시작되었으니 시작 Section은 완료 상태로 바꾼다.
                                    state.SectionContents.State = Workstate.State.NORMAL;
                                    state.SectionContents.EnableControl = true;
                                    continue;
                                }
                            }
                ...
                {
                    if (section == null)
                        SelectSectionState(null);
                    else
                    {
                        SectionState state = FindState(section);

                        if (state == null)
                            return;

                        SelectSectionState(state);
                    }
                }

                public void SelectSectionState(SectionState state)
                {
                    if (m_selectedSection != state)
                    {
                        if (m_selectedSection != null)
                        {
                            m_selectedSection.IsSelected = false;

                            if (m_selectedSection.Section.Shape.ShapeStyler != null)
                                m_selectedSection.Section.Shape.ShapeStyler.SetCurrent(false);
                        }

                        m_selectedSection = state;

                        if (m_selectedSection != null)
                        {
                            m_selectedSection.IsSelected = true;

                            if (m_selectedSection.Section.Shape.ShapeStyler != null)
                                m_selectedSection.Section.Shape.ShapeStyler.SetCurrent(true);
                        }
                    }
                }
                ...
                public void SetSectionContents(Dictionary<Section, ISectionContents> sectionContents)
                {
                    if (sectionContents == null)
                        return;

                    ISectionContents contents;

                    foreach (SectionState state in mSectionList.Values)
                    {
                        if (sectionContents.TryGetValue(state.Section, out contents))
			...
			public class WorkFlowManager
			{
                ...
                public static History.IHistoryManager HistoryManager
                {
                    get;
                    set;
                }
                ...
                public WorkFlow Get(int nActionStepHistoryID)
                {
                    foreach (WorkFlow work in mWorks.Values)
                    {
                        if (work.ActionStepHistoryID == nActionStepHistoryID)
                            return work;
                    }
                ...
                public SectionState Find(Section section, bool bReal, out WorkFlow workflow)
                {
                    workflow = null;
