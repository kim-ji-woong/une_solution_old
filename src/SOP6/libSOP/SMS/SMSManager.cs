    ...
    namespace SOP
    {
        namespace SMS
        {
            public class SMSManager : IDisposable
            {
                ...
                private ISMSOwner m_owner = null;

                public ISMSOwner SMSOwner
                {
                    get { return m_owner; }
                    set { m_owner = value; }
                }
                ...
                public void SetVIPPhoneNumbers(WebDBManager dbMgr, int nSiteID)
                {
                    ...
                    foreach (object obj in arrResult)
                    {
                        ...
                        try
                        {
                            string strPhoneNumber = AES256Cipher.AES_decrypt(strEncrypt, key);
                ...
                private string SendSMS(string strPhoneNumber, string strSendPhoneNumber, string strMsg)
                {
                    ...
                    {                        
                        ...
                        m_msgClient = libSMS.MessageClientFactory.CreateMessageClient(UnE.SOP.ProxySOP.Instance.SiteID);
                    ...
                    if (m_msgClient.SendSMS(content))
                        return "OK";
                ...
                public bool SendSMS(ArrayList arrPhoneNumbers, string strSendPhoneNumber, string strMsg)
                {
                    ...
                    if (m_owner != null)
                    {
                        return m_owner.SendSMS(arrPhoneNumbers, strSendPhoneNumber, strMsg);
                    }
                    else
                    {
                        ...
                        foreach (string szMsg in arrMessages)
                        {
                            foreach (string szPhone in arrPhoneNumbers)
                            {
                                if (szPhone != null && !szPhone.Equals(""))
                                {
                                    ...
                                    content.Caller = strSendPhoneNumber;
                                    content.PhoneNumbers.Add(szPhone);
            ...
            public interface ISMSOwner
            {
                bool SendSMS(ArrayList arrPhoneNumbers, string strSendPhoneNumber, string strMsg);
            }
        }
    }
}
