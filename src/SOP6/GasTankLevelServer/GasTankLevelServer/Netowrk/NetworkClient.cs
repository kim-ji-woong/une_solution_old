...
namespace GasLevelServer
{
	public class NetworkClient
	{
		...
		public NetworkClient(WebDBManager dbMgr, string strServerAddr, int nSiteID)
		{
            m_instance = this;
            m_nSiteID = nSiteID;
            ...
            strServerAddr = m_dbMgr.WebServerURL;
		...
		public bool SendSensorData(int nEquipZoneID, int nSensorType, int nData, string szBuilding, string szTag, bool bPSM = false)
		{
            ...
            byte[] zoneIDBytes = SOPClientProvider.MakeBytes(nEquipZoneID);
            ...
            // 바로 보내지 않고 Queue에 쌓아둔다.
            // Queue에 쌓인 데이터는 ConnectionThread에서 한꺼번에 보낸다.
            m_msgQueue.Add(new QueueData_SensorData(bytes, nEquipZoneID, nSensor));
            return true;
