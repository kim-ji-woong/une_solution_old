...
namespace SensorTester
{
    public class NetworkManager
	{
		...
		public NetworkManager(WebDBManager dbMgr, string strServerAddr, int nSiteID)
		{
            m_nSiteID = nSiteID;
            ...
            m_strServerAddr = m_dbMgr.WebServerURL; 
		}

		public int GetServerPort()
		{

            string strSQL = string.Format("Select Port from SensorServerPort where SiteID = {0} AND Name='{1}'", m_nSiteID.ToString(), "S1SensorServer");
        ...
        public void RunConnectionThread(string strServerAddr, int nPort)
        {
            KeyValuePair<string, int> pair = new KeyValuePair<string, int>(strServerAddr, nPort);
        ...
        {
            KeyValuePair<string, int> pair = (KeyValuePair<string, int>)arg;

            string strServerAddr = pair.Key;
            int nPort = pair.Value;

            int nIndex = strServerAddr.IndexOf(':');

            if (nIndex > 0)
                strServerAddr = strServerAddr.Substring(0, nIndex);

            ArrayList arrDatas = new ArrayList();
            byte[] bytes = TcpLib2.TcpHelper.MakeBytes(SDMS.TCP_ID.ARE_YOU_THERE, arrDatas);
            ...
            while (!shutdownThread)
            {
                if (m_provider.IsConnected)
                {
                    ...
                    {
                        m_provider.Send_NoLengthByte(bytes, 0, bytes.Length);
                    }
                    else
                    {
                ...
                if (!m_provider.IsConnected)
                {
                    ...
                    if (nPort > 0)
                        m_provider.Connect(strServerAddr, nPort);
                }
