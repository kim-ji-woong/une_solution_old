...
namespace SensorTester
{
    public class DataManager
    {
        ...
        private bool LoadSensorTagID()
        {
            string strSQL = "select ID, TagID from SensorTagInfo";
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL);
            ...
            for (int i = 0; i < nResultCount - 1; i += 2)
            {
                ...
                if (m_dicSensorTags.TryGetValue(id.Data, out tag))
                {
                    tag.SensorTagID = tagID.Data;
                }
            }
        ...
        private TreeNode GetSensorParent(TreeView tree, SensorTag tag)
        {
            ...
            else if (tag.TagType == SensorTag.SensorType.PSM센서)
            {
                nTag = (int)tag.TagType;
                szTagName = "누출센서";
            }
            else if (tag.TagType == SensorTag.SensorType.정전센서)
            {
                nTag = (int)tag.TagType;
                szTagName = "정전센서";
            }
