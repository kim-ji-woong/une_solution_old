...
namespace UnE.Spatial
{
    ...
    {
        private class FloorSort : IComparer
        {
            public int Compare(object obj1, object obj2)
            {
                ...
                if (obj1 is Zone && obj2 is Zone)
                {
                    Zone zone1 = (Zone)obj1;
                    Zone zone2 = (Zone)obj2;

                    if (zone1.FloorIndex < zone2.FloorIndex)
                        return -1;
                    else if (zone1.FloorIndex == zone2.FloorIndex)
                        return 0;
                    else
                        return 1;
                }
        ...
        public ZoneManager()
        {
            int nSiteID = UnE.Util.UtilMethods.ReadSiteID();
        ...
        // Sheter 타입별 대피소 리스트
        // Value : Key => Shelter ID
        private Dictionary<Shelter.ShelterTypes, Dictionary<int, Shelter>> m_dicShelters = new Dictionary<Shelter.ShelterTypes, Dictionary<int, Shelter>>();
        ...
        public void LoadShelters(ArrayList arrResult)
        {
            m_dicShelters.Clear();
            ...
            for (int i = 0; i < nResultCount - 6; i += 7)
            {
                ...
                VariousData<int> shelterType = WebDBManager.GetIntField(arrResult[i + 2].ToString());
                VariousData<int> shelterIDType = WebDBManager.GetIntField(arrResult[i + 3].ToString());
                VariousData<int> shelterID = WebDBManager.GetIntField(arrResult[i + 4].ToString());
                ...
                if (id == null || strShelterName == null || shelterType == null || shelterIDType == null)
                    continue;

                int nShelterType = shelterType.Data;
                ...
                if (shelterID != null)
                {
                    if (shelterIDType.Data == (int)Shelter.ShelterIDTypes.Building)
                        shelter.LinkedBuilding = this.GetBuilding(shelterID.Data);
                    else if (shelterIDType.Data == (int)Shelter.ShelterIDTypes.Zone)
                        shelter.LinkedZone = this.GetZone(shelterID.Data);
                    else if (shelterIDType.Data == (int)Shelter.ShelterIDTypes.EquipZone)
                        shelter.LinkedEquipZone = this.GetEquipZone(shelterID.Data);
                }

                if (strBoundary != null)
                {
                    MakeMultiZonePolygon(strBoundary, shelter.Boundaries);
                    //shelter.Boundary = MakeZonePolygon(strBoundary);
                }

                foreach (Shelter.ShelterTypes type in Enum.GetValues(typeof(Shelter.ShelterTypes)))
                {
                    int nType = (int)type;

                    if ((nShelterType & nType) != nType)
                        continue;

                    Dictionary<int, Shelter> dicShelters = null;

                    if (m_dicShelters.TryGetValue(type, out dicShelters) == false)
                    {
                        dicShelters = new Dictionary<int, Shelter>();
                        m_dicShelters[type] = dicShelters;
                    }

                    shelter.ShelterType = (shelter.ShelterType | nType);
                    dicShelters[shelter.ID] = shelter;
                }
            }
        }
        ...
        public Dictionary<int, Shelter> GetShelters(Shelter.ShelterTypes shelterType)
        {
            Dictionary<int, Shelter> dicShelters = null;
            m_dicShelters.TryGetValue(shelterType, out dicShelters);
            return dicShelters;
        }
        ...
        private void MakeMultiZonePolygon(string szBoundary, List<UnE.Geometry.Polygon> polygons)
        {
            if (szBoundary == null || szBoundary == "" || szBoundary == "null")
                return;
        ...
        public void LoadZones()
        {
            ...
            string strSQL = "select id, ZoneName, BuildingID, FloorIndex, Boundary, DXFFileName, DXFAccessedTime, _3DFileName, _3DAccessedTime, BroadcastName, AddFloor, Azimuth, DisplayText from Zone" +
                            " where SiteID = " + webDB.SiteID.ToString();
            ...
            for (int i = 0; i < nResultCount - 12; i += 13)
            {
                ...
                if (m_dicBuildings.ContainsKey(nBuildingID))
                {
                    ...
                    // FloorIndex가 10000 이상일 경우 건물전체 대표 EquipZone이므로 FloorList에 추가하지 않는다.
                    if (zone.FloorIndex < 10000)
            ...
            FloorSort sort = new FloorSort();

            foreach (KeyValuePair<int, Building> pair in m_dicBuildings)
            {
                pair.Value.FloorList.Sort(sort);
            }
        }
        ...
        public string CheckZoneBroadcastName(float x, float y)
        {
            Zone zone = GetOutsideZone(x, y);
        ...
        public string GetBuildingName(float x, float y)
        {
            // 영흥인경우
            if (m_dbMgr.SiteID == 2)
            {
            ...
            foreach (KeyValuePair<int, Building> kv in m_dicBuildings)
            {
                Building ar = kv.Value;
                if (ar == null || ar.FloorList.Count == 0)
                    continue;

                Zone obj = (Zone)ar.FloorList[0];
                if (obj != null && obj.Polygon != null && obj.Polygon.GetVertexCount() >= 3)
                {
                    if (obj.Polygon.HitTest(vertex) != 0)
                    {
                        if (obj.Building == null)
                            ...
                            return obj.Building.BuildingName;
                    }
                }
            }
            return "";
        ...
        //public Zone GetBuildingZone(float x, float y)
        //{
        ...
        //    foreach (KeyValuePair<int, ArrayList> kv in m_dicBuildingZones)
        //    {
        //        ArrayList ar = kv.Value;
        ...
        /// <summary>
        /// 유해화학설비가 가설된 EquipmentZone 목록 가져오기
        /// </summary>
        /// <param name="building">Building Object</param>
        /// <returns>EquipmentZone Array</returns>
        public EquipmentZone[] GetPSMEquipmentZone(Building building = null)
        {
            List<EquipmentZone> liEquipmentZone = new List<EquipmentZone>();

            // 유효한 EquipmentZone정보를 가진 EquipmentZone목록 필터링
            foreach (EquipmentZone equipzone in from equipzones in m_dicEquipZones.Values
                                                where equipzones.ZoneType == EquipmentZone.EquipZoneType.PSM_TYPE
                                                && (equipzones.Building == building || building == null)
                                                orderby equipzones.ZoneName ascending
                                                select equipzones
                                               )
            {
                if (liEquipmentZone.Contains(equipzone) == true)
                    continue;

                liEquipmentZone.Add(equipzone);
            }

            return liEquipmentZone.ToArray();
        }
        ...
        public void LoadEquipmentZone()
        {
            ...
            string strSQL = "select id, ZoneName, Boundary, LinkedZoneIDList, type, BroadcastName, DisplayText from EquipmentZone where ID > 0" +
                            " and SiteID = " + m_dbMgr.SiteID.ToString();
        ...
        public List<EquipmentZone> GetEquipmentZoneList(Zone zone)
        {
        ...
        public ArrayList FindZoneListByEquipZoneID(int[] nEquipZoneIDs)
        {
            ArrayList arrSelectZoneList = new ArrayList();

            foreach (int nEquipZoneID in nEquipZoneIDs)
            {
                if (m_dicEquipZones.ContainsKey(nEquipZoneID) == true)
                {
                    foreach (Zone zone in m_dicEquipZones[nEquipZoneID].LinkedZoneList)
                    {
                        if (arrSelectZoneList.Contains(zone) == false)
                            arrSelectZoneList.Add(zone);
                    }
                }
            }

            return arrSelectZoneList;
        }
        ...
        public void Load3DText()
        {
            ...
            string strSQL = "select ID, Name, DisplayText, TextColor, TextFontHeight, TextCenter from _3DText where SiteID = " + m_dbMgr.SiteID.ToString();
            ArrayList arrResult = webDB.GetResultData(strSQL);
    ...
    public class Shelter
    {
        ...
        private Building m_linkedBuilding = null;
        public Building LinkedBuilding
        {
            get { return m_linkedBuilding; }
            set { m_linkedBuilding = value; }
        }
        ...
        private EquipmentZone m_linkedEquipZone = null;
        public EquipmentZone LinkedEquipZone
        {
            get { return m_linkedEquipZone; }
            set { m_linkedEquipZone = value; }
        }
