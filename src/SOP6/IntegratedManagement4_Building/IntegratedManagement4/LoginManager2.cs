...
namespace IntegratedManagement4
{
	public class LoginManager
	{
		...
		private void RejectLoginThread(object param)
		{
			...
			m_frmMain.Invoke((MethodInvoker)delegate
			{
				if (nType == 1)
				{
					...
					FormMessageBox msgBox = new FormMessageBox("아이디 또는 비밀번호가 맞지 않습니다.", "로그인 경고", MessageBoxButtons.OK);
				...
				else if (nType == 2)
				{
					...
					FormMessageBox msgBox = new FormMessageBox("이미 로그인 중인 아이디입니다.", "로그인 경고", MessageBoxButtons.OK);
					msgBox.StartPosition = FormStartPosition.CenterParent;
					msgBox.ShowDialog();
				}
				else if (nType == 3)
				{
					...
					FormMessageBox msgBox = new FormMessageBox("삭제된 사용자이거나 사용할 수 없는 아이디입니다.", "로그인 경고", MessageBoxButtons.OK);
					msgBox.StartPosition = FormStartPosition.CenterParent;
					msgBox.ShowDialog();
				}
		...
		public bool JoinUser(int nID, string szMemberID, string szPass, string szNickName, int nLevel)
		{
			string strEncrypt = AES256Cipher.AES_encrypt(szPass, key);

			return m_NetMgr.RegisterUser(nID, szMemberID, strEncrypt, szNickName, nLevel);
		}
        ...
        public bool CheckID(string strID)
        {
			bool bRet = false;

			string szText = "select ID from SOPGenUser where SiteID = {0} and UserID = '{1}'";
			string strSQL = string.Format(szText, m_dbMgr.SiteID, strID);
		...
		public bool CheckJoinCode(string strJoinCode)
		{
			bool bRet = false;

			string szText = "select PropertyName, PropertyValue from OptionSDMS where SiteID = {0} and PropertyName = 'UserJoinCode' and PropertyValue = '{1}'";
			string strSQL = string.Format(szText, m_dbMgr.SiteID, strJoinCode);
