...
namespace IntegratedManagement4
{
    public partial class FormMain : Form
	{
        ...
        private bool m_bSystemAutoRun = false;

        public FormMain(bool systemAutoRun) : this()
        {
            if (!systemAutoRun)
                return;

            m_bSystemAutoRun = systemAutoRun;
        }
		...
		public FormMain()
		{
            ...
            //m_cbLevel.customComboBox.SelectionChanged += EleTypeComboBox_SelectionChanged;
		...
		private void FormMain_Load(object sender, EventArgs e)
		{
            if (SiteID == 205)
            {
                this.BackgroundImage = Properties.Resources.background_urban;
                pictureBox1.Image = IntegratedManagement4.Properties.Resources.title_urbanbricks;
                pictureBox1.Size = new Size(200, 43);

                pictureBox2.Image = IntegratedManagement4.Properties.Resources.title_urbanbricks;
                pictureBox2.Size = new Size(200, 43);
            }
        ...
        private void SystemAutoRun()
        {
            ...
            btnApp_Click(btnSOPSimulator, null);
            btnApp_Click(btnSDMS, null);
        }
        ...
        private void btnRegNext(object sender, EventArgs e)
        {
            try
            {
                ...
                /*else if (txtAddNickName.Text.Length == 0)
                {
                    ...
                    throw new ApplicationException("소속이 설정되지 않았습니다.");
                }*/
                ...
                // db 접근하여 옵션sdms에서 UserJoinCode 검사
                if (!m_logInMgr.CheckJoinCode(textBoxCode.Text))
                {
                    FormMessageBox msgBox = new FormMessageBox("회원가입 코드가 일치하지 않습니다.\r\n다시 한번 확인해 주세요.", "알림", MessageBoxButtons.OK);
                    msgBox.StartPosition = FormStartPosition.CenterParent;
                    msgBox.ShowDialog();
                ...
                //if (!m_logInMgr.JoinUser(nCompanyMemberID, textBoxMemberID.Text, textBoxMemberName.Text, m_strNickName, m_Chief)) 
                if (!m_logInMgr.JoinUser(nCompanyMemberID, textBoxMemberID.Text, textBoxMemberName.Text, m_strNickName, m_cbLevel.customComboBox.SelectedIndex))
                {
                    //throw new ApplicationException("SOP서버가 연결되어 있지 않습니다.\n서버 실행 상태를 확인하세요.");
                }
        ...
        private void InitLevelComboBox()
        {
            ...
            dicLevel = GetGenLevel();

            if (dicLevel == null)
                return;

            for (int i = 0; i < dicLevel.Count; i++)
            {
                m_cbLevel.customComboBox.Items.Add(dicLevel[i]);
            }

            m_cbLevel.customComboBox.SelectedIndex = 0;
        }
