...
{
    public class TemperatureHumidityAlarmProcess : IDisposable, ProcessIF
    {
        ...
        private ProcessType mType = ProcessType.TemperatureHumidityAlarm;
        public ProcessType ProcessType
        {
            get { return mType; }
        }

        public TemperatureHumidityAlarmProcess()
        {
        }
        ...
        public override string ToString()
        {
            if (TargetZone != null && TargetZone.LinkedZone != null)
            {
                ...
                if (szZoneName == TargetZone.ZoneName)
                    return "[온도/습도 알람]" + TargetZone.ZoneName;

                return "[온도/습도 알람]" + szZoneName + "/" + TargetZone.ZoneName;
            }
        ...
        public void BeginProcess()
        {
            ...
            m_alarmThread = new Thread(ConfirmEarthquake);
            m_alarmThread.Name = "TemperatureHumidityAlarm_ConfirmStauts";
            m_alarmThread.Start();
        }
        ...
        public void AbortProcess()
        {
            try
            {
                if (m_alarmThread != null && m_bProcess == true)
                {
                    m_bProcess = false;

                    if (m_alarmThread.IsAlive)
                    {
                        m_alarmThread.Interrupt();
                        m_alarmThread.Abort();
                    }
                }
            }
