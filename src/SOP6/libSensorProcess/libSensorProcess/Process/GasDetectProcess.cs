...
namespace libSensorProcess
{
    ...
    /// 유해화학물질 센서
    /// </summary>
	public class GasDetectProcess : IDisposable, ProcessIF
	{
		...
		private Thread m_PSMAlarmThread = null;
        ...
        private ProcessType mType = ProcessType.PSMAlarm;
		...
		public override string ToString()
		{
			if (TargetZone != null && TargetZone.LinkedZone != null)
			{
                ...
                if (szZoneName == TargetZone.ZoneName)
                    return "[누출]" + TargetZone.ZoneName;

				return "[누출]" + szZoneName + "/" + TargetZone.ZoneName;
			}
		...
		public void BeginProcess()
		{
			...
			m_PSMAlarmThread = new Thread(ConfirmSpill);
            m_PSMAlarmThread.Name = "PSMAlarm_ConfirmPSM"; 
			m_PSMAlarmThread.Start();
		}
        ...
        public void ReadyProcess()
        {
            try
            {
                ...
                //ProcessManager.Instance.ProcessOwner.ShowSensorAlarmInvoke(this, ReactionType.NOTIFY_PSM);
                /*FormMain.Instance.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (m_LastLog != null && m_LastLog.ReactionType != (int)ReactionType.NOTIFY_PSM)
		...
		public void AbortProcess()
		{
			try
			{
				if (m_PSMAlarmThread != null && m_bProcess == true)
				{
					m_bProcess = false;

					if (m_PSMAlarmThread.IsAlive)
					{
						m_PSMAlarmThread.Interrupt();
						m_PSMAlarmThread.Abort();
					}
				}
			}
			catch (System.Exception)
		...
		public bool Select()
		{
            //System.Diagnostics.Trace.WriteLine("Call Select");
			...
			/*FormMain.Instance.Invoke((MethodInvoker)delegate
			{
                ...
                else
                {
                    if (m_TargetZone != null && m_TargetZone.LinkedZone != null)
                    {
                        ...
                        FormMain.Instance.PageHome.ShowBigCCTV(m_TargetZone.LinkedZone, 2, true);
                ...
                //{
                    if (m_TargetZone != null && m_TargetZone.LinkedZone != null)
                    {
                        ...
                        FormMain.Instance.PageHome.ContentForm.SetEvacDistance(m_nSensorID);
                        FormMain.Instance.PageHome.ContentForm.SetEvacCenter(m_TargetZone);
                        FormMain.Instance.PageHome.ContentForm.ShowEvacCircle(m_nAlarmLevel);
                ...
                //    if (m_TargetZone != null && m_TargetZone.LinkedZone != null)
                //    {
                //        if( FormMain.Instance.IsShowCCTVForm() == true)
                //        {
                //            FormMain.Instance.PageHome.ShowBigCCTV(m_TargetZone.LinkedZone, true, true);
                //            FormMain.Instance.SelectCCTVTab(false);
                //        }
        ...
        protected void OnChangeAlarmLevel(int nLevel)
        {
            ...
            if (m_bBeginProcess == true)
            {
                ...
                /*FormMain.Instance.Invoke((MethodInvoker)delegate
                {
                    FormMain.Instance.PageHome.ContentForm.ShowEvacCircle(nLevel);
                });*/
            }
		...
		public void ConfirmSpill()
		{
            ...
            //m_arCCTVs = ProcessManager.Instance.ProcessOwner.ConfirmDisasterInvoke(this, ShowFireDetectTooltipCCTV, m_nSensorID, ReactionType.NOTIFY_PSM, m_nAlarmLevel);
            try
			{
				/*FormMain.Instance.Invoke((MethodInvoker)delegate
				{
				...
				try
				{
					/*FormMain.Instance.Invoke((MethodInvoker)delegate
					{
                        ...
                        PageBackstageHome.Instance.ShowBigCCTV(m_TargetZone, 2);						
