...
namespace ServerProcess.Client
{
    ...
    public class SOPSimulatorServer : BaseClient, ISOPSimulatorServer
    {
        ...
        private static SOPSimulatorServer m_instance = null;

        // 현재 제어권 소유자
        // Key : ActionStepHistory ID
        private ConcurrentDictionary<int, ClientData> m_dicControlClient = new ConcurrentDictionary<int, ClientData>();
        // 현재 실행중인 SOP 정보
        // Key : ActionStepHistory ID
        ...
        public static SOPSimulatorServer Instance
        {
        ...
        public SOPSimulatorServer()
            : base()
        {
            m_instance = this;
        }

        public SOPSimulatorServer(Factory factory, IPostOffice postOffice)
        ...
        protected override ServerProcess.Client.ClientData MakeClientData(int nClientType, int nClientSubType, OperationContext ctx, string strIP, int nPort)
        {
            if (m_postOffice != null)
            {
                ...
                SOPSimulatorServer.ClientData data = new SOPSimulatorServer.ClientData(ctx.SessionId, postMan, nClientType, nClientSubType);
                data.IP = strIP;
                data.Port = nPort;
        ...
        protected override int OnReceiveEvent(ServerProcess.Client.ClientData data, OperationContext ctx, int header, byte[] messages, ArrayList arrDatas)
        {
            ...
            /*else if (header == SOPWebServer.Header.CONFIRM_SOP_CONTROL)
                ...
                return ProcessRequestControl((ClientData)data, arrDatas);
            else if (header == SOPWebServer.Header.CANCEL_REQUEST_CONTROL)
                return ProcessCancelRequestControl((ClientData)data, arrDatas);
                ...
                return ProcessStealControl((ClientData)data, arrDatas);
            else if (header == SOPWebServer.Header.RETURN_CONTROL)
                return ProcessReturnControl((ClientData)data, arrDatas);
                ...
                return ProcessRequestComponentHistoryList((ClientData)data, arrDatas);
            else if (header == SOPWebServer.Header.REQUEST_SENSOR_SOP_PERMIT)
                return ProcessRequestSensorSOPPermit((ClientData)data, arrDatas);

            return SOPWebServer.ErrorMessageType.UNKNOWN_HEADER;
        }

        private int ProcessRequestSensorSOPPermit(ClientData data, ArrayList arrDatas)
        {
        ...
        private int ProcessRequestComponentHistoryList(ClientData data, ArrayList arrDatas)
        {
            if (arrDatas == null)
                return SOPWebServer.ErrorMessageType.INVALID_MESSAGE;
        ...
        private int ProcessReturnControl(ClientData data, ArrayList arrDatas)
        {
            if (arrDatas.Count > 0 && arrDatas[0] is int)
            {
                ClientData controlClient;
                ...
                {
                    if (client != data)
                        clientList.Add((ClientData)client);
                }
        ...
        private int ProcessStealControl(ClientData data, ArrayList arrDatas)
        {
            if (arrDatas.Count > 0 && arrDatas[0] is int)
            {
                ClientData controlClient;
            ...
            if (arrDatas.Count >= 1 && arrDatas[0] is int)
            {
                ...
                for (int i = 1; i < arrDatas.Count; i++)
                {
                    if (arrDatas[i] is string)
                    {
                        ...
                        ClientData client = GetClientData(clients, strUserID);
        ...
        private int ProcessGiveControl(ClientData data, ArrayList arrDatas)
        {
            if (arrDatas.Count >= 2 && arrDatas[0] is int && arrDatas[1] is int)
            {
                ...
                if (nUserIDCount > 0 && arrDatas.Count >= 3 && arrDatas[2] is string)
                {
                    ...
                    ClientData controlClient = GetClientData(clients, strControlUserID);
                    ...
                    for (int i = 1; i < nUserIDCount; i++)
                    {
                        ...
                        if (arrDatas[nIndex] is string)
                        {
                            ...
                            ClientData client = GetClientData(clients, strControlUserID);
        ...
        private int ProcessCancelRequestControl(ClientData data, ArrayList arrDatas)
        {
            if (arrDatas.Count > 0 && arrDatas[0] is int)
            {
                int nActionStepHistoryID = (int)arrDatas[0];
        ...
        private int ProcessRequestControl(ClientData data, ArrayList arrDatas)
        {
            if (data.ID < 0)
                return SOPWebServer.ErrorMessageType.NO_PERMISSION;
        ...
        private ClientData GetOptimalControlClient(int nActionStepHistoryID, List<ClientData> clientList)
        {
            if (m_agent != null)
            {
        ...
        private ClientData GetClientData(List<Client.ClientData> clients, int nSOPGenUserID)
        {
        ...
        private ClientData GetClientData(List<Client.ClientData> clients, string strUserID)
        {
            foreach (Client.ClientData data in clients)
            {
                if (data is ClientData)
        ...
        private void SendNoSOPControl(int nActionStepHistoryID, ClientData data)
        {
        ...
        private void SendRequestSOPControl(int nActionStepHistoryID, ClientData controlClient, ClientData requestClient)
        {
            ArrayList arrDatas = new ArrayList();
        ...
        private bool SetControlClient(int nActionStepHistoryID, ClientData data)
        {
            DirectDBManager dbMgr = m_dbMgr.Clone();
            ...
            foreach (int nActionStepHistoryID in actionStepHistoryIDs)
            {
                if (m_dicControlClient.TryGetValue(nActionStepHistoryID, out temp) == false)
                {
                    if (m_dicActionStepHistory.TryGetValue(nActionStepHistoryID, out actionStepHistory))
                    {
                        foreach (Client.ClientData client in clients)
                        {
                            ClientData _client = (ClientData)client;
                ...
                if (m_dicControlClient.TryGetValue(nActionStepHistoryID, out temp) == false)
                {
                    if (m_dicActionStepHistory.TryGetValue(nActionStepHistoryID, out actionStepHistory))
                    {
                        ...
                        if (span.TotalSeconds >= m_nControlWaitSeconds)
                        {
                            if (m_agent != null)
                            {
                                ClientData controlClient = GetOptimalControlClient(nActionStepHistoryID, clientList);
