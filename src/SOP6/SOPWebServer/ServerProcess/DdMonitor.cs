...
namespace ServerProcess
{
    ...
    public class DdMonitor
    {
        ...
        private static AgentFactory.ILogger m_logger = null;

        public static AgentFactory.ILogger Logger
        {
            get { return m_logger; }
            set { m_logger = value; }
        }
        ...
        private static bool m_bWrietLog = false;

        /// <summary>Acquires an exclusive lock on the specified object.</summary>
        /// <param name="monitor">The object on which to acquire the monitor lock.</param>
        public static void Enter(object monitor, bool bAddLog = false)
        {
            if (bAddLog == true && m_bWrietLog == true)
            {
                if (m_logger != null)
                {
                    System.Diagnostics.StackFrame callStack = new System.Diagnostics.StackFrame(1, true);
                    string szCallStack = "File: " + callStack.GetFileName() + ", Line: " + callStack.GetFileLineNumber();
                    m_logger.Write("Lock Enter : Thread " + Thread.CurrentThread.ManagedThreadId + "Call :" + szCallStack);
                    //ConnectionLogEx.Instance.WriteLine("Lock Enter : Thread " + Thread.CurrentThread.ManagedThreadId + "Call :" + szCallStack);
                }
            }
        ...
        public static void Exit(object monitor, bool bAddLog = false)
        {
            ...
            lock (_globalLock)
            {
                ...
                if (bAddLog == true && m_bWrietLog == true)
                {
                    if (m_logger != null)
                    {
                        System.Diagnostics.StackFrame callStack = new System.Diagnostics.StackFrame(1, true);
                        string szCallStack = "File: " + callStack.GetFileName() + ", Line: " + callStack.GetFileLineNumber();
                        m_logger.Write("Lock Exit : Thread " + Thread.CurrentThread.ManagedThreadId + "Call :" + szCallStack);
                        //ConnectionLogEx.Instance.WriteLine("Lock Exit : Thread " + Thread.CurrentThread.ManagedThreadId + "Call :" + szCallStack);
                    }
                }
            }
        }
                ...
                foreach (MonitorState ms in locksHeldByThread)
                {
                    ...
                    foreach (Thread nextThread in nextThreads)
                    {
                        ...
                        if (currentChain.ContainsThread(nextThread)) throw new SynchronizationLockException(
                            CreateDeadlockDescription(currentChain, locksHeldByThreads));
