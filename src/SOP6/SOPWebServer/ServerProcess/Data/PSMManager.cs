...
namespace ServerProcess.Data
{
    ...
    {
        private static PSMManager m_instance = new PSMManager();
        private IPostOffice m_postOffice = null;
        ...
        public bool Initialize(DirectDBManager dbMgr, IPostOffice postOffice)
        {
            m_postOffice = postOffice;
            m_usePSM = CheckPSM(dbMgr);
        ...
        protected void SensorStatusThread()
        {
            if (m_postOffice == null)
                return;

            while (!m_postOffice.SystemClose)
            {
        ...
        private bool LoadSensors(DirectDBManager dbMgr)
        {
            m_dicSensors.Clear();
        ...
        private bool LoadTanks(DirectDBManager dbMgr)
        {
            m_dicTanks.Clear();
            ...
            for (int i = 0; i < nResultCount - 9; i += 10)
            {
                ...
                tank.EquipZone = SensorZoneManager.Instance.GetEquipmentZone(nEquipZoneID);
        ...
        private bool LoadMaterials(DirectDBManager dbMgr)
        {
        ...
        private bool CheckPSM(DirectDBManager dbMgr)
        {
            string strSQL = "Select PropertyValue from OptionSDMS where PropertyName = 'UsePSM' and SiteID = " + dbMgr.SiteID.ToString();
            ArrayList arrResult = dbMgr.GetResultData(strSQL);
