...
namespace AccessSecurityServer
{
    public class AccessWatcher_SQLServer : AccessWatcher
    {
        ...
        public override bool Run()
        {
            ...
            NetworkWebManager.Instance.AccessDBConnectionString = m_strConnection;
        ...
        private void MonitoringThread()
        {
            if (ReadLocation())
            {
                ...
                {
                    m_runThread = true;
                    VariousData<DateTime> dtLastEvent = GetMaxEventTime();
        ...
        private VariousData<DateTime> GetMaxEventTime()
        {
            VariousData<DateTime> eventTime = null;
            ...
            try
            {
                ...
                string strSQL = "Select max(EventDateTime) from View_External_Alarm";
                ...
                if (reader.Read())
                {
                    ...
                    eventTime = new VariousData<DateTime>((DateTime)reader[0]);
                }
        ...
        private void ReadAlarm(ref VariousData<DateTime> dtLastEvent)
        {
            try
            {
                SqlConnection connection = new SqlConnection();
                ...
                if (dtLastEvent == null)
                    strSQL = "Select AlarmID, AlarmState, EventDateTime, RecvDateTime, DeviceID, State, PreState, CardNo, Content1, Content2, Content3, Content4 from View_External_Alarm";
                else
                {
                    string strLastTime = string.Format("'{0}-{1:00}-{2:00} {3:00}:{4:00}:{5:00}'",
                        dtLastEvent.Data.Year, dtLastEvent.Data.Month, dtLastEvent.Data.Day, dtLastEvent.Data.Hour, dtLastEvent.Data.Minute, dtLastEvent.Data.Second);
                    strSQL = "Select AlarmID, AlarmState, EventDateTime, RecvDateTime, DeviceID, State, PreState, CardNo, Content1, Content2, Content3, Content4 from View_External_Alarm where EventDateTime > " + strLastTime;
                }
                ...
                while (reader.Read())
                {
                    ...
                    alarm.EventTime = reader.IsDBNull(2) ? null : new VariousData<DateTime>((DateTime)reader[2]);
                    alarm.ReceivedTime = reader.IsDBNull(3) ? null : new VariousData<DateTime>((DateTime)reader[3]);
                    ...
                    if (alarm.Device != null)
                    {
                        ...
                        if (alarm.AlarmState == Alarm.StateType.NONE || alarm.AlarmState == Alarm.StateType.UNKNOWN)
                        {
                            RemoveAlarm(alarm.Device, alarm);
                        }
                        else
                        {
                            AddAlarm(alarm);
        ...
        public override void RemoveAlarm(Device device)
        {
            Alarm alarm = null;

            foreach (KeyValuePair<int, Alarm> pair in m_dicAlarm)
            {
