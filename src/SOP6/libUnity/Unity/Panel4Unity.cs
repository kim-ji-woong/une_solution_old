...
namespace UnE.Util.Unity
{
    public partial class Panel4Unity : ToolStripContainer, ISensorTooltipOwner, IBaseView
    {
        ...
        private class POIFileData
        {
            private string m_strAddFilePath = "";
            private string m_strPOIIDFilePath = "";
            private string m_strShowFilePath = "";
            // Key : IconPath
            private Dictionary<string, POI> m_dicPOIs = new Dictionary<string, POI>();
            private Dictionary<POI, int> m_dicPOIIndex = new Dictionary<POI, int>();
            private Dictionary<int, POI> m_dicIndexPOI = new Dictionary<int, POI>();
            private List<int> m_poiIDs = null;
            private List<string> m_poiTypes = null;
            private List<bool> m_poiVisibles = null;

            public string AddFilePath
            {
                get { return m_strAddFilePath; }
                set { m_strAddFilePath = value; }
            }

            public string POIIDFilePath
            {
                get { return m_strPOIIDFilePath; }
                set { m_strPOIIDFilePath = value; }
            }

            public string ShowFilePath
            {
                get { return m_strShowFilePath; }
                set { m_strShowFilePath = value; }
            }

            public Dictionary<string, POI> POIIconPath
            {
                get { return m_dicPOIs; }
            }

            public Dictionary<POI, int> POIIndex
            {
                get { return m_dicPOIIndex; }
            }

            public Dictionary<int, POI> IndexPOI
            {
                get { return m_dicIndexPOI; }
            }
            ...
            public List<string> POITypes
            {
                get { return m_poiTypes; }
                set { m_poiTypes = value; }
            }

            public List<bool> POIVisibles
            {
                get { return m_poiVisibles; }
                set { m_poiVisibles = value; }
            }

            public POIFileData(List<POI> pois, string strAddFilePath)
            {
                m_strAddFilePath = strAddFilePath;
        ...
        // Key : POIType
        private Dictionary<string, POIFileData> m_dicPOIFileDatas = new Dictionary<string, POIFileData>();
        ...
        public int ProcessID
        {
            get
            {
                try
                {
                    return m_ProcessUnity == null ? 0 : m_ProcessUnity.Id;
                }
                catch (Exception)
                {
                }
        ...
        {
            get { return m_PipeServer != null; }
        }

        public string CurrentSceneName
        {
            get { return m_strCurrentSceneName; }
        }
        ...
        {
            System.Diagnostics.Trace.WriteLine("SetUnity");
            ...
            {
                m_ProcessUnity = StartUnityPocess(m_szUnityExePath, m_szUnityExePath, "-logFile NUL");
            }

            int nTimeout = 50;

            for (int i=0;i<nTimeout && m_hWndUnity == IntPtr.Zero;i++)
            {
                m_hWndUnity = NativeMethods.FindWindowEx(IntPtr.Zero, IntPtr.Zero, null, szUnityName);
            ...
            if (m_hWndUnity == IntPtr.Zero)
            {
                System.Diagnostics.Trace.WriteLine("Unity 실행오류");
                return;
            }
        ...
        private int AddIconPOIFile(StreamWriter writer, Dictionary<string, POI> dicPOIIconpath, POI poi, string szIconName, float x, float y, float z)
        {
            int nLayerID = poi.Facility.GetLayerID();
            ...
            if (nID <= 0)
            {
                return _AddIconPOIFile(writer, szIconName + "_" + nLayerID + "_" + poi.Facility.ID, x, y, z);
            }
            else*/
            {
                _AddIconPOIFile(writer, dicPOIIconpath, poi, szIconName + "_" + nLayerID + "_" + poi.Facility.ID, x, y, z);
            }
            return poi.Facility.ID;
        }

        private void _AddIconPOIFile(StreamWriter writer, Dictionary<string, POI> dicPOIIconpath, POI poi, string szIconName, float x, float y, float z)
        {
            string szCmd = string.Format("{0},{1},{2},{3}", x, y, z, szIconName);
            writer.WriteLine(szCmd);
            dicPOIIconpath[szIconName] = poi;
        }
        ...
        public void AddGroupNameFile(string strFilePath, List<string> cmdList)
        {
            if (m_PipeServer != null)
            {
                ...
                string szCmd = string.Format("CMD:AddReverseLODTextPOIFile('{0}')", strFilePath);
                m_PipeServer.Send(szCmd);
            }
        }
        ...
        {
            string szCmd = string.Format("CMD:SelectScene('{0}')", strSceneName);
            if (m_PipeServer != null)
            {
                m_PipeServer.Send(szCmd);
        ...
        {
            string szCmd = string.Format("CMD:ShowAlarmZone('{0}', {1})", strZoneName, hideAllOthers ? "True" : "False");
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }
        ...
        {
            string szCmd = string.Format("CMD:HideAlarmZone('{0}')", strZoneName);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }
        ...
        {
            string szCmd = "CMD:HideAllAlarmZones()";
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }
        ...
        public bool GetWalls(string strPath)
        {
            ...
            string szCmd = string.Format("CMD:GetWalls('{0}')", strPath);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        {
            strPath = strPath.Replace('\\', '/');
            string szCmd = string.Format("CMD:LoadWalls('{0}','{1}')", strPath, strSceneName);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        {
            if (m_Owner != null)
                m_Owner.ChangeSpaceText();
        }

        public void SetWallSnap(bool bUse)
        {
            string szCmd = string.Format("CMD:SetUseSnap({0})", bUse ? "True" : "False");
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }
        ...
        {
            string szCmd = string.Format("CMD:SetWallEditMode({0})", bEdit ? "True" : "False");
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }
        ...
        {
            if (m_Owner != null)
                m_Owner.GetWallInfo(x, y, scale, rotate);
        }

        public bool AddSpaceText(string strTxt)
        {
            string szCmd = string.Format("CMD:AddSpaceText('{0}')", strTxt);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        public bool ChangeColorSpaceText(string hexColor)
        {
            string szCmd = string.Format("CMD:ChangeColorSpaceText('{0}')", hexColor);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        public bool ChangeFontSpaceText(string name, float nSize, int nStyle)
        {
            string szCmd = string.Format("CMD:ChangeFontSpaceText('{0}',{1},{2})", name, nSize, nStyle);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        public bool GetSpaceTexts(string strPath)
        {
            strPath = strPath.Replace('\\', '/');
            string szCmd = string.Format("CMD:GetSpaceTexts('{0}')", strPath);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        public bool LoadSpaceTexts(string strPath, string strSceneName)
        {
            strPath = strPath.Replace('\\', '/');
            string szCmd = string.Format("CMD:LoadSpaceTexts('{0}','{1}')", strPath, strSceneName);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        ...
        private void SetBlinkMode(bool blink)
        {
            string szCmd = string.Format("CMD:SetBlinkMode({0})", blink.ToString());
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }

        public void SelectEditMode(EditModeType type, string strParameter = "")
        {
            string strParam = "";

            if (type == EditModeType.AddIcon)
            {
                strParam = "AddIcon_" + strParameter;
            }
            else if (type == EditModeType.MoveIcon)
            {
                strParam = "MoveIcon_" + strParameter;
            }
            else if (type == EditModeType.DeleteIcon)
            {
                strParam = "DeleteIcon_" + strParameter;
            }
            else if (type == EditModeType.PickIcon)
            {
                strParam = "PickIcon_" + strParameter;
            }
            else
                return;

            string szCmd = string.Format("CMD:SelectEditMode('{0}')", strParam);
            if (m_PipeServer != null)
                m_PipeServer.Send(szCmd);
        }
        ...
        public void ShowIconPOIFile(string strPOIType, string strFilePath, List<int> poiIDs, List<string> poiTypes, List<bool> poiVisibles)
        {
            if (m_PipeServer == null)
                return;
            ...
            if (m_dicPOIFileDatas.TryGetValue(strPOIType, out data))
            {
                data.ShowFilePath = strFilePath;
                data.POIIDs = poiIDs;
                data.POITypes = poiTypes;
                data.POIVisibles = poiVisibles;

                if (data.POIIDFilePath.Length > 0)
                {
                    ReadPOIIDFile(data.POIIDFilePath, data);
                    ShowIconPOIFile(strPOIType, data);
                }
            }
        }

        private void ShowIconPOIFile(string strPOIType, POIFileData data)
        {
            m_dicPOIFileDatas.Remove(strPOIType);

            int nPOICount = data.POIIDs.Count;

            if (nPOICount == 0)
                return;

            StreamWriter writer = new StreamWriter(data.ShowFilePath, false, Encoding.UTF8);

            for (int i = 0; i < nPOICount; i++)
            {
                int nID = data.POIIDs[i];
                string strType = data.POITypes[i];
                bool visible = data.POIVisibles[i];
                POI poi = data.IndexPOI[i];

                m_dicPOIs[strType + "_" + nID.ToString()] = poi;

                string strCmd = string.Format("{0},{1},{2}", nID, visible, strType);
                writer.WriteLine(strCmd);
            }
            ...
            if (m_PipeServer != null)
            {
                string strCmd = string.Format("CMD:ShowIconPOIFile('{0}')", data.ShowFilePath);
                m_PipeServer.Send(strCmd);
            }
        }
        ...
        public void OnAddPOI(string szID, float x, float y, float z)
        {
            POI poi = CreatePOI(szID, x, y, z);
            ...
            if (m_Owner != null)
                m_Owner.OnAddPOI(poi);
        }

        private POI CreatePOI(string szID, float x, float y, float z)
        {
            string[] tokens = szID.Split('_');
        ...
        private POI CreatePOI(string strType)
        {
            POI poi = new POI();

            if (string.Compare(strType, "CCTV", true) == 0)
                poi.Facility = new CCTV();
            else if (string.Compare(strType, "Fire", true) == 0)
                poi.Facility = new FireSensor();
            else
                poi.Facility = new EtcSensor();
            ...
            IFacility facility = poi.Facility;
            poi.Popup = poi.Facility.CreatePopup(this, m_Factory);
            poi.Facility = facility;
            poi.Facility.IconPath = strType;
        ...
        public void AddPOIFile(string strPOIType, string strFilePath, List<POI> pois)
        {
            if (m_PipeServer == null)
                return;
            ...
            POIFileData data = new POIFileData(pois, strFilePath);
            m_dicPOIFileDatas[strPOIType] = data;
            ...
            foreach (POI poi in pois)
            {
                ...
                if (!m_isIndoor || (m_isIndoor && poi.Zone == m_currentIndoorZone))
                {
                    if (poi.Facility.IconPath == null || poi.Facility.IconPath.Length == 0)
                        poi.Facility.IconPath = strPOIType;
                    //string strIconPath = strPOIType;
                    string strIconPath = poi.Facility.IconPath;
                    int nID = AddIconPOIFile(writer, data.POIIconPath, poi, strIconPath, poi.X, poi.Y, poi.Z);
                    poi.ID = nID;
                    /*string szKey = string.Format("{0}_{1}", strIconPath, nID);
            ...
            string szCmd = string.Format("CMD:AddIconPOIFile('{0}', '{1}')", strPOIType, strFilePath);
            m_PipeServer.Send(szCmd);
            ...
            if (lastPOI != null)
            {
                System.Diagnostics.Trace.WriteLine("Finish AddIconPOIFile : " + lastPOI.Type.ToString());
            }
        }
        ...
        public void ClearPOI(string strPOIType)
        {
            ...
            string szCmd = string.Format("CMD:ClearIconPOI('{0}')", strPOIType);
            m_PipeServer.Send(szCmd);
        }
        ...
        public void ChangePOIIcon(POI poi, string strPOIType)
        {
            ...
            string szCmd = string.Format("CMD:ChangePOIIcon('{0}_{1}','{2}')", poi.Facility.IconPath, poi.ID, strPOIType);
            m_PipeServer.Send(szCmd);
        }

        // poi의 아이콘을 strPOIType으로 바꾼다.
        public void ChangePOIIcons(List<POI> pois, List<string> poiTypes)
        {
            if (m_PipeServer == null)
                return;

            string strFilePath = System.IO.Path.GetDirectoryName(System.Windows.Forms.Application.ExecutablePath) + "\\ChangePOIIcons.txt";
            strFilePath = strFilePath.Replace('\\', '/');
            ...
            {
                POI poi = pois[i];
                string strPOIType = poiTypes[i];

                string szCmd = string.Format("{0}_{1},{2}", poi.Facility.IconPath, poi.ID, strPOIType);
                writer.WriteLine(szCmd);
            }

            writer.Close();
            m_PipeServer.Send(string.Format("CMD:ChangePOIIconFile('{0}')", strFilePath));
        }
        ...
        public void RollBackPOIIcon(string strPOIType)
        {
            ...
            string szCmd = string.Format("CMD:RollBackPOIIcon('{0}')", strPOIType);
            m_PipeServer.Send(szCmd);
        }

        public void ReadPOIIDFile(string strPOIType, string strIDFile)
        {
            POIFileData data;

            if (m_dicPOIFileDatas.TryGetValue(strPOIType, out data) == false)
                return;

            data.POIIDFilePath = strIDFile;

            if (data.ShowFilePath.Length > 0 && data.POIIDs != null && data.POITypes != null && data.POIVisibles != null)
            {
                ReadPOIIDFile(strIDFile, data);
                ShowIconPOIFile(strPOIType, data);
            }
        }

        private void ReadPOIIDFile(string strIDFile, POIFileData data)
        {
            int nID;
            POI poi;
            int nPOICount = data.POIIDs.Count;

            StreamReader reader = new StreamReader(strIDFile, Encoding.UTF8);
            ...
            while (reader.EndOfStream == false)
            {
                ...
                string strID = strLine.Substring(nIndex + 1).Trim();
                string strIconPath = strLine.Substring(0, nIndex).Trim();

                if (int.TryParse(strID, out nID))
                {
                    if (data.POIIconPath.TryGetValue(strIconPath, out poi))
                    {
                        poi.ID = nID;

                        string szKey = string.Format("{0}_{1}", strIconPath, nID);
                        m_dicPOIs[szKey] = poi;
                        System.Diagnostics.Trace.WriteLine("ReadPOIIDFile : " + strIconPath + ", " + nID);

                        if (data.POIIndex.TryGetValue(poi, out nIndex) && nIndex < nPOICount)
                        {
                            data.POIIDs[nIndex] = nID;
                        }
                    }
                }
            }
        ...
        {
            if (m_Owner != null)
                m_Owner.OnMessage(strMessageType, strMessage);
        }

        public void OnRemoveIcon(string strIconType, int nID)
        {
            if (m_Owner != null)
            {
                string strKey = strIconType + "_" + nID.ToString();
                POI poi;

                if (m_dicPOIs.TryGetValue(strKey, out poi))
                {
                    m_dicPOIs.Remove(strKey);
                    m_Owner.OnDeletePOI(poi);
                }
            }
        }
        ...
        {
            string strCmd = string.Format("CMD:SetSceneTitle('{0}')", strTitle);
            if (m_PipeServer != null)
                m_PipeServer.Send(strCmd);
        }

        public void SetSceneTitleColor(Color color)
        {
            string strCmd = string.Format("CMD:SetSceneTitleColor({0},{1},{2},{3})", color.R / 255.0f, color.G / 255.0f, color.B / 255.0f, color.A / 255.0f);
            if (m_PipeServer != null)
                m_PipeServer.Send(strCmd);
        }

        public void MoveSceneTitle(float x, float y)
        {
            string strCmd = string.Format("CMD:MoveSceneTitle({0},{1})", x, y);
            if (m_PipeServer != null)
                m_PipeServer.Send(strCmd);
        }

        public void ShowIconLayer(string strIconTypeName)
        {
            string strCmd = string.Format("CMD:ShowIconLayer('{0}', True)", strIconTypeName);
            if (m_PipeServer != null)
                m_PipeServer.Send(strCmd);
        }
        ...
        {
            string strCmd = string.Format("CMD:VisibleViewButton('{0}',{1})", btnName, visible);
            if (m_PipeServer != null)
                m_PipeServer.Send(strCmd);
        }

        public void ShowIconLayers(List<string> iconTypeNames, bool hideOthers)
        {
            if (m_PipeServer == null)
                return;
            ...
            foreach (string strIconTypeName in iconTypeNames)
            {
                if (strParams.Length == 0)
                    strParams = strIconTypeName;
                else
                    strParams += "_" + strIconTypeName;
            }

            string strOthers = hideOthers ? "True" : "False";
            string strCmd = string.Format("CMD:ShowIconLayers('{0}', {1})", strParams, strOthers);
            m_PipeServer.Send(strCmd);
        }

        public void ShowAllIconLayers()
        {
            if (m_PipeServer == null)
                return;

            string strCmd = string.Format("CMD:HideIconLayers('', True)");
            m_PipeServer.Send(strCmd);
        }

        public void HideIconLayer(string strIconTypeName)
        {
            string strCmd = string.Format("CMD:ShowIconLayer('{0}', False)", strIconTypeName);
            if (m_PipeServer != null)
                m_PipeServer.Send(strCmd);
        }

        public void HideIconLayers(List<string> iconTypeNames, bool showOthers)
        {
            if (m_PipeServer == null)
                return;
            ...
            string strOthers = showOthers ? "True" : "False";
            string strCmd = string.Format("CMD:HideIconLayers('{0}', {1})", strParams, strOthers);
            m_PipeServer.Send(strCmd);
        }

        public void HideAllIconLayers()
        {
            if (m_PipeServer == null)
                return;

            string strCmd = string.Format("CMD:ShowIconLayers('', True)");
            m_PipeServer.Send(strCmd);
        }
        ...
        public void SetPoiLod(string strPOIType, bool useLOD)
        {
            ...
            string strUse = useLOD ? "True" : "False";
            string strCmd = string.Format("CMD:SetPoiLod('{0}', {1})", strPOIType, strUse);
            m_PipeServer.Send(strCmd);
        }
        ...
        public void AddPoiLodValue(float fMinZoomValue, float fMaxZoomValue, float fDistance)
        {
            ...
            string strCmd = string.Format("CMD:AddPoiLodValue({0}, {1}, {2})", fMinZoomValue, fMaxZoomValue, fDistance);
            m_PipeServer.Send(strCmd);
        }
        ...
        public void ClearPoiLodValue()
        {
            ...
            string strCmd = "CMD:ClearPoiLodValue()";
            m_PipeServer.Send(strCmd);
        }
    }
