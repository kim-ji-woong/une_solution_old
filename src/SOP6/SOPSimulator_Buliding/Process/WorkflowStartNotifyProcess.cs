		...
		class WorkFlowStartNotifyProcess : ProcessSectionIF
		{
			...
			public override void Progress()
			{
				...
				if (option.HasPosition == true)
				{
					...
					if (m_bNoPopup == true)
					{
                        VariousData<DateTime> detectTime = option.DetectTime == null ? null : new VariousData<DateTime>(option.DetectTime.Data);
						popup.btnRunClick(null, null);

                        if (detectTime != null)
                            option.DetectTime = detectTime;
					...
					else
					{
                        ...
                        FormSOP.Instance.Invoke((MethodInvoker)delegate
                        {
                            ...
                            System.Drawing.Point pt = FormSOP.Instance.Location;
                            x = pt.X;
                            y = pt.Y;
                        });
                        ...
                        { 
                            if (tabPage != null && tabPage.LinkedZoneName != null && tabPage.LinkedZoneName.Length > 0 && tabPage.LinkedZoneID >= 0)
                            {
                                ...
                                option.DetectTime = new VariousData<DateTime>(tabPage.LinkedTime);
				...
				else
				{
                    if (!m_bNoPopup)
                    {
                        if (popup2 != null)
                        {
                            ...
                            popup2.Location = new System.Drawing.Point(FormSOP.Instance.Location.X + ptCurrent.X, FormSOP.Instance.Location.Y + ptCurrent.Y);
            ...
            public static UnE.SOP.Workstate.WorkflowOption MakeWorkflowOption(string strCategoryName, string strSubCategoryName)
            {
                ...
                else if (strCategoryName == "강풍" || strSubCategoryName == "강풍")
                    option = new UnE.SOP.Workstate.WorkflowOptionWind();
                else
