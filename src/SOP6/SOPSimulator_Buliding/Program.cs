   ...
   static class Program
	{
		...
		static void Main(string[] args)
		{
			...
			if (args.Count() < 2)
			{
                string strArguments = "1 1 0 0 1";

                Application.Run(new Popup.Login.FormLogin(strArguments));
			...
			else
			{
                ...
                if (args.Length >= 6)
                {
                    ...
                    if (int.TryParse(args[5], out nLoginMode))
                        useLogin = (nLoginMode == 1);
                }
                ...
                if (args.Length >= 7)
                {
                    ...
                    if (int.TryParse(args[6], out nFromLogin))
                        fromLogin = (nFromLogin == 1);
                }
				...
				if (nSOPGenUserID >= 0)
				{
					...
					else*/
					{
                        ...
                        */
                        ...
                        if (useLogin)
                        {
                            string strArguments = string.Format("{0} {1} {2} 0 1", nMonitor, args[3], args[4]);
                            ...
                            Application.Run(new SOPMonitoringSystem.FormSOP(nSOPGenUserID, strSOPGenUserRealName, isSimulationMode, onlySDMS, nMonitor, fromLogin));
					}
				}
			}			
		}
