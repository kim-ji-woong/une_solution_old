...
namespace SOPMonitoringSystem
{
    ...
    public partial class FormMissionStatus : Form
    {
        ...
        private void SetSectionContents(DataGridView grid, ArrayList arrData, bool isLeftGrid, Sections.Section section)
        {
            ...
            if (SectionTabPage.SectionContentsFactory != null)
                contents = SectionTabPage.SectionContentsFactory.CreateSectionContents(section, null);
        ...
        public void SetContents(ISectionContents contents, ItemType type)
        {
            if (type == ItemType.PREV_ITEM)
        ...
        private void SetInternalContents(DataGridView grid, Sections.Section section, int nRowCount, bool isLeftGrid, ArrayList arrData)
        {
            ...
            if (isLeftGrid == false)
            {
                ...
                Sections.SectionCommander commander = SectionCommanderInfo.GetCommanderInfo(section, out strCommanderName, out strCommanderName2, out strCommanderPhoneNumber);
                ...
                DBUtility2.VariousData<DateTime> completeTime, executeTime, unCompleteTime;
                int nBroadcastCount;
                bool useSiren;
                string strMessage;
                ISectionContents contents = grid.Tag as ISectionContents;
        ...
        private void SetProcessContents(DataGridView grid, Sections.SectionProcess section, int nRowCount, bool isLeftGrid, ArrayList arrData)
        {
            ...
            int nItemCount = data.MissionItems.Count;
            DBUtility2.VariousData<DateTime> completeTime, executeTime, unCompleteTime;

            for (int i = 0; i < nItemCount; i++)
            {
                if (contents.GetItem(i, out isSendSMS, out isComplete, out strSender, out strItem, out strReceivers, out strPerformer, out executeTime, out completeTime, out unCompleteTime))
                {
                    if (i >= nRowCount)
                    {
                        AddNewLine(0, grid, isLeftGrid);
        ...
        private void SetContents(DataGridView grid, ArrayList arrData, bool isLeftGrid, ISectionContents contents)
        {
            ClearGrid(grid, arrData, isLeftGrid);
            ...
            switch (section.GetComponentType())
            {
                case Sections.Section.ComponentType.PROCESS:
                    SetProcessContents(grid, (Sections.SectionProcess)section, nRowCount, isLeftGrid, arrData);
                    break;
        ...
        private ISectionContents m_currContents = null;

        public void SelectRows(ArrayList arrRowIndices, ISectionContents contents)
        {
