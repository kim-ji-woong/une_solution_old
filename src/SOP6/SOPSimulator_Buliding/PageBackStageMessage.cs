...
namespace SOPMonitoringSystem
{
    public partial class PageBackStageMessage : Form, UnE.GUI.IRibbonButtonOwner
    {
        ...
        private void UpdateDB(DisasterType type, ArrayList arrNewSubDisasters)
        {
            ...
            UnE.SOP.RollbackData data = new UnE.SOP.RollbackData();
            data.AddInsertRollback(dbMgr, "Select id, DisasterType, DisasterSubType, Description from InternalTransmissionMessageType where DisasterType = " + ((int)type).ToString(), 0, 0, 1, 1);
            ...
            for (int i = 0; i < nDataCount; i++)
            {
                ...
                if (strRollbackQuery.Length == 0)
                    strRollbackQuery = "Delete from InternalTransmissionMessageType where ID in (" + nID.ToString();
                else
                    strRollbackQuery += ", " + nID.ToString();
            }
            ...
            if (nDataCount > 0)
            {
                ...
                {
                    strRollbackQuery += ")";
                    rollback.AddData(new UnE.SOP.RollbackData(strRollbackQuery));
                }

                foreach (string sql in sqls)
                {
                    if (dbMgr.GetResultData(sql) == null)
                    {
                        rollback.Rollback(dbMgr);
                        return;
                    }
