...
namespace SOPMonitoringSystem
{
	...
	public class IOManager
	{
		...
		public bool Load(FormSOP frm, WebDBManager dbMgr, VersionInfo version, ArrayList arrActionSteps, string strCategoryName, string strSubCategoryName, string strDisasterName, List<TabPage> tabPages)
		{
            ...
            foreach (ActionStepInfo actionStep in arrActionSteps)
            {
                if (actionStep != null)
                {
                    FormSOP.Instance.GetPageHome().CurrentDisasterID = actionStep.DisasterID;
                    break;
                }
            }
            ...
            bool addTabPage = !FormSOP.Instance.GetPageHome().BackgroundLoading;

            if (!LoadPane(dbMgr, pageHome, arrActionSteps, arrTeams, tabPages, addTabPage))
			{
				frm.Cursor = Cursors.Arrow;
				return false;
			}
        ...
        public void LoadSOPThread(FormSOP frm, WebDBManager dbMgr, int nDisasterID)
        {
            bool isNormal, isRegular;
            DisasterInfo disaster = frm.SOPManager.GetDisaster(nDisasterID, out isNormal, out isRegular);
            ...
            VersionInfo version = frm.SOPManager.GetActionStepVersionInfo(actionStep.ActionStepID);
            ...
            bool isReal = frm.IsReal;
            LoadPaneThread(dbMgr, pageHome, arrActionSteps, arrTeams, isReal);
        }
        
        private void LoadPaneThread(WebDBManager dbMgr, PageBackstageSOP pageHome, ArrayList arrActionSteps, ArrayList arrTeams, bool isReal)
        {
            ArrayList arrParams = new ArrayList();

            arrParams.Add(dbMgr);
            arrParams.Add(pageHome);
            arrParams.Add(arrActionSteps);
            arrParams.Add(arrTeams);
            arrParams.Add(isReal);

            FormSOP.Instance.Cursor = Cursors.WaitCursor;

            Thread t = new Thread(new ParameterizedThreadStart(LoadPaneThread));
            t.Start(arrParams);
        }

        private void LoadPaneThread(object arg)
        {
            ArrayList arrParams = (ArrayList)arg;

            WebDBManager dbMgr = (WebDBManager)arrParams[0];
            PageBackstageSOP pageHome = (PageBackstageSOP)arrParams[1];
            ArrayList arrActionSteps = (ArrayList)arrParams[2];
            ArrayList arrTeams = (ArrayList)arrParams[3];
            bool isReal = (bool)arrParams[4];

            List<TabPage> tabPages = new List<TabPage>();

            if (LoadPaneBackground(dbMgr, pageHome, arrActionSteps, arrTeams, tabPages, isReal))
            {
                if (tabPages != null)
                {
                    //pageHome.BackgroundLoadingTabPages = tabPages;
                }
            }

            FormSOP.Instance.Invoke((MethodInvoker)delegate
            {
                FormSOP.Instance.Cursor = Cursors.Arrow;
            });
        }
        ...
        private bool LoadProcessMission(WebDBManager dbMgr, string strSectionIDs, Dictionary<int, ArrayList> dicSectionMissionItems)
        {
            ...
            for (int i = 0; i < nResultCount - 7; i += 8)
            {
                ...
                Sections.MissionItem item = SectionContents.Utility.MissionItemExternal.IsExternalMissionText(strMissionText) ? new SectionContents.Utility.MissionItemExternal(strMissionText) : new Sections.MissionItem();
                //Sections.MissionItem item = strLower.StartsWith("#exec") ? new MissionItemExternal(strMissionText) : new Sections.MissionItem();

                if (prevItem != null && prevItem is SectionContents.Utility.MissionItemExternal)
                {
                    string strDescription;

                    if (SectionContents.Utility.MissionItemExternal.IsExternalMissionDescriptionText(strMissionText, out strDescription))
                    {
                        SectionContents.Utility.MissionItemExternal _item = (SectionContents.Utility.MissionItemExternal)prevItem;
        ...
        private static Sections.SectionCommander LoadDutyMemberListCommander(WebDBManager dbMgr, string strDisplayText)
        {
            ...
            for (int i=0;i<nResultCount-2;i+=3)
            {
                VariousData<int> jobPosition = WebDBManager.GetIntField(arrResult[i + 2].ToString());
            ...
            VariousData<int> memberType = WebDBManager.GetIntField(arrResult[nIndex].ToString());
            VariousData<int> memberID = WebDBManager.GetIntField(arrResult[nIndex + 1].ToString());
		...
		{
            string strSQL = "select id, x, y, width, height, text, ComponentID, usePopupMessage, useMobileApp, useBroadcast, useSiren, BroadcastMessage, valign, halign, TeamList, onlyTeamLeader, CommanderMemberType, CommanderMemberID, CommanderDisplayText, AutoRun from InternalTransmission where StepMemberID = " + data.StepMemberID.ToString();
			ArrayList arrResult = dbMgr.GetResultData(strSQL);
            ...
            for (int i = 0; i < nResultCount - 19; i += 20)
            {
                int nID = WebDBManager.GetIntField(arrResult[i].ToString(), 0);
                float x = WebDBManager.GetFloatField(arrResult[i + 1].ToString(), 0.0f);
                float y = WebDBManager.GetFloatField(arrResult[i + 2].ToString(), 0.0f);
                ...
                bool useSiren = WebDBManager.GetIntField(arrResult[i + 10].ToString(), 0) == 0 ? false : true;
                string szMessage = WebDBManager.GetStringField(arrResult[i + 11], "");
                ...
                string szTeamList = WebDBManager.GetStringField(arrResult[i + 14], "");

                bool bOnlyTeamLeader = WebDBManager.GetIntField(arrResult[i + 15].ToString(), 0) == 0 ? false : true;

                int nCommanderMemberType = WebDBManager.GetIntField(arrResult[i + 16].ToString(), -2);
                int nCommanderMemberID = WebDBManager.GetIntField(arrResult[i + 17].ToString(), -2);
                string szCommanderDisplayText = WebDBManager.GetStringField(arrResult[i + 18], "");
                bool autoRun = WebDBManager.GetIntField(arrResult[i + 19].ToString(), 0) == 0 ? false : true;
                ...
                if (useBroadcast)
                {
                    if (strText.Contains(strBroadcast) == false)
                        strText = String.Format("{0}{1}", strBroadcast, strText);
                }
                else if (useMobileApp)
                {
                    if (strText.Contains(strSMS) == false)
		...
		private ArrayList LoadActionSteps(WebDBManager dbMgr, ArrayList arrActionSteps)
		{
            ...
            string strFormat = "Select step.ID, StepName, PeriodType, BeginTime, EndTime, WeekDayOption, Iteration, IterationType, ProcessTime, ProcessTimeType, ParentStepID, UserDefinedConfigID, v.isNormal, step.DisasterID ";
            strFormat += "from ActionStep as step, Disaster as d, Version as v ";
            strFormat += "where step.DisasterID = d.ID and d.VersionID = v.ID and step.ID in ({0})";
            string strSQL = string.Format(strFormat, strIDs);
			ArrayList arrResult = dbMgr.GetResultData(strSQL);
			...
			for (int i = 0; i < nResultCount - 13; i += 14)
			{
                ...
                VariousData<int> isNormal = WebDBManager.GetIntField(arrResult[i + 12].ToString());
                int nDisasterID = WebDBManager.GetIntField(arrResult[i + 13].ToString(), -1);
		...
		public bool LoadPane(WebDBManager dbMgr, PageBackstageSOP pageHome, ArrayList arrActionSteps, ArrayList arrTeams, List<TabPage> tabPages, bool addTabPage)
		{
            ...
            int nFirstActionStepID = arrActionSteps.Count > 0 ? ((ActionStepInfo)arrActionSteps[0]).ActionStepID : -1;
			...
			{
				TabPage page = pageHome.AddTabPage(data, addTabPage);
				dicActionStep[data.ID] = page;
                tabPages.Add(page);

                if (nFirstActionStepID == data.ID)
                    firstTabPage = page;
			}
			...
			foreach (ActionStepInfo actionStep in arrActionSteps)
			{
                ...
                SectionTabPage tabPage = pageHome.GetTabPage(actionStep.ActionStepID, true);

                if (tabPage == null)
                    tabPage = pageHome.GetTabPage(actionStep.ActionStepID, false);
            ...
            /*if (firstTabPage != null)
                pageHome.SelectTab(firstTabPage);*/

            return true;
		}

        private bool LoadPaneBackground(WebDBManager dbMgr, PageBackstageSOP pageHome, ArrayList arrActionSteps, ArrayList arrTeams, List<TabPage> tabPages, bool isReal)
        {
            Dictionary<int, ArrayList> dicStepMembers = LoadStepMemberData(dbMgr, arrActionSteps, arrTeams);
            if (dicStepMembers == null)
                return false;
            ...
            foreach (Data_ActionStep data in arrStepDatas)
            {
                TabPage page = pageHome.MakeTabPage(data, -1, isReal);
                dicActionStep[data.ID] = page;
                tabPages.Add(page);
            ...
            foreach (ActionStepInfo actionStep in arrActionSteps)
            {
                ...
                if (dicActionStep.TryGetValue(actionStep.ActionStepID, out tabPage) == false)
                    continue;
                ...
                if (page.CreateNew == true)
                {
                    ArrayList arrPanels = pageHome.AddPane(arrTeams, actionStep.ActionStepID, tabPage, false);
                    ...
                    pageHome.CreateComponentContentsSet(page, true);
                    page.CreateNew = false;
		...
		public ArrayList LoadBarPage(PageBackstageSOP pageHome, ArrayList arrActionSteps, WebDBManager dbMgr)
		{
			if (arrActionSteps == null || arrActionSteps.Count == 0)
				return null;
