    ...
    public partial class BarLevelTree : Form, ISOPTreeContainer
    {
        ...
        private void treeView_AfterSelect(object sender, TreeViewEventArgs e)
        {
            ...
            if (node == nodeDisaster)   // Disaster 노드를 선택한 경우
            {
                ...
                if (LoadSOP(nodeDisaster, -1) == false)
                    return;
            }
            else                        // 대응 단계 노드를 선택한 경우
            {
                ...
                if ((int)nodeDisaster.Tag == treeView.PrevSelectedDisasterID)
                {
                    if (FormSOP.Instance.GetPageHome().IsChangeCurrentTab())
                    {
                        if (LoadSOP(nodeDisaster, nActionStepID) == false)
                            return;
                    }
        ...
        public void ChangeTab(int nActionStepID)
        {
            ...
            SectionTabPage page = pageHome.GetTabPage(nActionStepID, true);

            if (page == null)
                page = pageHome.GetTabPage(nActionStepID, false);
        ...
        public bool LoadSOP(TreeNode nodeDisaster, int nActionStepID)
        {
            ...
            // 이미 로딩되어 있으면 다시 열지 않는다.
            Data.SOPScenarioTab scenarioTab = FormSOP.Instance.GetPageHome().ScenarioTab;

            if (scenarioTab.GetTabPage(nActionStepID, true) != null)
                return true;

            if (scenarioTab.GetTabPage(nActionStepID, false) != null)
                return true;
        ...
        public bool LoadSOP(string strCategoryName, string strSubCategoryName, string strDisasterName, int nDisasterID, DisasterInfo disaster = null, VersionInfo version = null, int nActionStepID = -1)
        {
            ...
            if (mgr.Load(frm, frm.DBManager, version, disaster.ActionSteps, strCategoryName, strSubCategoryName, strDisasterName, tabPages))
            {
                pageHome.EnableTabPages(tabPages);
                FormSOP.Instance.SelectViewTab();
                ...
                if (tabPage != null)
                {
                    ...
                    {
                        FormSOP.Instance.GetPageHome().SelectTab(tabPage);
                        FormSOP.Instance.GetPageHome().RefreshSelectedTabPage();
                    }
                    else
                        FormSOP.Instance.GetPageHome().RefreshCurrentTab();
                }
                else
                {
                    ActionStepInfo actionStep = GetActionStep(nActionStepID, disaster.ActionSteps);
                    string strSOPPath = strCategoryName + "/" + strSubCategoryName + "/" + strDisasterName;

                    if (actionStep != null)
                        strSOPPath += "/" + actionStep.ActionStepName;

                    MessageBox.Show("SOP를 찾을수 없습니다.\r\n" + strSOPPath);

                    foreach (SectionTabPage addPage in tabPages)
                    {
                        PanelSectionEx.CloseTabPage(addPage, addPage.ActionStepID);
                        //pageHome.RemoveTabPage(addPage);

                        FormSOP.Instance.SOPManager.RemoveActionStepVersionInfo(addPage.ActionStepID);
                        FormSOP.Instance.SOPManager.RemoveActionStepInfo(addPage.ActionStepID);
                    }
        ...
        private ActionStepInfo GetActionStep(int nActionStepID, ArrayList actionSteps)
        {
            foreach (ActionStepInfo actionStep in actionSteps)
            {
        ...
        public new bool Load(UnE.SOP.SOPManager sopMgr, bool isRegular, bool isNormal)
        {
            ...
            {
                bResult = treeView.Load(sopMgr, isRegular, isNormal);
                treeView.ExpandAll();
            });
            return bResult;
        }
