	...
	public partial class DockingBottomSOPLog : Form, ISOPLogContainer
	{
		...
		private string SearchDisa(string actionStepID)
		{
			try
			{
				...
				ArrayList arrResults = dbMgr.GetResultData(strSQL);

                if (arrResults == null || arrResults.Count < 2)
                    return null;

                string strActionStepName = WebDBManager.GetStringField(arrResults[1]);
                VariousData<int> disasterID = WebDBManager.GetIntField(arrResults[0].ToString());

                if (strActionStepName == null || disasterID == null)
                    return null;

				strSQL = "select ID, DisasterName, SubDisasterID from Disaster where ID = " + disasterID.Data.ToString();
				arrResults = dbMgr.GetResultData(strSQL);

                if (arrResults == null || arrResults.Count < 3)
                    return null;

                string strDisasterName = WebDBManager.GetStringField(arrResults[1]);
                VariousData<int> subDisasterCategoryID = WebDBManager.GetIntField(arrResults[2].ToString());

                if (strDisasterName == null || subDisasterCategoryID == null)
                    return null;

                strSQL = "select ID, SubCategoryName, DisasterID from SubDisastercategory where ID = " + subDisasterCategoryID.Data.ToString();
				arrResults = dbMgr.GetResultData(strSQL);

                if (arrResults == null || arrResults.Count < 3)
                    return null;

                string strSubDisasterCategoryName = WebDBManager.GetStringField(arrResults[1]);
                VariousData<int> disasterCategoryID = WebDBManager.GetIntField(arrResults[2].ToString());

                if (strSubDisasterCategoryName == null || disasterCategoryID == null)
                    return null;

                strSQL = "select ID, CategoryName from DisasterCategory where ID = " + disasterCategoryID.Data.ToString();
				arrResults = dbMgr.GetResultData(strSQL);

                if (arrResults == null || arrResults.Count < 2)
                    return null;

                string strCategoryName = WebDBManager.GetStringField(arrResults[1]);

                if (strCategoryName == null)
                    return null;

                string result = strCategoryName + szDeli + strSubDisasterCategoryName + szDeli + strDisasterName + szDeli + strActionStepName;
				return result;
			}
			catch
			{
		...
		private bool SearchDB()
		{
			...
			m_arReadMessages = dbMgr.GetResultData(strSQL);
					...
					if (row.ActionStepID == nActionStepID && row.IsRealMode == isRealMode && row.ActionStepHistoryID == nActionStepHistoryID)
					{
						...
						if (section != null)
						{
							...
							if (updateComponentContents)
							{
								...
								FormSOP.Instance.GetPageHome().GetComponentContents(nActionStepHistoryID, row.ActionStepID, row.ComponentHistoryID, row.ComponentType, time, strComponentType, strTask, strStatus, section, row.SectionState, nCheckNotify1, nCheckNotify2);
							}
						}
					}
                ...
                foreach (DataLogGridViewRow row in arTemp)
                {
                    if (row.ActionStepID == nActionStepID && row.IsRealMode == isRealMode && row.ActionStepHistoryID == nActionStepHistoryID)
                    {
                        ...
                        if (section != null)
                        {
                            ...
                            string strStatus = row.Cells[6].Value.ToString();
                            FormSOP.Instance.GetPageHome().GetComponentContents(nActionStepHistoryID, row.ActionStepID, row.ComponentHistoryID, row.ComponentType, time, strComponentType, strTask, strStatus, section, row.SectionState, nCheckedNotify1, nCheckedNotify2);
                        }
                    }
                    else if (row.ActionStepID < 0)
                    {
		...
		{
			if (HistoryManager2.Instance.Exit)
				return;
            ...
            if (FormSOP.Instance.InitComponentHistoryID > 0 && nComponentHistoryID >= FormSOP.Instance.InitComponentHistoryID)
                FormSOP.Instance.InitComponentHistoryID = 0;

            WorkFlow work;
			SectionState state = WorkFlowManager.Instance.Find(section, isRealMode, out work);
			if (state != null)
			{
				...
				if (!FormSOP.Instance.CloseThread)
				{
                    ...
                    pageHome.Invoke((MethodInvoker)delegate
                    {
                        ...
                        {
                            work.SelectSectionState(state);
                        }

                        pageHome.GetComponentContents(nActionStepHistoryID, nActionStepID, nComponentHistoryID, section.GetComponentType(), time, strComponentType, strTask, strStatus, section, data.State, nCheckNotify1, nCheckNotify2);
                    });
			...
			if (m_isWorkingTimer == false)
			{
				...
				if (_DBAlive == true) // DB 감시
				{
					if (_DBSetting == false) // DB 초기 세팅
					{
						...
						if (arrResult.Count > 0)
						{
							...
							{
                                DBUtility2.VariousData<DateTime> time = WebDBManager.GetDateTimeField(arrResult[i]);
						...
						{							    
                            ...
                            strSQL = sb1.ToString();
                            arResult2 = dbMgr.GetResultData(strSQL);
						}
						catch(Exception ex)
						{
                        ...
                        if (arResult2.Count > 0)
						{
							...
							{
                                DBUtility2.VariousData<DateTime> time = WebDBManager.GetDateTimeField(arResult2[i]);
					...
					else if (SearchDB())
					{
						...
						if (m_arReadMessages.Count > 0)
						{
							...
							for (int i = 0; i < m_arReadMessages.Count - 4; i += 5)
							{
								try
								{
                                    DBUtility2.VariousData<DateTime> time = WebDBManager.GetDateTimeField(m_arReadMessages[i]);
				...
				//if (!FormSOP.Instance.HasControl)
				{
					...
					/*if (SOPScenarioManager.Instance.FinishLoadingComponentHistory)
					{
                        ...
                        List<int> actionStepHistoryIDs = FormSOP.Instance.NoControlSOPList();

                        if (actionStepHistoryIDs.Count > 0)
                            HistoryManager.Instance.DoMonitoring(actionStepHistoryIDs, FormSOP.Instance.GetPastActionStepHistoryIDs());
					}*/
				}
			}
		...
		{
            SOPScenarioManager.Instance.AddSOPSectionLog(nActionStepID, nActionStepHistoryID, arrComponentHistoryID, arrSections, arrStatus, arrProcessDirections, arrTask, arrTime, arrDescription, arrShowBoard, arrCheckedNotify1, arrCheckedNotify2, arrCheckedRun, arrCheckedComplete, arrAccessedUserID, isRealMode, dicDetailDatas, workFlow);
		}
