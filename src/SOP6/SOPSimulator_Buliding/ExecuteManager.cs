...
namespace SOPMonitoringSystem
{
    public class ExecuteManager
    {
        ...
        public enum APP_TYPE { CONTROLROOM_WORKER_EDITOR = 0, SMS_SENDER = 1, BROADCAST_TESTER  = 2, SOP_MANAGER = 3, TEAM_MANAGER = 4 };

        private FormSOP m_frmMain = null;

        public ExecuteManager()
        {
        }

        public ExecuteManager(FormSOP frmMain)
        {
            m_frmMain = frmMain;
        }
        ...
        public void Run(APP_TYPE type)
        {
            ...
            else if (type == APP_TYPE.SOP_MANAGER)
            {
                int nSiteID = FormSOP.Instance.SiteID;
                string strUserID = FormSOP.Instance.LoginID;
                int nUserID = FormSOP.Instance.UserID;

                System.Diagnostics.Process process = RunSOPManager(nUserID, strUserID);
                if (process != null)
                {
            ...
            {
                System.Diagnostics.Process p = RunTeamManager(FormSOP.Instance.UserID);
                if (p != null)
                {
        ...
        private System.Diagnostics.Process RunSendSMS()
        {
            ...
            if (process == null)
            {
                ...
                System.Drawing.Point location = FormSOP.Instance.Location;
                string strOpt = string.Format("{0} {1}", location.X + 200, location.Y + 200);
        ...
        private System.Diagnostics.Process RunSOPManager(int nUserID, string strUserID)
        {
            System.Diagnostics.Process process = RunCheckProcess("SOPManager");

            if (process == null)
            {
                string strValue = nUserID.ToString() + " " + strUserID;
                return RunStartProcess("SOPManager2.exe", strValue);
            }
            return process;
        }
        ...
        private System.Diagnostics.Process RunControlTeamEditor(string strTitle)
        {
            ...
            if (process == null)
            {
                string strValue = strTitle + " " + FormSOP.Instance.Location.X.ToString() + "," + FormSOP.Instance.Location.Y.ToString() + " " + UnE.SOP.ProxySOP.Instance.SiteID.ToString();
        ...
        private System.Diagnostics.Process RunTeamManager(int nUserID)
        {
            System.Diagnostics.Process p = RunCheckProcess("TeamEditor");
            if (p == null)
            {
