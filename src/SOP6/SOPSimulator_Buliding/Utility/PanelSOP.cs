...
namespace SOPMonitoringSystem
{
    public class PanelSOP : Panel, UnE.GUI.IRibbonButtonOwner
    {
        ...
        private bool m_useColorFilter = false;

        public bool UseColorFilter
        {
            get { return m_useColorFilter; }
            set
            {
                if (m_useColorFilter != value)
                {
                    m_useColorFilter = value;
                    Refresh();
                }
            }
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (m_useColorFilter)
                e.Graphics.FillRectangle(new SolidBrush(Color.FromArgb(100, Color.Gray)), this.ClientRectangle);
        ...
        private void LocateButtons()
        {
            ...
            else
            {
                ...
                int nHorz = 4, nVert = 2;
                int nHorzSpace = 100, nVertSpace = 100;
                int nBtnWidth = btn.Size.Width;
                int nBtnHeight = btn.Size.Height;

                int xBegin = (this.Size.Width - (nBtnWidth * nHorz + nHorzSpace * (nHorz - 1))) / 2;
                int yBegin = (this.Size.Height - (nBtnHeight * nVert + nVertSpace * (nVert - 1))) / 2;

                for (int i=0;i<nButtonCount;i++)
                {
                    RibbonButtonQuick button = (RibbonButtonQuick)m_arrButtons[i];
                    int x = xBegin + (i % nHorz) * (nHorzSpace + nBtnWidth);
                    int y = yBegin + (i / nHorz) * (nVertSpace + nBtnHeight);
        ...
        private void InitializeComponent()
        {
            ...
            this.tsMenuLoadSOP.Text = "SOP 불러오기";
