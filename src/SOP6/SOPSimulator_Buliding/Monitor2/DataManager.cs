...
namespace SOPMonitoringSystem
{
	public class DataManager
	{
        ...
        public ArrayList GetZoneList(string buildingCode)
        {
			...
			foreach (KeyValuePair<int, Zone> kv in m_dicZones)
			{
                ...
                // BuildingCode로 Zone 검색
                if (obj.Building != null && buildingCode == obj.Building.BuildingCode)
				{
					arList.Add(obj);
				}
			}
		...
		public Zone GetZone(int nZoneID)
		{
            ...
            else if (nZoneID > 0 && m_dicZones.ContainsKey(nZoneID) == false)
            {   // nZoneID 값이 -1이 아님에도 null을 리턴하는 경우도 있어 - K.D.R
                LoadZones();
        ...
        public EquipmentZone GetEquipmentZoneFromSensorZoneID(int nSensorZoneID)
        {
            SOPMonitoringSystem.WebDBManager webDB = SOPMonitoringSystem.FormSOP.Instance.DBManager;
            string strSQL = "Select EquipZoneID from SensorZone where ID = " + nSensorZoneID.ToString();
            ...
            return GetEquipZone(equipZoneID.Data);
        }
        ...
        public EquipmentZone GetEquipZone(int nEquipZoneID)
        {
            ...
            else if (nEquipZoneID > 0 && m_dicEquipZones.ContainsKey(nEquipZoneID) == false)
            {   // nEquipZoneID 값이 -1이 아님에도 null을 리턴하는 경우도 있어 - K.D.R
                LoadEquipZones();
        ...
        //    ArrayList arrResult = SOPMonitoringSystem.FormSOP.Instance.DBManager.GetResultData(strSQL);
        ...
        //        if (webDB.DatabaseType == WebDBManager.DBType.sqlserver)
        //        {
        ...
        //        }
        //        else if (webDB.DatabaseType == WebDBManager.DBType.mysql)
        //        {
        //            szText = "select Description, PosX, PosY, PosZ, FloorIndex, DisasterType, BuildingID , BroadcastName ";
        ...
        public bool GetUserDefinedTeamInfo(int nTeamID, out string strPhoneNumber, out string strTeamName)
        {
            ...
            ArrayList arrResult = UnE.SOP.ProxySOP.Instance.DBManager.GetResultData(strSQL);
        ...
        private void LoadTemporaryTeams(bool isNormal)
        {
            ...
            for (int i=0;i<nResultCount-3;i+=4)
            {
                ...
                string strRegularTeamIDs = WebDBManager.GetStringField(arrResult[i + 3]);
