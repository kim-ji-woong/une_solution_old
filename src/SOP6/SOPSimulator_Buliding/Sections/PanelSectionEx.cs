...
namespace Sections
{
    public partial class PanelSectionEx : PanelSection
    {
        ...
        /// 타이틀 바  
        /// </summary>
        private TitleLabel label = new TitleLabel();
        private int m_nTitleBarInitHeight = 106;
        private int m_nTitleBarPSMHeight = 106;
        // 좌우 양끝단으로부터의 간격
        private int m_nTitleBarSpace = 50;
        private int m_nTitleBarVPos = 10;
        ...
        private int m_nVTransForTitle = 90;

        private static Brush m_backgroundBrush = Brushes.White;

        public static Brush BackgroundBrush
        {
            get { return m_backgroundBrush; }
            set { m_backgroundBrush = value; }
        }
        ...
        private void PrintTitle()
        {
            ...
            labelTitle.Font = new System.Drawing.Font("나눔바른고딕", 9F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
            ...
            labelInfo1.Font = new System.Drawing.Font("나눔바른고딕", 9F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
            ...
            labelInfo2.Font = new System.Drawing.Font("나눔바른고딕", 9F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
            ...
            labelPSMMaterial.Font = new System.Drawing.Font("나눔바른고딕", 9F, System.Drawing.FontStyle.Regular, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
        ...
        public void SetWorkflowOption(WorkflowOption option)
        {
            if (option == null)
                return;

            label.Time = option.DetectTime.Data.ToString();

            if (option.HasPosition && option.PositionName.Length > 0)
            {
                labelInfo1.Text = "재난위치: " + option.PositionName;
                label.Position = option.PositionName;
            }

            if (option is WorkflowOptionEarthquake)
                SetWorkflowOptionEarthquake((WorkflowOptionEarthquake)option);
            else if (option is WorkflowOptionPSM)
                SetWorkflowOptionPSM((WorkflowOptionPSM)option);
            else if (option is WorkflowOptionWind)
                SetWorkflowOptionWind((WorkflowOptionWind)option);
        }

        private void SetWorkflowOptionEarthquake(WorkflowOptionEarthquake option)
        {
            string strOption = "";

            if (option.Intensity > 0)
            {
                strOption = "진도 : " + GetRomanString(option.Intensity);
                //strOption = "진도 : " + option.Intensity.ToString();
            }
            else if (option.Magnitude > 0)
                strOption = string.Format("규모 : {0:F1}", option.Magnitude);

            label.SetSubText("", strOption, "");
        }

        private string GetRomanString(int nIntensity)
        {
            if (nIntensity == 1)
                return "I";
        ...
        private void SetWorkflowOptionPSM(WorkflowOptionPSM option)
        {
            string strPosition = option.PositionName.Length > 0 ? string.Format("위치 : " + option.PositionName) : "";
            string strMaterial = option.PSMMaterial != null ? string.Format("물질 : " + option.PSMMaterial.MaterialName) : "";
            string strDistance = option.PSMDistance > 0 ? string.Format("대피거리 : {0}미터", option.PSMDistance) : "";

            string strTop = "", strMiddle = "", strBottom = "";

            if (strPosition.Length > 0)
            {
                if (strMaterial.Length > 0 && strDistance.Length > 0)
                {
                    strTop = strPosition;
                    strMiddle = strMaterial;
                    strBottom = strDistance;
                }
                else if (strMaterial.Length > 0)
                {
                    strTop = strPosition;
                    strBottom = strMaterial;
                }
                else if (strDistance.Length > 0)
                {
                    strTop = strPosition;
                    strBottom = strDistance;
                }
            ...
            {
                strMiddle = strDistance;
            }

            label.SetSubText(strTop, strMiddle, strBottom);
        }

        private void SetWorkflowOptionWind(WorkflowOptionWind option)
        {
            string strOption = "";

            if (option.WindSpeed > 0)
                strOption = string.Format("풍속 : {0:F1}m/s", option.WindSpeed);
        ...
        public void AddPanelTitle(string strTitle)
        {
            if (label.Text == "")
            {
                label.Location = new Point(m_nTitleBarSpace, m_nTitleBarVPos);
                label.Size = new Size(this.Size.Width - m_nTitleBarSpace * 2, m_nTitleBarInitHeight);
                ...
                label.Font = new System.Drawing.Font("나눔바른고딕", 22F, System.Drawing.FontStyle.Bold);//, System.Drawing.GraphicsUnit.Point, ((byte)(129)));
        ...
        protected override void OnPaint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.TranslateTransform(m_fTranX, m_fTranY + m_nVTransForTitle);
        ...
        protected override void OnPaintBackground(PaintEventArgs e)
        {
            ...
            if (this.BackgroundImage != null)
                e.Graphics.DrawImage(this.BackgroundImage, 0, m_nVTransForTitle);
        }
        ...
        private void toolStripMenuDecision_Click(object sender, System.EventArgs e)
        {
            ...
            if (section != null)
            {
                ...
                {
                    SectionData data = state.Section.Data;
        ...
        private void toolStripMenuDecisionExec_Click(object sender, System.EventArgs e)
        {
            ...
            if (section != null)
            {
                ...
                if (btn != null)
                {
                    ...
                    if (stateDecision != null)
                    {
                        ISectionContents contents = FormSOP.Instance.GetPageHome().GetComponentContents(m_sectionSelected);

                        if (contents != null)
                        {
                            // 재실행하는 것이므로 일단 초기화시킨후 실행시킨다.
                            SectionContentsHelper.RunDecision(contents, btn, true);
                        }
                    }
                }
            }
        ...
        protected override void OnMouseDown(object sender, MouseEventArgs e)
        {
            ...
            e = new MouseEventArgs(e.Button, e.Clicks, e.X, e.Y - m_nVTransForTitle, e.Delta);
            ...
            else if (e.Button == System.Windows.Forms.MouseButtons.Right)
            {
                ...
                else if (m_sectionSelected != null)
                {
                    ...
                    if (page != null && SOPMonitoringSystem.FormSOP.Instance.HasSOPControl(page.ActionStepHistoryID))
                    {
                    ...
                    PopupRClickMenu(e.X, e.Y + m_nVTransForTitle);
            }
        ...
        private void PopupRClickMenu(int x, int y)
        {
            ...
            if (workFlow == null || workFlow.State == WorkFlowState.DONE || workFlow.ActionStepHistoryID < 0)
            {
        ...
        protected bool ProcessButtonClick(MouseButtons mouseButton, float x, float y, out ProcessButton btn, out ConfirmButton btn2)
        {
            ...
            if (page == null || FormSOP.Instance.HasSOPControl(page.ActionStepHistoryID) == false)
                return false;
        ...
        public override bool CompleteSection(Section section, int processDirection = (int)ProcessDirectionHistory.NONE, bool refresh = true, Sections.Section sectionNext = null)
        {
            if (section != null)
            {
                ...
                if (state != null)
                {
                    if (state.Section.GetComponentType() == Section.ComponentType.LINK)
                    {
                        SectionData data = state.Section.Data;
                        data.AggSection = section;
                    }
        ...
        private new void SelectSection(Section section)
        {
        ...
        public static void CloseTabPage(SectionTabPage page, int nActionStepID)
        {
            ...
            if (!SOPScenarioManager.Instance.RemoveTabPage(page))
            {
                ...
                if (actionStep != null)
                {
                    if (pageHome.ScenarioTab.CurrentScenario != null)
                    {
                        if (pageHome.ScenarioTab.CurrentScenario.DisasterID == actionStep.DisasterID)
                        {
                        ...
                        {
                            pageHome.ScenarioTab.Select(pageHome.ScenarioTab.CurrentScenario);
                        }
                    }
                    else
                    {
            ...
            SOPScenarioManager.Instance.AfterRemoveTabPage(page);

            if (FormSOP.Instance.GetAllSenario().Count == 0)
            {
                FormSOP.Instance.EnableOptions(false);
                FormSOP.Instance.GetPageHome().EmptySOP();
            }
            else
            {
