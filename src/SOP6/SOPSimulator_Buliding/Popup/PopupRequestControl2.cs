    ...
    public partial class PopupRequestControl : Form, UnE.GUI.IImageButtonOwner
    {
        ...
        public void OnImageButtonMouseUp(object sender, MouseEventArgs e)
        {
            if (sender == btnOK)
            {
                ...
                {
                    FormSOP.Instance.NetworkManager.SendControl(GetUserIDs((string)trgRow.Tag), trgRow.Cells[0].Tag.ToString(), (int)trgRow.Cells[2].Tag);
            ...
            else if (sender == btnCancel)
            {
                ...
                List<string> userIDs = GetUserIDs(out nActionStepHistoryID);
                //foreach (DataGridViewRow row in gridRequest.Rows)
                {
                    FormSOP.Instance.NetworkManager.SendRejectRequestControl(userIDs, nActionStepHistoryID);
                }
        ...
        private List<string> GetUserIDs(out int nActionStepHistoryID)
        {
            ...
            foreach (DataGridViewRow row in gridRequest.Rows)
            {
                if (row.Tag != null && row.Tag is string)
                {
                    ...
                    {
                        nActionStepHistoryID = (int)row.Cells[2].Tag;
                    }
                }
            }
        ...
        public void AddUser(int nActionStepHistoryID, string strUserID, string strUserNickName, string strIP)
        {
            lock (this)
            {
                foreach (DataGridViewRow row in gridRequest.Rows)
        ...
        public int RemoveUser(int nActionStepHistoryID, string strUserID)
        {
            lock (this)
            {
                foreach (DataGridViewRow row in gridRequest.Rows)
                {
                    int actionStepHistoryID = (int)row.Cells[2].Tag;
                    string strID = (string)row.Tag;

                    if (strID == strUserID && actionStepHistoryID == nActionStepHistoryID)
                    {
        ...
        {
            this.Location = new Point(FormSOP.Instance.Location.X + this.Location.X, FormSOP.Instance.Location.Y + this.Location.Y);
        ...
        public int RemoveUser(int nActionStepHistoryID)
        {
            ...
            foreach (DataGridViewRow row in gridRequest.Rows)
            {
                ...
                if (actionStepHistoryID == nActionStepHistoryID)
                {
                    removeRows.Add(row);
                    break;
                }
