...
namespace SOPMonitoringSystem.Popup
{
    public partial class PopupSelectIntrusionSensorSOPLink : Form
    {
        ...
        private int m_curSopGridIndex
        {
            get
            {
                if (gridSOP.CurrentRow == null)
                    return -1;

                return Convert.ToInt32(gridSOP.CurrentRow.Cells["colNo"].Value);
            }
        }

        private int m_curSopID
        {
            get
            {
                if (gridSOP.CurrentRow == null)
                ...
                return Convert.ToInt32(gridSOP.CurrentRow.Cells["colSopID"].Value);
            }
        }

        private string m_curSopName
        {
            get
            {
                if (gridSOP.CurrentRow == null)
                ...
                return gridSOP.CurrentRow.Cells["colSOPFullPath"].Value.ToString();
            }
        }

        private int m_curSecurityID
        {
            get
            {
                if (gridSecurityFacilityType.CurrentRow == null)
                    return -1;

                return Convert.ToInt32(gridSecurityFacilityType.CurrentRow.Cells["colSecurityID"].Value);
            }
        }
        ...
        private void LoadLinkedSOP()
        {
            ...
            ArrayList arrSopList = m_dbMgr.GetResultData(strSQL.ToString());
            if (arrSopList == null) return;

            for (int i = 0; i < arrSopList.Count; i += 2)
            {
                VariousData<int> nID = WebDBManager.GetIntField(arrSopList[i].ToString());
            ...
            ArrayList arrSopLinkList = m_dbMgr.GetResultData(strSQL);
            if (arrSopLinkList == null) return;

            for (int i = 0; i < arrSopLinkList.Count; i += 6)
            {
                VariousData<int> nID = WebDBManager.GetIntField(arrSopLinkList[i].ToString());
                VariousData<int> nSopID = WebDBManager.GetIntField(arrSopLinkList[i + 1].ToString());
        ...
        private void DeleteSOP()
        {
            ...
            if (strIDList.Length != 0)
                m_dbMgr.GetResultData(string.Format("Delete from securitysensorsoplink where ID in ({0})", strIDList));

            if (strSopIDList.Length != 0)
                m_dbMgr.GetResultData(string.Format("Delete from securitysensorsoplist where ID in ({0})", strSopIDList));
        } 
        ...
        private bool UpdateSOP(SOPList sop)
        {
            foreach (KeyValuePair<int, LinkedSOP> item in dicSopList[sop.ID])
            {
                ...
                if (nCount.Data > 0) //Update
                {
                    ...
                    if (m_dbMgr.GetResultData(strSQL) == null) return false;
