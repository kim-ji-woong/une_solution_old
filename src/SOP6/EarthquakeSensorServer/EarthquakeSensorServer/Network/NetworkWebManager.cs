...
namespace EarthquakeSensorServer.Network
{
    public class NetworkWebManager : IPostMan
    {
        ...
        public void ClearEarthquakeSignal(int nSensorZoneID, int nSOPGenUserID, string strDescription)
        {
            ArrayList arrResult = m_dbMgr.GetResultData("Select Max(ID) From SensorZoneHistory Where SensorID = " + nSensorZoneID);
            if (arrResult == null || arrResult.Count == 0)
                return;

            int nSensorzoneHistoryID = DBUtility2.WebDBManager.GetIntField(arrResult[0].ToString(), -1);
            ...
            byte[] bytes = SOPWebServer.BinaryHelper.MakeBytes(arrDatas);
            SendMessage(SOPWebServer.Header.SENSOR_USER_RESET, bytes);
        }
