...
BOOL				InitInstance(HINSTANCE, int, std::wstring);
...
//std::map<VectorGraphics::Polygon*, Space*> g_mapSpace;
...
void MakeGeometry(XMLManager& xmlMgr)
{
	vectorCtrl->Clear();

	MakeLayers();

	std::map<int, POIType*>& mapPOITypes = xmlMgr.GetPOITypes();
	int nProjectCount = xmlMgr.GetProjectCount();

	if (nProjectCount > 0)
	{
		// Sample로 하나의 Project만 사용
		Project* project = xmlMgr.GetProject(0);
...
//
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow, std::wstring strPath)
{
	HRESULT hOle = CoInitialize(0);
	if (!SUCCEEDED(hOle))
   ...
   if (xmlMgr.LoadXML(strPath.c_str(), "../../../POI"))
   {
	   MakeGeometry(xmlMgr);
   }
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		...
		/*if (pShape != 0)
		{
			std::map<VectorGraphics::Polygon*, Space*>::iterator iter = g_mapSpace.find((VectorGraphics::Polygon*)pShape);

			if (iter != g_mapSpace.end())
			{
