...
namespace BIMViewer.BIM
{
    public class Level : IComparable
    {
        ...
        private List<Shapes.Layer> m_layers = new List<Shapes.Layer>();
        private List<DXFLayer> m_dxfLayers = new List<DXFLayer>();
        ...
        {
            if (string.Compare(m_strLevelID, "Roof", true) == 0)
                m_nFloorIndex = RoofFloorIndex;
            else
            {
                if (m_strLevelID.StartsWith("B"))
                {
                    int nIndex = m_strLevelID.IndexOf('F');

                    if (nIndex >= 0)
                    {
                        string strFloor = m_strLevelID.Substring(1, nIndex - 1);

                        if (int.TryParse(strFloor, out m_nFloorIndex))
                            m_nFloorIndex = -m_nFloorIndex;
                    }
                }
                else
                {
                    ...
                    if (nIndex >= 0)
                    {
                        string strFloor = m_strLevelID.Substring(1, nIndex);
                        int.TryParse(strFloor, out m_nFloorIndex);
                    }
                }
            }
        }*/

        public List<Shapes.Layer> SetLayers(List<Shapes.Layer> layers)
        {
            m_layers.Clear();

            foreach (Shapes.Layer layer in layers)
            {
                m_layers.Add(layer.Clone());
            }

            return m_layers;
        }

        public List<Shapes.Layer> GetLayers()
        {
            return m_layers;
        }
        ...
        public void MakeShapes(List<Shapes.Layer> layers, Project.UnitOfLength unit)
        {
            Layer spaceLayer = SelectLayer(layers, typeof(Space));
            Layer wallLayer = SelectLayer(layers, typeof(Wall));
            Layer doorLayer = SelectLayer(layers, typeof(Door));
            Layer windowLayer = SelectLayer(layers, typeof(Window));
            Layer poiLayer = SelectLayer(layers, typeof(POI));
            Layer wireLayer = SelectLayer(layers, typeof(Wire));
            Layer columnLayer = SelectLayer(layers, typeof(Column));
            Layer alertAreaLayer = SelectLayer(layers, typeof(AlertArea));

            foreach (KeyValuePair<int, Space> pair in m_dicSpaces)
            {
                pair.Value.MakeShape(spaceLayer, unit);
            }

            foreach (KeyValuePair<int, AlertArea> pair in m_dicAlertAreas)
            {
                pair.Value.MakeShape(alertAreaLayer, unit);
            }

            #region 기존 외곽벽체영역 계산
            // 공간에 속해있지 않은 외곽벽체들을 이용하여 Polygon을 생성한다.
            //Wall.MakeOutsideWallLine(m_dicWalls, unit);
            #endregion

            // 벽체영역 계산
            foreach (KeyValuePair<int, Wall> pair in m_dicWalls)
            {
                pair.Value.MakeShape(wallLayer, this);
            }

            // 문, 창문의 영역 계산
            foreach (KeyValuePair<int, Wall> pair in m_dicWalls)
            {
                pair.Value.MakeOpeningShapes(doorLayer, windowLayer);
            }

            foreach (Column column in m_columns)
            {
                column.MakeShape();
                columnLayer.AddShape(column);
            }
            ...
            if (poiLayer != null)
            {
                ...
                {
                    poiLayer.AddShape(pair.Value);
                }
            }

            if (wireLayer != null)
            {
                foreach (KeyValuePair<int, Wire> pair in m_dicWires)
                {
                    wireLayer.AddShape(pair.Value);
                    if (pair.Value.POIIcon != null)
                        wireLayer.AddShape(pair.Value.POIIcon);
                }
            }
        ...
        private Layer SelectLayer(List<Layer> layers, Type type)
        {
