...
namespace BIMViewer.BIM
{
    public class BIMManager
    {
        ...
        private Dictionary<int, POIType> m_dicPOIWires = new Dictionary<int, POIType>();
        ...
        public Dictionary<int, POIType> POIWires
        {
            get { return m_dicPOIWires; }
        }
        //ym. Delete Project
        ...
        private void LoadPOIVisible(List<Shapes.Layer> layers, Level level, Project project)
        {
            ...
            string fileName = key + ".ini";

            if (!File.Exists(Application.StartupPath + "\\POITypeVisible\\" + fileName))
                return;

            using (StreamReader sr = new StreamReader(Application.StartupPath + "\\POITypeVisible\\" + fileName))
            {
                ...
                if (item is Shapes.POILayer)
                {
                    ...
                    foreach (Shapes.Shape shape in poiLayer.Shapes)
                    {
                        if (shape is Shapes.POI)
                        {
                            ...
                            poi.POIVisible = GetPOIVisible(level.XMLID, poi.PoiType.ID);
                        }
                    }
                }
            }
        ...
        // key : poiType code
        private Dictionary<string, Color> m_dicPOIColor = new Dictionary<string, Color>();
        public Dictionary<string, Color> DicPOIColor
        {
        ...
        public void LoadPOIColor()
        {
            ...
            using (StreamReader sr = new StreamReader(Application.StartupPath + "\\POITypeColor\\POITypeColor.ini"))
            {
                while (!sr.EndOfStream)
                {
                    ...
                    //int nPoiID = Convert.ToInt32(property[0]);
                    string poiCode = property[0];
                    Color color = GetColor(property[1]);

                    m_dicPOIColor[poiCode] = color;
                }
            }           
        }
        ...
        { 
            ...
            {
                foreach (KeyValuePair<string, Color> item in m_dicPOIColor)
                {
            ...
            foreach (Shapes.Layer item in layers)
            {
                if (item is Shapes.POILayer)
                {
                    ...
                    foreach (Shapes.Shape shape in poiLayer.Shapes)
                    {
                        ...
                        {
                            Shapes.POI poi = shape as Shapes.POI;
                            if (m_dicPOIColor.ContainsKey(poi.PoiType.Code))
                                poi.FillColor = m_dicPOIColor[poi.PoiType.Code];
                        }
                    }
                }
            }
        ...
        public bool GetPOIVisible(string xmlLevelID, int poiTypeID)
        {
            string key = "_" + xmlLevelID;
            if (!m_dicPOIVisible.ContainsKey(key))
                return true;
                ...
                if (reader != null)
                {
                    while (reader.Read())
                    {
                        ...
                        if (isGroup == false)
                        {
                            ...
                            if (m_dicPOIColor.ContainsKey(poiType.Code))
                                poiType.Color = m_dicPOIColor[poiType.Code];
        ...
        public void LoadPOIWires() // Wire
        {
            ...
            foreach (KeyValuePair<int, POIType> item in m_dicPOITypes)
            {
                if (item.Value.Properties.Count > 0)
                {
                    ...
                    if (!m_dicPOIWires.ContainsKey(poiType.ID))
                        m_dicPOIWires[poiType.ID] = poiType;

                    // 해당 부모 POI 싹다 저장
                    AddPOIWireParent(poiType.Parent);
                }
            }
        }


        public void AddPOIWireParent(POIType parent)
        {
            if (m_dicPOIWires.ContainsKey(parent.ID))
                return;

            m_dicPOIWires[parent.ID] = parent;

            if (parent.Parent != null)
                AddPOIWireParent(parent.Parent);
        }
    }
}
