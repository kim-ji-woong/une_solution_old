...
namespace BIMViewer
{
    public partial class FormLocation : Form
    {
        ...
        public FormLocation(string strLoginID, string strLoginPW, Dictionary<string, string> poiUserList)
        {
            InitializeComponent();
        ...
        private void GetBuildingNumber(out string sMainNumber, out string sSubNumber)
        {
            ...
            //공백삭제후.
            GetBlankRemovedName(txtMainNumber.Text, out sNumber);
        ...
        private void RbtnOK_Click(object sender, EventArgs e)
        {
            ...
            //if(grdAdress.SelectedRows[0].Cells[3].Value.ToString() == "None")
            //{
            //    MessageBox.Show("공간데이터가 없습니다. 공간데이터를 먼저 업로드해주세요.");
            //    return;
            //}
        ...
        private bool GetLastPOIDate(string strResult, out string strDate, out string strUser)
        {
            int i;
            ...
            while (i > 0)
            {
                ...
                if (m_poiUserList.ContainsKey(tmpSUser))
                {
                    if (!flag)
                    {
        ...
        private void GrdAdress_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            ...
            string strResult = m_webServiceMgr.GetPOIInfo(m_sSelBuildMngNo, m_sID, m_sPW);
            if (strResult.Length == 0 || strResult.IndexOf("RS101") < 0)
                return;

            string strDateAndUser = "";
            string sDate, sUser;
            if (GetLastPOIDate(strResult, out sDate, out sUser))
                strDateAndUser = sDate + " / " + sUser;
            else
                strDateAndUser += "None";
