    ...
    public class XMLManager
    {
        ...
        private bool WriteIndoorModelFile(Project project, Dictionary<int, POIType> dicPOITypes, XmlTextWriter writer)
        {
            try
            {
                ...
                writer.WriteString(TARGET_VERSION.ToString());
        ...
        private bool WritePOIWire(Shapes.Wire wire, Level level, Dictionary<int, POIType> dicPOITypes, XmlTextWriter writer)
        {
            ...
            try
            {
                ...
                writer.WriteStartAttribute("type");
               //  writer.WriteString(poiType.XMLID);//ym. 서버에서 down받은 xml(id==code)과 맞추기위해 편집작업시 code 로 쓰기
                writer.WriteString(poiType.Code);
                writer.WriteEndAttribute();
        ...
        private bool WritePOI(Shapes.POI poi, XmlTextWriter writer)
        {
            try
            {
                ...
                writer.WriteStartAttribute("type");
                //writer.WriteString(poi.PoiType.XMLID);//ym. 서버에서 down받은 xml(id==code)과 맞추기위해 편집작업시 code 로 쓰기
        ...
        private Project ReadIndoorModelFile(XmlTextReader reader, Dictionary<int, POIType> dicPOITypes)
        {
            ...
            try
            {
                ...
                {   // XML 문서 버전이 1.5 이상만 읽기가 가능
                    m_strErrorMessage = "문서의 버전이 현재버전과 다릅니다.\r\n문서버전 : " + strVersion + ", 타겟버전 : " + TARGET_VERSION + ", " + MINIMUM_VERSION + " 이상 읽을 수 있습니다.";
                    return project;
                }
        ...
        private bool ReadPOI(XmlTextReader reader, Level level, Dictionary<int, POIType> dicPOITypes)
        {
            try
            {
                ...
                if (FormMain.Instance.BimManager.DicPOIColor.ContainsKey(poi.PoiType.Code))
                    poi.FillColor = FormMain.Instance.BimManager.DicPOIColor[poi.PoiType.Code];

                poi.POIVisible = FormMain.Instance.BimManager.GetPOIVisible(level.XMLID, poi.PoiType.ID);

                level.AddPOI(poi);

                bool readName = false, readAngle = false;
                Vertex2D vPos = null;
                bool bPropertyCheck = false;
                ...
                while (reader.Read())
                {
                    switch (reader.NodeType)
                    {
                        ...
                        case XmlNodeType.EndElement:
                            if (bPropertyCheck == false)
                            {
                                List<Property> properties = poi.Properties;
                ...
                if (strTypeID == "F9999")
                {
                    ...
                    // 속성 이름에 맞게 타입 확인 및 생성
                    foreach (Property pro in poi.Properties)
                    {
                        if (pro.Name == "사용자 POI 이름")
                        {
                            strPOIName = pro.Value;
                        }
                    }
                    ...
                    if (strPOIName != "")
                    {
                        ...
                        foreach (KeyValuePair<int, POIType> item in dicPOITypes)
                        {
                            if (item.Value.Code == strTypeID && item.Value.Name == strPOIName)
                            {
                                poiType = item.Value;
                                poi.PoiType = poiType;
                                bChk = true;
                        ...
                        if (bChk == false)
                        {
                            ...
                            // 일치하는 타입이 없으므로 새로 생성
                            POIType newPOIType = new POIType();
                            newPOIType.ID = nID;
                            newPOIType.XMLID = strXMLID;
                            newPOIType.Name = strPOIName;
                            newPOIType.ParentID = poi.PoiType.Parent.ID;
                            newPOIType.Parent = FormMain.Instance.BimManager.POITypes[poi.PoiType.Parent.ID];
                            newPOIType.UserDefined = true;
                            newPOIType.Code = "F9999";

                            poi.PoiType = newPOIType;
                            FormMain.Instance.BimManager.POITypes.Add(nID, newPOIType);
                        }
                    }
        ...
        private bool ReadDoor(XmlTextReader reader, Level level)
        {
            try
            {
                ...
                if (wall == null)
                {
                    ...
                    // .TODO: LinkedWall 관련 예외처리
                    Console.WriteLine("ReadDoor 예외처리 (Level: " + level.Name + ", Door ID: " + strID + ", " + m_strErrorMessage + " )");
                }
        ...
        private bool ReadWindow(XmlTextReader reader, Level level)
        {
            try
            {
                ...
                if (wall == null)
                {
                    ...
                    // .TODO: LinkedWall 관련 예외처리
                    Console.WriteLine("ReadWindow 예외처리 (Window ID: " + strID + ", " + m_strErrorMessage + " )");
                    return true;
                }
                        ...
                        case XmlNodeType.Element:
                            ...
                            else if (string.Compare(reader.Name, "LinkedWall", true) == 0)
                            {
                                ...
                                if (wall == null)
                                {
                                    ...
                                    // .TODO: LinkedWall 관련 예외처리
                                    Console.WriteLine("Space LinkedWall 예외처리 (Space ID: " + space.XMLID + ", " + m_strErrorMessage + " )");
                                    continue;
                                }
                    ...
                    switch (reader.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (string.Compare(reader.Name, "AlertAreaProperties", true) == 0)
                            {
                                ...
                                bool bChkRx = false;
                                bool bChkLoop = false;
                                bool bChkAddress = false;
                                bool bChkChannel = false;
        ...
        private bool ReadPOIType(XmlTextReader reader, POIType parent, bool isGroup, Dictionary<int, POIType> dicPOITypes, bool isEmptyElement)
        {
            try
            {
                ...
                if (!bExist)
                {
                    ...
                    if (FormMain.Instance.BimManager.DicPOIColor.ContainsKey(poiType.Code))
                        poiType.Color = FormMain.Instance.BimManager.DicPOIColor[poiType.Code];
                ...
                // Line POIType
                if (poiType != null && m_strPOIWireTable.ContainsKey(strPOICode))
                {
                    bool bChk = false;
