...
namespace BIMViewer
{
    ...
    public partial class FormProperty : PopupFormBase
    {
        ...
        public FormProperty(IUIMaster master)
        {
            ...
            tbSpaceID.BackColor = Color.FromArgb(217, 217, 217);
            m_master = master;
        }
        ...
        private void SetShape()
        {
            ...
            {
                pnSpace.Visible = pnPOI.Visible = pnWall.Visible = pnUser.Visible = pnDoor.Visible = pnWindow.Visible = pnAlertArea.Visible = btnModifity.Visible = pnColumn.Visible = false;
                ...
                else if (m_shape is AlertArea)
                {
                    pn = pnAlertArea;
                    pnAlertArea.Visible = true;
                }
                else if (m_shape is POI)
                {
                    ...
                    if (poi.PoiType.UserDefined) // 사용자 POI
                    {
                        ...
                        btnModifity.Click += btnModifity_Click;
                    }
                    else
                    {
                        pn = pnPOI;
                        ...
                        btnModifity.Visible = true;
                        btnModifity.Click += btnPoiModifity_Click;
                    }
                }
                ...
                else if (m_shape is Column)
                {
                    pn = pnColumn;
                    pnColumn.Visible = true;
                }
                ...
                if (m_shape is POI)
                {
                    this.Size = new Size(382, pn.Location.Y + pn.Height + 40);
                    btnModifity.Location = new Point(pn.Location.X + pn.Size.Width - btnModifity.Size.Width, pn.Location.Y + pn.Size.Height + 10);
                }
                    
                else
                    this.Size = new Size(382, pn.Location.Y + pn.Height);
        ...
        private void SetPropertyValue()
        {
            ...
            if (m_shape is Wall)
            {
                ...
                {
                    if (prop.Name == "재질") sTypeName = prop.Value;
                    break;
                }
                strProperty = string.Format("타입 : {0}\r\n재질 : {1}", wall.Component.TypeName, sTypeName);
            }
            else if (m_shape is Door)
            {
            ...
            {
                labelShapeType.Text = this.strTitle = "기둥";

                Column column = (Column)m_shape;
                lblColumn.Text = "기둥";

                if (column.Type == Column.ColumnType.Rect)
                    lblColumnType.Text = "사각기둥";
                else // if (column.Type == Column.ColumnType.Circle)
                    lblColumnType.Text = "원기둥";

                lblColumnID.Text = column.XMLID;

                strProperty = string.Format("타입 : {0}", column.Type.ToString());
            }
            else if (m_shape is Window)
            {
            ...
            else if (m_shape is POI)
            {
                ...
                tbUserPoiRx.Text = "";
                tbUserPoiLoop.Text = "";
                tbUserPoiAddress.Text = "";
                tbUserPoiChannel.Text = "";
                ...
                if (!poi.PoiType.UserDefined)
                {
                    ...
                    {
                        if (property.Name == "rx")
                            tbPoiRx.Text = property.Value;

                        if (property.Name == "loop")
                            tbPoiLoop.Text = property.Value;

                        if (property.Name == "address")
                            tbPoiAddress.Text = property.Value;

                        if (property.Name == "channel")
                            tbPoiChannel.Text = property.Value;
                ...
                else
                {
                    ...
                    lblUserPoiID.Text = poi.XMLID.ToString();
                    lblUserPoiHeight.Text = poi.Height.ToString();

                    foreach (Property property in poi.Properties)
                    {
                        if (property.Name == "rx")
                            tbUserPoiRx.Text = property.Value;

                        if (property.Name == "loop")
                            tbUserPoiLoop.Text = property.Value;

                        if (property.Name == "address")
                            tbUserPoiAddress.Text = property.Value;

                        if (property.Name == "channel")
                            tbUserPoiChannel.Text = property.Value;
            ...
            else if (m_shape is Space)
            {
                ...
                //lblSpaceName.Text = space.Name.Substring(0, space.Name.IndexOf("_"));
                if (space.Name.IndexOf("_") == -1)
                    lblSpaceName.Text = space.Name;
                else
                    lblSpaceName.Text = space.Name.Substring(0, space.Name.IndexOf("_"));


                tbSpaceID.Text = space.XMLID;
            ...
            {
                AlertArea alertArea = (AlertArea)m_shape;
                //lblAlertAreaName.Text = alertArea.Name.Substring(0, alertArea.Name.IndexOf("_"));
                lblAlertAreaName.Text = alertArea.Name;
                lblAlertAreaID.Text = alertArea.XMLID;
                lblAlertAreaType.Text = "경계구역";

                foreach(Property property in alertArea.Properties)
                {
                    if (property.Name == "grouping")
                    {
                        lblAlertAreaGroup.Text = property.Value;
                    }
                }
        ...
        private void btnModifity_Click(object sender, EventArgs e)
        {
            ...
            if (m_shape != null && m_shape is POI)
            {
                POI poi = (POI)m_shape;
                ...
                {
                    if (property.Name == "rx")
                        property.Value = tbUserPoiRx.Text;

                    if (property.Name == "loop")
                        property.Value = tbUserPoiLoop.Text;

                    if (property.Name == "address")
                        property.Value = tbUserPoiAddress.Text;

                    if (property.Name == "channel")
                        property.Value = tbUserPoiChannel.Text;
                }
            }
        }
        private void btnPoiModifity_Click(object sender, EventArgs e)
        {
            if (m_systemInput || m_master == null)
                return;
            ...
            if (m_shape != null && m_shape is POI)
            {
                ...
                {
                    if (property.Name == "rx")
                        property.Value = tbPoiRx.Text;

                    if (property.Name == "loop")
                        property.Value = tbPoiLoop.Text;

                    if (property.Name == "address")
                        property.Value = tbPoiAddress.Text;

                    if (property.Name == "channel")
                        property.Value = tbPoiChannel.Text;
                }
            }
        }
    }
