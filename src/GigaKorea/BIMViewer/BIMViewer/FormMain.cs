...
namespace BIMViewer
{
    public partial class FormMain : Form, IProjectOwner, ILevelOwner, IUIMaster
    {
        ...
        private Dictionary<string, string> m_poiUserList = null;
        private uSave m_uSave = null;//save as.ym
        private bool m_bLogin = false;//logout. login = true;
        private string m_strLoginID = "";
        private string m_strLoginPW = "";

        // XML 열 때, XML에 표시된 사용자 정의 POI 목록
        List<POIType> m_userPOITypeGroup = new List<POIType>();

        // 현재 열린 층 정보
        BIM.Level m_level = null;
        ...
        public Shapes.POI SelectedPOIInfo
        {
            get
            {
                Shapes.POI poi = new Shapes.POI();

                if (txtPOIHeight.Text != "")
                    poi.Height = Convert.ToInt32(txtPOIHeight.Text);

                Property property = new Property();
                property.Name = "rx";
                property.Value = txtPOIRx.Text;
                poi.Properties.Add(property);
               
                property = new Property();
                property.Name = "loop";
                property.Value = txtPOILoop.Text;
                poi.Properties.Add(property);
               
                property = new Property();
                property.Name = "address";
                property.Value = txtPOIAddress.Text;
                poi.Properties.Add(property);
            
                property = new Property();
                property.Name = "channel";
                property.Value = txtPOIChannel.Text;
                poi.Properties.Add(property);
        ...
        public FormMain()
        {
            ...
            // POI 관리자1명, 사용자 2명
            m_poiUserList = new Dictionary<string, string>();
            m_poiUserList.Add("user_spatial", "spatial1234");
            m_poiUserList.Add("ACC_0001_20190822153807522", "poiInfo100");
            m_poiUserList.Add("ACC_0001_20190822153909701", "poiInfo200");
            ...
            cbPOIList.SelectedIndexChanged += cbPOIList_SelectedIndexNormalChanged;
        }
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            //cbLineList.ValueMember = "Name";
            cbLineList.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbLineList.AutoCompleteSource = AutoCompleteSource.ListItems;


            // POI Category ComboBox
            cbPOIMain.DrawItem += CbPOIMain_DrawItem;
            cbPOIMain.ValueMember = "Name";
            cbPOIMain.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbPOIMain.AutoCompleteSource = AutoCompleteSource.ListItems;

            cbPOIMiddle.DrawItem += CbPOIMiddle_DrawItem;
            cbPOIMiddle.ValueMember = "Name";
            cbPOIMiddle.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbPOIMiddle.AutoCompleteSource = AutoCompleteSource.ListItems;

            cbPOISub.DrawItem += CbPOISub_DrawItem;
            cbPOISub.ValueMember = "Name";
            cbPOISub.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbPOISub.AutoCompleteSource = AutoCompleteSource.ListItems;
            LoadPOIComboMain();

            // POI Wire Category ComboBox
            cbWireMain.DrawItem += CbWireMain_DrawItem;
            cbWireMain.ValueMember = "Name";
            cbWireMain.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbWireMain.AutoCompleteSource = AutoCompleteSource.ListItems;

            cbWireMiddle.DrawItem += CbWireMiddle_DrawItem;
            cbWireMiddle.ValueMember = "Name";
            cbWireMiddle.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbWireMiddle.AutoCompleteSource = AutoCompleteSource.ListItems;

            cbWireSub.DrawItem += CbWireSub_DrawItem;
            cbWireSub.ValueMember = "Name";
            cbWireSub.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            cbWireSub.AutoCompleteSource = AutoCompleteSource.ListItems;
            LoadWireComboMain();
            ...
            if (!IsTabMode)
            {
                if (splitContainerMain.Panel2.Controls.Count > 1)
                {
                    ...
                    if (m_frmPOILayer != null && !m_frmPOILayer.IsDisposed)
                    {
                        ...
                        m_frmPOILayer.SetLayers(level.GetLayers(), project.Name, level.Name, level.XMLID, level.POIs);
                    }
        ...
        private void rbtnOpen_Click(object sender, EventArgs e)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (project != null)
                {
                    ...
                    m_bimManager.LoadPOIWires();        // POIWire 리스트 생성
                    LoadWireComboMain();
                }
            }
#endif
        ...
        {
            //  TabPage page = new TabPage(frm.Level.Name + "   X");
        ...
        private void LoadPOIComboMain()
        {   // POI 대,중,소 분류 UI
            
            cbPOIMain.Items.Clear();
            ...
            foreach (KeyValuePair<int, POIType> item in m_bimManager.POITypes)
            {
                ...
                {
                    cbPOIMain.Items.Add(poiType);
                }
            }

            if (cbPOIMain.Items.Count > 0)
                cbPOIMain.SelectedIndex = 0;

        }

        private void LoadWireComboMain()
        {   // Wire 대,중,소 분류 UI

            cbWireMain.Items.Clear();

            foreach (KeyValuePair<int, POIType> item in m_bimManager.POIWires)
            {
                POIType poiType = item.Value;

                if (poiType.Parent == null)
                {
                    cbWireMain.Items.Add(poiType);
                }
            }

            if (cbWireMain.Items.Count > 0)
                cbWireMain.SelectedIndex = 0;
        ...
        public void SetPOIComboList(POIType chgPoiType, bool isNew, bool isDelete = false)
        {
            ...
            cbPOIList.Items.Clear();
            //LoadPOIComboList();
            cbSearch_CheckedChanged(null, null);
        ...
        private void CbLineList_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (e.Index >= 0)
            {
                ...
                //_cbData = cbLineList.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;
        ...
        private void CbPOIMain_DrawItem(object sender, DrawItemEventArgs e)
        {
            POIType _cbData = null;
            ...
            if (e.Index >= 0)
            {
                _cbData = cbPOIMain.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;

                e.Graphics.DrawString(_cbData.Name, e.Font, new SolidBrush(cbPOIList.ForeColor), e.Bounds.Left + 2, e.Bounds.Top);
            }
        }

        private void CbPOIMiddle_DrawItem(object sender, DrawItemEventArgs e)
        {
            POIType _cbData = null;
            ...
            if (e.Index >= 0)
            {
                _cbData = cbPOIMiddle.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;
        ...
        private void CbPOISub_DrawItem(object sender, DrawItemEventArgs e)
        {
            POIType _cbData = null;
            ...
            if (e.Index >= 0)
            {
                _cbData = cbPOISub.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;
        ...
        private void CbWireMain_DrawItem(object sender, DrawItemEventArgs e)
        {
            POIType _cbData = null;
            ...
            if (e.Index >= 0)
            {
                _cbData = cbWireMain.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;

                e.Graphics.DrawString(_cbData.Name, e.Font, new SolidBrush(cbLineList.ForeColor), e.Bounds.Left + 2, e.Bounds.Top);
            }
        }

        private void CbWireMiddle_DrawItem(object sender, DrawItemEventArgs e)
        {
            POIType _cbData = null;
            ...
            if (e.Index >= 0)
            {
                _cbData = cbWireMiddle.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;
        ...
        private void CbWireSub_DrawItem(object sender, DrawItemEventArgs e)
        {
            POIType _cbData = null;
            ...
            if (e.Index >= 0)
            {
                _cbData = cbWireSub.Items[e.Index] as POIType;
                if (_cbData == null)
                    return;
        ...
        public bool UpdatePOIToXML(Shapes.POI poi, BIM.Project project, BIM.Level level, bool isDelete)
        {
            ...
            // POI를 추가하거나 삭제하였을때 POI Layer에 적용
            if (m_frmPOILayer != null && !m_frmPOILayer.IsDisposed)
                m_frmPOILayer.SetLayers(level.GetLayers(), project.Name, level.Name, level.XMLID, level.POIs);
        ...
        private void ShowPOILayer(FormView frm, List<Shapes.Layer> layers)
        {
            ...
            m_frmPOILayer.SetLayers(layers, projectName, frm.Level.Name, frm.Level.XMLID, frm.Level.POIs);
            //m_frmPOILayer.Layers = layers;
            m_frmPOILayer.FormClosed += (layerS, layerE) =>
            {
                rbtnLayer.IsChecked = false;
        ...
        private void tabControl1_DrawItem(object sender, DrawItemEventArgs e)
        {
            try
            {
                ...
                // e.Graphics.DrawString(data.Name, page.Font, brush2, rect.Left + 5, rect.Top + 5);
                e.Graphics.DrawString(page.Text, page.Font, brush2, rect.Left + 5, rect.Top + 5);
                brush2.Dispose();
                ...
                data.SetTabPageRect(rect.Left, rect.Top, rect.Width, rect.Height);
        ...
        private void SetPOIHeight()
        {
            BIM.Level level = m_level;

            if (level != null)
            {
                List<Property> properties = level.Properties;
                bool bChk = false;
                ...
                foreach (Property property in properties)
                {
                    if (property.Name == "Height")
                    {
                        ...
                        if (cbPOIList != null)
                        {
                            poiType = cbPOIList.SelectedItem as POIType;

                            if (poiType != null)
                                strPOICode = poiType.Code;
                        }

                        if (POIData.IsFloorPOI(strPOICode))
                            txtPOIHeight.Text = "0";
                        else 
                            txtPOIHeight.Text = property.Value;
        ...
        private void rbtnUpload_Click(object sender, EventArgs e)
        {
            ...
            FormLocation fLocation = new FormLocation(m_strLoginID, m_strLoginPW, m_poiUserList);
            ...
            if (fLocation.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                {
                    m_webServiceMgr.Upload(m_currentProject, m_bimManager.POITypes, fLocation.m_sSelBuildMngNo, m_strLoginID, m_strLoginPW, m_poiUserList);
                }
            }
        }
        ...
        private void rbtnUpload_Click(object sender, EventArgs e)
        {
            ...
            if (fLocation.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                if (fLocation.m_sSelBuildMngNo != "")
                {
                    ...
                    if (mgr.Upload(m_currentProject, m_bimManager.POITypes, fLocation.m_sSelBuildMngNo, m_strLoginID, m_strLoginPW, m_poiUserList))
        ...
        */
        ...
        {
            FormLogin fLogin = new FormLogin(m_poiUserList);
        ...
        private void cbPOIMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbPOIMain.SelectedItem == null || cbPOIMain.Items.Count == 0)
                return;

            cbPOIMiddle.Items.Clear();

            POIType selectPOIType = (POIType)cbPOIMain.SelectedItem;
            int nID = selectPOIType.ID;

            foreach (KeyValuePair<int, POIType> item in m_bimManager.POITypes)
            {
                POIType poiType = item.Value;

                if (poiType.Parent != null && poiType.Parent.ID == nID)
                {
                    cbPOIMiddle.Items.Add(poiType);
                }
            }

            if (cbPOIMiddle.Items.Count > 0)
                cbPOIMiddle.SelectedIndex = 0;
        }

        private void cbPOIMiddle_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbPOIMiddle.SelectedItem == null || cbPOIMiddle.Items.Count == 0)
                return;

            cbPOISub.Items.Clear();
            cbPOISub.Text = "";
            POIType selectPOIType = (POIType)cbPOIMiddle.SelectedItem;
            int nID = selectPOIType.ID;

            if (selectPOIType.IsGroup == false)
            {
                cbPOISub.Enabled = false;

                cbPOIList.Items.Clear();
                cbPOIList.Items.Add(selectPOIType);
            ...
            else
            {
                ...
                foreach (KeyValuePair<int, POIType> item in m_bimManager.POITypes)
                {
                    ...
                    if (poiType.Parent != null && poiType.Parent.ID == nID)
                    {
                        cbPOISub.Items.Add(poiType);
                    }
                }
            }

            if (cbPOISub.Items.Count > 0)
                cbPOISub.SelectedIndex = 0;
        }

        private void cbPOISub_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbPOISub.SelectedItem == null || cbPOISub.Items.Count == 0)
                return;

            cbPOIList.Items.Clear();
            cbPOIList.Text = "";
            POIType selectPOIType = (POIType)cbPOISub.SelectedItem;
            int nID = selectPOIType.ID;
        ...
        private void cbWireMain_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbWireMain.SelectedItem == null || cbWireMain.Items.Count == 0)
                return;

            cbWireMiddle.Items.Clear();

            POIType selectPOIWire = (POIType)cbWireMain.SelectedItem;
            int nID = selectPOIWire.ID;

            foreach (KeyValuePair<int, POIType> item in m_bimManager.POIWires)
            {
                POIType poiWire = item.Value;

                if (poiWire.Parent != null && poiWire.Parent.ID == nID)
                {
                    cbWireMiddle.Items.Add(poiWire);
                }
            }

            if (cbWireMiddle.Items.Count > 0)
                cbWireMiddle.SelectedIndex = 0;
        }

        private void cbWireMiddle_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbWireMiddle.SelectedItem == null || cbWireMiddle.Items.Count == 0)
                return;

            cbWireSub.Items.Clear();
            cbWireSub.Text = "";
            POIType selectPOIWire = (POIType)cbWireMiddle.SelectedItem;
            int nID = selectPOIWire.ID;

            if (selectPOIWire.IsGroup == false)
            {
                cbWireSub.Enabled = false;

                // Wire 형이 다를기에 수정부분
                cbLineList.Items.Clear();
                //cbLineList.Items.Add(selectPOIWire);

                POITypeProperty property = new POITypeProperty();
                property.POITypeID = selectPOIWire.ID;
                property.PropertyName = selectPOIWire.Name;// item.Value.Properties[0].Name;
                property.ProperetyValue = selectPOIWire.Properties[0].Value;
                property.Description = selectPOIWire.Properties[0].Description;
            ...
            else
            {
                ...
                foreach (KeyValuePair<int, POIType> item in m_bimManager.POIWires)
                {
                    ...
                    if (poiType.Parent != null && poiType.Parent.ID == nID)
                    {
                        cbWireSub.Items.Add(poiType);
                    }
                }
            }

            if (cbWireSub.Items.Count > 0)
                cbWireSub.SelectedIndex = 0;

        }

        private void cbWireSub_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (cbWireSub.SelectedItem == null || cbWireSub.Items.Count == 0)
                return;

            cbLineList.Items.Clear();
            cbLineList.Text = "";
            POIType selectPOIWire = (POIType)cbWireSub.SelectedItem;
            int nID = selectPOIWire.ID;
            ...
            if (selectPOIWire.IsGroup == false)
            {
                // Wire 형이 다를기에 수정부분
                cbLineList.Items.Clear();
                //cbLineList.Items.Add(selectPOIType);
            ...
            else
            {
                foreach (KeyValuePair<int, POIType> item in m_bimManager.POIWires)
                {
                    ...
                    {
                        // Wire 형이 다를기에 수정부분
                        //cbLineList.Items.Add(poiType);

                        POITypeProperty property = new POITypeProperty();
                        property.POITypeID = poiType.ID;
                        property.PropertyName = poiType.Name;// item.Value.Properties[0].Name;
                        property.ProperetyValue = poiType.Properties[0].Value;
                        property.Description = poiType.Properties[0].Description;
        ...
        private void cbSearch_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                cbPOIMain.SelectedIndexChanged -= cbPOIMain_SelectedIndexChanged;
                cbPOIMiddle.SelectedIndexChanged -= cbPOIMiddle_SelectedIndexChanged;
                cbPOISub.SelectedIndexChanged -= cbPOISub_SelectedIndexChanged;

                cbPOIMain.Enabled = false;
                cbPOIMiddle.Enabled = false;
                cbPOISub.Enabled = false;
                ...
                POIType _cbData = new POIType();
                _cbData.Name = "";

                cbPOIMain.Items.Add(_cbData);
                cbPOIMiddle.Items.Add(_cbData);
                cbPOISub.Items.Add(_cbData);
                ...
                LoadPOIComboList();
                cbPOIList.SelectedIndexChanged -= cbPOIList_SelectedIndexNormalChanged;
                cbPOIList.SelectedIndexChanged += cbPOIList_SelectedIndexChanged;
            }
            else
            {
                cbPOIList.SelectedIndexChanged -= cbPOIList_SelectedIndexChanged;
                ...
                cbPOIMain.Enabled = true;
                cbPOIMiddle.Enabled = true;
                cbPOISub.Enabled = true;
        ...
        private void cbPOIList_SelectedIndexChanged(object sender, EventArgs e)
        {
            POIType _cbData = null;
            POIType _cbPOIMain = null;
            POIType _cbPOIMiddle = null;
            POIType _cbPOISub = null;

            _cbData = cbPOIList.SelectedItem as POIType;
            if (_cbData == null)
                return;

            _cbPOISub = _cbData.Parent;
            _cbPOIMain = _cbPOIMain;
            _cbPOIMiddle = _cbPOIMiddle;

            if (_cbPOISub.Parent != null)
            {
                _cbPOIMiddle = _cbPOISub.Parent;
                _cbPOIMain = _cbPOIMiddle;

                if (_cbPOIMiddle.Parent != null)
                {
                    _cbPOIMain = _cbPOIMiddle.Parent;
                }
            }
            ...
            cbPOIMain.Items.Add(_cbPOIMain);
            cbPOIMiddle.Items.Add(_cbPOIMiddle);
            cbPOISub.Items.Add(_cbPOISub);
        ...
        private void cbPOIList_SelectedIndexNormalChanged(object sender, EventArgs e)
        {
            SetPOIHeight();
        }
