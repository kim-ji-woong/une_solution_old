...
namespace SOPSimulator
{
    public class FormMain : SOPMonitoringSystem.FormSOP
    {
        ...
        private TrainingLinkManager m_linkMgr = null;
        public TrainingLinkManager LinkManager
        {
            get { return m_linkMgr; }
        }
            ...
            : base(nSOPGenUserID, strSOPGenUserRealName, isSimulationMode, onlySDMS, nTargetMonitor, bSituationRoomMode)
        {
            ...
            m_instance = this;
            m_linkMgr = new TrainingLinkManager();
        }
                ...
                if (control.Name == "panelBackImage")
                {
                    foreach (Control child in control.Controls)
                    {
                        ...
                        else if (child.Name == "panelScenarioTab")
                        {
                            ...
                            // 도상 훈련 UI 수정
                            child.BackColor = Color.FromArgb(223, 142, 10);

                            if (FormSOP.Instance.DBManager.SiteID == 300)
                            {
                                // 돌발 상황발생 버튼
                                RibbonButton btnOutbreak = new RibbonButton();

                                btnOutbreak.ClickedImage = global::SOPSimulator.Properties.Resources.Outbreak_Click;
                                btnOutbreak.MouseOverImage = global::SOPSimulator.Properties.Resources.Outbreak__Hover;
                                btnOutbreak.NormalImage = global::SOPSimulator.Properties.Resources.Outbreak_Normal;
                                btnOutbreak.CustomImageRect = new System.Drawing.Rectangle(0, 0, btnOutbreak.ClickedImage.Width, btnOutbreak.ClickedImage.Height);
                                btnOutbreak.InitButtonWidth = btnOutbreak.ClickedImage.Width;
                                btnOutbreak.Text = "";
                                btnOutbreak.Size = new Size(btnOutbreak.ClickedImage.Width, btnOutbreak.ClickedImage.Height);
                                btnOutbreak.Click += btnOutbreak_Click;

                                btnOutbreak.Anchor = AnchorStyles.Right;

                                nBtnLocation = child.Size.Width - btnOutbreak.Width - 375;
                                btnOutbreak.Location = new Point(nBtnLocation, 20);

                                child.Controls.Add(btnOutbreak);
                            }
                            

                            // 기관연계 프로그램 버튼
                            RibbonButton btnTraining = new RibbonButton();

                            btnTraining.ClickedImage = global::SOPSimulator.Properties.Resources.Training_Click;
                            btnTraining.MouseOverImage = global::SOPSimulator.Properties.Resources.Training_Hover;
                            btnTraining.NormalImage = global::SOPSimulator.Properties.Resources.Training_Normal;
                            btnTraining.CustomImageRect = new System.Drawing.Rectangle(0, 0, btnTraining.ClickedImage.Width, btnTraining.ClickedImage.Height);
                            btnTraining.InitButtonWidth = btnTraining.ClickedImage.Width;
                            btnTraining.Text = "";
                            btnTraining.Size = new Size(btnTraining.ClickedImage.Width, btnTraining.ClickedImage.Height);
                            btnTraining.Click += btnTraining_Click;

                            btnTraining.Anchor = AnchorStyles.Right;

                            if (nBtnLocation == -1)
                            {
                                nBtnLocation = child.Size.Width - btnTraining.Width - 375;
                            }
                            else
                            {
                                nBtnLocation = nBtnLocation + btnTraining.Width + 10;
                            }

                            btnTraining.Location = new Point(nBtnLocation, 20);

                            child.Controls.Add(btnTraining);
                        }
                    }
        ...
        {
            // FireSimulator 프로그램 띄우기 
            ExecuteManager2 exeMgr2 = new ExecuteManager2();
            exeMgr2.Run(ExecuteManager2.APP_TYPE.FIRE_SIMULATOR);
        }

        private void btnTraining_Click(object sender, EventArgs e)
        {
            // TrainingLink 프로그램 실행
            ExecuteManager2 exeMgr2 = new ExecuteManager2();
            exeMgr2.Run(ExecuteManager2.APP_TYPE.TRAINING_LINK);
        }
