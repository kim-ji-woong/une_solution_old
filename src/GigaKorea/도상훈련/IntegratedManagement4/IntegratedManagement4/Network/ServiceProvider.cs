...
namespace IntegratedManagement4
{
    public class ServiceProvider : TcpServiceProvider
    {
        ...
        public void SendDataToOther(byte[] bytes, ArrayList arrDatas, ClientData sender, bool nolock = false, ClientData.ClientType type = ClientData.ClientType.ALL)
        {
            ICollection<ConnectionState> arClient = null;
            SDMSServer.DdMonitor.Enter(LockObject, true);
            {
            ...
            foreach (ConnectionState state in arClient)
            {
                ...
                if (type == ClientData.ClientType.ALL || type == client.Type)
                {
                    if (sender != client)
                    {
                        try
                        {
                            if (Send(bytes, 0, bytes.Length, state, nolock, arrDropStates))
                                client.RecvLog(sender.Type, bytes, arrDatas);
                        }
                        catch (System.Exception ex)
                        {
                        }
        ...
        // 자기 자신을 제외한 다른 클라이언트에 전송
        public void SendDataToOther(ArrayList arrDatas, ClientData sender, bool nolock = false, ClientData.ClientType type = ClientData.ClientType.ALL)
        {
            ICollection<ConnectionState> arClient = null;
            SDMSServer.DdMonitor.Enter(LockObject, true);
            {
        ...
        public void SendShowSDMS()
        {
            SendShowProcess(InternalMessage.SOP_SIMULATOR_2_SDMS, InternalMessage.SopSimulatorToSdms.SHOW_WINDOW, ClientData.ClientType.SDMS_CLIENT);
        }

        public void SendShowSOPSimulator()
        {
            SendShowProcess(InternalMessage.SDMS_2_SOP_SIMULATOR, InternalMessage.SdmsToSopSimulator.SHOW_SOP_SIMULATOR, ClientData.ClientType.SOP_SIMULATOR);
        }

        private void SendShowProcess(byte messageType, short messageTag, ClientData.ClientType clientType)
        {
            ArrayList arrDatas = new ArrayList();
            arrDatas.Add(messageType);
            arrDatas.Add(messageTag);
            ...
            {
                ClientData client = (ClientData)state.Tag;
                if (client == null || client.Type != clientType)
                    continue;
