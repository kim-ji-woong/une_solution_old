...
namespace SOPManager
{
	public partial class FormMain : Form, ITextPictureBoxOwner, IRibbonButtonOwner, IMenuCommandOwner
	{
        ...
        public void UpdateWindowRate(Control ctl, double pWindowRateWidth, double pWindowRateHeight, String pFontFamily = "나눔스퀘어 Bold")
        {
            if (ctl == null)
                return;
		...
		public FormMain(int nSOPGenUserID, string strSOPGenUserID, string strSOPGenUserRealName)
		{
            ...
            UnE.Utility.UMessageBoxRibbon.Font = new System.Drawing.Font("나눔스퀘어", 12f, FontStyle.Bold);
		...
		public void OpenSOP(FormOpenDB frm)
		{
			...
			if (!mgr.Load(this, m_dbMgr, version, arrActionSteps, strCategoryName, strSubCategoryName, strDisasterName))
			{
                FormFrame.Instance.Text = FormFrame.Instance.Title;
				m_versionCurrent = null;
                UnE.Utility.UMessageBoxRibbon.Show("시나리오 불러오기가 실패하였습니다.", "불러오기 오류", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
				return;
			}
        ...
        // 팀정보가 바뀌었는지 확인한다.
        public void ReadTeamInfo(bool readTeam = true)
        {
            string strSQL = "Select PropertyValue from OptionSDMS where PropertyName = 'SDMSConfig' and SiteID = " + m_nSiteID.ToString();
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL, 0);
            ...
            if (strValue != "0" && strValue != "-2")
            {
                ...
                // strValue가 -1이면 SOPSimulator가 읽었다는 뜻
                string strMark = strValue == "-1" ? "0" : "-2";

                strSQL = "Update OptionSDMS set PropertyValue = '" + strMark + "' where PropertyName = 'SDMSConfig' and SiteID = " + m_nSiteID.ToString();
                m_dbMgr.GetResultData(strSQL, 0);
            }
        }
    }
