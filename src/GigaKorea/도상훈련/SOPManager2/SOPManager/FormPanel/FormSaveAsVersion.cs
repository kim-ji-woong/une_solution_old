...
{
    public partial class FormSaveAsVersion : Form, IRibbonButtonOwner
    {
        ...
        public FormSaveAsVersion(WebDBManager dbMgr, int nSOPGenUserID, string strDisasterCategory, string strSubDisasterCategory, string strDisasterDetail, bool isRegular, bool isNormal, VersionInfo versionCurrent)
        {
            ...
            m_dbMgr = dbMgr;
            m_strCategory = strDisasterCategory;
            m_strSubCategory = strSubDisasterCategory;
            m_strDetailCategory = strDisasterDetail;
            ...
            if (FormMain.Instance.LevelDisasterOption == LevelDisasterOption.Use)
            {
                ...
                m_usingLevelIDs = FormSaveVersion.SetDefaultLevelIDs(m_versionCurrent, m_dbMgr);
            }
        ...
        public void UpdateControlSize()
        {
            ...
            FormMain.Instance.UpdateWindowRate(btnUserType, WindowRateWidth, WindowRateHeight);
            FormMain.Instance.UpdateWindowRate(btnDelUserType, WindowRateWidth, WindowRateHeight);
        ...
        private IOManager mgr = new IOManager();

        public bool CheckOnlyNewVersion(string strDisaster)
        {
        ...
        private DataGridViewRow GetSubCategoryBtn(string szName)
        {
            ...
            {
                if (row.Cells[0].Value.ToString() == szName)
                    return row;
            }
            return null;
        ...
        private void InitRibbonButton()
        {
            ...
            m_btnCategoryList.Add(btnCategorySecurity);
        ...
        private void btnCategorySecurity_Click(object sender, EventArgs e)
        {
            OnCategorySelect(sender, e);
        }
        ...
        public void SetNewVersionOnly(bool bTrue)
        {
            m_bNewVersionOnly = bTrue;

            checkNewVersion.AutoCheck = !bTrue;
            checkNewVersion.Checked = bTrue;
            if (bTrue == true)
                checkNewVersion.CheckState = CheckState.Checked;
            else
                checkNewVersion.CheckState = CheckState.Unchecked;
        }
        ...
        private void ReadVersion()
        {
            ...
            string strSQL = string.Format(strFormat, SelectedCategory, m_nSiteID, SelectedSubCategory, SelectedDetailCategory, m_isRegular ? 1 : 0, m_isNormal ? 1 : 0);
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL);

            AddData(arrResult);

            bool bCheckNew = dataGridView.Rows.Count == 0;
            //bool bCheckNew = CheckOnlyNewVersion(SelectedDetailCategory);
            SetNewVersionOnly(bCheckNew);
