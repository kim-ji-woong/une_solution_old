...
namespace SOPMonitoringSystem
{
    public partial class DockingRightPersonnel : Form
    {
        ...
        public new bool Load(UnE.SOP.SOPManager sopMgr)
        {
            m_sopMgr = sopMgr;
        ...
        {
            FormSOP.Instance.GetPageHome().GetTeamList();
            ArrayList arrMember = new ArrayList();
            ArrayList arrTeam = FormSOP.Instance.GetPageHome().TeamList;
            m_arrMember.Clear();
            ...
            foreach (Sections.SOPTeam team in arrTeam)
            {
                if (team.LevelNo > 0)
                {
                    ArrayList arrSOPMember = FormSOP.Instance.GetLevelMember(team.LevelNo);
                ...
                {
                    ArrayList arrLinkedMembers = team.LinkedMembers;
                    if (arrLinkedMembers == null)
                        continue;
                    //ArrayList arrRegularTeam = team.RegularTeamIDList;
                    //if (arrRegularTeam == null) continue;
                    foreach (Data_CompanyMember data in m_arrCompanyMember)
                    {
                        foreach (TemporaryMember temporaryMember in arrLinkedMembers)
                        //foreach (int nRegularTeamID in arrRegularTeam)
                        {
                            if (temporaryMember._MemberType != TemporaryMember.MemberType.RegularTeam)
                                continue;

                            int nRegularTeamID = temporaryMember.MemberID;

                            if (data.ContainsTeam(nRegularTeamID))
                            //if (nRegularTeamID == data.RegularTeamID)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        if (member.ContainsTeam(nRegularTeamID))
