...
namespace SOPMonitoringSystem
{
		...
		, IWorkflowContainer
	{
        ...
        {
            string strSQL = "Delete from BroadcastHistory";

            if (m_dbMgr.GetResultData(strSQL, 0) == null)
                return;

            strSQL = "Delete from SensorReactionHistory";
            ...
            strSQL = "Delete from ActionStepAutoClose";

            if (m_dbMgr.GetResultData(strSQL, 0) == null)
                return;

            strSQL = "Update ActionStepHistory set SensorZoneHistoryID = NULL";
		...
		public void RunWorkflowWithoutEvent(int nZoneID, int nSensorID, int nSensorHistoryID)
		{
            try
            {
                ...
                else
                {
                    ...
                    if (szName.Contains("지진") || szName.Contains("미세먼지"))
                        szPositionName = "";
