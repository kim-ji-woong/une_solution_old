...
namespace SOPMonitoringSystem
{
		...
		, IWorkflowContainer
	{
		...
		public FormSOP(int nSOPGenUserID, string strSOPGenUserRealName, bool isSimulationMode, bool onlySDMS, int nTargetMonitor, bool bSituationRoomMode)
		{
            ...
            string strSQL = "Delete from SensorReactionHistory";
            ArrayList arrResult = m_dbMgr.GetResultData(strSQL, 0);

            strSQL = "Delete from SensorZoneHistory";
            arrResult = m_dbMgr.GetResultData(strSQL, 0);

            strSQL = "Delete from ActionStepHistory";
            arrResult = m_dbMgr.GetResultData(strSQL, 0);

            strSQL = "Delete from HistoryDisasterPos";
            arrResult = m_dbMgr.GetResultData(strSQL, 0);

            MakeHistoryDisasterPos();
        ...
        private void MakeHistoryDisasterPos()
        {
            string strSQLBase = "Insert into HistoryDisasterPos (ID, PosX, PosY, PosZ, FloorIndex, HistoryActionSetpID, DisasterType, DisasterTime, Description, BuildingID, SiteID, BroadcastName) values ";
            string[] spaces = new string[5] { "교장실", "직원휴게실", "교무실", "경비실", "진로상담실" };

            for (int i = 0; i < 5; i++)
            {
                string strSQL = strSQLBase + string.Format("({0}, 0, 0, 0, 0, {0}, '화재', NULL, '{1}', '가상 중학교', 1, '{1}')", i + 1, spaces[i]);
        ...
        {
            btnWork.Visible = btnControl.Visible = btnReturnControl.Visible = pictureBox2.Visible = false;
            labelSelectMode.Visible = labelReal.Visible = labelVirtual.Visible = pictureBox3.Visible = false;
            panelMode.Visible = pictureBox1.Visible = false;
            btnSDMS.Visible = btnSOP.Visible = btnBulletin.Visible = btnMissionStatus.Visible = btnDefaultCCTV.Visible = pictureBox5.Visible = false;

            int nDiff1 = pictureBox8.Location.X - btnOption.Location.X;
            int nDiff2 = btnStartSOP.Location.X - pictureBox1.Location.X;
            int nDiff3 = btnCancelSOP.Location.X - btnStartSOP.Location.X;
            int nDiff4 = btnFitToScale.Location.X - btnFitToCurrentComponent.Location.X;
            int nDiff5 = panelStatus.Location.X - pictureBox6.Location.X;
            int nDiff6 = pictureBox7.Location.X - panelStatus.Location.X;

            btnOption.Location = btnWork.Location;
            pictureBox8.Location = new Point(btnOption.Location.X + nDiff1, pictureBox8.Location.Y);
            btnStartSOP.Location = new Point(pictureBox8.Location.X + nDiff2, btnStartSOP.Location.Y);
            btnCancelSOP.Location = new Point(btnStartSOP.Location.X + nDiff3, btnCancelSOP.Location.Y);
            pictureBox4.Location = new Point(btnCancelSOP.Location.X + nDiff1, pictureBox4.Location.Y);
            btnFitToCurrentComponent.Location = new Point(pictureBox4.Location.X + nDiff2, btnFitToCurrentComponent.Location.Y);
            btnFitToScale.Location = new Point(btnFitToCurrentComponent.Location.X + nDiff4, btnFitToScale.Location.Y);
            pictureBox6.Location = new Point(btnFitToScale.Location.X + nDiff1, pictureBox6.Location.Y);
            panelStatus.Location = new Point(pictureBox6.Location.X + nDiff5, panelStatus.Location.Y);
            pictureBox7.Location = new Point(panelStatus.Location.X + nDiff6, pictureBox7.Location.Y);
            panelRealTimeInfo.Location = new Point(pictureBox7.Location.X + nDiff5, panelRealTimeInfo.Location.Y);
        }
		...
		public void RunWorkflowWithoutEvent(int nZoneID, int nSensorID, int nSensorHistoryID)
		{
            try
            {
                ...
                else
                {
                    ...
                    Zone zone = DataManager.Instance.GetZone(nZoneID);
                    string szPositionName = zone.BroadcastName;
        ...
        // SOP를 실행시키기 전에 팀정보가 바뀌었는지 확인한다.
        private void ReadTeamInfo(bool readTeam = true)
        {
            ...
            {
                if (readTeam)
                    DataManager.Instance.ReloadMembers();

                // strValue가 -2이면 SOP 생성기가 읽었다는 뜻
                string strMark = strValue == "-2" ? "0" : "-1";

                strSQL = "Update OptionSDMS set PropertyValue = '0' where PropertyName = 'SDMSConfig' and SiteID = " + m_nSiteID.ToString();
                m_dbMgr.GetResultData(strSQL, 0);
            }
        }
		...
		private void timer1_Tick(object sender, EventArgs e)
		{
            ...
            SOPMonitoringSystem.TTS.TTSManager.Instance.Run();
        ...
        public System.Diagnostics.Process ToggleSOPManager()
        {
            ExecuteManager mgr = new ExecuteManager();

            System.Diagnostics.Process process = mgr.RunCheckProcess("SOPManager");

            if (process == null)
            {
                string strParams = string.Format("{0} {0} {1} 2", m_nSOPGenUserID, m_strSOPGenUserRealName);
                return mgr.RunStartProcess("SOPManager.exe", strParams);
            }
            else
        ...
        public void ToggleCCTV()
        {
            ...
            System.Diagnostics.Process process = mgr.RunCheckProcess("TeamEditor");

            if (process == null)
            {
                string strParams = string.Format("{0} {1} {2}", m_nSOPGenUserID, m_nSiteID, m_strSOPGenUserRealName);
                mgr.RunStartProcess("TeamEditor.exe", strParams);
            }
            else
                process.CloseMainWindow();
