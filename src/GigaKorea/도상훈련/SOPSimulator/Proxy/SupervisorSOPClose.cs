    ...
    public class SupervisorSOPClose : IDisposable
    {
        ...
        public static void SupervisorSOPSensorClose(int nSensorZoneID, int nSensorZoneHistoryID)
        {
            if (m_instance != null)
            {
                ...
                string strSQL = "Select ActionStepID, RealMode from ActionStepHistory where SensorZoneHistoryID = " + nSensorZoneHistoryID;
                ...
                if (arrResult != null && arrResult.Count >= 2)
                {
                    ...
                    if (actionStepID != null && realMode != null)
                    {
                        ...
                        {
                            FormSOP.Instance.StopWorkflow(DateTime.Now, false, actionStepID.Data, realMode.Data == 1);
                        });
                        //FormSOP.Instance.CloseWorkflow(actionStepID.Data, realMode.Data == 1);
                    }
                }
            }
        }
        ...
        private void CloseThread(object param)
        {
            ...
            UnE.SOP.Workstate.SOPScenario sco = SOPScenarioManager.Instance.GetSOPScenario(nActionStepHistoryID);
            if (sco != null)
        ...
        private string GetEndStatusString()
        {
            ...
            types.Add(libSensorProcess.ReactionType.IGNORE_FIRE);
        ...
        public bool CheckSensorClose(int nSensorHistoryID)
        {
            ...
            string szTimeString = DBUtility.WebDBManager.MakeDateTimeString(dtTarget);
            ...
            ArrayList arData = m_dbMgr.GetResultData(szSQL, 0);
        ...
        private void SaveData(SOPCheckData data)
        {
            if(!ExistCheckDataInDB(data))
            {
                ...
                string szSQL = string.Format(szTemp, nMaxID, data.ActionStepHistoryID, "NULL",
                             ...
                             DBUtility.WebDBManager.MakeDateTimeString(data.CheckTime), //8
        ...
        {
            string szSQL = "SELECT ID, ActionStepHistoryID, ActionStepID, UseCloseNoInput, UseCloseSensorReset, UseCloseSensorResetWaitTime, " +
                            " InputWaitTime, SensorResetWaitTime, BeginTime, SensorZoneID, SensorZoneHistoryID, Description " +
                            " FROM ActionStepAutoClose";
