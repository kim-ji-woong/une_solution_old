    ...
    public partial class GdiPanel : Panel, IPainter, DXFViewer.IPainter
    {
                        ...
                        if (layer is WireLayer)
                        {
                            ...
                            foreach (Shape item in layer.Shapes)
                            {
                                if (item is Wire)
                                {
                                    ...
                                    if (beginPOI.PoiType.POIVisible && endPOI.PoiType.POIVisible)
                                        wire.Visible = true;
                                    else
                                        wire.Visible = false;
                                }
        ...
        private void GdiPanel_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else if (m_isDXFPanning)
            {
                ...
                if (m_owner != null)
                {
                    ...
                    m_owner.MoveDXF(vDXFMove - m_vDXFOrigin);//ym
                    needRefresh = true;
                }
            }
        ...
        {             
            ...
            if (poiType != null)
            {
                ...
                if (layer != null)
                {
                    ...
                    bool visible = FormMain.Instance.BimManager.GetPOIVisible(m_project.Name, m_level.ID, poiType.ID);
                                ...
                                if (item is POILayer)
                                {
                                    foreach (Shape shape in item.Shapes)
                                    {
                                        if (shape is POI)
                                        {
                                            ...
                                            poi2.PoiType.POIVisible = FormMain.Instance.BimManager.GetPOIVisible(m_project.Name, m_level.ID, poi2.PoiType.ID);
                                        }
                                    }
                                }
                            }
        ...
        private void AddWire(Point pt, POI selectedPoi)
        {
            ...
            if (property != null)
            {
                ...
                //if (layer != null && layer is WireLayer)
                //{
                    double x = 0.0, y = 0.0;
