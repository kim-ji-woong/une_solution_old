...
namespace PreSafe
{
    ...
    internal partial class FormMain : Form, IMenuCommandOwner, IRibbonButtonOwner, ITextCommanderOwner
    {
        ...
        private void FormMain_Load(object sender, EventArgs e)
        {
            ...
            XMLManager.ActionStep newActionStep = new XMLManager.ActionStep();
            newActionStep.StepName = "새 시나리오";
            ar.Add(newActionStep);
            m_formTreeStep.SetTreeView("PreSafe", "SOP", "성범죄", ar);
        }
        ...
        private bool SaveXML(string strFileName, out string szError)
        {
            ...
            using (XMLManager mgr = new XMLManager())
            {
                ...
                Sections.PanelSectionEx panel = FormMain.Instance.ContentForm.SectionPanel;

                if (!mgr.Save(panel, strFileName, strVersionName))
                {
        ...
        public void NewSenario()
        {
            if (contentForm.InitSectionPanel())
            {
                ...
                if (formNewScenario.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    ar.Add(newActionStep);
                    m_formTreeStep.SetTreeView("PreSafe", "SOP", panel.DisasterType, ar);
        ...
        public bool OpenSenarioFile()
        {
            if (contentForm.InitSectionPanel())
            {
                ...
                if (mOpenFileDialog.ShowDialog(FormFrame.Instance) == DialogResult.OK)
                {
                    ...
                    Sections.PanelSectionEx panel = FormMain.Instance.ContentForm.SectionPanel;
                    if (mgr.Load(panel, szOpenFilePath))
                    {
                        SenarioManager.Instance.SenarioFilePath = szOpenFilePath;
                        ClearSelection();
                        contentForm.ClearModify();
                        this.m_formTreeStep.SetTreeView(mgr.CategoryName, mgr.SubCategoryName, mgr.DisasterName, mgr.ActionSteps);
                        return true;
                    }
                    else
                    {
        ...
        public bool SaveAsSenarioFile()
        {
            using (XMLManager mgr = new XMLManager())
            {
                using (SOPChecker checker = new SOPChecker())
                {
                    if (!checker.CheckSOP(contentForm.SectionPanel))
                    {
                        return false;
                    }
                }
        ...
        public bool SaveSenarioFile()
        {
            using (XMLManager mgr = new XMLManager())
            {
                ...
                if (String.Compare(strSenarioName, "새 시나리오", true) == 0)
                {
