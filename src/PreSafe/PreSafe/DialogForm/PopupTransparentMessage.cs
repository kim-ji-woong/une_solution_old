...
namespace PreSafe
{
    public partial class PopupTransparentMessage : Form
    {
        ...
        private void MoveToCenterParent()
        {
            Form target = FormMain.Instance;
            Size size = this.Size;
            Point p = target.Location;
            p = target.PointToScreen(p);
            int x = p.X + (target.Size.Width / 2) - (size.Width / 2);
            int y = p.Y + (target.Size.Height / 2) - (size.Height / 2);
