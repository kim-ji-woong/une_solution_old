...
public class SelectionModel : MonoBehaviour
{
    ...
    private void LeaveObject()
    {
        ...
        if (proxy != null && this.gameObject != null)
        {
            ...
            if (mf != null)
            {
                string szObjName = mf.name == null ? "" : mf.name;
                string szMsg = string.Format("LeaveObject(\"{0}\")",szObjName);
    ...
    private void EnterObject()
    {
        ...
        if (proxy != null && this.gameObject != null)
        {
            ...
            {
                string szObjName = mf.name == null ? "" : mf.name;
                string szMsg = string.Format("EnterObject(\"{0}\")", szObjName);
                //Debug.logger.Log(szMsg + " : Shared Mesh :" + mf.mesh.name);
                if(PassivePipeProxy.Instance != null)
                    PassivePipeProxy.Instance.SendServer(szMsg);
            }
    ...
    void Update()
    {
        ...
        if (boxCollider != null)
        {
            ...
            if (ModelManager.Instance != null && distance < ModelManager.Instance.TextLODDistance)
            {
