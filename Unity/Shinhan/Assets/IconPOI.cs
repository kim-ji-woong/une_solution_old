...
public class IconPOI : MonoBehaviour {
    ...
    private static IconPOI m_selectedPOI = null;

    public static IconPOI SelectedPOI
    {
        get { return m_selectedPOI; }
        set { m_selectedPOI = value; }
    }

    private Vector3 m_vOrigin = new Vector3();

    public BoxCollider mColldier;
	void Start ()
    {
        Debug.Log("IconPOI is Started");
    ...
    public void Pick()
    {
        ...
        if (proxy != null && m_bVisible == true)
        {
            ...
            if (ModelManager.Instance.Model.EditMode)
            {
                ...
                string strPOIInfo = GetPOIType() + "_" + m_nID.ToString();
                POIManager.Instance.ChangePOIIcon(strPOIInfo, "CCTVSelected");
    ...
    {
        int nIndex = m_szType.IndexOf('_');

        if (nIndex < 0)
            return m_szType;

        string strType = m_szType.Substring(0, nIndex).Trim();
        return strType;
    }

    public void MoveLeft(Vector3 vHorzUnit, Vector3 vVertUnit)
    {
        Move(vHorzUnit, vVertUnit, -5, 0);
    }

    public void MoveRight(Vector3 vHorzUnit, Vector3 vVertUnit)
    {
        Move(vHorzUnit, vVertUnit, 0.1f, 0);
    }

    public void MoveUp(Vector3 vHorzUnit, Vector3 vVertUnit)
    {
        Move(vHorzUnit, vVertUnit, 0, 5);
    }

    public void MoveDown(Vector3 vHorzUnit, Vector3 vVertUnit)
    {
        Move(vHorzUnit, vVertUnit, 0, -5);
    }

    public void Move(Vector3 vHorzUnit, Vector3 vVertUnit, float x, float y)
    {
        Vector3 vMove = vHorzUnit * x + vVertUnit * y;
        Position = m_vOrigin + vMove;
        //m_vOrigin = Position;
    }
