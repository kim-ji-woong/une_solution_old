...
public class MainModel : MonoBehaviour
{
    ...
    public enum MouseWorkMode { NONE = 0, PICK, PANNING, ORBIT, ZOOM, NEW_FIRE_SENSOR, NEW_COOLER_SENSOR, NEW_PRESSURE_SENSOR, NEW_CCTV, DEL_FACILITY, POI_DRAG };
    ...
    // Key : SceneName
    private Dictionary<string, UnityScene> m_dicSceneIndoors = new Dictionary<string, UnityScene>();
    // Key : SceneName
    private Dictionary<string, UnityScene> m_dicSceneOutdoors = new Dictionary<string, UnityScene>();
    private UnityScene m_currentScene = null;

    private GameObject m_sceneTitle = null;
    private GameObject m_bkShort = null, m_bkLong = null;
    ...
    private static bool m_isDragPOIMode = false;
    ...
    private void AddPythonFunction()
    {
        ...
        if (proxy != null)
        {
            ...
            proxy.UserObject.SetVariable("SetSceneTitle", new Action<string>(SetSceneTitle));
            proxy.UserObject.SetVariable("SetSceneTitleColor", new Action<float, float, float, float>(SetSceneTitleColor));
            proxy.UserObject.SetVariable("MoveSceneTitle", new Action<float, float>(MoveSceneTitle));
        }
    }
    ...
    private UnityScene SetOutdoorScene(Dictionary<string, UnityScene> dicScenes, Dictionary<string, GameObject> dicCameraDatas, string strCameraName, string strModelName, Vector3 vOrbitCenter, bool isMainCamera, bool isActiveModel, float fPanningScale, UnityScene.SceneOption option = UnityScene.SceneOption.None)
    {
    ...
    private UnityScene SetIndoorScene(Dictionary<string, UnityScene> dicScenes, Dictionary<string, GameObject> dicCameraDatas, string strCameraName, string strModelName, string strAlarmModelName, Vector3 vOrbitCenter, bool isMainCamera, bool isActiveModel, float fPanningScale, UnityScene.SceneOption option = UnityScene.SceneOption.None)
    {
        UnityScene scene = SetOutdoorScene(dicScenes, dicCameraDatas, strCameraName, strModelName, vOrbitCenter, isMainCamera, isActiveModel, fPanningScale, option);
    ...
    private void ChangeScene(UnityScene scene)
    {
    ...
    private void Awake()
    {
        ...
        float fIndoorPanningScale = m_bEditMode ? 0.1f : 10.0f;

        InitText();

        UnityScene outdoor = SetOutdoorScene(m_dicSceneOutdoors, dicCameraDatas, "OutdoorCamera", "out_door_191112_final", new Vector3(0, 0, 0), false, false, 1.0f);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_1F", "01f", "01f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_2F", "02f", "02f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_3F", "03f", "03f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_4F", "04f", "04f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_5F", "05f", "05f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        UnityScene indoor = SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_6F", "06f", "06f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_7F", "07F", "07f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_B01F", "b01f", "b01f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_B02F", "b02f", "b02f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_B03F", "b03f", "b03f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_B04F", "b04f", "b04f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
        SetIndoorScene(m_dicSceneIndoors, dicCameraDatas, "Camera_B05F", "b05f", "b05f_zone", new Vector3(-824, 0, -1113), false, true, fIndoorPanningScale);
    ...
    {
        m_sceneTitle = GameObject.Find("SceneTitle");
        m_bkShort = GameObject.Find("BK_Short");
        m_bkLong = GameObject.Find("BK_Long");

        if (m_sceneTitle == null)
            Debug.Log("SceneTitle is null");
        else
        {
            UnityEngine.UI.Text text = m_sceneTitle.GetComponent<UnityEngine.UI.Text>();
        ...
        if (m_bkShort != null && m_bkLong != null)
        {
            m_bkShort.SetActive(false);
            m_bkLong.SetActive(false);
        }
    }
    ...
    void Update()
    {
        ...
        if (mode == MouseWorkMode.PANNING)
            Pan();
        else if (mode == MouseWorkMode.ORBIT)
            Orbit();
        else if (mode == MouseWorkMode.ZOOM)
            Zoom();
        else if (mode == MouseWorkMode.PICK)
            Pick();
        else if (mode == MouseWorkMode.POI_DRAG)
            DragPOI();
    }
    ...
    private void DragPOI()
    {
        if (m_plane != null && IconPOI.SelectedPOI != null)
        {
            Vector3 vMove = m_vMouseOrigin - Input.mousePosition;
            IconPOI.SelectedPOI.Move(m_vHorzUnit, m_vVertUnit, -vMove.x, -vMove.y);
        }
    }
    ...
    private void Orbit()
    {
        ...
        Vector3 vCurrent = Input.mousePosition;
    ...
    private void Pick()
    {
        if (IconPOI.SelectedPOI == null)
            Get3DPosition((int)Input.mousePosition.x, (int)Input.mousePosition.y);
        else
        {
            SetOrigin();
            IconPOI.SelectedPOI.MoveRight(m_vHorzUnit, m_vVertUnit);
        }
    }
    ...
    private MouseWorkMode GetMode()
    {
        ...
        else if (Input.GetMouseButtonDown(MOUSE_LEFT))
        {
            if (m_bPickMode)
                return PickMode();
            else if (m_isDragPOIMode)
                return POIDragMode(MouseState.LButtonDown);
            else if (m_bEditMode == false && m_bOrbitMode)
                return OrbitMode(MouseState.LButtonDown);
            else if (m_bTranslateMode)
                return PanningMode(MouseState.LButtonDown);
        }
        else if (Input.GetMouseButtonDown(MOUSE_RIGHT))
        {
        ...
        else if (m_prevMovingMode == MouseWorkMode.POI_DRAG)
        {
            if (m_prevState == MouseState.LButtonDown && Input.GetMouseButtonUp(MOUSE_LEFT))
            {
                InitDrag();
    ...
    {
        OnPOIMoved(CanvasPOI.SelectedPOI, true);
        //OnPOIMoved(IconPOI.SelectedPOI, true);
        m_vPanMovedFromFirstPosition += m_vCurrentPanMoved;
    }
    ...
    private MouseWorkMode POIDragMode(MouseState mouseState)
    {
        MainModel.WriteLog("Set POIDragMode");
        SetOrigin();

        m_prevState = mouseState;
        m_prevMovingMode = MouseWorkMode.POI_DRAG;
        return MouseWorkMode.POI_DRAG;
    }
    ...
    private void ManualZoom(float zDelta)
    {
        ...
        Zoom(zDelta);
        /*Debug.unityLogger.Log("0Zoom Deleta :" + zDelta);
        RaycastHit hit1;
        ...
        if (coll.Raycast(ray1, out hit1, Mathf.Infinity))
        {
            ...
            Debug.unityLogger.Log("1Zoom Deleta :" + zDelta);
            if (zDelta != 0)
            {

                Debug.unityLogger.Log("2Zoom Deleta :" + zDelta);
    ...
    public static void SetDragMode(bool drag)
    {
        m_isDragPOIMode = drag;
    }

    public static void OnPOIMoved(CanvasPOI poi, bool finishDrag)
    //public static void OnPOIMoved(IconPOI poi, bool finishDrag)
    {
        m_isDragPOIMode = !finishDrag;

        if (poi == null)
            return;

        CanvasPOI.SelectedPOI = null;

        string strMessage = string.Format("{0},{1},{2},{3}", poi.ID, poi.Position.x, poi.Position.y, poi.Position.z);
        string strMessageType = "POIMoved";

        if (PassivePipeProxy.Instance != null)
            PassivePipeProxy.Instance.SendServer(string.Format("OnMessage({0},{1})", strMessageType, strMessage));
    }
    ...
    public void SetSceneTitle(string strTitle)
    {
        ...
        if (m_bkShort != null)
        {
            ...
            else
            {
                ...
                {
                    m_bkShort.SetActive(true);
                    m_bkLong.SetActive(false);
                }
                else
                {
                    m_bkShort.SetActive(false);
                    m_bkLong.SetActive(true);
                }
            }
        }
    }

    public void SetSceneTitleColor(float r, float g, float b, float a)
    {
        if (m_sceneTitle == null)
            return;
    ...
    public void MoveSceneTitle(float x, float y)
    {
        ...
        text.rectTransform.position = new Vector3(text.rectTransform.position.x + x, text.rectTransform.position.y + y, text.rectTransform.position.z);
    }
}
